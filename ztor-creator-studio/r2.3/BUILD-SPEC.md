...
**區塊出現條件**（分頁＝頁面的 section-nav）：

| 分頁 | 區塊 | 出現條件 |
|---|---|---|
| 總覽 | 銷售摘要 | 一律 |
| 總覽 | 商品素材（主圖＋附圖） | 一律（數位是封面＋附圖） |
| 總覽 | 商品資訊（名稱／分類／描述） | 一律 |
| 總覽 | 詳細規格 Specifications | 僅實體 |
| 總覽 | 數位內容檔 | 僅數位；介面再依次分類切四種——單檔上傳（文檔）／專輯曲目管理／會員卡卡面自訂／IP 素材槽 |
| 總覽（右欄） | 上架設定・開賣設定（三開關與排程）＋隱藏時的非公開連結 | 僅總覽分頁顯示（其餘分頁以 `--full`／`--norail` 收起） |
| 銷售設定 | 當前庫存（逐選項組合表） | 僅實體；**單一選項與多選項共用同一張表**，單一選項＝只有一列、用商品名（D258） |
| 銷售設定 | 商品選項（選項與值的摘要＋編輯） | 僅多選項 |
| 銷售設定 | 折扣設定／低庫存提醒／每人限購 | 一律 |
| 交付與取貨 | 取貨與交付 | 僅實體；欄位再依物流／現場 QR 領取切換 |
| 商品推廣 | 商品標籤・電影關聯 | 一律 |
...
- **型別分流在元件裡完成**：`partials/upload-tile.js` 依選到的檔案決定塞 `<img class="upload-tile__thumb">` 還是 `<video class="upload-tile__video">`，並在格上掛 `.upload-tile--video`／`.upload-tile--playable`。頁面不必自己判斷檔型。
