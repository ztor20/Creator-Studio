...
**區塊出現條件**（2026-09-11 重組：分頁是「總覽」「商品庫存」「商品設定」三個；商品設定＝建立商品的整頁，
攤在同一頁、上方一列分節錨點（`.list-toolbar` ＋ `[data-nav]`，同活動詳情），預設唯讀、右上「編輯商品」切成可改）：

| 分頁 | 區塊／分節 | 出現條件 |
|---|---|---|
| （頁首） | 頂列（麵包屑＋以粉絲身分預覽；捲過頁首後換成主圖縮圖＋商品名、黏在上緣）＋ hero（主圖／狀態徽章／名稱／描述／次分類與來源連結） | 一律；與項目／系列／活動詳情同一組 `.detail-topbar`＋`.pd-hero`（2026-09-11） |
| 總覽 | 銷售摘要 | 一律 |
| 商品庫存 | 四塊 KPI 磚（在庫／單售可售／鎖給組合／補貨中）→ 補貨中（計時補貨的單＋到貨確認，沒有就不出現）→ 庫存管理（逐選項組合表：組合＋SKU／在庫／鎖給單售／鎖給組合／未鎖定／補貨中／狀態；卡頭批次鎖定／批次補貨，列上補貨，列尾 ⋯ 鎖定／單獨下架）→ 庫存去向（哪個組合包鎖了幾件）→ 庫存歷史紀錄（補貨＋鎖定，最近四筆＋查看全部） | 分頁僅實體（2026-09-11 A 案，探索頁 `lab-stock-tab.html`）；**單一選項與多選項共用同一張表**，單一選項＝只有一列、用商品名（D258）。表格唯讀，補貨與鎖定各自在彈窗裡存；價格／SKU／成本在商品設定 |
| 總覽 | 商品設定概覽（每個設定區塊一行一句摘要，行尾箭頭跳到商品設定的那一節） | 一律；列數隨主分類／選項模式增減（內容檔僅數位、低庫存提醒僅實體） |
| 總覽 | 使用中（被哪個專案／組合引用） | 一律（S24） |
...
| 分頁 | 區塊 | 出現條件 |
|---|---|---|
| 總覽 | 銷售摘要・組合內容（名稱／描述）・秀出組合（素材） | 一律 |
| 組合商品 | 組合內含（成員清單）・成員與變更影響 | 一律 |
| 銷售設定 | 組合價格（成員自動加總、唯讀）＋折扣 % | 一律 |
| 銷售設定 | 折扣設定（排程特價） | 一律 |
| 銷售設定 | 庫存（成員分配表） | 一律；成員是多選項商品時該列展開成逐選項組合 |
| 銷售設定 | 上架設定・開賣設定・組合概況 | 一律（2026-09-10 起排在該分頁主體底下，不再是常駐右欄） |
