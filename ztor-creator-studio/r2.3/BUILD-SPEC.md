...
> **2026-09-22 · 已封存的販售管道只能「解除封存」回到已下架，不再直接重新上架（A spec-derived · D298，修訂 D284／D289；主規格 §7.14「封存與不可刪除」；UI-CHANGES 五十九、ASSUMPTIONS UIA-158）**：**共用狀態層** `js/listing-state.js`：新增 `unarchive(entity)`（已封存 → 已下架：`archived=false`、`listed=false`，排程／開賣／顯示／`unlistReason` 一律不動；未封存的直接回原物件）；`relist(entity)` 改只服務已下架→上架（不再寫 `archived=false`，對已封存回原物件不動），D290 的 `resetSale` 與 `unlistReason=null` 照舊；`bundleRelistPlan`／`relistBundle` 邏輯不變、`blocked` 的語意改「要先各自解除封存」；api 補 `unarchive`。**icon** `js/icons.js` 新 `archive-restore`（自繪：Tabler `archive` 的蓋子 `M3 6a2 2 0 0 1 2 -2h14…` 與箱身 `M5 8v10…` ＋ 箭頭 `M12 17v-6`／`M9 14l3 -3l3 3` 取代提把）。**電子商店** `e-shop.html`：`syncRowActions()` 已封存列由 `ensureMenuItem(menu, 'data-eshop-unarchive', 'archive-restore', 'e-shop.a.unarchive')` 長出「解除封存」（排在「編輯」前、與「封存」互斥），`data-eshop-relist` 不再產生；`doUnarchive(row)`＝`L.unarchive`＋`S.commit`＋`syncShopToggle`／`renderStatus`／`renderStock`／`syncRowActions`／`applyFilter`／`updateStatusCounts`（不開彈窗）；D289 版 `doRelist()` 與清單上的組合包成員閘門整段退場（清單沒有上架入口），`askConfirm` 的 `noCancel` 仍供其他情境。**三個細節頁** `product-detail.html`／`bundle-detail.html`／`auction-detail.html`：頁首第三顆鈕改 `[data-pd-unarchive]`／`[data-bd-unarchive]`／`[data-ad-unarchive]`（`btn--primary`、icon `archive-restore`、`product-detail.btn.unarchive`，`syncPrimaryAction()` 依 `isArchived` 露出）；點擊＝`unarchive`＋`commit`（bundle 由 `paint()` 的 `mirror()` 寫回 store）＋重畫（`paintListingCard()`／`paint()`／`paintCards()`）→ `applyArchived()` 拿掉 `data-archived`、上架開關回到可用、`canArchive` 為真「封存」鈕露出；D290 的 `[data-*-relist-sale-banner]` 條件（上架中且未封存且未開賣）不動，所以解除封存後不出現、上架開關切開後才出現；`ZtorArchivedGate.install` 的 `allow` 由 `[data-*-relist]` 改 `[data-*-unarchive]`；bundle 的 `tryRelist()` 開頭補 `if (LS.isArchived(bundleModel)) return`（只從已下架的上架開關進來）；三頁封存 banner、`cp.listing.archived-lock`、`e-shop.archive.body` 的 fallback 文案同步。**文案** `js/i18n.js`：見 UI-CHANGES 五十九。**DS**：Leave dialog 第三張示範內文與敘述、§4.202 Archived gate 示範鈕（`[data-ds-gate-unarchive]`，切換時 icon 在 `archive-restore`／`archive` 間換）與敘述、Icon 圖庫「自繪的第三顆例外」、Film picker 條目；`design-components.html` 重生。無新元件、無新 token。

> **2026-09-21 · 組合包三段改名「內容物／定價與庫存／命名與上架」＋頂列固定分節分頁（B 反饋，UI-CHANGES 五十五；STYLE-DECISIONS Q122 待裁決）**：**新元件 Section tabs**（§4.214）`ds-components/section-tabs.css`＋`js/section-tabs.js`——殼 `nav.section-tabs[data-section-tabs]`（`--control-h-md` 44px、`display:flex; align-items:stretch`、`flex:none`、`position:sticky; top: var(--section-tabs-top, 0)` 後備）> `.tabs.tabs--underline-short.tabs--underline-label` > `button.tabs__item[data-st-tab]` > `span`；目前段 `.tabs__item--active`＋`aria-current="location"`；目標段 `[data-st-section="content|price|name"]`。修飾 `--dialog`（`padding-inline: --sp-6 --sp-20`、`border-bottom: 1px --border`）、`--wizard`（`max-width: 1000px`、`padding-inline: calc(--sp-28 − --sp-14)`）。JS：`data-st-scroller`（`nav.closest` → 同層 `querySelector` → document；預設最近可捲祖先／window）、`data-st-offset`（逗號分隔的固定列選擇器，nav 在捲動區內時自算一個）；點擊 `scrollTo({top: 段上緣 − 偏移 − 16, behavior: smooth|auto(reduced-motion)})`，讓路以到站（±2px）／捲到底／`scrollend` 解除、2.5s 保險；scrollspy rAF 節流、「上緣 ≤ 偏移＋17 的最後一段」、捲到底＝末段；鍵盤 ←／→ 循環、Home／End；`ZtorSectionTabs.init(root?)`／`sync(root?)`、`data-st-ready` 防重複；初始化寫 `--section-tabs-h` 到捲動容器（無容器→`<html>`）。**放法＝併進既有頂列當第二排**（Q115 精神，畫面只有一條固定列）：建立活動第 6 步彈窗（`js/bundle-editor.js` `sectionTabsHTML()`，插在 `.payout-dialog__head` 與 `__body` 之間、捲動區外；`render()` 還原捲動位置後 `init(list)`）；`create-bundle.html` `.wizard__top` > `.wizard__top-bar` 之後（`data-st-offset=".wizard__top"`，頂列 80 → 124px；頁面 `<style>` `.preview-split { --preview-col-top: calc(96px + var(--section-tabs-h, 0px)) }`，`--preview-col-top` 是 preview-column 的消費者變數）。文案：`cpp.bd.sp.sec.content`「內容物／Contents」、`cpp.bd.sp.sec.price`「定價與庫存／Pricing & stock」、`cpp.bd.sp.sec.name`「命名與上架／Name & listing」同時當段標題與分頁標籤；`st.aria`「跳到區段／Jump to a section」；`cb.items`「內容物／Contents」（bundle-detail）。募資兩頁 `layout:'sections'` 無此排。

> **2026-09-21 · 電子商店「建立組合」與組合細節頁對齊第 1 批建構體驗（B 反饋，承五十三；5.1.5.4 §4 F2／F3／F4／F6／F7／F8／F10、5.1.5.9 §2.3／§2.5／§3、D292–D296）**：**建立組合** `create-bundle.html`——版面 `wizard__body--form.wizard__body--narrow.preview-split`（左 `.preview-split__form` 三張 `form-section--outlined.cb-sec`、右 `.preview-col`）＋ `wizard__bottom`（左 `.bd-foot-price`、右 `.bd-foot-actions`）；**不掛 `js/bundle-editor.js`**，本頁自己的 IIFE 重用零件 CSS 與 `cpp.bd.sp.*` 文案（取捨見 UI-CHANGES 五十四、STYLE-DECISIONS Q121）。分卡內節奏 `.stack.stack--loose`（小節之間）＋ `.stack`（小節內）＋ `.bd-sub`。**內容段**：票券三態 `showTixStage('entry'|'events'|'tiers')`——`[data-cb-tix-entry]`（`.cb-add-tile`）→ `[data-cb-event-panel]`（既有 `buildEvents()`：`EV.list()` 濾草稿、已取消／已結束 `--disabled`＋`aria-disabled`、每列徽章與外開連結，取消鈕回入口）→ `[data-cb-tier-panel]`（頁首 `[data-cb-tier-ev]` 活動名連結＋日期、`[data-cb-tier-back]`／`[data-cb-tix-remove]`；`renderTierTable()` 畫 `.bd-tbl--tix`（`[data-cb-tier-check]` 預設全不勾、`.bd-tbl__short`、表尾 `[data-cb-tix-foot]`）、`[data-cb-tier-qty]` `.bd-tix-qty`、`[data-cb-sem]` insight-row）；票券狀態 `tix = { event, tierIds, qty }`，`syncTicketMember()` 把它同步成固定 key `tix:member` 的假商品（`price`＝最低票價、`priceFrom`、`pool.total`＝n × Σ floor(剩餘 ÷ n)、`listed` 依活動狀態）與 `bundleDraft.members` 的票券成員，勾／張數改動即同步、不再有「加入為票券成員」步驟。商品：候選清單沿用 `buildCandidates()`（`[data-cb-cand]`；已封存 → 已下架 → 販售結束 → 即將開賣徽章、已加入標 `cb.tix.added` 並 `--disabled`），`addProduct()`／`removeProduct()` 改成員後 `renderItems()` 畫 `.bd-tbl--items.bd-tbl--items-qty`（`specText()`／`itemPriceText()`／`itemStock()`，`[data-cb-qty]` 每組用量、`[data-cb-item-remove]`）；`[data-cb-notes]` 相容性提示；`[data-cb-perks]` textarea → `perkLines()`。**定價與數量段**：`[data-cb-calc]` ← `calcListHTML()`（票券列 `cpp.bd.sp.calc.how.one|any`、商品列件數 > 1 時「$X × n · 規格說明」）；`[data-cb-disc-toggle]`＋`#cb-discount`（`.bd-disc` zstep）→ `#cb-sellprice`（`.field-readout.bd-readout--big`）＋`[data-cb-save]`（`saveText()`），關著時 `[data-cb-sell-plain]`（`cpp.bd.sp.nodisc`）；`#cb-lock-sets`（`.lockset__sets`＋`.bd-disc`，沒成員時 disabled）＋`[data-cb-sets-hint]`（`lockHintText()`：`capWho()` 由 `LS.bundleMaxSets().by` 推「票券」或商品名）；`[data-cb-sets]` ← `setsText()`（`sellableSets()`＝`LS.bundleQty`；`setsBeforeCap()` 暫時拿掉 cap 再問一次）；`#cb-edition` `.segmented.radio-cards`（`[data-cb-edition]`，限量露出 `#cb-total` `.bd-disc`＋`[data-cb-capmax]` `capHintText()`，`capOver()` 轉紅並擋）；`[data-cb-alloc-field]`（既有 `renderAlloc()`／`refreshAlloc()` 鎖定分配表，`bd-field--table`）；折扣設定小節（`#cb-sale-toggle`／`#cb-sale-percent` `.bd-disc`／起訖日／`[data-cb-stack]`）。**命名與上架段**：`#cb-name`（`syncName()`：未 `nameTouched` 且未聚焦時填 `suggestName()`＝`tixLineText()` ＋ 商品名頓號列；`[data-cb-usesug]` `.bd-suggest`）、`#cb-desc`、十格 `upload-assets--fill`（`upload:change` → `renderFan()` 取封面縮圖）、`[data-cb-film-picker]`、`[data-cb-listing-follow]`（`.kv-list.bd-listing` 一列「跟著活動」＋`evTinyBadgeHTML()`，`syncFollowEvent()`）與 `[data-cb-listing-controls]`（既有 D241 三開關兩塊，`.bd-sub` 小標）互斥。**右欄** `[data-cb-fan]` ← `fanHTML()`（同第 1 批的結構：`.preview-card.bpc`、票券列＋`data-cb-fan-tier` chip、商品列＋`data-cb-fan-var` chip、權益列、`fanPrice()` 收斂、`.bpc__meta` 剩 N 組／售罄、CTA＋`.bpc__why`；`FAN = { tier, variants }` 只影響卡）；外殼 `form-section--outlined`＋`.bpc__note`。**footer** `[data-cb-foot-final]`／`[data-cb-foot-sub]`（`footSubText()`）／`[data-cb-why]`（`gateWhy()`：成員 → 名稱 → 折扣 → 鎖定 → 限量上限 → 限時折扣 → 停售晚於開賣 → 定時上架時間）／取消 `data-wizard-back`／`#cb-create`。就地同步：`recompute()` 一次更新算式、售價、省、鎖定提示、可售、上限提示、提示、名稱、上架區、預覽卡、footer 與主鈕；票種表與已加入表只在成員變動時重畫；`i18n:applied`（一次）與 `ztor:lang-changed` 重畫 JS 拼出的字。**送出記錄**補 `perks`、`description`；含票券時 `listed:true`／`shown:true`、四個時間為 null。**組合細節** `bundle-detail.html`——`specTextOf(item)`（讀 `ztorGetProduct(item.id).options`）；`[data-bd-calc]` ← `bdCalcListHTML()`（取代 `bdCalcPartsText()`＋`#bd-basetotal` 大字）；`#bd-autoprice` 改 `.field-readout.bd-readout--big`＋`[data-bd-save]`（`--fact`）；`[data-bd-sets-hint]` 改 `cpp.bd.sp.lock.none|hint|over`（who＝票券／商品名）；`[data-bd-kpi-qty-meta]` 改 `cpp.bd.sp.sets.locked|capnote|capped|by`（已封存／已下架成員的覆蓋句不變）；補連 `bundle-editor.css`。**元件** `ds-components/bundle-editor.css`：`.bd-tbl--items-qty`（欄軌 `minmax(0,1fr) 120px 64px 112px 28px`）＋`.bd-tbl__per`；`.field-readout.bd-readout--big`（原單一 class 輸給 shared.css 末尾 @import 的 field-system.css）。DS 三件套同步（Bundle editor 卡 demo／Classes／Evidence、Bundle preview card Usage／Consumers）。呈現假設與缺口見 ASSUMPTIONS BDL-001／BDL-002 第 7 批追記；UI-CHANGES 五十四。

> **2026-09-21 · 建立活動第 6 步「新增組合包」兩欄版面（B 反饋，demo `docs/bundle-create-demo-2026-09-21.html` 定案；D292 裁決七／D293／D294／D295／D296、5.1.6.1 §4.6 F20、5.1.5.4 §4 F2／F3／F4）**：**共用編輯器** `js/bundle-editor.js` 新版型 `layout:'split'`（`SPLIT ⇒ SECTIONS`，彈窗、收合列、票券表、每場展開機制照用）——彈窗 `.payout-dialog--xwide.bd-dialog--split`（高度上限放到視窗高）、body `.payout-dialog__split.bd-split`（`--payout-split-side: 340px`）：`.bd-split__main` 三段 `.bd-sec--stack`（`secContentSplitHTML`／`secPriceSplitHTML`／`secNameSplitHTML`），`.bd-split__side`（殼的 `__split-side` sticky）放 `.bpc__title`＋`[data-bd-fan]`（`fanHTML()`）；footer `.bd-foot-price`（`[data-bd-calc-final]`「從 $Y 起」＋`__sub` `[data-bd-foot-sub]`）＋`.bd-foot-actions`（`[data-bd-why]`＋`[data-bd-cancel]`＋`[data-bd-primary]`）。內容段：`subHeadHTML`（`.bd-sub`）、`secScopeHTML`（SPLIT 下每場各一組補「第 1 場、第 2 場 各建立一組」）、`tixTableHTML`（`.bd-tbl__short`「不足 n 張」、鎖定時 `.bd-tbl--locked`＋勾選框 disabled）、`tixQtyFieldHTML`、insight-row `[data-bd-sem]`（`semanticText()`）、`sessionsFieldHTML`／`perFieldHTML`（把 `sessionsTableHTML`／`perTableHTML` 包成 `.field.bd-field--table`）、`productsBlockHTML`（`.fc-pick`＋`itemsTableHTML` `.bd-tbl--items`，`specText()` 單一規格／多規格 · 選項名）、額外權益 `textarea[data-bd-perks]`（一行一項 → `b.perks`）。定價段：`calcListHTML`（`.bd-calc-list` kv 列＋`__how`＋`__total` `[data-bd-calc-base]`）、折扣 `control-group`＋`stepperHTML('data-bd-f="discount"')`（`.bd-disc`）＋`.bd-readout--big [data-bd-calc-final]`＋`[data-bd-save]`（`saveText()`）、`.lockset__sets`（`data-bd-f="lockSets"`、`[data-bd-lock-hint]` `lockHintText()`／`.is-over`）、`[data-bd-sets]`（`setsText()`）、限量 radio-cards＋`data-bd-f="cap"`（`[data-bd-capmax]` `capHintSplit()`）。命名段：名稱 value＝`nameTouched ? name : suggestName()`（`suggestName` 活動變體＝`tixLineText` ＋ 全部商品）、`textarea[data-bd-f="desc"]`、`coverFieldHTML`（`.upload-tile-aside.bd-cover`）、上架 `.kv-list.bd-listing`「跟著活動 {getEvent().name}＋徽章」（掛載選項 `event: () => ({ name, status, label })`；draft 時 hint 寫「隨活動發布」）。**推導**：`memberCapWho()`（硬頂＋是誰壓的，`whoText`）、`lockN`／`lockOver`／`setsBeforeCap`／`capN`／`sellableSets`（capOver 在 SPLIT 比 setsBeforeCap）、`readiness()` 補 `!lockOver`、`gateOffFor()` SPLIT 看全部、`gateWhyText()`；商品成員帶 `options`／`variants`（`catalogue()`／`resolveItem()`／picker push），`itemMin`／`itemMax`／`itemRange`／`optName`（雙語選項名照 `documentElement.lang` 取一邊）、`itemsTotal` 活動變體取各規格最小、`priceRange()`＝多票種或任一商品區間 → `moneyFrom()` 前綴「從 … 起」；`isLocked(b)`＝`SPLIT && sold > 0`。**預覽卡**：`FAN[b.id]`（session／tier／variants，不放在 b 上）、`fanLeft`（票種在粉絲挑的那一場剩幾張）、`variantValueInStock`、`variantOf`、`fanPrice()`（選齊才收斂）；chip `data-bd-fan-sess`／`-tier`／`-var`＋`-opt`／`-val`。**就地同步** `syncSplit()`（算式、售價、省、鎖定提示、可售、上限提示、表尾、語意句、整張票券表（表內無焦點時）、每場預覽、名稱自動建議（未 touched 且未聚焦時）、預覽卡、footer、主鈕與理由）。**取消**：開列時 `b._snap = snapOf(b)`（SNAP_KEYS＋items＋tickets），`cancel()` 還原或丟掉新卡；✕／Esc 在 SPLIT 走 cancel；完成前濾空 perks、名稱未 touched 用建議名。**狀態欄位**新增 `lockSets`／`sold`／`nameTouched`（種子帶 name ⇒ touched；`commit()` 每場展開複製 discountOn／avail／cap／lockSets）；`ZtorBundleEditor.mounted` 登記實例。**建立活動** `create-event.html`——掛載 `layout:'split'`＋`event` getter（名稱讀 `[data-ce="name"]`、狀態 draft、徽章 `events.badge.draft`），補連 `insight-row.css`／`lock-sets.css`／`bundle-preview-card.css`（JS 動態 markup、check 15 掃不到）；右軌「影響範圍」與 Review 列不動。**元件**：`ds-components/bundle-preview-card.css`（新，namespace `bpc`，以 `.preview-card` 為底，媒體格 2:1＝Q120 待裁決）、`bundle-editor.css` 兩欄版型段、`chip.css` `.chip:disabled`；DS 三件套同步（§4.213、Bundle editor、Chip）。募資兩頁仍 `layout:'sections'`，零改動。呈現假設與缺口見 ASSUMPTIONS BDL-001／BDL-002 2026-09-21 第 6 批追記；UI-CHANGES 五十三。

> **2026-09-22 · 組合包三段的文字階層重整（STYLE-DECISIONS Q123 使用者裁決「方案 A · 三層收斂」；lab `docs/bundle-copy-hierarchy-lab-2026-09-22.html` 逐一稽核 67 個文字元素）**：純呈現決策，不動任何產品規則、欄位、狀態或算式——只減少文字元素並改由畫面表達。適用範圍＝表單分段，一段內最多三個文字層級（段標題 16 → 欄位標籤與小標 14 → 內文 12）。`js/bundle-editor.js`（`layout:'split'`，建立活動第 6 步）：三段的 `.bd-sec__sub` 不再產生；票券小節去掉小標 hint、「允許票種」標籤與表下規則 hint，表尾 `.bd-tbl__foot` 在 SPLIT 不畫（`layout:'sections'` 照舊）；勾到的票種剩餘不足一組改成整列灰化（新 `.bd-tbl__row--short`）＋「不足 n 張」`.ztor-badge`；唯一的語意句由 `.insight-row` 降成每組張數欄底下的 `.field__hint`（`[data-bd-sem]` 改指那個 hint）；商品小節「選填」降進搜尋 placeholder、已加入表表頭改「商品」；額外權益 hint 併進 placeholder；定價段去掉「原價合計」「折扣」「限量」三個標籤與關折扣時的「沒有組合優惠…」句（footer 有同一個數字），新增 `.bd-disc-grid` 補回兩欄基線，**可售套數升成 `.field-readout.bd-readout--big` 並移到鎖定套數之前**（讀數「最多 N 組」＋hint「受…限制」兩層，上限由三處收斂成一處），鎖定與限量的 hint 各縮到一句，單位「套」統一「組」；命名段去掉「上架」標籤與封面上傳格 CTA（改 `aria-label`）。`create-bundle.html` 同一套處置（電子商店專有的限時折扣說明與電影關聯說明另縮成一句；`insight-row.css` 不再消費、連結移除）；`bundle-detail.html` 跟上同一套用詞（刪「成員原價合計」標籤、鎖定套數 hint 只留「留空會怎樣」、可售套數 KPI 副標改由新 key 拼回同一句），**不重排**。`ds-components/bundle-editor.css` 新增 `.bd-tbl__row--short`／`.bd-disc-grid`；`js/i18n.js` 刪 18 把（墓碑註解在 `cpp.bd.sp.*` 段開頭）、縮短 10 把、新增 5 把（`sp.search`／`sp.sets.val`／`sp.sets.why.by`／`.why.cap`／`.why.lock`）。名稱欄**維持填入自動建議值、不降成 placeholder**（那是方案 B 的做法，使用者明確不採）。
>
> **2026-09-22（第二輪）· 內容物段的標題分級與場次重排（B 反饋，承 Q123；使用者看線上多場次活動第 6 步：「title 和區塊一樣沒有分清楚」）**：純呈現決策，不動產品規則。`js/bundle-editor.js`（`layout:'split'`）：內容物段改成段標題 → 三個 `.bd-group`（`groupHTML`）——**票券**（`ticketBlockHTML`：小標 → `tixTableHTML` → `tixQtyFieldHTML`（SPLIT 不再加 `mt-16`）→ **`datesFieldHTML`**）、**商品**（`productsBlockHTML`）、**額外權益**（新 `perksBlockHTML`，由 `.field__label` 升成小標，textarea 改 `aria-label`）。`datesFieldHTML`＝多場活動才有的單一欄位 `.field.bd-field--table[data-bd-dates]`：標籤 `cpp.bd.sp.dates`「場次／Dates」→ 二選一卡（`scopeCardsHTML`，由原 `secScopeHTML` 拆出控件本體；sections 版型的 `secScopeHTML` 改包它）→ `scope=='shared'` 接 `sessionsTableHTML` ＋ hint `cpp.bd.sec.sess.sub`、`scope=='per'` 接 `perTableHTML` ＋ hint `cpp.bd.per.preview.sub|empty`；`isMultiDate()` 判定（票帶有名字的 group 且 > 1 組），單場活動整格不產生。墓碑 `sessionsFieldHTML`／`perFieldHTML`；`cpp.bd.scope.per.names` 在 SPLIT 不再產生（表本身逐場列出）。`ds-components/bundle-editor.css`：`.bd-sub__title` 改群組小字標配方（`--font-display`、`--fs-14`、`--fw-regular`、`--muted-foreground`、0.04em、uppercase；同 `.bpc__title`／`.bd-tbl__gname`）；新增 `.bd-group`（flex column、gap `--sp-12`、群內 `.field` 下外距歸零）。`js/i18n.js` 新 key `cpp.bd.sp.dates`。`create-bundle.html`：額外權益升小標（`[data-cb-perks-group]`）、`[data-cb-tier-ev]` 活動名改走 `.field__label`（避免被小標的 uppercase 轉大寫）。DS 三件套同步（Bundle editor demo 新增「小標 vs 欄位標籤＋合成一格的場次」、Classes、md 敘述）；STYLE-DECISIONS Q123 補「小標與欄位標籤的分別」一句；UI-CHANGES 五十七。
>
> **2026-09-21 · 組合包票券成員＝允許票種清單 × 固定張數、粉絲任選一種；商品成員規格由商品決定（D295／D296；主規格 §7.14「庫存池與商品」票券成員條／「組合包成交條件」、5.1.5.4 §4 F2／F3／F4、5.1.5.9 §2.3、5.1.6.1 §4.6 F20、5.1.6.3 §2.6.1）**：**資料形狀**三處一致——`js/bundle-editor.js` 的 `b.tickets`、`js/events-store.js` 的 `bundles[].tickets`、`create-bundle.html` 送出記錄的票券成員都是 `{ tierIds:[…], qty:n }`（舊 `[{id, qty}]`／字串陣列只在相容層：`tixNorm()`、`ztorEvents.bundleTickets()`），站上不留每列張數的讀取。**共用編輯器** `js/bundle-editor.js`（活動變體）——票券清單表 `.bd-tbl--tix` 收成四欄（`.zcheck` 勾選框｜票種｜單價｜剩餘），每列步進器與小計退場，表下一個整組張數欄 `.bd-tix-qty`（`.amount-field--suffix.amount-field--readonly.zstep`＋「張」，`[data-bd-tix-qty]` input 就地同步、change 補回生效值）；表尾「允許 K 種 · 每組 n 張 · 原價從 $X 起」；`ticketValue()`＝最低允許票價 × n、`moneyFrom()` 在允許多種時把算式（`cpp.bd.calc.tix.any`）、原價合計、粉絲實付、footer、收合列全部前綴「從 … 起」；`memberCapMax()` 票券部分＝各允許票種 floor(剩餘 ÷ n) 加總（多場：shared 全部加總、per 取最緊的一場）再與商品部分取最小；`summaryMeta()`／每場預覽 `tixLineText()`「VIP／搖滾區 任選 × 2」（一種「VIP × 2」）；`commit()` 每場各一組展開每組 `{ tierIds:該場票, qty }`；`itemsHTML()` 對 `variant:'multiple'` 的商品補「規格由粉絲購買時選」；掛載選項 `defaultAllTiers`（create-event 傳 true）讓「新增」預帶全部票種、張數 1。**建立活動** `create-event.html`——Review「N 組 · 每組 n 張」（各組張數不同時「1／2」並列）、右軌「N 組可選 · 每組 n 張」。**建立組合** `create-bundle.html`——票種清單改 `.zcheck` 勾選列、清單下方 `.cb-tix-foot`（整組張數 `.zstep`＋「加入為票券成員」＋提示：沒勾「至少勾選一種票種」、已有票券成員「要改先移除」並停用清單）；票券成員包成一顆假商品 `tix:<ev>:<ids>`（`price`＝最低票價、`priceFrom`、`pool.total`＝n × Σ floor(剩餘 ÷ n) 讓 `ListingState.bundleQty()`／`bundleMaxSets()` 算出 D296 決定四的加總，改張數時 `refreshTicketPool()`）；已選列標題「活動名 · VIP／Floor 任選 × n」、meta 逐票種「VIP 剩 60 張 · Floor 剩 140 張 · 每張各自 QR」、價格「從 $X 起」；`priceParts().hasVar` 把 `data-price-from` 列算進區間；多選項商品成員列在 `pick()` 補 `[data-cb-spec-note]`。**組合細節** `bundle-detail.html`——`fromEventBundle()` 把票券成員合流成一筆（同上假商品；`memberItems` 帶 `tierNames`／`tierLefts`／`priceFrom`），`memberRowHTML()` 票券列標題「活動名 · VIP／Floor 任選 × n」＋meta 逐票種剩餘＋`data-bd-price-from`，商品列 `multi` 時補「規格由粉絲購買時選」；`bdMoneyFrom()` 讓原價合計／售價／hero 價在允許多種時前綴「從 … 起」，算式走 `cpp.bd.calc.tix.any`；數量欄改動時票券假商品的池與名稱跟著 n 重算；已售出鎖定（`syncMembersLock`）涵蓋允許票種清單與張數（本頁沒有改票種清單的入口，只讀）。**活動詳情** `event-detail.html`——票務分頁組合包卡「內含票券」＝`cpp.bd.sum.any`／`cpp.bd.sum.line`，已佔用票券＝已售組數 × n（逐票種分配示範資料沒有、只顯總數）；銷售分頁量表列副標「VIP／Floor 任選 ×n」。**電子商店** `e-shop.html`——`injectEventBundles()` 成員摘要同一句；`eventBundleModel()` 可售套數＝Σ floor(剩餘 ÷ n) 再與上限 − 已售取最小。**示範資料** `js/events-store.js`：`bd-vip-zine`（VIP／Floor 任選 × 1，售價 4,300）、`bd-infield-tee`（內野／外野 任選 × 2，內野售罄仍可售）。元件：`ds-components/bundle-editor.css`（欄軌與 `.bd-tix-qty`）；DS 三件套同步。呈現假設與產品缺口見 ASSUMPTIONS BDL-001／BDL-002 2026-09-21 D296 追記；UI-CHANGES 五十二。
>
> **2026-09-21 · 兩段式面板的表單欄位檢視態改純文字、按「編輯」才變輸入框（STYLE-DECISIONS Q119 使用者裁決，取代 2026-07-27／2026-09-11 兩條舊裁示）**：元件層一次改——**JS** 新增 `js/view-mode.js`（`window.ZtorViewMode = { mount, sync, configure, syncAll }`）：對每個 `[data-editable]` 面板，把控件根（`[data-vm-group]` 併句組 → `.amount-field`〔含 `.zstep`〕→ `.date-input` → 單獨的 `input`／`textarea`）標 `data-vm-control`，後面插 `<span class="field-readout" data-vm-readout data-vm-kind>` 寫現值；跳過 `[data-edit-exempt]`、`.variant-table`（Q118）、`.tag-input`、`.zselect__native`、`.combobox`、`.todo-list`、開關／單選／選擇卡、checkbox／radio／file／hidden。讀數文字：空「—」、金額前綴單位＋千分位（`configure(panel, { format })` 讓頁面掛自己的格式函式）、後綴單位是字隔空白、`datetime-local` 去 T、`data-vm-suffix` 補單位；控件 inline style 的 flex／width／max-width／grid-* 抄給讀數（欄軌不變）。重算：面板 `data-mode`／`hidden`／子樹 MutationObserver（微任務，`takeRecords()` 吞掉自己造成的紀錄）、面板 `change`、`i18n:applied`、`load`、各頁 `applyReadonly` 明呼 `sync(panel)`。**CSS** `shared.css` `[data-mode="view"]` 區塊重寫：`[data-vm-readout]` 預設 `display:none`、檢視態 `[data-vm-control]` none／讀數 block（textarea 讀數 `pre-wrap`）；下拉 `.zselect__trigger`／`.select` 去框去底去內距去箭頭、`--fs-14` ui 字體前景色、`pointer-events:none`（`:not([data-edit-exempt] *)`）；`.tag-input__field` 去框去薄膜去內距、`.chip--removable` 內距回 `--sp-12`；`.switch`／`.radio-list`／`.segmented.radio-cards`／`.selection-card` `pointer-events:none`（例外清單；`.segmented` 只鎖 `.radio-cards`，總覽分頁的圖表區間切換與待辦 `.zcheck` 不受影響）；`.field__req` 收起。**放 `shared.css` 而不抽 `ds-components/view-mode.css` 的理由**：`[data-mode]` 是面板的屬性契約、不是 class 元件，check_ds_sync 檢查 15 靠 class 名對 `<link>`、抽出去後哪一頁漏連只會靜默失效；而 `shared.css` 每一頁都載、既有 `[data-mode="view"]` 規則（按鈕白名單、圖磚、hint）本來就住這裡，讀數本身沿用既有 `.field-readout`（field-system.css）。`ds-components/store-settings.css` 補 `.ss-url` 檢視態（去殼、前綴與讀數同行）。**消費頁**：`product-detail.html`（`configure` 掛 `money()`、尺寸 grid 標 `data-vm-group=" × " data-vm-suffix=" cm"`）、`bundle-detail.html`、`event-detail.html`（早到政策單選卡補 `data-view-safe`，此前檢視態被白名單整組藏掉只剩標籤）、`project-detail.html`、`store-settings.html` 各在 `applyReadonly`／`ssSetMode` 加一行 `ZtorViewMode.sync`，並 `<script src="js/view-mode.js">`（放 date-input／stepper／social-links 之後）。`design-system.html` Field system 卡新增可切換的「view mode / edit mode」示範並載入該 JS；`design-system.md` Field system／Zselect／`locked-field-ink` 條目同步。實測（Playwright，zh／en）：七個面板檢視態 0 個可見輸入框（`.zselect__native` 為 clip 隱藏的資料源，不計）、編輯態輸入框如常、改值→取消→讀數還原、儲存→讀數新值、console 0 錯誤、0 raw key。
>
> **2026-09-21 · 活動草稿可刪除、草稿活動的組合包不進 Bundles 清單、含票券組合包三開關只收窄不回寫（D294；5.1.6 F1／F3、5.1.5 F4 組合、5.1.5.9 §2.3、主規格 §7.14「封存與不可刪除」活動比照）**：**活動清單** `events.html`——草稿列（`data-stage="draft"` 或 `data-go="create-event.html?draft=…"` 的列，含草稿系列母列與其子場）kebab 的「刪除」改 `.dropdown__item.dropdown__item--danger[data-ev-delete]`（同 e-shop 草稿刪除的 `data-eshop-delete` 語彙）；發布過的列的殘留「刪除」項整批退場（靜態 markup 37 處），列操作只剩開啟／編輯／複製（進行中列另有現場報到台），取消活動的入口維持在詳情頁頁首。確認彈窗借 `ds-components/leave-dialog.css` 的殼（`[data-ev-confirm-dialog]`，`</body>` 前、用時才查）：標題 `events.delete.title` 帶活動名、內文 `events.delete.body`、主鈕 `.btn--destructive` `events.delete.confirm`、取消／✕／Esc 都不動資料；確認後 `row.remove()`＋從 `rows`／`children` 移除（母列連同 `data-rowdis-child` 子列）→ `apply()`（分頁計數、頁尾）＋`syncKpis()`（頁首活動總數）。原型只到呈現：清單是靜態 HTML，重整回來仍在。**電子商店** `e-shop.html`——載入 `js/events-store.js`；`injectEventBundles()` 在 DOMContentLoaded（排在 products-store 的 persona patch 之後）把各場活動的 `bundles[]` 注入 Bundles 面板、接在最後一個非草稿組合列後面，`ev.status === 'draft'` 的整場跳過；列 `data-type="bundle" data-bundle-id data-event-bundle="<活動 id>"`，結構同靜態組合列（`.product-list__meta` 放所屬活動名＋`calendar` icon、`.product-list__members` 逐行「票種 × 張數」與商品名、粉絲分級「—」、價格 `$` 整數、狀態徽章、庫存 `可售套數 / 上限`、顯示開關、kebab＝編輯（`bundle-detail.html?id=&ev=`）＋前往活動頁 `e-shop.a.openevent`）；`rowModel()` 見 `data-event-bundle` 先問 `eventBundleModel()`（由活動狀態組 ListingState 實體：`listed`＝已排程／售票中／進行中／已結束、`onSale`＝非已排程、`saleEnd`＝已結束的活動日、已取消帶 `unlistReason: event-cancelled`；可售套數＝各票種剩餘 ÷ 每套張數取最小再與上限 − 已售取最小；`shown` 讀列上的 `data-shown`＝只收窄這一列）；`syncRowActions()` 對這種列不補封存／重新上架／補貨；`js/products-store.js` `patchBundleRows()` 選擇器排除 `[data-event-bundle]`。**組合商品細節頁** `bundle-detail.html`——上架卡新增三句 `.lctl__lock-note`（`[data-bd-event-draft]`、`[data-bd-event-list-lock]`、常駐 `[data-bd-event-scope]`「這裡的設定只收窄這個組合包，不會影響活動本身」）、開賣卡新增 `[data-bd-event-sale-lock]`；`syncEventBounds()` 依所屬活動的階段套範圍：草稿→`bundleModel.listed=false`、上架開關 `aria-disabled`＋`.switch--disabled`、藏顯示列／`.lctl__rule`／`.lctl__derived`／整個開賣 section、藏「已下架」lock-note、顯示「隨活動發布」；未上架（已取消）→上架開關停用＋說明；未開賣（已排程／已結束）→`onSale=false`、`saleStart=null`、`paintBdSaleCard()`、開賣卡 `.lctl--locked`＋說明、藏未開賣提醒 banner；開賣中→全部可操作。掛在上架開關 write、開賣 radio change、`applyArchived()`（它會重設 `aria-disabled`）與 `ztor:lang-changed` 之後；`bindSwitch()` 的 flip 對 `aria-disabled` 的開關直接 return（鍵盤路徑；指標路徑由 `switch.css` 的 `pointer-events:none` 擋）。切換只寫 `bundleModel`（工作副本），`mirror()` 對活動組合包本來就不 commit，`events-store` 零寫入。**示範資料** `js/events-store.js`——`nick-symphonic-taipei`（已排程）掛 `bd-lower-program`、`onstage-encore-draft`（草稿）補一個票種與 `bd-encore-tee`。元件零新增（leave-dialog、dropdown-menu `--danger`、listing-controls `--locked`／`__lock-note`、switch `--disabled` 全是既有）。呈現假設見 `ASSUMPTIONS.md` EDIT-001／BDL-001 2026-09-21 追記。
>
> **2026-09-21 · 表格檢視態純文字／編輯態輸入框（Q118）**：兩段式編輯面板（`data-editable` ＋ `data-mode="view"|"edit"`）裡的 `.variant-table`，檢視態不再放 `readonly` 輸入框——`product-detail.html` 的 `renderEditCombos(m, editable)` 依模式把價格／上限／SKU／成本畫成 `.variant-cell--ro`（`ecText()`，金額走 `money()`、空值「—」）或 `.input`（`ecInput()`）；`setMode()`／`cancelEdit()`／`saveEdit()` 既有的重畫路徑因此自然切換，`applyReadonly()` 不用改。元件層 `ds-components/variant-builder.css`：`--vt-input-pad-x`（表格輸入框左右內距 9px 收成變數）、`.variant-table:has(.input)` 讓 `--stock`／`--ro` 吃同一個左內距（編輯態文字起點對齊，實測 0px 差）、`[data-mode="view"] .variant-table` 安全網把殘留的 `.input`／`.amount-field` 渲染成純文字。`restock-modal.css` 的 `.restock-table__ro` 同原則去邊線。表單欄位的檢視態維持 2026-09-11 的 disabled 外觀（Q119 待裁決→**同日裁決跟進純文字，見上一則**）。

> **2026-09-21 · 商品細節頁 Edition 鎖定呈現與限量上限欄（spec 5.1.5.1 §2.8／§2.10、§7.2、5.1.5.2 F3.3／F3.4）**：`product-detail.html` 商品設定分頁「規格與價格」面板新增 Edition 卡（`[data-pd-edition-section]`）——`#pd-edition` 沿用建立商品的 `.segmented.radio-cards` 加 DS `.segmented--locked`（`aria-disabled`、各段 `disabled`＋`data-view-safe`，只由 `syncLockedSeg()` 依 `state.edition` 亮段）；單一規格＋限量露出「上限數量」`input[data-pd-edit-cap]`（`type=number`、`.field__hint--fact` 已售事實句、`.field__error[data-pd-cap-err]`），多選項＋限量露出提示句（`data-when-var`＋`data-when-edition` 由 `applyVis()` 算）。逐選項組合定價表（`--preview`）與庫存分頁管理表（`--stock`）各補「上限」欄：表頭 `span.variant-cell--total`、列格分別為 `.variant-cell--input.variant-cell--total > input[data-ec-field=cap]` 與 `.variant-cell--ro.variant-cell--total[data-vc=total]`；`applyVis()` 對兩張表一起 toggle `variant-table--limited`；`ds-components/variant-builder.css` 新增 `--preview.--limited`（庫存後插 84px）與 `--stock.--limited`（在庫後插 56px）欄軌。資料：`copyModel()`／`regen()` 帶 `cap`／`sold`（商品層與 `variants[]`），`checkCaps(m)` 逐格驗「不得低於已售」（`aria-invalid`＋錯誤句；多選項點名列）、`saveEdit()` 不合法即 return；`syncLocksToStore()` 寫回 `variants[].cap`、多選項商品層 `cap`＝加總、單一規格寫 `product.cap`，儲存後 `pdRefreshStock()` 重畫 KPI；`setovRows()` 加「庫存版本」列（`cp.edition`，值 `cp.edition.unlimited`／`product-detail.setov.edition.limited-total`／`.per-variant`，歸 `ops` 組、跳 `pricing`）。封存唯讀：新欄位皆 `input`，`archived-gate.js` 預設清單已擋。`js/products-store.js` 檔頭補 `variants[].cap / sold` 定義。示範資料與清單退路數字見 `UI-CHANGES.md`（四十六）；呈現假設 UIA-156。
>
> **2026-09-18 · 電子商店組合包接「活動票券」成員、兩份組合包資料合流（D292／D293 第 3 批；5.1.5.4 §4 F2／F3／F4／F6、5.1.5.9 §2.2／§2.3）**：**建立組合** `create-bundle.html`：成員區加 `.segmented` 成員型別切換 `[data-cb-kind]`（商品｜活動票券），商品路徑（`#cb-prod-pick`）不變；活動票券路徑（`#cb-tix-pick`）＝活動候選清單（`.picker`＋`.data-list` `#cb-events`，讀 `js/events-store.js` 的 `ztorEvents.list()`；預設列前 5 筆＋第一筆不可加入的、搜尋涵蓋全部活動；列＝主視覺縮圖｜活動名｜日期 · 場地 · 類型｜狀態徽章（`EV_BADGE` 對照 `events.badge.*`／`event-detail.badge.cancelled`，色態同 event-detail）｜活動詳情外開連結；已取消／已結束 `.data-list__row--disabled`＋`aria-disabled` 不可點；已有票券成員時其他活動照列、標「一組只含一場活動的票」不可點）→ 點活動換成票種清單（`#cb-tiers`：票種名｜單價 · 剩 N 張（0 紅字）｜列尾 `.zstep` 張數（min 1、預設 1）＋「加入」；已加入的票種標「已加入」停用；「換一場活動」回活動清單）。票券成員以假商品 `ticketProduct()` 進 `products[]`（`pool.total`＝票種剩餘、`listed` 依活動狀態），原價／鎖定套數上限／可售套數與商品成員同走 `ListingState`；已選列 `data-cb-kind="ticket"`，標題「活動名 · 票種 × 張數」（活動名連到 event-detail）、meta「剩 N 張 · 每張各自 QR、各自報到」。**每個已選成員列開放數量（每套用量）欄**：`.zstep` `[data-cb-qty]`（商品「件」／票券「張」）插在列尾最前，改動寫 `bundleDraft.members[].qty` → `applyLock`＋`refreshAlloc`＋`recompute`。定價區：`sumMembers()`＝Σ 價 × 數量；合計旁新增算式行 `#cb-calc-parts`（`.field__hint--fact`，沿用 `cpp.bd.calc.*` 片語，件數 1 的商品不寫 × 1）；含票券時露出 `#cb-fee-note`「整筆依活動票務費率」。上架設定卡頂加 `.lctl__lock-note [data-cb-follow-event]`「跟著活動「名」（狀態）…」（`syncFollowEvent()`）。最少成員數 `ok2 = n >= 1`、`cb.min-items` 改「至少加入 1 個成員」；成員摘要卡含票券時多一條 `cb.note.tickets`。送出記錄 `members[]` 逐筆帶 `kind`（`ticket`＝`eventId`＋`tierId`＋`qty`；`product`＝`productId`＋`qty`）與 `eventId`；票券成員不寫 `pool.locks` 回 products-store。補掛 `stepper.css`／`partials/stepper.js`／`js/events-store.js`。**組合細節** `bundle-detail.html`：`?id=` 先 `ztorGetBundle`，查不到再 `findEventBundle()` 掃 `ztorEvents.list()` 各場 `bundles[]`（可帶 `&ev=` 指定同 id 多場中的哪一場），`fromEventBundle()` 轉成本頁物件（票券成員→假商品 `tix:<ev>:<tier>`、商品成員先用名稱對回當前 persona 的商品記錄、對不到就假商品 `evp:<bundle>:<i>` 帶 events-store 的示範單價；`evSeed` 供 `bundleModel` 讀三開關／cap／sales，`isEventBundle` 不 `commit` 進 products-store 工作階段；活動取消→`unlistReason.type='event-cancelled'`＋`bd.unlist.reason-event`；`intMoney` 整數顯示；hero／cover 用活動主視覺 `imgSrc`，附圖槽只填商品成員）；兩邊都查不到→`.page` 換成 `.empty-stub`（`bd.notfound.*`＋「回到電子商店」），不再退回示例組合（補掛 `empty-stub.css`）。成員列改用單一模板 `memberRowHTML(item)`（載入與編輯態加入共用）：`data-bd-kind`／`data-bd-per`／`data-bd-price`，票券列標題「活動名（連 event-detail）· 票種 × N」＋meta 行「日期 · 場地 · 每張各自 QR」，列尾 未鎖定／已鎖定 小字｜`.zstep` 數量 `[data-bd-qty]`｜單價；`bundleModel.members` 由列讀 `kind`／`qty`；`productOf()`／`allProducts()` 先查 `extraProducts` 再查 store。定價：`bdBaseNow()`＝Σ `data-bd-price` × `data-bd-per`（改成員或數量即時重算，舊版是載入時的常數）、算式行 `[data-bd-calc-parts]`、含票券露出 `[data-bd-fee-note]`；成員區底部 `[data-bd-ticket-note]`（數量池與 QR 口徑）。**已售出後鎖定** `syncMembersLock()`：`bundle.sold`（products-store 讀 `sales.units`、活動側讀 `b.sold`）> 0 → 成員區頂 `.info-banner`（`lock`）`bd.members.locked`、編輯態加入區 `[data-bd-edit-only]` 收起、移除鈕藏、數量欄 `disabled`；售價／折扣／上限／開賣照常可改。最少成員數：移除鈕在只剩 1 個時藏、0 個露出 `cb.min-items`。上架卡 `[data-bd-follow-event]`（`syncFollowEvent()`，成員變動與切語言重算）。**元件** `ds-components/field-system.css` 新增 `.field__hint--fact`（事實說明，`[data-mode="view"]` 不收起；例外規則在 `shared.css` 那條改成 `:not(.field__hint--fact)`），DS 頁 Field system 卡加 `--fact` 示範。**資料** `js/events-store.js` 七組活動組合包的 `products[]` 補 `price`（示範單價＝組合售價 − 票券小計）。i18n 新增 `cb.kind.*`／`cb.qty.label`／`cb.tix.*`／`cb.note.tickets`／`cb.fee.ticket-rate`／`cb.follow-event`／`bd.members.ticket-note`／`bd.members.locked`／`bd.unlist.reason-event`／`bd.notfound.*`；`cb.items` zh「商品」→「成員」、`bd.hero.items`／`product-detail.kpi.links-meta-items` zh「件商品」→「個成員」。呈現假設與缺口見 ASSUMPTIONS BDL-001／BDL-002／EDIT-001 的 2026-09-18 第 3 批追記。
>
> **2026-09-18 · 活動票務商品改「票券清單（票種 × 張數）」模型（D292／D293；5.1.5.4 §4 F2／F3／F4、5.1.6.1 §4.6 F20）**：共用套組編輯器 `js/bundle-editor.js` 的活動變體（掛載時有 `tickets`）——資料 `tickets` 由 `[票種 id]` 改 `[{id, qty}]`（`tixNorm()` 把舊字串陣列視為 qty 1）；票種卡由 `.radio-cards` 複選卡改成表格化票券清單 `.bd-tbl--tix`（勾選框 `.zcheck`｜票種｜單價｜張數 `.zstep` 家用步進器（`--control-h-xs`，未勾選停用）｜小計＋表尾 `.bd-tbl__foot` 合計「共 N 張 · 票券原價 $X」）；`ticketValue()`＝Σ（票價 × 張數）、`listPrice` 旁列算式 `[data-bd-calc-parts]`（活動變體原價合計永遠顯示，折扣關著也看得到）；`memberCapMax()`（前身 `ticketCapMax`）＝各成員 floor(剩餘 ÷ 每套用量) 取最小（票券看票種張數、商品看 `stock`），不限量選項下也寫出硬頂；`isValid()`／`hasContent()` 活動變體改「票券任一筆或商品任一件」；`summaryMeta()` 逐筆報「VIP × 2 · 搖滾區 × 1 · 1 件商品」（售價由收合列自己的價格欄承擔）；`commit()` 每場各一組展開時每組沿用 `[{id, qty}]`、沒挑票種的 per 組合包當一組收起。`create-event.html`：補掛 `stepper.css`／`partials/stepper.js`，Review 步驟補「組合包」列（`ce.rev.bundles`／`ce.rev.bundlesn`／`ce.ph.nobundles`），右軌「影響範圍」改報「由 N 組使用 · 每組 M 張」，`bundleEd` 改 `var` 讓 `syncReview()` 在初始化前可安全讀到 null。募資兩頁（create-project／project-detail）無 `tickets`，行為與文案不變（`cpp.bd.items.sec.sub` 的活動版改字放 `.ev` 變體）。呈現假設與產品缺口見 ASSUMPTIONS BDL-001／BDL-002 的 2026-09-18 追記。

> **2026-09-18 · 下架清除四個排程時間並退回未開賣、重新上架後須再設開賣（D290，修訂 D265／D289；主規格 §7.14「下架的副作用」、5.1.5.1 §2.16、5.1.5.9、5.1.5.8）**：**共用狀態層** `js/listing-state.js`：新欄位 `onSale`（boolean，缺值＝true）；`unlist(entity, reason)` → `listed=false`、`listAt`／`unlistAt`／`saleStart`／`saleEnd`＝null、`onSale=false`、（拍賣）`listedAt=null`；`relist(entity)` → `archived=false`、`listed=true`、`unlistReason=null`、四個時間維持 null、`onSale=false`（不再恢復開賣；`relistBundle` 連帶成員同）；`isOnSale(entity)`；`expireScheduled(entities, now)`（`listed` 且 `unlistAt` 已過 → `unlist()`，回被落實的清單；要在 `scheduledUnlistCascade` 之後跑）；`saleStatus` 第一條 `!isOnSale → 'coming'`（不帶日期）、`canBuy` 未開賣 false、`bundleQty` 成員未開賣視同 0、`auctionStart` 未開拍回 null（結標時間跟著 null、`auctionStarted` false）。**資料** `js/products-store.js`：`SESSION_FIELDS` 八欄（加 `saleStart`／`saleEnd`／`onSale`）；`runScheduledCascade()` 末尾對當前 persona 商品＋組合包＋全部拍賣跑 `expireScheduled` 並 `commit`。**三個細節頁**：開賣（開拍）radio-list 第三項 `data-pd-sale-mode="off"`／`data-bd-sale-mode="off"`／`data-ad-start="off"`（`cp.listing.sale-off`／`ca.start.off`）；進頁與重畫時模式＝`onSale===false ? 'off' : (saleStart ? 'schedule' : 'follow')`；選項切換寫 `onSale`（off＝false，其餘 true；非 schedule 一併清 `saleStart`）並 `commit`；下架／重新上架後 `paintSaleCard()`（product）／`paintBdSaleCard()`（bundle，`mirror()` 多鏡射三欄）／`paintCards()`（auction）把模式、開賣時間、定時停售開關重畫；上架開關切開改呼叫 `ListingState.relist`；hero 下新增 `.info-banner`（`clock`）`[data-pd-relist-sale-banner]`／`[data-bd-relist-sale-banner]`／`[data-ad-relist-sale-banner]`，條件＝上架中且未封存且 `onSale===false`；設定總覽開賣列多 `product-detail.setov.ls.not-on-sale`。**文案**：`e-shop.unlist.body`／`blocked-body` 補「下架會清除上架與開賣排程，重新上架後需再設定開賣」；拍賣改用 `ad.unlist.body`（開拍版）；`e-shop.relist.members-body` 英文 `go back on sale` → `are relisted`。**清單** `e-shop.html` 不改：列徽章由推導層算。DS 無新元件（沿用 radio-list／info-banner）。呈現假設與缺口見 `ASSUMPTIONS.md` UIA-155。

> **2026-09-18 · 下架確認與組合包連動、組合包重新上架連帶已下架成員（已封存才擋，D289）、封存改單純確認、封存唯讀補齊逐列與關聯（D288／D289，主規格 §7.14「下架確認與組合包連動」新小節、「組合包成交條件」不變式、「封存與不可刪除」四處；5.1.5.1 §2.2／§4、5.1.5.9 §2.2／§2.3／§4、5.1.5 F4）**：**共用狀態層** `js/listing-state.js` 新增 `unlistWithBundles(product, bundles)`（逐個組合包 `unlist` 並記 `unlistReason={type:'member-unlisted', productId, productName, auto:false}`、再下架單售；回傳被連動的組合包）、`bundleRelistPlan(bundle, productsById)`（回 `{ unlisted, blocked }`：unlisted＝已下架未封存的成員、blocked＝已封存／草稿／查不到的成員；`productsById` 可為物件或 function；`unlistedMembers` 為相容別名＝兩者合併）、`relistBundle(bundle, productsById)`（D289：blocked 非空回 `{ ok:false, blockers }` 且什麼都不改；否則 unlisted 成員逐個 `relist`、再 `relist` 組合包，回 `{ ok:true, relisted:[product…] }`；反方向不連動）、`scheduledUnlistCascade(bundles, productsById)`（成員 `unlistAt` 已過而組合包仍上架中 → 組合包 `unlist` 並記 `auto:true`；回 `[{bundle, product}]`）；`archiveWithBundles` 移除（墓碑）、`archive()` 本來就不看組合包。**資料** `js/products-store.js`：`archiveBlockers` 改名 `unlistBlockers`；新增 `relistPlan(bundle)`／`relistBundle(bundle)`（餵當前 persona 的商品；成功時 `commit` 組合包與連帶上架的成員）、工作階段覆蓋 `commit(entity)`／`forgetSession()`（`sessionStorage['ztor.listing.session']`，只記 `listed`／`listAt`／`unlistAt`／`archived`／`unlistReason` 五欄，`seedListing`／`seedBundle`／`seedAuction` 套回）、`autoUnlisted()`；所有 seed 補完後跑一次 `runScheduledCascade()`；示範 `LISTING_SEED.patch` 改 `{}`（上架中）、`coaster: { unlistAt: '2026-09-15T23:59:00' }`（新單售，`coaster-pack.webp`）、`BUNDLE_SEED.backstage-set`（coaster＋cap，seed 上架中、載入時被自動連動下架）、`postcard-set.unlistReason.type` 改 `member-unlisted`。**電子商店** `e-shop.html`：`askArchive()` 改單純確認（不查 blockers）；`doRelist()` 對組合列先 `S.relistPlan()`：`blocked` 非空 → `askConfirm({ title:'e-shop.relist.blocked-title', items, okKey:'e-shop.relist.blocked-ok', noCancel:true })`；`unlisted` 非空 → `askConfirm({ title:'e-shop.relist.title', body:'e-shop.relist.members-body', items, okKey:'e-shop.relist.members-confirm' })` 確認後 `S.relistBundle()`＋`refreshAll()`；都上架中直接 `relistBundle`；`askConfirm` 新選項 `noCancel` 收起取消鈕；`patch` 列 `data-status="live"`、新增 `coaster` 列（`e-shop.rowCoaster.*`）。清單沒有「下架」入口（維持在細節頁），不新增。**單售細節頁** `product-detail.html`：新增 `askUnlist()`——`S.unlistBlockers(product.id)` 有值時 `askConfirm({ title:'e-shop.unlist.blocked-title', body:'e-shop.unlist.blocked-body', items, okKey:'e-shop.unlist.blocked-confirm' })`、確認 `L.unlistWithBundles()` 並 `commit` 全部；否則 `e-shop.unlist.title/body/confirm`；頁首 `[data-pd-unlist]` 與 `#pd-listed-toggle` 切關（capture 階段 `stopImmediatePropagation`）都走它；封存彈窗改單純確認；`PRUNE` 加 `[data-variant-listed],[data-variant-lock]`，`prune()` 另把所有項目都藏起來的 `.variant-table__row details.dropdown` 一併 `hidden`（解除封存時還原）；選片器建立時 `readonly: product.archived`、`applyArchived()` 呼叫 `window.pdFilmPicker.setReadonly(archived)`；`ZtorArchivedGate.install(main, { block: PRUNE＋選項編輯器與庫存表的按鈕, allow: 重新上架／預覽／[data-rowdis] })`。**組合細節頁** `bundle-detail.html`：`askUnlist()`（單純確認）、`tryRelist()`（`LS.bundleRelistPlan(bundleModel, id => store.get(id))`：blocked → 擋下列成員、`noCancel`；unlisted → 確認「一併重新上架」後 `LS.relistBundle()`、連帶成員 `store.commit`、`paint()`＋`syncMemberBadges()`；否則直接）；頁首「下架」／「重新上架」與 `#bd-listed` 兩個方向（capture 攔下）分別走這兩支；`syncReason()` 判 `type==='member-unlisted'`，`auto` 時用 `bd.unlist.reason-auto`；成員列徽章由新函式 `syncMemberBadges()` 填 `[data-bd-member-badge]`（已封存／已下架）；成員已下架（非封存）時可售套數說明用 `bd.member.unlisted`；`askConfirm` 補 `items`／`okKey`／`noCancel`，彈窗 markup 補 `[data-bd-confirm-list]`、主鈕文字改成 `<span data-i18n>`；`mirror()` 同步 `store.commit(seed)`；選片器 readonly 與閘門同商品頁。**拍賣細節頁** `auction-detail.html`：`askUnlist()`（Live／已下架／已封存直接 return；其餘 `e-shop.unlist.*` 確認）；頁首鈕與 `#ad-listed-toggle` 切關走它；封存確認與重新上架加 `commit()`；裝閘門（預設清單）。**元件** `partials/film-picker.js`：`createFilmPicker(host, { readonly })` → `{ getSelected, setReadonly, isReadonly }`；唯讀時 host `data-readonly`、搜尋格 `hidden`＋`disabled`、建議列容器 `hidden`、chip 不帶 `--removable`、click handler 直接 return。`partials/archived-gate.js`（新）：`ZtorArchivedGate.install(root, { block, allow })`，預設擋 `input/textarea/select/[contenteditable]/.switch/.segmented__btn/.radio-list__item/.radio-card/.zcheck/.chip__remove/[data-fp-add]/[data-fp-remove]/.tag-input__entry/.upload-tile/[data-upload]/[data-linkf-reset]`、預設放行 `a/summary/[role=tab]/.tabs__item/.section-nav__item/[data-tab-jump]/[data-rowdis]/.leave-dialog/[data-archived-allow]`；監聽 click／keydown（Enter・Space）／beforeinput／input／change 的 capture 階段；判斷即時（每次事件重讀 `data-archived`）。**通知** `js/sidebar.js` `NOTIF_INFO` 加 `{ icon:'calendar-clock', href:'bundle-detail.html?id=backstage-set', k:'notif.auto-unlist' }`（靜態）。**Cheat code** `js/devtools.js` Reset 呼叫 `ProductsStore.forgetSession()`。**DS** `design-system.html`：Film picker 卡補唯讀示範（`[data-ds-filmpicker-ro]`）與 API；新增 §4.202 Archived gate 卡（`[data-ds-gate]` 帶 `data-archived`、示範開關／欄位／選片器被擋、「重新上架」放行並切換）＋TOC＋總表列；Leave dialog 卡第三張示範改成「下架時列組合包」、新增「重新上架：這些已下架的成員會一起重新上架」與「有成員已封存」兩張；`design-system.md` Film picker／Leave dialog 條目更新、新增 Archived gate 列；`design-components.html` 重生。i18n 新增與墓碑見 `UI-CHANGES.md`（四十）。呈現假設與缺口見 `ASSUMPTIONS.md` UIA-154。

> **2026-09-18 · 拍賣結標未出價即流標（D287，與 D285／D284／D286 同一批拍賣三開關落地）**：`js/listing-state.js` 拍賣段新增 `auctionBidCount(a)`（讀 `a.bids`／`a.bidCount`，缺值或非正數當 0）；`AUCTION_STATUS_META` 加 `unsold: { i18n: 'e-shop.astatus.unsold', tone: 'neutral' }`；`deriveAuctionStatus()`／`deriveAuctionFlags()` 在算出 `ended` 之後再判一次出價數，無出價（`auctionBidCount(a) <= 0`）改回傳 `unsold`，有出價維持 `ended`——`ended` 的「完售」語意（D286 定案）不變、不重新命名，流標是同層新增的桶。`js/products-store.js` 的 `AUCTION_SEED` 新增流標示範 `tour-enamel-pin`（結標、`bids:0`／`bidders:0`）。`e-shop.html`：`STATUS_SETS.auctions` 篩選集在 `ended` 之後加 `['unsold', 'e-shop.astatus.unsold']`；新增流標示範列（`data-status="unsold"`、`data-auction-id="tour-enamel-pin"`），kebab 只有「編輯」（無履約可追蹤）。`auction-detail.html`：右欄「競標狀態」卡新增 `[data-ad-unsold-row]`（預設 `hidden`），`renderBadges()` 依 `flags.status === 'unsold'` 切換顯示「流標 · 無人出價」；徽章沿用既有 `AUCTION_STATUS_META` 推導，無需額外分支。`js/i18n.js` 新增 `e-shop.astatus.unsold`、`e-shop.a7.name/meta/bid/activity`、`ad.info.result`、`ad.result.unsold`。未達保留價、流標能否重新開拍：待確認，不做 UI，見 `ASSUMPTIONS.md` UIA-153。
>
> **2026-09-18 · 拍賣三開關追加三條裁決（D286，取代／補述 D285／D284 的三處呈現）**：`auction-detail.html` 頁首「下架」在 Live（競標中）時改**停用**（disabled，非隱藏）而不是先出確認彈窗——不論有無出價都不可下架；鈕外層包一個 `[data-ad-unlist-tip]` span 帶 title 提示「競標結束後才能下架」（`ad.unlist.live-disabled`），因為 `.btn:disabled` 的 `pointer-events:none` 會讓 hover 落不到按鈕本身，title 要放在有 pointer-events 的外層才會觸發原生 tooltip。Upcoming／Ended 才可按下架、按下直接執行（不確認，與商品一致）；原本 Live 專屬的下架確認彈窗整組撤除，`ad.unlist.live-title`／`live-body`／`confirm` 立墓碑。`js/i18n.js` 的 `e-shop.astatus.ended` 文案改「完售」／「Sold」（key 名沿用 `ended`，同時驅動 `auction-detail.html` 徽章、`e-shop.html` Auctions 分頁徽章與篩選 tab 三處）。`e-shop.html`：`canPin()` 排除 `archived`（原只排除 `draft`）；`syncRowActions()` 新增——已封存列的釘選 kebab 項收起，若封存當下原本已釘選則一併 `delete row.dataset.pinned` 並呼叫 `reflowPins()`；`updatePinChrome()` 的 `hasPinnable` 與「立即釘選」CTA 的可釘選列判斷改要求 `[data-eshop-pin]:not([hidden])`，避免已封存但 DOM 仍留著（隱藏）的釘選項被誤判成可釘選。一同下架的組合包「重新上架」不擋（原型現況已正確，未改程式）。呈現假設與缺口更新見 `ASSUMPTIONS.md` UIA-153／UIA-152。
>
> **2026-09-18 · 拍賣三開關與封存落地（D285／D284，主規格 §7.14「適用範圍」與「封存與不可刪除」；5.1.5.8 §2.2／§2.7／§4、5.1.5.10 共用操作、5.1.5 F3／F4 Auctions）**：**共用狀態層** `js/listing-state.js` 補拍賣段——`AUCTION_STATUS_META`（coming→`e-shop.astatus.upcoming`、live→`e-shop.astatus.live`、ended→`e-shop.astatus.ended`，其餘四態與商品同 key）、`auctionStart()`（saleStart＝開拍時間；沒填＝跟著上架＝listAt／listedAt）、`auctionSaleEnd()`（結標＝開拍＋`duration` 天，導出、不存）、`auctionStarted()`、`deriveAuctionStatus()`／`deriveAuctionFlags()`（走同一支 deriveStatus，qty 固定 ∞，soldout／low 不出現）、`auctionBadgeClass()`；單售／組合的推導一行未動。**資料** `js/products-store.js` 新增 `AUCTION_SEED`（兩 persona 共用；欄位名同單售：`listed／shown／listAt／unlistAt／saleStart／privateLink／archived`＋拍賣獨有 `duration`）與 `ProductsStore.getAuction／auctions／auctionStatusOf／auctionFlagsOf`；時間以「相對今天」的 offset 產生（Live／Upcoming／Ended 是時間推出來的，寫死日期會在某天全部變成已結標）；示範 8 筆：Live `stage-worn-jacket`、Upcoming `signed-tour-poster`、Ended＋Shipped `vintage-synth`、已下架 `lyric-sheet`、已封存 `tour-laminate`、隱藏＋競標中 `demo-cassette`，nick 的 `realive-tour-guitar`／`wyagl-nike-dunk`。**電子商店** `e-shop.html` Auctions 分頁：每列帶 `data-auction-id`，`rowModel` 多一支拍賣分支（`m.auction`），`renderStatus` 走拍賣推導（篩選桶 coming 寫成 `upcoming`，與 F6 即時橫條的 `data-status="live"` 判斷相容）、`paintBadges` 依列型別換對照表；篩選集改為全部／競標中／密封／即將開始／已結標／已隱藏／已下架／草稿／已封存（Sealed 篩選值原樣保留、無列落入）；顯示開關對拍賣列生效（已下架／已封存停用）；`syncRowActions` 對拍賣列生效（已下架→封存、已封存→重新上架＋編輯；複製連結在已下架與已封存都收起）；新增已下架／已封存／隱藏三列示範；Ended 列補「編輯」進詳情。**建立拍賣** `create-auction.html`：右欄舊單卡「上架設定 → 定時開拍」換成與建立商品同構的兩段——上架設定（`#ca-listing-mode` 不上架／立刻上架／定時上架＋顯示於商店 `#ca-shown-toggle`＋第 2 層上架時間／定時下架 `#ca-unlist-toggle`）與開拍設定（`#ca-start-mode` 跟著上架一起開拍／定時開拍＋第 2 層開拍時間、結標時間唯讀 `#ca-sale-end`＝`ListingState.auctionSaleEnd({saleStart, duration})`，時長讀 F4 `#ca-duration` 選中卡；沒有具體開拍時間時 placeholder 寫「上架後起算競標時長 · N 天」）；選「不上架」時第 2 層與整個開拍設定收起；編輯態 `#ca-delete` 與 `window.confirm` 撤除；補掛 `js/listing-state.js`。**拍賣細節頁** `auction-detail.html`：頁首徽章改 `[data-ad-badge-main]`（三處：頁首、拍賣資訊分頁、右欄狀態列）＋`[data-ad-badge-hidden]` 由 `deriveAuctionFlags` 重畫；頁首三顆互斥動作 `[data-ad-unlist]`（outline；競標中先確認、內文含「已出價者處理方式產品待確認」、主鈕 destructive；其餘狀態直接下架）／`[data-ad-archive]`（outline，確認）／`[data-ad-relist]`（primary，不確認）、無刪除；「複製連結」在下架後收起；右欄新增上架設定卡（上架開關升卡頭 `#ad-listed-toggle`、顯示於商店 `#ad-shown-toggle`＋非公開連結 `.linkf`、第 2 層上架時間／定時下架）與開拍設定卡（`#ad-start-mode` 二選一、開拍時間、時長唯讀、結標時間唯讀），下架時顯示列與開拍卡 `--locked`（D265）、競標已開始時開拍卡也鎖並露出 `ad.sale.started-lock`；封存態＝`main[data-archived]`＋`.info-banner`（`ad.archived.banner`）＋上架卡整組 `.lctl--locked`、上架開關 `aria-disabled`、鎖定說明換 `cp.listing.archived-lock`；拍賣資訊分頁「開始／結束」與右欄「結束」由 `auctionStart`／`auctionSaleEnd` 算；確認彈窗 `[data-ad-confirm-dialog]`（leave-dialog 殼）；補掛 listing-controls／switch／radio-list／link-field／info-banner／leave-dialog／input／date-input／control-row CSS 與 listing-state／link-field／radio-list／date-input JS。i18n 新增 `e-shop.a4/a5/a6.*`、`ad.archived.banner`、`ad.unlist.live-title/live-body/confirm`、`ad.sale.sec/started-lock/duration/days`、`ca.start.sec/follow/follow-sub/end/end-hint/end-follow`、`ca.listing.shown-hint`；`ca.start.schedule/-sub` 改文案；`e-shop.astatus.ended` 中文改「已結標」；墓碑 `ca.start.none/none-sub/now/now-sub/pending`、`ca.delete.confirm`、`cp.delete`。呈現假設與缺口見 ASSUMPTIONS UIA-153。
>
> **2026-09-18 · 封存與不可刪除落地（D284，主規格 §7.14「封存與不可刪除」；拍賣三頁本波不動、D285 下一波）**：**共用狀態層** `js/listing-state.js` 多一態 `archived`（`STATUS_META`／`STATUS_ORDER` 第一位，優先於其他所有徽章；`deriveStatus`／`deriveFlags` 先判它），`isUnlisted()` 對已封存一律回 true（封存必然下架），新增 helper `isArchived`／`canArchive`（已下架、非草稿、未封存；含排定上架未到與定時下架已過）／`archive`（archived＝true、總閘門關、上架與下架排程清空；顯示與開賣沿用）／`relist`（archived＝false、listed＝true、排程清空、`unlistReason` 清空，不經過已下架）／`unlist(entity, reason)`／`listedBundlesUsing(productId, bundles)`（仍在哪些「上架中」組合包裡；已下架／已封存／草稿的組合包不算）／`archiveWithBundles(product, bundles)`（先把那些組合包 `unlist` 並記 `unlistReason={type:'member-archived', productId, productName}`、再封存單售）。狀態轉移寫在 listing-state（不在 store）——它已經是「算出來的結果一律問這裡」的單一層，store 只補 persona 篩選（`ProductsStore.archiveBlockers(productId)`）。**資料** `js/products-store.js`：`seedListing()` 讀 `LISTING_SEED[id].archived`、封存時一併關總閘門與排程；`seedBundle()` 補 `archived`／`unlistReason` 預設；預設 persona 新增示範三單售（`postcard` 已封存＋有銷售、`mug` 已封存無銷售、`patch` 已下架仍在上架中組合包裡）與三組合（`launch-set` 已封存、`postcard-set` 因成員封存而下架＋`unlistReason`、`roadie-set` 上架中但成員 patch 已下架）。**電子商店** `e-shop.html`：三個分頁的狀態篩選各加「已封存」放最後，**All 不含已封存**（`applyFilter`／`updateStatusCounts` 都排除）；列徽章多「已封存」，已封存列的顯示開關比照已下架停用（title 換句）；kebab 依狀態互斥由 `syncRowActions()` 按需長出來（不寫進靜態列）——已下架列多「封存」、已封存列只剩「重新上架」＋「編輯」（複製連結與補貨收起）；草稿「刪除」改走確認；三種確認（封存／擋下／草稿刪除）共用一個新的 `[data-eshop-confirm-dialog]`（`.leave-dialog` 殼＋新元素 `.leave-dialog__list` 列受影響的組合包）、既有「顯示於商店」彈窗不動。**單售細節頁** `product-detail.html`：頁首 `.page-intro__actions` 三顆互斥動作 `[data-pd-unlist]`（outline，＝把上架開關切關、不確認）／`[data-pd-archive]`（outline，先確認；仍在上架中組合包裡改出擋下彈窗）／`[data-pd-relist]`（primary，不確認）；封存態＝`main[data-archived]`＋ `.info-banner` 說明條（icon archive）＋上架卡整組 `.lctl--locked`、上架開關 `aria-disabled`、鎖定說明換 `cp.listing.archived-lock`＋「編輯商品」／補貨／鎖定／到貨確認等會改資料的入口一律 `hidden`（`MutationObserver` 在庫存表每次重畫後再收一次）；補掛 `leave-dialog.css`。**組合細節頁** `bundle-detail.html`：同一套三顆動作與封存態（`bundleModel.archived`／`unlistReason` 從 seed 帶入，改動同步寫回 seed 讓清單看得到）；因成員封存而下架時 hero 下多一條 `.info-banner`＋上架卡鎖定說明多一句 `bd.unlist.reason`；成員列若已封存標 `badge` 「已封存」、可售套數那一句改成 `bd.member.archived`（組合不可售）；補掛 `info-banner.css`／`leave-dialog.css`。**建立商品** `create-product.html`：頂列 `#cp-delete-top` 與編輯態 `#cp-delete`＋`window.confirm` 撤除（發布過即不可刪除；草稿刪除入口在清單），`wiz.delete`／`cp.delete.confirm` 立墓碑（`cp.delete` 留給 create-auction）。i18n 新增 `shop.status.archived`、`e-shop.a.archive/relist`、`e-shop.archive.*`、`e-shop.delete.*`、`e-shop.shown.archived`、`product-detail.btn.unlist/archive/relist`、`product-detail.archived.banner`、`cp.listing.archived-lock`、`bd.unlist.reason`、`bd.member.archived`、示範三列的 `e-shop.rowPostcard/Mug/Patch.*`。呈現假設與缺口見 ASSUMPTIONS UIA-152。
>
...
> **2026-09-09 · D253「作廢全面退款」三批改動的工程決策**：平台不再提供任何退款動作，取消一律走 Admin 作廢（§2.6／§2.8／§7.2／§7.3）。**作廢確認彈窗第二個 consumer**——`event-detail.html` 的逐票／整場作廢沿用 `order-detail.html` 首個 consumer 立下的 §5.1.11 Destructive confirm pattern（`.payout-modal`／`.payout-dialog` 殼＋`.btn--destructive`，四段式內容：動到哪一項→平台外前置→會發生什麼→不可逆），零新 modal CSS；同輪補掛 `js/toast.js`（原本沒有，作廢後的完成回饋需要它）。**作廢可用範圍擴大後，訂單品項停用態由三種收成兩種**：`order-detail.html` 的作廢鈕只剩「非 Admin」與「已完成履約」兩種停用理由（原「待產品確認」一種隨出貨型／數位可作廢一併拿掉），可用條件涵蓋取貨型未核銷／出貨型未出貨／數位未交付。**數位品項的出貨狀態徽章改依 `delivery.on` 動態切換**：有交付日＝已完成／已交付終態，無交付日＝「尚未發行」（`badge--warning`），取代原本寫死「已完成」的做法；交付日欄位同步顯示「尚未發行」。**Earnings 的 Void reserve 改名**：F8 KPI 由「退款準備金（Refund reserve）」全站改名「沖銷準備金（Void reserve）」，出處由 D041 改記 D253，比例維持既有 0%；`event-detail.void.absorb` 吸收順序文案同輪補上這個新名（原殘留 Refund Reserve 舊字，2026-09-09 收尾複核時一併修掉）。呈現層留下的產品缺口見 ASSUMPTIONS UIA-145。
>
> **r2.3 版本血統（2026-08-28）**：本目錄由 `site/r2.2` 完整複製而來（排除 screenshots／scratch），是「ztorUI 換裝」的開發版本——把 `demo/ztorui-skin/` 沙盒收斂出的新視覺語彙（星空背景、玻璃殼、光語彙）熔接進 token／shared.css／ds-components 本體。遷移計劃與波次進度見 `../../r2.3-遷移計劃.md`。r2.2 同日起凍結為穩定版（只修 bug），協作與上線仍指向 r2.2。
>
> **2026-09-02（D238）· 創作者活動管理改自動匯入：表格變唯讀清單，新增「最後檢查 ＋ 重新檢查」**：使用者裁示「綁定以後，會自動匯入，實時檢查更新」。**撤除**手動匯入整組——`admin-creator-events.html` 的勾選欄、全選框與「匯入已選 N 場」主鈕拿掉（頁面因此不再引用 `checkbox.css`），頁面 `<style>` 的 `.ace-col-pick`／`.ace-row--locked` 一併退場留墓碑；「可匯入」這個**概念**退場（不是只藏起來——自動搬完之後沒有「還沒搬進來」的活動可以數），篩選由四個收成三個（全部／有待設定／未綁定）、活動狀態由三值收成兩值（待設定套組／已完成，「已匯入·」前綴同輪拿掉）、下段標題列摘要撤掉「可匯入 N」；`bookyayStats()` 的 `importable` 欄位零消費、就地移除留墓碑。**新增** F6「最後檢查 ＋ 重新檢查」放在選定者的標題列動作組（`roster-picker.css` 新增 `__head-sync` ＋ `__head-actions[hidden]`）：時間寫絕對值（`YYYY-MM-DD HH:MM`）、鈕是 `btn--outline btn--sm`、未綁定整組收起來（已綁定但無活動仍顯示——那正是最想按的時候）；原型行為＝忙碌態 900ms → 更新時間 → toast，**不假裝抓到新活動**，也**不落 localStorage**（那是平台側同步的時刻、不是 Admin 的編輯；理由記 ASSUMPTIONS EVIMP-003）。**元件層**：`button.css` 新增通用忙碌態 `[aria-busy="true"]`（圖示轉圈 ＋ `cursor: progress`，其餘維持原重量；刻意不等同 disabled，重複點擊由消費頁 JS 擋，遵守 `prefers-reduced-motion`），DS 頁補 default／busy 矩陣。**資料層**：Denise 的 `bookyayEvents` 擴成整個 `bookyayPool`（自動匯入的前提就是兩者一致）、`bookyaySetup` 補成 2 場（於是「已匯入 5 · 待設定 3」），新增 `lastCheckedAt`；`lastImportAt` 保留但不再有人寫。i18n 移除 9 把、新增 5 把（另 5 把改寫值不改鍵）。F5 未綁定空狀態補一句「綁定之後活動會自動搬進來並持續檢查更新」。三處數字同源與「繼續設定」深連結逐項實測不變。無新裸值（check_ds_sync 5/10/12 基準未動）；產品缺口記 ASSUMPTIONS PG-EVIMP-006。
>
> **2026-09-02（同日第四次調整，第三輪視覺迭代）· creator 卡片壓成兩列、數字改成小標籤**：使用者「那些數字太佔空間 改小標籤就好」＋「需要整張卡片UI一起調整」。卡由「身分兩層 ＋ 分隔線 ＋ 三個 20px 大數字」四層改成**兩列**：左邊 32px 識別圓（新增 `avatar.css` 的 `--sm`，字級 `--fs-13`），右邊上列名稱（15）、下列狀態列；上下內距 14→12，分隔線與 `__card-foot`／`__card-top` 一併退場（墓碑在 `roster-picker.css`）。**卡高 136px → 70px**（1440 寬、同一位 creator 實測），一排放得下的人數翻倍。數字改掛 `stat-row.css` 新增的 **`--inline`**（欄名與值同一行、值 12／欄名 11、`grid-auto-flow: column` 等分欄，兩欄三欄都對得齊、位數變多不推擠後面那一欄；欄名可截斷、值永不截斷）；同日上午的 `--compact` 走墓碑退場（改的是那個兩層結構本身，不是把 20 調小；留著會零消費、也讓下一個人不知道該用哪個），站上「卡內數字排」仍只有 `.stat-row` 一支（鐵律 11）。**狀態列是二選一**（使用者追加裁示「未綁定：不會有匯入與設定資料」「已綁定：有已匯入與待設定，可匯入不一定需要」）：未綁定＝「未綁定」徽章、**不印任何場次數字**（印三個 0 會讀成「查過了、結果是零」；徽章佔的正是數字的位置，就地說明空的原因，卡仍等高）；已綁定＝「已匯入」「待設定」兩個小標籤，**不掛「已綁定」徽章**（有數字就代表帳號通了；且 260px 的卡扣掉 58px 徽章只剩 119px 給兩欄，英文「Imported」單獨要 50px、會截成「Import…」）——綁定狀態改由「有沒有數字」與那顆例外徽章共同表達，**呈現決策記 `ASSUMPTIONS.md` EVIMP-002**。可匯入不上卡但值沒刪：下段標題列摘要（`ace.n.importable`）與「有待匯入」篩選（`ace.filter.importable`）算法未動、實測正確。`.stat__value--zero` 保留並在 `--inline` 底下連欄名一起退（已綁定但為 0 是真的查過的結果）。i18n：移除 `ace.lbl.importable`／`ace.linked` 兩把並留墓碑，本輪無新增鍵。行為完全不變（搜尋、四個篩選、`?creator=` 深連結、匯入即生效、toast、三處數字同源、「繼續設定」深連結，逐項實測）。比較頁 `docs/creator-card-lab.html` 四個變體全部換成小標籤方向（1 兩欄小標籤／2 三欄小標籤含可匯入／3 一行流水句／4 兩顆數字晶片），頁面標明編號換過、上一輪已退場；正式頁採 1 號。無新裸值（check_ds_sync 5/10/12 基準未動）。
>
> **2026-09-02（同日第三輪）· creator 卡片減成四個資訊、數字重新設計**：使用者裁示「卡片留這四個資訊就好，並重新設計卡片的 UI」——保留頭像、名稱、綁定狀態徽章、三個數字；撤除店鋪識別（`__card-meta`，仍可搜尋、不再顯示）與最後匯入時間（`__card-last`，上游同輪由 D235 從規格 5.1.0.6 F2 移除）。卡改成兩段：上段身分（頭像 ＋ 名稱 15 ＋ 綁定徽章**排在名稱底下**，徽章擺右邊會從固定卡寬切走一段、長名字先被截斷），下段一條 `--border` 細線後接三個數字。數字那一段**沿用既有的 `.stat-row`** ＋ 本輪新增的 `--compact` 密度變體（`stat-row.css`：三等分 grid、無欄間分隔線、數字 32→20、欄名 12→11、`margin-bottom` 歸零、`.stat` 改直排 flex 讓 `<span>` markup 也能堆疊），站上「卡內數字排」仍只有一支元件（鐵律 11）；同輪新增 `.stat__value--zero`（0 值退 `--faint-ink`）。字級階序＝數字 20 ＞ 名稱 15 ＞ 欄名 11。**刻意不畫分段進度條／圓餅**——三個數字不是同一個整體的三份（可匯入＋已匯入＝池總數，待設定是已匯入的子集）。已選態不動（`.selection-card--active` 的 1px 橘 outline ＋中性陰影）。`.roster-picker__card-side` 隨徽章換位一併退場，三支退場 class 全庫零殘留＋墓碑註解。i18n：新增 `ace.lbl.importable`／`.imported`／`.pending` 三把只有欄名的鍵（帶數字的 `ace.n.*` 留給下段標題列的摘要），移除 `ace.last`／`ace.last-never` 兩把並留墓碑。行為完全不變（搜尋、四個篩選、`?creator=` 深連結、匯入即生效、toast、三處數字同源、「繼續設定」深連結）。品味迭代照 `6-UI設計專案規則.md` 走比較頁：`docs/creator-card-lab.html` 四個變體（三格數字列／主數字領銜／緊湊列卡／數字優先磚）共用同一組含極端值的資料，正式頁採 1 號。無新裸值（check_ds_sync 5/10/12 基準未動）。
>
> **2026-09-02 · 活動匯入頁（5.1.0.6 · D233）＋兩支新元件**：新增 `admin-creator-events.html`——Admin Creator Studio **第六個**同層目的地（`sidebar.js` 的 `ADMIN_NAV`／`ADMIN_ROUTES` 由五項增為六項，icon `download`，排在 Creator Management 正下方），跨 creator 的 bookyay 匯入工作台。版面＝新元件 `roster-picker.css`（**同日改版後的現況**：上下堆疊，上段選人、下段看那一位；原本的兩欄殼 `master-detail.css` 已整支改寫並改名，見下一則）。上段組成：`field-pill` 搜尋 ＋ `filter-tabs` 四個帶數量的篩選 ＋ `roster-picker__grid` 的 creator 卡（`.selection-card` ＋ 新元件 `avatar.css` 的 `.ztor-avatar` ＋ `badge`）；下段＝`roster-picker__head`（標題／摘要數字／主鈕）＋ `admin-table-wrap--fluid` 包 `ztor-table`（`zcheck` 勾選欄、`badge` 狀態欄、列尾 `btn--outline btn--sm`）；六種狀態走 `empty-card`（載入／錯誤／無 creator 另以 `?state=` 演示）；匯入回饋走既有 `js/toast.js`。**新元件三件套齊**（CSS ＋ design-system.md §4.202／§4.203 ＋ design-system.html demo 卡與 TOC）。**既有元件兩項修正**：`checkbox.css` 補 `.zcheck__control{display:inline-block}` 與 `.zcheck__box{pointer-events:none}`（檔頭宣稱支援的「表格儲存格裡的裸 control」路徑其實是壞的：inline 元素不吃寬高、畫出來的方塊會把點擊接走）；`admin-ip-bank-table.css` 新增 `--fluid`（840px 最小寬收成 560px、左右儲存格內距 `--sp-20`→`--sp-12`，給坐在窄欄裡的表格用）。**撤除**：`creators.html` 名冊 ⋯ 選單的「Import events」與 `?tab=` 參數、`creator-detail.html` 的分頁列與整個 bookyay 面板（該頁六支 stylesheet 與九條頁面級排版一併退場，改留一行唯讀事實連到新頁）。**資料層**：`CREATORS` 補 `bookyayLinked`／`bookyayPool`／`bookyaySetup`／`lastImportAt`，新增共用算法 `window.ztorCreator.bookyayStats()`／`bookyaySetupDone()`（名冊徽章、新頁數字、詳情頁唯讀事實三處同源）。全部走既有 token、無新裸值（check_ds_sync 5/10/12 基準未動）；產品缺口記 ASSUMPTIONS EVIMP-001，元件重複記 STYLE-DECISIONS Q105。>
>
> **2026-09-02（同日）· 活動匯入改成「上面選人、下面看那一位」**：使用者裁示「creator 選擇在上、活動列表在下」「creator 改成一張一張卡片橫著排列」，純呈現層（上游 5.1.0.6「頁面佈局」本來就寫明主從兩欄非約束）。`ds-components/master-detail.css` **改寫並改名為 `ds-components/roster-picker.css`**（唯一消費者就是這一頁；上下堆疊之後「主從兩欄」這個名字與它的每一條寬度理由——340px、左欄吸附、左欄自捲——都不成立，改名而不是另建一支，站上不留零消費死檔）。上段＝`roster-picker__bar`（`field-pill` 搜尋 ＋ `filter-tabs` 四個篩選，可換行）＋ `roster-picker__grid`（`repeat(auto-fill, minmax(260px, 1fr))`，**auto-fill 不是 auto-fit**：auto-fit 會把空軌道收掉、三張卡撐成頁寬三分之一的橫幅；窄畫面自然掉成一欄）；每張卡同時掛 **`.selection-card`**，卡面／陰影／hover／**已選態（1px 橘 outline ＋中性陰影，2026-06-24／06-25 裁決）全部沿用那一支**，本元件不寫任何 `--active`。名稱與店鋪識別用自有的 `__card-name`／`__card-meta`（尺寸對齊 `.selection-card__title`／`__sub`，但不進 shared.css 的「標題一律大寫」名單——那兩行是資料不是標題）。下段＝`roster-picker__body` → `__detail`（`__head` ＋ 表格／空狀態），佔整個內容寬度；三層容器各自持有 `gap`（24／12／16），成員不自帶 margin（Q103）。**撤除**：窄畫面折疊列整組（`master-detail__bar`／`#ace-toggle`／`data-list="collapsed"`）——名單已在最上面、不擋路，選完又會自動捲到下段；連帶 3 把 i18n 鍵走墓碑（`ace.list.open`／`ace.list.close`／`ace.list.title`）。**新增行為**：點卡片後只在下段頂端不在視窗內時 `scrollIntoView({block:'start'})` 一次，尊重 `prefers-reduced-motion`。深連結／搜尋／篩選／匯入即生效／toast／localStorage／「繼續設定」深連結全部不變。無新裸值（check_ds_sync 5/10/12 基準未動）；STYLE-DECISIONS Q105 隨之標為已失效。
>
...
> **2026-06-29 更新**: 新增**平台營運（Admin）層**（spec §4.1／§3.2.1 Tier 0-1／D086）。`creators.html` 為 Tier 0 Admin 落地（creator 名冊＋建立，roster／data-list＋badge＋button 組成，建立用 inline card 表單）；現有頁＝Tier 1 creator scope。`sidebar.js` 依 `window.ztorCreator`（活躍 creator 存 localStorage `ztor.activeCreator`）渲染：roster 頁只露 Creator 管理 marker＋Tier 1 模組鎖定（`.app-topbar__link--locked`）；creator scope 在 logo 前加返回名冊 icon（`.app-topbar__back`，使用者裁示固定此位）＋「管理中 <creator>」chip（`.app-topbar__context`），導航解鎖。切換 creator 走 `devtools.js`「Creator · Admin」cheat code（呼叫 `window.ztorCreator.set`，派 `ztor:creator-changed` 重繪）。新增 icon arrow-left／shield-check。樣式住 shared.css（隨 nav shell），DS 頁 Header 段說明。**2026-09-08 更新**：`js/sidebar.js` 新增 `window.ztorCreator.adminScope()` 對外開放（等同既有 `isManagingCreator()`），供其他頁判斷「現在是不是代管態」；`devtools.js` 的 Cheat Code 對應拆出「Role · 身分」組，直接讀 `window.ztorCreator.list` 名冊渲染。**2026-09-08 二次更新（persona × role 矩陣，使用者裁決）**：Admin 層改吃兩個維度，2×3 就是全部組合。**Persona · 資料人格**（`ztor.persona`＝`default`／`nick`／`userB`）同時就是 Creator 名冊的三位本人——`sidebar.js` 的 `CREATORS` 三筆 `handle` 直接等於 persona id，名冊與工作區資料從此同一組人。**Role · 身分**（新 key `ztor.role`＝`general`｜`admin`）決定用什麼身分看：`window.ztorCreator` 新增 `role()`／`setRole()`，`get()` 回「role=admin 時的目前 persona 名冊項」，`adminScope()` 改答 `role==='admin'`（API 名稱與呼叫形狀不變）。舊的 `ztor.activeCreator`／`ztor.adminHandoff` 兩把 key 退役，`js/theme.js` 的 `seedPersonaAndRole()` 讀到舊值即翻譯成新模型後刪除；`?creator=` 仍支援（套成 persona＋role=admin 後從網址移除），`?from=` 留在網址上當返回目的地。**Admin 頁門禁**：`ADMIN_ROUTES` 的每一頁在 role=general 時，`sidebar.js` 的 `applyAdminGate()` 把主內容整段收起、注入一則無權限狀態（重用既有 `.empty-card`＋`.btn`，零新 CSS，文案 `admin.gate.*`），同時 `adminView()` 讓 Admin nav 與 Admin 標記整區不出現、導航退回一般創作者那一套。`admin-video-review.html` 原本頁面級的假開關「檢視身分」與 `#vr-noaccess` 一併退役，改讀共用身分。
>
...
索引     index.html              · Dashboard           · 完整 ✓
        ...
        e-shop.html             · 商品管理 Products / Bundles / Auctions · 互動 tab＋搜尋/狀態篩選＋顯示開關→隱藏（僅顯示軸，2026-09-03 D241 取代舊「Shop 開關→Hidden」）＋See as fan 分割預覽＋低庫存橫條＋補貨入口 ✓（spec 5.1.5 / 5.1.5.6）
        ...
        bundle-detail.html      · 組合商品細節頁         · 組合內容 + 銷售摘要 + 成員影響 + 三開關與鎖定庫存 ✓（spec 5.1.5.9 / §7.14 · D241；本表原缺此列，2026-09-03 補上）
        ...
        settings.html           · 設定 (8 section：個人檔案［帳戶與登入方式：暱稱（＝店面顯示名稱、唯讀）／真實姓名／Email／電話／密碼／第三方登入，D270］／外觀／語言／通知／隱私與安全／付款／整合／合規)    · 完整 ✓（spec 03 §5.1.9）
...
| Account menu | `.app-account__wrap / .app-account__panel` | Avatar 下拉 · Store settings / Settings / Payments / Logout（個人資料項 2026-09-14 依 D269 撤除；側欄帳戶子選單同步刪除該項，商店設定入口＝動作區既有獨立列）（Settings 入口在帳戶選單 ≠ 主導航，spec §3.2.2.5）· 同樣 `--background` bg |
...
| Data list | `.data-list / .data-list__row` | Row-divider 為主，沒有 card-per-row；`.data-list__end` 右欄堆疊金額 + 狀態 pill（Dashboard F3）。**可展開子列不在這支**：`__row--child` 已於 2026-09-03（Q107）退場，改疊 Row disclosure（下一列）|
| Row disclosure | `.rowdis__group`（`--gutter`／`--static`）／`.rowdis__frame`／`.rowdis__head`／`__label`／`__toggle`／`__child`（`--compact`）＋ `js/row-disclosure.js` | 2026-09-03 依 STYLE-DECISIONS Q107 新建，2026-09-04 改寫成 **L3「整組換層」**視覺模型（探索頁 `lab-row-disclosure.html`／`lab-row-layers.html` 逐輪否決到定案，見 STYLE-DECISIONS Q107 追加段）。站上「母列收成一列、點開才看到底下每一筆」的**單一答案**，取代 `product-list` 的 `__row--group`／`__row--child` 與 `data-list` 的 `__row--child`（兩份舊做法 2026-09-03 退場、CSS 原處留墓碑）；原本的縮排＋左緣線模型（`--rowdis-indent`／`--level-2`／`--level-3`／子列左緣 2px 線／組尾線）2026-09-04 隨 L3 定案一併退場、原處留墓碑。**只擁有揭露語意與階層視覺**（把手位置與轉向、`aria-expanded`、群組是否成面、母子列要不要畫線），欄位版面仍歸所在容器，所以表格型（`.product-list__row`）與清單型（`.data-list__row`）兩種骨架都能疊。**L3 模型三句話**：未展開整列與其他資料列同層（L1 卡面）；展開時母列與子項一起被包進 `.rowdis__group`（`--ztu-film` 底、inset 1px `--border-soft`、`--radius-lg`，成一塊 L2 薄膜面板，面板內部列底線轉透明）；子項欄位與表頭零縮排、嚴格對齊。`--gutter` 給清單型容器（本身無左右內距）用負外距＋等值內距把面板邊界往外推、欄位起點不動；`--static` 給唯讀群組（面板恆亮，不看任何把手狀態）。第二層（三層巢狀時的中間層）改用 `.rowdis__frame`（1px inset 框、不填色）——依據是 `ds-components/nest.css` 既有規則「只有兩層填色（L1／L2），第三層以後改邊框」（Q24），非新規則。互動走 document 級單一委派：把手 `data-rowdis="<gid>"`（必須是 `<button>`，`aria-expanded` 放在它身上）、子列 `data-rowdis-child="<gid>"`、母列選配 `data-rowdis-row`（整列可點）；API `init(root,{onToggle})`／`set`／`toggle`／`sync`／`isOpen`。消費頁：`pickup-detail.html`（買家分組，`.rowdis__group`）· `order-detail.html`（品項單位與組合成員，兩層，`.rowdis__group`＋`.rowdis__frame`）· `scanner.html`（F3 名單，唯讀，`.rowdis__group--static`，不載 JS）· `events.html`（活動系列場次，`.rowdis__group`）· `project-detail.html`（我的收益 › 計畫項目收益的收支小計，`.rowdis__group--gutter`，母列補 `rowdis__head` class，實測左緣 514px = 514px、金額右基準線 1476px = 1476px）。詳見 design-system.md §4.204 |
...
| Settings layout | `.settings-layout / .settings-nav / .settings-section / .settings-row` | 220 px sticky 左側 + 右側單一 active section；左側選項以 URL hash 切換，其他 section 不佔頁面高度。**2026-09-14 響應式**：<900px 單欄＋導覽橫捲（照 `.section-nav-layout`）；帶 `.settings-row__control` 的列上下堆疊、輸入框吃滿 |
...
| Listing controls | `ds-components/listing-controls.css` | 上架、顯示、開賣三開關卡（spec §7.14／D241，2026-09-03 施工中）：建立頁的上架列用既有 `radio-list` 三選一（草稿／立即上架／排程）＋下架日期與時間，細節頁三列皆改 `switch`；顯示列關閉時展開非公開連結（`ds-components/link-field.css`）；開賣列兩個選填 `datetime`，只校驗停售日期與時間須晚於開賣日期與時間。消費頁：create-product.html／product-detail.html／create-bundle.html／bundle-detail.html | **2026-09-04（D245）**：改成兩個區塊——上架設定（主決定＋第 2 層的上架與下架時間、顯示於商店、非公開連結）與開賣設定（跟著上架一起開賣／定時開賣二選一＋第 2 層的開賣與停售時間）；第 2 層為新增的 `.lctl__derived`，外觀套 `.card.card--muted`。細節頁的上架開關升到 `form-section__head--actions`。四頁（create-product／create-bundle／product-detail／bundle-detail）同輪落地；create-auction 仍是舊卡。 **2026-09-18（D284）**：細節頁封存態把整組 `.lctl` 上 `.lctl--locked`、上架開關 `aria-disabled`，`.lctl__lock-note` 換成 `cp.listing.archived-lock`（三開關與排程全部不可調、重新上架後才能調）；bundle-detail 另在 note 之後多一條 `[data-bd-unlist-reason]`（因成員封存而下架）。 **2026-09-18（D285）**：create-auction 換成同構的兩段（上架設定＋開拍設定；開拍設定第 2 層多一格唯讀「結標時間」＝開拍＋時長，沒有定時停售開關）、auction-detail 右欄同一套兩卡（開拍卡在競標已開始時整組 `--locked`＋`[data-ad-started-note]`）。消費頁補 create-auction.html／auction-detail.html，元件 CSS 未改。
| Stock allocation | `ds-components/stock-allocation.css` | 庫存池分配表（spec §7.14／D241，2026-09-03 施工中）：目前在庫 → 各管道鎖定量 → 沒有被鎖定的庫存量，含組合包視角變體（成員逐列沒有被鎖定的庫存量唯讀＋鎖定給本組合的量可填）。推導邏輯（沒有被鎖定的庫存量、各管道可售量、組合可售量取最小）集中在 `js/listing-state.js`。消費頁：product-detail.html／create-bundle.html／bundle-detail.html |
| Link field | `ds-components/link-field.css` | 唯讀連結欄＋複製鈕＋重置鈕（spec §7.14／D241，2026-09-03 施工中）：管道顯示時＝公開連結，切隱藏時系統產生獨立非公開連結，可重置（舊連結作廢、產新連結），切回顯示即作廢。消費頁：product-detail.html／create-bundle.html／bundle-detail.html（皆掛在 Listing controls 的顯示列旁）|
...
| 03 §5.1.9 Settings | settings.html | ✓ Full | Profile（2026-09-14 D269／D270：帳戶與登入方式——暱稱＝店面顯示名稱唯讀、真實姓名（KYC；驗證鈕開 `#realname-modal`＝英文姓名兩格唯讀彈窗）、Email、電話、密碼（自安全組移入）、第三方登入 Facebook／Google／Apple／LINE；頭像／名稱／簡介／使用者名稱／網站／所在地併入 store-settings 店面）/ Appearance / **Language（D220 新增、D221 一度拆成兩列、D222 撤除拆分：一列——預設語言 en/zh-Hant/zh-Hans/id 接 window.ztorLang，語言的唯一 API）** / Notifications (Email + Push) / Privacy & Security / Payments / Integrations |
...
## 4b. 商品詳情頁的型態對照表（2026-09-10）

> 這兩張表回答「同一個頁面會長成幾種樣子、哪些區塊在什麼條件下出現」。**規則本身不在這裡**——
> 分類看主規格 §7.1、狀態看 §7.2、庫存與鎖定看 §7.14、欄位定義看 5.1.5.1／5.1.5.9；
> 本節只是把散在那幾處的條件收成一張可對照的表，供實作與驗收使用。條件與規格衝突時以規格為準。

### 4b.1 單售商品詳情頁（`product-detail.html` · spec 5.1.5.1）

**四個型態軸**（互相獨立，相乘就是這一頁的所有樣子）：

| 軸 | 值 | 建立後可否改 | 影響 |
|---|---|---|---|
| 主分類 | 實體 Physical／數位 Digital | 不可改（D137） | 詳細規格、內容檔、庫存、取貨四塊的有無 |
| 選項模式 | 單一選項／多選項 | 不可改（D137，改了會弄壞既有訂單的 SKU） | 庫存表的列數、商品選項卡的有無 |
| 庫存版本 | 不限量／限量 | 不可改（對買家的限量承諾） | 上限欄與「/ N」分母 |
| 取貨方式 | 物流配送／現場 QR 領取 | 可改 | 交付與取貨分頁的欄位；**僅實體** |

> 數位商品一律不限量、沒有補貨、沒有取貨方式。另依 D251，數位的影視類與音樂類共 7 個次分類
> 不進電子商店，所以這一頁實際會遇到的數位次分類只有 **文檔、會員卡、IP 資產**。

**區塊出現條件**（2026-09-11 重組：分頁是「總覽」「商品庫存」「商品設定」「關聯」四個；商品設定＝建立商品的整頁，
攤在同一頁、上方一列分節錨點（`.list-toolbar` ＋ `[data-nav]`，同活動詳情），預設唯讀、右上「編輯商品」切成可改）：

| 分頁 | 區塊／分節 | 出現條件 |
|---|---|---|
| （頁首） | 頂列（麵包屑＋以粉絲身分預覽；捲過頁首後換成主圖縮圖＋商品名、黏在上緣）＋ hero（主圖／狀態徽章／名稱／價格 · 選項模式） | 一律；與項目／系列／活動詳情同一組 `.detail-topbar`＋`.pd-hero`（2026-09-11） |
| 總覽 | 銷售摘要（三個 KPI＋第四格「關聯中」：被幾個組合包／項目用到，點了切到關聯分頁） | 一律 |
| 商品庫存 | 四塊 KPI 磚（在庫／單售可售／鎖給組合／補貨中）→ 補貨中（計時補貨的單＋到貨確認，沒有就不出現）→ 庫存管理（逐選項組合表：組合＋SKU／在庫／鎖給單售／鎖給組合／未鎖定／補貨中／狀態；卡頭批次鎖定／批次補貨；列尾 ⋯＝上架中開關＋補貨＋鎖定庫存，照 DS §4.58 列操作選單）→ 庫存歷史紀錄（一張單一筆、點開看明細：逐組合數量／供應商／預計到貨／備註；最近四筆＋查看全部） | 分頁僅實體（2026-09-11 A 案，探索頁 `lab-stock-tab.html`）；**單一選項與多選項共用同一張表**，單一選項＝只有一列、用商品名（D258）。表格唯讀，補貨與鎖定各自在彈窗裡存；價格／SKU／成本在商品設定 |
| 總覽 | 商品設定概覽（每個設定區塊一行一句摘要，行尾箭頭跳到商品設定的那一節） | 一律；列數隨主分類／選項模式增減（內容檔僅數位、低庫存提醒僅實體） |
| 關聯 | 組合包（用到這件商品的組合包、各鎖走幾件，可點過去）→ 項目（把它當回饋或引用的項目，S24）→ 電影關聯（可改） | 一律（2026-09-11 使用者指示新增；原總覽「使用中」卡與商品庫存「庫存去向」併入此分頁） |
| 商品設定 · 商品資訊 | 商品素材（主圖＋附圖）・名稱／分類／描述・詳細規格 | 一律（詳細規格僅實體；數位素材是封面＋附圖） |
| 商品設定 · 內容檔案 | 數位內容檔 | 僅數位；介面再依次分類切四種——單檔上傳（文檔）／專輯曲目管理／會員卡卡面自訂／IP 素材槽 |
| 商品設定 · 規格與價格 | 定價（價格＋成本） | 僅單一選項 |
| 商品設定 · 規格與價格 | 商品選項（選項名與值的編輯器）・各選項組合價格與庫存（逐組合價格／SKU／成本，庫存唯讀） | 僅多選項 |
| 商品設定 · 折扣與限購 | 折扣設定／低庫存提醒／每人限購 | 一律（低庫存提醒僅實體） |
| 商品設定 · 交付與取貨 | 取貨與交付（分節名隨取貨方式：物流配送／現場 QR 領取／交付與存取） | 一律；實體再依物流／現場 QR 領取切換欄位 |
| 商品設定 · 標籤 | 商品標籤 | 一律（電影關聯移到「關聯」分頁） |
| 商品設定 · 上架與開賣 | 上架設定・開賣設定（三開關與排程）＋隱藏時的非公開連結 | 一律 |

**狀態**：三個獨立開關（上架／顯示／開賣，§7.14）＋由開關、庫存與時間推導的八個徽章（§7.2）——
草稿／已下架／已隱藏／販售結束／售罄／即將開賣／急需補貨／販售中。徽章不是自己設的，一律問
`js/listing-state.js`（`deriveFlags`／`STATUS_META`）。細節頁擺得下兩顆：主徽章＋「已隱藏」。

### 4b.2 組合包商品詳情頁（`bundle-detail.html` · spec 5.1.5.9）

**兩個型態軸**（比單售少兩個：組合本身沒有選項模式，也不分實體／數位——那是成員的屬性）：

| 軸 | 值 | 影響 |
|---|---|---|
| 庫存版本 | 不限量／限量 | 限量時多一列「組合上限（硬上限）」 |
| 成員型態 | 單一選項成員／多選項成員 | 多選項成員在分配表展開成逐選項組合、各自鎖定（D258） |

**區塊出現條件**（2026-09-11 對齊單售商品詳情：三個分頁——組合的成員與庫存版本跟建立流程一樣屬設定，使用者裁示不另開分頁）：

| 分頁 | 區塊／分節 | 出現條件 |
|---|---|---|
| （頁首） | 頂列（麵包屑＋以粉絲身分預覽；捲過頁首後換成封面縮圖＋名稱、黏在上緣）＋ hero（封面／狀態徽章／名稱／組合價 · N 件商品） | 一律；與商品詳情同一組 `.detail-topbar`＋`.pd-hero` |
| 總覽 | 銷售摘要（三個 KPI＋第四格「商品」＝幾件成員，點了前往組合設定的「商品」節）・組合設定概覽（每區一行一句現值，前往修改）・項目引用（S24） | 一律 |
| 組合庫存 | 四塊 KPI 磚（組合可售量／已鎖定成員／未鎖定成員／卡住的成員）→ 成員分配表（鎖定量直接在表裡改；限量時上限列唯讀顯示）→ 鎖定歷史紀錄（一次改動一筆、點開看逐成員前 → 後，同 stock-history） | 一律；成員是多選項商品時分配表展開成逐選項組合 |
| 組合設定 · 組合包資訊 | 秀出組合（素材）・組合包資訊（名稱／描述） | 一律；預設唯讀、「編輯組合」切換 |
| 組合設定 · 商品 | 成員清單（可點到商品詳情，各自未鎖定／鎖定量唯讀；編輯態可移除、底下搜尋候選加入，≥2 件） | 一律 |
| 組合設定 · 價格與折扣 | 組合價格（成員自動加總＋折扣 %）・庫存（不限量／限量＋組合上限）・折扣設定（排程特價） | 一律 |
| 組合設定 · 上架與開賣 | 上架設定・開賣設定 | 一律 |

**狀態**：與單售同一套八態，差別在推導來源——組合的可售量＝各成員在本組合的可售量取最小值
（多選項成員＝各選項組合加總），所以「售罄」多半是被某一個成員卡住；任一成員下架或未開賣，
整個組合不可售（§7.14／D087 語意由 D241 更新）。

---

## 4c. 展示素材槽：圖片或影片（2026-09-10 · D259）

一格素材可能是圖，也可能是片。工程上的三條約定：

- **型別分流在元件裡完成**：`partials/upload-tile.js` 依選到的檔案決定塞 `<img class="upload-tile__thumb">` 還是 `<video class="upload-tile__video">`，並在格上掛 `.is-video`／`.upload-tile--playable`。頁面不必自己判斷檔型。
- **要把素材畫到別處，一律問型別**：`create-product.html` 的 `tileMedia(tile)` 回 `{kind, src}`、`mediaTagHTML(m, cls)` 產出對應標籤。直接讀 `.upload-tile__thumb` 的 `src` 是禁止的寫法——影片沒有那個節點的 src，畫出來就是破圖，而且狀態仍是已填。
...
### 詳情頁頂列的黏附行為（2026-09-01 使用者裁示）

往下捲的順序固定三段，兩個消費頁（項目詳情、系列詳情）與兩種模式（整頁、detail-sheet 浮層）一致：

1. 麵包屑那一列直接被推掉，跟著內容捲走
2. 整個頁首（海報＋標題＋簡介）也捲掉
3. 這時才有一條精簡列（縮圖＋名稱＋頁面動作）從上緣滑進來

工程決策：

- 靜止態是一般流內元素，黏住態才切 `position: fixed`。**不能只用 sticky**——元素一旦捲出上緣，sticky 就不再作用，做不出「先捲走、之後再回來」。
...
- ~~E-Shop Auctions 建立流程 builder 待補（D026）~~ create-auction.html 已建（2026-06-18）；2026-09-18 起三開關與封存落地（D285／D284）（Bundles builder = create-bundle.html 已建 2026-06-12；Bundles 清單分頁已建 2026-06-14）
