...
> 2026-08-31 撤除：`home-canvas`（首頁展示版 Demo）。那一版的用途是「讓人看得到 home-canvas.html」，而該頁同日已進正式導覽（總覽下面一格「新版總覽」），改接規則就沒有存在的理由；留著反而會擋路——切版本用的回程守衛會把「停在別版目標頁」的人導回來源頁，正式進站的人一點新版總覽就被彈回總覽。`isFullBaseVersion()` 兩份白名單仍留著這個鍵，只為了讓還存著舊 devstate 的瀏覽器不會掉進「未知版本＝限縮導覽」。
...
**平台滿額折扣是全平台協定（2026-09-15 D273）**：Admin 平台折扣設定頁（`admin-platform-discounts.html`）與其他 Admin 目的地一樣不受開發版本 gate；創作者端沒有對應開關，只在訂單詳情看到「平台滿額折抵」扣項列（D274）。
...
| `S51` | 　商店優惠碼（清單／新增／編輯／期間／停用）           | Store discount codes                             | 🟢 Phase 1 | ✅ built  | D183；2026-09-15 D272 補範圍三級、每單件數／每人總次數／總兌換、已用完。自用碼各版本皆有；代理碼（推廣者／分成／代理銷售紀錄）只在 Phase 4，走保留 gate `full`（D185） |
