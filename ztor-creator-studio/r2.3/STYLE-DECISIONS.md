...
> **2026-09-09 D253 三批改動收尾複核**：eShop 訂單線／活動模組／Earnings 三批「退款全面改作廢沖銷」原型改動（D253）逐一核對站上既有視覺裁決——作廢確認彈窗在 `event-detail.html` 新增第二個 consumer，沿用 §5.1.11 Destructive confirm pattern 與既有 `.payout-modal`／`.payout-dialog` 殼層；帳務異動描邊徽章沿用 Q110 既有裁決（僅改名對象由「退款」變「作廢沖銷」，視覺規則不變）。三批沿用既有裁決、無新增待裁決條目。
...
|---|---|---|---|---|---|
| Q126 | **同一組選項列，單選與複選要不要長同一個樣子**：優惠碼「適用範圍」在 Phase 1 是兩列 radio-list（圓點＋標題＋副說明），Phase 4 卻是三個裸 `zcheck` 勾選框、沒有副說明——同一個欄位在兩個版本長成兩種東西 | **列的長相統一、記號分家**：兩邊都用 `radio-list` 的列（標題＋副說明、同樣的 hover 與節奏）；單選用基底的圓點，複選用新變體 `--check`（記號＝ `checkbox.css` 的 `.zcheck__control`，列本體改 `<label>`）。形狀就是「這裡能不能多選」的答案，不可互換 | 2026-09-23 | Phase 4 的三級是可混選（規格 D272：類型與商品可同時勾），不能改成單選；但那不是「長相要不一樣」的理由——差異只該落在記號上。使用者裁示「照推薦」 | 已執行：`radio-list.css --check` 變體＋ `store-settings.html` 適用範圍（Phase 4）；DS 頁 4.84 補 demo 與變體列 |
| Q125 | 表單分段的**區塊標題要由什麼換來**，以及彈窗裡那一疊區塊的**垂直節奏誰持有**：單張門票彈窗（D311）四個區塊全都掛正常那一階的段標題，其中「基本」與「入場」這一層沒有任何控制項；同時段內的距離是「標題 24／欄位 16／段上下各 20」湊出來的——標題離自己的內容（24）比欄位彼此（16）還遠，段內最後一個 `.field` 的 16 又疊進段內距讓段間實際變成 56。**2026-09-23 第二輪追問：那一疊區塊的分界要用什麼畫**——第一輪做的「32px 留白＋1px 淡分隔線」被使用者否掉 | **（一）標題分三階，由「區塊層有沒有自己的控制項」或「是不是必須連著讀的一組」決定**：有（購買條件的跟隨開關、價格與數量的算式列表＋bookyay 徽章、販售的暫停開關）→ 正常那一階；都沒有、但這一群還需要一個名字（入場）→ `.form-section__title--quiet` 輕標題（字級降一階、字重 regular、`--muted-foreground`、0.04em 字距，＝站上群組小字標既有配方 `.bd-sub__title`／`.bpc__title`／`.bd-tbl__gname`／`.dropdown__cap`，不另造第四種標題樣式）；都沒有且彈窗標題／副標已經是脈絡（基本）→ 整個標題列不畫，區塊容器留著撐段間節奏。**（二）彈窗內的節奏收成一把梯子，寫在 `.form-section--dense` 這一層**：標籤→控制 `--sp-6`／欄位→欄位 `--sp-16`／區塊標題→第一個內容 `--sp-12`／區塊→區塊 `--sp-32`／彈窗 body 內距 `--sp-24`。標題那一階刻意**小於**欄位間距，標題才會貼住自己統轄的那一組。欄位間距改由容器 `row-gap` 發（Q103），`.field` 的全站 `margin-bottom` 不動——作用域停在 `--dense`（只有兩個票券彈窗在用），不動全站表單 **（三）2026-09-23 同日修訂——分段改用卡片，分隔線退場（第一輪的「留白＋hairline」版本作廢）**：每個區塊是一張卡（新修飾類 `.form-section--card`，配方＝巢狀層 L2：`--layer-2-surface` ＋ `inset 0 0 0 1px var(--layer-2-line)` ＋ `--radius-xl` ＋ `border: 0`，不掛 `backdrop-filter`），卡內距 `var(--sp-20)`、**卡與卡之間 `gap: var(--sp-20)` 由容器（`.payout-dialog__body` 改 grid）持有**（Q103），`.form-section + .form-section` 的 hairline 在這個作用域被 `border: 0` 收掉。**理由＝站上既有做法**：「一個區塊到哪裡結束」在這個站的既有答案本來就是卡片而不是線——`ds-components/settings.css:60` 的 `.settings-section` 檔頭寫明「分區內放兩張以上的卡是常態，間距由容器的 gap 承擔」，所以彈窗照同一套走、不新增第三種分段語彙。內距選 20 不選頁面級卡的 `--card-pad`（24）：與卡間 gap 同值（卡內留白＝卡間留白，一把尺量到底），且 620px 的彈窗不再讓出 8px 寬度。兩張卡的內容對內容因此是 20＋20＋20＝60px（第一輪是 32）。**（四）卡內的算式列表去框**：`.form-section--card .calc-list { padding:0; border:0; border-radius:0 }`——外層邊界由卡承擔，卡中卡等於同一條線畫兩次；**列與列的分隔線與列內距保留**（那是算式單元自己的內部結構）。寫成情境規則而不是修飾類，卡片之外的消費者（create-bundle、bundle-detail、bundle-editor 定價段）一律不受影響。（一）（二）的標題三階與段內梯子（6／16／12）不動 | 2026-09-23 | 使用者實測後指出兩件事：區塊標題「四個都一樣重，但有些根本沒有東西要控制」；間距「標題離內容比欄位彼此還遠，而且段與段之間在 620px 的彈窗裡太鬆」。標題那一條沿用 Q123 的骨架（一段內最多三個文字層級、分辨靠色階與字距不靠再造字級）；節奏那一條是 Q103 在彈窗情境的落地——原本的 `.field` 自帶 margin 正是 Q103 點名的病。**第二輪（同日）**：使用者看過第一輪成品後說「用分隔線好像不是很明確」「間距還是太小」。1px hairline 在玻璃彈窗上本來就是站上最弱的分界訊號，而卡片是這個站已經在用、也已經被使用者接受的分段語彙（設定頁、建立流程的 outlined 卡）；改用卡片同時把「分界」與「間距」兩個抱怨一次解決——卡面自己就是邊界，留白從 32 自然長到 60 | ✅ 已執行 2026-09-23（第一輪：`form-section.css` `--dense` 改 grid＋row-gap／`__head` 負外距收緊／`> .field` 歸零／新增 `.form-section__title--quiet`；**第二輪同日修訂**：`form-section.css` 新增 `.form-section--card`（L2 薄膜＋`--radius-xl`＋內距 `--sp-20`＋`border:0`）、`--dense` 的首末段歸零內距改 `:not(--card)`；`payout-modal.css` `.payout-dialog__body:has(> .form-section--card)` 改 `grid`＋`gap: var(--sp-20)`＋`align-content:start`；`calc-list.css` 新增 `.form-section--card .calc-list` 去框；`create-event.html` `tierSecHTML()` 與 `event-detail.html` 四個區塊掛 `--card`；DS 三件套同步（`form-section.css`＋`design-system.md` Form section／Calc list 兩條＋`design-system.html` 示範改用真正的 `.payout-dialog__body` 容器與「為什麼是卡片不是分隔線」說明）。四情境 Playwright 實測：卡內距 20／卡間 gap 20／卡內容對內容 60／標題→內容 12／欄位→欄位 16／標籤→控制 6／`border-top-width` 0px；calc-list 在卡內 padding 0、border 0、列線 1px 保留；create-bundle 與 bundle-detail 的算式列表外觀未變（4px 14px／1px／16px）；console 0 錯誤；check_ds_sync 無新 FAIL、檢查 13 維持 3 處）<br>**（五）卡片圓角降一階＝`--radius-lg`（16）、不是 `--radius-xl`（24）**（2026-09-23 使用者裁示「改」）：這一階卡坐在圓角 24 的彈窗殼裡、又內縮 24，同心圓角的內圈必須比外圈小（外 24 − 內距 24 ⇒ 內圈理論值趨近 0，取現成的 16 最近），內外同角會鼓起來。token 檔本來就把 `--radius-lg` 定義成「卡內子卡、下拉面板」，`.form-section--outlined` 的巢狀規則（`form-section.css`）同一個理由、同一個做法。 |
| Q124 | 建立流程（wizard）的頂列與底列材質：頂列是頂到兩側的玻璃橫帶（只靠底線分界）、清單頁工作列是浮起的圓角膠囊——同是「頁面頂端的固定控制列」兩種做法；底列是另一塊顏色的玻璃帶，內容卡下緣圓角浮在它上面 | **頂列＝浮起的圓角磨砂膠囊，材質與 `.list-toolbar` 同一份**（玻璃底＋24px 模糊＋1px `--border`＋`--shadow-card`/`--shadow-edge-top`＋`--radius-xl`；淺色白底＋卡片陰影），膠囊外圍透明、內容捲上去從四周露出並在膠囊底下變模糊；頂列有第二排（分節分頁）時同一顆膠囊包住兩排。**底列＝跟內容卡同色同點陣的遮色片**，疊在延伸到視窗底的內容卡上，沒有邊線與玻璃，上緣漸層淡出。步驟列維持分段進度軌（分頁底線變體未採用） | 2026-09-23 | 使用者並排圈選建立活動頂列與活動清單工作列：「所有創建頁的 header 都要改成像工作列」；底列「變成一整條與背景相同顏色的色塊，不用特別有一塊色塊…只是一塊疊在上面的遮色片」；看過 `lab-wizard-chrome.html` 後補三條：「這裡要漸消」「這一塊不該黑底」「這一塊要半透明磨砂背景」；定案「將新版改上正式，並且保留進度軌」 | ✅ 已執行 2026-09-23（`shared.css` Wizard frame 段＋`partials/wizard-chrome.js` 底列高度；元件層一次生效、12 個 `.wizard` 消費頁 markup 不動；check_ds_sync PASS）|
| Q123 | 表單分段裡的文字層級該有幾層：段標題／段副標／小標／小標 hint／欄位標籤／表頭／表尾／hint／info 列現況共七到八層，字級卻只有三階（16／14／12），讀者分不出誰比誰重要 | **五條階層規則，適用範圍＝表單分段（`.form-section`／`.bd-sec` 這一類「一段問一件事」的區塊），不及於文章型頁面、儀表板與 marketing 版面。** ①一段內最多三個文字層級：段標題（16）→ 欄位標籤與小標（14）→ 內文（12：表格、hint、readout 說明同一級）；段副標與表尾摘要這兩層整個退場。②段副標一律移除——段標題已經是主詞，副標只能把它換句話說；真的非講不可的事貼到它實際作用的那一格去講。③hint 只寫「做決定才需要的資訊」（上限、單位、後果、留空會怎樣），每欄最多一句；把標籤換句話說、重述畫面已表達的操作、講內部公式一律刪。④同一事實只出現一次，放在最靠近操作的地方（例：「可售上限」只在「可售套數」講一次，鎖定套數與限量的 hint 不再各講一次；「粉絲實付」只在底部固定列講）。⑤規則優先用畫面表達、不用句子（剩餘不足一組的票種灰化＋徽章；「選填」由 placeholder 與空狀態承擔；「不用填」由「沒有輸入框」承擔）。**錯誤態不受②③限制**：超過上限這類句子仍要把數字說完整。**補句（2026-09-22 第二輪，使用者看線上第 6 步：「title 和區塊一樣沒有分清楚」）——小標與欄位標籤同在 14 這一階，但角色不同、樣式要分得開**：小標回答「這一群在講什麼」、標籤回答「這一格是什麼」；分辨不靠字級（仍是 14）也不靠字重（500 只留給 active／染色底微型字），用站上「群組小字標」既有配方——display 字型＋小型大寫＋0.04em 字距＋`--muted-foreground`（同 `.bpc__title`／`.bd-tbl__gname`／`.dropdown__cap`），並把小標與它統轄的欄位包成一個群組（沿用 `.bd-group`：群內 12，群與群之間一條 hairline、線上 20 線下 16；第一群緊接段標題不畫線），讓「小標包著底下這幾格」由距離說出來；同一件事的控制只給一格（多場活動的模式卡與適用場次表合成「場次」一格，不拆成兩格） | 2026-09-22 | 使用者看過 lab 頁 `docs/bundle-copy-hierarchy-lab-2026-09-22.html`（組合包三段 67 個文字元素逐一稽核、現況／方案 A 三層收斂／方案 B 行內合併三欄對照）後裁示「方案 A」，且明確不吸收 B 的兩處（建議名不降成 placeholder、每組張數不與小標合成一行）——A 骨架不動、只減元素，風險比 B 的行內合併低（B 的那一行同時是標題、輸入與說明，行動版折行後關係會散） | ✅ 已執行 2026-09-22（`js/bundle-editor.js` SPLIT 三段、`create-bundle.html`、`bundle-detail.html` 同用詞；`ds-components/bundle-editor.css` 新增 `.bd-tbl__row--short`／`.bd-disc-grid`；`js/i18n.js` 刪 18 把、縮短 10 把、新增 5 把、墓碑註解；DS html demo＋md 三條（bundle-editor／field-system／form-section）；截圖 `screenshots/2026-09-22-bundle-copy-A-*`；check_ds_sync PASS、檢查 12 計數未升）|
| Q119 | 兩段式編輯面板（`[data-mode="view"]`／`"edit"`）裡的**表單欄位**，檢視態怎麼呈現：disabled 外觀的輸入框 vs 純文字；清單列裡的步進器算哪邊 | **檢視態＝純文字、按「編輯」才變輸入框，與表格（Q118）同一原則：檢視就是看、不該長得像能填。** 取代 2026-07-27「回到編輯畫面的格式、只是不能編輯」與 2026-09-11「全站檢視態＝disabled 外觀」兩條舊裁示。做法：`js/view-mode.js`（元件層一次）在每個控件根後面寫一顆 `.field-readout[data-vm-readout]`，`shared.css` 檢視態藏控件、顯示讀數；文字／數字／網址／日期時間 → 值原樣（datetime-local 的 T 換空白）；金額 → 前綴單位＋千分位或該頁格式函式（product-detail 傳 `money()`）；步進器 `.zstep` → 數字＋單位（清單列裡的數量歸表單這邊，同樣純文字）；textarea → pre-wrap 段落；併排的長×寬×高 → `[data-vm-group]` 合成一句「21 × 15 × 2 cm」；空值「—」、placeholder 不當值、必填星號收起；控件 inline 的 flex／width 原封抄給讀數（角色／姓名列仍兩欄對齊——2026-07-27 擔心的塌陷由此解掉）。下拉（`.zselect__trigger`／`.select`）與標籤 chips 不換節點、去框去箭頭去 × 留字。**例外（狀態展示，維持原元件但不可操作）**：開關 `.switch`、單選卡 `.segmented.radio-cards`、單選清單 `.radio-list`、選擇卡 `.selection-card`、checkbox（`pointer-events:none`＋既有 `--locked`／`aria-disabled`）；`[data-edit-exempt]` 區段（方案編輯器、活動準備清單）自有編輯態不歸此管；圖片槽照既有規則只顯示圖 | 2026-09-21 | 使用者看 Q118 表格改純文字後裁定表單欄位一併跟進：「檢視就是看、不該長得像能填；按編輯才變輸入框」 | ✅ 已執行 2026-09-21（`js/view-mode.js` 新增；`shared.css` `[data-mode="view"]` 區塊重寫；`ds-components/store-settings.css` `.ss-url` 檢視態；五頁 `applyReadonly` 明呼 `ZtorViewMode.sync`、product-detail 掛 `money()`、`event-detail` 早到政策單選卡補 `data-view-safe`；DS html Field system「view mode / edit mode」可切換示範、md 條目；截圖 `screenshots/r2.3/viewmode-form-*`；check_ds_sync PASS）|
| Q118 | 兩段式編輯面板（`[data-mode="view"]`／`"edit"`）裡的**表格**，檢視態的格子怎麼呈現：唯讀輸入框 vs 純文字；編輯態下不可編輯的格怎麼對齊 | **檢視態＝純文字、編輯態才出現輸入框；編輯態的不可編輯格補輸入框同款左內距**。檢視態的價格／上限／SKU／成本一律畫成 DS 既有的唯讀讀數格 `.variant-cell--ro`（與庫存欄 `.variant-cell--stock`、庫存分頁那張表同族；金額走該頁唯讀金額的寫法、空值「—」），不再用 `readonly` 的 `.input`；按「編輯」才由 JS 換成 `.input`。編輯態下仍不可編輯的格（庫存讀數 `.variant-cell--stock`、任何 `.variant-cell--ro`）由 `.variant-table:has(.input)` 補 `--vt-input-pad-x`（＝輸入框的 9px 左內距，收成表格層變數）左內距，文字起點與相鄰輸入框裡的文字同一條線（實測差 0px）；表頭不動（表頭字對齊框邊、內容字對齊框內字，兩層各成一條線，與 create-product 現況一致）。元件層另加檢視態安全網：`[data-mode="view"] .variant-table .input`／`.amount-field` 也渲染成同一種純文字。同一原則套到補貨彈窗 `.restock-table__ro`（當前／補後兩格）：去邊線與指標、只剩置中數字。**範圍限表格**——兩段式面板裡的一般表單欄位仍照 2026-09-11「全站檢視態＝disabled 外觀」（shared.css `[data-mode="view"] .input`），要不要跟進見 Q119（**2026-09-21 同日裁決跟進**：表單欄位也改純文字） | 2026-09-21 | 使用者看商品細節頁「商品設定 → 逐選項組合定價表」檢視態：「表格中的欄位在現在的狀態下是不能編輯的，不能編輯就用（庫存欄）這種文字就好。輸入框在點擊編輯才出現。點擊編輯後才會有輸入框，不過 stock 欄位不能編輯，間距也要加上去，讓它跟其他欄位的位置有一致性。同時看看 DS 規則要不要調整、檢查其他元件有沒有同樣問題」 | ✅ 已執行 2026-09-21（`product-detail.html` `ecText()`＋`renderEditCombos(m, editable)` 依 editable 畫文字或輸入框、選項值改字重畫補傳 `true`；`ds-components/variant-builder.css` `--vt-input-pad-x`／`:has(.input)` 對齊／`[data-mode="view"]` 安全網；`ds-components/restock-modal.css` `.restock-table__ro`；DS html 補「檢視態／編輯態」demo、md 條目；截圖 `screenshots/r2.3/viewmode-01…09`；check_ds_sync PASS）|
| Q117 | 逐選項組合的縮圖（D268）在列出組合的表裡怎麼擺：獨立「圖片」欄 vs 併進規格名格 | **併進規格名（Variant）格，不另開「圖片」欄、表頭不出現「圖片」**：縮圖 `.variant-thumb`（`--own` 實線／`--ro` 唯讀）坐在 `.variant-cell--variant` 最前面、後接文字區 `.variant-cell__text`（規格名＋SKU 小字），flex 橫排、垂直置中、**間距 `--sp-12`**（使用者：8px 太擠）；縮圖自帶 34px 寬、`flex:none`，欄軌回到沒有縮圖欄的原狀。所有列出組合的表同一做法：建立商品 `#cp-var-table`、商品細節庫存管理表（`--stock`）／編輯彈窗組合表（`--preview`）／鎖定庫存彈窗（`.salloc__name--thumb`）／補貨矩陣（`restock-table__id` 內、`item.thumb`）、組合包建立與詳情的成員展開列（`.lockset__child .lockset__name`）。表格層的 `--img` 修飾類與所有「34px 開頭」的疊加欄軌（variant-builder 五組、lock-sets、stock-allocation、restock-modal）整組退場，`cp.var.col.img` 移除 | 2026-09-12 | 使用者看商品細節 Stock 表：表頭「Image Variant」擠在一起看起來沒對齊，裁決「縮圖併進 Variant 欄，像一般電商後台那樣」；追加「圖片的表頭不需要」「縮圖與文字間距要大一點」 | ✅ 已執行 2026-09-12（`ds-components/variant-builder.css` `.variant-cell--variant`／`.variant-thumb`；`stock-allocation.css` `.salloc__name--thumb`；`restock-modal.css` `__id` gap；`lock-sets.css` 子列 gap；`create-product.html:597`、`product-detail.html:224／594`、`partials/restock-modal.js`；DS html／md 四條同步；check_ds_sync PASS）|
| Q116 | ztorUI 換裝殘留稽核（`docs/ztorUI換裝稽核-2026-09-11.md`）C 表 23 題怎麼收 | **改 13、留 10**。改：`.amount-field__unit`（含 hover／readonly）→ 薄膜／玻璃 hover（金額欄的 $ 前綴不再是實色白塊）；`.combobox` 欄內已選 chip → 玻璃強階＋前景字（同 Q114）；`.radio-cards .segmented__btn`、`.check-card`、`.selection-card--icon` 三種可互動填色面 `--input-surface` → `--ztu-film`（「比卡亮一階」的意圖用薄膜疊在玻璃卡上仍成立）；`.radio-list--collapsible` 展開觸發 hover、`.alert--card[data-go]:hover`、`.sce__col-input:focus`、`.todo-list__input:focus`、`.vault-tile--label` chip → 玻璃。留：`.hero`／`.hero__slide`（Wave 3 C 類已裁「hero 保留」）、`.ticket-preview__tear`（撕孔錯覺技法）、`.canvas-home-shell` 窄版側欄實色（響應式攤平）、`.vault-gate__panel`／`__icon` 與 payout 彈窗內兩處（彈出層一律實色）、`.btn--destructive` 停用態與 `.linkf--disabled`（停用＝退到背景，Q58）、`.ztor-card--muted`（DS 展示用純色變體） | 2026-09-11 | 使用者：「全面檢查所有 r2.3 的元件，是否還有沒換成新的 DS 的」→「改吧」→「發一版以後繼續改完」。判準沿用 §4 定案語彙：表面一律中性玻璃，例外只有彈出層、停用態、裝飾錯覺與已裁決保留的 hero | ✅ 已執行 2026-09-11（A 表 115 處＋C 表 13 處；每處留舊值註解；check_ds_sync PASS）|
| Q115 | 詳情頁同時有黏住的頂列與吸頂的分節橫列時，畫面上該有幾條固定列 | **只准一條**：頂列（`.detail-topbar`）黏住後收編分頁的 `.list-toolbar`——橫列捲到頂列底下時 tabs 搬進頂列的 tabs 槽（起點對齊內容欄、左邊那段給縮圖＋名稱）、動作群搬進動作槽（放整頁動作的右邊）；原位留高不留內容，往回捲或切分頁還回去。黏住態的動作一律只留 icon（36px 方鈕、`--radius`；文字進 title），編輯中那顆例外要看得到「儲存變更」。標籤同步：「以粉絲身分預覽」→「預覽」、「編輯商品／組合／活動」→「編輯」（對象已在脈絡裡）。沒有頂列的頁維持原本的橫列 sticky | 2026-09-11 | 使用者圈選組合詳情設定分頁的兩層固定列：「如果有兩層固定的話，應該要合併成一個…考慮到其他詳情頁也有類似的狀況，應該怎麼改才有一致性」；看過 `lab-detail-dock.html` 後補兩條：「置頂的時候選單的位置要一樣，紅線左側的空間留給商品名稱」「只留 icon，用只有 icon 的元件，編輯放右邊」 | ✅ 已執行 2026-09-11（`js/detail-topbar.js` 收編邏輯、`shared.css` Dock 段；product-detail／bundle-detail／event-detail 三頁加槽位與 `data-dock-icon`；project／series 詳情沒有分節橫列、行為不變；check_ds_sync PASS）|
| Q114 | 標籤輸入欄（`.tag-input__field`）裡已選 chip 的對比 | **字改 `--foreground`、底維持玻璃、加 1px `--border` 髮線**（`tag-input.css` 覆寫 `.chip--active`）：`.chip--active` 的玻璃底（白 12%）坐在薄膜欄位（白 8.5%）上只差 3.5%，12px 的橘小字整顆糊成一片。欄位外建議列的 chip 不動（Q8／Q19 的橘＝已套用仍成立，只是欄內的值要先讀得到） | 2026-09-11 | 使用者圈選商品詳情的電影關聯欄：「這個都看不清楚，怎麼有這種元件？」。Q111 已把欄內 chip 定性為「已輸入的資料值」，資料值的第一要務是可讀 | ✅ 已執行 2026-09-11（`tag-input.css`；check_ds_sync PASS）|
| Q113 | 密集控件（`--control-h-xs`）的高度 | **28 → 32px，改在 token 一次生效**：`.variant-table .input`、`.vault-tile__rename .input`、`.btn--sm`、規格分配輸入框（lab-bundle-lock）全部跟著長 4px；仍是 4 的倍數，並與 `.btn--icon.btn--xs` 的 32 邊長對齊。`--rowdis-lead-w` 用的是 `--sp-28`、不動 | 2026-09-11 | 使用者圈選組合鎖定 demo 的規格輸入框：「這個小的 input 高度可以再大一點點，未來要從元件去調整，所有這種小的都要調大一點點」。Q25 訂的是「高度一律取 token、不用 padding 湊」，所以調高只改 token 一個數字、消費者不各自寫死 | ✅ 已執行 2026-09-11（`_tokens.css`；design-system.md／.html 控件尺寸表同步；check_ds_sync PASS）|
| Q112 | 卡片級大圓角（`--radius-xl`）外框裡的表格列，左右內距要多少 | **內距下限 `--sp-16`（列的左右）**：`.variant-table__head/__row`（12 → 16）、`.restock-table__head/__row`（14 → 16）、`.sce__head/__row`（12 → 16）、`.admin-table-wrap--fluid` 的 th/td（12 → 16）；`.ztor-table` 本來就是 20、`.radio-list--collapsible` 09-04 已改 16，不動。右側末欄若是 icon 按鈕（kebab／移除），按鈕自己的內距已把圖示推離圓弧，維持現狀。**同族但不是表格的圓角容器**（`.info-banner`／`.insight-row`／`.npp-product`／`.variant-option` 等 `--radius-xl` 容器內距 12）本輪不動、列為候選，等使用者看過再裁 | 2026-09-11 | 使用者圈選商品庫存分頁的庫存管理表：「這種大圓角的表格在左邊的 padding 要多一點，右邊的 icon 相較來說視覺上是 ok 的」。幾何原因：24px 的圓角在列高 50px 左右的表格裡，首欄文字離左緣 12px 時視覺上已經壓在弧線上；把「內距要跟圓角一起長」寫成通則，09-04 收合式 radio-list 那次就是同一個問題 | ✅ 已執行 2026-09-11（四支元件 CSS；design-system.md Variant builder／Restock modal／Size chart editor 條目補一句；check_ds_sync PASS）|
| Q111 | 標籤輸入欄（`.tag-input__field`）的表面，以及欄內已選 chip 的圓角要跟誰 | **欄位照 `.input`、欄內 chip 照欄位**：欄位底改 `--ztu-film` 薄膜、1px `--border` 髮線用 box-shadow 畫、圓角 `--radius`、focus 環同 `.input:focus`（舊值：實色 `--card` 底＋真 border＋`--radius-md`＋outline 焦點環——ztorUI Wave 2a 把控件底換成薄膜時漏了這一支）。欄內的 `.chip` 圓角改 `--radius`（與欄位同一級距），欄位外的建議列 `.chip-group` 維持 `--radius-pill`。Combobox 重用同一個欄位、一併生效 | 2026-09-11 | 使用者圈選商品詳情的電影關聯欄：「這個要照 DS，且圓角也和裡面的標籤不一致」。表面：它是輸入控件，沒有理由跟 `.input` 長得不一樣。圓角：Q1 裁的是「可篩選／可點＝全圓、純顯示＝小圓角」，坐在欄位裡的 chip 是**已輸入的資料值**（有 × 可移除、但不是拿來點選的候選），語意更靠近 `.field-pill`／`.chip--value` 那一側；同一格裡 10px 的框包 9999 的 pill，兩種圓角語彙打架。沒有選「欄位改 pill」——多行折行的欄位配全圓外框，第二行起的 chip 會貼到圓弧上 | ✅ 已執行 2026-09-11（`tag-input.css` 元件層一次生效；design-system.md／.html Tag input 條目同步；check_ds_sync PASS）|
| Q110 | 同一張表裡有兩族狀態時，狀態徽章的視覺答案 | **形狀分族、色相不再分家**：填色 tint 徽章（`.badge--orange`／`--success`／`--info`／`--error`）＝收入事件的狀態；描邊徽章（`.badge--outline`，失敗態 `--outline-danger`）＝帳務異動（提款／作廢沖銷／扣款）的狀態。不為第二族另發明一組色相，也不讓兩族共用同一組色票 | 2026-09-05 | 規格 D250 把收入事件與帳務異動放進同一張表，明訂兩族的狀態不共用色票——收入的「已撥款」與提款的「已付款」不是同一件事，掃描時要分得出來。可選的做法有三種：①兩族共用色票（規格禁止）；②給第二族一組新色相（站上狀態色只有 6 個語意 slot，硬塞第七種會把「顏色＝狀態嚴重度」這條線稀掉）；③形狀差一階。選 ③：描邊是既有語彙（`.chip--add`、`.metric-pill` 都是這條線），而且 1px 實線在任何一層 surface 上都成立，帳務異動列坐在展開後的 `--muted` 深底上仍讀得出來。規則一句話：**填色＝賺到的錢的狀態，描邊＝帳的搬動的狀態** | ✅ 已執行 2026-09-05（`badge.css` 新增兩個變體；`earnings.html` 交易明細 14 列套用；`design-system.md` 元件總表 Badge 列與 `design-system.html` Badge matrix 兩列同步；check_ds_sync PASS）|
...
| Q109 | 可展開列的控制項配置：展開 chevron 該站哪裡 | **展開在列的最前緣、`⋯` 在列的最末端，兩者分踞兩端、從不相鄰**（使用者指名參考 **Braintrust**）。列首固定一個窄欄 `.rowdis__lead`（寬 `--rowdis-lead-w` ＝ `--sp-28`，等於 `.btn--icon.btn--xs` 的邊長）：母列放 chevron、**子列放它在該組內的序號**（1 起算，`.rowdis__num`，純文字不可聚焦）、沒有群組的一般列留空。這一欄是欄位版面的一部分、不是「有分組才長出來」的東西，所以**表頭也多一欄**。適用範圍＝表格型消費頁（`pickup-detail.html` 名單十欄、`events.html` 系列九欄）；三個清單型消費頁維持把手現況（理由見右欄註） | 2026-09-04 | 原本 chevron 貼在標題文字後面（列的中段），與列尾的 `⋯` 讀不出「這是同一列的兩個控制項」，而且各組標題長度不同時把手的 x 位置跟著浮動，一整欄的把手站不成一條線。使用者看過 Mobbin 上八個 web 實例——Midday／Evernote／Xero／Salesforce／Braintrust／ClickUp／Asana／Zoho CRM，八個一致「展開在最前緣、`⋯` 在最末端」——後裁定採用，並指名 Braintrust 的細節（列首窄欄不空著、子列放序號、群組列其餘欄留空只給一個彙總、群組名稱字重較重）。清單型不套：`order-detail` 兩層展開時成員列同時是第一層的子列（該有序號）與第二層框的母列（該有把手），一個列首格放不下兩種角色；`project-detail` 的列首已是 ＋／−／＝ 記號晶片，把手貼著金額擺才讀得出「這個數字展開成它的組成」；`scanner` 名單是 `--static` 唯讀恆亮、沒有把手可搬。三者都不是欄格線結構、也沒有表頭可對齊 | ✅ 已執行 2026-09-04（`ds-components/row-disclosure.css` 新增 `:root --rowdis-lead-w`／`.rowdis__lead`／`.rowdis__num`＋≤760px 的堆疊版位置；`product-list.css` 的 `--roster` 欄軌 9→10；`events.html` 頁內 `--events` 欄軌 8→9＋窄框欄位順移；兩頁表頭與所有列補列首格、標題格移除 chevron 與 `.rowdis__label`；`.rowdis__label` 正式頁面零消費、CSS 規則保留供凍結的 `lab-row-hover.html`，註解標記待退場；`design-system.md` §4.204 新增「列首欄」段＋Anatomy 兩條，`design-system.html` 三張 demo 卡同步；欄位對齊逐欄實測表頭／母列／子列／一般列起點完全相同；check_ds_sync PASS）|
...
| Q27 | 編輯／新增流程用側邊 drawer 還是中央彈窗 | **中央彈窗**：`.payout-modal`／`.payout-dialog` 是站上編輯／新增流程的唯一殼層，側邊滑出的 `.drawer` 不再用於「編輯或新增一筆資料」。2026-07-24 把 project-detail 的五個編輯彈窗（專案基本資料／合作者／里程碑／專案更新／支持方案）由 drawer 改為中央彈窗。`.drawer` 保留給「不離開當前頁看詳情／歷史／說明」的唯讀用途（目前唯一消費者 earnings-sony 的提領歷史與如何運作） | 2026-07-24 | 使用者裁示；站上編輯流程原本就以中央彈窗為主（出貨／作廢／補貨／費率例外／請款），drawer 是少數例外，收斂成單一答案 | ✅ 已執行 2026-07-24（project-detail 五個彈窗；earnings-sony 的唯讀 drawer 未動）|
...
| Q77 | ztorUI 換裝：表面材質的統一語言——玻璃化之後，「哪裡透、哪裡不透、顏色可不可以出現在面上」需要一條全站規則，不能每個元件各自判斷 | **表面一律中性玻璃，顏色只出現在光與文字，不出現在面**：卡片家族（`.card`／`.kpi`／`.alert--bar`／`.readiness`／`.preview-card`／`.form-section--outlined`／`.list-toolbar`／`.app-topbar` 等）統一 `--ztu-glass-bg`（7% 白）＋`backdrop-filter: blur(var(--ztu-blur-glass))`＋1px `--border`＋`--shadow-card`/`--shadow-edge-top`；巢狀層（卡中卡、控件凹槽、縮圖框、中性徽章）統一 `--ztu-film`（6% 白）、不上模糊。任何需要色相的地方（品牌橘、狀態色）一律走文字色或發光，不把顏色鋪進面的底色——面永遠是中性白玻璃或薄膜，色彩由 Q80 的發光家族承載 | 2026-08-28 | 沙盒季逐輪確認：使用者反覆比對站上元件與換皮沙盒 `demo/ztorui-skin` 後裁示「表面不要染色，染色留給文字跟光」，避免玻璃卡疊玻璃卡時色相互相污染，也讓「這是面」與「這是選中」兩件事不會靠同一個視覺手法表達 | ✅ 已執行 2026-08-28（Wave 1–2a 熔接，`_tokens.css` 新增 `--ztu-glass-*`／`--ztu-film` 家族；`card.css`／`kpi.css`／`alert.css`／`readiness.css`／`preview-card.css`／`form-section.css`／`list-toolbar.css`／`header.css` 等同步；詳見 `docs/wave1-熔接紀錄.md`／`docs/wave2-熔接紀錄.md`） |
| Q78 | ztorUI 換裝：彈出層（dropdown／popover／dialog）要不要也跟著卡片一起玻璃化 | **彈出層一律全不透明 `--popover`，不做玻璃**：`.dropdown__menu`／`.zselect__panel`／`.preview-panel__sheet`／`.payout-dialog`／`.app-topbar__dropdown` 改 `background: var(--popover)`＋`backdrop-filter: none`＋`border: 1px solid var(--ztu-glass-rim)`。理由不是美學偏好而是可讀性：卡片已經是玻璃，浮層再疊一層透明就是「看穿兩張玻璃」，分不出誰在誰上面；讓浮層不透明才能穩定蓋住底下的玻璃卡與星空背景。原本「集中把 backdrop-filter 套進每個浮層」的全域治理段（`_tokens.css` 檔尾）整段退役，`.overlay-surface` 掛牌 class 全站零消費、移除後無孤兒 | 2026-08-28 | 沙盒季逐輪確認：Q77 定案「卡片玻璃化」之後，使用者在比對浮層時明確反向裁示「彈出的東西要看得清楚，不能也是玻璃」——這與換裝前「所有浮層都要毛玻璃」的舊規則（見 design-system.md 舊版 §3.2）方向完全相反，屬刻意反轉、非疏漏 | ✅ 已執行 2026-08-28（Wave 2a §3.6 熔接；`dropdown-menu.css`／`zselect.css`／`preview-panel.css`／`payout-modal.css` 同步；design-system.md §3.2 已改寫記錄新舊兩版規則） |
| Q79 | ztorUI 換裝：玻璃卡的 `backdrop-filter` 模糊半徑要抓多大——太大會糊到看不出材質、太小會沒有磨砂感 | **模糊半徑必須遠小於該層高度**：一般玻璃卡 `--ztu-blur-glass`＝24px；頂欄捲動加強態 `--ztu-blur-heavy`＝40px（頂欄夠高、扛得住更厚的模糊）；主要按鈕的霧光層 `::before` 內縮 3px 再模糊 5px（40px 高的按鈕扣掉內縮只剩 34px，5px 已是柔邊上限，超過會糊成一片看不出漸層）。此原則同時管轄「該用哪一階 blur token」與「新元件要不要另開新的模糊值」——高度夠高才配用重模糊，矮的控件一律走輕量的 `--ztu-blur-glass` 或更小 | 2026-08-28 | 沙盒季逐輪確認：熔接主要按鈕霧光層時實測踩到——`blur` 給太大整片糊成一團看不出漸層，使用者裁示「模糊要跟著容器高度走，不能全站一個值」，把判準訂成比例關係而非固定數字 | ✅ 已執行 2026-08-28（`button.css::before`／`header.css` `.app-topbar.is-scrolled`／各玻璃卡家族依此原則分階；`_tokens.css` 的 `--ztu-blur-glass`／`--ztu-blur-heavy`／`--ztu-blur-shell` 三階已按用途分工） |
| Q80 | ztorUI 換裝：選中態的視覺語彙——側欄／主分頁／filter-tabs／chip／radio card 五個「已選中」場景，要不要共用同一套畫法 | **依場景分五種語彙、共用同一組 `--ztu-accent-*` 原料，不強行統一成一種畫法**：<br>側欄選中＝accent 玻璃膠囊（與主按鈕共用 `--ztu-accent-*`，`::before` 霧光層）。<br>主分頁（`.tabs--underline-short`/`--underline-label`）選中＝發光燈條（五段長度漸層＋三層外光暈，比主按鈕安靜一階）。<br>filter-tabs 選中＝字自體發光＋殼底光暈（中性玻璃殼＋`--ztu-orange-hi` 文字＋雙層 `text-shadow`），不用色塊。<br>chip 選中＝中性玻璃＋橘字（`--ztu-glass-strong`＋頂緣內光＋`--ztu-orange-hi`），取代原本的橘 tint 色塊。<br>radio card 選中＝涼白玻璃、無左線（`--ztu-cool-white` 11% ＋雙層白內光，2026-08-28 裁決「不要線」，指示仍交給既有右上圓點）。<br>五種場景的共通點是「顏色只留在光與字」，不共用具體畫法是因為場景的資訊密度與強調層級不同 | 2026-08-28 | 沙盒季逐輪確認，逐場景比對後裁定；radio card「不要線」為同日獨立裁示，記於此併入本題 | ✅ 已執行 2026-08-28（Wave 2b 熔接；`shared.css`／`tabs.css`／`filter-tabs.css`／`chip.css`／`radio-card.css` 同步；`_tokens.css` 新增 `--ztu-rim-soft`／`--ztu-rim-strong`／`--ztu-cool-white`／`--ztu-pill-active-glow`） |
| Q81 | ztorUI 換裝：app shell 檯面要不要有裝飾層——換裝前是純色實面 | **星空背景規格定案**：CSS 純繪製（不吃圖片素材），四層 `radial-gradient` 疊圖磚（620／680／740／810px 四種尺寸，各自位置與 alpha 值），`position: fixed`／`z-index: -1` 鋪滿全螢幕；四組 `@keyframes ztu-star-a`~`-d`（7／11／9／13 秒，負延遲 0／−3／−5／−8 秒錯開）慢速呼吸，只動 `opacity`（合成器動畫）；`prefers-reduced-motion: reduce` 時凍結在 `opacity: 0.85`、不播放動畫；列表頁（`:has(.list-dock)`）四層一起套 `mask`，36%→90% 高度區間線性漸消，避免星空與下方密集表格互相干擾。精靈頁另有獨立的靜態點陣背景（22px 固定格距，非動態），與星空不是同一機制 | 2026-08-28 | 沙盒季逐輪確認，`demo/ztorui-skin` skin §6 的規格照抄，磚位與 alpha 一字不差 | ✅ 已執行 2026-08-28（Wave 1 熔接；`shared.css` 檔尾新段；design-system.md §1.9 已補完整敘述） |
| Q82 | ztorUI 換裝：KPI 數字要不要換成規格書 §1.9 定義的 `--font-dot`（Doto 點陣字面） | **不採用，KPI 數字維持 `--font-numeric`（Satoshi）**：ztorUI 規格書自己註明 `--font-dot` 為 Creator 家族專屬、ztor mode（本站）不使用；換皮沙盒 `demo/ztorui-skin` 全檔對 Doto 零引用，沙盒實測 `.kpi__value` 用的也是 `--font-numeric`。`@font-face`（Doto 600／700）與 `--font-dot` token 已備而可用（Wave 2b 補宣告），純粹「備而不用」——沒有消費端就不會觸發下載，成本為零；真要換是一次獨立設計裁決（規格要先改），不是本輪熔接的一部分 | 2026-08-28 | 沙盒季逐輪確認：任務書一度要求「KPI 用 Doto」，比對規格書與沙盒實況後撤回，記錄避免下一輪誤以為漏做 | ✅ 已執行 2026-08-28（`fonts.css` 補 `@font-face`＋`--font-dot` token；`.kpi__value` 未套用，`design-system.md` §1.1b／§1.2 已標「待採用」） |
| Q83 | Wave 3 C-1（前半）：`.kpi--hero` 實色橘卡與 Q77「表面中性玻璃、顏色只在光與文字」字面衝突，要不要玻璃化 | **維持實色橘，一個位元組不動**：`kpi.css` 內 2026-07 的舊裁示「實色橘代表主角／主要動作」明訂為 Q77 的具名例外——hero 卡是一列 KPI 裡唯一的主角，靠實色橘與其餘玻璃卡拉開層級；玻璃化後主角會跟其他卡片同材質，反而失去「這張是重點」的訊號 | 2026-08-28 | 使用者裁決：hero 卡的實色橘不是「表面被染色」的違規案例，是刻意保留的單一主角語彙，Q77 管的是「不要每張卡都染色」，不是「禁止有主角」 | ✅ 已執行 2026-08-28（`ds-components/kpi.css` 的 `.kpi.kpi--hero` 未改動，本條只補裁決記錄） |
| Q84 | Wave 3 C-1（後半）：`.info-banner` 實色 `--accent` 面與 Q77 衝突，且是 Wave 2 熔接漏掉的元件層缺口（`wave3-巡檢-batch3.md`），要不要玻璃化 | **玻璃化**：面改中性玻璃四件套（`--ztu-glass-bg` ＋ blur(`--ztu-blur-glass`) ＋ 1px `--border` ＋ `--shadow-edge-top` 頂緣內光），文字維持 `--foreground-muted`；原本靠實色橘面表達的「這是說明、要留意」改由 leading icon 承擔——icon 色換 `--ztu-orange-hi`、加一層柔和 `drop-shadow` 橘光（量級比照但收斂於 filter-tabs 選中字光：單層、半徑更小，因為 banner 是被動說明而非使用者選中的互動態） | 2026-08-28 | 使用者裁決：info-banner 是說明性質、不是主角，不得用實色橘表面（與 Q83 的 kpi--hero 恰成對比——後者裁決保留實色正是因為它是主角） | ✅ 已執行 2026-08-28（`ds-components/info-banner.css` 全站生效；25 個消費頁與 `design-system.html` 同步；`design-system.md` §4.7 已改寫） |
| Q85 | Wave 3 C-2：admin 表格密集頁（`.admin-table-wrap`）沒有掛 `.list-dock`，星空不會照 Q81 向下漸消 | **納入星空漸消慣例**：`shared.css` 的漸消 `:has()` 觸發條件從單一 `.list-dock` 擴到 `.list-dock, .admin-table-wrap`，同一條 mask 規則加 selector、不複製第二份。受影響：admin-ip-bank／admin-platform-fees／admin-video-review／ip-bank-reporting／store-settings（後者巢狀在同一支表格元件，一併漸消，實測合理，未另開分支） | 2026-08-28 | 使用者裁決：admin 表格密集頁與一般清單頁同屬「星空會干擾密集資料」的場景，沒有理由排除在漸消慣例之外 | ✅ 已執行 2026-08-28（`shared.css` 已改；五頁 localhost 目視驗證） |
| Q88 |（**部分被 Q96 取代**）儀表板 KPI 的成長 delta：膠囊該放數字下方（2026-08-19 現行）還是移回數字右側 | **移回數字右側同一行，常駐只顯示百分比，對照期 hover 才浮現**：`.kpi__value-row` 讓金額與膠囊 baseline 對齊；膠囊內拆兩段——`+12.6%` 常駐、`.kpi__delta-ctx`（vs last week／較上週）平時收起，hover／focus-within 時以不透明小浮層（`--popover`＋`--shadow-float`）浮現在膠囊正下方。**不用就地展開**：實測 span-3 卡內寬 208px、數字 115＋間距 8＋展開後膠囊 97＝220px，中英文都塞不下同一行，就地展開必然把膠囊擠到下一行、hover 時整顆跳位；絕對定位不參與版面計算，數字與膠囊永遠待在原處。收起時仍留在無障礙樹（不用 `display:none`／`visibility`），讀屏才唸得到「跟什麼比」 | 2026-08-28 | 使用者裁示。**這推翻 2026-08-19「delta 改回數字下方」那次裁決**——當時撤掉的 `.kpi--inline-delta` 是「膠囊與數字齊平、完整字串」的做法，行寬吃不消；本次成立的關鍵是把字串拆成「值」與「對照期」兩段，靜止時只佔百分比那幾個字 | ✅ 已執行 2026-08-28（`kpi.css` 新增 `.kpi__value-row`／`.kpi__delta-ctx`；`js/components.js` 的 `kpiTile` 改渲染；`js/i18n.js` 的 `ops.revenue-delta` 拆成值與 `-ctx` 兩個 key） |
| Q89 | KPI 卡的微型走勢圖：要放在哪些卡、線要不要跟著卡片主色染色 | **只給「量隨時間變化」的卡，且各卡同一個中性色**：儀表板四張 KPI 只有總收入與站上粉絲拿到 `.sparkline`——待處理事項與進行中項目是當下的計數，畫成折線會暗示一個它們沒有在講的趨勢。線色一律 `--muted-foreground`（`--spark-ink` 預設值），**不隨卡片主色走**：總收入卡雖然是橘數字，線仍中性——顏色留給數字與 delta 膠囊，替某一條線染橘會暗示那張卡比較重要。兩條軸各只標兩個刻度（最大／最小、起點／終點），刻度字串自帶單位 | 2026-08-28 | 使用者裁示「1 和 2 一樣的顏色」「1,2 要再更多細節，例如橫縱軸的單位」；同輪裁示總收入卡不需要「2 小時前更新」註腳——走勢圖已經把最近的變化講完 | ✅ 已執行 2026-08-28（新元件 `ds-components/sparkline.css`；`kpi.css` 掛貼底規則；`components.js` 渲染器支援刻度；design-system §4.134 已建） |
| Q90 | KPI 走勢圖的細節程度：只有一條折線（Q89 版）vs 參照圖那種帶格線、目標線、標記區間的完整微型圖表 | **照參照圖畫**：加水平虛線格線（畫在有帶值的縱軸刻度上）、目標值虛線＋圖上緣註記、標記區間（直立色塊＋區間內重畫的線段＋兩端圓點）；縱軸由 2 段增為 3～5 段、橫軸增為 5 段；資料密度由 12 點提高到 84 點（近 12 週每日）。繪圖區隨之由 30px 加高到 76px（`--detail`）——四五段刻度擠在 30px 裡會疊字，密集折線在太矮的區域也會糊成一條帶子。資訊層次由淡到實＝格線→目標線→折線→標記端點。值域改吃刻度值而非資料極值，否則資料一動格線就跟著飄 | 2026-08-28 | 使用者出示 Operational Efficiency 參照圖並指示「畫成這樣」 | ✅ 已執行 2026-08-28（`sparkline.css` 新增 grid／target／band／seg／dot／cap／`--detail`；`components.js` 的 `sparkPaths` 支援固定值域與區間工具；KPI 卡高 172→233px） |
| Q91 | 走勢圖的標記區間要標哪一段、要不要帶數值 | **只留一段、放在最後面、標上週增率**：撤掉中段的中性標記區間，只保留一段品牌橘區間涵蓋「上週到本週」（84 點日資料的最後 14 點），兩端各一顆橘點，上方以 `.sparkline__mark` 標出週增率（總收入 +12.6%，與卡片 delta 膠囊同一個數字；粉絲 +2.3%）。標記區間的用途因此從「這裡發生過什麼」收斂成「這段就是卡片上那個成長率」——圖與數字互相指認，而不是各講各的 | 2026-08-28 | 使用者圈出橘色區間指示「只要有這段在最後面，上週到這週的成長率標示」 | ✅ 已執行 2026-08-28（`sparkline.css` 新增 `__canvas`／`__mark`；`components.js` 算標籤位置；**尾段假資料重算**——原隨機尾段實際週增率是 −1.5%，與卡片標示的 +12.6% 矛盾，已重新產生讓兩者一致） |
| Q92 | 走勢圖標記區間的顏色與構件：品牌橘＋端點圓點＋上緣目標註記，是否都保留 | **改綠、撤點、撤註記**：① 標記區間的線段與標籤由品牌橘改成 `--up`／`--down` 語意色（漲＝`--status-success-ink`、跌＝`--destructive-ink`），與 delta 膠囊同一套——它標的是成長率、屬狀態，而橘色在本系統代表主角／主要動作，用在這裡會撞語彙。② 端點圓點撤除：區間的起訖已由色塊左右邊界講清楚。③ 圖上緣的「目標 $24k/wk」註記行撤除，目標只留一條無標籤虛線 | 2026-08-28 | 使用者三次連續裁示：「不用點」→「不需要」（指上緣註記）→「用綠色」 | ✅ 已執行 2026-08-28（`sparkline.css` 的 `__dot`／`__cap` 整組移除、`__seg`／`__mark` 改 `--up`／`--down`；`components.js` 對應改 `trend` 欄位） |
| Q93 | 收入走勢圖該畫什麼量：逐日收入 vs 滾動七日收入——使用者質疑「收入不太有負數，頂多退費，應該不太會向下」 | **畫滾動七日收入（每天回看過去七天的總額）**：卡片的值是單週 $24,830、目標是每週，畫逐日值的單位對不上（日收入 $24k 會讓週收入變 $175k）；滾動七日的單位就是週收入，終點正好等於卡片上的數字。回答使用者的疑問——這個指標是**單週流量**不是累計總額，所以淡的一週本來就會往下，向下是合理的；真正不對的是原本那種逐日劇烈抖動（照參照圖抄來的，但參照圖畫的是機場人流那種感測器數據），滾動加總本身會把單日波動抹平。同輪把終點與週增率都對齊卡片數字 | 2026-08-28 | 使用者提問「目前的收入走勢，不太有負數，頂多是退費，所以應該不太會有向下的狀況？」 | ✅ 已執行 2026-08-28（`components.js` 假資料重產為滾動七日序列，終點 24.83／wow +12.61%；粉絲卡同步 1,283／+2.31%；縱軸刻度改 $25k–$10k 四段、粉絲 1.3k–1.0k 四段） |
| Q94 | 走勢圖的時間窗口與縱軸刻度密度 | **窗口收成 9 週、縱軸收成 3 段**：資料由 84 點（12 週）改 63 點（9 週），縱軸由 4 段改 3 段（收入 $25k／$20k／$15k、粉絲 1.3k／1.2k／1.1k），橫軸相應為 9 週／6 週／3 週／本週。範圍收窄後折線佔滿的高度比例提高、刻度也不再擠 | 2026-08-28 | 使用者裁示「只算到 9 週就好」「用 3 個節點就好」 | ✅ 已執行 2026-08-28（`components.js` 假資料重產為 63 點，終點與週增率仍對齊卡片數字） |
| Q95 | 橫軸最後一個刻度（本週）沒有對齊折線終點 | **結構修正，非位移微調**：`__axis-x` 原本是 `.sparkline` 的直接子層，寬度含右側縱軸那一欄，最後一個刻度因此被推到縱軸標籤底下。新增 `.sparkline__main` 把畫布與橫軸包成同寬的一欄，縱軸改吃固定高度對齊畫布。實測本週刻度右緣與畫布右緣差距 0px | 2026-08-28 | 使用者框出折線右緣指「本週寫在這」 | ✅ 已執行 2026-08-28（`sparkline.css` 新增 `__main`、`__canvas` 改吃固定高度；`components.js` 對應調整 DOM 巢狀） |
| Q96 | 儀表板收入卡：走勢圖已標出 +12.6%，數字旁的 delta 膠囊是否還需要 | **撤除膠囊**：同一張卡上同一個數字出現兩次是重複，走勢圖的標記區間本來就把「上週到本週漲多少」講完了，膠囊留著只是把同一句話再說一遍。連帶 **Q88 的 hover 展開對照期機制（`.kpi__delta-ctx`）一併退場**——它是為這顆膠囊而生，全站再無消費端，不留死碼。`.kpi__delta` 元件本身保留：earnings 等 7 個靜態頁仍在用 | 2026-08-28 | 使用者裁示「這個就不用了」 | ✅ 已執行 2026-08-28（`components.js` 4-up 收入卡移除 delta 欄位、渲染器撤 ctx 分支；`kpi.css` 移除 `.kpi__delta-ctx` 整組規則；`i18n.js` 的 `ops.revenue-delta-ctx` 退場，`docs/` 舊版儀表板改用新增的 `ops.revenue-delta-full` 保住完整句） |
| Q86 | Wave 3 C-3：`creator-detail.html` 的 Account source／Shop and contact 兩段表單直接落在星空上，只靠分隔線分區，站上其餘表單頁都是玻璃卡 | **補玻璃卡**：兩段 `.form-section` 加 `--outlined` 修飾類，與全站表單頁一致；原本的 `padding-top:28px` 頁面級微調一併移除，改由玻璃卡自身的邊界與 24px 內距承接呼吸感。Import events（第二分頁）不在本次範圍，維持無卡 | 2026-08-28 | 使用者裁決：兩段表單與全站其餘表單頁的視覺角色相同，不該是唯二的例外 | ✅ 已執行 2026-08-28（`creator-detail.html` 兩處 `<div class="form-section">` 加 `form-section--outlined`；localhost `?handle=denise` 目視驗證） |
| Q87 | Wave 3 範圍裁決：C-4（fans-guide 編輯風說明頁）／C-5（funding-simulate 內部試算 demo）／C-8（section-test／design-layers／create-event-legacy 測試與 legacy 頁）要不要拉進 ztorUI 主語彙 | **維持現狀，不熔接**：`fans-guide.html` 是獨立編輯敘事版型（橘時間軸），自成一格；`funding-simulate.html` 頁面自標「demo，正式版不會出現」；`section-test.html`／`design-layers.html`／`create-event-legacy.html` 屬測試與 legacy 頁，非正式產品面。三類頁面的共通點是都不代表「使用者會看到的正式介面」，熔接成本不值得投入 | 2026-08-28 | 使用者裁決：把三個範圍裁決合併記一條，避免三個各自成獨立議題卻結論相同的裁決分散在檔案各處 | ✅ 已執行 2026-08-28（本條為範圍裁決，五頁均未改動；`docs/wave3-巡檢-總表.md` 已加註「不執行」） |
| Q97 | 詳情浮層（detail-sheet）的外框還是換裝前的樣子：不透明面板貼著視窗底邊、頭列鋪一層黑底 | **外框改吃 ztorUI 殼層材質，內容頁不動**：留白 `--ztu-shell-gutter` 四邊等寬（原貼底）、面板 `--ztu-glass-bg` 玻璃底＋四角 `--radius-shell`＋頂緣 `--ztu-glass-rim` 內光＋`--ztu-shadow-card`（原不透明 `--surface-shell`、只圓上兩角）、頭列改透明＋`--border-soft` 分隔線（材質由面板承擔，同 `.app-topbar`／`.wizard__top`）、控件 hover `--accent`→`--ztu-film`。**第二輪修正（同日使用者回饋「看起來是放一個新的背景，不是毛玻璃的 popup；header 也不是同一塊」）**：第一輪的兩個錯互相抵消——背板自己先糊了（面板沒有東西可以透過去）、iframe 又把檯面與星空整套再畫一次（完全不透），結果是一塊比較亮的方形貼在畫面上。改成**模糊只做在面板這一層**（`--ztu-blur-heavy` 40px），背板退成純暗紗（`--ztu-canvas` 80%、無 `backdrop-filter`），iframe 整片透明，並在 `shared.css` 內嵌模式關掉 `.main` 的不透明底、`<body>` 的檯面色與暖光、以及四層星空 pseudo-element。**80% 是實測值**：55% 底下封面照透上來搶讀、72% 詳情頁自己的半透明卡片裡會浮出鬼影、84% 玻璃感消失。**第三輪修正（同日使用者回饋「這一個不該有黑色背景」「整個頁面要往上，上方有一個很大的空白」）**：詳情頁的 sticky 頂列（`.pd-detail__topbar`／`.detail-topbar`，Wave 2b 漏掉的一支殼層頂列）靜止態去掉不透明 `--surface-page` 底、改成只在 `is-stuck` 時由 `::before` 長出玻璃＋`--border-soft` 分隔線；浮層裡的 `.page` 上內距與 `<body>` 上緣留白歸零。連帶修好一個既有 bug：`is-stuck` 判斷讀 `main.main` 的 scrollTop，而內嵌模式的捲動在 `<body>`，所以浮層裡永遠不會黏住（去掉底色後才露餡）。**第四輪（使用者「要背景模糊；不確定是不是有一個黑色 overlay」）**：前三輪的模糊其實從未生效——`.detail-sheet` 的入場淡入動的是 `opacity`，帶 opacity 的祖先會成為 **backdrop root**，底下那頁被排除在後代的 backdrop 之外，面板的 `backdrop-filter` 有宣告卻糊不到任何東西；把暗紗一路加濃到 80% 是在替沒生效的模糊止痛。改法：暗紗淡入改動 `background-color`（keyframes `detail-sheet-veil`），面板的 rise 也拿掉 `opacity` 只留 `transform`；暗紗回到 60%、濃度收成單一旋鈕 `--sheet-veil`。**紅線**：這一層永遠不可以動 `opacity`。**第五輪（使用者「陰影太重了」）**：站上既有的「浮層裡的卡片退薄膜、無陰影」規則（祖先清單本來就含 `.detail-sheet`）在 iframe 裡選不到，卡片帶著實色檯面用的重投影坐在玻璃上；把 `html[data-embed]` 併進 `card.css`／`kpi.css`／`alert.css`／`preview-card.css` 同一條規則的選擇器，不另立配方。**範圍是元件層一次生效**：十個清單頁共用同一支 CSS，所有 `*-detail` 浮層一起換。**明確不含**詳情頁本身的版面與元件（使用者裁示「只換彈窗外框」）。已知遺留：淺色模式頭列字看不清，與側欄同一筆換裝欠帳，非本輪新增 | 2026-08-31 | 使用者裁示「r2.3 的項目詳情 popup 也要換成新的樣式」，並在追問時選定「只換彈窗外框」＋「所有 `*-detail` 詳情頁一起」 | ✅ 已執行 2026-08-31，同日依回饋二修（`detail-sheet.css`＋`shared.css` 內嵌模式與 detail-topbar＋`project-detail.html`／`series-detail.html` 的 is-stuck scroller；dev server 實測 project-detail／event-detail 兩條路徑、computed 值逐項核對、黏住態實測 `blur(24px)`；check_ds_sync PASS）|
| Q99 | 主按鈕白字在橘玻璃上不夠站得住、hover 幾乎看不出來；外框鈕 hover 反而變暗 | **三件一起改**：(1) 主按鈕加 `text-shadow: var(--ztu-accent-ink-shadow)`（同日兩次：先 `0 1px 2px` 30% 黑，使用者看實機後說「再多一點」→ `0 1px 3px` 45%）；(2) 主按鈕 hover 由單一 `brightness(1.07)` 改成「同色薄洗（`::after` ＋ `--ztu-accent-wash-hover`）＋更外擴的 `--ztu-accent-glow-hover` ＋ `brightness(1.04)` 收尾」——半透明漸層整顆提亮 7% 在深色檯面上看不出來，色票仍不換；(3) `.btn--outline` 的 hover 由 `--accent`（深色＝`#2A2B2C` 實色深灰）改成新增的 `--control-raise-hover`（深色＝與靜止態同族的薄膜、濃度由 4% 提到 12%）＋ border 提到 `--ztu-glass-rim`。第三點是真 bug 不只是品味：靜止態是淺色薄膜、hover 換成實色深灰，等於從薄膜掉回一塊比周圍更暗的色塊，玻璃面愈亮愈明顯，**影響全站每一顆外框鈕**。同輪回退 Q98 的主按鈕透明度（50／40／40 → 回到 78／78／82），`--ztu-accent-fg` 維持 1 | 2026-08-31 | 使用者在浮層裡逐顆圈選裁示：「這個按鈕的文字加個陰影吧但不要太多，然後 hover 效果要在明顯一點」「這個 hover 要亮的，現在變暗了」「主按鈕剛剛調了一些透明度的參數，透明度的部分調回來吧」 | ✅ 已執行 2026-08-31（`button.css`＋`_tokens.css` 4 個新 token；DS 文件同步；computed 值逐項核對＋浮層內實際 hover 目視；check_ds_sync PASS）|
| Q98 | 浮層與主按鈕的材質參數：玻璃多白、薄膜多亮、巢狀層要不要畫線、暗紗要不要留、主按鈕要多實 | **使用者在調參器（`docs/sheet-button-tuner.html`）調定，整段落地**：`--ztu-glass-bg` 0.07→**0.05**、`--ztu-film` 0.06→**0.085**、`--nest-line` 0.10→**0**、`--border-soft` 0.10→**0.06**、`--ztu-blur-heavy` 40→**50px**、`--ztu-accent-surface` 色停濃度 78／78／82→**50／40／40**、`--ztu-accent-fg` 0.95→**1**、`--sheet-veil` 60%→**0%**。三條連動要一起記：(1) `--nest-line` 歸零＝把 Wave 2b「玻璃層次靠線、不靠薄膜」退回去，層次改由提亮的 `--ztu-film` 接手，兩者是一組；(2) 主按鈕變成透光的橘玻璃，**側欄已選中態共用同組 token 會一起變淡**（2026-08-28 刻意的共用）；(3) 暗紗歸零後浮層可讀性全靠 50px 模糊與 5% 白膜——之前把暗紗加濃是在模糊還沒生效的前提下比的，旋鈕留在 `.detail-sheet` 的 `--sheet-veil` 一行。這一題結掉 `r2.3-遷移計劃.md` 掛著的「主按鈕最終參數」未決項 | 2026-08-31 | 使用者在調參器調完後把輸出整段貼回；調參器本身是依 `Config/claude-ops/6-UI設計專案規則.md`「品味迭代用比較頁」條款開的（浮層視覺已來回五輪） | ✅ 已執行 2026-08-31（`_tokens.css` 7 個 token＋`detail-sheet.css` 暗紗；浮層 computed 值逐項核對、projects／e-shop 兩頁目視；check_ds_sync PASS）|
...
### Q122 · 「一頁多段」的分節導覽站上有兩種做法：橫向固定分頁（section-tabs，跳轉＋scrollspy）vs 左側 section-nav（切換面板）；分頁列併進頂列當第二排是否算違反 Q115「只准一條固定列」（2026-09-21 提出，暫依使用者指示做橫向固定分頁、待裁決）

使用者對「新增票務商品（活動票券組合包）」彈窗的反饋：「在創建的 popup 中有 fix 在最上面的 tab 可以快速滾動到指定區塊」。落地成新元件 `ds-components/section-tabs.css`＋`js/section-tabs.js`（§4.214），裝在建立活動第 6 步彈窗與電子商店建立組合頁。這一題有兩個層次：

- 現況 A（本輪新）：橫向一排 `.section-tabs` > `.tabs.tabs--underline-short.tabs--underline-label`，三段都在畫面上，點了平滑捲到段、捲動時 scrollspy 亮目前段（證據：`js/bundle-editor.js` `sectionTabsHTML()`；`create-bundle.html:109`）。
- 現況 B（既有）：詳情頁的左側 `section-nav`（Q75 分隔線式）——選了切換面板、一次只看一段（證據：`product-detail.html`、`bundle-detail.html:148`、`series-detail`、settings 共九頁）。
- 兩者的角色不完全相同：B 是「同一個東西的哪一段」切換視圖，A 是「長表單裡跳到哪一段」定位。但對使用者而言都是「一頁多段的導覽」，同一站兩種語彙要不要收斂（例如長表單也改左側 section-nav 不捲動、或詳情頁的設定分頁也改成橫向 scrollspy）是裁決題。
- 固定列數量：Q115 裁「只准一條固定列」（詳情頁頂列收編分節橫列）。本輪做法是**把分頁列併進既有頂列當第二排**——彈窗＝標題排＋分頁排（都在捲動區外）、建立流程＝`.wizard__top` 頂列＋分頁排（同一個 sticky 塊）——畫面上仍是一塊固定區、沒有第二條浮在內容上，本輪視為符合 Q115 精神；但頂列因此長高 44px（建立流程 80 → 124）。
- 選項：**A** 維持本輪（橫向分節分頁＝長表單專用、左側 section-nav＝詳情頁切換視圖，兩種語彙各司其職，寫進 §4.214／§4.22h0 的 Usage）；**B** 長表單也改左側 section-nav（彈窗要挪出一欄給導覽，1180px 特寬彈窗左欄會再縮）；**C** 分頁排改成捲動到頂列底下才滑入（像 detail-topbar 的收編），頂列靜止態維持 80px。
- 建議：A——長表單的分節本來就要一次看得到（邊填邊看預覽卡），切換視圖會把「粉絲看到的」對照關係切斷；C 多一個狀態換 44px 的靜止態高度，划不來。裁決權在使用者。
...
Q118 依使用者指示把**表格**的檢視態改成純文字，但同一個面板裡的**一般表單欄位**仍照 2026-09-11 的裁示（「全站檢視態都改成 disabled 外觀」，幾何不動、透明底、`--locked-field-ink` 淡墨），於是同一頁同一時刻並存兩種「不能改」的樣子：

- **現況 A（表格，Q118）**：檢視態純文字、無框；按「編輯」才出現輸入框。證據：`product-detail.html` 商品設定分頁「逐選項組合定價表」`[data-pd-edit-table]`（`ecText()`）；`ds-components/variant-builder.css` `[data-mode="view"] .variant-table` 段。
- **現況 B（表單欄位，2026-09-11）**：檢視態仍是輸入框、只換成 disabled 外觀。證據：`shared.css:1932` `[data-mode="view"] .input …`；`product-detail.html:593`／`:597`／`:684`（單一規格的價格／成本／上限 `.amount-field`）；`bundle-detail.html:401`／`:453`（折扣 %）與 `:782` 成員列的數量 `.zstep`（清單列裡的輸入框，最接近表格的情形）；`event-detail.html:846`／`:922`；`store-settings.html:212`；`project-detail.html` setup 面板。

兩者各有依據：B 是 2026-07-27「回到編輯畫面的格式，只是不能編輯」（攤平會讓角色／姓名兩欄失去對齊、textarea 塌成內容高度）＋ 2026-09-11 的 disabled 外觀裁示；A 是本次「不能編輯就用文字」。表格裡文字本來就有欄軌對齊、不會有 B 擔心的塌陷問題，所以 A 在表格成立；表單欄位若也改純文字，得先解決 2026-07-27 指出的對齊與節奏問題（例如標籤／值改成 `.data-list` 或 `.kv` 讀數列）。

**要裁決的**：(1) 表單欄位的檢視態要不要跟進純文字（若要，需一併設計欄位的讀數版型，不能只把框拿掉）；(2) 清單列裡的輸入框（組合細節成員列的數量 `.zstep`）算表格那邊還是表單那邊。裁決前維持現狀：表格＝A、表單＝B，不新增第三種。
...
D221 修訂 D220，帳戶選單（topbar 下拉、sidebar 子選單）新增語言列：常駐一列（標籤＋目前值＋
chevron），點了才展開四個語系選項。D222（同日）撤除 D221 引入的雙語言概念（不再有「顯示語言」
「預設語言」之分，語言收回單一概念），這一列因此簡化——原本標「預設」的徽章與「去設定改預設語言」
的提示行一併撤除。**D223（同日）再簡化收合列**：拿掉「預設語言」前置 label 與兩行 title/sub 結構，
只顯示目前語言名稱＋chevron，縮排字級對齊上方純文字列（Profile／Settings／Payments），補
`aria-label` 維持螢幕報讀語意——這只改收合列的**內容**，「收合式單選」這個**形狀**本身不受
影響，仍是站上第一次出現這種組合，本題待裁決範圍不變：

- **不是**卡式或分頁式 1-of-N（Q17 分工表的三支既有元件都假設常駐攤平，不假設「先收起來」）
- **不是** e-shop 分級選單的 `.dropdown__item--choice`／`.dropdown__menu--ladder`（那支殼服務的是完全不同的資料形狀——多層級分級，不是四個平行選項）
- 實作刻意沿用兩個選單各自既有的列 class（`.app-topbar__dropdown-option`／`.app-sidebar__sub-link`）疊加 `--toggle`／`--lang` 修飾子，不建立第三支獨立元件；展開態靠 `[hidden]` 切換＋chevron 旋轉，選中語意靠 `role="menuitemradio"`＋`aria-checked`＋check icon，證據：`js/sidebar.js` 的 `langMenuHtml()`／`langOptionsHtml()`、`ds-components/header.css`（`.app-topbar__dropdown-option--toggle`/`--lang` 一段）、`shared.css`（`.app-sidebar__sub-link--lang-toggle`/`--lang` 一段）

與 Q17／Q51／Q55 的既有 1-of-N 分工爭議交叉引用：Q17 是「卡式 1-of-N 該用哪一支卡」，Q51 是「清單列 vs 表單 select」，Q55 是「單選鈕兩套實作」——三者共同點是「同一個視覺角色隨版面／密度換做法」；本題是同一角色第四種做法（「收合式單選」），觸發條件是**空間極窄且選項數固定只有四個**（帳戶選單本來就窄，攤平四語系會撐爆，sidebar.js 已有 700px 視窗下側欄爆版的前例）。

選項：**A** 承認第四種形狀成立，寫進 Q17 分工表補一行「選單內窄欄＋固定少量選項＝收合式單選」；**B** 建成獨立元件（如 `.menu-select`），統一給未來類似情境重用；**C** 視為帳戶選單專屬的一次性寫法，不升成通用元件。暫依 **C**（目前只有這一個消費者，先不預先泛化），標 Q70；裁決權在使用者。
...
| 做法 | 寬度 | 比例 | 消費頁 | 證據 |
|---|---|---|---|---|
| `.pd-hero__cover` | 220px | 2 / 3 | `project-detail.html` | `shared.css:1432`、`:1441` |
| `.ip-hero__cover` | 200px | 3 / 4 | `ip-detail.html` | `shared.css:1304`、`:1306` |
| `.page-intro__media` | 96px（窄螢幕 64px） | 2 / 3 | `event-detail.html` | `ds-components/page-intro.css:34-46` |

**怎麼跑出三種的**：三者各自為當下的頁面需求調過一次，沒有人在同一輪裡看到另外兩個。`.pd-hero` 這一版在 2026-08-17 由 300px 收到 220px，是為了修 D193 搬走營運摘要之後右欄空掉一大半的破洞（見 ASSUMPTIONS HERO-001）——那次只解掉「破洞」，沒有解掉「站上有三種」。`.ip-hero` 的 3 / 4 也與全站統一的 `--img-portrait`（2 / 3，Q39 使用者裁決）不一致。

**本輪的處置**：只把 `.pd-hero` 調到「跟它自己右欄的內容高度對得上」，**沒有動另外兩支**，也沒有新增第四種。裁決權在使用者。

**選項**：
- **A** 三頁統一成一種尺寸（要挑一個，並接受活動頁的頁首變高、或項目頁的封面變小）。
- **B** 依「封面在這一頁有多重要」分兩級：作品類詳情（項目、IP）用大封面，活動類用小縮圖——那就要把兩級寫成 design system 的規則，而不是三個各自為政的數字。
- **C** 維持現狀，明白承認這是三個獨立版型。
...
- **往亮**：`--nest-surface` `rgba(222,223,233,.04)`（Q24 的 `.nest`）與 `design-layers.html` 的 `--layer-2-surface`（同值，照 Figma `856:27798` 定的）。模型是「疊在誰身上就跟著誰亮一階」。
...
證據：`ds-components/_tokens.css:607`（`--card`）、`:612`（`--muted` 註解「比卡深」）、`:643`（`--nest-surface`）、`:676`（`--surface-shell`）、`design-layers.html:33-37`、`ds-components/nest.css` 檔頭的兩層填色說明、`docs/bundle-popup-demo.html` 的 `.bdp-sec`、`docs/階層稽核-2026-08-13.md`。
...
**提案 demo：`design-layers.html`**（本輪產出，`--layer-*` 只在該頁作用域內，未進 `_tokens.css`，其他頁面零影響）。四題如下，裁完才動 foundation：
...
證據：`ds-components/_tokens.css:605-643`（現況 C 的 `--nest-surface` 與 `--card`／`--muted`／`--accent`／`--border`）、`ds-components/_tokens.css:657`（`--sidebar`）、`ds-components/_tokens.css:676-677`（`--surface-shell`／`--surface-page`，現況 A 的其餘兩階）、`ds-components/_tokens.css:744-746`（`--shadow-nest-up`）、`ds-components/nest.css:1-44`（現況 C 的元件與「只有兩層填色」的既有結論）、`design-system.md:596-608`（E0–E4）、`design-system.md:3975-3997`（§4.53 Nest）、`design-layers.html`（提案 demo）。
