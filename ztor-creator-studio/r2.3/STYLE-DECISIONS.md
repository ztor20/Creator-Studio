...
| Q87 | Wave 3 範圍裁決：C-4（fans-guide 編輯風說明頁）／C-5（funding-simulate 內部試算 demo）／C-8（section-test／demo-layer-system／create-event-legacy 測試與 legacy 頁）要不要拉進 ztorUI 主語彙 | **維持現狀，不熔接**：`fans-guide.html` 是獨立編輯敘事版型（橘時間軸），自成一格；`funding-simulate.html` 頁面自標「demo，正式版不會出現」；`section-test.html`／`demo-layer-system.html`／`create-event-legacy.html` 屬測試與 legacy 頁，非正式產品面。三類頁面的共通點是都不代表「使用者會看到的正式介面」，熔接成本不值得投入 | 2026-08-28 | 使用者裁決：把三個範圍裁決合併記一條，避免三個各自成獨立議題卻結論相同的裁決分散在檔案各處 | ✅ 已執行 2026-08-28（本條為範圍裁決，五頁均未改動；`docs/wave3-巡檢-總表.md` 已加註「不執行」） |
