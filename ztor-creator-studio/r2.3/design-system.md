...
| Data list | 🟠 organism | ✓ App | Recent earnings, transactions, payouts, products, projects (row-divider)。列圖示晶片 `__icon` 2026-07-20 三度修正後併入與 `.product-list__thumb` 同一組標準（52×52／`--muted` 底／1px `--border-soft`／icon 色 `--muted-foreground`），取消與 `.alert--card .alert__icon` 的尺寸家族關係（Q20）。註：`.project-list__icon` 曾同屬此家族，2026-07-24 project-list 改真圖縮圖後退出、不再是成員。**`.data-list__icon--sm`（2026-07-25 新增）**＝32px 小晶片（與同列 `.btn--icon.btn--sm` 等高）＋16px 圖示，用於「圖示是記號（＋／−／＝）而非內容縮圖」的列；52px 預設在那裡會讀成過大的框。**可展開子列已不歸本元件（2026-09-03／Q107，2026-09-04 改 L3 整組換層）**：`.data-list__row--child`（2026-07-25 建）連同它那顆縮排變數一起退場，CSS 原處留墓碑；清單型容器的可展開子列改在 `.data-list__row` 上疊 Row disclosure（§4.204）的 `.rowdis__child.rowdis__child--compact`，並在外層包一層 `.rowdis__group.rowdis__group--gutter`（階層改由整塊面板說，不再靠縮排）。本元件仍擁有列的版面（grid、列線、圖示晶片、金額欄）。**`.data-list__row--disabled`（2026-09-09 新增）**＝這一列還在清單裡、看得見，但不能被選；只淡化 `__row-main`（opacity .5），右側的狀態徽章與列上的連結維持原強度——不能被選的理由正是那顆徽章在說的話，連它一起調淡等於把理由藏起來。首個消費情境＝建立組合的候選商品清單（已下架的商品照列、標徽章、不可加入組合，但仍點得進商品詳情） | [data-list.css](./ds-components/data-list.css) |
...
**Variants** — `.restock-log--with-option`：多選項商品掛上它才開出「選項組合」欄，欄數由四欄變五欄；不掛時該欄連同表頭整欄不顯示。
`.restock-log--history`（2026-09-11，使用者裁示補貨紀錄＋鎖定紀錄合併成「庫存歷史紀錄」）：欄軌改成日期／類型／選項組合／變動／說明／狀態六欄，`min-width: 0`（頁內卡與彈窗共用）。變動欄補貨寫 `+20`、鎖定寫 `2 → 4`，鎖定那一格加 `.restock-log__qty--lock`（前景色，不走 __qty 的強調——鎖定不是增減）。消費者：product-detail.html 商品庫存分頁的頁內卡與「查看全部」彈窗。
...
- **H1 提亮一階** — hover 時底色在面板底 `--ztu-film` 之上再提亮一階（`--ztu-glass-strong`）。不浮起、不模糊、不加陰影、不改圓角——同一平面上的暫時強調，並明確中和容器 hover 帶來的 `position:relative`／`backdrop-filter`／`box-shadow`。
