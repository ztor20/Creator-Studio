# Ztor Creator Studio · R 2.2 — Design System
...
| Field | Value |
|---|---|
| `name` | Ztor Creator Studio · R 2.2 |
| `source` | Ztor Creator Studio — creator-economy operations dashboard (this project, R 2.2) |
| `version` | R 2.2 |
...
> **完整清單看 `design-tokens.html`（2026-09-14 新增）**：該頁執行期讀 `_tokens.css` 原文自動產生（亮色全部 token、暗色覆寫、每條上方註解、活的預覽）＋ `js/icons.js` registry；本節與 `design-system.html` Pillar 1 仍是人工策展的解說，兩者有出入時以 `_tokens.css` 為準。

### 1.1 Color

Ztor Creator Studio · R 2.2 runs on a **clean white canvas with neutral light-gray surfaces and a single high-saturation brand color** — highlighter orange `#ffa33f` plays the role of primary CTA and nothing else does. Status accents stay reserved for dashboard data dots.
...
### 1.1b ztorUI 換裝 · Glass & Glow 原料層（2026-08-28 熔接，Wave 1–2b）

**這是什麼** — `site/r2.3` 從換皮沙盒 `demo/ztorui-skin` 熔接進來的一組新視覺語言：深色近黑檯面上的白色低透明玻璃、暖橘霧光、發光選中態。原料值集中定義在 `ds-components/_tokens.css:608–674`（`--ztu-*` 家族，28 支）＋同段內不加前綴的 `--bloom-surface`（規格書 §3.8 指名的字面），另有 `--font-dot` 定義在 `fonts.css`。上層語義 token（`--primary`、`--border`、`--shadow-card`…）改成引用這批原料，元件層再引用語義 token——原則不變，只是原料換了一批新值。詳細熔接對照見 [`docs/wave1-熔接紀錄.md`](docs/wave1-熔接紀錄.md) 與 [`docs/wave2-熔接紀錄.md`](docs/wave2-熔接紀錄.md)。

**⚠ 深色語彙、light 模式尚未調校** — 這整組原料是為 `[data-theme="dark"]` 的近黑畫布調的（白色低透明玻璃、深投影、暖橘光）。多數消費它們的元件規則（`.card`、`.btn--primary`、`.chip` 等）**不分主題、寫在基礎選擇器上**，所以 light 模式目前也會吃到同一批值——例如白色調上疊 7% 白玻璃幾乎不可見。這是 Wave 1 就已知並記錄的現況（light 模式調校排在後續波次），文件如實記載、不代為修正。

**Foundation · Glass 三階**

| Token | Value | 用途 | 消費者（實測 var() 引用數） |
|---|---|---|---|
| `--ztu-glass-bg` | `rgba(255,255,255,0.07)` | 玻璃卡半透明底 | `.card`／`.kpi`／`.alert--bar`／`.readiness`／`.preview-card`／`.form-section--outlined`／`.list-toolbar`／`.app-topbar`／`.wizard__top::before`（頂列膠囊）／filter-tabs 已選殼 等（13 處；`.wizard__bottom` 2026-09-23 改同色遮色片後不再吃玻璃） |
| `--ztu-glass-strong` | `rgba(255,255,255,0.12)` | 玻璃 hover 加強 | 上述玻璃卡的浮層內排除補丁 hover、`.field-pill:hover`、`.chip:hover`、`.upload-tile:hover` 等（6 處） |
| `--ztu-glass-rim` | `rgba(255,255,255,0.15)` | 頂緣內光／浮層邊框色 | `.dropdown__menu`／`.zselect__panel`／`.preview-panel__sheet`／`.payout-dialog` 的 `border`、breadcrumb hover 圓鈕的 inset 邊 等（9 處） |
| `--ztu-film` | `rgba(255,255,255,0.06)` | 巢狀薄膜——卡中卡、控件凹槽、縮圖框、中性徽章共用的「疊上去的一層」，比玻璃低一階、刻意不帶 `backdrop-filter` | `.card--muted`／`.kpi.card--muted`／`.nest`／form-section 巢狀內層／`.input`／`.textarea`／`.select`／`.field-pill`／`.upload-tile`／`.badge`／`.stock-bar` 軌道／`.chip`／側欄項 hover 等（全站最多消費者，29 處） |

**Foundation · App shell 檯面與外殼**

**側欄收合成只有 icon（2026-08-31 建，全站）** — `.app.is-nav-rail`，鈕由 `sidebar.js` 產出所以每一頁的側欄都有，**與品牌同一行**（`.app-sidebar__top`）：它是這一欄的控制、不是導覽的一項，跟 logo 並排才讀得出「這顆管的是整條側欄」，另起一列會被當成清單的第一項。**圖示要先登記進 registry**——第一版用 `chevrons-left`／`panel-left-close`，那幾個名字不在 `js/icons.js` 裡，`applyIcons` 不認得就把 `<i>` 原封不動留著，按鈕因此是一個看不見的空方塊（實測 32×32、`renderedAsSvg: false`）；改用新登記的 `sidebar-collapse`／`sidebar-expand`（面板輪廓＋箭頭，不用單純的雙箭頭——單獨的箭頭在站上已經是「上一張／下一張」與「查看更多」）。`aria-label` 要**直接查表寫值**，不能靠 `applyI18n(btn)`：那支只掃傳進去那個元素的後代，標籤掛在按鈕自己身上永遠掃不到。**品牌區橫向置中要用 `align-items` 不是 `justify-content`**：它是直向 flex（標誌在上、Creator Studio 在下），`justify-content` 在直向 flex 裡管的是垂直——寫了會顯示成 center 但記號仍貼左（實測偏 12px＝記號的半寬）。**收起時品牌在最上、收合鈕緊接在它底下自成一列，下面一條分隔線**（2026-08-31 三次裁示：先「移到上面」、再「縮小後可以放在下面」、最後「這個 icon 要移上去，並在下面加一條分隔線」）：76px 的欄裡那一行只放得下一樣東西（記號 24 ＋間距 8 ＋鈕 32 ＝ 64，扣掉內距只剩約 60，並排會折行且貼左），所以鈕自成一列。**分隔線畫在導覽清單的上緣、不是畫在鈕底下**：鈕只有 32px 寬，線跟著它就只有 32px，看起來像鈕的一部分而不是一條分界；掛在清單上，線才吃滿整條欄的寬度（實測 48px＝欄寬扣內距）。線在做什麼：鈕獨自站在導覽上方時會像「第一個導覽項」，一條線把「這一欄的控制」與「要去哪一頁」分成兩段。做法是收合時把包裝層設成 `display: contents`，讓品牌與鈕直接成為側欄的 flex 子項，排序才跟得上 DOM（品牌 → 鈕 → 導覽）。**未讀點改貼 icon 的右上角**（只在收合狀態）：展開時標籤還在，點跟在標籤後面讀得出「這一項有未讀」；收起後標籤沒了，點就變成 icon 旁邊一顆浮著的橘圓、看不出它在講誰。展開的側欄是全站每一頁在用的，這條不動它。狀態存 `localStorage['ztor.navRail']`，跨頁維持同一個選擇——每點一次導覽就重新展開的話，那顆鈕等於沒用。欄寬 248 → 76px。**收起時只藏字不藏路**：每個 icon 留在原位（圖示與名稱的對應關係不必重學）、子選單整段收起（它要靠名稱才讀得懂）、品牌換成只有記號的 `SYMBOL_SVG`（使用者 2026-08-31 提供）——ztor 的字標在 76px 裡扣掉內距只剩約 48px，硬縮會被切成「zto」。**兩個選擇器的坑**：① 欄寬要寫 `html[data-nav-mode="sidebar"] .app.is-nav-rail #sidebar` 並改 `flex-basis`——真正的來源是 shared.css 帶 id 的 `flex: 0 0 248px`，純 class 壓不過（實測改了 `width` 欄寬仍是 248px），而 flex row 裡只改 width 也不決定尺寸；② 字標的 class 是 `app-topbar__brand-logo`（兩個外殼共用同一支標誌），寫成 `app-sidebar__brand-logo` 完全選不到。**900px 以下這一整套退場**，那裡本來就由 burger 接手——兩套收合機制同時在，讀者不知道哪一顆管哪一件事。

| Token | Value | 用途 | 消費者 |
|---|---|---|---|
| `--ztu-canvas` | `#0a0a09` | app shell 檯面色——比任何面板都深，圓角玻璃面板浮在其上 | `body` 的 `background-color`（sidebar 模式） |
| `--ztu-shell-gutter` | `14px` | 玻璃外殼與視窗邊之間的留白，四邊等寬 | `.app` 的 `gap`／`height`（扣兩倍留白）／`padding` |
| `--ztu-shell-solid` | `#1c1c1b` | 外殼玻璃疊在檯面上換算出的實色等價——給「必須是實色但要跟外殼同一階」的填色用（background-image 吃不到 backdrop-filter） | `.list-dock.is-snapped .list-dock__bars` 兩塊向外凹角的徑向漸層填色 |
| `--ztu-blur-shell` | `14px` | **待採用**——與 `--ztu-blur-glass`／`--ztu-blur-heavy` 同段定義，全站目前無 `var()` 引用 | — |

**Foundation · 星空背景** — 四層星空以 CSS 偽元素（`html::before`／`::after` 等）與 `radial-gradient` 疊圖繪製，不是 token 化的顏色值（磚位與 alpha 一字不差照抄沙盒），詳見下方「App shell 與星空背景」一節；`--ztu-bloom-*` 是與星空同色系但用途不同的霧光色票，見下表。

**Foundation · 模糊、陰影與發光**

| Token | Value | 用途 | 消費者 |
|---|---|---|---|
| `--ztu-blur-glass` | `24px` | 玻璃卡的 `backdrop-filter` 模糊半徑 | 卡片家族、頂欄、浮層排除補丁等（全站最多消費，32 處） |
| `--ztu-blur-heavy` | `40px` | 加強模糊——貼頂捲動態需要更厚的毛玻璃 | `.app-topbar.is-scrolled` |
| `--ztu-shadow-card` | `0 24px 60px rgba(0,0,0,0.45)` | 玻璃卡投影原料 | **待採用**——`--shadow-card` 語義 token 在 dark 已改成完全相同的值，但走的是**改寫語義 token 本身**、不是 `var(--ztu-shadow-card)` 引用（Wave 2a 刻意決定：寫語義 token 比寫死值多留一條 light 模式的活路），故本 token 定義後全站無 `var()` 引用 |
| `--ztu-shadow-pop` | `0 12px 40px rgba(0,0,0,0.5)` | 一般浮起投影 | `.switch` 開／關滑塊 |
| `--ztu-shadow-pop-up` | `0 -12px 40px rgba(0,0,0,0.5)` | `--ztu-shadow-pop` 的向上版——給浮在內容之上、光從上方來的元件 | `.sticky-actions`（底部吸附操作列） |
| `--ztu-rim-soft` | `rgba(255,255,255,0.14)` | 頂緣內光弱階 | `.chip--active` |
| `--ztu-rim-strong` | `rgba(255,255,255,0.20)` | 頂緣內光強階——卡面比 chip 大，光要撐得住 | `.radio-cards .segmented__btn--active` |
| `--ztu-cool-white` | `#eaf2ff` | 涼白——帶藍相的白，給「要比中性白選中態更冷一階」的面用 | `.radio-cards .segmented__btn--active` 的選中卡面 |
| `--ztu-bloom-1` | `#d0553d` | 霧光色票 1（暖紅端） | 側欄選中 `::before` 霧光層 |
| `--ztu-bloom-2` | `#e8894f` | 霧光色票 2 | **待採用**——只在 `--bloom-surface` 的字面漸層值裡以 hex 出現，全站無 `var(--ztu-bloom-2)` 引用 |
| `--ztu-bloom-3` | `#f0a85e` | 霧光色票 3 | **待採用**（同上） |
| `--ztu-bloom-4` | `#f5cd80` | 霧光色票 4（暖黃端） | **待採用**（同上） |

**Foundation · 發光家族**（主要動作、側欄選中、segmented、switch 共用一份配方，不各寫各的）

| Token | Value | 用途 | 消費者 |
|---|---|---|---|
| `--ztu-accent-surface` | 150° 橘漸層（`#ffc178`→`#ffa33f`→`#f2871f`，各自 color-mix 透明化） | 發光面的底 | `.btn--primary`／`.segmented__btn--active`／`.switch--on` 軌道 等（7 處） |
| `--ztu-accent-blur` | `blur(16px) saturate(1.25)` | 發光面的 backdrop 模糊 | 同上（10 處） |
| `--ztu-accent-glow` | 4 層 `box-shadow`（外光暈＋雙 inset 高光暗邊＋外緣描邊） | 主要動作的光暈——四層裡最亮的一階 | `.btn--primary`／`.switch--on` |
| `--ztu-pill-active-glow` | 同配方、光暈與內光都調低一階（4px→2px、30%→24%、55–60%→60–70%） | 「選中」比「主要動作」安靜一階 | `.segmented__btn--active` |
| `--ztu-accent-fg` | `rgba(255,255,255,0.95)` | 發光面上的字色／滑塊色 | `--primary-foreground`／`--on-primary`（語義別名引用，另有 sidebar 家族的同名待採用別名）＋ `.switch--on` 滑塊 等（8 處） |
| `--bloom-surface` | 176° 六階暖漸層（`#d0553d`→`#e8894f`→`#f0a85e`→`#f5cd80`） | 主要按鈕 `::before` 霧光層——規格書 §3.8 指名的字面，刻意不加 `--ztu-` 前綴 | `.btn--primary::before` 及其 `--lg`／`--sm` 尺寸變體（3 處） |

**Foundation · 品牌橘三階**

| Token | Value | 用途 | 消費者 |
|---|---|---|---|
| `--ztu-orange` | `#ffa33f` | 品牌橘原色——實色按鈕、選中態 | `--primary`／`--chart-1` 等語義 token 改引用它（24 處，另有 sidebar 家族的同名待採用別名一併改寫但未實際消費） |
| `--ztu-orange-hi` | `#ffc178` | 橘的亮版——漸層頂端、當文字用的 accent-ink | `--primary-hover`／`--brand-ink` 等語義 token 引用（12 處） |
| `--ztu-orange-lo` | `#f2871f` | 橘的暗版——漸層底端、內陰影折射色 | **待採用**——`--gradient-brand` 等仍寫死同值 `#f2871f`，未透過 `var(--ztu-orange-lo)` 引用 |

**Foundation · 拉丁字面** — `--font-dot: "Doto", 'Poppins', var(--font-numeric), monospace`（定義於 `fonts.css`）。**待採用**——ztorUI 規格書把它列為 Creator 家族專屬，ztor mode（本站）明確不使用；`@font-face` 與字檔已備（Wave 2b），純粹「備而可用」，目前零消費端，`.kpi__value` 仍走 `--font-numeric`（Satoshi）。詳見 §1.2 Typography 的字面堆疊更新。

### 1.2 Typography

> **ztorUI 換裝（2026-08-28 熔接，Wave 1）**：拉丁字面從 Satoshi／Geist／Inter 換成 **Poppins**，三個角色（display/ui/body）共用同一支——ztorUI 的語言不靠「換字面」區分標題與內文，靠字級與字重。中文維持 `--font-cjk`（昭源黑體 → Noto Sans TC）；**`--font-cjk-display`（LINE Seed TW）本波退出 `--font-display` 的堆疊**，中文標題與內文現在同一支昭源黑體，`--font-cjk-display` 這支 token 仍定義著、但目前全站無 `var()` 引用（待採用）。`:lang(zh-Hant)` 的三條覆寫也同步改成同一份 Poppins 堆疊（見 `shared.css`），繁中模式不再把中文字面提到第一位——中文字的 `unicode-range` 仍釘死在 CJK 區塊、逐字落在 `--font-cjk`，改變的只有行高／基線改由 Poppins 決定。`--font-mono` 未動。以下為熔接後的現況：
...
--font-cjk-display: 'LINE Seed TW', 'Chiron Hei HK', 'Noto Sans TC';   /* 待採用，見上方換裝說明 */
--font-cjk:         'Chiron Hei HK', 'Noto Sans TC';
```

- **Poppins** — primary Latin face for all three roles, 4 static weights self-hosted（300/regular/500/600；`ds-components/fonts.css:39–66`）. 幾何圓體，`--font-display`／`--font-ui`／`--font-body` 現在是同一支，不再靠換字面分標題與內文。
- **Satoshi** — 換裝前的主字面，現退為 `--font-numeric`（金額／KPI 數值等「要被讀成數量的字」）的字面，非 `--font-display`／`--font-ui`／`--font-body` 的備援鏈成員。
- **Geist / Inter** — 換裝前的拉丁備援，已從 `--font-display`／`--font-ui`／`--font-body` 的堆疊移除；`--font-mono`（Geist Mono）不受影響。
- **Chiron Hei HK** (昭源黑體) — CJK body/UI (`--font-cjk`)，換裝後同時承擔中文標題與內文，stays legible at small sizes.
- **LINE Seed TW** — 待採用（`--font-cjk-display`）：換裝前的 CJK 標題字面，本波退出 `--font-display` 堆疊，目前無消費端，token 仍保留備查。
- **Noto Sans TC** — final CJK fallback.
- **Doto** — 待採用（`--font-dot`，見 §1.1b）：ztorUI 規格書的點陣數字字面，Creator 家族專屬、ztor mode 不使用，`@font-face` 已備。

| Scale token | Family | Size | Weight | Line height (`--lh-*`) | Tracking |
|---|---|---|---|---|---|
| `display-64` | Poppins（原 Satoshi，2026-08-28 換裝） | `64px` | 300 | `none` 1.0 | `-1.28px` |
| `display-44` | Poppins | `44px` | 300 | `tight` 1.1 | `-1px` |
| `title-40` | Poppins | `40px` | 400 | `tight` 1.1 | `-0.8px` |
| `title-32` | Poppins | `32px` | 400 | `tight` 1.1 | `-0.6px` |
| `title-24` | Poppins | `24px` | 400 | `snug` 1.2 | `-0.48px` |
| `label-15` | Poppins | `15px` | 400 | `none` 1.0 | `-0.3px` |
| `label-14` | Poppins | `14px` | 400 | `snug` 1.2 | normal |
| `body-16` | Poppins | `16px` | 400 | `loose` 1.6 | normal |
| `body-14` | Poppins | `14px` | 400 | `relaxed` 1.5 | normal |
| `caption-12` | Poppins | `12px` | 400 | `normal` 1.3 | `0.05em` |
...
2. `ds-components/fonts.css` 排在其後載入，**完整重新宣告**同一批 token。2026-08-28 ztorUI 換裝前：拉丁換 Satoshi 打頭（`Geist`／`Inter` 降為備援）、中文兩支角色分工（`--font-cjk-display`＝`LINE Seed TW` 標題、`--font-cjk`＝`Chiron Hei HK` 昭源黑體 內文／UI）。**換裝後（現況）**：拉丁三個角色統一換成 `Poppins` 打頭（`--font-cjk-display` 退出 `--font-display` 堆疊，`--font-cjk` 不變），並新增 `--font-dot`（Doto，待採用）；持有全部 `@font-face`（含本波新增的 Poppins 四字重與 Doto 兩字重）。
3. `shared.css` 的 `:lang(zh-Hant)` 只影響拉丁／中文的排序，不改變「用哪一支中文字面」——那是由 `fonts.css` 的 `--font-cjk`／`--font-cjk-display` 決定。2026-08-28 換裝後這三條也改成 Poppins 打頭（與 `:root`／`fonts.css` 同一套堆疊），繁中模式不再把中文提到第一位，理由見本節開頭的換裝說明。

**實際生效的是第 2 層**（`fonts.css` 覆寫贏過 `_tokens.css`），文件與程式碼都要以它為準。

Tight negative tracking (`-1.28px` on H1, `-0.8px` on H2) carries over from the Geist-era signature and still applies under Poppins.
...
**六個具名 token（`_tokens.css` canonical，值以此為準）：**

| Token | Value | Note |
|---|---|---|
| `--radius-sm` | `3px` | Inner sub-radii on nested components (chips inside cards, code chips, swatches) |
| `--radius` | `6px` | Buttons, inputs, form controls — primary radius |
| `--radius-md` | `6px`（`--radius` 別名） | Q2 2026-07-13 併入 `--radius`；保留別名避免 67 處呼叫點 churn，原值曾為 7px |
| `--radius-lg` | `8px` | Nav panels, dropdown 浮層 |
| `--radius-xl` | `16px` | Card / panel / dialog-level containers（Q16 2026-07-17，原 6px 放大到與 form-section 一致） |
| `--radius-shell` | `28px` | Route-page corner inside the sidebar app shell |
| `--radius-pill` | `9999px` | Status dots, avatar circles, badge chips, full-round pills |

| Semantic role | Value | Where |
|---|---|---|
...
| `shadow-header` | `0 3px 16px rgba(0,0,0,0.10)` (dark `0.45`) | 下緣柔和投影（由元素後內縮圓角色塊投出，只露下緣；`.edge-shadow` 工具）。原本也給 sticky wizard header 用，2026-09-23 頂列改膠囊後不再使用 |
...
**壓在圖片上的字（2026-08-31）** — `--scrim-media`（`rgba(0,0,0,0.82)`）、`--on-media`（`#FFFFFF`）、`--on-media-muted`（`rgba(255,255,255,0.66)`）。這三支是**常數 token，不隨主題翻轉**：它們貼在照片上，而照片不會因為使用者切成亮色主題就變亮——文字色若接上 `--foreground`，亮色主題下就會變成白圖上的淺灰字。壓底的工作是保證圖片再亮也讀得到上面那行字。目前消費端：`canvas-wall.css` 的海報牆磚（`.canvas-tile__cap` / `__name` / `__kind`）與釘點的墨色。
...
- ~~`fan-store.css` phone screen (`.fan-store__phone` scoped `--fst-*` dark neutrals + bezel/hero-gradient/fan-ring hexes)~~ **2026-09-11 退場**：預覽改成粉絲端創作者頁的 token 鏡像，`--fst-*` 整組刪除、檔內零裸色；下面這段保留為沿革。原文：`fan-store.css` phone screen (`.fan-store__phone` scoped `--fst-*` dark neutrals + bezel/hero-gradient/fan-ring hexes) — the See-as-fan phone renders the **fan app's fixed-dark surface** (theme-independent, same rationale as vip-card): the fan app is its own product with its own dark theme, so the phone must not flip with the creator-backoffice light/dark toggle. Brand accent stays `var(--primary)`; rank-ring colors (#A78BFA/#4ADE80/#2DD4BF/#86EFAC) are fixed decorative rank hues (2026-07-02).
...
**`--overlay-scrim`（2026-09-01 新增）** — 浮層遮罩本身，值是 `color-mix(in srgb, var(--background) 40%, var(--overlay-tint))`。使用者裁示「後面的遮罩不用這麼暗，統一將所有 popup 後的黑色遮罩都改」：配方原本在 `payout-modal.css`／`embed-modal.css`／`explainer.css` 三處各抄一份，三份一樣的字串遲早會有一份沒跟上，所以收成一支。同一次把畫布色的比重由 68% 降到 40%——真正把後面那一頁壓死的是這一項，不是 `--overlay-tint`（68% 的實色畫布疊上去，合成 alpha 0.85；只調亮遮罩色也只降到 0.83，降比重之後是 0.68）。`--overlay-tint` 本身同時由 0.45／65% 調亮到 0.32／46%。**元件層的局部遮罩不吃這一支**（抽屜、掃描器、上傳格 hover、媒體庫）：它們蓋的是一個元件、不是整頁，仍單獨用 `--overlay-tint`。
...
- ~~`detail-sheet.css:34` — `background: color-mix(in srgb, #000 52%, transparent)` — 覆蓋層遮罩底色~~ **已於 2026-08-31 殼層換裝收斂為 `--sheet-veil`（`color-mix(in srgb, var(--ztu-canvas) 60%, transparent)`），不再是裸值。** 原判定留存：**非乾淨例外，建議收斂**——站上已有 `--overlay-tint`（`rgba(0,0,0,0.45)`，見 §1.5 Edge & overlay tokens）承擔同一種「modal/覆蓋層遮罩」角色，本檔用 52% 而非既有 `--overlay-tint` 的 45%，屬可以但尚未對齊既有 token 的裸值漂移，非「無法用 token 表達」的那一類。
- ~~`detail-sheet.css:47` — `box-shadow: var(--shadow-overlay, 0 -8px 40px rgba(0,0,0,.45))`~~ **已於 2026-08-31 殼層換裝改為 `var(--ztu-shadow-card)`＋內光，fallback 字面量一併消失。** 原判定留存：主路徑已引用 `--shadow-overlay`，fallback 非實際生效值；**不計入裸值，優先度最低**。
...
- ~~`sticky-actions.css:31` — `box-shadow: 0 -6px 16px rgba(0, 0, 0, 0.08)`~~ — **已於 2026-08-28 ztorUI 換裝 Wave 2b 收斂**：改引用新增的 `--ztu-shadow-pop-up` token（見 §1.1b），這筆裸值例外已不存在，check_ds_sync 檢查 5 的存量計數同步下修（46→45）。
...
**小結**：19 支中判定為「無法用既有 token 表達、應保留」的例外原共 6 處（brand-card、checkbox、media-vault ×4、fans-guide ×2、sticky-actions、chart-tip）；**sticky-actions 已於 2026-08-28 ztorUI 換裝收斂進 `--ztu-shadow-pop-up`，現餘 5 處**；`detail-sheet.css:34` 判定為**應收斂**、非合理例外——**已於 2026-08-31 殼層換裝時一併收斂（見 §4.97）**。棘輪基準 `ds-baseline.json` 是否要為上述合理例外調高對應檔案的允許值，由下游負責 CSS／baseline 的 agent 決定。
...
**自繪的第三顆例外：`archive-restore`（2026-09-22，D298）** — Tabler 沒有「從封存箱拿回來」的字符：`archive-off` 是斜線（讀成停用）、`restore` 是倒轉箭頭（讀成重做／還原）。做法是沿用 Tabler `archive` 的蓋子與箱身、只把提把換成往上的箭頭——同一顆箱子、東西往外走，與 Lucide 的 `archive-restore` 同義，跟旁邊的「封存」`archive` 一看就是一對。用在「解除封存」（已封存的販售管道回到已下架）：e-shop 已封存列的列操作、三個細節頁頁首的主要動作、§4.202 Archived gate 示範。

**自繪的第二顆例外：`check-bold`（2026-08-19）** — Tabler 沒有這個字符，是照使用者指定的圖案自繪的：粗體勾勾、兩段深淺（短撇 45%／長捺 100%，明度差＝筆順）。它不是狀態符號而是**「這裡有事要勾掉」**，所以刻意不長成驚嘆號或三角警示牌。用在儀表板告警通知條的左槽——嚴重度已經寫在標題裡（例：「提款已暫停」），再放一顆紅色警示符號是把同一件事講第二次，而深色介面上描邊的紅圈／紅三角一律讀成「一個框裡有東西」（實測退了三版才確定這條）。opacity 寫在各 path 上而不是分三個顏色，所以整組跟著 `currentColor` 走，中性與警示兩種情境共用同一支。
...
**App shell 檯面與星空背景（ztorUI 換裝，2026-08-28 熔接，Wave 1）** — sidebar 導覽模式下，`body` 底色改為 `--ztu-canvas`（`#0a0a09`，全站最深的檯面），玻璃化的 `.app` 外殼（見 §1.1b）以 `--ztu-shell-gutter`（14px）留白浮在檯面正中央，不再貼齊視窗邊。檯面疊三層裝飾，一律 CSS 繪製、不吃任何圖片素材：

- **兩團暖光**：檯面上兩個大範圍、低飽和的徑向漸層暈染（`--ztu-bloom-1` 系），只在 sidebar 模式生效，與外殼留白同一條規則寫在 `body` 上。
- **四層星空**：四組 `radial-gradient` 疊圖磚（620／680／740／810px 四種尺寸，各自的位置與 alpha 值），各自掛一組 `@keyframes ztu-star-a` ~ `-d`（7／11／9／13 秒，負延遲 0／−3／−5／−8 秒錯開），只動 `opacity`（合成器動畫、不佔用主執行緒），`position: fixed` 鋪滿全螢幕、`z-index: -1` 墊在最底。`prefers-reduced-motion: reduce` 時凍結在 `opacity: 0.85`，不播放呼吸動畫。
- **精靈頁靜態點陣**：`.wizard`／`.wizard__sheet--sectioned` 用固定 22px 格距的點陣背景（非動態），與四層星空是不同機制、不疊在一起。

**列表頁漸消**：帶 `.list-dock` 的頁面（列表型頁面），四層星空一起套 `mask`，由 36% 高度處開始往下線性淡出到 90% 完全透明——星空只在頁面上段可見，避免與下方密集的表格內容互相干擾閱讀。

**與換裝前的差異**：換裝前 app shell 檯面／sidebar 模式外殼是實色面板（`--surface-shell`／`--surface-page` 語義 token 的純色），沒有星空或暖光裝飾層；玻璃化與星空背景都是本波新增的視覺語彙，只在 `html[data-nav-mode="sidebar"]` 下生效，`topbar` 導覽模式的外殼另有獨立處理（見 §3.2）。
...
| **Input surface** (填色互動面) | `--input-surface` | `var(--card)`（dark `#2A2B2D`，2026-07-26 由 `#262729` 定案調整）| 暗色下比 `--card` 亮一階的「填色互動面」：`.input/.textarea/.select`（2026-07-17 Q19）＋型別選項卡 `.selection-card--icon`＋上傳投放區 `.upload-tile`（2026-07-18）皆用，讓這些互動面在 section 卡（`--card`）上浮得出來；亮色＝白卡靠 border 分界 |
...
| **Tier ramp**（分層色階） | `--tier-1..4` | 外 `#7a4a12` · `#c07a1e` · `#f0a83c` · 內 `#ffd9a0` | 2026-09-01 新增（調參器 `docs/chart-palette-tuner.html` 定案）。給**有順序、層層包含**的資料，目前唯一消費者是粉絲分層（Fan ⊇ Ranked fans ⊇ Superfan ⊇ Inner Circle，`js/components.js` 的 `TIER_COLORS`）。**與分類色的分工**：分類色是「幾個彼此獨立的類別」，挑色準則是兩兩拉開、沒有誰比較深；分層要的正好相反——同一個色相走深淺，深淺本身就是「第幾層」。用分類色畫分層，四層會讀成四群不相干的人。由外層（人最多）到內層（人最少、最重要）愈來愈亮。⚠ 四個值是在暗色上調的，`:root` 那一份兩個主題共用，亮色主題需要另外調一份覆寫（未做）。 |
| **Charts** | `--chart-1..8` | 橘 `#ffa33f` · 藍 `#266DF0` · 綠 `#22C55E` · 黃 `#F8D749` · 紫 `#8B5CF6` · 玫紅 `#EC4899` · 青 `#06B6D4` · 赭 `#7C4A2D` | 收益類型與排行色相；6–7 於 2026-07-27 隨商品／門票收益加入，8 於 2026-08-09 隨貼文帶貨收益加入（D181）。**2026-09-01 曾換成偏暖的一組、同日改回**（使用者裁示「分類色還是換回原本的」）：那一輪換的是兩件事——新增 `--tier-*`（保留）與重調分類色（撤回）。色階解掉的是「用分類色畫層層包含的資料」這個病，分類色本身沒有那個問題。撤回的那組（橘 `#ffa33f` · 赭紅 `#e0713f` · 芥黃 `#c9a227` · 苔綠 `#7fb069` · 鴨藍 `#5f9ea0` · 藕紫 `#8f7bb5` · 玫粉 `#c47ba0` · 玫褐 `#a8836a`）留在調參器 `docs/chart-palette-tuner.html` 裡，要再試隨時叫得回來。**這一組只負責彼此獨立的類別**，有順序的資料走上一列的 `--tier-*`。 |
...
**垂直節奏法（2026-09-01 立法，Q103）——直排兄弟之間的距離由「容器」持有，不由兄弟自己想辦法**：

- **建立節奏用容器 `gap`**（flex column／grid 的 `gap`），不用 `.x + .x { margin-top }` 相鄰選擇器、也不用「每個成員自帶 margin-bottom」。這兩種寫法的病是一樣的：距離綁在「成員是誰」上，**排進來一個異類鄰居就貼成 0**——實例三發：media-vault 的上傳格不是 `.vault-group`，與第一個分區貼死（2026-09-01 使用者回報）；form-section 的 radio-cards 不是 `.field`，身上沒有 `.field` 自帶的留白（2026-08-10，當時只補了那一處）；同日在區段裡的 `.form-grid` 也是同型風險。
- **相鄰選擇器只准用來「收緊」**：`margin-top: 0`／負值／貼齊配對（label＋hint 這種語意上是一組的），不准用來創造節奏。判別法：把兩個鄰居中的任何一個換成別種元件，距離還該存在嗎？該存在→那是容器的事。
- 既有的同類相鄰規則（`.stat + .stat`、`.rule + .rule` 等）列存量觀察，check_ds_sync 檢查 13 以 WARN 盤點；新增會讓數字上升，先想容器 gap。
...
| Token | Height | Use |
|---|---|---|
| `--control-h-xs` | 32px | 表格密集列（`.variant-table .input`）／`.btn--sm`／compact toolbar。2026-09-11 由 28 調到 32（使用者：小控件要再高一點點、從 token 一次調），與 `.btn--icon.btn--xs` 的 32 邊長對齊 |
...
### 2.5a Surface layers（表面階層 `--layer-*`，Q61 A／Q63 B，2026-09-15）

E0–E4 管「抬多高」（陰影），`--layer-*` 管「疊上去該多亮」（填色與線）——兩條正交的軸，同一個元件各取一階。級數寫在 token 名字裡，看名字就知道這塊面是第幾層。全部是 Role 別名、指向既有 Foundation，不帶新顏色、不另設暗色覆寫（亮暗跟著被指向的那支走）。模型出自 §6.0.1／Figma `856:27798`。

| 階 | Token | 指向 | 用在 |
|---|---|---|---|
| L0 檯面 | `--layer-0-surface` | `--ztu-canvas` | body 星空底（`shared.css` body，Q64 第一批已遷） |
| L1 卡片 | `--layer-1-surface` | `--ztu-glass-bg` | 玻璃卡本體：`.card`／`.ztor-card`／`.kpi`／`.form-section--outlined`／`.preview-card` 等 29 支（Q64 第一批已遷）。頂欄、側欄、hover 浮起、按鈕 hover 仍直接吃 `--ztu-glass-bg`——那是材質，不是層 |
| L2 薄膜 | `--layer-2-surface` | `--ztu-film` | `.nest`／`.card--muted`／`.kpi.card--muted`，以及 Q66 落地的 15 支分組面（`.funding-panel`、`.payout-bank-*`、`.scanner-access`、`.review-status`、`.ss-status`、`.msg-schedule`、`.bd-sec`…；Q64 第二批已遷，濃度由 4% 統一到 8.5%） |
| L2 邊 | `--layer-2-line` | `--nest-line` | L2 上緣髮絲線，亮色可見、暗色歸零（已採用） |
...
- **卡內表格與清單的表面（2026-09-11 立為機器檢查）**：表頭用薄膜 `--ztu-film`、列一律透明、靠 hairline 分隔——不用 `--muted`／`--card` 這類**實色表面**（Q24／Q71 的 L2 語彙：L1 卡是玻璃，實色疊上去是一塊不透明補丁）。同一種錯 09-09（逐選項組合表）與 09-11（庫存歷史紀錄）各被抓到一次，改由 `check_ds_sync` 檢查 14 以棘輪擋新的：元件 CSS 裡 `__head`／`__row`／`__group`／`th`／`tr` 這類選擇器的 background 不得新增實色表面 token；存量 7 處（列選態、凹槽等非表格用法：artist-picker、embed-modal、link-field、pickup、table 的展開列）留基準逐步清。
- **用了元件就要連它的 CSS（2026-09-11 立為機器檢查）**：元件 CSS 是逐頁 `<link>` 的，頁面用了某支元件的 class（含 `--` 變體）卻沒連那支 CSS，樣式會靜默失效——看起來只是「間距不見了」（product-detail 的 `.stack--tight` 沒連 `stack.css`、series-detail 的 `.stat-row` 沒連 `stat-row.css`，兩次都被使用者先看到）。`check_ds_sync` 檢查 15 掃頁面靜態 HTML 的 class 對照唯一擁有它的元件 CSS，漏連即 FAIL；JS 動態產生的 markup 掃不到，寫 partial 時自己確認。
...
| `--card` | `#FFFFFF` | `#212223` | 浮在 content 上的卡；2026-06-25 改實色（原 rgba .10 疊層）；2026-07-17 midnight-v2（原 `#303131`）。**token 本身的值未變**，但 2026-08-28 ztorUI 換裝後 `.card`／`.kpi`／`.alert--bar`／`.readiness`／`.preview-card` 等玻璃卡家族改直接引用 `--ztu-glass-bg`，不再把 `var(--card)` 畫成自己的底色（見 §1.1b、§3.2）；`--card` 仍是彈出層 `--popover` 等其他角色的來源值 |
...
| `--surface-preview` | `#0A0A0A` | `#0A0A0A` | 前台預覽的畫布＝粉絲前台頁底色（`fan-shop.css` `.fep-shop` 的 `--neutral-900`），前台永遠是深色、兩主題同值。消費：預覽與發布全頁畫面（`.wizard__body--stage` 所在的 `.wizard`／`.wizard__sheet`，2026-09-23）|
...
| `--muted-foreground` | `#6E6E68` | `#979797` | 2026-07-13 light 微暖（原 `#737373`）；dark 2026-06-25 改實色（原 rgba .4），2026-07-28 全站 43 頁 WCAG 掃描再提亮（原 `#757575` 在 4 個面上全部不到 AA） |
| `--primary` | `#ffa33f` | `#ffa33f` | **orange stays** — paired across modes for brand consistency |
| `--border` | `#EAEAEA` | `#373839` | hairline（= `--input`）· 2026-07-13 light 更淡（原 `#E5E5E5`）；dark 2026-06-25 實色化、2026-07-17 midnight-v2（原 `#3A3A3C`）→ 2026-07-21 Q22 提亮至 `#333435`（原 `#2C2D2E`）→ 2026-07-26 再提亮一階定案 `#373839`（`--input-surface` 調亮後對比不夠，3 階候選選 B） |
...
> **ztorUI 換裝反轉了本節的結論（2026-08-28，Wave 2a §3.6）**：換裝前的規則是「浮層一律吃毛玻璃」；換裝後的規則正好相反——**內容卡片改玻璃、彈出層改實色**。以下先記錄現況，換裝前的舊規則留在小節末尾備查。

**現況（Wave 2a 起）**：`.card`／`.kpi`／`.alert--bar`／`.readiness`／`.preview-card`／`.form-section--outlined`／`.list-toolbar`／`.app-topbar` 等內容表面改用 `--ztu-glass-bg` 半透明玻璃（見 §1.1b）；**2026-09-01 補漏十四處**（使用者回報「還是有一些頁面沒套用新的 DS」）——`.project-list__row:hover`、`.vault-reach`／`.vault-door`／`.vault-gridwrap`／`.vault-tracks`／`.vault-ovcard`、`.vault-keys`、`.notif-matrix`、`.funding-panel--card`、`.event-preview-card`、`.tabs--card`、`.selection-card`（含 `--active`）、`.fg-demo`、`.ss-fan__card` 當時沒跟上，還停在實色 `--card`，於是同一種卡在不同模組是兩種材質。**留著沒動的是刻意的**：彈出層（`.vault-gate__panel`／`.vault-ovcard` 以外的浮層、`.dropdown__menu` 那一批）依本節規則本來就不透明；巢狀在卡裡的小塊與控件（`.vkey`／`.vshare__opt`／`.fg-perk`／`.ss-url`／`.qr-box`／`.pickup-select__box` 等）不是內容卡，玻璃疊玻璃正是本節要避免的；`.ss-actionbar`（釘在底的動作列）改玻璃會讓底下的內容透上來壓到按鈕，屬待裁決。但**彈出層一律退回全不透明的 `--popover`，不做玻璃**——`.dropdown__menu`、`.zselect__panel`、`.preview-panel__sheet`、`.payout-dialog`、`.app-topbar__dropdown` 這批「浮在別的東西之上」的層級改成 `background: var(--popover)` ＋ `backdrop-filter: none` ＋ `border: 1px solid var(--ztu-glass-rim)`。理由：卡片已經是玻璃，浮層再疊一層透明就是「看穿兩張玻璃」，讀不出前後層次；讓浮層不透明才能穩定地蓋住底下的玻璃卡與星空背景。`_tokens.css:938–953` 原本「集中把 `backdrop-filter` 套進每個浮層」的治理段整段移除（只留說明註解）；`.overlay-surface` 這個掛牌 class 全站零消費，移除後沒有孤兒。`--overlay-blur` 這支 token 仍保留定義，但不再被上述四個浮層引用。

**例外**：`.readiness__pop .readiness`（懸浮版備齊度卡）與 `.preview-panel__backdrop`（遮罩層，半透明＋blur）維持浮層材質——前者是「浮層裡的內容仍是內容」，後者是遮罩本來就該半透明。

**換裝前的舊規則（2026-06-12 起，已由上述現況取代）** — Because `--card` was translucent in dark mode, any panel that floats **on top of content** (dropdown menus, slide-over panels, modals) bled the content behind it unless it frosted that background. This was governed in one place via the `--overlay-blur` token (`blur(14px) saturate(140%)`) plus a centralized rule in `_tokens.css` that applied `backdrop-filter` to every floating surface together. New floating layers only needed the `.overlay-surface` class to opt in. **This centralized rule and the `.overlay-surface` opt-in no longer exist as of 2026-08-28** — see 現況 above.
...
| Layer | Definition | R 2.2 examples |
|---|---|---|
...
| R 2.2 example | Alert organism (§4.13) | "Dashboard-as-hero" layout pattern (§5.1) |

**Status tags (orthogonal to layer):**

- **✓ App** — R 2.2 has custom code in `ds-components/*.css`
...
**無自帶 CSS 的 JS 元件也要進 `design-system.html`**（2026-08-11 使用者裁決，取代先前「只寫進本檔」的慣例）。這類元件（`partials/work-fields.js`、`js/work-taxonomy.js`、`partials/film-picker.js`、三支彈窗模板模組、`js/components.js`）建在既有元件的 class 之上、不擁有任何樣式，過去因此只登記在本檔。改的理由：使用者檢視元件的唯一入口是 `design-system.html`，只寫進本檔等於他看不到這個元件存在。做法——

- 本檔給一列（CSS 欄放那支 JS 檔的連結，不留空）。
- `design-system.html` 給 TOC 項、元件總表列與 demo 卡，內容與本檔這一列對齊。
- demo 卡一律**載入那支 JS 當場產生**，不照著模組的模板手抄一份靜態 markup——手抄的那一刻起兩邊就開始分岔。
- 做不出互動 demo 的純資料模組（例如只提供值域的 Work taxonomy），改用「值域一覽」呈現，並在卡片裡寫明它為什麼沒有互動 demo。
- 彈窗類模組的覆蓋層在 demo 卡內以頁面級樣式壓回文件流（`.demo--inline-modal`），元件本身不動。

`check_ds_sync.py` 抓不到這一類（它的檢查以 `ds-components/*.css` 為起點），所以這條慣例只能靠人維護：新增這類 JS 元件時，兩份文件一起補。

| Component | `_layer` | Status | Where seen (Ztor Creator Studio) | CSS |
|---|---|---|---|---|
| Button | 🟢 atom | ✓ App | Primary CTAs, wizard action bar, header actions (primary / outline / ghost) **`.btn--soft.btn--destructive`（2026-09-11）**：淡紅底（`--destructive` 12%）＋紅字、無邊框——訂單明細每列一顆的「撤銷」鈕（使用者：「換成有底色的」）；比 ghost 紅字看得見、又不回到 08-06 否決的實心紅塊。 | [button.css](./ds-components/button.css) |
| Badge / Status pill | 🟢 atom | ✓ App | Dashboard / Earnings / Payouts + category chips — flat soft-tag look (no dot/ring), variants orange / success / error / info / warning / accent (purple) / neutral。**`--outline` / `--outline-danger`（2026-09-05 · D250）**：透明底＋1px 邊的描邊徽章，用在「同一張表裡不是收入的那一族」——交易明細把收入事件與帳務異動（提款／作廢沖銷／扣款）放同一張表，兩族的狀態不共用色票（收入的「已付款」與提款的「已付款」不是同一件事）。分工是**填色＝賺到的錢的狀態、描邊＝帳的搬動的狀態**，不多發明一組色相。`--outline-danger` 是同形狀的失敗態（提款退回），只換邊與字為 destructive、底仍透明，才不會與收入族的 `.badge--error` 撞成同一塊紅貼紙。`--inline`：接在標題文字後的安靜限定詞（regular 字重、middle 對齊、左 6px），如訂單「Limit 2/person」「Awaiting pickup」（2026-07-21：E-Shop 清單的「N variants／限量」用法已移除，規格數與限量狀態改由 `__meta` 副標與 `__stock` 欄本身呈現，見下方 Product list Variants） | [badge.css](./ds-components/badge.css) |
| Status dot | 🟢 atom | ✓ App | Dashboard status / source dots | [badge.css](./ds-components/badge.css) |
| Chip | 🟢 atom | ✓ App | Earnings transactions filter, Tax year filter, supported regions。變體：`.chip--active` 品牌橘 tint（篩選已選，Q8-A 2026-07-27；原反白黑底作廢）／`--static` 唯讀／`--removable` 帶行內 ×／**`--value`（2026-07-21 新增）** 中性淡填（`--input-surface` 底＋`--foreground` 字＋`--border` 邊）＝創作者剛輸入的值，用於多選項選項值——這種 chip 是正在輸入的資料、不是被選中的篩選條件，不該反白搶視覺（使用者裁示，參照 Webflow Designer 的 class chip）。分工（2026-07-27 Q8-A 後）：`.chip--value` 灰＝剛建立的值、`.chip--active` 橘 tint＝已選（篩選已選與 tag-input 的已套用分類現在同一個外觀，Q19 的專屬覆寫已刪）。**新消費情境（2026-08-09）**：建立活動 `create-event.html` 的「加入商品」清單，已加入商品改用 `.chip-group` ＋ `.chip--removable`，價格併進 chip 文字、不另立一欄。**Q71 全面無框化（2026-08-25）**：未選底改 `--input-surface`、`border:0`，hover 改 `--accent`；`--static` hover 不變色；`--value` 現與預設外觀相同（保留只為語意標記，列為下一輪合併候選）；`.chip--active` 一直是 `--selected-surface`＋`--selected-ink` 橘 tint，站上曾有「active 反轉成 foreground 底」的舊描述，該描述已過時、以本行為準。新增 `.chip--add`（1px 虛線框＋透明底），是唯一保留邊框的變體——語意為「這排的下一格」。**`.filter-row--group`（2026-09-05）**：同一組篩選的第二排（交易明細的「狀態與資料來源」接在「收入分類」下面），上緣一條 `--border-soft` 發線＋`--sp-12` 上內距；兩排裸 chip 之間沒有分界會被讀成同一排折行。**`.chip:disabled`（2026-09-21）**：現在選不到的選項（剩餘不足一組的票種、沒貨的規格）——淡化（opacity .4）＋劃線＋禁止游標、hover 不變色，留在排上不藏；首個消費者＝組合包預覽卡（§4.213）的票種／規格選擇器 | [chip.css](./ds-components/chip.css) |
| Switch | 🟢 atom | ✓ App | Settings notifications, E-Shop visibility, My IP marketplace, Earnings auto-payout. **`.switch--locked`（2026-08-09 從 `notification-matrix.css` 上收進本檔）**：合規／來源鎖定的停用態（`cursor:not-allowed`＋`opacity:.65`）。上收原因——這個修飾類原本只定義在 `notification-matrix.css`，但 `tier-settings.html`／`create-event.html` 都只連 `switch.css`、沒連那支檔，掛了 class 卻吃不到任何視覺，只剩 JS 擋點擊；三個消費頁（settings／tier-settings／create-event）現在都連 `switch.css`，鎖定態理當定義在這裡，不必為借兩行樣式多連一支不相干的元件檔。Consumers：settings.html（合規通道鎖定開啟）、tier-settings.html、create-event.html（bookyay 帶入鎖住「發布後直接開賣」開關） **停用態（2026-09-11 D265）**：`.switch[aria-disabled="true"]`／`.switch--disabled` 變淡、不吃事件——e-shop 已下架列的顯示開關。 | [switch.css](./ds-components/switch.css) |
| Info banner | 🟢 atom | ✓ App | Contextual explanations (timing, region note, legal hint) | [info-banner.css](./ds-components/info-banner.css) |
| Upload tile | 🟢 atom | ✓ App | Create-flow upload slots（hero／thumbs／file drop，Add new item）；opt-in 互動上傳（`[data-upload]`＋`partials/upload-tile.js`）：選圖→假進度→hover 替換／刪除（站上標準 2 鈕，`data-upload-ai` opt-in 第三鈕 AI 優化）。**2026-08-07 新增 `.is-processing`**（`data-upload-processing` opt-in）：檔案傳完、伺服器還在轉檔／檢查的那一段，沿用上傳中的 frosted 浮層但不給進度條，`upload:change` 的 `filled` 維持 false，所以就緒檢查照舊算它未完成、送出被擋（作品上架流程 F1 的上傳與處理狀態、§8 檔案就緒）；事件同時新增 `detail.state` 讓消費頁講得出「現在是哪一段」。**2026-07-31：全站圖片槽收斂單一直式 750×1125**（`--upload-img-ratio`），舊 `--1x1`／`--3x4`／`--3x2` 退場、`--16x9` 僅存於檔案槽。`--hero` 圖示外加圓角晶片框（`--accent` 底＋1px `--border`＋`--radius-lg`，2026-07-17 Q18）。投放區底色 `--input-surface`（暗色比 section 卡亮一階＝填色投放區、亮色白卡，2026-07-18，對齊 midnight；原 transparent）。**2026-08-09 新增撐滿型素材列**：`.upload-assets--fill`（`--upload-asset-cols` 決定一排幾格、寬度平分容器、第五格換行同尺寸）＋容器層 `[data-upload-reveal]`（一次只露出一個空格，填滿才長出下一格），建立商品「展示它」由「主圖大格＋附圖 2×2」改為五格同尺寸 **2026-09-10（D259）展示素材槽收影片**：非 `data-upload="content"` 的格子預設 `accept="image/*,video/*"`（頁面可用 `data-upload-accept` 收窄），影片走既有 `.upload-tile__video` 節點、靜音停在第一影格當縮圖，格上掛狀態 class `.is-video`＋`.upload-tile--playable`（與 16:9 版型修飾詞 `--video` 是兩件事）；展示槽的影片是 `object-fit: cover`＋透明底（內容檔那套 contain＋深色底會在 2:3 直式格留兩塊黑），AI 優化的兩顆鈕與徽章在影片格隱藏。消費頁要把素材畫到別處時必須先問型別，只讀 `.upload-tile__thumb` 的 src 會讓影片變破圖 | [upload-tile.css](./ds-components/upload-tile.css) |
| Bundle editor | 🟠 molecule | ✓ Project | 共創回饋套組編輯器（2026-07-24 promote → 2026-07-28 第二代改版〔create-project「回饋套組」步驟〕→ 2026-07-30 抽成共用行為模組 `js/bundle-editor.js` → 2026-07-30 第二批：價格改唯讀自動計算＋新增套組優惠欄位 → **2026-07-30 第三批：優惠改百分比欄位，卡片欄位重排，撤除左緣未完成色線**）。`window.ZtorBundleEditor.mount({list,addBtn,t,getPool\|poolInput\|pool,getPerSlot\|perSlotInput\|perSlot,initial,collapsedByDefault,shares,onChange})`，回傳 `{getBundles,setBundles,add,ensure,render,refresh,isValid,listPrice,finalPrice,maxDiscount,maxPct,perSlot,maxUnits,autoUnits,committedSlots,pool,num}`；**2026-08-03（D166）新增 `shares` 選項**（布林，預設 `true`；預購掛載傳 `false`）——共創與預購的方案是同一件事，差別只在預購不含分潤，`shares:false` 時關掉五個地方：<br>分潤名額欄位（`cpp.bd.slots`）整格不渲染。<br>販售上限的「自動」不出現，只剩「不限量」（內部值 `unlimited`）／「限量」兩個由創作者自己決定的選項（共創的非限量內部值是 `auto`，兩型故意用不同字串，讓狀態自己說得出「這是推導出來的」還是「這是真的沒有上限」，所有判斷式一律寫成「是不是 `limited`」）。<br>自動上限指向名額池的說明不出現。<br>價格算式（`priceHintText()`）的名額段落消失，只剩商品加總。<br>`isValid()` 判準改為「有名稱 ＋（至少一件商品或至少一項有內容的權益）」（共創版是「有名稱 ＋（至少一件商品或名額數 > 0）」），因為預購沒有名額可以當對價，權益要能算數，否則純體驗方案（如「一場線上見面會」）永遠是無效卡；價格空狀態文案也換一組（`cpp.bd.qty.price.empty` vs `cpp.bd.price.calc.empty`）。其餘欄位（名稱、一句話說明、商品、額外權益、套組優惠、價格計算）兩型完全相同、共用同一批 `cpp.bd.*` key。規格出處：`documents/5.1.2.1-建立專案流程.md` §5.3.2 F28／F29、`decisions.md` D166。名額池與每名額單價各自三種注入方式（即時 getter／讀某 input／固定數字）讓同一支模組同時服務「會即時變動」（create-project 讀 `#fd-slots`／`#fd-perslot`）與「固定值」（project-detail，`fund.goal÷200` 推導名額池、每名額單價固定 $200，原型假資料）兩種頁面；`refresh()` 供外部注入值變動時就地重算全部卡片，不重畫、不搶焦點。**計價規則**：原價＝名額數×每名額單價＋商品定價加總。**2026-07-30 第三批：套組優惠改百分比欄位**（0–100%，沿用既有 `.amount-field--suffix.amount-field--readonly` 做法，非新造百分比欄樣式）——套組價＝原價－min(優惠%×原價, 商品定價加總)，折抵範圍仍只吃商品那一段，地板仍是股份總價（分潤名額的價值不可折讓）；上限同樣改以百分比表達：新函式 `maxPct()`＝floor(商品定價加總÷原價×100)。純回饋（0 名額）卡可折到 $0，只含名額無商品的卡 `maxPct()` 為 0、一分都不能折；填超過上限時優惠欄下方 `.field__hint` 轉紅（`.field__hint.fc-hint--over`）並說出實際生效的百分比與金額，不默默截掉。每張套組＝`.card.fc-bundle`：收合態 `.fc-bundle--collapsed` 顯示摘要列 `.fc-sum`（序號 `.fc-bundle__index`＋`__name`／`__meta`／`__price`，有優惠時原價劃線並列於前，`.fc-sum__was`）＋`.fc-bundle__actions`（編輯或收合／移除）；**2026-07-30 第三批展開態欄位重排（由輸入到結果）**＝套組名稱＊／一句話說明／含分潤名額＋販售上限 `.segmented`（自動＝從名額池推導可賣份數／限量＝手動硬上限，`.fc-slotnote`／`--over` 超賣提示）／商品用**引用**不用自由輸入——`.fc-pick`（`__results`／`__opt`／`__opt-price`／`__empty`）挑選既有 E-Shop 商品成 `.fc-ref`（`__thumb`／`__name`／`__meta`）／額外權益仍為自由文字 `.fc-perk`／最後才是獨立的**定價區塊**（新增 `.fc-pricing`，卡片最後，上緣一條分隔線）：優惠%（`.fc-pricing__row` 窄欄）＋價格（唯讀 `.amount-field--readonly`，`.fc-pricing__out`）＋橫跨整列的組成說明與上限說明（`.field__hint.fc-pricing__note`，兩行，取代已移除的 `.field__hint.fc-pricecalc`；與 `.fc-hint--over` 同樣寫成 0,2,0 權重，因為 field-system.css 在本檔之後載入）。滿版加高鈕：`.fc-add`（56px，新增套組）／`.fc-add-item`（48px，本代重用作「新增權益」）。**卡片左緣「未完成」色線已依使用者裁決移除**（`.fc-bundle--invalid` 與其 `border-color` transition 一併刪除，CSS 留墓碑；`isValid()` 判準不變——缺名稱、或無名額也無商品仍為無效，仍驅動建立頁教練提示與 Continue 擋關，只是卡片自己不再顯示那條色線；`--status-warning` 在本元件已無引用）。`.fc-bundle__body > .form-grid` 補回格線與下一欄位的間距（2026-07-30 新增）。消費頁（**2026-08-03 由 2 個增為 4 個**，D166 新增預購方案掛載點）：create-project.html（回饋套組步驟，共創）、create-project.html（預購方案步驟，`shares:false`）、project-detail.html（方案與承諾›支持方案，共創，`collapsedByDefault:true`＋3 張種子套組）、project-detail.html（方案與承諾›預購方案，`shares:false`，`collapsedByDefault:true`＋2 張種子方案）。**create-campaign.html 仍保留自己的第一代頁內 `.fc-*` 副本**（僅自由輸入商品列 `.fc-item-row`／`.fc-item-fields`，無收合／引用挑選器／權益／名額推導／計價），未遷移到本模組（治理待辦）。**2026-07-30 第五批（使用者裁決）**：展開態的價格移到標題列右上角——復用第三批已有的 `.fc-sum__tag`（「套組價」小標，展開態才顯示）＋放大的 `.fc-sum__val`（`--fs-20`），有折扣時原價劃線 `.fc-sum__was` 仍並列在前；收合態摘要價格不變。展開態的動作同輪從標題列移到卡片底部——`.fc-bundle__head` 展開時右上角只留價格；卡片底部新增 `.fc-collapse`（滿寬可點的收合把手，`<button>`＋`<i data-lucide="chevron-up" class="ztor-icon">`＋文字，上緣一條分隔線，`:hover`/`:focus-visible` 均已定義）與 `.fc-bundle__foot`（右對齊容器，內含 `.btn.btn--destructive.btn--sm` 移除鈕，僅卡片數 >1 時出現）；收合態動作仍留在標題列右側（編輯／移除文字連結，行為不變）。卡內間距同輪對齊建立商品頁 `.form-section--outlined` 的節奏（使用者指定參考對象）：`.fc-bundle` 內距由 `--sp-16`/`--sp-18` 改 `--sp-24`（`margin-bottom` 維持 `--sp-12`）；`.fc-bundle--collapsed` 左右內距同步改 `--sp-24`（收合↔展開時名稱不左右位移）；新增 `.fc-bundle__body .field__label { font-size: var(--fs-14) }` 與 `.field__label + .field__hint { margin-top: calc(-1 * var(--sp-4)) }`，欄位間距本身仍吃基礎 `.field` 16px（Q6 已統一全站，本輪未動）。**移除鈕用 `.btn--destructive`**（非 `.btn--ghost.btn--destructive`）——Q37 曾裁決為實色紅底，**2026-08-06 使用者裁示全站改紅框紅字**（見 Button 與 STYLE-DECISIONS Q37 的推翻紀錄），本元件不必改 markup、跟著 button.css 一起變。i18n 新增 3 key（中英）：`cpp.bd.collapse.long`／`cpp.bd.remove.long`／`cpp.bd.price.tag`；既有 `cpp.bd.collapse`／`cpp.bd.remove` 仍供收合態短連結使用。 **2026-08-06 適用票種改用商品那一列的形式**（使用者指示）：原本的一排勾選框（`.bd-tickets` 容器＋`.bd-ticket__name`／`__price`，已墓碑）換成 `.fc-ref` 引用列——這一塊回答的是「這組裡面有什麼」，商品用引用列、票券也是內容物之一，長成另一種樣子會讓人以為那是設定不是內容。勾選框留著（一組可賣給多種票的持有者，仍是複選）、移到列首與縮圖並排，勾到的列加一條左緣橘線（`.bd-ticket--on`）。**2026-08-13 修兩處**：<br>票券列比商品列多了列首的勾選框，欄位樣板必須自己再宣告一次——`.fc-ref` 的 `44px minmax(0,1fr) auto` 是為「縮圖／文字／移除鈕」寫的，票券列丟進去整排錯位一格（勾選框佔掉縮圖那 44px、票券縮圖被拉去撐滿中間、名稱與價格擠進最右邊貼邊），改宣告`auto 44px minmax(0,1fr)`（票券列沒有移除鈕，含不含這張票靠勾選框）。<br>分隔線改落在兩列正中間——`.fc-ref` 的節奏是「上下各 8px 內距＋列底一條線」，前提是外層不再插空隙，但這些列是 `.field` 的直接子元素，`.field` 的 `gap: 6px` 會加在線**之後**，線於是距上一列 8px、距下一列 14px。`.field:has(> .bd-ticket)` 收掉那段 gap，節奏交還給列自己的內距，整排頭尾再各給 8px，否則六列會直接黏上標籤與說明、讀起來是一整塊而不是一份清單 **2026-09-01 三項 UX 補強**：picker 空欄＝瀏覽模式（列全部商品＋分類，上限 12）；新增「建立新商品…」出口（`?embed=1` 彈窗嵌 create-product，postMessage 回填，與電子商店組合包同一套機制；UIA-127）；套組價旁 `.fc-sum__unpriced` 徽章標出未定價項目數（警示黃——未定價是還沒做完，不是做錯）。 **同日三段式重排（使用者裁示「需要重新設計這一塊」）**：卡內分三段小標（`.bd-group`／`__title`，hairline 分段、首段不畫線）——①這是什麼方案（名稱＋說明）②支持者拿到什麼（名額/份數＋作品＋商品＋權益）③怎麼賣（販售上限＋優惠＋價格）。名額/份數離開與販售上限的並排（它是內容的量不是供給），販售上限搬到定價旁（賣幾份與賣多少錢是同一個問題的兩半）。SECTIONS 活動變體不受影響。 **同日分段版型通用化＋建立頁改彈窗**：`layout:'sections'` 由活動專用開放給共創／預購（create-project 兩步改掛），第 1 步內容分卡依變體組合——活動＝票種/場次三張（不動）、共創＝新 `secSlotsHTML`（分潤名額）、預購＝新 `secWorkHTML`（作品＋份數）；商品/權益/販售設定/數量四張共用。共創的數量首選項掛「自動」＋名額池提示；舊種子帶折扣自動開 `discountOn`（否則靜默蒸發）。 **同日第二輪：project-detail 兩個掛載跟進分段彈窗**（使用者裁示「這個也要跟著創建流程改」）——全站四個消費點現在都是清單列＋兩步彈窗，行內三段卡（`.bd-group`）成為無消費頁的備用版型。接上詳情頁時抓到並修掉三件事：<br>`refreshAll()` 原本只就地同步帶 `data-bd-card` 的編輯卡，分段版收合列（`data-bd-open`）沒人更新——詳情頁的預購單價欄與方案清單同屏，改單價列上價格會過期；補「就地改列的摘要與價格」（不整列重畫：列是可聚焦按鈕，換掉會踢焦點）。<br>帶種子（`initial`）的組 `fresh` 誤留 `true`——彈窗標題把既有方案標成「新增」、原樣關掉還會被「空卡直接丟掉」規則收走；`newBundle(seed)` 一律設 `fresh:false`（活動 per 展開的組同修）。<br>預購彈窗「不限量」副標漏活動字彙（「只受它含的票券張數限制」）——補 `cpp.bd.qty.unlim.sub.pre`（接單不設上限）；編輯標題同輪補齊字彙：基礎 `cpp.bd.edit` 改「編輯套組」、新增 `.pre`「編輯方案」（`.ev`「編輯組合包」原有）。 **同日 `.fc-ref--work` 補正 Q66**（使用者：「這個沒有照 DS」）：作品列原本填 `--muted`、圖示縮圖填 `--card`，兩個絕對表面色在彈窗這種巢狀情境裡等於把實色檯面貼進玻璃面板，層次讀成往下陷。改成列疊一層 `--ztu-film` ＋ `--nest-line`（坐在 `.bd-sec` 的 4% 薄膜上自然亮一階，「被抬起來的固定項」仍讀得出來），縮圖回到與相鄰商品圖同一個 `--muted` 媒體井（card.css 對 Q66 的明列例外）。活動的票種／佔位列共用 `.fc-ref__thumb--work`，同輪一起回井色、與同列的商品縮圖對齊。 **2026-09-18（D292／D293）活動變體改「票券清單（票種 × 張數）」**：`tickets` 由 `[票種 id]` 改 `[{id, qty}]`（`tixNorm()` 把舊字串陣列視為 qty 1；讀寫一律走 `tix*` 層），多筆＝同時內含（家庭套票）、不再是粉絲擇一。票種卡由 `.radio-cards` 複選卡改成表格化票券清單 **`.bd-tbl--tix`**（欄軌 `16px minmax(0,1fr) 80px 96px 88px`：`.zcheck` 勾選框｜票種｜單價｜`.bd-tbl__qty` 內的 `.zstep.zstep--nounit.bd-tbl__step` 家用步進器（`--control-h-xs`、等寬數字右對齊；未勾選時 input 停用、`.zstep` 自動收起按鈕欄）｜小計；表尾 `.bd-tbl__foot`「共 N 張 · 票券原價 $X」右對齊、上緣 `--border-soft`、不畫底色（Q68）；未勾選列沿用 `--off` 淡化；墓碑 `kindCardsHTML`）。多場活動列仍以票種為列、張數跟票種走，「哪幾場適用」由場次對應與適用場次表回答；適用場次表的「張數」欄改名「剩餘張數」（`cpp.bd.tbl.qty`）與每套張數（`cpp.bd.tbl.qtyper`）分名。原價合計＝Σ（票價 × 張數）＋商品（`ticketValue()`），定價分卡在活動變體永遠顯示原價合計＋算式 `[data-bd-calc-parts]`（「VIP $4,200 × 2 ＋ T 恤 $800 ＝ $9,200」，5.1.5.4 §4 F3）；販售上限硬頂 `memberCapMax()`＝各成員 floor(剩餘 ÷ 每套用量) 取最小（票券看票種張數、商品看 `stock`），不限量選項下也寫出「依票券與商品庫存，最多可賣 N 組」；`isValid()` 活動改「票券任一筆或商品任一件」（D293，商品卡副標 `.ev` 標「選填」）；`summaryMeta()` 逐筆「VIP × 2 · 搖滾區 × 1 · 1 件商品」；`commit()` 每場各一組展開每組沿用 `[{id, qty}]`。消費頁 `create-event.html` 補掛 `stepper.css`／`partials/stepper.js`。募資兩個消費頁沒有 `tickets`，不受影響。 **2026-09-21（D295／D296）票券成員改「允許票種清單 × 固定張數，粉絲任選一種」**：`tickets` 改成單一物件 `{ tierIds:[…], qty:n }`（`tixNorm()` 把舊 `[{id, qty}]`／字串陣列相容成 tierIds＝各 id、qty＝第一筆；讀寫走 `tix*` 層：`tixIds`／`tixHas`／`tixQty`／`tixAdd`／`tixDrop`／`tixSetQty`／`tixCount`），「同時內含」的每列張數模型退場。票券清單表 `.bd-tbl--tix` 欄軌收成 `16px minmax(0,1fr) 96px 96px`（勾選框｜票種｜單價｜剩餘），每列步進器與小計欄退場（墓碑 `.bd-tbl__qty`／`.bd-tbl__step`）；表下新增整組張數欄 **`.bd-tix-qty`**（`.amount-field--suffix.amount-field--readonly.zstep`＋「張」後綴，`max-width:160px`、等寬數字；`[data-bd-tix-qty]`，input 就地同步、change 補回生效值）；表尾 `cpp.bd.tix.foot`「允許 K 種 · 每組 n 張 · 原價從 $X 起」（一種 `cpp.bd.tix.foot.one` 不寫「起」）。原價 `ticketValue()`＝`tixMinPrice()` × n；`moneyFrom()` 在允許多種時把算式（`cpp.bd.calc.tix.any`）、原價合計、粉絲實付、footer、收合列的金額前綴 `cpp.bd.price.from`「從 … 起」。`memberCapMax()` 票券部分＝各允許票種 floor(剩餘 ÷ n) **加總**、再與商品部分取最小；`hasContent()`＝允許票種 ≥ 1 且 n ≥ 1 或商品 ≥ 1；`summaryMeta()`／每場預覽用 `tixLineText()`「VIP／搖滾區 任選 × 2」（`cpp.bd.sum.any`；一種「VIP × 2」）；`commit()` 每場各一組展開每組 `{ tierIds:該場票, qty }`。活動變體商品列（`itemsHTML`）對 `variant:'multiple'` 的商品在 meta 補 `cpp.bd.item.spec`「規格由粉絲購買時選」（`catalogue()`／`resolveItem()` 帶 `variant`）。新掛載選項 `defaultAllTiers`：「新增」時預帶全部票種、張數 1（5.1.6.1 F20）。新增匯出 `tierCount`／`ticketLine`；`ticketCount` 語意改「每套張數 n」。**2026-09-21 第 2 批（電子商店建立組合對齊）**：`bundle-editor.css` 新增已加入表變體 **`.bd-tbl--items-qty`**（欄軌 `minmax(0,1fr) 120px 64px 112px 28px`，多一欄 `.bd-tbl__per` 放每組用量 `.zstep`；電子商店的商品成員每套可用不只一件，5.1.5.4 §4 F2；活動變體每組各 1 件、不用）；`.bd-readout--big` 選擇器改 **`.field-readout.bd-readout--big`**（shared.css 末尾再 @import 一次 field-system.css，單一 class 輸給後載的 `.field-readout` 14px——bug 修，三個消費頁同時受惠）。`create-bundle.html` 不掛 `js/bundle-editor.js`，改在自己的頁面級表單重用這些 CSS 零件與 `cpp.bd.sp.*` 文案（取捨與待裁決見 STYLE-DECISIONS Q121）；`bundle-detail.html` 定價卡改用同一份算式列表（2026-09-22 D311 promote 成 `.calc-list`，見 §4.217）＋`.field-readout.bd-readout--big`。 |

**分段版型 `layout: 'sections'`（2026-08-13 活動專用建立 → 2026-09-01 通用化，現為所有消費頁的版型）** — 清單列＋兩步彈窗、彈窗內一段一張分卡；票種／場次相關分卡仍是活動變體獨有：

  1. 內容（適用票種／商店商品／額外權益）
  2. 販售設定（原價合計 → 折扣 → 粉絲實付，一列算完；販售上限）
  3. 呈現方式（名稱＊／簡短說明／封面圖同一列）

- **為什麼倒過來**：預設版型是名稱＊ → 說明 → 封面圖 → 內容 → 數量 → 權益 → 定價，等於要創作者先替一個還沒有內容的盒子取名字、上傳封面。活動套組腦中的句子是「VIP 票 ＋ 巡演 T 恤，賣 4,800」——先有內容才有名字與價格。名稱移到最後並自動擬建議名（票種名 ＋ 第一件商品），必填欄從空白格變成確認。
- **票種挑選器**：票項可多帶 `group`（哪一場）／`kind`（哪個票種）／`qty` 三個選配欄位，帶了才啟用對應能力——依場次分組（組頭可整場全選）、一排跨場次的票種捷徑、選完收合成 chip（`.bd-chosen`）。實測三場 × 兩票種時預設版型一張空卡 1,464px、視窗只有 944px；分段版型收合挑選器後降到約 1,065px，且不再隨場次數膨脹。
- **`scope`（多場才問）**：`shared`＝一組通用所有場次；`per`＝每場各一組——只挑票種，送出時在每一場各展開成一組（`commit()`），省掉手動複製 N 次。
- **三步彈窗（2026-08-13 使用者指示「把 1,2,3 分成步驟」）**：三段改成真的三步，用站上既有的分步彈窗契約 `.payout-view`（請款彈窗就是三步）。一次只渲染當前那一步、不留 `[hidden]` 的兄弟——同一個 `data-bd-f` 出現三份會讓焦點還原與就地同步抓錯節點；`.payout-view` 這一層仍要留著，dialog 的 head／body／foot 釘住是靠 `.payout-dialog > .payout-view` 那條規則傳下去的。步驟指示器用建立流程頂部同一支 `.progress-stepper--segmented`（`.bd-steps` 只負責它在 dialog 裡的位置與間距），不另做小圓點；步驟標題因此由進度條承擔，body 不再重複段標題。**第 1 步要挑到票才走得下去**——「一組至少含一張票」本來就是 `isValid()` 的條件，把它前移到步驟閘門比讓人填完三步才在最後被擋下誠實。走過的步驟可以點進度條跳回去，還沒到的不給點。
- **六張分卡、每一段一個決定（2026-08-13 第二輪，使用者指示「參考建立商品」）**：場次對應／組合內容／販售設定／販售數量／基本資料／封面圖。三處對齊建立商品的既有做法——
  - **折扣改成開關**（`.control-group` ＋ `.switch`，同「折扣設定」）：多數組合包不打折，把折扣欄永遠攤在算式裡等於預設每一組都要回答一個多數人不需要回答的問題。關著時算式只有兩段（原價合計＝粉絲實付），開了才長出中段。關掉會一併清值，避免「看不見卻仍在算」。
  - **販售數量改成不限量／限量二選一**（`.segmented.radio-cards`，同「庫存」的 Edition）：原本是一個「留空＝不限」的數字欄，留空是安靜的預設、讀不出「我選了不限量」，而且空欄與 0 在畫面上長得一樣。限量才出現上限欄且為必填。
  - **「呈現方式」拆成「基本資料」與「封面圖」兩張**（同「商品資訊」與「展示它」）：名稱／說明與圖片是兩件事，混在一張卡裡右邊那格圖看起來像名稱欄的附屬。原標題也太白話，改用站上既有的欄目命名。
- **「每場次各一組」列出實際會建立哪幾組（2026-08-13）**：選了模式卡只說「會長出 3 組」，沒說是哪 3 組。`secPerHTML()` 照 `commit()` 的展開邏輯算出真正的清單、逐場列出。還沒挑票種時分卡照樣出現、只把說明句換成引導，整塊消失會接不上剛剛那一下點擊。同輪修掉 `commit()` 的一個漏洞：原本展開時對每一個場次都生一顆組合包，某一場沒賣到那個票種時會生出空的一顆；改成先濾掉沒有票可放的場次，展開結果因此與這張分卡逐列相同。
- **兩塊表格化（`.bd-tbl`，2026-08-13 使用者指示「第二個要和第一個一樣是一個 section，並且兩個都要表格化」）**：「會建立的組合包」從票種分卡裡的一段欄位升格成自己的 `.bd-sec`（它回答的是「按下完成之後會多出哪幾組」，與「這一組賣哪一種票」是兩個決定）；它與「適用場次」的內容一起從引用列（`.fc-ref`：縮圖＋名稱＋一行次要資訊）改成表格。原本一列的事實全串在同一行字裡——「$3800 · 共 150 張」要讀句子才分得出哪個數字是價格、哪個是張數，多列之間也對不齊；表格把每一種事實各給一欄，同欄數字上下對齊（`tabular-nums` 右對齊）就能直接比大小。每列一律一個票券圖示是純裝飾（分卡標題已經說了這裡列的是票），一併拿掉。<br>欄位：適用場次＝勾選框／票種／售價／張數，依場次分組（`.bd-tbl__group`，組頭帶「全選本場次」）；會建立的組合包＝組合包名稱／內含票種／票券原價，名稱寫的是展開後的完整名字（`stem · 場次名`），也就是按下完成後清單上真的會出現的字。<br>表頭沿用 2026-07-31 使用者裁示的全站列表慣例（不畫底色帶、不畫下框線，只有 `--column-head-ink` 的欄名）——`.restock-table__head` 那種 `--muted` 底色帶不能照抄，L2 分卡裡再壓一條暗底會變成往下沉一階，與 Q66 的疊層方向相反。列與列之間的分隔用相鄰選擇器（`.bd-tbl__row + .bd-tbl__row`），所以組頭底下的第一列不會被劃線、組頭也就不會被讀成某一列的底線。墓碑：`.bd-grp`／`__head`／`__name`／`__all` 併入 `.bd-tbl__group`。
- **表格第二輪：照外部參考圖收斂（2026-08-13，使用者選定變體 C，見 STYLE-DECISIONS Q68）**：底色帶那一案退場，表頭維持 2026-07-31 的「不畫底色帶、只有 `--column-head-ink` 欄名」。採用的是另外三件事——<br>表頭第一格放全選勾選框（`data-bd-all`，範圍＝這張表列得出來的票）。<br>場次組頭改吃跟列一樣的欄位樣板，勾選框因此坐在列首那一欄、與每一列的勾選框上下對齊，「全選本場次」那段文字標籤拿掉（語意由位置承擔，markup 補 `aria-label`）。<br>沒勾到的列整列淡化（`.bd-tbl__row--off`），取代原本「勾到的列給一條左緣色線」（`--on` 留墓碑）。<br>兩顆全選都用 `indeterminate` 表達「勾了一部分」——只有 checked 的話，取消其中一場會讓組頭整個彈回未勾，看起來像自己的操作被撤銷；這是 DOM 屬性、寫不進 HTML 字串，所以 `render()` 之後要再跑一次 `syncTriBoxes()`。整張表加一圈 `--border` 外框（同 `.bd-pick`／`.bd-calc` 的 L3 處理）。參考圖的彩色狀態點未採用——我們的票券沒有對應的資料欄位。
- **兩步彈窗、不放 stepper（2026-08-13 使用者指示「基本資料與封面圖放在下一步」）**：第 1 步回答「這一組賣什麼、怎麼賣」（票種／會建立的組合包／適用場次／商店商品／額外權益／販售設定／販售數量），第 2 步才是「它長什麼樣」（基本資料／封面圖）。名稱與封面本來就排在最後，而且名稱會從內容自動擬好，等於是確認而不是填空。**不放 stepper**：兩步的進度條要佔掉 dialog 頂部一整條，卻只表達「兩步中的第幾步」，而分卡標題已經說了現在在哪；前後由 footer 的兩顆按鈕承擔（第 1 步＝「下一步」，第 2 步＝「上一步」＋送出）。步驟存在 `b.step`（開啟與關閉時都重設回 1，不接續上次停在哪）；換步時 `render({top:true})` 不還原捲動位置——兩步的內容不一樣，套上一步的位置只會落在別的地方。
- **閘門分兩步判**（`gateOffFor()`）：第 1 步只看它自己那一步答得完的兩件事（有沒有含票、售出上限有沒有超過票券張數），名稱在第 2 步、還沒走到就要求它會擋在一個看不到的欄位上；第 2 步才用完整的 `readyMiss()`。
- **定價改成三個欄位各一行（2026-08-13 使用者指示）**：原本是一列橫向算式（`.bd-calc`：原價 − 折扣% 一行算完），三個數字擠在同一行、標籤又比值小一階，要左右掃過去才讀得完。改成三個 `.field` 上下堆疊——套組優惠（唯一的輸入）／原價合計／粉絲實付，後兩者用 `.field-readout`（唯讀顯示、不畫框），停用的 `.input` 會讀成「這裡本來可以填」。折扣欄同輪拿掉 `amount-field--readonly`（那個修飾詞是給前綴唯讀欄用的，會給後綴 unit 上 `--card` 底並在輸入框左邊留 40px）。最終價因此有兩個落點（分卡欄位與 footer 結果），`syncCard()` 改用 `querySelectorAll` 兩顆一起更新，劃線原價的插入點則明確指向 footer 那一顆。墓碑：`.bd-calc`／`__part`／`__k`／`__v`／`__v--final`／`__op`／`__final`。
- **售出上限不得超過所含票券的最低張數（2026-08-13 使用者指示）**：賣掉一組要從每一種含的票各扣一張，所以能賣幾組由**張數最少的那一種**決定——`ticketCapMax()` 取最小值而不是加總，沒有張數資料的票不參與。欄位掛 `max`、底下一行 `[data-bd-capmax]` 寫出上限來源，超過時轉紅（`.fc-hint--over`）＋`aria-invalid` ＋主要按鈕停用。打字當下由 `syncCard()` 就地同步提示與停用狀態，不重畫（重畫會踢掉游標）。
- **footer 撤掉就緒 chip（2026-08-13 使用者裁決「不需要」）**：原本主動作旁邊有一顆 `.readiness__chip`（hover 浮出缺項清單），撤除；還不能建立時由「完成」按鈕自己的 `disabled` 表達。理由——條件都齊時那顆 chip 只寫「可以建立了」，等於把按鈕已經說過的話再說一次。`readiness()`／`readyMiss()` 兩支判準不動，它們仍是按鈕停用與否的依據；`cpp.bd.ready.*` 五個 key 同輪留墓碑。footer 的最終價同輪改成**靠左**（同日兩輪：先把標籤與金額切齊左緣，使用者回報「還是沒有靠左」後，再把整塊移出 `.bd-foot-actions`、成為 footer 的第一個子元素、貼左緣）。原本它跟主要動作擠在右邊那一團，那兩行字會被讀成按鈕的說明；分到兩端之後，左邊回答「這一組賣多少」、右邊才是「要不要送出」。多於一組時才出現的「移除」接在最終價後面，主要動作由 `.bd-foot-actions` 的 `margin-left: auto` 推到右緣——footer 的 `justify-content: space-between` 只有兩個子元素時才夠用，中間那顆會被推到正中間。`readiness.css` 仍有 create-product／create-auction／publish-work 三個消費頁，未退場。
- **最終價釘在 footer（2026-08-13 使用者指示「價格放在 footer」）**：`.bd-foot-price` 坐在 `payout-dialog__foot`，捲到任何一段都看得到自己定出來的價格；有折扣時把劃線原價並列（`__was`，`syncCard()` 會在折扣歸零時把它移除，避免留下與最終價相同的劃線數字）。定價分卡因此只留「輸入」——原價合計與折扣欄；**折扣關掉時整個算式塊都不出現**，沒有可填的東西時留一個只有一個數字的框是空殼。
- **場次對應與票種同一張分卡（2026-08-13 第三輪，使用者指示「放同一個 section」）**：場次對應是卡內的欄位群、不是獨立分卡——它與票種回答的是同一件事的兩半（賣哪一種票、那一種票怎麼對應場次），拆開會讓人以為是兩個不相干的決定。單場活動整組不出現，副標也跟著換成不提場次的版本。
- ~~**模式選擇自成一張分卡（2026-08-13 使用者指示「這種選項要自己一個 section，參考建立商品」）**：「場次對應」（全場次共用一組／每場次各一組）由原本擠在「適用票種」標籤底下的 `.filter-tabs`，改成自己一張 `.bd-sec`，~~（同日第三輪再併回票種那一張，見上一條；`.filter-tabs → .radio-cards` 的換法仍然有效）帶標題與說明，控件換成 `.segmented.radio-cards`（帶標題與說明的二選一卡）。比照建立商品把「單選項／多選項」放進自己的 Variations 區塊——這種「先決定用哪一種模式、後面欄位跟著換」的選擇是一個獨立決定，擠在某個欄位的標籤底下會讀成那個欄位的附屬設定。**不用 `.filter-tabs`**：那是「篩清單」的語彙，這裡是在設定模式。「會長出幾組」的說明同輪從挑選器移進選項卡的 `.radio-card__sub`，挑選器只留「是哪幾場」。單場活動不出現這張卡。
- **收合列 `.bd-row`** ＋ **彈窗**（2026-08-13 使用者指示改成 popup）：清單上只有收合列（縮圖＋名稱＋內容摘要＋折後價，原價刪除線），點列或按新增才把編輯卡浮成彈窗（殼＝`payout-modal` ＋ `payout-dialog--wide`；三段表單是自己的區塊 `.bd-form`，套在 dialog 的 body 上——dialog 那個元素不再兼任第二個元件的根，否則 `.fc-bundle` 的卡片內距與下外距會被帶進 dialog 的 head／body／foot 版面）。就地展開時它會把底下已建好的列推到畫面外、讀起來像清單自己長高了；浮上來之後背景那份清單維持原位。用 `--wide` 不用 `--narrow`：票種挑選器在三場兩票種時要 500px 以上才排得開。彈窗一次只開一組，Esc 與 ✕ 都關得掉；**什麼都沒填的新卡關掉時直接丟棄**（否則清單會多一列「未命名套組 · 還沒有內容」，那不是使用者建的東西）。**新卡在送出前不先佔一列**，已存在的那些就算正在編輯也留著列。按鈕寫「完成」不寫「取消」——這支編輯器即時寫入狀態，沒有可以還原的取消，那會是一句做不到的承諾。
- **票價進了原價**：`ticketValue()` 取勾選的票裡**最貴的那一張**（多選時粉絲只挑一種，取最貴是保守估）。在此之前活動套組的原價只加商品，一組「VIP 票 ＋ T 恤」報的價把票本身漏掉了。折扣基準的爭議記在 ASSUMPTIONS BDL-002。
- **`summaryMeta()` 修條件**：原本「N 份作品」掛在 `!SHARES`（有沒有分潤名額），但「有沒有作品」歸 `WORK` 管——活動變體傳了 `work:false`，收合列仍寫著「1 份作品」。改掛 `WORK`，並補報「N 張票」。
- **`plain()`**：`cpp.bd.perks.sub` 前面有「· 」、`cpp.bd.discount` 後面有「 · %」，那是預設版型的排版分隔符；分段版型把它們各自放到自己的一行、且真的有 % 後綴欄，分隔符變成雜訊，所以就地剝掉而不是另建兩個 key（同一句話兩份文案會分岔）。還沒建立票種時放一列虛線佔位（`.fc-ref--placeholder`），把「這裡將來會有一張票」畫出來，而不是只留一句提示文字。同輪修掉封面圖的尺寸——上傳格要包在 `.upload-assets` 裡才有固定高，少了外層會沿著 `.field` 撐滿整列寬再依比例長到半個畫面高。**2026-08-04 新增活動變體的三個選配旗標**（使用者裁決「活動套組、募資套組、電子商店組合包是同一件事的三個變體」，所以不開第二支編輯器）：`work:false` 收掉「作品」本體與含作品份數（活動賣的是票＋商品，不是一份作品的份數）；`cover:true` 每張卡加一格封面圖（活動每一組賣的東西不同，卡與卡要靠圖分辨，走 Q40 的共用上傳格產生路徑）；`tickets` 收陣列或 getter，長出「適用票種」複選清單（`.bd-tickets`／`.bd-ticket`，勾選框用正典 `.zcheck`），有票種時 `isValid()` 要求至少勾一個——活動套組一定含一張票。**預購字彙（方案／預購者）改跟著 `work` 走、不跟 `shares`**：活動同樣 `shares:false`，套上預購講法會讓每個標籤都說錯話。三個旗標都不傳時行為與先前完全相同，募資兩個消費頁不受影響（已實測共創仍有名額、預購仍有作品與份數）。**電子商店組合包（`create-bundle.html`）仍是另一套實作**，尚未併進本模組（治理待辦）。 **`.fc-bundle-col`（2026-08-31）＝編輯器整欄的閱讀寬度 640px**：展開後是一組表單欄位，放在全寬的欄裡數字輸入框會被拉到 400px 以上；640 讓兩欄各約 310px，與建立流程精靈裡同一個編輯器的 584px 表單欄同一個量級。套在整個 section（標題、新增鈕、註記要跟卡片切齊），不是單張卡。消費頁：project-detail 方案分頁。。**2026-09-21（第二輪）分段兩欄版型 `layout:'split'`**（使用者看 `docs/bundle-create-demo-2026-09-21.html` 後裁示「做上正式」，建立活動第 6 步掛載）：彈窗改 `.payout-dialog--xwide`＋殼自己的 `.payout-dialog__split`（`.bd-split` 設 `--payout-split-side: 340px`；`.bd-dialog--split` 高度上限放到視窗高）——左欄三段分卡 `.bd-sec--stack`「內容 → 定價與數量 → 命名與上架」、右欄 Bundle preview card（§4.213）黏在上方、footer 粉絲實付（「從 $Y 起」）＋`.bd-foot-price__sub`「原價 $X · 最多 N 組」＋`.bd-foot-why`＋取消／完成；兩步彈窗在此版型退場。內容段：`.bd-sub`（小標＋提示）、票券表在勾到的票種剩餘不足一組時「剩餘」底下標 `.bd-tbl__short`「不足 n 張」、張數底下 insight-row 語意句（`.bd-sem`：「粉絲從 VIP、Floor 任選一種，拿到 2 張」／「鎖定 VIP…」）、多場時場次對應與適用場次／每場預覽表改成票券小節底下的欄位群（`.bd-field--table`）、商品搜尋 `.fc-pick` ＋「已加入」表 `.bd-tbl--items`（`.bd-tbl__cell` 名稱＋規格小字｜原價｜庫存｜`.bd-tbl__act` ✕；多規格商品「多規格 · 尺寸，粉絲購買時選」、各規格價不同時「從 $X 起」）、額外權益改一個 textarea 一行一項。定價與數量段：原價合計逐項算式 `.calc-list`（kv 列＋`__how` 一句怎麼算＋`__total`；2026-09-22 D311 自本檔的 `.bd-calc-list` promote 成共用元件 §4.217，本檔留墓碑）、組合折扣開關 → `.bd-disc` % 步進器＋`.bd-readout--big` 售價＋「粉絲省 $Z（12%）」、鎖定套數列（lock-sets 的 `.lockset__sets`，提示「最多 N 套（受「票券／商品」限制）」、超過 `.is-over`）、唯讀可售套數（寫出受誰限制）、不限量／限量＋上限步進器。命名與上架段：名稱由內容自動建議（「VIP／Floor 任選 × 2 ＋ 巡演 T 恤」）、使用者改過就不再覆蓋（`nameTouched`）；說明 textarea；`.bd-cover` 120px 封面格；上架只有一列 kv `.bd-listing`「跟著活動 {活動名}（徽章）」，沒有三開關（D293／D294）。已售出態（`sold > 0`，D292 裁決七）：票種、張數、場次對應、商品鎖定（`.bd-tbl--locked`、`.bd-lock-note`、搜尋框與 ✕ 收起），折扣／鎖定套數／上限照常可改、主鈕「儲存變更」。取消＝還原打開時的快照（新卡直接丟掉）。商品成員帶 `options`／`variants`（原價區間、預覽卡規格 chip）。募資兩頁仍傳 `layout:'sections'`，不受影響 **2026-09-22 文字階層重整（方案 A · 三層收斂；lab `docs/bundle-copy-hierarchy-lab-2026-09-22.html` 稽核三段 67 個文字元素，使用者裁示採純 A；五條規則見 STYLE-DECISIONS Q123）**：一段內最多三個文字層級（段標題 16 → 欄位標籤與小標 14 → 內文 12）。三段的 `.bd-sec__sub` 不再產生（段副標整層退場）；票券小節去掉小標 hint、「允許票種」`.field__label` 與表下規則 hint，表尾 `.bd-tbl__foot` 在 SPLIT 不畫（`layout:'sections'` 仍畫）；剩餘不足一組改用畫面表達——**新增 `.bd-tbl__row--short`**（整列 `--muted-foreground`，配方同 `--off`）＋ `.bd-tbl__short` 由小字改成 `.ztor-badge` 徽章；唯一的語意句由 `.insight-row.bd-sem` 降成 `.bd-tix-qty` 欄底下的 `.field__hint`（`[data-bd-sem]` 現在就是那個 hint 本身）。商品小節去掉小標 hint（「選填」降進搜尋 placeholder `cpp.bd.sp.search`、每組用量由表頭欄名承擔），已加入表表頭由「已加入」改 `cpp.bd.sp.items`；額外權益 hint 併進 placeholder。定價段去掉「原價合計」「折扣」「限量」三個 `.field__label`（合計列、% 後綴、選項值各自承擔）與關折扣時的 `.bd-nodisc` 句（footer 有同一個數字）；**新增 `.bd-disc-grid`**（`.form-grid` 的 `align-items:end`，補回拿掉折扣標籤後兩欄的基線）；**可售套數升成 `.field-readout.bd-readout--big` 並移到鎖定套數之前**，讀數 `cpp.bd.sp.sets.val`「最多 N 組」＋ hint `cpp.bd.sp.sets.why.*`「受…限制」兩層（**新增四把 key，墓碑 `sets.locked`／`.capped`／`.by`**），上限因此由三處收斂成一處；鎖定套數 hint 只剩「空白＝與單賣共用剩餘」、限量 hint 只剩「賣完自動售罄」（超過上限的錯誤句 `lock.over`／`cap.over` 保留完整數字）；單位「套」統一成「組」（墓碑 `cpp.bd.sp.unit.set`）。命名段去掉「上架」標籤、封面上傳格的 CTA（改 `aria-label`，欄位標籤已經說了「封面」）與封面側欄那句。i18n 同輪刪 18 把、縮短 10 把（墓碑註解在 `js/i18n.js` 的 `cpp.bd.sp.*` 段開頭）。同一套處置也套進 `create-bundle.html`（頁面級表單，另加電子商店專有的限時折扣與電影關聯兩句縮短）與 `bundle-detail.html`（同用詞、不重排） **2026-09-22 第二輪（使用者看線上多場次活動第 6 步彈窗：「title 和區塊一樣沒有分清楚」）——內容物段的標題分級與場次重排**：①小標 `.bd-sub__title` 與欄位標籤原本同字級同字色同字型，改用站上「群組小字標」既有配方（display 字型＋小型大寫＋0.04em＋`--muted-foreground`，同一彈窗右欄 `.bpc__title`、票券表場次組頭 `.bd-tbl__gname`、`.dropdown__cap` 同一支；字級照 Q123 停在 14、字重不動）；②**新增 `.bd-group`**（沿用 2026-09-01 三段式那支 `.bd-group` 的 hairline 分段：小標＋它統轄的欄位＝一個直排群組，群內 gap 12，群與群之間一條 hairline、線上 20（`.bd-sec--stack` 的 gap）線下 16；第一群緊接段標題，不畫線不留上內距；`.bd-group__title` 同輪收成同一支配方、收掉違反字重規則的 500），三個群組＝票券／商品／額外權益（額外權益由 `.field__label` 升成小標，否則會讀成商品群底下的一格）；③票券群順序改成小標 → 票種表 → 每組張數 → **「場次」**：多場活動的二選一卡（`scopeCardsHTML`）與適用場次表／每場預覽表合成同一個 `.field.bd-field--table[data-bd-dates]`（`datesFieldHTML`，標籤 `cpp.bd.sp.dates`「場次」），選「一組通用」接適用場次表＋「預設每一場都適用」一句、選「每場各一組」接每場預覽表＋「共 N 組」一句（原本那句「Day 1、Day 2、Day 3 各建立一組」與表重複，退場）；單場活動整格不出現。墓碑 `sessionsFieldHTML`／`perFieldHTML`。`create-bundle.html` 同步：額外權益升小標、選到的活動名改走 `.field__label`（它是值不是群組標，不該被轉大寫）。見 STYLE-DECISIONS Q123 補句 | [bundle-editor.css](./ds-components/bundle-editor.css) |
...
| Card group | 🔵 atom | ✓ App | 卡內分組三件組（Q71 定案 2026-08-25）：一張卡裡有多組資訊時，判準是「這塊需要多強的獨立性」，三級由弱到強——第 1 級（預設，本元件）＝次級標題或分隔線（`.group-title`＋可選 `.group-desc`、`.group-divider`），不加填色層；第 2 級＝1px 線框，沿用既有 `.control-group`（`control-row.css`），本檔不重複定義；第 3 級＝整層新填色，沿用 [Nest](#nest)，本檔不重複定義。另含比子組再高一階、服務整張卡的大標 `.card-head`（`.card-head__title`＋可選 `.card-head__desc`＋`.card-head__rule` 滿版線）。間距階梯：列與列 14＋14、小標→內容合併總距 32、組分隔線上下各 32（線出血到卡緣，讀 `--card-pad`）、卡頭線上 24／下 32。附屬 `.attr-value`／`.attr-link`＝卡內列式版面裡「純文字值＋底線文字動作」的搭配（取代黑底標籤）。demo-eshop-styles.html only as of 2026-08-25，**待套用**、尚無消費頁 | [card-group.css](./ds-components/card-group.css) |
| Card | 🟡 molecule | ✓ App | Section wrappers w/ head row across all product pages。圓角 `--radius-xl`（16px，Q16 2026-07-17；原 6px）。邊界 2026-07-26 由 1px 邊框改陰影浮起（Q23 決 C） **`.card--muted` 不帶浮起陰影（2026-08-13）**：坐在卡或頁面分組裡的分組面交出 `.card` 的 raise 陰影與上緣高光（`.card .card--muted, .form-section .card--muted { box-shadow: none }`），只留 `--nest-surface` 薄膜與亮色的 `--nest-line`——留著陰影會被讀成「浮在卡上的第二張卡」，與 Q66 的「疊上去的一層」是兩種語彙。消費頁：建立活動（場次盒、票務的一場一盒）、活動詳情（場次分頁的場次盒）、publish-work、auction-detail。 **Q66 全站落地（2026-08-13，使用者「都改」）**：新增 `.payout-dialog .card` → 薄膜。彈窗面板本身已經是 `--card`，裡面再放一張 `--card` 就是同色疊同色（粉絲 CRM 的分級設定彈窗 4 張）。 | [card.css](./ds-components/card.css) |
| KPI | 🟡 molecule | ✓ App | Dashboard summary, Earnings tabs, page KPI rows (headline metric set in display size, not colour)。變體：`--compact`（去 min-height、內距收小，側欄/摘要用，如商品細節頁 Sales summary）／`--tappable`（整塊是 `<button>`、開 in-place popup）／狀態染色 `--success`｜`--warning`｜`--destructive`（染 `.kpi__value`＝這個數字的**狀態**好不好）／**`--hero`（2026-07-27 新增）** 整張卡實色橘 `--primary`＝這張卡是本頁**主角**（編輯權重，與狀態不同軸）。**一列只給一個 `--hero`**，否則沒有主角。與 Q8-A 的「已選中」不衝突：已選中是 **tint**（14% 半透明）且只出現在 pill／nav 小控件；`--hero` 是**實色**且是一整張卡，而實色橘在本系統一直代表「主角／主要動作」（主 CTA、`.pager` 目前頁），語彙一致。卡內文字一律 `--on-primary` 深墨（**不是 `--primary-foreground`**——後者亮色是白字，白對 `#ffa33f` 只有 1.99:1；深墨 9.02:1）；label/meta 用同一支深墨壓透明度做層級（6.33:1／6.85:1），不另挑灰（灰在橘底會發濁）。`.kpi__delta` 在 `--hero` 內＝**深綠半透明底＋白字**（形狀語彙不動：一樣的 pill、一樣的內距、一樣是半透明染色，改的是染多深與字色）。理由：12% 淡染疊在飽和橘上色相幾乎不位移（合成 `#e9aa47` vs 卡的 `#ffa33f`），綠撐不起來、只能靠墨色暗示；改成深綠底之後「綠」由**底本身**承載，字換白，識別與對比同時變強。**仍然半透明**（90% 而非 100%），讓橘透 10% 上來把綠暖化一點，像長在這張卡上而不是貼上去的色塊——這也是它與先前被否決的「純黑底」的本質差異：那是中性黑、讀起來就是黑斑，這是**有色相的深綠**，一眼還是綠的。實作是在 `.kpi--hero` 這一層重新定義 fill／ink 四個 token（`--status-success-fill`／`-ink`／`--destructive-fill`／`-ink`），custom property 往下繼承，亮暗兩主題同時成立、不必比特異性。**墨色跟著「坐在什麼面上」走，不是跟著主題走**——實色橘卡在深色主題下依然是亮底，主題型 token 解不了它。選擇器寫 `.kpi.kpi--hero`（0,2,0）以贏過 `.card .kpi` 巢狀底色覆寫。首用：儀表板總收入（`js/components.js` `kpiTile()` 的 `hero: true`）。圓角 `--radius-xl`（16px，Q16 2026-07-17）。`.kpi[hidden]` 已顯式歸零 display（元件為 flex，會蓋過 UA 對 hidden 的預設；2026-07-30 補上——在此之前凡靠 `el.hidden` 收 KPI 的地方都藏不掉，例如活動詳情依活動類型收起不適用的指標） | [kpi.css](./ds-components/kpi.css) | **2026-08-28 使用者裁示（兩項，其中①同日稍晚再收斂，見 STYLE-DECISIONS Q96）**：① delta 膠囊移到金額右側同一行（`.kpi__value-row`），常駐只顯示百分比，對照期（vs last week）拆成 `.kpi__delta-ctx`、hover 才以不透明小浮層浮現——實測 span-3 卡內寬 208px、數字 115＋間距 8＋展開後膠囊 97＝220px，中英文都塞不下同一行，就地展開必然把膠囊擠到下一行而 hover 時跳位，所以改絕對定位、不參與版面計算；收起時仍留在無障礙樹（不用 display:none），讀屏才唸得到「跟什麼比」。這推翻 2026-08-19「delta 改回數字下方」那次裁決，成立關鍵是把字串拆成值與對照期兩段。**同日稍晚（Q96）儀表板收入卡連膠囊本身都撤除**——走勢圖的標記區間已標出同一個 +12.6%，卡上放兩次是重複；hover 展開對照期的 `.kpi__delta-ctx` 隨之全站無消費端，整組退場。`.kpi__delta` 膠囊元件保留，earnings 等 7 個靜態頁仍在用。② 帶 `.sparkline` 的卡改由走勢圖當貼底錨點，註腳的 `margin-top:auto` 相應收窄到「沒有走勢圖」的卡（兩個 auto 併存會把剩餘空間對半分、註腳浮到中間）。總收入卡同時撤掉「2 小時前更新」註腳——走勢圖已經講完最近的變化。
| Admin IP Bank table | 🟠 organism | ✓ SiteSpecific | Admin IP Bank 與 Reporting 的 Film／Owner 分配表與報表篩選列；共用 token-driven table wrapper、owner identity 與日期範圍操作列。**2026-07-31 使用者：`.admin-table__thumb` 改直式**——原 36×36 正方，改「鎖高算寬」：`height:36px`／`width:calc(36px * var(--img-portrait))`（≈29px，29×36），與 `.ztor-table__thumb` 同一個推導邏輯 **`--menu`（2026-08-20 · D212）**：列尾動作是浮動下拉（kebab）的清單掛這個修飾子——外框的 `overflow-x:auto` 連縱向一起裁，選單長出去只剩半截（最後一列必中）；它放行 overflow（外框與 `.ztor-table` 兩層都要——表格自己也是 `overflow:hidden`，只放行外框選單仍然出不來）、並把 840px 最小寬歸零（那個寬度是給欄位多得多的 Admin IP 銀行表用的），overflow 放行後外框圓角不再 clip 內容，所以首列表頭與末列自己補圓角貼齊內緣。做法沿用 `variant-table-wrap--menu`。消費頁：商店設定的尺寸指南與優惠碼兩張清單（`store-settings.css` 原本那條 `#sec-codes … min-width:0` 隨之退場、留墓碑）。**ztorUI 換裝（2026-08-28 Wave 3 B 級巡檢補修）**：`.admin-table-wrap` 外框改玻璃四件套（`--ztu-glass-bg` ＋ `backdrop-filter: blur(var(--ztu-blur-glass))` ＋ 1px `--border` ＋ `--shadow-card`/`--shadow-edge-top`，配方同 `.card` 黃金基準／Q77 裁決），取代原本的實色底＋單層 `--shadow-card`；巢狀 `.ztor-table` 維持自己既有的表頭／hover token 不變，實測（admin-ip-bank／admin-platform-fees／admin-video-review／store-settings）表頭與 hover 列在玻璃底上仍可讀。**`--fluid`（2026-09-02）**：表格坐在比整頁窄的欄裡時用（首個消費者：創作者活動管理頁的活動表格）——840px 最小寬收成 560px，左右儲存格內距同時由 `--sp-20` 收到 `--sp-12`（六欄各吃 40px 內距＝240px，在 700px 的欄裡光內距就佔三分之一，欄位反而被逼換行）；上下內距不動，列高是閱讀節奏、不隨欄寬變擠。  **Q112（2026-09-11）**：`--radius-xl` 外框內的列左右內距下限 `--sp-16`（首欄文字不壓在圓弧上；右側 icon 欄不動）。 **2026-09-21 限量的上限欄（spec 5.1.5.1 §2.8／§2.10、§7.2）**：`--stock.--limited` 與 `--preview.--limited` 各多一軌——商品細節頁的限量商品在庫存分頁多一欄唯讀「上限」（`.variant-cell--ro.variant-cell--total`，在庫欄右邊、56px），商品設定的逐選項組合定價表多一欄可改的上限（`.variant-cell--input.variant-cell--total`，84px）；顯隱沿用建立商品的 `.variant-cell--total` ↔ `--limited` 那一組、不另造；不限量商品不掛 `--limited`。編輯規則（§2.10 上限不得低於已售）：違反的格 `aria-invalid="true"`（shared.css 紅框），表格底下一行 `.field__error` 點名列；單一規格的上限在 Edition 卡的欄位（`.field` ＋ `.field__hint--fact`「已售 n」＋ `.field__error`）。Edition 本身建立後鎖定（D137），細節頁用 `.segmented.radio-cards.segmented--locked`（各段 `disabled`＋`data-view-safe`）只呈現。 | [admin-ip-bank-table.css](./ds-components/admin-ip-bank-table.css) |
| Alert | 🟡 molecule | ✓ App | Dashboard alerts panel (`--card`) + inline page warnings (`--row`) + page announcement (`--banner` — **ztorUI 換裝 2026-08-28 補熔接**：改玻璃四件套，配方同 `--bar`；同時拿掉 `[data-theme="dark"] .alert--banner` 那條 `background: var(--card)`，它權重 0,2,0 會蓋掉玻璃底，是儀表板公告條「沒改到」的真正原因) + notification bar (`--bar` — rounded + shadow, flush in E-Shop low-stock F2) | [alert.css](./ds-components/alert.css) |
...
| Filter tabs | 🟡 molecule | ✓ App | Secondary status filter pills with live counts, row below the primary tabs (E-Shop F3). Base = grey-muted active (orders, auction-detail); `.filter-tabs--brand` = soft-orange active + orange/bubble-less counts (E-Shop, paired with underline type tabs); `.filter-tabs--source` = active pill tinted with the item's own `--dot` colour + optional `.filter-tabs__dot` leading dot, colourless items fall back to `--primary` (Earnings F5 revenue-source filter, colour shared with chart line + source list). **ztorUI 換裝（2026-08-28 Wave 2b）取代上述所有已選態塗色**：已選態從「橘 tint 色塊」或「灰底」統一改成「中性玻璃殼（`--ztu-glass-bg` ＋ `backdrop-filter: blur(var(--ztu-blur-glass))`）＋ 字自體發光（`--ztu-orange-hi` ＋雙層 `text-shadow`）＋ 殼底 26px `blur(4px)` 光暈（`::after` 兩層徑向漸層）」，`box-shadow: none`、無底線。顏色一律走 `var(--dot, …)`：`--source` 變體用 inline `--dot` 帶自己的色，其餘頁面沒設就落回品牌橘，computed 值不變。連帶刪除 `.filter-tabs--brand .filter-tabs__item--active .filter-tabs__count` 與 `.filter-tabs--source .filter-tabs__item--active`（含 `:hover`）兩條舊覆寫。⚠ 已選計數規則必須寫在 `--brand` 那段之後（源碼順序決勝），否則 e-shop 的已選計數會退回 `--muted-foreground` 灰 | [filter-tabs.css](./ds-components/filter-tabs.css) **`--legend`（2026-09-01 使用者裁示）**：圖例即篩選的變體，用在圖表卡的「依方案看走勢」。每一項都是「色點＋文字」的預設樣子，**已選不套**基底的玻璃殼／字光／底部光——一排四項裡有一項發著光，會先被讀成「這一項比較重要」而不是「現在選的是它」。選中的訊號改由未選的 `opacity: .4` 承擔（hover 回 .7），整組 `justify-content: flex-end`。變淡只在真的挑了某一項時發生（呼叫端掛 `.is-filtered`）——預設的「全部」不算挑，那時候每一條線都畫著，把其餘圖例調淡等於說它們沒被畫。 |
...
| ~~Composer~~ | — | **已退場 2026-08-25** | 全庫只有 design-system.html 掛過 `<link>`，連產品頁的 `<link>` 都沒有——純展示、從未進入任何工作流程（與同名相近但獨立在用的 `post-composer.css` 是不同構想，不是誤植重複）。樣式已移除、檔案保留為墓碑；DS 頁的 demo 與 TOC 一併撤除。之後真要做拖放輸入卡請重寫，不要復活舊實作 | [composer.css](./ds-components/composer.css)（tombstone）|
...
| Global nav · sidebar mode | 🟠 organism | ✓ App | Display-mode alternative to the topbar (spec §6.9 / D016): 248px left rail (`.app-sidebar`, same `sidebar.js`) sits on `--surface-shell`; dropdowns → expandable groups（`.app-sidebar__group`，accordion，現役）。另保留 **section-label 變體**（`.app-sidebar__section-label` ＋子項平鋪）可切回。Driven by `data-nav-mode` (theme.js)。**D221（2026-08-24）新增、D222（同日）簡化、D223（同日）收合列再簡化**：帳戶子選單有一列可展開的語言列（`.app-sidebar__sub-link--lang-toggle`/`--lang`），刻意不用 `.app-sidebar__group` class（避免巢狀 accordion 互相干擾），改由 `js/sidebar.js` 的 `refreshLangRows()` 監聽 `ztor:lang-changed` 就地 patch，不整段 remount。D221 曾在此列另掛「預設」徽章與「去設定改預設語言」提示行，D222 語言收回單一概念後撤除；D223 再拿掉收合列的「預設語言」前置 label 與兩行 title/sub 結構，改成只顯示目前語言名稱＋chevron，縮排字級對齊 Profile／Settings／Payments 純文字列，並補 `aria-label`（`settings.lang.toggle-label`）維持螢幕報讀語意 | [shared.css](./shared.css) · [sidebar.js](./sidebar.js) |
...
| Data list | 🟠 organism | ✓ App | Recent earnings, transactions, payouts, products, projects (row-divider)。列圖示晶片 `__icon` 2026-07-20 三度修正後併入與 `.product-list__thumb` 同一組標準（52×52／`--muted` 底／1px `--border-soft`／icon 色 `--muted-foreground`），取消與 `.alert--card .alert__icon` 的尺寸家族關係（Q20）。註：`.project-list__icon` 曾同屬此家族，2026-07-24 project-list 改真圖縮圖後退出、不再是成員。**`.data-list__icon--sm`（2026-07-25 新增）**＝32px 小晶片（與同列 `.btn--icon.btn--sm` 等高）＋16px 圖示，用於「圖示是記號（＋／−／＝）而非內容縮圖」的列；52px 預設在那裡會讀成過大的框。**可展開子列已不歸本元件（2026-09-03／Q107，2026-09-04 改 L3 整組換層）**：`.data-list__row--child`（2026-07-25 建）連同它那顆縮排變數一起退場，CSS 原處留墓碑；清單型容器的可展開子列改在 `.data-list__row` 上疊 Row disclosure（§4.204）的 `.rowdis__child.rowdis__child--compact`，並在外層包一層 `.rowdis__group.rowdis__group--gutter`（階層改由整塊面板說，不再靠縮排）。本元件仍擁有列的版面（grid、列線、圖示晶片、金額欄）。**`.data-list__row--disabled`（2026-09-09 新增）**＝這一列還在清單裡、看得見，但不能被選；只淡化 `__row-main`（opacity .5），右側的狀態徽章與列上的連結維持原強度——不能被選的理由正是那顆徽章在說的話，連它一起調淡等於把理由藏起來。首個消費情境＝建立組合的候選商品清單（已下架的商品照列、標徽章、不可加入組合，但仍點得進商品詳情） **`.data-list__thumb--lg`（2026-09-11）**：與 `__icon` 同尺寸（52px、`--radius`、`--border-soft` 髮線）的商品縮圖，用在「這一列就是一件商品」的清單——首用訂單明細的品項列（圖來自 products-store 該 persona 的商品記錄，store 沒有的才退回通用圖示）。**`.data-list__group-label`（2026-09-11）**：把同一張清單分成幾段的小字標題列（表頭同款 caption：`--column-head-ink`、`--fs-12`、.05em 字距；上留 `--sp-16`、下 `--sp-6`、第一個貼頂），不畫組線、靠留白分段。首用商品／組合詳情的「設定概覽」——使用者裁示照設定分頁的區段順序分四組：基本資料 → 選項與定價（組合：商品與定價）→ 庫存與交付 → 上架與開賣；第四組只有一列時列名寫「目前狀態」、不重述組名。 | [data-list.css](./ds-components/data-list.css) |
| ~~Avatar stack~~ | — | **已退場 2026-08-25** | 從未被任何頁面的 markup 消費（元件巡檢報告判定）：`project-detail.html` 曾掛 `<link>` 但全檔搜尋不到任何一個 class，等於連了線沒接電器；死 link 已清除。樣式已移除、檔案保留為墓碑。之後真要做重疊頭像堆疊請重寫，不要復活舊實作 | [avatar-stack.css](./ds-components/avatar-stack.css)（tombstone）|
| Drawer | 🟠 organism | ✓ App | 右側滑出面板（`.drawer` 殼＋`.is-open` 開關 ＞ `__scrim`／`__panel` ＞ `__head`(`__title`/`__close`)／`__body`），用於不離開當前頁看詳情／歷史／說明；scrim/Esc/關閉鈕皆關，尊重減動效偏好。Token `--card`／`--border`／`--overlay-tint`／`--overlay-blur`／`--shadow-overlay`。Evidence／使用頁：earnings-sony（提領歷史／如何運作，由 `partials/finance-overview.js` 以 `data-drawer-*` 掛勾控制）。**2026-07-24 起 project-detail 不再使用 drawer**——該頁五個編輯／新增流程改用中央彈窗殼 `.payout-modal`／`.payout-dialog`（使用者裁示：編輯與新增一律走中央彈窗，不用側邊滑出） **變體 `--wide`（2026-09-22，D310）**：面板由 480 放寬到 880，給表格型抽屜用。**變體 `--full`（2026-09-23，B 反饋）**：面板鋪滿整個視窗（`width:100vw`，`border-left:0`），`__head`／`__body` 內距各放大一階，給「每格是一段文字」的大表用——先只改翻譯表（欄位 × 四語系，使用者：「翻譯表的 popup 要整頁，同時裡面的每一個欄位都要變大」），同輪使用者追加「（價格表）可以統一」，`js/publish-stage.js` 的 `ensureDrawer()` 改成翻譯表與價格表**都**用 `--full`。**`--wide` 現況（2026-09-23）**：全庫 grep 零消費（trans／price 兩個 key 都已改吃 `--full`），CSS 規則保留不刪，列為**退場候選**、待使用者確認後才走墓碑，不要自行刪除 | [drawer.css](./ds-components/drawer.css) |
| Pager | 🟡 molecule | ✓ App | 數字分頁列（`.pager` 容器＋`.btn.btn--ghost.btn--sm` 頁碼，目前頁 `aria-current="page"` 實色底＋`.pager__ellipsis`）。頁碼沿用 Button 元件、不另造控件。Token `--primary`／`--primary-foreground`／`--muted-foreground`／`--sp-*`。**行為住在 `js/pager.js`**（2026-08-10 從 `partials/finance-overview.js` 抽出）：`window.ztorPager.mount(getRows, size, nav)`，回傳 `render(page)` 供資料重畫後再呼叫。呼叫端只給「目前要分頁的 `<tr>` 陣列」的 getter、每頁筆數與 `.pager` 容器；模組只管哪幾列現在看得見，不接管資料。只有一頁時不畫頁碼列。Evidence／使用頁：earnings-sony（項目表 8/頁、提領歷史 6/頁，由 `partials/finance-overview.js` 呼叫）、earnings ＋ earnings-sony 的內容收益·貼文報告明細（5/頁） | [pager.css](./ds-components/pager.css) |
...
| List toolbar | 🟠 organism | ✓ App | 清單頁頭的兩層控制骨架：`.list-toolbar` 殼層工作列（實色 `--surface-shell`、四角 16px、固定 58px 高，左放主軸 tabs、右放 `.list-toolbar__actions` 動作群）＋ 殼層左右內距刻意不對稱——左 `--sp-8`、右 `--sp-20`（2026-07-26 使用者：建立鈕旁邊太擠）：第一個 tab 自己還有 14px 內距，左側視覺留白其實是 22px，右側動作群沒有內距，8px 會貼著邊。**右側主 CTA 一律用 `.btn.btn--primary` 基礎尺寸（36px／13px），不要加 `btn--sm`**——projects 原本掛了 `--sm` 變 28px，與 e-shop／my-ip／events 不一致，2026-07-26 修齊。`.list-status-row` 次層篩選列（透明、放 pill 篩選或 select；到清單的間距 `margin-bottom` 2026-07-26 依使用者指示由 24px 加大到 40px，訂單／取貨兩頁的頁內同構列 `.ord-list-controls`／`.pk-list-controls` 同步），`--end` 變體把篩選推到右側（projects 兩個下拉用）。2026-07-23 由 e-shop 頁內 `.eshop-list-topbar` promote（projects 為第二個消費者）；sticky 貼頂屬各頁捲動容器決定，留在頁面 `<style>`。詳見 §4.90。Evidence／使用頁：e-shop、projects。**2026-09-11 貼頂態改四角同圓角的方形卡**（使用者：「改成方形圓角就好了，並且要靠上方那條線對齊」）：r2.3 內容面板透明，「上緣切平＋兩側凹角接外框」沒有外框可接、凹角（`--ztu-shell-solid` 實色徑向漸層）變成星空上兩塊深色方角，整段退場；貼頂位置與側欄上緣同一條線，由 `shared.css` 把 `.main` 的 16px 上留白從 margin 改成面板內 `::before` 佔位承擔（Chrome 的 sticky 停不到容器 margin 之外、padding 也會被算進去） | [list-toolbar.css](./ds-components/list-toolbar.css) **Snap dock 的骨架要求（2026-08-01 放寬）**：`.list-dock` 內只有 `.list-toolbar` 是必要的，`.list-status-row` 選配——站上出現了第二種骨架（只有分頁、沒有次層篩選的工作列，首例 `earnings-sony.html` 的總覽／版稅），原本兩排俱全才生效，那種頁面只能各自手刻一份貼頂樣式＝同一視覺角色兩種做法。**貼頂時通知條讓位**：`.main:has(.list-dock.is-snapped) .alert-inset` 解除 sticky，讓它隨頁面捲走；沒有這條時 dock 貼在 `top:0`、通知條貼在 `top:16`，兩者高度不同，通知條下緣會從 dock 底下探出約 15px（e-shop 舊有現象，一併修掉）。 |
| Segmented | 🟡 molecule | ✓ App | 2/3-way text toggle；選中段自 2026-08-28 ztorUI 換裝起是 accent 漸層＋毛玻璃＋光暈＋白字（原「白色浮起 pill ＋橘字」已退場）。`--icon` 變體為純圖示段（2026-07-23 接收退場的 `.view-switch`，清單／卡片檢視切換）。**2026-09-01 收斂**：`chart.css` 原本有一份平行實作（`.segmented__item`／`--active` ＋ 自己的軌道），而它比本檔晚載入、會把這裡蓋掉，圖表卡的分段因此還停在換裝前的樣子。類名已全站改成 `.segmented__btn`（7 檔 55 處）、chart.css 那份撤除；圖表卡底的折線／長條切換也由 `.chart-card__foot-actions` 自畫軌道改成掛 `.segmented` ＋ `--icon` | [segmented.css](./ds-components/segmented.css) |
...
| PDP preview | 🟠 organism | SiteSpecific | 粉絲端商品頁的手機版鏡像（建立商品的即時預覽，D248） | [pdp-preview.css](./ds-components/pdp-preview.css) |
| Preview column | 🟡 molecule | ✓ Project | 建立流程「表單｜預覽」兩欄版面＋右側 sticky 預覽欄。**`.preview-split--narrow`（2026-08-10 使用者指示）**：右欄由 320px／間距 40 收成 **264px／間距 24**，數值直接取自建立活動在用的 `.wizard-split--narrow`——兩個建立流程並排看時右欄本來一寬一窄，收斂成同一組數字、不另立第三種寬度。首個消費頁＝create-product；其餘消費頁維持預設寬度（標題＋灰副標＋Preview card）；取代滑出式 Preview panel。**卡邊界 2026-07-21**：`.preview-col .card` 與 `.preview-col .preview-card` 改 `border:0` ＋ `--shadow-card, --shadow-edge-top`，與左欄 `form-section--outlined`（Q14／Q18 已去邊框改浮起）一致——原本左欄無邊框、右欄 1px 邊框，同一畫面兩種卡邊界（使用者裁示）。scope 限 `.preview-col` 內，全站其他 `.card` 仍照 Q3 用 1px 邊框、未推翻 Q3。消費頁＝create-product／create-bundle／create-auction（各 2 個盒子）＋create-campaign（僅預覽卡），4 頁共 7 處。**2026-09-11 三個新修飾詞**（首個消費頁＝電子商店 F5 商店預覽）：`--toggle`＝可開關（沒 `is-open` 就單欄、右欄不佔位）；`--tall`＝預覽比視窗高時欄限高在視窗內自己捲（sticky 欄釘頂時下半截原本永遠看不到）；`--dim`＝分隔線（`__head--rule`）以下的內容區鋪畫布色 24% 暗膜（同日兩輪：「背景要暗一點點」→「標題那塊用原本的玻璃質感背景，內容那塊用現在的背景」），卡本體與標題列維持玻璃；內容區負外距出血到卡邊、下緣跟卡的圓角。同輪 `--phone` 的欄寬改走 `--preview-col-w`（預設 270）：電子商店與商店設定 2026-09-12 起覆寫成 394＋zoom 1（鏡像 1:1＝360＋卡內距 32＋邊框 2；前一天曾是 320／0.8），建立商品維持 270／0.75，不另立第三個修飾詞；`--tall` 的卡捲軸藏起（古典捲軸會吃掉 16px 把版型壓成 344）。**`--tall` 二改（2026-09-11 使用者：「範圍底部不要破窗」）**：捲的改成卡的內容而不是欄（卡是 flex 子項 `min-height:0; overflow-y:auto`），四角與邊框永遠在視窗裡；實際高度由 `partials/preview-col.js` 依欄當下離視窗頂的距離寫成 `--preview-col-max`（純 CSS 的 100vh 算式只在釘住時對，欄還在自然位置時會多出頁首那一段）；消費頁 e-shop／store-settings 都載這支。**同日：卡內標題列釘頂**（使用者：「這塊滾動時可以 fix」）——`--tall` 卡裡的 `__head--rule` sticky 在卡頂（負外距吃掉 padding-top 再補回）、82% 畫布色＋模糊底；高度由 preview-col.js 量成 `--preview-head-h`，卡內其他要釘頂的東西（fan-store 分頁列）以它為 top | [preview-column.css](./ds-components/preview-column.css) |
| Preview panel | 🟠 organism | ✓ App | 右側畫面分割面板承載即時預覽——壓窄 wizard、非浮層（§5.2.5） | [preview-panel.css](./ds-components/preview-panel.css) |
| Fan store preview | 🟠 organism | ✓ App | See-as-fan 內的粉絲端創作者頁（E-Shop F5、商店設定 F1 與三個細節頁預覽面板共用 `partials/fan-store.js`，§6.7 同源）。**2026-09-11 依使用者提供的粉絲端實際畫面重做成手機版鏡像**（做法同商品頁鏡像 §4.22g／D248）：版型與比例照粉絲端搬進來、整支 `zoom: 0.75`、欄寬 270px、不畫手機外框；app bar／底部導航／「情境展示」浮標不進預覽（使用者劃掉）。結構：`.fan-store__page` ＞ `__profile`（頭像＋名字＋身分・所在地・追蹤數＋分享；**所在地 2026-09-14 新增**（D269，設定頁個人資料併入店面）：`__location`＝map-pin＋文字、與身分同一條分隔線節奏，店面沒填就不畫）→ `__bio`（兩行＋更多）→ `__socials`（ztor／IG／Threads／X／TikTok）→ `__ctas`（加入社群＋彩蛋解鎖）→ `__nav`（商店・活動・排行榜・貼文・項目）→ `__featured-row`（精選商品橫列）→ `__tabs`（商品・套組・競標）→ `__grid`（雙欄卡：圖＋`__badge--new/--limited`＋`__heart`＋名稱＋價格）。顏色與間距只用 token——舊版 scoped `--fst-*` 寫死色盤（25 處裸色）退場，§1.5 的 fan-store 裸色例外不再適用。資料：人格決定頭像／名字／身分／追蹤數／簡介，商品卡問 `ProductsStore`（名稱、清單同一套幣別寫法、圖），卡序固定（`CARD_IDS`）供 E-Shop 依上架／顯示狀態收起。`window.ZTOR_PARTIALS.fanStore` 為 getter（讀取當下才組）。**2026-09-11 資料改問 `js/store-profile-store.js`**（頁面有載的話）：頭像／名字／身分／所在地／簡介／社群圓鈕（有填網址的平台才出現、順序照店面設定、ztor 固定第一；可加平台 2026-09-14 多一個「個人網站」＝`world` icon，D269）都從店面資料畫；示範資料仍走 i18n key、使用者打的字用字面值；`storeprofile:changed` 時整份重畫（`remountFanStore`，保留 e-shop 收起的卡）。加入社群／彩蛋解鎖兩顆 CTA 在預覽裡隱藏。**2026-09-11 分頁列重量＋釘頂**（使用者：「指出的部分樣式和原本不同」「下滑的時候有東西置頂在上面」）：`__nav` 改照粉絲端 `.glass-tabs` 量——48px 膠囊、padding 4/10、`--ztu-film` 底、`--ztu-rim-soft` 邊、blur(8px) saturate(1.5)；項目高 40、字 14、不換行（放不下橫捲、捲軸藏起）；選中＝`--primary` 粗體＋只在頂端一線的淡橘漸層底＋左上／右下兩道橘色內光（粉絲端 `--lg-fill-active`／`--lg-shine-active`，以 token 的 `color-mix` 表達）。分頁列住在 `__stick`（sticky top 0）；`partials/fan-store.js` 看到它貼到捲動容器頂緣（往上找第一個會捲的祖先，沒有就是視窗；比對線含容器 padding-top）就加 `.is-pinned`：上方長出 `__minihead`（24px 小頭像＋13px 粗體名字），底下 `::before` 一片帶（出血 12/16/24、70% `--ztu-canvas`→透明、`--ztu-blur-shell` 模糊、下緣 mask 淡出）壓暗滑過的內容——即粉絲端 `.tabstick-host.is-pinned`＋`.creator-minihead`。提案欄位（追蹤數／社群／加入社群／彩蛋解鎖／精選／收藏／分頁列）見 ASSUMPTIONS UIA-026 | [fan-store.css](./ds-components/fan-store.css) |
| Pickup management | 🟠 organism | SiteSpecific | 現場 QR 核銷工作區（spec 5.1.5.11 · D111），E-Shop 下與訂單管理並列。**場次詳情第一屏＝沿用 product-detail 的骨架**（2026-08-01，使用者裁決「以 E-Shop／建立商品用到的設計為主」）：分區卡＝`.form-section.form-section--outlined`、兩欄＝`.form-grid`、區段標題＝`.form-section__head`＋`__title`、大數字＝`.bento` 內的 `.kpi.kpi--compact`（`__label`/`__value`/`__meta`）、鍵值摘要＝`.kv`、地點時間＝既有 `.pickup-detail__meta` 加新的 `--stack` 直排變體。**`.pickup-board` 只補那套沒有的三件事**：欄間 1px `--border` 分隔線（`.form-grid` 只給欄不給界線，≤860px 轉上緣線＋堆疊）、`__stage`（隨場次狀態換的那一段，下緣 `--border-soft` 與交付列分開）、`__bar`＋`__pct`（量條與百分比同列，量條吃 `.stock-bar`，百分比是註腳小字＋`tabular-nums`）；另有 `__acts` 收尾動作列。分工固定——左欄＝領取的活動與物品，右欄＝工作人員操作取貨要知道的。**`.scanner-hand`**（同日 promote）＝唯讀「欄名＋值＋複製」的一列（`__row`/`__k` 固定寬對齊＋圖示複製鍵，`code.is-unset` 表示未設密碼）；**值的外觀與 `.scanner-access__url code` 共用同一條規則**，不另定義第二套，只負責唸出去與複製；重設網址／停用／改密碼已移進建立編輯彈窗。**名單** ＝ `.product-list--roster`（`product-list.css` 的欄位模板，非 `.ztor-table`），`pickup.css` 只留 `__code`（領取碼，等寬對齊）與 `__none`（—）兩個小工具 class。**2026-09-03（D240 一物一碼）**：名單改成**一列＝一個領取單位**（一件商品一個領取碼）——「數量」欄退場（一列本來就是一件，`__num` 同日墓碑）、新增「領取碼」與「來源組合」兩欄，欄數八改九；買多件的買家收成一列可展開的群組列，**直接沿用 `product-list.css` 既有的 `__row--group`／`__row--child`**（與活動系列清單同一套，見 §4.117），本輪沒有新增第二種分組列做法、無新元件。人工核銷與反轉的操作對象跟著改成單一領取單位。`.scanner-access`（F6 完整設定卡：`__qr`/`__main`/`__label`/`__url`/`__actions`/`__pw`）＋`.qr-box`(`--lg`/`--disabled`，faux QR via `window.ztorFauxQr()`) 保留給獨立 scanner 頁。另有 `.pickup-detail__header`/`__meta`、`.pickup-stats`、`.pickup-select`＞`.pickup-select__row`(`.is-checked`·`__box`/`__img`/`__text`)。共用建立場次 popup＝`partials/pickup-session-modal.js`（F3，亦掛 create-product.html／product-detail.html）：**2026-08-01 由三頁籤改為兩步流程**（`.pks-step`＋`.pks-step__intro`，`[data-pks-form]` 保 `min-height` 免兩步高度落差）——step 1「這場是什麼」含選填掃碼密碼與（編輯時才有的）掃碼網址重設／停用，step 2「可以領什麼」；刻意不放 stepper，位置由 dialog 副標與上一步／下一步表達。result step（QR＋URL）撤除，建立成功由 `hooks.onCreate` 落地到場次詳情頁。場次清單／可核銷項目／核銷紀錄重用 `.data-list`；核銷紀錄改開 `.payout-dialog` popup。camera 視窗用 `--surface-inverse` role token、無裸色 | [pickup.css](./ds-components/pickup.css) |
| Mobile scanner | 🟠 organism | SiteSpecific | 獨立手機 scanner（spec 5.1.5.14 · D111/D121/D122/D240）——獨立 URL、無主工作台導航。**2026-09-03 一物一碼（D240）**：核銷單位改成「領取單位」——一個領取碼一件，結果 bottom sheet 只顯示掃到的這一件（買家／來源訂單或票號／商品名／選項／來源組合，明細吃 `.scanner-buyer`＋`.scanner-result__rows`），主按鈕＝核銷這一件；確認後回相機連續掃描，相機分頁頂端的 `.scanner-progress`(`__icon`/`__body`/`__row`/`__name`/`__count`/`__note`) band 常駐顯示該買家在本場次已領幾件、還剩幾件。名單分頁改按買家分組（父列＝買家＋已領 M／N，子列＝一個領取單位，吃 `.rowdis__child.rowdis__child--compact`＋`.scanner-rosterrow`；2026-09-03 Q107 收斂後縮排即元件預設的一階 32px，`scanner.css` 原本那條同值覆寫已移除）；頂部計數與項目統計改由領取單位加總；訂單來源的單位讀 `js/orders-store.js`(`unitsOf`)，票券與已取消（作廢）示範單位寫在頁內。**退場**：`.scanner-claims`＞`.scanner-claim`(`__name`/`__opt`/`__qty`)（一次核銷整筆時代的多件清單）已移除，CSS 留墓碑註記。**2026-08-19 流程重規劃**：相機即主畫面、掃描結果改框內 bottom sheet（確認後直接回相機，支撐連續掃描）、密碼閘門先亮場次身分＋選填工作人員標示、頂部待核銷/已核銷即時計數、底部三分頁（掃描/名單/項目）。`.scanner-page`（grid `1fr auto 1fr`：手機框恆置中，原型情境側欄佔右軌不影響置中）＞`.scanner-frame`＞`.scanner-top`(`__name`，解鎖後才出現)＋閘門（`.scanner-brand` ztor 標記＋`.scanner-pw`(`__err`)＋`.scanner-session`(`__name`/`__meta`)）／`.scanner-lockout`(`__icon`/`__title`/`__count`/`__body` 五次鎖定)／`.scanner-counts`＞`.scanner-count`(`--done`)／`.scanner-body`（`.scanner-progress`(`__icon`/`__body`/`__row`/`__name`/`__count`/`__note` 買家進度 band，掃描分頁頂端)＋`.scanner-cam`(`__reticle`/`__line` 掃描動畫，respects `prefers-reduced-motion`)＋名單/項目吃 data-list（名單列＝`.scanner-rosterrow` 可點帶入代碼、項目進度吃 stock-bar）＋覆蓋層：`.scanner-scrim`(＝`--overlay-tint`)/`.scanner-sheet`(`--ok`/`--warn`/`--bad`·`__band`/`__body`/`__foot`；內容件 `.scanner-buyer`/`.scanner-result__rows`(`__row`))/`.scanner-flash`(`__inner`)/`.scanner-block`(`__inner`/`__title`/`__body` 未開始/已結束擋核銷)/`.scanner-offline`(`__queue` 離線保留，呈現假設 SCAN-002)）＋`.scanner-tabbar`＞`__btn`(`.is-on`)。原型情境側欄＝`.scanner-demo`(`__title`/`__note`，prototype-only、非產品 UI)。**2026-09-08 相機示意照片＋名單可讀性（使用者反饋）**：相機區在對準框背後鋪一張靜態示意照片——`.scanner-cam__feed`（絕對定位墊底，`images/scanner-cam-feed.jpg` ＋一層 `--surface-inverse` 的 `color-mix` 漸層暗紗，無裸色；正式產品換掉這一層即為真實相機串流），`.scanner-cam` 自己噴 `--surface-inverse` 當底，照片沒載入時退回原本的深色相機意象。照片恆為暗色，故區內文字改吃 `--foreground-on-inverse`／`--foreground-on-inverse-muted`（`.scanner-cam__hint` 與 `.scanner-manual`；後者另補半透明 `--surface-inverse` 底與 on-inverse 邊框，是 `.scanner-manual` 自己的組合、不改 button.css）。名單五項調整：群組列的通用人形圖示換成共用元件 `.ztor-avatar.ztor-avatar--sm`（姓名首字，票券群組維持 `.data-list__icon--sm` 票券晶片、同 32px）；右欄主行改成「還剩幾件」（`.scanner-rostercount`，沿用同檔 `.scanner-count b`／`.scanner-progress__count` 的 display＋bold＋tabular-nums 做法），`已領 M／N` 退成註腳（`.scanner-rostercount__sub`），整組領完主行改 `--done`（退回內文字級轉灰）；子列在 `.rowdis__lead` 放組內序號 `.rowdis__num`（同 pickup-detail 的 Q109 做法，欄寬以 `--rowdis-lead-w: var(--sp-32)` 對齊識別圓）；已核銷子列 `.scanner-rosterrow--done` 降一階，整組領完的群組 `.scanner-rostergroup--done` 整塊降一階（只動透明度，L3 面板填色／邊框／面板內 hover 一條未改）；群組列的來源編號 `.scanner-rosterhead__src` 弱化到 `--fs-11`。無新元件（全部重用 avatar／row-disclosure／data-list／badge）。掃描分頁用 `.scanner-screen--cam` 去掉內距讓相機滿版。`scanner.html` inline controller | [scanner.css](./ds-components/scanner.css) |
...
| Readiness card | 🟡 molecule | ✓ App | 上架前就緒檢查＋還差幾項 banner。footer 兩種掛法共用同一個 `__pop`（hover/focus 浮出完整 readiness 卡當 tooltip）：`__chip`（貼 footer 主動作的常駐就緒 chip，`--ready` 轉綠；create-auction／publish-work）與 `__anchor`（包住主動作按鈕本身、footer 不留常駐 chip；create-product，2026-08-31）。publish-work 只在最後一步顯示，因為送出只發生在那一步；create-campaign 另有自身 pill 變體，待後續收斂 | [readiness.css](./ds-components/readiness.css) |
| Post composer | 🟡 molecule | ✓ App | 發文框（2026-08-18 promote，自 `progress-timeline.css` 的 `.ptl__composer*`）：**一個發文框、三個落點**——進度時間軸「今天那一格」的內嵌版（`.composer__stub` 收合成一行提示，點了才加 `.is-open`）、「發布更新」彈窗，以及**總覽「進度摘要」卡最上面的收合入口**（2026-08-31 使用者裁示，同日三版定案：先卡頭底下 → 獨立區塊 → 收回卡裡擺最上面）。第三個落點只放收合那一行（`.post-composer--inset`：`.card .post-composer` 整組去面之後，這一行得補回薄膜底＋圓角才讀得出是輸入框），placeholder 用自己的 key `pd-ov.post.stub`（時間軸問「發生了什麼事」、這裡問「要不要發一則更新」）。點了把同一個活的發文框搬到它下面展開——不另做一份欄位。**這個落點沒有位置訊號**，所以類型選「里程碑更新」時要把里程碑下拉打開讓人自己選（`composerHost` 旗標控制；送出時 `patchMilestone` 讀下拉的值）；時間軸的落點仍由位置回答（D197），下拉維持收著。**換分頁要把它還回時間軸**：分頁切換不重繪，不還的話進度分頁那一格會變成空的。兩邊長一樣、行為一樣，差別只有**有沒有位置訊號**：軸上的落點本身就是發文類型的預設值（D197），彈窗沒有位置所以類型與要完成的里程碑都得問一次。promote 的理由——留在時間軸檔裡的話，彈窗要用就得跨元件借樣式，而借來的樣式沒有人負責，改一邊會靜默弄壞另一邊。組成：`__stub` ／ `__head`（`__who` ＋ `__avatar` ＋ 靠右的受眾下拉）／ `__fields`（類型、標題、內文、`__media`［`--image` 160px ／ `--video`・`--audio` 360px］，每塊自帶 `__media-head` ＋ `__media-drop` ×）／ `__foot`（`__tools` 夾帶入口 ＋ `__acts` 取消與送出）。**一則只能一種夾帶**：三種的版面、播放行為與粉絲端呈現都不一樣，允許混用等於要為每一種組合各定一次規則。**名字不叫 composer** 是因為站上已有一支同名元件（AI 產圖那類「拖放或輸入 ＋ 底部工具列與 credit 計量」的輸入卡，根 class 也是 `.composer`）——兩支做的事不同，同名會在 CSS 與 design-system 錨點上直接對撞。**面跟著 `.card` 走**（2026-08-31 換裝補課）：原本寫死 `--card` ＋ 卡陰影，用意是「跟站上的卡同一個面」；換裝時 `.card` 改玻璃、這支沒跟上，拆成獨立區塊後與進度摘要卡上下並排就露餡。改引用 `--ztu-glass-bg` ＋ `--border` ＋ 卡陰影，並補浮層／`html[data-embed]` 的薄膜規則（同 `card.css`／`kpi.css`）。**疊在一張卡裡時整組去面**（`.card .post-composer`）：它是那張卡的內容、不是卡中卡，留著面會在卡裡再畫一個框；`max-width` 一併解除（560px 是給時間軸落點限的行長）。消費頁：project-detail（進度分頁的內嵌發文框＋總覽進度摘要的收合入口） | [post-composer.css](./ds-components/post-composer.css) |
| Fact list | 🔵 atom | ✓ App | 唯讀事實列（2026-08-19 建）：左邊欄目名、右邊值、一項一行、髮絲線分隔（`.fact-list` ＞ `.fact-list__item`，兩個 `<span>` 分別是名與值）。給詳情彈窗這類「回答問題、不能編輯」的內容用。**不用 `.field`**——表單標籤是「等你填」的語彙，唯讀內容穿上它會讀成一張被停用的表單；**也不用 `.data-list`**——那支給「一列一個東西、可能可以點進去」的清單，有 40px 圖示磚與較大行高，事實不是物件，八項排出來每一項都被放大成獨立的東西。與 `.review-status__meta`／`.ptl__meta`（行內流式、適合一兩項附註）的分工：本支一項一行、值靠右，適合五項以上要逐行對照。消費頁：project-detail（更新詳情彈窗） | [fact-list.css](./ds-components/fact-list.css) |
...
| Link field | 🟡 molecule | ✓ App | 系統產生、不可編輯、要能整條帶走的連結：唯讀等寬輸入框吃滿寬、行尾複製與重置、底下一行說明。首個角色＝隱藏品項的非公開連結（§7.14）。回饋長在按鈕自己的字上（`--copied` 同時讓整列亮 1.6 秒），不跳 toast。行為走 `js/link-field.js` 的 `data-linkf` 契約；重置只發 `linkfield:reset`，新連結由頁面算。`vault-share` 的 `.vshare__link` 是先行 pattern，這一輪不動。詳見 §4.206 | [link-field.css](./ds-components/link-field.css) |
...
| Product list | 🟠 organism | ✓ App | E-Shop inventory table: product identity + status + stock + visibility + edit action, borderless row-divider layout。變體：`--eshop`/`--bundles`/`--auctions`/`--orders`/`--pickup`/`--ip`（2026-07-20 新增，spec 5.1.4 §F6：IP 清單欄位，8 欄——icon／名稱＋標籤／權利資訊／租出數／收入／租金／Mktplace 開關／Manage；`my-ip.html` 由原本 `.data-list` 卡片式清單改用此變體，對齊 e-shop 表格版面，同時把原本擠成一行的 meta 字串拆成獨立欄位）；拖曳握把 `__drag`（grip，抓它才重排）＋抬起態 `.product-list__row.is-dragging`（抬升陰影＋置頂，僅 --eshop Products 分頁；跟手 transform 由頁面 JS 控制）。草稿列（`[data-status="draft"]`）握把隱藏、不可拖曳（無粉絲端陳列順序），頁面 JS 置頂。列縮圖 `__thumb` 統一成描邊框，對齊 `__image--placeholder` 的**真實呈現值**：76×76（2026-07-26 使用者裁示：全站清單縮圖統一 76px，原 52×52）／`--muted` 底／1px `--border-soft`／icon 色 `--muted-foreground`（2026-07-20 二次修正——首版誤對齊 `__image` 從未單獨呈現過的基礎規則，已用 Playwright 逐項核對訂單/取貨與電子商店一致，Q20）；反白變體 `__thumb--cover` 邊框同色不露描邊。**2026-07-31 使用者：`.product-list__image` 改直式**——寬度維持 76px、高度由 `--img-portrait` 推導（不再從直式原圖裁一塊方形），列高隨之從 116 自然長高。**2026-08-05（D176／Q52）**：比例值由 750/930 改 `2 / 3`，縮圖成 76×114、列高 154；同日活動清單的方形例外撤除——原本 `.product-list--events .product-list__image` 覆寫回 `1 / 1`，使用者裁示改直式後整條刪除，**八支清單縮圖現在沒有例外、全部同一個比例**（原例外理由「活動素材有直有橫」不成立：清單取的是直式主視覺，橫幅另有 `event-preview-card--landscape` 版位）。取貨頁 QR 圖示用的是不同 class（`.product-list__thumb`，Q20 icon-chip 家族），不受本次收斂影響，仍是正方 chip。**2026-08-04 新增釘選區**（`__divider` ＋ `__divider-body` ／ `__divider-count` ／ `__pin-mark`，規格 5.1.5 F4／D172）：分隔線以上＝釘選並顯示在商店最前（上限 10），跨線拖曳即釘選／取消釘選，不掛粉絲端徽章，詳見 §4.26。**2026-08-06 新增橫向捲動包層 `.product-list-scroll`**（八個消費頁全數套用）：清單列有 7–9 欄、最窄 830–1160px，而 app 外殼的 `.main` 是 `overflow-x: hidden`，視窗一窄右側的「狀態」「操作」幾欄會被切掉**而且捲不到**——每一列的 ⋮ 在那個寬度區間整個點不到（既有 ≤760px 堆疊版型只救得了手機，760px 到各變體最小寬之間是漏的，筆電常見的 1280–1440 就落在裡面）。做法沿用站上既有的表格橫捲（`.ztor-table-scroll`／`.admin-table-wrap`／`.variant-table-wrap`）；`min-width: min-content` 讓清單撐到欄寬加總那麼寬，欄數少的變體不會長出捲軸。上下的 `padding-block` 是給 `:hover` 浮起陰影讓位（`overflow-x` 一旦不是 `visible`，`overflow-y` 也會被算成 `auto`），用等量負外距抵銷、版面高度不變。**2026-08-06 新增群組列**（`__row--group`＋`__group-chevron` 母列／`__row--child` 子列，見 §4.117）；同日新增頁內變體 `--series`（系列母頁的場次表，7 欄，欄數為該頁專屬故與 `--events` 同法留在頁內）：一筆清單項目其實是好幾筆時收成一列、點開才展開；首次用於活動的系列場次。**2026-09-03（D240）**：`--roster`（取貨場次詳情的名單）欄數由 8 改 9——「數量」欄退場，新增「領取碼」與「來源組合」；群組列第二個消費場景上線（買家分組，`pickup-detail.html`），元件本身不動、只是被重用。**2026-09-04（D241 · spec §7.14）**：`--eshop`／`--bundles` 的 `__status` 改 flex＋wrap，因為狀態欄現在可能同時掛兩顆徽章——一顆講販售軸（售罄／販售結束／即將開賣），一顆講顯示軸（已隱藏）；「隱藏但照樣在賣」是規格裡真實存在的組合（私下販售），一顆講不完，欄寬不夠就換行（做法與 `--auctions` 既有那條相同，不另立第三種）。同輪新增 `__stock-note`：庫存數字下面的降級註記，只在「所有販售管道都設了鎖定」時出現（那時沒有被鎖定的庫存量沒有人能賣，要提醒創作者重新分配）。狀態文字與徽章色不再由清單頁自己決定，統一問 `js/listing-state.js` 的 `STATUS_META`／`badgeClass`（八態，五個頁面共用一份）**2026-09-22（使用者反饋）**：`--eshop`／`--bundles`／`--auctions` 狀態欄的「已隱藏」副徽章拿掉——清單列右側已有「顯示於商店」開關講同一件事，副徽章是重複；狀態欄現在只剩單一販售軸徽章，`flex＋wrap` 留著純粹是垂直置中單顆徽章用（見 [product-list.css](./ds-components/product-list.css) 對應註解）。篩選 tab 的「已隱藏（Hidden）」與計數不受影響。 **2026-09-04（使用者裁示「這個間距不需要這麼大就縮短」）**：清單縮圖 `__image` 由 **76×114 收成 60×90**（寬度改 60，高度仍由 `--img-portrait` 2:3 推導），列高隨之由 154 變 **130**；上下留白維持 `--sp-20` 不動——真正撐高列的是直式海報而不是留白。縮圖欄的欄寬跟著收：原本 84px 的（`--eshop`／`--bundles`／`--auctions`）收成 68、原本 76px 的（`--ip`／`--rented`／活動清單）收成 60，各自維持原來的 gutter。**`--pickup` 的欄寬維持 76**：那一欄放的是 `__thumb`（76px 方形 QR 圖示晶片、刻意不縮），不是會縮的 `__image`。**分隔線改 `::after` 直線（2026-09-11）**：列為了 hover 浮起先備好 `--radius-lg`，`border-bottom` 會跟著圓角在兩端彎起來（使用者：「預設狀態是不要圓角的」）；改由絕對定位的 1px `--border-soft` 線畫，hover／上一列被 hover／最後一列／釘選方框上一列改隱藏 `::after`。 **2026-09-11 兩端固定欄（使用者指示）**：在 `.product-list-scroll` 內，握把（第一欄）與動作欄（最後一欄）`position: sticky` 各釘在左右緣、表頭同欄位跟著釘；中間的欄先依 minmax 縮到各自的最小寬，min-content 到底才長出捲軸，內容從動作欄底下滑過。被遮那一側的內容**漸變透明**（同日第四版，前三版都失敗於同一件事：釘住的格自己沒有底、底下是會動的星空，凡是在格上畫東西——實色底、毛玻璃、漸進模糊——都是一塊看得見的方形；在容器上掛 mask 又把釘住的格一起 mask 掉）：mask 掛在**中間每一格自己身上**，一條 `linear-gradient` 由 `partials/list-scroll.js` 算好的變數定位——容器上的 `--pls-lx`／`--pls-rx`（淡出帶起點＝釘住格內緣＋scrollLeft；該側沒被遮就寫 ±99999px 推出視野＝關掉）與 `--pls-fade`（帶寬 `--sp-56`），每格自己的 `--cell-x`（在清單本體裡的 x，只在版面變時重量）；格的 mask 座標＝容器座標減 `--cell-x`，同一條 gradient 對每格都對。捲動只改容器兩個變數，格的 mask 由瀏覽器重算。釘住的格、星空與 hover 玻璃都不在 mask 裡，真正淡掉的只有滑到底下的圖與字；同支 js 也依捲動位置寫 `data-scrolled-left`／`-right`，沒被遮的清單不掛 mask。釘選分隔列 `__divider` 整條 sticky 釘左緣、寬度＝看得到的那一段（`--pls-w`＝容器 clientWidth，js 寫上），「立即釘選」永遠在右端（使用者：「要隨著寬度縮放，立即釘選要一直在右邊」）。**橫捲拉桿 `.product-list-scrollbar`**（使用者：「當有遮蔽時，下方要有一條拉吧，粗一點好拖動，上面要有點點矩陣紋理，像盲人點字」）：清單自己的捲軸在整張表最底下搆不到，js 在容器後面補一條只有橫向捲軸的代理容器（內容寬＝清單本體、兩邊 scrollLeft 互相同步），sticky 在頁面捲動容器底緣，只在放不下時出現；拉桿 20px 膠囊（首版 14，使用者：「再粗一點」）、`::-webkit-scrollbar-thumb` 鋪 5px 間距、`--foreground` 22% 的圓點，點只畫在透明 border 內縮出的 padding-box（上下 5＝兩整排、左右 10 避開圓弧）、橫向 `background-repeat: round` 不切半顆（首版 3px／全值、直接鋪滿，使用者兩輪：「點字不要這麼密、顏色不要這麼亮」「還是太亮，有些點點被切掉」）；不寫 `scrollbar-width`／`scrollbar-color`（Chrome 121 起標準屬性一有值就忽略 `::-webkit-scrollbar`）；≤760px 堆疊版型不介入。站上還沒設計更小螢幕版型之前，每一張清單都先照這一套走 | [product-list.css](./ds-components/product-list.css) |
...
| Table | 🟠 organism | ✓ App | Earnings transactions 9-col table。**ztorUI 換裝（2026-08-28 補熔接）：表格自身退成透明**——原為 `--card` 實色底＋1px 框＋圓角＋`--shadow-card`，但實測 13 個消費頁的 `.ztor-table` 一律巢狀在玻璃容器內（儀表板／earnings／project-detail 在 `.card`，admin 系在 `.admin-table-wrap`），自己再當一張卡就是把外層玻璃的通透吃掉；依既有巢狀律改 `background: transparent`＋`border: 0`＋`box-shadow: none`，邊界與陰影交給外層。2026-07-28「每張卡都要有細微陰影」那條裁示的前提（本元件自己是一張浮起的卡）已不成立。**表頭無底色、也無下框線**（2026-07-31 使用者裁示，同日兩段：先去底色——底色在卡片內會像另一個區塊；再去下框線——全站列表表頭一致，見 STYLE-DECISIONS Q41）。表頭與內容的分隔改由留白承擔，`tbody` 每一列的分隔線不動。伴生類：`.ztor-table-scroll`（橫向捲動容器，2026-07-31 自 project-detail 的行內 `overflow-x:auto` promote；坐在 `.card` 內時表格自框會被取消、避免框中框）、`.ztor-table__nowrap`（短值欄不換行——兩字詞會被逐字斷成直行）、`.ztor-table__sub`（主欄位第二行，放來源／模組／分類這類歸屬資訊，省下一整欄寬度）、`.ztor-table--truncate`（modifier：整張表一律單行，主欄位 `.ztor-table__namecell` 用 `max-width:0`＋`width:100%` 吃掉剩餘寬度，超出以 ellipsis 收掉而不撐寬表格；不用 `table-layout:fixed`，那會改掉站上所有表格的欄寬演算法）、`.ztor-table__mediatext`（`.ztor-table__media` 裡要放兩行文字時的包裝；flex item 預設 `min-width:auto` 不會縮，不歸零 ellipsis 永遠不觸發）。**縮圖一律用 `.ztor-table__media` 併進名稱格、不要自成一欄**——獨立欄會在圖兩側各吃一次 20px 儲存格內距，圖會浮在空欄中間（2026-07-31 使用者回報）。縮圖尺寸 **40px**、間距 `--sp-12`（2026-07-31 使用者：各再大一點點，覆蓋 07-26 的 32px 裁決）；40px 剛好等於資料列的內容高，列高不變。 | [table.css](./ds-components/table.css) |
| Sparkline | 🟢 atom | ✓ App | 數字的附註型微型走勢圖（2026-08-28 自儀表板 KPI 列 promote，使用者：「下面要有走勢圖，但小小的」）。與 `.linechart` 的分工＝那支是「圖表本身就是內容」（180px、格線／座標／tooltip），這支只回答「往上還往下」，沒有格線、資料點與 hover；要讀出曲線上某一點的值時該換 `.linechart`，不是把這支放大。`__plot` 讓繪圖欄與縱軸**並排**而非疊放（疊放時長刻度會蓋住折線）；橫軸放在 `__main` **裡面**才會跟畫布同寬——放到上一層會連縱軸那欄一起撐開，最後一個刻度（本週）就落在縱軸標籤底下而不是折線終點（2026-08-28 修）；`__axis-y`／`__axis-x` 選配、各只標兩個刻度（最大／最小、起點／終點），刻度字串自帶單位，渲染器不做數字格式化。墨色唯一鉤子 `--spark-ink` 預設 `--muted-foreground` 且**各卡一致**——顏色留給數字與 delta 膠囊，替某一條線染色會暗示那張卡比較重要（2026-08-28 使用者裁示）。**2026-08-28 同日擴充成細節版**（使用者出示參照圖「畫成這樣」）：新增 `__grid`（水平虛線格線，畫在有帶值的縱軸刻度上，折線才跟刻度對齊）、`__target`（目標值虛線）＋`__cap`（圖上緣註記，如「目標 $24k/wk」）、`__band`＋`__seg`＋`__dot`（標記區間三件套：直立色塊＋區間內重畫的線段＋兩端圓點；色塊只用明度、色相留給線段與端點，線段疊在主線之上所以主線不必切段）、`--detail` 修飾子把繪圖區加高到 76px。資訊層次由淡到實＝格線→目標線→折線→標記端點，眼睛照這個順序讀。值域改吃刻度值而非資料極值（否則資料一動格線就飄）。`__mark` 是區間的數值標籤（HTML 疊在繪圖區上，不用 SVG 文字——`preserveAspectRatio="none"` 只拉伸 x，SVG 文字會壓扁）。**標記區間的語意色走 `--up`／`--down`（與 delta 膠囊同一套狀態色），不用品牌橘**——它標的是成長率、屬狀態，橘色在本系統代表主角／主要動作，用在這裡會撞語彙（2026-08-28 使用者裁示）。端點圓點與圖上緣的目標註記同日撤除：區間起訖由色塊邊界講清楚了，目標值留一條無標籤虛線即可。資料是 84 點的**滾動七日收入**（每天回看過去七天的總額），不是逐日收入——卡片的值是單週金額、目標也是每週，逐日值的單位對不上（日收入 $24k 會讓週收入變 $175k）；滾動加總的單位就是週收入，終點正好等於卡片上的數字，而且滾動本身會把單日波動抹平，讀起來像收入而不是感測器雜訊。固定假資料、重整不變，且**終點與週增率都對齊卡片上的數字**（$24,830／+12.6%；粉絲卡 1,283／+2.3%）。一律 `aria-hidden`：精確值 `.kpi__value` 已唸過。消費：儀表板總收入與站上粉絲兩張 KPI。 **堆疊面積 `.sparkline__stack`**（2026-09-01 新增，首個消費者＝展示版左欄的收入走勢）：傳 `stack: [{key, en, color, values}]` 就由下往上疊出幾塊面，最上面那條邊界正好是主線畫的總額，有堆疊時預設的 `.sparkline__area` 不畫（同一塊地方的兩種畫法，疊起來會糊）。**格線同一輪改成最後才畫**：堆疊會把畫布填滿，先畫的格線埋在下面，縱軸那幾個刻度就沒有對應的線了。塊的顏色由行內 `--fill` 帶進來、與圖例的色鍵同一支；填充半透明 0.45 加一條同色細邊——這幾塊常坐在照片上，實色會變成貼在圖上的色板，而相鄰兩塊明度接近時純填充看不出界線。**什麼時候用堆疊、什麼時候用多條線**：問「總額怎麼組成」用堆疊（厚度加起來一定等於總額，不會有「剩下的呢」這種問題）；問「這幾條各自的走勢」用 `extra` 的附加線。 | [sparkline.css](./ds-components/sparkline.css) |
| Issue panel | 🟠 organism | ✓ App | 待辦事項面板（2026-08-31 由「待處理事項」改名，KPI 卡、彈窗標題與面板三處同步）：把「還等著創作者處理的事」整批攤開，依來源模組分組、一次展開一則（2026-08-28 promote，使用者裁決把儀表板同一份 `dash-alerts` 的三個出口——一條摘要通知條 `.alerts-strip`、「待處理事項」彈窗 `ops-pending`、「查看全部」彈窗 `todo-all`——併成單一頁內面板，三者同日移除）。資訊層次：面板標題列（可收合，筆數寫在標題句子裡＝「5 件待處理事項」）→ 分組（模組 icon 18px＋模組名，與事項標題同字級）→ 事項卡（嚴重度直條＋標題＋相對時間＋展開鈕）→ 展開後的「受影響的項目」小標＋細項清單（每項左緣一條細直線、各自帶動作鈕）＋（選配）卡底列。**表面律（Q77）：卡面一律中性，且收合時完全無框**（2026-08-31 改）——一則事項在收合狀態只是一列可掃視的文字，畫框等於把「這是一張卡」宣告五次，面板自己的邊界反而讀不出來；框留給真正需要界定範圍的兩個時刻，`:hover`（這一列可以點）與展開（`:has(.issue-card__toggle[aria-expanded="true"])`，好幾行內容要收在一起），兩者都只上 `--ztu-film`、不上 backdrop 模糊（巢狀層再模糊只會取樣到自己的母面）。**hover 與展開兩個狀態 2026-08-31 都改掛到 `.issue-group`**（使用者裁示，同日兩次：先是 hover，接著「展開後是 hover 的匡，不是原本的繼續匡起來」）：卡片本身完全不畫框、只負責內距——兩個框疊在一起會讓人以為那是兩層東西。一組＝一個模組底下等著處理的所有事，整塊一起亮，讀者才知道自己站在哪個模組裡；群組帶 `--sp-8` 內距與同值負邊距，讓底色滿到面板的內容邊而不是縮在文字寬度裡。取捨要知道：一組有多筆時，滑過會整組一起亮，而可點的仍然是單一列。組與組的間距同時由 `--sp-20` 加到 `--sp-28`——模組名不再是全大寫小標之後，20px 會讓上一組的最後一列與下一組的標題讀成同一串。嚴重度改由 `__sev` 一小條 3×14 直線承載，色票沿用 `--status-error`／`--status-warning`／`--status-info`，本元件不另造色票。**用直條不用圓點**（2026-08-31 再調）：站上的圓點已經被「未讀通知」佔走（側欄通知中心那顆），同一個形狀再拿來講嚴重度，讀者會把「這則很嚴重」讀成「這則還沒看過」；直條沒有那層既有語意，而且與展開後每個細項左緣那條細直線是同一套線的語彙。參照圖給的紅底卡沒有照做：五張紅底同時出現讀起來是「整個工作站在燒」，實際上那是五件各有下一步的例行事務。面板本身是頁面級表面，玻璃四件套直接吃 `.card`，本檔只補排版與焦點樣式（`tabindex="-1"` ＋ `:focus-visible`，因為 F2 的「待處理事項」KPI 卡點了會捲到它並把焦點交給它——只捲不移焦點會把鍵盤使用者留在原地）。**開合**：兩層都是原生 `<button aria-expanded>` ＋ `aria-controls`，CSS 只轉 chevron、顯示與否交給 `hidden`；**compact 的標題只有名稱、不帶筆數**（2026-08-31 使用者裁示改成「待辦事項」）：那一格固定出三則，標題不宣稱總數就沒有「說了 5 卻只看得到 3」的問題；完整版仍寫「N 件待辦事項」，那裡列的就是全部。**同日再撤除 `.issue-panel__more`**（標題旁的查看更多箭頭，使用者裁示）——它原本是這個形態的必要配件，前提正是「標題寫著實際總數」；總數拿掉之後這條路不再非有不可，但取捨要記著：現在畫面上沒有通往其餘幾則的路。**模組 icon 底改成圓形薄膜**（2026-08-31 再調，使用者裁示「這個 icon 多一個圓的玻璃背景」）：當天稍早拿掉底色的理由是「卡本身已經是一片玻璃、面上不該再疊面」，卡面退場之後那個理由不成立了——閒置時 icon 直接躺在照片上，一層底反而讓它站得住。**圓形不是方形**（方框會讀成一顆按鈕，那正是上一版被拿掉的原因）；**用薄膜不上 backdrop 模糊**（滑過時它的母面就是卡片自己的玻璃，玻璃上再糊一層只會取樣到自己的母面，Q77 巢狀層規則）。**compact 形態的卡面 2026-08-31 一天之內走過三版，最後完全撤除**（有框的卡 → 無框毛玻璃 → 什麼都不畫；末版依使用者裁示「讓這裡不要這麼明顯，融入在背景中」）：排法是 icon ＋（標題／模組名）兩行，直接躺在底圖上。理由是這一欄的另外兩格本來就是壓在底圖上的字，三個亮塊排在暗處會先被讀成「三個方塊」、才被讀成三件待辦。**讀得到嗎**：底圖左三分之一本來就壓著 `--scrim-media`（見 Canvas stage），那層壓底正是為了讓左欄的字站得住，這一格與另外兩格站在同一片底上。界線改由節奏承擔，滑過時才浮出一塊有厚度的玻璃面（內距 8/12，2026-08-31 再調——4% 的薄膜在照片上根本看不見，讀起來像「字後面多了一層灰」）；內距接手之後列距由 16 一路收到 **0**（界線的事完全由內距承擔，列距只是第二份空白；16 還會把第三則推出這一格，實測被切掉 22px）。中間那版的毛玻璃留作紀錄：**在照片上模糊不違反 Q77**——母面是一張照片、不是另一片玻璃，模糊有東西可以取樣；退場的理由是「太顯眼」，不是「不合法」。**compact 的標題寫的是要做的事**（同日使用者裁示，例：補交稅務表單，不是「提款已暫停 — 需補稅務表單」）：這一格叫待辦事項，每一列就該讀成一件事情本身。這句不是新編的文案——它是這一則本來的 `cta` 標籤，與彈窗裡那顆按鈕上的字完全相同；狀況、說明與受影響的項目收進彈窗。**compact 點開的是彈窗、不是就地展開**（同日使用者裁示「點擊開啟 popup」），箭頭因此向右不向下——向下承諾「在下面打開」，向右承諾「帶你去別的地方」。就地展開在這裡不可行：這一格住在永不捲動的版型裡、高度只夠三列，展開會把第三列推出畫面。彈窗沿用站上既有的 `.payout-dialog`（窄版），接住那句被讓出來的狀況標題（列說要做什麼、彈窗說為什麼要做），底列不放主要動作——每個受影響的項目自己就掛著同一個動作，只有一個細項時兩顆會上下相鄰、看起來像兩個不同的去處。**完整版（儀表板）三樣都不動**：色條、分組、狀況標題與就地展開都留著（實測改動後仍是 5 則 5 組、色條 3×14、chevron 向下、展開層俱在）。icon 說的是「這是哪個模組」，**一律白色、也沒有底色**（同日再調）：卡本身已經是一片玻璃，icon 再墊一層方塊就是面上疊面，三則排下來會讀成三個小按鈕；28px 的框留著只為了對齊——幾支 icon 的視覺重心不一樣，沒有固定框的話兩行文字的左緣會跟著晃（實測三則皆 354px）。**取捨**：嚴重度在這個形態下不再有視覺標記，靠的是這一欄本來就照急迫度排序。**完整版（儀表板）維持色條與分組**，那裡有群組標題在講模組。**動作下放到每個細項**（2026-08-31 再調）：`__details` 的每一列自己帶一顆 `.issue-detail__action`（`.btn--outline.btn--sm`），取代整張卡共用的那一條「下一步」連結——「庫存過低 · 3 項商品」實際上是三件各自要做的事，共用一條連結等於把三件併回一件。文案與去處都取自該則告警既有的 `cta`／`ctaHref`（細項可用 `d.href` 覆寫），渲染器不生成產品事實。**窄欄會自己讓位**：`.issue-detail__body` 掛 `min-width: 150px`，面板窄過門檻時鈕掉到名稱下一行靠右站，而不是把商品名擠成三行；不用 media query，因為面板寬度跟著 bento 欄走、不跟著視窗走。清單上方補一行 `__affected` 小標（「受影響的項目」），沒有它這串會被讀成「這一則的說明文字」。細項名稱升到 `--fs-14`、列距 `--sp-16`，展開後的卡內距加大到 `14/16/16`。次要動作（暫緩／阻斷說明）改放 `__foot`、只在有內容時渲染，**一律不掛在收合列**（2026-08-31 改）：純圖示鈕在收合列讀不出意思（時鐘不等於暫緩、鎖頭不等於「只能在來源模組解決」），而且會跟「整列可以點開」這個唯一訊號搶注意力。阻斷型改成一句說明 `__note`（「處理完才會消失，無法暫緩」），資訊型是有字的 `.btn--ghost.btn--sm` 暫緩鈕；`data-snooze` 走的是委派監聽，搬位置不影響行為。**筆數寫進標題句子**（2026-08-31 再調）：原本是標題旁一顆徽章，改成「5 件待辦事項」一句（同日再改名，原本叫「待處理事項」）——徽章要讀者自己把數字跟名詞連起來。數字是獨立的 `__n`（等寬數字），名詞走 i18n，兩個語言各照自己的語法：中文帶量詞「件」，英文換單複數兩個鍵，不做「數字＋固定字串」的硬拼。**筆數口徑**：整個面板只留這一個數字，一律排除已軟關（snoozed）的項目，與 F2 那顆同名 KPI 對齊。各組筆數與卡片計數徽章 2026-08-31 移除——組筆數在五則五組時每組都是 1，卡片徽章的數字是細項數又與標題自己寫的數量（「Low stock · 3 items」）重複，一列三個數字讀者只能猜哪個是哪個；已軟關的卡照樣渲染、`--snoozed` 降階並沉到組底（spec 5.1.1 §F4「當下不需要處理但仍要看得到」）。**「下一步」那一行不是推薦**：站上沒有推薦引擎，文字與去處都取自該筆告警既有的 `cta`／`ctaHref`（ASSUMPTIONS ISSUE-001）。與鄰居的分工——`alert.css` 的 `.alert--card` 是「單則通知」一則一張、彼此獨立；`collapse-group.css` 是表單欄位的折疊盒、折疊態只留一行摘要值、只有一層；`accordion.css` 是 FAQ 語彙、整列都是文字沒有徽章與右側控制鈕。三者都保留、不互相取代。渲染器 `data-component="issue-panel"`（js/components.js），資料集 `dash-alerts`；細項 `details[]` 與相對時間 `ago` 由資料端提供，渲染器不生成任何產品事實。消費：儀表板 F4。 **hover 的玻璃底由 `--ztu-glass-bg`（5% 白）改成 `--on-media` 10%**（2026-09-01 使用者回報「這個 hover 壞掉了」）：毛玻璃的可讀性有一半靠模糊，而 compact 這一版坐在照片最暗的那一角、模糊沒什麼可取樣，5% 的白在那裡幾乎看不見，滑過去像沒有反應。把面自己撐起來之後模糊變成加分項，不是唯一的依靠。（同一輪的另一半在 Canvas home：左欄那層漸消遮罩會讓 `backdrop-filter` 整個失效，遮罩改成捲動之後才掛。） | [issue-panel.css](./ds-components/issue-panel.css) |
| Work card | 🟡 molecule | ✓ App | 儀表板的**封面磚列**（2026-08-28 自儀表板 F6 promote，同日改了四版）。**兩個分頁裝兩種東西**（2026-08-28 使用者裁示）：**進行中**＝creator 還在跑的作品（live＋scheduled，取自 projects store）；**近期**＝規格 5.1.1 §F5 的系統事件（取自 `dash-activity`）。原本兩邊都是同一批作品的切片，等於同一件事講兩次——「帥到分手 MV 上線」這則事件，與同一支 MV 狀態標成「已上線」的磚，本來就是一件事；分成這樣之後左分頁＝我在做什麼、右分頁＝剛剛發生了什麼，已結束的作品仍由右上「更多」進 projects.html。原下方的 F6 表格與 F5 卡都已於同日移除，磚列是這兩塊規格在儀表板上唯一的呈現。**三版沿革**：初版是 16:9 封面＋狀態徽章＋進度條＋摘要的資訊卡，數字與正下方表格完全重複；第二版剝到只剩封面與名稱，結果磚列認得出是哪幾件卻答不出「這件現在怎樣了」；第三版加回**分層過**的三行——類型（含狀態點）→ 名稱 → 進度或期限，仍然沒有摘要句、沒有徽章、沒有金額（那些只在表格）。**2:3 直式、但不裁切素材**（2026-08-28 使用者裁示改直的）：素材是專輯封面式正方圖（實測 400×400、562×562）。正方放進 2:3 的框用 `object-fit: cover` 會切掉**左右**而不是上下，實測把作品名切斷（「LOVE RAGE HOPE」只剩「VS RAGE HO」、「BILLIE」只剩「ILLIE」）——而封面上的作品名正是這張磚拿來認人的依據，切掉等於磚失去用途。做法：真正的圖 `contain` 置中、一個像素不裁，底下墊同一張圖放大 1.25 倍並模糊（`__bg`）補滿左右；兩個 `<img>` 指同一個 src、只下載一次。換成真正的直式素材不必改這裡。**類型與狀態同一行（`.work-card__cat`）、坐在名稱上方**：兩者都是「關於這件作品的後設資訊」（它是什麼、到哪個階段），名稱才是作品本身；三樣擠一行時名稱前面永遠有東西擋著，分層後名稱獨佔一行、成為整張磚最強的識別點。類型標籤取自 `ztorProjects.catLabel()`（store 既有雙語詞彙），本層不造類型字串。狀態點仍取代徽章：150px 放不下徽章，這尺寸值得回答的只有「在不在跑」；精確狀態詞在下方表格，點旁掛視覺隱藏標籤讓讀屏唸得到。點色沿用既有 `.ztor-dot` 修飾類，未知 variant 退中性點而不是靜默無點。**進度或期限（`.work-card__timing`）三選一，順序即判斷順序**：① `bar.pct` 有值 → 細條（整條沿用 shared.css 的 `.project-bar`，不另裁一套）＋一行「pct% · 剩餘天數」（天數取 `fund.left` 雙語物件，沒有就只印 pct）；② 否則 `ztorProjects.releaseDate(p)` 有值 → 上線日，走 i18n key `work.released` 的 `{date}` 樣板（EN 動詞在前 Released {date}、ZH 在後 {date} 上線，拼接式必有一邊語序壞掉）；③ 都沒有 → **整行不畫**，不留破折號佔位（短一截讀起來是「沒有排定的東西」，破折號讀起來是「資料壞了」）。**禁止剖析 `p.meta`**：那是給人讀的摘要句（「審核中 · 9/05 上映」）、沒有格式契約，從裡面挖日期等於下次改文案就靜默拿到錯的日子。**分頁鉤子自成一套、不與任何人共用**：磚列用 `[data-rail-tabs]`／`[data-rail-tab]` ＋ `.work-rail__tab`，i18n key 也自成一組（`rail.tab.*`）。同屏曾經有第二組分頁（F6 表格的 `[data-dash-tabs]`／`.filter-tabs__item`），共用鉤子的話點磚列會靜默把表格也切掉；那張卡雖已移除，這條規則保留——磚列的委派寫死在自己的容器內，日後同屏再出現第二組也不會互相命中。機制也不同：面板式分頁切 `.tab-panel` 兄弟節點，磚列原地重畫同一塊 DOM，所以**切換後必須重算翻頁鈕**（換了內容、可捲距離也換了，沿用舊的顯隱會出現「明明還有磚，右鈕卻收起來」）。**§F5 五項在磚上的落位**（每筆必須齊全）：事件類型→眉標左段（`.work-card__kind-text`，取該筆既有 `meta` 字串的第一段；切不出來就留空、只顯示來源，**不發明新分類**）／事件來源→眉標右段（`.work-card__source`，`mod`）／事件名稱→名稱行（`title`）／事件狀態→狀態列左（`status` 的譯詞）／發生時間→狀態列右（`date`）。眉標的兩段版是 `.work-card__kind--split`，兩端對齊與下方狀態列相同——兩行同一種對齊才會被讀成一組。事件磚的字在資料端就用 `window.i18nT()` 譯好、不掛 data-i18n：磚上本來就有一半是 store 的雙語**資料**，語言一換整列重畫，兩種來源走同一條路徑才不會半新半舊。**落點與 `.work-card--static`**：作品磚指 project-detail，事件磚指觸發它的模組頁，對照表 `DATA['activity-module-href']` 放**資料端**（Projects→projects.html、Fans CRM→fans-crm.html、My IP→my-ip.html、Earnings→earnings.html、E-Shop→e-shop.html、Events→events.html、IP Market→ip-market.html、Settings→settings.html）而不是寫死在元件裡——某個來源該連去哪是產品的路由事實。查不到對照就吐 `<div class="work-card work-card--static">`：沒有 href、不進 Tab 鍵序，hover 的抬升與提亮一併關掉（會跟著游標動的東西等於承諾了一次點擊）。**換語言要整列重畫**：磚上的作品名、類型、剩餘天數都是 store 的雙語**資料**、不是 data-i18n，applyI18n 掃不到；監聽 `i18n:applied` 但比對語言碼才重畫（該事件每次套用都發，含初次載入）。**自帶區塊標題**（`.section-head`，沿用頁面級共用 pattern）：刻意與下方表格的「近期活動與項目」不同名——磚列講「作品」、表格講「紀錄」，同屏兩個同名標題會讓人以為是兩批東西。分頁放同一列右端（`.section-head` 本來就 space-between，只需把 `align-items` 從 baseline 改成 center，baseline 是給標題＋副標用的、會讓膠囊掉到基線以下）。**版面**：`.work-cards` 是 `scroll-snap` 橫向捲動列（不是會換行的 grid），右緣露半張＝「還有更多」的唯一提示，不畫捲軸（macOS 原生橫捲軸預設隱形）；磚寬 150px、`flex: none`，窄視窗改捲動而不是把每張擠瘦。三行字全部 `white-space: nowrap` ＋ ellipsis：換行會把單一一張磚撐高，一列裡就只有那張比別人高。**整磚可點**＝本體就是 `<a>`（錨點自帶鍵盤焦點、Enter 與另開分頁），focus 走 `--ring`。與 `event-preview-card` 的分工＝那支是**粉絲端**活動卡的即時預覽（spec 5.1.6.1，欄位是海報／活動名／日期場地／票價），這支是**創作者端**的作品辨識，兩支不可互換。**資料來源＝projects store 而非 F6 的簡化假資料**（2026-08-28）：store 的每筆本來就同時有 `cover`（卡片圖）與 `poster`（直式海報），磚列要的正是後者。**標題本身就是篩選**（2026-08-28 使用者裁示）：`.work-rail__tabs` 的兩個標題級詞（進行中／近期・Active／Recent）同時是區塊名稱與篩選值——前一版是中性標題＋右側膠囊分頁，兩者在回答同一個問題，合而為一就少一個元件、也少一次視線往返；未選中的詞維持可讀（是入口不是註腳）。膠囊讓出來的右側位置改放 `.work-rail__more` 出口通往完整項目清單——不是下方表格的重複，磚列取自 store 的 27 筆只顯示 12，F6 表格看的是另一份 6 筆。這組分頁與 F6 表格那組不共用任何 class，互不連動。**文字塊四段（2026-08-28 第四版重排，使用者：「應該分開不同行，並且重新整理」）**：`__kind`（作品類型，最淡）／`__name`（名稱，獨佔一行、最強落點）／`__state`（左＝項目類型、右＝最可行動的值，兩端對齊）／`__bar`（只有真有百分比才畫）。順序就是眼睛需要它的順序。`__state` 用兩端對齊而不用「·」串接——中英文長度差太大（預購 vs Pre-order、剩餘 20 天 vs 20 days left），串接在英文會爆行，兩端對齊時中間空白會自己吸收差額。右值優先序＝剩餘天數＞百分比＞上線日：天數最可行動、日期是既成事實。**狀態點已撤除**：純顏色沒有圖例，看得見的人得不到資訊（使用者實際問了「這是什麼」），而磚列的分頁已在篩狀態、右值也把處境講得更清楚；狀態詞以視覺隱藏文字保留給讀屏。同輪修掉一個重複——改版前 go-live 的磚會同時出現「上線」（類型）與「2025/08/22 上線」（日期），現在右值只回數值、不再重述類型。兩套詞彙的分工（`catLabel(p.cat)` 講「作品是什麼」（音樂／音樂專輯／MV／電影…），`typeLabel(p.type)` 講「項目是什麼」（預購／共創／上線）——後者正是下方那條進度條在量的東西，少了它讀者看到 74% 也不知道那是募資、預購還是上線進度。兩支標籤都住在 store（它擁有這套詞彙），元件不自建對照表。`typeLabel` 刻意不帶「項目」二字：儀表板整排都是項目，寫了是廢字，150px 也放不下；F6 表格那組完整寫法（預購項目／共創項目）是表格自己的文案、不共用。取 `poster`、缺才退 `cover`，兩者皆缺就跳過該筆（寧可少一張，不要空白灰磚）。**注意 `poster` 不保證是直式**：實測周湯豪那批的 poster 仍是正方專輯圖，真正 2:3 的電影海報屬另一個 persona；所以 `__bg` 模糊背景板不能拿掉——它正是讓正方與直式素材共用同一個框的機制。**消費頁要載 `work-review-store.js`**：影片家族的上映日期依 D180 存在送審件裡，那支不載的話日期分岔對影片家族永遠讀到空字串、整行不畫，看起來像資料缺漏（index.html 已於 2026-08-28 補載）。**翻頁**：`.work-cards__nav` 兩顆鈕絕對定位在捲動區左右緣（放進 `.work-cards` 會跟著捲走，所以外面包一層 `__wrap`），一次捲一個可視寬度而不是固定像素——視窗寬窄不同時每頁張數就不同，捲固定值會在窄視窗一次跳過兩三張；到頭收左鈕、到底收右鈕，resize、`load`（圖 lazy 載入完寬度才定案）與**切分頁**都重算。消費：儀表板 F6（進行中分頁）＋ F5（近期分頁）——`js/components.js` 的 `work-cards` 渲染器＋衍生資料集 `dash-events-cards`（近期分頁改讀 `dash-activity`，經 `activityCard()` 轉成磚的欄位） | [work-card.css](./ds-components/work-card.css) |
| Earnings feed | 🟡 molecule | ✓ App | 儀表板第一屏右欄的「近期收入」：**一筆交易一個框**（2026-08-28 promote，使用者裁示「近期收入搬到第一屏、一筆一個 section 匡、每 10 秒更新一次」）。**與 `transaction-list` 的分工，兩支都留**：那支（表格版 `txTableRow` ／清單版 `txRow`）的版面前提是「一整欄上下對齊、一次掃完、逐欄比較」，適合 Earnings 那種整頁帳目；這支的前提是「窄欄裡一筆一個完整敘述」。同一份資料搬進第一屏第三欄之後只剩 259px（1440）／219px（1280），表格的每一欄都被壓到極限、名稱幾乎全截斷，表格的優勢一個都用不到——所以是**新增一支渲染器**，`transaction-list` 一個位元組未改（earnings 等頁仍在消費）。**框的表面照 Q77**：巢狀在玻璃卡裡 → `--ztu-film`（6% 白）＋1px `--nest-line`，**不上 `backdrop-filter`**（母卡已是玻璃，子層再模糊只會取樣到母卡自己），**面不染色**；金額正負只換文字色（`__amount--neg` 走 `--destructive`），不鋪底——一欄五個框各染一次底會讀成一片色塊。實測 computed：`background rgba(255,255,255,0.06)`／`border rgba(255,255,255,0.1)`／`backdrop-filter none`。**三行的順序＝名稱 → 金額 → 來源與時間**：金額用字級（`--fs-18`）與 `--font-numeric` 承擔權重，不必再靠位置強調一次；名稱放最上面，一整欄由上而下讀就是「這是什麼／這是什麼／…」，掃視時對得起來。反過來把金額擺第一行，一欄五個數字連成一片、名稱淪為註腳。**來源與時間同一行兩端對齊**，不用「·」串接：串接在英文會爆行（Story world licensing · Nov 20），兩端對齊時中間的空白自己吸收中英文長度差。**時間永遠完整**（`flex: none` ＋ `nowrap`），名稱與來源可截斷——時間只有一兩個詞，截掉等於沒有資訊。**三行全部單行截斷**：折行會讓框高低不一，而這欄每 10 秒換一次內容，高度一跳整欄就在抖。1280 的英文會截到來源（Streaming royalty → Streaming …），1440 起中英文都完整。**每 10 秒輪替一次，模擬「收入持續進來」**：資料是既有 8 筆（`DATA['dash-recent']` 的同一個陣列，`dash-recent-feed` 只是指過去、不複製）的環狀輪替，**沒有生成任何新交易**（ASSUMPTIONS FEED-001）。`offset` 每次往回退一格 ⇒ 頂端出現「更前面的一筆」、其餘往下推、最末一筆退出可視範圍，讀起來就是收入流的方向。**計時器三條硬規則**：① `start()` 一律先 `stop()`，任何路徑重入都只會有一個 interval（實測連呼叫 5 次仍只有 10005／20004ms 兩次觸發）；② `visibilitychange` 隱藏即停、回前景再開，背景分頁不空轉（瀏覽器會把背景 interval 節流，回來時反而補跑一串）；③ `pagehide` 一併停，bfcache 存起來的頁面不掛計時器。狀態掛在 `window.ZtorEarningsFeed`（`running()`／`ticks()`）供驗證。**`prefers-reduced-motion: reduce` 時內容照換、動畫不播**：進場動畫（`__item--enter`，位移 6px ＋淡入）由媒體查詢關掉，輪替是資訊不是裝飾所以不停——headless 實測開啟 reduce 後 3 秒／13 秒／23 秒的頂端分別是 spotify／adshare／cdPreorder，畫面無位移。**可捲＋底部漸消**（2026-08-31，使用者回報「下方要漸消、可以往下 scrolling、現在和按鈕黏在一起」）：欄改成縱向 flex，清單 `flex: 1 1 0` ＋ `overflow-y: auto`。**`flex-basis` 必須是 0 不是 auto**——這張卡住在 bento 格線列裡，列高由最高那一欄決定；用 auto 起算的話八筆的自然高度會反過來把整列撐高、另外兩欄跟著拉長，捲動永遠不會發生。改成 0 之後清單不貢獻高度（保底 `min-height: 240px`），列高由 KPI 疊與待處理面板決定，清單長回剩下的空間、超出自己捲。**漸消用 `mask-image` 不用覆一層漸層**：卡是玻璃面，蓋實色漸層會出現接縫；漸層色寫 `var(--foreground)` 而不是 `#000`，因為遮罩只讀 alpha、任何不透明色效果相同，寫死色值會被 token 棘輪擋下。**捲到底就把漸消關掉**（JS 加 `is-at-end`，捕獲式 scroll 監聽，需擋掉沒有 classList 的 document／window），否則最後一筆永遠半透明、讀起來像沒載完。**指標進入這一欄時停止 10 秒輪替**、移開恢復：正在讀或正在捲的時候把整份內容換掉，讀者當場失去位置。渲染筆數同時由 5 改成**全部 8 筆**（`limit: 0`）——只渲染 5 筆會讓漸消說謊，看起來下面還有、捲下去卻是空的。底部「查看全部」與清單的間距改由欄的 `gap` 給（原本掛在按鈕上的 `mt-16` 被頁面級規則的 `margin-top: auto` 蓋掉，所以視覺上黏在一起）。**當天的那幾筆顯示時分秒**（2026-08-31 使用者裁示「如果是今天就顯示幾點幾分幾秒」）：`timeCell` 判斷這一筆有沒有「進帳時刻」，有就渲染 `HH:MM:SS`、沒有就照舊顯示日期。時刻有兩個來源——資料端的 `todayMinsAgo`（示意用的「幾分鐘前」，原型層沒有真實交易時間戳，見 ASSUMPTIONS FEED-001），以及輪替時蓋上的時戳。**輪進頂端的那一筆會被蓋上當下時間**：在這個原型的設定裡它就是「剛進帳的一筆」，少了這一步，輪替會把今天 11:46 那筆推到「Nov 19」下面，一欄號稱由新到舊的清單自己打自己的臉。時戳一旦蓋上就不再變，往下推的時候仍排在正確的位置；時戳存在 `dash-recent-feed` 自己的 `stamps` 上（key＝`titleKey`），**不寫回共用的 `dash-recent`**——那份陣列同時餵著磚列的「近期」分頁。這一格不掛 `data-i18n`：時分秒兩個語言寫法相同。**空狀態不放在卡內**：這張卡住在 `.dash-data-only` 的那一列，空資料時整列不顯示、卡內的空狀態永遠輪不到；改與 F2／F4 一樣做成獨立的 `.dash-empty-only` 區塊。消費：儀表板 F3（`js/components.js` 的 `earnings-feed` 渲染器＋資料集 `dash-recent-feed`） | [earnings-feed.css](./ds-components/earnings-feed.css) | **2026-08-28 同日兩次調整（最終為有卡框）**：一度依使用者裁示拿掉外層卡框——每一筆本來就各自成框，外面再包一張卡就是框中框，而且兩層都是「表面」，疊起來讀不出哪一層才是內容。拿掉之後逐筆的框從巢狀薄膜（`--ztu-film`＋`--nest-line`）**升成頁面級的玻璃四件套**：薄膜是靠母卡底色才顯形的，直接放到畫布上會幾乎看不見。欄的標題改用頁面級 `.section-head`（與磚列同一套），右側「查看全部」取代原本卡底那顆整寬按鈕——整寬按鈕是卡的收尾，沒有卡框就沒有可收的尾。同輪首屏欄寬由 4／5／3 改 **3／5／4**（KPI 收窄、收入加寬）。**稍後使用者改回加上卡框**（「還是加上 section 吧」）：三欄裡另外兩欄都是卡，只有這一欄沒有邊界時會讀成「掉在版面上」而不是與另外兩欄並列的一區。外框回來後逐筆的框**必須退回巢狀薄膜**（`--ztu-film`＋`--nest-line`、不上模糊），否則就是玻璃疊玻璃；無框版本手動補的 `--sp-20` 上內距也一併移除，改由 `.card` 自己負責，否則與卡的內距疊成兩倍。標題列改回 `.card__head`／`.card__title`（`--fs-18`，與隔壁 `.issue-panel` 同級、實測齊平 1px 內），「查看全部」留在標題列右側而不是回到卡底整寬按鈕——省一整行，且與標題同一列時讀者一眼知道這一區通往哪裡。 
| Canvas stage | 🟠 organism | ✓ App | 一屏不捲展示版（`index.html`）的滿版底圖（2026-08-31 建，取代同日稍早的海報牆 `canvas-wall.css`，該檔已刪）。**使用者裁示右邊那一大塊是一件作品或商品的整張背景畫面，不是把所有東西列出來**：參考版型裡那張圖不是一個區塊，它是整個畫面的地，左欄的字直接浮在上面；一旦變成一格一格的磚，畫面就退回「又一張卡」。**對照參考版型**：那棟樓＝當前這件的主視覺、樓層選擇器＝右緣的作品切換器（`__rail`，只列前 6 件，全部列出來就變回海報牆；2026-08-31 三調：位置由垂直置中改成接在模式切換下方，模式切換撤除後再上移到浮層頂端；**外框拿掉**，框讓這一列讀成「一塊面板」，但它其實只是幾個選項；**未選中的字白色帶 12% 透明、放大後 `--fs-16`**（2026-08-31 使用者裁示四則：「字小一點」「hover 時和已選一樣要有一個玻璃底」「未選的字也要是白色的」）：降階主要交給字級（12 vs 16）與那三樣只有展開才有的東西（類型小字、玻璃面、箭頭）；顏色只差 12%（`--on-media` 的 88%，同日再調「有一點透明度但不要太多」）——早先用 66% 的 `--on-media-muted` 時，這一排在暗底圖上有一半讀不出來，那正是先改成純白的原因。滑過的那一列與當前那一列同一塊玻璃底。**名稱整句顯示、不截斷**（2026-08-31 使用者裁示「名字要完整顯示，不要 ...」；原本 `max-width: 180px` ＋ ellipsis）：這一排貼著右緣、往左長不會擠到任何東西，而被截成「LOVE·RAGE·HOPE 巡…」的那幾列讀者認不出是哪一件——切換器認不出選項就沒有用；列寬上限放到 420px 只當保險。**左內距 12 → 20、圓角 10 → 24**（2026-08-31 使用者裁示）：字是靠右排的，左邊那一段是玻璃面在名稱前面留的呼吸——太小時短名稱那幾列的玻璃會貼著字收尾、看起來像被裁掉一角；展開時這一塊有 50px 高，10px 的圓角在那個高度上幾乎讀不出來。**滑過就換底圖，點擊＝前往**（2026-08-31 使用者裁示「這些選項是 hover 上去就換背景」）：選一件作品本來要點兩下（先選、再進去），改成滑過即換之後，點擊就專心做一件事。因此「當前這一列」與「滑鼠底下那一列」是同一列，**「前往」只在滑鼠底下那一列出現**（2026-08-31 走過兩版：先是已選固定掛著，使用者再裁示「已選不要出現前往和箭頭，hover 上去才顯示」——滑過就換底圖之後，已選與滑鼠底下多數時候是同一列，固定掛著等於畫面上永遠有一組動作在等人按，而這一排的工作是「看接下來有什麼」）。「前往」站在**名稱與箭頭之間**、字級小兩階（同日三次改到位：名稱前面 → 只在已選＋滑過 → 現在這裡）：動詞緊鄰它要作用的箭頭，讀起來是「前往 →」一句話；放名稱前面會先被讀成名稱的一部分。「前往」與箭頭包成一組（`__pick-cta`）、由同一個 flex 互相置中（2026-08-31 使用者裁示「兩者水平置中對齊」）：文字 17px、箭頭 22px，各自對齊底線時中線差 2–3px，讀起來像箭頭掉下去一點。這一組**站在玻璃面裡面**（同日最後一次裁示「前往和箭頭要在框框裡面」，中途曾搬到面外）：面外的版本要往右多吃 34px，而玻璃面的右緣已經抵著內容區的邊，再往外長，面本身就會被外殼的 `overflow: hidden` 切掉一角。滑鼠只在索引真的變了才重畫（同一列裡移動會連續觸發）。**白字的陰影降到 `--scrim-media` 的 45%**（同日使用者裁示「淡一點」）：原本直接吃 82% 黑，在深底圖上會在每個字周圍結一圈灰邊。**同一時間只有一列是放大的**（2026-08-31 使用者裁示）：滑鼠一進到這一排，當前那一列就收回未選中的樣子（連玻璃一起），放大的名額交給滑鼠底下的那一列；滑鼠離開整排它自己長回來。兩列同時是大的，等於畫面同時指著兩件事。**選中態拿掉邊框、玻璃面留著**（使用者裁示「不要有 border」「玻璃背景還是要在」；中間短暫連玻璃也一起收掉過，同日補回）——玻璃比「字比較大」多做一件事：滑鼠滑過別列時那一列也會放大，兩列同時是大的，玻璃是唯一還在說「你正在看的是這一件」的東西。過程中走過的兩個選項留作紀錄：品牌橘不用（橘在站上代表主要動作，而切換看哪一件是狀態不是動作，整面圖上唯一的橘塊也會把目光從圖拉走），`.btn--outline` 的 `--control-raise` 也不用（那 4% 白是為卡片底設計的，躺在照片上與未選中幾乎同明度）。每一項直接躺在照片上，未選中的字吃 `--on-media-muted` ＋ text-shadow，不用會隨主題翻轉的 `--muted-foreground`）、異常釘點＝浮在圖上的 `__marker`（玻璃膠囊＋垂線＋脈動點；膠囊講是什麼、點講在哪裡）。**底圖靠右擺、保留自己的比例**（2026-08-31 再調，使用者裁示「直式圖片、左暗右亮」）：`right: 0` ＋ `height: 100%` ＋ `width: auto`（`max-width: 62%` 只用來擋橫式素材吃掉整個畫面）。前一版是 `inset: 0` ＋ `object-fit: cover` ＋ 整張 `brightness(.62)`——那會把直式主視覺橫拉滿版、上下裁掉大半，而且左右一樣暗，右半也就沒有「亮」可言。**壓底用兩層漸層**（橫向壓左、縱向壓下）而不是一片均勻遮罩：左三分之一是左欄每一行字的落點必須夠暗，而一片足夠暗的均勻遮罩會把照片壓成灰紙。圖本身不再上 filter——統一壓暗等於把亮的那半也一起壓掉。**2026-08-31 三調（前兩次是使用者回報「中間有一塊黑黑的、左邊遮罩太暗、圖片邊界太明顯」，第三次是調參器 `docs/canvas-bg-tuner.html` 的定案值：模糊 112px、亮度 1.5、不透明度 1、放大 1、遮罩斜坡 50%）**：模糊底層 `.6` → `.85`、壓底曲線整條往下移（「夠暗」的區間收在 0–16%，之後放得比原本快）、清晰那張的遮罩斜坡 26% → 58%。那條發黑的直帶其實是「清晰那張還沒溶進來、壓底又還沒退掉」的一段，底層厚了就有東西可看；斜坡拉長之後，交界讀起來是同一張圖從模糊走到清晰（景深），不是兩張圖的接縫。左邊 0–16%（左欄字的落點）一點都沒放鬆。**不透明度收到 1 是最後一手**（調參器定案）：先前留著的那 10% 透出的是頁面的近黑底色，等於在整片色場上又壓一層黑，深色區因此再暗一階；填滿之後底下那層黑完全不參與，剩下的明暗只來自這張圖本身，再由亮度 1.5 拉起來。**同日第二次回報「有一塊黑黑的」是另一種形狀**（雲塊，不是直帶）：模糊 48px 還留得住輪廓，主視覺左半只要有一塊深色就會在左欄後面糊成一團看得出形狀的黑雲。這一層的工作是借顏色鋪底、不是重放一次這張圖，因此改成 `blur(90px)` ＋ `brightness(1.22) contrast(.85)`、不透明度 .9、放大 1.3——糊到只剩色場再壓掉自身明暗差，深色區攤平成整面的一階暗；放大加大是因為模糊愈重、邊緣往內透底色的那一圈愈寬。**寬度上限 62% → 70% → 84%**（2026-08-31 使用者裁示兩次，第二次由調參器定案）：這個上限只在橫式素材時起作用，直式圖由自己的比例決定寬度；84% 之後只剩左邊約六分之一是純底層，那一段正好蓋住左欄的字。**四層的參數有調參器**：`docs/canvas-bg-tuner.html`（iframe 開真正那一頁、四層各有開關、背景圖用縮圖選、附一個尚未採用的「主色鋪底」備案層）。**素材前提**：來源取 `poster || cover`，有直式版就用直式；目前 nick 這位人格的專案圖全是 400×400 方圖（`cover` 與 `poster` 指同一個檔），所以實際渲染出來是接近正方的一塊，這是素材缺口不是版面問題。**只有需要處理的標記染色**，進度與上線日維持中性（Q77 的同一條理由：三顆紅膠囊會讀成整件作品都在燒）；最多兩顆。**右下角的作品說明（`__caption`／`__kind`／`__title`／`__go`）2026-08-31 整塊撤除**（使用者裁示），它的工作整份搬進右緣的切換器：**當前那一列自己就長成類型小字＋放大的名稱（`--fs-18`）＋最右邊的前往箭頭**，滑過任何一列也換上同一套裝扮（那是「換過去之後會變成這樣」的預覽）。同一件事在一個畫面上寫兩次，第二次就只是佔位置，而那一塊佔的還是照片最完整的一角。**每一列兩個點擊目標**：名稱那一段切換底圖、箭頭進到那一件的頁面——兩件事不同，併成一個就得犧牲其中一個；名稱吃掉整列扣掉箭頭的寬度，主要動作仍然是一整行可點的。**動作那一組（「前往」＋箭頭）站在玻璃面裡面、與名稱那一行齊平**（2026-08-31 使用者裁示四次走到位：先「箭頭要往右邊凸出」，改成「箭頭要包在框框裡」，再「名稱的尾巴與上面時間切齊、前往和箭頭會凸出來」，最後定案「前往和箭頭要在框框裡面」）：**名稱的右緣與頂列時間收在同一條線上**（實測時間與六個作品名皆為 1316px），而移動的是時間、不是切換器——玻璃面的右緣已經抵著內容區的邊，往右推就會被外殼裁掉，所以由頂列讓出一段等於「玻璃面右內距 10 ＋ 面內預留的 60」的右內距去接名稱那條線。面內預留 60px（箭頭 22 ＋ 兩段間距 ＋「前往」約 24／英文 Open 約 27，以寬的那個為準）：**每一列都留著那一格**，不管當下有沒有顯示動作，只給展開的那一列預留的話其餘幾列會多伸出去一段；**寫死一個值而不是跟著字長**，跟著字長換語言時整排會跳一次。與名稱那一行齊平而不是整列垂直置中——置中會把它抬到類型小字與名稱之間，看起來像在標示那兩行。**名稱與玻璃面右緣之間 12 → 10px**（兩次「間距小一點」）。**類型小字預設不佔位**：六項全都掛著類型，這一列就從切換器變成一張清單，而它的工作是切換不是閱讀。**浮層一律靠右**（`__rail` 右上、`__caption` 右下）：左三分之一是左欄的地盤，放那裡會壓到收入的標題與待辦清單（實測）。**模式切換（`__modes`：作品與商品／未來 90 天）與它打開的 90 天時間軸 2026-08-31 整組撤除**（使用者裁示「不需要未來 90 天了」）：畫布只剩單一作品舞台一個模式，而一顆沒有第二個目的地的切換器比拿掉它更難解釋；`canvasDated()`／`canvasTimeline()` 與 `.canvas-timeline*`／`.canvas-event*` 同日一併移除（兩邊各留一則墓碑）。作品切換器隨之上移到浮層頂端，原本讓給模式切換的 64px 沒有理由留成空白。標記在圖上的落點是**畫面座標不是資料**——原型沒有「這件事發生在圖上哪裡」這種空間資訊（ASSUMPTIONS CANVAS-001）。 | [canvas-stage.css](./ds-components/canvas-stage.css) **清晰主視覺的寬度＝整頁的 70%**（2026-08-31 使用者裁示；先前調的是 `max-width` 上限，而上限是天花板——這張圖的寬度平常由「高度 × 自己的比例」決定，上限比它大就不起作用，使用者因此回報「拉到 83、84% 就不能再調」。改成直接指定 `width: 70%`、`max-width: none`，多出來的由 `object-fit: cover` 裁切）。 **壓底漸層右緣也有一道**（2026-08-31 使用者回報「背景太白的時候看不到字」）：右邊那一排是白字浮在照片上，遇到亮色主視覺（白底海報、逆光人像）整排會消失。字級與陰影都救不了純白底——白字上再加陰影只會糊，唯一有效的是把它底下那一段壓暗。做成與左邊同一種手法（從邊緣往內漸消），兩側才讀成同一套語彙、不是「右邊多了一塊方形暗片」；只壓到 40% 寬，再往左會吃掉中間的主體。調參器（`docs/canvas-bg-tuner.html`）同步補上右端濃度／中間濃度／寬度三根滑桿。 **壓底再加一層暗角**（2026-08-31 使用者裁示「讓焦點聚焦在中間那一塊」）：四邊往內壓暗、中間放掉，視線就落在照片的主體上——先前四邊一樣亮時，右邊那一排白字與中間的主體在互相搶。它同時把右緣再壓一階，亮色主視覺下的白字更站得住。用橢圓不用正圓：畫面是 16:9，正圓的暗角會在左右留下看得出來的弧線。調參器補上暗角的濃度／範圍／中心三根滑桿。 **主色鋪底已上線**（2026-08-31 使用者裁示「主色鋪底要開」）：這張圖的平均色鋪成一整面漸層，墊在模糊層底下。模糊層退到 0.6 之後，暗的那一半會露出一片與作品無關的黑；鋪上主色之後，整屏最暗的地方仍帶著這件作品的顏色。**顏色由 JS 逐張算**（`components.js` 的 `canvasTint`：把圖畫進 1×1 畫布讓瀏覽器做平均，用 src 當快取鍵，換圖由 MutationObserver 觸發），寫死 hex 只對某一張圖成立——而這裡的圖是使用者自己上傳的。算不出來（跨源、載入失敗）就退回檯面色。 |
| Split bar | 🟡 molecule | ✓ App | 一個總數＋它的組成：分段長條與圖例（2026-08-31 建）。首個消費情境是展示版左欄的收入分布與粉絲等級分布。**用橫條不用圓餅**：那條欄只有 300px 寬，圓餅要佔掉一個正方形、而旁邊還有一個必須夠大的總數；讀者在這裡要的是「哪一塊比較大、差多少」，橫條吃一行就講完。**分段之間用 2px `gap` 不用邊框**（邊框會算進寬度、比例就不準；完全不留縫時明度接近的相鄰兩段會讀成一整條），極小的段有 `min-width` 保底，否則圖例會有一列對不到條。**圖例的點是實心 7px、標籤是白字**（2026-08-31 使用者裁示；原本是空心 9px＋灰標籤，理由是「條上的顏色有面積代表量、圖例上只是索引」；改實心後直徑同日再收到 7px、列內間距 8 → 6——同樣直徑實心比空心重得多，而點愈小、固定的間距看起來就愈寬）：翻過來的理由是這一格躺在照片上——9px 的空心圈只剩一圈 2px 的線，而顏色正是這一列與條上那一段對應起來的唯一線索，在花的底圖上會直接消失；右邊的百分比本來就是白的，名稱跟著它同色，一列讀起來是一件事。**只有兩欄，中間不放金額或人數**——把佔比乘回總數會得到一個看起來很具體、實際上混了期間的數字（收入的百分比取自收入總覽頁、總額是本週），寧可少一欄不要多一個編出來的數字。資料來源：收入＝`earnings-overview.html` 收入來源分布的前四大＋其他；等級＝`DATA['dash-insight'].fans.tiers`。 | [split-bar.css](./ds-components/split-bar.css) |
| Donut mix | 🟡 molecule | ✓ App | 一個總數拆成幾塊、畫成圓環（2026-08-31 建，依使用者提供的參考圖；首個消費情境是展示版左欄的粉絲分布）。**站上三支圓環並存、互不取代**：`.donut`（chart.css）只吃兩段，`.pie`（chart.css）是沒有縫也沒有圓端的 conic-gradient，本支是多段＋段間留縫＋圓端。差別不在裝飾——圓餅讀的是「切開的整體」，有縫的環讓每一段成為獨立的一筆。**為什麼不擴充 `.pie`**：conic-gradient 畫不出圓端與縫，那兩件事正是這個圖的樣子，所以改用 SVG 的 `stroke-dasharray` 畫弧。**縫要多扣一整個線寬**：`stroke-linecap: round` 會在每一端各多畫半個線寬，不扣的話段與段會黏在一起、縫等於沒有（實測 12px 線寬把 8px 的縫整個吃掉）；SVG 整個轉 -90°，第一段才從十二點鐘開始。**總數住在環心**，所以這一張不再另排上方那個 56px 大數字（同一個數字在同一張卡上出現兩次不多講任何事）；環心的字比標題數字小是因為它要裝進 80px 的洞，放大就得把環放大，而環一大旁邊的圖例就沒有寬度。**圖例借 `.split-bar__legend`**：同一份資料的圖例沒有理由有兩套寫法。**與 split-bar 的取捨**：環用寬度換高度（總數收進中心、不必另排一行），長條要那一行、但幾乎不吃高度。 **2026-09-01 起在展示版那一格被 `.tier-arc` 取代**（粉絲分層是層層包含的資料，見該列）：這一支目前沒有消費頁，屬**退場候選**，但不逕自退場——它畫的「一個總數拆成幾塊、段間留縫」仍是站上唯一的那種畫法，等使用者裁決。CSS 與 DS demo 保留。 | [donut-mix.css](./ds-components/donut-mix.css) **環放大、線收細**（2026-08-31 使用者裁示「大一點、窄一點」）：`--donut-mix-size` 116 → 150、`--donut-mix-thickness` 12 → 9。厚度這個數字與 `components.js` 的 `donutMix()` 裡的 `W` 是同一件事——段間的縫要扣掉一整個描邊寬（圓端各吃半個），兩邊不同步縫就會忽寬忽窄。圖例同時收窄（`max-width: 190px`）：名稱是 `flex: 1` 撐開的，欄位有多寬百分比就被推多遠，四行讀起來像四段各自為政的長句。**圖例整組貼著整段的右緣**（`justify-content: space-between`，同日使用者裁示「這一堆在整個 section 靠右」）：環與圖例之間的空白因此是「這一格有多寬」的結果，不是寫死的間距；百分比也與這一格其他靠右的東西（標題列的兩顆箭頭）收在同一條線上（實測皆 432px）。 **在輪播裡多讓一段上緣**（`.distribution .donut-mix { margin-top: 16px }`，2026-08-31 使用者裁示「粉絲分布加一點上 padding，收入不用動」）：收入那張的下一件事是一行大數字，字的上緣本來就有行高撐著；環是一個實心的圓，緊接在標題底下會像貼在標題上。只加在這一張，收入那張不受影響（實測環的上緣 12 → 28px，輪播外框仍是 210px）。 **2026-09-01 復活＋新變體 `--stats`**：8/31 展示版那格改版後這支一度零消費（退場候選）；使用者裁示項目詳情的「方案統計」改成圓環圖（附參考圖：環在左、總數收進環心、圖例在右），資料形狀正是本元件的定義，不另造第三種環。`.donut-mix__legend--stats` 解掉 190px 上限、補 `.donut-mix__amt` 數字欄（售出／累積金額），另加 `.split-bar__row--head` 欄名列與`.donut-mix--stack`（767px 以下環與圖例上下疊）。**幾何已抽成共用 `js/donut.js`**（`window.ZtorDonut.segsHTML/ringHTML`，比照 linechart.js）；⚠ `js/components.js` 的 `donutMix()` 仍是同一份計算的第二份副本，下一輪要改成呼叫共用檔。 |
| Tier arc | 🟡 molecule | ✓ App | 層層包含的分層弧（2026-09-01 建；使用者說明粉絲分層是 Fan ⊇ Ranked fans ⊇ Superfan ⊇ Inner Circle，並從 `docs/fan-tiers-demo.html` 的四個方案選定這一支，形狀由使用者自己提）。**在展示版左欄那一格取代 `.donut-mix`**：互斥四塊的圓環前提是「一個整體被切開」，把包含關係畫成那樣會讀成四群並列的人，而 Inner Circle 其實是同一群人裡走得最近的一小撮——圖型用錯，不是尺寸或顏色的問題。**成立的關鍵是兩個維度分開編碼**：角度＝這一層有多少人（累積佔比，最外層 100%），線寬＝第幾層（最外層最粗、往內遞減）。任何一個角度往中心看，看得到幾層就代表那個角度的人走到了第幾層。四層同半徑、都從十二點鐘開始，線寬 26/20/15/11 於半徑 60（外緣 73，畫布半徑上限 75；再粗會被裁一角）。線寬由行內屬性給、不是 CSS 常數——它是資料的一部分。**吃累積佔比不是「只到這一層」**：store 是 12/28/37/23（和為 100），由最內層往外累加成 12 → 40 → 77 → 100，是換一種讀法、不新增資料（ASSUMPTIONS CANVAS-004）。**顏色吃 `--tier-1..4` 不是 `--chart-*`**（見 Pillar 2 的色彩角色表）。圖例的圓點大小跟著該層線寬走，圖例與圖共用同一個編碼。**曾試過「末端加粗」的漸變、當日撤回**：線寬一漸變就不再是定值，而定值正是用來編「第幾層」的維度。 | [tier-arc.css](./ds-components/tier-arc.css) |
| KPI rotator | 🟡 molecule | ✓ App | 展示版左欄第一格：收入走勢與粉絲分布輪流講，8 秒一次（**2026-08-31 兩張各換一種圖，使用者裁示並附參考圖**：收入改成走勢曲線——借 `.sparkline` 畫，總收入那條直接用 KPI 卡那條「近 9 週滾動七日收入」、不另造一份；**圖上共三條線**（總收入＋電子商店＋共創計畫，2026-08-31 使用者裁示「曲線要多幾條」），附加線走 `.sparkline__line--sub`（細一階、吃 `--line` 的顏色、不畫面積——面積疊起來會蓋掉底下那條），且與主線共用同一組值域，各自算極值就會變成「每條線都填滿整個高度」那種不能比的圖。**兩條來源線是示意資料**（站上沒有逐來源的時間序列，只有當期份額；末端對齊真實份額、中段形狀是編的，記在 ASSUMPTIONS CANVAS-002），刻意不用「總收入 × 固定份額」去推——那會畫出與總收入完全平行的線，看起來有比較、實際上沒有。**刻度 2026-09-01 加密成三上四下**（縱軸 $20k／$15k／$10k、橫軸 9 週／6 週／3 週／本週，使用者裁示「橫軸增加兩個 divide、縱軸增加一個」；新增的那條橫格線插在原本兩條之間，$20k 與 $10k 的位置不動——畫面上只是多一條線，不是整組刻度重排。實測四個橫軸標籤在 269px 的軸上不互疊）。前一版是**兩上兩下**（縱軸 $20k／$10k，橫軸 9 週／本週，使用者裁示「不要太多」），**值域另外指定 0–26k**：格線要落在整數、值域卻得包住 24.8k 的最高點，兩件事分開寫才不會把線畫到畫布外（實測踩過，線在頂邊被切平）；從 0 起算是因為附加線只有 5–7k，不從 0 起算它們會擠在底邊變成兩條直線。線下另有一排小色鍵標出哪條是誰；**標題跟著改成「收入走勢」**，畫的不是分布了，名字不改就是說謊。粉絲改成圓環 `.donut-mix`，總數收進環心、圖例並排在右。**兩張不同型是刻意的**：它們講的本來就是兩件事，硬要同一種畫法一定有一張在遷就；輪播的高度取最高的那一張，兩型並存不會讓外框跳動）（2026-08-31 建；同日再調——內容由兩張 KPI 卡改成兩張 `split-bar` 分布卡，標題改與另外兩區同階的白色 `--fs-14`，前一版沿用 `.kpi` 的 muted 小標在照片上讀起來像註腳）。**進度點退場、改成標題右邊可點的左右兩顆箭頭**（`.canvas-step`，2026-08-31 由本元件提升成左欄共用，定義在 `canvas-home.css`——進行中那一格同日也改成左右切換，同一個手勢不該有兩顆長得一樣卻各自定義的按鈕）：點只是純指示，箭頭同時讓你自己切換，同一個位置能多做一件事就沒理由留只會看的那種；兩顆而不是一顆（同日再調）——一顆只能往前，走過頭要繞一圈才回得來。它們與另外兩區的「查看更多」同字形同尺寸（三區標題列因此對齊），差別是這兩顆不離開頁面所以擺在標題內側。總數字級 `--fs-32` → `--fs-44` → `--fs-56`（同日兩次裁示）：這一格是左欄的頭一件事，總數要能一眼讀完，32px 在滿版照片上撐不住；進行中那格的數字也放大之後，44px 又顯得不夠。**並排不行**：兩張都有大數字與走勢圖，並排會把數字壓到讀不出份量，而份量是展示版唯一要保住的東西。**淡入淡出不用滑動**：滑動帶方向，方向會被讀成「還有下一頁、可以自己翻」，這裡沒有可翻的東西。**所有張放同一個 grid 格**（不是絕對定位）：絕對定位的元素不參與高度計算，外框就得靠 JS 量高、換語言換字型都要重量一次。指標停在上面時暫停。**全站一次只有一個東西自己在動**：這一格與儀表板收入欄的 10 秒輪替不會同時出現。資料直接借 `dash-ops-4up` 的前兩張，不另造一份——同一個數字兩份定義遲早對不起來。 | [kpi-rotator.css](./ds-components/kpi-rotator.css) **色鍵預設只留三個、其餘收成「…」，滑過這一格才全部展開**（2026-09-01 使用者裁示）：六個色鍵在 300px 的欄裡要排三行，而它是讀圖的**前提**、不是內容——常駐三行等於讓圖例比圖還大。留下的是 Total ＋ 前兩支來源，**照清單既有的順序取前兩個**（那份順序本來就是由大到小，只有「其他」被刻意釘在最後——它是把尾巴加總起來的那一項，不該因為數字大就排到前面代表一個來源）。展開用 CSS 不用 JS（純顯示切換、沒有狀態要記，用 class 還得處理重畫後的還原），觸發同時吃 `:hover` 與 `:focus-within`（這一格裡有兩顆切換箭頭，鍵盤走到那裡也該看得到全部）。展開只長高不改寬，多出來的兩行吃在格線第一列的餘裕裡（見 Canvas home 的 `--rail-split`），不會把待辦那一格往下推。 **曲線的色鍵排在圖上方、靠右**（2026-08-31 使用者裁示）：色鍵是讀圖的前提，先看到才知道等一下的三條線各是誰；排在圖下面時，讀者是看完圖再回頭找對照。靠右是為了與左邊的大數字分工——同一段的兩端，一邊是答案、一邊是圖例。 **大數字 56 → 44px**（2026-08-31 使用者裁示「字小一點」）：56 之後它比第二屏卡上的數字還大一階，整頁最大的字落在一個示意數字上；44 仍是這一欄最大的字。 |
| Canvas home | 🟠 organism | SiteSpecific | `index.html` 的版型層（2026-08-31 建）：窄的左欄三格（收入輪播／兩個進行中項目／三則待處理）浮在**鋪滿整個視窗**的底圖上，整頁釘在視窗高度、不捲。**底圖用 `fixed` 不是 `absolute`**（2026-08-31 使用者裁示「是一整個背景都換」）：`absolute` 只鋪得到內容區，側欄與頂列會露出底色，讀起來就變成「右邊那一格放了張圖」。**但浮層另成一層 `.canvas-home__overlay`、貼齊內容區四邊**（同日再調，使用者裁示「這些是一個區塊」）：底圖要滿到視窗邊才是「地」，模式切換／作品切換器／右下說明卻必須跟左欄同一個框——貼視窗邊的話它們會比左欄多凸出頁面內距那一段，四個角對不齊。這一層 `pointer-events: none`、只有裡面的控制吃事件，否則它蓋在左欄上會讓左欄整個點不動。左欄的內距一併歸零，四個角實測差 0px。**外殼高度要扣掉自己的上下留白**（`calc(100vh - gutter*2)`，2026-08-31）：外殼是從視窗邊往內縮 14px 開始畫的，再給它整整 100vh，下緣就會掉出視窗 14px——看起來是「上面有間距、下面沒有」。**頁面下內距 0**：內容區的底邊要與側欄的底邊切齊，留 24px 的話左欄最後一格與右下的作品名都會比側欄早結束一截，三根柱子的底部對不齊（實測改後三者差 0）。側欄保留它原本 4% 的薄膜玻璃、只抬到底圖之上——那一層幾乎看不見，但讓導覽的字在照片上仍有自己的底。**三格之間的間距 28 → 48px**（2026-08-31 使用者裁示，與同日的列距歸零成對）：區內不留空、區與區之間放大，三格才讀成三件事而不是一長串；多出來的高度由 `flex: 1` 的最後一格（待辦）吸收。**左欄三格一律不畫卡框**（`.card`／`.kpi`／`.issue-panel` 在此容器內背景邊框陰影全清），**待辦那三則同日也交出卡面**（見 Issue panel 的 compact 段；**面板本身要吃 12px 左右內距再用負外距抵銷**——面板是左欄最後一格、帶著 `overflow: hidden`，而卡片用負外距把 hover 的玻璃面撐到文字之外，兩者相撞會把那塊面左右各切掉 12px、圓角削平，2026-08-31 踩過）：一天之內走過三版（有框的卡 → 無框毛玻璃 → 什麼都不畫），最後的理由是另外兩格本來就是直接壓在底圖上的字，只有這一格是三塊浮起來的面。**換語言要整塊重畫**（2026-08-31 修，使用者回報「這區塊的英文語系沒做」）：這幾格的字是渲染當下用 `canvasZh()` 從 store 挑出來的（作品名、類型、單位詞、倒數），不是掛 `data-i18n` 的靜態標籤，`applyI18n` 掃不到。事件是 **`i18n:applied`**、不是 `ztor:langchange`（後者站上沒有任何地方發，這一頁的頂列本來也掛錯在它上面）；並且要比對語言碼才重畫——`applyI18n` 每次套用都會發這個事件，無條件重畫會與 i18n 來回觸發。**右緣的切換器＝一條時間線：從最近的進行中，排到未來即將開始的**（2026-08-31 使用者兩次裁示：先重新定義第一屏「錢與粉絲的狀況、必須做的待辦、接下來要進行哪些項目，並用這些項目的視覺當背景」，再定「這裡從最近的進行中，排到未來即將開始的活動」）：六列、近的在上，收募資／預購中的項目（剩幾天截止）、售票中與今天正在舉行的活動、還沒到上線日的項目、準備中的活動，全部換算成「距今幾天」排在一起；每一列的小字寫的是**這一件現在在做什麼**（募資／預購／售票中／進行中／準備中）再接那個數字（2026-08-31 使用者裁示「項目應該顯示募資、預購等，不是顯示類型」）——只有上線那一種仍寫分類（電影／MV／音樂專輯），因為它的倒數本身已經說了「12 天後上線」，小字再寫一次「上線」是同一件事講兩次。選一件就換整屏的地。**天數的來源**：有日期的直接算；募資與預購沒有截止日欄位，從 store 寫好的那句「剩餘 20 天」取數——**只認倒數句型**，實測抓到數字就算會把「審核已過，11/07 上映」讀成 11 天。**同一天只留一件**：store 的 live 活動日期是動態今天（ASSUMPTIONS EVD-006），不擋六格會全被今天塞滿（實測）。**這一排自己帶一行標題**（`.canvas-stage__rail-title`「項目與活動」，2026-08-31 使用者裁示）：字級與左欄的區塊標題同一階，沒有名字時六行靠右的字讀起來像裝飾不像清單。**名稱那條線固定不動，滑鼠進到這一排時往右長的是玻璃面**（2026-08-31 兩次裁示走到位）：先前是「整排往左讓出 60px」，字會跳；使用者裁示改成「字的右緣固定切齊那條線，前往與箭頭從那條線往右凸出來、但還是在框內」。做法是 hover 時同時給右內距與**等量的負右外距**——這一排是 `align-items: flex-end`，只加內距一定把字往左推。整排因此往內縮 24px，長出去的 60px 才有地方去（實測靜止 1352／hover 時框右緣 1422，外殼邊界 1426）；頂列的時間對齊那條線一次就永遠對齊。**左欄兩格的間距 48 → 72**：「上線進行中」撤除後只剩兩格，48 讀起來像同一段落斷了一行。**左欄兩格由第一屏的垂直中線切開**（2026-09-01 使用者在畫面上畫線裁示「待辦事項的起始點要從那條紅線開始」）：上半是「現在的狀況」，下半是「要處理的事」。做法是 `display: grid` ＋ `grid-template-rows: var(--rail-split) auto`，第一列的高度由頁尾腳本 `sizeRailSplit()` 量出來（＝第一屏的中線減左欄的上緣，實測中線 470、待辦上緣 470）——**那兩個位置都不是常數**（問候可能兩行、外殼有內距），寫死一個數字換個視窗高度就跑掉；量不到之前退回 `auto`。列距同時歸零（分界由中線決定，再加列距就會把待辦推離那條線），最後一格補 `align-self: start`（grid 預設 stretch 會把面板拉到列底、裡面的清單卻不會跟著長）。**先前是 `space-between`**（2026-08-31「讓左欄的底邊與右欄的視覺重量對稱」）——那個做法的代價是待辦的上緣由它自己有多高決定，多一則整格就往上長一截，那條線每次落在不同的地方。先前的說明留著：：上面那格貼頂、待辦貼底（最後一格若還 `flex: 1`，剩下的高度會被它自己吸走——框到底邊、清單卻停在原處，實測框底 886／最後一列 731），中間的空白由這一屏剩多少決定，72 因此變成下限。**同一支規則裡多寫的 `min-height: 0` 一併移除**——它把上面那行 `min-height: 100%` 整條蓋掉，左欄一直停在內容結束處（實測 731，底下空 100px）；`.canvas-hero` 同日踩過一模一樣的坑。**左欄的「上線進行中」那一格同時撤除**（墓碑）：它講的正是接下來要上線的，已被這一排收走；渲染器與 `.work-row` 樣式留著沒刪。**同一輪修掉一個舊 bug**：`.canvas-hero` 在同一支規則裡把 `min-height: calc(一屏)` 又寫了一次 `min-height: 0`，後者一直贏——先前看不出來是因為左欄夠長；左欄少一格之後第一屏縮到 653，第二屏從視窗中間就開始露出（實測 liveTop 707 < 856）。**進行中分成四組**（左欄那一格的歷史，現已撤除）：上線／募資／預購／活動。**右緣的作品切換器與左欄第一格切齊**（2026-08-31 使用者裁示「選項要放在綠色的位置」）：它原本緊接在頂列底下，第一項會落在頂列與工作區之間那段 56px 的呼吸裡，看起來像頂列的附屬品；往下讓開同一段間距之後，左右兩邊的內容從同一條線開始（實測皆 161px）。**同日再調成垂直置中、不再切齊上緣**（使用者裁示「這一整個是不是稍微往下移一點」）：這一排只有 315px，左欄那一欄有 725px，兩邊都從同一條線開始時右半邊的重量全壓在上緣、下半邊整片空著。做法是把這一排的下緣接到 sticky 容器的底（＝左欄的下緣）再 `align-items: center`，**位移不寫死**——容器高度是腳本按左欄算出來的，待辦一多左欄就變長，置中會自己跟著走；那個距離由 `sizeStick()` 寫成 `--picks-span`（相對定位的參考框是頂列，只有腳本量得到「頂列下緣到左欄下緣」有多遠）。框拉高之後自己不吃指標事件，只有裡面那一排吃，否則它會蓋掉右半邊的底圖（實測置中後 367–681，左欄 161–886）。**2026-09-01 再調成「上緣固定、成長往下推」**（使用者裁示「再往上移一點，然後標題是 fix 的，hover 上去的時候往下推動位置」）：垂直置中的代價是這一排的高度一變、整排就上下跳——滑過任何一列都會長出一行類型小字，連那行標題都跟著動。改成從容器上緣量 `calc(var(--picks-span) * 0.2)`（實測上緣 318，比置中高 49），標題因此是一條不動的線，長出來的那一行只把它下面的項目往下推。用比例不寫死的數字，理由同上：左欄一長，這一排會自己跟著往下。**左欄的下留白同時 72 → 96px**（使用者裁示「往上移動一點」，實測最後一列底邊 814 → 790）。**兩區的切換箭頭都靠右**（2026-08-31 使用者裁示「試試看看」）：取捨要知道，靠右之後箭頭離它控制的標題最遠，但整欄多了一條對齊線——收入分布與進行中的兩組箭頭收在同一個右緣（實測皆 392–432），讀起來是同一種控制；一次只有兩顆、位置固定，找一次就記得住。做法是 `margin-left: auto` 而不是把標題列改成 `space-between`（將來標題列多一個東西時，`space-between` 會把它攤到中間）。**切換用標題旁的左右兩顆箭頭、不列分頁標籤**（同日再調，使用者裁示「改成和收入分佈一樣，不需要列出上線募資預購等，直接左右切換即可」）：四個標籤在 300px 的欄裡是四個一直在那裡、卻九成時間不會被按的字；箭頭是與左欄第一格共用的同一顆 `.canvas-step`（同日由 `.distribution__step` 提升，定義搬到本檔；寬度 24 → 20px，兩顆才讀成一組前後而不是兩顆分開的按鈕，高度維持 24px 不縮點擊面積）。**整句標題跟著組別換**（2026-08-31 再調，使用者裁示「xxx進行中，每次換頁就換名」）：「上線進行中」「募資進行中」「預購進行中」「活動進行中」，不是「進行中 · 上線」那種主標＋灰字限定詞的拼法（`.canvas-block__sub` 同時撤除）。一句話比兩截好讀，兩個語言各照自己的語序寫——中文把組名放前面，英文放後面（Ongoing releases）。組名不能省：它是這幾列在講誰的唯一線索，每一列上的「類型」講的是作品分類（電影／MV），不是這一組。四組頭尾相接、環狀走；**不自動輪播**，那個名額在收入那格（一個畫面只能有一個東西自己在動）。標題列由渲染器整塊畫出來，不留在頁面 HTML 裡靠 JS 事後改 DOM——標題跟著組別變，兩條路徑遲早會分岔。**「進行中」的口徑各自不同，這是刻意的**——專案三種取 `scheduled` ＋ `published`（正在跑），**不含 `live`**（專案的 live 是「已經上線了」，那件事做完了）；活動取 `scheduled` ＋ `on-sale` ＋ `live`（活動的 live 是「正在舉行」，與專案的 live 意思相反，那正是最該盯著的時候）。**每一列固定三段，但四個分頁的白字各自不同**（2026-08-31 使用者裁示）：灰字寫「名稱 · 類型」、白字寫關鍵數字、底下是進度條與小字。白字帶分母（2026-08-31 使用者裁示「金額與票數要有 1000/1000，分母用小字」）：分子是這一列的答案（賣了多少、募到多少），分母是讀懂它需要的尺度，同樣大小的話「18,400 / 20,000」會讀成兩個並列的數字，而它們是一件事的兩半；**單位詞跟著分母走**（「18,400 / 20,000 張票」才是一句話，跟在分子後面會變成「18,400 張票 / 20,000」）。上線那一組沒有分母——倒數本來就沒有分母可言。白字＝上線是「12 天後上線」（2026-08-31 使用者裁示；那一列沒有別的東西在講「這是要上線的」，倒數本身就得把事情說完，英文寫 Live in 12 days——兩個語言各照自己的語序）／募資是已募金額／預購是已售筆數／活動是已售票數；**剩餘天數掛在白字那一行的右邊、基線對齊**（2026-08-31 再調）——兩者講的是同一件事的兩面（賣了多少／還剩多久），排成上下兩行會讀成兩件事，也讓每一列多長一行；進度條因此緊接在這一行底下，三者連成一組。**上線那一組的日期同日撤除**：倒數已經是同一個日期的另一種說法，兩個都寫等於同一件事講兩次。**名稱刻意退成灰字、白字全列只留一個**：這一格一次只看得到兩件事，人是先掃「賣得如何、還剩幾天」，名稱是回頭確認用的；名稱與數字同為白字時兩者互搶，一列要讀兩次。預購的數字取自 `meta` 那串三段字（「62 / 100 筆預購 · 單價 $28 · 剩 9 天」），第一段切出已售數、最後一段當小字，**單位詞沿用 store 自己寫的那一個**（電影票寫「張票」、周邊寫「筆預購」）——改寫任何一邊都是竄改產品事實。組別鍵與 store 的類型分開存（上線那一組的類型是 `go-live` 帶連字號，靠鍵名去猜會全部比對不到，實測踩過）。**進行中項目是橫的一列一件**（`.work-row`，**2:3 直式**縮圖在左、三段字在右）：直式封面在 300px 的窄欄裡會排成兩根柱子、跟上下兩區的節奏對不起來，也把高度吃掉一半。**縮圖 2026-08-31 由 1:1 改成 2:3 直式**（使用者裁示）：站上的作品主視覺本來就是直式海報，切成正方會把海報上下裁掉、變成一張認不出來的方塊。尺寸 44×66——它現在是決定列高的那一項（文字只剩兩行），縮圖與它幾乎等高，橫排的節奏還在；再大就會把整列撐成一張卡（實測列高：上線 78px、其餘含進度條 86px，左欄底邊仍與側欄齊平、溢出 0）。字級三段拉開兩階以上（12 → 20 → 11），只靠顏色分主次不夠，會讀成三行等重的說明。**只有數字是白的**（2026-08-31 再調，使用者裁示「只有數字要用白色，其他字都用灰色變小」；**緊貼數字的單位另補 2px 外距**——幣別符號與數字之間沒有空白時，小字的 `$` 會讀成大數字被壓扁的一部分，判斷靠相鄰段是不是數字，自己帶空白的單位不補）：白字同日再放大到 `--fs-24`（單位退掉之後，白字只剩那個數字，字級跟著補上），圍著它的單位（幣別符號、「還有」「天」「張票」）降成灰的 13px 小字，切分在渲染器的 `workValue()` 做，整串沒有數字時（「尚未排期」「今天」）原樣輸出——那時候那句話本身就是答案。名稱與類型那一行同時 13 → 12。**每一列等高（`min-height: 78px`，原本 88，最底下那行小字收掉後四組的自然高度都是 78——由縮圖 66 加上下內距決定）**：讓每一組各自算高度的話，切到上線那一組整塊會往上縮一截，四組輪流看就是畫面在跳；列距 12 → 20 → 12 → **0**（同日三次裁示，最後一次是「直接去掉間距」：每一列等高之後，空白已經由那個高度與每一列自己的內距提供，列距是第三份空白；實測四組皆 88px、左欄底邊與側欄齊平、溢出 0）。**「查看更多」2026-08-31 兩區一起撤除**（`.canvas-block__more`／`.issue-panel__more`，使用者裁示）：取捨要知道，左欄只出兩件進行中與三則待辦，現在沒有一條路通往其餘的；之所以還過得去，是兩區的標題都不再宣稱總數（待辦同日拿掉筆數、進行中寫的是組名）。展示版的前提是一眼看完，不是查得完。**頂列的日期與時間 `--fs-13` → `--fs-15`**（2026-08-31 使用者裁示「字再大一點」）：問候 22px 的另一端擺 13px 會讀成註腳，而它是這一頁少數幾個要被看見的事實之一。**這一頁的區塊標題一律同一階**（2026-08-31 使用者裁示「都改成像收入走勢一樣的文字大小」）：基準是 `.distribution__title`（`--fs-14`／500）。原本這一頁上有三種標題字級——14（收入走勢、進行中）、18（待辦事項）、22（進行中／近期、平台表現），而它們在版面上是同一階的東西（都是「這一區叫什麼」），字級不一致會讓人以為分了層。**只在這一頁改**：`.issue-panel__title` 與 `.section-head__title` 都是全站共用的，正式儀表板那邊的節奏是對的（區塊更大、階層更多），所以選擇器一律綁在這一頁自己的容器上（實測 index.html 仍是 18／22）。（磚列的分頁詞隨磚列一起退場，那一條規則同日刪。）**問候底下補上與正式儀表板同一句副標**（`dash.sub`，2026-08-31 使用者裁示「加到問候底下」）：兩頁講的是同一件事，這一句不另寫一份。**這一句一行寫完、不斷行**（同日使用者裁示；原本有 `max-width: 46ch`，一併撤除）：斷成兩行之後它在版面上的份量就跟底下的區塊標題一樣重，讀起來像一個段落而不是一句話。實測中文 462px、英文 506px，離右邊的日期時間都還有四百多像素；真的擠到時以 ellipsis 收尾。**頂列只放日期與時間**（2026-08-31 再調，原本還有「今日進帳」——那個數字在左欄第一格的收入分布裡講得更完整）；問候字級 28 → 22，它是招呼不是這一頁的標題，真正要被讀的大數字在下面那一格。頂列與工作區的間距 20 → 40 → 56px。**側欄收合**同日由「只在這一頁」升級成**全站功能**（見 Pillar 1 的 App shell 段），這一頁不再自帶一份。**往下捲的提示**（`.canvas-hint`，2026-08-31 使用者裁示；**2026-09-01 再調兩次**：先是「往右邊一點，然後大一點」（字級 12 → 14、內距 6/14 → 10/20、箭頭 16 → 18；內容區的正中偏左，左欄那一欄的重量本來就壓在左邊，提示站在正中會被讀成左欄的收尾），同日再依「改成待辦事項那樣，只要有文字和箭頭就好」**把玻璃膠囊整個拿掉**——這一頁左欄的三個區塊標題本來就是直接壓在照片上的字、沒有任何面，這顆鈕是同一階的東西，掛著一片膠囊就變成畫面上唯一一張浮起來的貼紙；可讀性改由 `text-shadow` 一個人扛（左欄靠的是舞台壓底，這顆站在右下角、壓底較弱），字級與字重對齊 `.issue-panel__title`（14 / 500），左右內距歸零、上下留 10px 當點擊面積（鈕高仍 40px）。同日使用者也在畫面上把它一路指到右邊、再裁示「和『近期項目與活動』靠右對齊」，於是**改成靠右定位、右緣切齊右排那一行標題**：`right: calc(--sp-24 + --sp-10)`——值等於右排自己的 `right: --sp-24` 加上它那 10px 的右內距（滑鼠進到那一排時讓玻璃面往右長的預留，見 canvas-stage.css）。**用這兩支相加而不是寫死 34**：那兩個數字哪天動了，這條線會自己跟著走。實測標題、每一列名稱與這顆鈕的右緣都是 1352。先前那個「畫面正中再往右讓一段」的算式，參考點是內容區的中線，而中線會跟著側欄收合左右跑——側欄一收提示就換一個位置；改成從右緣量之後它固定不動，而且與右排收在同一段（實測提示中線 1222、右排中線 1228））：貼著第一屏的底——左右兩欄之間那道空隙的底端是整屏最空的一塊，提示站在那裡不擠掉任何內容，而「往下」的手勢與位置一致。它是按鈕不只是裝飾：按下去用 `scrollIntoView` 捲到第二屏（不自己算位移，那一段的位置會隨左欄長短變動）。捲動一開始（`y > 8`）就淡出並停止吃指標事件——留著會擋到第二屏的卡片。只有 16px 的箭頭在動，整顆鈕不動：一個畫面只能有一個東西自己在動，那個名額給了收入輪播。**一屏之後還有第二、三段，一段一屏**（2026-08-31 使用者裁示兩次：「往下滑，內容要換」「這個要自己佔一頁的篇幅」）：第一屏（`.canvas-hero`）至少一個視窗高，往下的每一段（`.canvas-sheet`）也各自至少一屏、內容垂直置中——這兩段各自只講一件事（手上有哪些作品／它們表現如何），擠在同一屏會變成「捲到一半的兩個半區塊」。第二段是這一頁自己的進行中列（`live-cards` ＋ `canvas-live`，2026-08-31 取代原本的作品磚列），第三段是平台表現（`[data-perf="dash"]` ＋ performance-store）——後者**直接沿用正式儀表板的標記與資料、不另做一套**（同一件事在兩頁各寫一份，遲早會有一頁沒跟上），前者的資料也仍走 projects／events 兩支 store 的同一組口徑。三個實作要點：① 第一屏的高度要扣三段留白（外殼上下各 14、`.main` 上外距 16、`.page` 上內距 24），少扣一段就會比視窗高、下緣被裁；② `.canvas-hero` 必須 `flex: none`——`.page` 是 `min-height: 0` 的 flex column，不寫的話它會被下面的內容壓成 0（實測，第二段直接疊到第一屏上）；③ **捲動發生在 `.main` 身上、不是視窗**（外殼是固定高度的玻璃殼、內容在面板裡自己捲），監聽掛 `window` 永遠不會觸發；④ **頂列與切換器的 sticky 容器下緣要切齊左欄的下緣**（`.canvas-hero__stick`，2026-08-31 使用者裁示兩次：先「下一段出現時，這兩個也一起往上滑不見」，再「在左欄那一塊滑走之前，頂列與右側都不要動」）：sticky 是被自己容器的下緣推走的，容器等於整個第一屏時，它們會在左欄早就捲完之後還壓在畫面上（第一屏比左欄長）。容器高度＝左欄下緣減容器上緣，並用負下外距把底下的內容擺回原位（負值要把頂列自己的下外距加回來——容器一旦有指定高度，子元素的下外距就不再穿透出來，第一版漏掉時問候與第一格之間 56px 的呼吸整段消失）、自己不吃指標事件；高度只在版面改變時算一次，左欄掛 ResizeObserver 跟著重算——**不要每一幀算位移**，那是會抖的老路。**第二段（進行中的項目與活動）的排版**（2026-08-31 使用者裁示重做三次，取代原本的作品磚列）：一次一件整的、右緣把下一件切一半，翻頁鈕一左一右夾著這一列；不畫卡面、海報放大到約 380×570 佔掉這一屏大半的高度，件與件之間的縫 40px。第二、三屏都**沒有區塊標題**（2026-08-31 使用者裁示刪除「進行中」「平台表現」）：兩屏各自只有一件事，卡片與卡上的字已經說完它是什麼，標題只是重複一次。這一屏的出口是置中的主按鈕（`btn--primary btn--lg`，同日走過置中 → 靠右 → 靠左 → 置中；上外距 16 → 40，卡面回來之後卡的下緣本身就是一條邊界，出口離太近會讀成卡的一部分）；**文字與去處跟著正中央那一件走**（2026-08-31 使用者裁示）：看的是項目就是「查看更多項目」→ `projects.html`，是活動就是「查看更多活動」→ `events.html`（新增 `canvas.more.projects`／`canvas.more.events` 兩個 key）。判準用資料端的落點（`event-detail…`），不猜名稱；文字掛回 `data-i18n` 讓換語言時由 i18n 接手，同時把當下語言的字寫上去，否則要等下一次語言套用才更新——輪播改成置中之後，這一屏的視覺重心在中線上，出口跟著回到中線。內容換成**正在跑的**那幾件——募資、預購、售票中／進行中的活動（`canvas-live`，三組接成一串）；磚列答的是「我有哪些作品」，這一屏要答的是「手上這幾件跑得怎麼樣」。一件的排法見 §4.200 Live item。三個實作要點：① 活動只收售票中與進行中，準備中的連票都還沒開賣、進度條會是一整排的 0（實測 31 張降到 22 張）；② 翻頁用 `data-rail-step="2"`（一次兩張卡的間距），共用的磚列腳本預設捲一個可視寬度，在這一列會停在半張中間；③ 取數與左欄的四組輪播共用 `canvasRowsOf(tab)`，同一個「進行中」的口徑不寫兩份。
| Live item | 🟡 molecule | ✓ App | 展示版第二屏的主角：**一件正在跑的事，講在一張佔掉大半屏高度的海報旁邊**（2026-08-31 建，使用者裁示兩段：「一次兩個活動、第二個顯示一半、右邊留白；每一個活動的簡要資訊與狀態很重要」「海報要大一點、佔整頁高度多一點，**不要用卡片的形式**，文字可以大一點」）。**玻璃卡面，躺在自己的海報上**（2026-08-31 走過三版：有卡 → 不畫卡面 → 使用者裁示「還是把原本的玻璃卡片加回來吧」）：同日這一屏也換成「用正中央那一件的海報當整個視窗的地」（`.live-bg`，`fixed` 鋪滿視窗、畫在所有東西最後面，糊 16px＋暗紗，只在這一屏蓋住視窗中線時亮起來），卡面因此有了理由——字躺在一張糊掉的照片上讀不乾淨，玻璃是把它撐起來的那一層；模糊在這裡合乎 Q77 的但書（下面真的是照片）。**背景節點放在第一屏的底圖旁邊、不放在這一屏裡**：放在裡面會落在那一屏自己的 z-index 之內、把側欄蓋掉（實測）。調這一層有專用的調參器 `docs/live-bg-tuner.html`（模糊／彩度／亮度／對比／不透明／放大＋暗紗兩端，可單獨關掉任一層、可指定海報）；**改完定案值要同步回調參器裡的 `BASE`**，否則下次打開會看到舊畫面。資訊分三個落點——左上是身分（階段藥丸＋作品分類、名稱、場次或條件），左下是狀態（已賣對總量、進度條＋百分比、還剩多久），右下是動作（「前往」，二級按鈕；先前是一行灰字，與「剩 9 天」同一階，讀者分不出哪一行是動作）；那顆按鈕是掛著 `.btn.btn--outline.btn--sm` 的 `<span>`，不是連結——整張卡就是那個連結。海報的高度綁視窗（`min(50vh, 520px)`）不綁這一件的寬度：這一屏剛好一屏高，上下的呼吸就是這一塊沒吃掉的部分（實測各 130px；先前綁寬度時，螢幕愈寬海報愈長、呼吸愈少，正好相反）。中間留白由海報的高度換來；黏在一起這一屏就變回一張小卡。**與 Work card 的分工**：磚列答「我有哪些作品」，這一支答「哪幾件正在跑、跑到哪」，兩支並存。版面：**置中的環狀輪播**（2026-08-31 使用者裁示「置中輪播，左邊放最後一個」）——中間那一件是現在在講的，左右各露出約四分之一件，翻頁鈕一左一右夾著、一次轉一件。列沒有盡頭：渲染器把最後一件排在最前面，所以一開始置中的是第一件、左邊露出的是最後一件；轉動用 `transform` 不用捲動（環狀的列沒有端點，原生捲動一定有兩個），每次動畫結束後把第一件搬到最後、位置歸零，那一步就是轉了一格。基準位移＝（可視區 − 一件）÷ 2 − 間距，改變視窗／換語言／ResizeObserver 都要重算；動畫進行中不接受下一次點擊。一件的寬度用算的——（可視區 − 縫）÷ 1.5（1440 實測：可視區 1134、一件 729、海報 379×569，縫 40）。**四個實測踩過的坑**（前兩個是原生捲動那一版的，改成 transform 之後不再適用，留著是因為下一個做橫向列的人一定會再撞）：① 漸消遮罩掛在可視區、不能掛在捲動區——捲動區帶 mask 時平滑捲動會死掉（走 33px 就停、翻頁鈕沒反應）；② 吸附要用 `proximity` 不能用 `mandatory`——mandatory 會把翻頁的平滑動畫拉回原地（實測 0 → 26 → 0）。③ 兩顆鈕住在三欄格線的頭尾（環狀之後兩顆恆常都在，不再有「到頭收起來」這回事），**每一個子元素都要指定欄位**——到頭那顆是 `display: none`（站上全域規則），靠自動配位後面的東西會整個往前遞補一格（實測可視區縮成 40px）；④ 共用磚列腳本的 `sync()` 原本只看捲動區的父層找鈕，這一列的捲動區外面多包了一層可視區，父層不再是放鈕的那一層——鈕的顯隱從此不更新（翻到第二輪「上一頁」仍收著），改成往上找到第一個裝著鈕的祖先。翻頁鈕與捲動區同在 `__wrap` 底下（共用腳本從鈕的父層往下找捲動區），翻頁改走這一支自己的 `data-live-nav`（一次轉一件；2026-08-31 走過一次兩件 → 一次一件 → 置中環狀）。**活動只收售票中與進行中**：準備中的連票都還沒開賣，進度條會是一整排的 0（實測 31 件降到 22 件）。數字的切法（白字是數字、單位與分母降成灰小字）與左欄那一列共用 `workValue()`，差別只在類別字首。 **底圖 2026-09-01 再糊再暗一階**（使用者裁示「與這一頁呼應的背景再多一點點模糊，並且背景再暗一點點」）：`blur(16px)` → `blur(24px)`、亮度 0.85 → 0.74、暗紗 46–72% → 56–80%。卡是玻璃，底圖一亮就從卡面透上來把卡上的字吃掉；糊得多一階則是讓這一層更明確地退成「地」，不與卡上的海報搶。⚠ 這一組值在 `docs/live-bg-tuner.html` 的 `BASE` 有一份副本，已同步。 |
**第三段（平台表現）的排版**：一塊約三分之一寬、由上而下一塊一行（2026-08-31 使用者裁示三段：「這些區塊都不要這麼寬，大概佔頁面三分之一就好」「這三個各一行」「這也換一行」）。最早是「一個類型一行、標題固定 200px 在左、清單吃剩下的寬度」，在 1440 的視窗裡把長條拉到 1000px 以上——長條的工作是比長短，只需要夠長到看得出差距；拉滿一整行之後第一名與第五名隔著半個螢幕，眼睛反而得來回掃。中途試過三塊並排，使用者裁示改成一塊一行：版稅／影視／音樂／平台觸及／觀眾在哪裡五塊，各自標題在上、清單在下（`.perf-twin` 與它底下的兩個容器都改成單欄 flex），寬度一律 `max-width: 33%`（實測 400px），五塊共用同一條左緣，第一名的長條起點因此全部對齊。bento 收成單欄、覆寫卡片自帶的 `--span-6`。**卡標題前面那顆圖示一律不畫**（2026-08-31 使用者裁示）：卡收到 480 之後，標題只有四五個字，前面掛一顆彩色方塊會比標題本身更先被看到；這一頁的節奏是「字自己排出層次」（第一屏三格、第二屏的卡都沒有圖示）。只在這一頁改——正式儀表板的同一批卡仍有圖示，那一頁的卡更大、卡的種類也更多，圖示在那裡是分辨用的。卡片本身也不吃滿頁寬（2026-08-31 使用者裁示「不要充滿頁寬」）：收到 `max-width: 480px`，清單吃滿卡的內寬（實測 438px）——清單只有 400 上下、卡面卻橫跨 1254 時，一張卡有三分之二是空的，那塊空白會讀成「這裡本來還有東西」。後兩張卡（平台觸及／觀眾在哪裡）的名稱欄改吃內容寬——預設上限 148px 是為作品全名訂的，平台與地區一律 2–7 個字，照 148 留位會把長條壓到只剩 12px。**踩過的坑**：渲染器在兩個家族之間插了一個 `.mt-16` 當間隔，留著會多一段雙倍空白（間隔改由 `gap` 給）；早先它在格線版裡還會佔掉一格、把後面的 cap／rank 全部錯開一位。**每一類只留前五名**：第六名之後在這個位置不幫忙做決定，完整清單走每張卡自己的「依項目查看」。**代價要知道**：五塊疊起來 1379px、比一屏高約六成，這一段要捲一下才看得完（第一、二段仍是一屏）。
**空的樣子分兩類（2026-09-01 使用者裁示「空畫面有很多種」）**——**全新帳號**與**有資料但很少**，不是同一件事。cheat code › 情境 › DATA STATE 因此由兩檔加到四檔（Empty ／ Only product ／ One project ／ Has Data；`empty` 與 `has-data` 的鍵刻意不動——全站三十幾頁的空狀態都掛在 `html[data-data-state="empty"]`，改鍵會一次弄壞所有頁）。

**① 全新帳號 → 引導畫面（`.canvas-start`）**：不假裝是儀表板。頂列留著（這仍是他的工作台），底下換成一句主張 ＋ 三個起點（建立項目／建立商品／建立活動，玻璃卡，目的地站上都真的存在）。**刻意不做「零值儀表板」**：對什麼都還沒開始的人顯示 `$0`、`0 位粉絲`，讀起來是「你不及格」而不是「來開始吧」。一屏不捲——左欄、右排、往下看更多、下面兩屏一律 `.dash-data-only`（實測整頁高 856 < 900，不出現捲軸）。**不畫 `.empty-card`**：那是給有卡框的頁面用的，這一頁左欄本來就不畫卡，在滿版的地上貼一張白卡會是整屏唯一浮起來的東西。**底圖那幾層一起關掉**（`__bg`／`__img`／`__scrim`／`.live-bg`）——沒有作品就沒有海報可以當地。

**② 有資料但很少 → 逐區降級，不是另一個畫面**。使用者點出的關鍵案例是**只建了商品**：右排只吃項目與活動，所以它必然是空的。右排沒有東西時**留一行**「還沒有排定的項目或活動」＋一個建立入口，不整排消失（右半邊莫名空一大塊比留一行糟，而這一行同時是這一頁唯一提示「你還可以建立項目或活動」的位置）；沒有可選主視覺時**退用商品主圖**（`ZTOR_PRODUCTS` 既有欄位，只讀）；收入／粉絲收成一張零值卡（`mode: 'zero'`，大數字＋一句話、**不畫圖**——全平的線或空的環讀起來像壞掉，只剩一張時箭頭自己收掉）；待辦留一則（不歸零：剛建立第一件東西的人，站上本來就有必辦事項在等他）；下面兩屏依狀態收放（只有商品→兩屏全收、提示跟著收；只有一個項目→第二屏成立、第三屏仍不成立）。

**兩個實作要點**：**取數層改在這一頁、不動 store**（`canvasScope`／`canvasSparse`／`canvasTrim` 讀 `<html>` 上的狀態，把餵給渲染器的資料裁到該有的量——store 是三十幾頁共用的，為了一頁的示範去改它等於把風險攤到全站）；**要掛 `ztor:devstate-changed` 重畫**（devtools 只改屬性、不會叫任何人重畫，不掛這一條在面板上切檔位畫面會停在上一種資料，看起來像開關壞了，實測踩到）。權宜與待補見 ASSUMPTIONS CANVAS-005。**主色鋪底的歸位要在 JS 做、不能只寫 CSS**：`--canvas-tint` 是 `canvasTint()` 寫成行內樣式的（當下那張海報的平均色），行內贏過任何選擇器；不清掉的話空狀態會是一片說不出來歷的橘褐（實測踩過）。該函式因此改成先看 `data-data-state`，空就移除變數，並掛上 `ztor:devstate-changed` 重算，空↔有資料切回來時顏色會回來。文案沿用既有的 `canvas.empty.title`／`canvas.empty.text`／`dash.empty.ops.cta` 三支 key，不另寫一份。

**捲動要在整頁都有效**：底圖 `pointer-events: none`（2026-08-31 使用者回報「滑鼠只有對著左邊內容滑動畫面才會往下」）——它是 `fixed` 的，滾輪落在它身上時瀏覽器找的是它的容器塊，那是視窗，而視窗這一層鎖住不捲；放行之後滾輪穿過去交給 `.main`。**頂列與右緣切換器包在同一塊 sticky（`.canvas-hero__chrome`）**（同日使用者裁示兩次：先「要 fix 住，直到下一段出現才滑掉」，再「他們應該同步往上滑不見」）：容器是 `.canvas-hero`，整塊只在第一段的範圍內釘著，第二段推到頂時被推走。**兩者一定要在同一塊裡**——sticky 的元素是被自己容器的下緣推走的，時機取決於元素自己的高度，切換器 250px、頂列 46px，各自 sticky 一定不同步（實測捲到 860 時頂列還在 4、切換器已到 -139）。切換器因此從浮層裡搬出來、自成一個渲染器（`canvas-picks`），在 chrome 裡用 `position: absolute; top: 100%` 掛在頂列底下、貼內容區右緣（不佔版面高度）。**中途曾用捲動事件算位移，那是錯的**：JS 比瀏覽器的捲動繪製慢半拍，右側會一路抖。**左欄往上捲時上緣漸消**（淡出長度跟著捲動量長、不是固定值——固定值在還沒捲時就把最上面那段蓋掉一半，使用者回報「還沒開始滑就被遮住了」）：遮罩位置由 JS 依捲動量寫進 `--rail-fade-start/-end`（遮罩是相對元素自己的框算的，「畫面頂端切到這一欄的哪裡」只有捲動量知道）；用遮罩不是蓋實色漸層——底下是照片，任何一塊實色都會留下看得見的邊。**遮罩只在 `is-scrolled` 之後才掛上**（2026-09-01 使用者回報「這個 hover 壞掉了」）：帶遮罩的元素會自成一層「背景根」，它裡面任何 `backdrop-filter` 就只取樣得到這一層自己的內容——待辦那幾則 hover 時的毛玻璃因此整個失效，畫面上只剩一層幾乎看不見的白。還沒捲動時遮罩的兩個端點都是 0px、本來就沒在遮任何東西，掛著只有副作用。**待辦事項有幾則就排幾則**（同日使用者裁示）：原本固定三則、多的看不到；現在清單撐高左欄，第一段跟著長高（`.canvas-hero` 是 `min-height` 不是 `height`），往下捲仍在第一段裡。**第一段快走完時底圖與切換器一起淡掉**（門檻＝第一段底邊剩四分之一個螢幕；原本寫成固定的「半個螢幕」，第一段變長之後那個位置還在半路上，照片消失時左欄才捲到待辦事項、看起來像圖突然壞掉。切換器跟著一起走：它是「這張照片是哪一件作品」的控制，照片不在了它就沒有對象，不跟著走會比頂列晚 128px 離開、正好壓在第二段的磚列上）：底圖是 `fixed` 的、不會自己離開，而磚列與排行這種工作內容躺在一張人臉上讀不乾淨；淡掉之後第二段就回到站上原本的星空檯面，與其他頁一致（背景要不要整段換回舊設計，使用者尚未定案）。**做成另一頁而不是改儀表板**：「不捲」是很強的限制，會逼著砍掉工作台需要的深度（分組、完整清單、往下的區塊）；兩種需求各自最佳化比一個版型兩邊遷就好。**入口 2026-08-31 由 cheat code 的 Demo 版本改成正式導覽**（使用者裁示）：側欄「總覽」下面一格「新版總覽」（`nav.dashboard-new`），原本的總覽**不取代**、兩份並存（規格 §3.2.1 沒有這一項，屬刻意的過渡狀態，記在 ASSUMPTIONS CANVAS-001）。新頁進正式導覽時要動三處：`sidebar.js` 的 `NAV` 與 `FULL_ROUTES`、`devtools.js` 的同名 `FULL_ROUTES`（兩份清單必須一致，只改一邊會讓低版本導覽藏得掉、頁內連結卻點得進去）。**原本的 `home-canvas` 版本鍵同時撤除**：留著的話，切版本用的回程守衛會把「停在別版目標頁」的人導回來源頁——正式進站的人一點新版總覽就被彈回總覽（實測踩過）；鍵仍留在兩份 `isFullBaseVersion` 白名單裡，只為了讓還存著舊 devstate 的瀏覽器不會掉進「未知版本＝限縮導覽」。1024px 以下解除高度鎖、改成上下堆疊。 | [canvas-home.css](./ds-components/canvas-home.css) |
| Chart | 🟠 organism | ✓ App | **走勢圖的產生器已抽成共用 `js/linechart.js`（`window.ZtorLinechart`，2026-08-31）**：座標系（viewBox 600×180、格線 23/68/113/158、0 落 158、max 落 23）、Catmull-Rom 平滑、縱軸四刻度、多條線（`seriesHTML`，可給 `cls` 走 s1…s7 語意色，或給 `color` 直接指定——線色要與同卡圖例對齊時用後者）。消費端：活動詳情售票趨勢、項目詳情支持者走勢。**「一段時間的走勢」站上只有這一種畫法**，要畫第二種先回來讀這條。<br> Linechart (Dashboard / Earnings trends) + stacked-bar + source-list legend + rank-bars + **donut**。`.donut`（2026-07-25 新增）＝兩段式比例圓環：外層設 `--donut-p`（第一段百分比 0–100），色彩 `--donut-a`／`--donut-b`（預設 `--chart-3` 綠／`--chart-5` 紫），尺寸 `--donut-size`（預設 220px），圓心挖空以 `::after` ＋ `--donut-thickness`（預設 28px）／`--donut-hole`（預設 `--card`，放在別的底色上要一併覆寫），中央 `.donut__center`（`__label`／`__value`）疊在挖空層之上；圖例沿用同檔 `.source-list`／`.source-row`。**兩段以上、且要放中央數字的比例用 `.stacked-bar`／`.rank-bar`，不要硬塞進 donut**。<br>`.pie`（2026-07-31 新增）＝N 段實心圓餅：外層行內設 `--pie-stops` 為 conic-gradient 色標串、段界寫累計百分比（每段起點一律寫 `0`，讓瀏覽器接續上一段終點），尺寸 `--pie-size`（預設 168px）；圓餅不帶文字，數字全由旁邊圖例承擔，故 consumer 必須給它 `aria-hidden="true"`。`.pie-figure` 是「圓餅＋圖例」的並排殼（`.pie-figure__legend` 內放 `.source-list`），560px 以下疊成單欄、圓餅置中。`.pie-figure--stack`（2026-08-09 新增）＝圓餅在上、圖例在下的直排版；並排版把圓餅擠在左半邊、卡片一窄就得跟著縮，直排讓圓餅吃滿卡片寬度可以畫大一點，圖例整排在底下也比擠在右側的窄欄好讀，段數少（三段以內）而卡片不寬時用它。`.pie--hollow`（2026-08-09 新增）＝中空版，厚度與挖空色沿用 `--donut-thickness`／`--donut-hole` 兩個變數，兩支的呼叫端不必記兩套；加在 `.pie` 而不是擴充 `.donut`，是因為 `.donut` 的引擎只吃兩段、要它做三段以上得整個換掉。消費者：粉絲分析 › 性別占比（女／男／其他三段）。**與 donut 的分工**：donut 只吃兩段且中心要放一個結果數字；三段以上、沒有單一結果數字要放的切分用 pie。**與 stacked-bar 的分工**：長條回答「誰比誰長」，圓餅回答「這一整塊是怎麼被分掉的」。消費者：粉絲總覽 › ztor 粉絲組成（四個分級的人數佔比）。<br>消費者：專案詳情 › 我的收益 › 淨收益分配（發起人／支持者 70/30） `.chart-card__foot` 為 space-between（左說明、右操作群）；**只有說明文字、沒有操作群時該文字靠右**（`> span:only-child` 補 `margin-left:auto`，2026-07-31）——否則單一子元素在 space-between 下等同靠左，看起來像漏了右邊那組。 | [chart.css](./ds-components/chart.css) |
...
| Detail overview | 🟠 organism | ✓ App | 詳情頁總覽的三欄骨架（2026-08-31 使用者裁決）。三欄各自回答一個問題，這是分欄依據：**左（窄）＝數字**（KPI 直排，`.detail-overview__kpis`）、**中（寬）＝進行到哪**（上架追蹤／狀態卡／進度摘要，同一條時間線）、**右＝是什麼 ＋ 要你動手**（識別事實、方案、下一步、通知；使用者通則「有通知的就往右放一欄」）。軌道 `3fr / 5fr / 4fr`；1080px 併兩欄、899px 單欄；右欄可加 `--sticky`。**`.detail-overview__kpi-row` 的排法（2026-09-01）**：桌機兩格一排，**剛好三格時排成一行**（使用者：「他們可以放同一行嗎」——三格是同層級的量體數字，讓其中一個獨佔一整排會讀成它比較重要），四格以上維持兩個一排；單數時最後一格佔滿一排（`:last-child:nth-child(odd)`，原本只寫 `:only-child`、三格時沒生效）。三欄的門檻是 **1081px** 而不是 899px——1080 以下這一欄只有 260px 左右，三格各 76px 會把註腳斷成三行，所以窄的時候改成一格一排。**同輪撤除 1080px 以下的 `.detail-overview__kpis { flex-direction: row }`**：那條寫在這一欄只裝 KPI 磚的時候，現在同一欄還裝著走勢卡與方案統計表，攤成橫排會讓三個不同的東西各分到三分之一欄寬（實測 1000px 下 KPI 那排只剩 30px）。**不用 `.bento`**：bento 一列一列往下鋪、要再包巢狀 bento 才做得出三根柱子，這裡的三欄彼此不對列。消費頁：project-detail（唯一）。**2026-08-31 使用者裁決：其餘詳情頁**先不套用****——各頁總覽的結構差異太大：商品／組合／競標是 form-section 堆疊、右欄沒有可放的識別卡（要新建才有，屬新增內容）；IP 詳情總覽只有一張卡，切三欄會有兩欄空著；訂單／取貨／品牌活動／創作者／粉絲／系列根本沒有分頁式總覽。活動詳情結構上可套（總覽已是兩欄），同輪一併暫緩。日後要套時先回頭讀這條，別把它當成沒做完的待辦 | [detail-overview.css](./ds-components/detail-overview.css) |
...
| Payout picker & dialog | 🟠 organism | ✓ App | Earnings · Payouts bank picker card grid + request-payout modal (legacy dialog shell, predates Modal). `--embed` variant (2026-06-17) is a near-fullscreen, head/foot-less shell that hosts a whole page in an iframe — used by Create bundle's "New item" → `create-product.html?embed=1` popup。**寬度三階**：預設 620px ／ `--wide` 880px（裝五欄矩陣，2026-07-31）／ `--xwide` 1180px（裝「一整個設定畫面」，2026-08-01 新增，首個消費者＝粉絲管理的分級設定彈窗；它裡面是 7fr／3fr 兩欄，880px 會把右欄壓到 250px 左右、圖例的金額欄先被擠爛。1180px 沿用 `.embed-modal` 的上限，兩者都是「把一整頁裝進浮層」）。**`.payout-dialog__split`**（2026-08-09，首個消費者＝建立活動的門票編輯彈窗）把 `__body` 分成主欄（要填的欄位）與側欄，`align-items:start` 避免側欄被主欄拉高，≤640px 收單欄。**側欄寬度改吃 `--payout-split-side`（2026-08-10）**，預設仍是原本寫死的 200px（門票編輯彈窗行為不變）；建立活動的「票務設定」把它設成 360px 裝生成的門票清單——一列五個欄位，200px 塞不下。用變數而不是再開一個修飾類，因為兩者的差別只有一個數字。**`.payout-dialog__split-side`（2026-08-10 新增，選配）**：側欄 `position: sticky` 黏在頂端、自己 `overflow-y` 內捲。用在「左欄是設定、右欄是那些設定產生的結果」的彈窗——左欄捲動時右欄跟著捲走就等於看不到結果，分欄也就白分了；側欄自己的 `max-height: 100%` 讓結果多時它內捲，而不是把整個彈窗撐長。≤640px 收單欄時自動退回 `static`（上下堆疊本來就跟著捲，黏著只會擋住下面的內容） **2026-09-01 面改成玻璃**（使用者裁示「彈窗要改成玻璃面」）：`background` 由實色 `--popover` 改成同色 72%，底下那一頁透得出來一點，浮層才讀得出是一片玻璃而不是一張貼上去的紙。**這裡不掛 `backdrop-filter`**——遮罩那一層自己帶著模糊，因此是一個「背景根」，掛在裡面的模糊只取樣得到遮罩自己那一片平色，等於白寫；推遠後面那一頁本來就是遮罩的工作。2026-08-28 改成不透明時擔心的「看穿兩層、底下的字浮上來」在這個配方裡不成立：透過來的已經是糊掉又壓過色的一層（實測遮罩合成 alpha 0.68）。遮罩本身同日收成 `--overlay-scrim` 一支 token，見 Pillar 2 的浮層配方段。**2026-09-22 新增兩個 footer 槽與標題列副標（D311）**：`.payout-dialog__foot-note`（左槽，擋下儲存的理由，例如「N 個欄位待修正」，`--status-error`／`--fs-12`）＋ `.payout-dialog__foot-actions`（右槽，按鈕群 `margin-left:auto`）——foot 本來就是 space-between，只放兩顆按鈕時它們會被推到兩端；理由放左槽讓它與主鈕站在同一條視線上，不必回頭掃整張表單找紅字。`.payout-dialog__head-titles` ＋ `.payout-dialog__sub` 讓標題列多一行副標（首例＝單張門票彈窗：標題是票名、副標是場次日期・場地——多場次活動只看票名分不出是哪一場的票），沒有副標的彈窗不用包、現況不動。消費頁：`create-event.html`／`event-detail.html` 的門票彈窗。**2026-09-23（Q125）`__body` 的內距隨內容走**：`.payout-dialog__body:has(> .form-section--dense)` 把內距由 `--sp-20` 提到 `--sp-24`（＝卡片的 `--card-pad`）——區塊堆疊自己已經是一把節奏梯子（欄位 16／標題 12／段間 32），body 內距是那把梯子最外面的一階。用 `:has` 而不是再發明一個修飾類：條件本來就是「裡面裝的是什麼」，寫成 `:has` 就不會有第二個彈窗忘記掛。 | [payout-modal.css](./ds-components/payout-modal.css) |
| Restock order (table) | 🟡 molecule | ✓ App | E-Shop restock popup (spec §5.1.5.6, D104 order + D106 member tabs). 2026-07-22 layout (Figma 861-28842): method as `.segmented.radio-cards` → product-name label → `.restock-table` (3 number columns current/restock/after, `__group`/`__row`/`__id`/`__img`/`__name`/`__col`/`__ro`) → supplier/notes/ETA below → footer pinned to dialog bottom (body scrolls); under-method hint + yellow stickynote removed. Product variants = matrix rows (2-option grouped), bundle members = Tabs (one `.tab-panel` each); reuses payout shell + Segmented + Tabs + Data-list (history)  **Q112（2026-09-11）**：`--radius-xl` 外框內的列左右內距下限 `--sp-16`（首欄文字不壓在圓弧上；右側 icon 欄不動）。**選項組合列的縮圖（2026-09-11 D268，規格 5.1.5.6 §4 ①；2026-09-12 Q117 改併入名稱格）**：呼叫端以 `item.thumb` 傳入唯讀 `.variant-thumb--ro`（實線＝該組合自己的圖、虛線＝沿用主圖），`partials/restock-modal.js` 的 `lineHTML` 把它放在 `__id` 裡取代 `__img` 方塊——不另開欄、欄軌不變，`__id` 的間距 `--sp-10` → `--sp-12` 與規格表統一；商品細節頁用 `ProductsStore.variantThumb` 產；沒帶的列（組合包成員、單一規格、e-shop 的補貨）維持 40px `__img` chip。**2026-09-21（Q118「不能編輯就用文字」）**：當前數量／補後數量兩格 `.restock-table__ro.input` 去掉 1px 邊線（`box-shadow: none`）與指標（`cursor: default`），只剩置中的弱色數字；仍是 `<input>` 讓 `recalc()` 照舊寫 `.value`，內距與置中和相鄰可填的補貨格相同，數字自然同一條線 | [restock-modal.css](./ds-components/restock-modal.css) |
| Store settings page | 🟠 organism | ✓ SiteSpecific | **E-Shop 商店層級設定頁**（`store-settings.html`）。**2026-08-11（D184）由彈窗改成獨立頁面**——內容量撐爆彈窗（五組設定量體差 18 倍、其中兩組各自長出「清單↔單筆編輯」兩層），改頁面後**不掛全域 app shell**（沒有側欄），但頁首用清單頁那一套 `.page-crumb` ＋ `.page-intro`（麵包屑＋大標題＋副標＋右側動作）——這是一個「頁」不是一個「流程」，標題該有頁的份量；麵包屑同時是回電子商店的路（2026-08-11 兩次修正後的定案）。**版面**：主欄上方橫向分頁四組（收款／銷售預設〔幣別＋出貨〕／尺寸指南設定／優惠碼，裝在 `.list-toolbar` 殼裡）＋右側 `.preview-col` 常駐直軌放店面門面（就地換封面、換頭像、改店名網址簡介，兼作編輯處與當下長相）；動作（粉絲視角預覽／放棄／儲存）移到 `.page-intro__actions`，`.ss-actionbar` 底列退場。分區標題與副標一併移除——分頁上已經寫了名字，下面再寫一次是重述。**返回電子商店會還原清單狀態**（分頁／狀態篩選／搜尋，`sessionStorage`），這是彈窗時期「不離開管理脈絡」的替代承諾。**優惠碼的新增與編輯改走彈窗**（2026-08-11 使用者指示）：清單頁只留一顆「新增優惠碼」，自用與代理不再是兩顆鈕——它們差的只有「誰在賣」，型別在彈窗裡用 `.segmented` 切，切到代理才長出推廣者、分成與代理銷售紀錄三塊；編輯從列尾 kebab 開同一個彈窗。彈窗殼用站上正典的 `.payout-modal`／`.payout-dialog`。此前是同頁兩層檢視（清單↔就地編輯），理由是那時整頁住在 iframe 裡、再疊一層要付 postMessage 與 z-index 成本；改成獨立頁面之後這個限制消失。此前為 popup（D035/D067，由 E-Shop F3 embed-modal 開啟、無頁首）：店面門面常駐（Base44/FB 式身分帶 `.ss-identity-card`/`.ss-band__*` + 逐欄就地編輯 `.ss-edit`）+ 商品陳列/付款/出貨 tab 群組 + 底部提交列 `.ss-actionbar` + See-as-fan 畫面分割預覽；含 `.ss-url`/`.amount-field`/`.ss-status`/`.ss-order`/`.ss-fan`。**2026-09-11 右欄改成商店預覽、店面編輯進分頁**（使用者：「這個怎麼長得和電子商店的列表頁的不一樣，而且重複了；或是這些都刪掉，預設在這個位置就預覽」）：「粉絲視角預覽」鈕與滑出側欄（`preview-panel--inset`）退場；右欄改成與 e-shop F5 同一張常駐的「商店預覽」卡（`preview-col--tall --dim`＋`form-section--outlined`＋`__head--rule`，欄 320／鏡像 zoom 0.8，變數寫在 `store-settings.css` 的 `#ss-split`）；原本右欄的店面編輯卡（`.ss-identity-card`）搬進分頁列成第一個分頁「店面」（`#sec-storefront`，預設開啟）。編輯在左、預覽在右，同一頁不再有兩份「粉絲看到的樣子」。**同日再改：店面分頁改成欄位表單**（使用者：「全部改成欄位設定，像創建那樣；可編輯欄位參考粉絲端編輯個人檔案；假資料對應上預覽畫面」）：三張 `form-section--outlined` 區段卡——個人檔案（`profile-avatar-row` 頭像列＋顯示名稱／身分／自我介紹）、商店網址（`.ss-url`）、社群連結（`social-links`，平台一列一格＋虛線新增鈕展開平台清單）；資料在 `js/store-profile-store.js`（依人格存 localStorage，示範值＝預覽現在畫的那組），右欄預覽與電子商店 F5 讀同一份、編輯中即時重畫（`storeprofile:changed`）。封面圖退場（粉絲端頁面沒有封面）；`.ss-identity-card`／`.ss-edit` 就地編輯整組退場。分頁列右側的編輯／儲存／取消同時服務店面與銷售預設，取消＝整份快照放回。**同日：頁級「放棄／儲存」退場、銷售預設改兩段式編輯**（使用者：「這個應該不需要了；唯一會用到編輯儲存的只有這些，把編輯按鈕放在選單 bar 右側，如同詳情頁」）：店面就地編輯、尺寸指南與優惠碼各走彈窗，需要「進入編輯→儲存」的只剩銷售預設；比照商品詳情設定分頁——分頁列右側 `.list-toolbar__actions` 一顆「編輯」（`[data-ss-editmode]`），只在銷售預設分頁出現，按下切 `data-mode="edit"`、同一顆變「儲存變更」、旁邊出現取消（取消還原快照）；換分頁視同取消。**2026-09-22（D299，修訂 D279）優惠碼 Phase 1 的適用範圍改可多選**：`radio-list`「全部商店／指定商品（可多選）」各帶一行 `__sub`（全部商店含活動票種），選指定商品展開與 Phase 4 共用的 combobox（`.ss-scope-combo` 移到兩組閘門之外、自帶 `--sp-10` 上距）；清單「範圍」欄綁多件時只寫件數、hover 走 `.stock-tip--anchored`（`.ss-code-scope` 只把它收成 inline-block）；碼欄 hint 改「不分大小寫」、儲存時重複檢查忽略大小寫 | [store-settings.css](./ds-components/store-settings.css) |
| Variant builder | 🟠 organism | ✓ App | 建立商品多選項（spec 5.1.5.2 §4.1④，僅實體）。**2026-08-10 逐規格圖片（使用者指示，上游無此欄位，ASSUMPTIONS PG-025）**：每列規格名格前放一個 34px 的直式小縮圖 `.variant-thumb`（08-10 原本是表格層 `--img` 修飾類開一個獨立欄，2026-09-12 Q117 改併進規格名格），實線＝這個規格自己的圖、虛線＝沿用商品主圖（沿用上傳格 Q59 的實線/虛線語彙，整列掃過去就知道哪幾個規格被單獨換過圖）；點開「規格圖片」彈窗換圖（`.payout-dialog--narrow` ＋ `.upload-tile-aside`，與建立活動的票種圖片同一個 pattern：這一列自己的圖，沒給就沿用上一層）。狀態只有一個來源（`varData[key].img`，有值＝自己的圖），商品主圖換掉時沿用中的列會跟著換。**2026-09-11 D268 正式納入規格（每個選項組合各一張商品圖，選填、未設定沿用主圖）**：新增 `.variant-thumb--ro` 唯讀縮圖（`<span>`、`cursor:default`、hover 不變色，實線／虛線語彙照舊）——**只有建立商品流程與商品細節頁編輯彈窗的組合表能改圖**（`<button>` 開「規格圖片」彈窗），其他列出組合的表一律唯讀：商品細節頁庫存管理表（`--stock`，單一規格不掛 `--img`）、編輯彈窗的檢視態、組合包鎖定清單 `lock-sets` 的逐組合列、鎖定庫存彈窗（`stock-allocation` 的 `.salloc__name--thumb`）、補貨矩陣（`restock-modal` 的 `item.thumb`）。縮圖 HTML 由 `ProductsStore.variantThumb(product, variant)` 統一產出（`variants[].img` 檔名或完整路徑，沒給沿用 `product.img`）。上游 PG-039 已裁決、`documents/` 由另一路同步。**2026-09-12 使用者裁決（STYLE-DECISIONS Q117）：縮圖併進規格名那一格、不另開「圖片」欄**——使用者看庫存表「Image Variant」兩個表頭擠在一起、看起來沒對齊；表格層的 `--img` 修飾類與所有「34px 開頭」的疊加欄軌整組退場（含 lock-sets／stock-allocation／restock-modal 的對應修飾類），表頭從「規格」開始、`cp.var.col.img` 移除；`.variant-cell--variant` 改 flex 橫排（`.variant-thumb` 自帶 34px 寬、`flex:none` ＋ 文字區 `.variant-cell__text`，間距 `--sp-12`＝使用者說 8px 太擠、垂直置中，像一般電商後台的商品欄），`--fluid` 的截斷改落在 `.variant-cell__text`。所有列出組合的表同一做法：create-product、商品細節庫存表／編輯彈窗表／鎖定彈窗／補貨矩陣、組合包建立與詳情的成員展開列。元件構成：`.segmented` 切單一/多選項 + `.option-set`（設定好的選項收成一行 `__row`，只有編輯中的展開成 `.variant-option`）+ `.variant-table`（逐規格價格/庫存/SKU/成本，`.--limited` 多出上限欄、`.is-excluded` 排除組合）；可編輯格重用 `.input`。**2026-07-21 庫存欄改唯讀**：庫存格是讀數 `.variant-cell--stock`（`--font-ui`／`--fs-13`／`--foreground`），低於低庫存門檻再加 `.variant-cell--stock-low` 轉 `--destructive` ＋ `--fw-medium`；庫存只能靠留有紀錄的補貨增加，做成 `.input` 會讓人以為可以直接打字覆蓋，轉紅則讓「哪一個規格快沒了」在表上一眼可見。**2026-07-21 逐值 input 改版（使用者裁示）**：值由 chip 膠囊改成「一個值一個 `.input`」（`__value` > `.input` ＋ `__value-remove`），可就地改字——設定選項時最常做的是改一個字，chip 得先刪再重打；底部 `__add-value` 虛線鈕再長一列，Enter 亦可。編輯態欄位加 `.field` 標籤（選項名稱／選項值），底部 `__actions` 靠右放「刪除」（`.btn--ghost.btn--destructive`，破壞性只用透明底）＋「儲存」（`.btn--outline`）。**列的外觀＝Q24 的 L3 規則首次落地**：整組坐在 `.nest`（L2）裡，所以 `__row`／`.variant-option` 去掉 `--input-surface` 填色改 `transparent` ＋ 1px 實線 `--border`；兩顆新增鈕（`__add`／`__add-value`）用 1px **虛線** `--border`（沿用 upload-tile／payout-modal 的「這裡還沒有東西」語彙）。表格外框貼齊內容寬靠左（`width:fit-content`＋`max-width:100%`）、選項組合欄＝內容寬（`max-content`＋110px 下限）不用 fr 吃滿剩餘寬度（2026-07-16）。**2026-07-22 列尾動作分兩種消費**：建立流程用 `.variant-row__remove`（X，把組合排除在建立集合外）；商品明細規格鎖定後不可移除，改列尾 `.dropdown` kebab 選單「單獨下架／重新上架」，下架列加 `.variant-table__row--delisted`（只壓「除下拉外的直接子代」灰階、輸入由 JS 加 `disabled`，kebab 仍可用來重新上架）。列尾用浮動選單的外框要加 `.variant-table-wrap--menu`：基礎 `.variant-table-wrap` 的 `overflow-x:auto` 會裁掉浮出的選單，此修飾在桌面寬度放行 `overflow:visible`（表格本就撐得下），只在 <560px 恢復 `overflow-x:auto` 橫捲。**放行 overflow 後外框圓角不再 clip 內容**（2026-07-23）：表頭（`--muted` 底）與末列的方角會戳出圓角，故改由內容自己貼齊——`--menu` 下給 `.variant-table__head` 上圓角、`.variant-table__row:last-child` 下圓角（半徑 `calc(--radius-xl - 1px)` 貼齊外框內緣）。**2026-07-22 商品明細整張表改唯讀**：價格／SKU／成本／總量欄由 `.input` 換成 `.variant-cell--ro`（純文字、與 `--stock` 同族、`text-overflow:ellipsis`），改值走「⋯ → 編輯」popup；每格加 `data-vc` 標角色（name/price/total/sku/cost）供 popup 讀寫回填。**2026-07-23 兩階層（顏色×尺寸）＋資料驅動**：商品明細的規格表改由 `products-store` 的 `options`／`variants` 驅動——單階層逐值一列、兩階層依第一個選項（顏色）用 `.variant-table__group`（撐滿整列的分組標題、同補貨矩陣語彙）分組、尺寸列在組底下。編輯 popup 用 `.variant-table--preview`（5 欄唯讀組合預覽：規格／價格／庫存／SKU／成本，無總量、無列尾動作），改選項值（加/改名/移除）時笛卡兒積即時重算。**2026-09-04（D249）快捷選項與顏色色號**：`.option-set` 在「新增選項」上方多一列 `.option-set__presets` > `.option-set__preset`（實線膠囊，與虛線的「新增選項」分得開——這兩顆是**已經有內容**的捷徑），尺寸＝S·M·L、顏色＝Black·White；按一下把整組選項連值一起建好並直接展開逐選項組合表，已經建過同名選項的那顆就不出現。顏色選項的值多帶一個色號：值編輯列變成 `.variant-option__value--colour`（色球 `.opt-dot` ＋名稱 `.input` ＋色號 `.variant-option__hex`，110px 固定寬），色球吃 `--opt-dot` 實際渲染那個顏色、`--sm` 是 8px 版，同一顆球用在值編輯列、收合摘要列與右欄粉絲端預覽的 chip。色號讀不到（空的或格式錯）時球給 `transparent`——球還在、版面不跳，但看得出來還沒設。資料模型：值本身仍是字串（組合鍵、逐選項組合表、預覽都吃字串），色號另存在該 option 的 `hex` 對照表、以值的字面當鍵，`kind === 'colour'` 才有。同輪退場：切到多選項時自動塞一組 Size/S·M·L 的做法（名稱寫死英文、也不見得是這件商品要的維度，要改還得先刪；改由快捷承接）。**`--fluid`（2026-09-09）**：表格坐在比整頁窄的欄裡時用（首個消費者＝商品明細「銷售設定 → 當前庫存」那張唯讀表，主欄只有 ~450px）——基礎版六欄是寫死的 px、加總就超過 `min-width:560px`，窄欄裡整張表會戳出卡片外，而且 `--menu` 為了讓 kebab 選單浮出已放行 overflow，連橫捲都接不住。此修飾把最小寬歸零、**文字欄**換成可壓縮的 fr（規格組合 0.9／價格 1／SKU 1.5／單件成本 0.9，SKU 分到最多是因為它最長），並讓 `.variant-cell--variant` 也會截斷（基礎版是 `max-content` 欄、撐得開所以不必）。**數字欄與欄距照基礎版**：庫存／鎖定／未鎖定各 78px（＝基礎版庫存欄的寬度）、欄距 `--sp-8`——2026-09-09 使用者裁示「要照 DS」，先前為了塞進 450px 主欄壓成 44–56px／`--sp-6` 的做法退場（商品明細的庫存卡同日改整頁寬，沒有理由再壓）。命名與做法沿用 `admin-ip-bank-table` 的 `--fluid`（2026-09-02）。**2026-09-09 欄位擴充**：商品明細那張表加「鎖定／未鎖定」兩欄（逐選項組合的鎖定量與剩餘量，見 `variant-lock-grid`），`--fluid` 的兩套欄軌隨之各多兩軌（48px／56px），未鎖定欄掛 `.variant-cell--free` 走 `--muted-foreground`——它是推導值不是使用者填的數字。同輪列尾 kebab 多一項「鎖定庫存」（`data-variant-lock`）。**2026-09-09 下午改版：鎖定改就地編輯**——`.variant-cell--lockedit` ＋ `.variant-lock`（`__ch` 販售管道名，多於一個販售管道才標）：「鎖定」格換成每個販售管道一格輸入、「未鎖定」即時預覽、鎖過頭那列掛 `.variant-table__row--error`（未鎖定轉紅）；入口是卡頭「編輯鎖定」（全部列）或列尾 ⋯（該列），取消／儲存在 `form-section` 的卡底動作列。同日早上建的單列鎖定彈窗與編輯彈窗裡的大表格 `variant-lock-grid`（`.lockgrid`）因此退場，檔案留墓碑。**2026-09-10（D258）三項改動**：**(1) 表面改吃層級**——表頭由實色 `--muted` 改薄膜 `--ztu-film`、列由實色 `--card` 改 `transparent`（分隔本來就靠 hairline）；這張表坐在已經是玻璃的卡上，再蓋兩層不透明面會把層次洗掉（Q24／Q71／nest.css）。**(2) 常駐輸入取代編輯模式**——新增 `.variant-cell--input`（格子讓輸入框吃滿欄寬並可縮），前一日的編輯態語彙 `.variant-cell--lockedit`／`.variant-lock`／`.variant-table__row--editing` 同輪退場；**2026-09-11 再修訂**：使用者裁示商品明細那張表**整張唯讀**（價格／SKU／成本改在編輯彈窗逐組合填、鎖定改在鎖定彈窗），`.variant-cell--input` 的消費者改為編輯彈窗裡的組合表（`--preview`）；`--fluid` 欄軌隨之回到數字欄 78px；`.variant-table__row--error`（鎖過頭整列的未鎖定轉紅）保留。同時補一條 `.variant-table .amount-field .input { padding-left: var(--sp-40) }`——Q25 那條 `.variant-table .input { padding: 0 9px }` 會蓋掉 amount-field 讓給輸入框的左內距，34px 寬的 `$` 方塊直接壓在數字上（實測只看得到最後一位）。**(3) `--fluid` 欄軌重分配**——數字欄 78→64、文字欄改吃 fr，輸入框版才塞得進整頁寬主體。**2026-09-21 檢視態＝純文字、編輯態＝輸入框（使用者反饋，STYLE-DECISIONS Q118）**：兩段式編輯面板（`[data-mode="view"]`／`"edit"`）裡的表，檢視態的價格／上限／SKU／成本一律畫成 `.variant-cell--ro`（`data-vc` 標角色；金額走該頁唯讀金額寫法、空值「—」），不再是 `readonly` 的 `.input`；按「編輯」才由 JS 換成 `.input`（`product-detail.html` 的 `ecText()`／`ecInput()`）。編輯態下仍不可編輯的格——`.variant-cell--stock`、`.variant-cell--ro`——由 `.variant-table:has(.input)` 補 `padding-left: var(--vt-input-pad-x)`（表格層變數＝輸入框原本的 9px 左右內距，`.variant-table .input` 同時改吃它），文字起點與相鄰輸入框裡的文字同一條線；表頭不動。元件層另有檢視態安全網：`[data-mode="view"] .variant-table .input`／`.amount-field`／`.amount-field__unit` 渲染成同一種純文字（無框、無底、無內距、`--font-ui`／`--fs-13`／`--foreground`，`$` 前綴變一般文字、`gap: --sp-4`），`[data-mode="view"]` 下 `--stock`／`--ro` 的左內距歸零。表單欄位的檢視態不在此條範圍（見 Q119） | [variant-builder.css](./ds-components/variant-builder.css) **2026-09-11 `.variant-table--stock`**：商品庫存分頁的庫存管理表——只留庫存欄位（組合＋SKU 小字／在庫／鎖給單售／鎖給組合／未鎖定／補貨中／狀態／⋯；補貨與鎖定都在 ⋯ 的列操作選單裡，照 §4.58：最上面一列「上架中」開關＋帶 icon 的動作項），價格、SKU、成本欄搬去商品設定；`min-width: 0`，**不與 `--fluid` 併用**（--fluid 的欄軌寫在後面會蓋掉）。狀態格用 badge（低庫存 error／到門檻 warning／售罄 neutral），在庫格對應 `.variant-cell--stock-low`／`--stock-warn`；補貨中格 `.variant-cell--incoming`（有量時 `.is-on` 轉前景色）。 **Q112（2026-09-11）**：`--radius-xl` 外框內的列左右內距下限 `--sp-16`（首欄文字不壓在圓弧上；右側 icon 欄不動）。 |
| Tag input | 🟡 molecule | ✓ App | 建立商品商品標籤（spec 5.1.5.2 §4.4 F10）：`.tag-input__field` 內已選/自建標籤（`.chip--removable`）＋無框輸入 `.tag-input__entry`＋建議 `.chip-group`；組合自 chip，可重用於專案/粉絲標籤。已選標籤 chip 用**品牌橘 tint**——Q19（2026-07-17）當初為此寫的 `.tag-input .chip--active` 專屬覆寫已於 **2026-07-27 Q8-A 刪除**：全站 `.chip--active` 本身就是橘了，覆寫變成純重複。Q19 的意圖（標籤＝已套用的分類，用橘標）完全保留，只是改由基底承接；順帶修掉原覆寫 `color: var(--primary)` 在亮色模式僅 1.92:1 的對比 bug（基底走 `--selected-ink`）。**2026-09-11 表面照 DS**（使用者指出「這個要照 DS，且圓角也和裡面的標籤不一致」）：欄位是輸入控件，底與邊線跟 `.input` 走——`--ztu-film` 薄膜底、box-shadow 畫的 1px `--border` 髮線、`--radius`、同一種 focus 環（舊值：實色 `--card` 底＋真 border＋outline 焦點環）；**欄內** chip 改吃欄位的 `--radius`（Q111：坐在欄位裡的是已輸入的值，不是 Q1 那種可點選的 pill），欄位外的建議 `.chip-group` 維持 pill。Combobox 重用同一個欄位，一併生效 | [tag-input.css](./ds-components/tag-input.css) |
| Combobox | 🟡 molecule | ✓ App | 多選 search-to-add typeahead：重用 `.tag-input__field`（已選 `.chip--removable` ＋無框 entry）當欄位，欄位右側 `.combobox__chevron`（開啟時翻轉）指示可展開，focus／輸入時彈出 `.combobox__menu` 下拉建議（浮層配方同 `.dropdown__menu`：`--popover` 實色面＋1px `--ztu-glass-rim`＋`--shadow-float`，Q78；2026-09-16 起，舊值 `--card`／`--border`；`--sp-8` 間距浮於欄位下；`.combobox__group` 分組＋`.combobox__opt`＝icon＋名稱＋meta，`.combobox__empty` 空狀態），點選加入為 chip、已選項自建議移除。已選 chip 用中性灰（`--accent` 底，非 `.chip--active` 反白）。與 Tag input 差異＝建議改「focus 觸發浮層下拉」而非常駐 chip-group。首用：建立取貨場次 modal「加入取貨項目」（`partials/pickup-session-modal.js`，spec 5.1.5.12 §4 F2）。**新消費情境（2026-08-09）**：建立活動 `create-event.html` 的「票種」欄（選票種帶入門票）與「加入商品」欄（搜尋帶入 E-Shop 商品），選單與選項皆用 `.combobox`／`.combobox__menu`／`.combobox__opt`，取代原本頁內自造的一套下拉候選樣式；已加入的商品改用 Chip 的 `.chip-group` ＋ `.chip--removable`（見 Chip）。**2026-09-22（D299）**：商店設定優惠碼彈窗的「指定商品」多選改成 Phase 1／Phase 4 共用同一顆（坐在兩組版本閘門之外，顯隱由頁面 JS 算），候選三組——單售、組合包、**活動票種**（`ztorEvents.list()` 售票中／已排程／進行中的活動逐票種一筆，`__opt-name`「活動名 M/D · 票種名」、`__opt-meta` 價格） | [combobox.css](./ds-components/combobox.css) |
| Film picker | 🟡 molecule | ✓ App | 電影關聯可搜尋多選（spec 5.1.5.2 §4.5 F12／5.1.5.1 §2.14／D140）：純 JS 元件 `partials/film-picker.js`，**建於 tag-input＋chip 之上、無自帶 CSS**——搜尋輸入格（`.tag-input__entry` type=search）過濾候選、建議 `.chip-group` 點選加入、已選 `.chip--active.chip--removable` 移除；候選來自 `window.ztorFilms`（films-store）。API `window.ZTOR_PARTIALS.createFilmPicker(host,{selected,onChange,readonly})`→`{getSelected(), setReadonly(bool), isReadonly()}`。consumer：create-product.html（§4.5 獨立區）、product-detail.html（關聯分頁）、**create-bundle.html／bundle-detail.html（關聯分頁；2026-09-11 使用者指示組合也要有電影關聯）**。候選為 mock（UIA-053）、可搜尋（BR-NEW-1）。**2026-09-18（D288）唯讀模式**：`readonly:true` 或事後 `setReadonly(true)` → host 標 `data-readonly`、搜尋格與建議列收起、已選 chip 拿掉 ×（`.chip--active` 不帶 `--removable`），元件層自己拒絕加入／移除；封存的細節頁（product-detail／bundle-detail）用它把「關聯」分頁收成唯讀，封存 ↔ 解除封存可來回切。demo 卡見 `design-system.html` §4.125 | [partials/film-picker.js](./partials/film-picker.js) |
| Work fields | 🟠 organism | ✓ App | 影片作品欄位區塊（spec 5.1.2.2.1 F1–F15）：純 JS 元件 `partials/work-fields.js`，**建於 form-section／form-grid／field-system／card／upload-tile／chip／tag-input／segmented／switch／control-row／amount-field／entry-list 之上、無自帶 CSS**。把「一個欄位區塊長什麼樣」收成單一來源，兩個宿主頁只給空容器＋順序：`publish-work.html`（共創／預購成立後的事後上架，四步）與 `create-project.html`（直接發佈 × 影片家族的建立流程，五步，2026-08-10 整併）。區塊 kind：`file`（含上傳與處理狀態徽章）／`audio`／`subs`／`cover`／`stills`／`bts`／`trailer`／`copy`（標題與說明，D220 起單卡；語言維度 D223–D322 期間由發布前預覽確認層 `partials/publish-preview.js` 承接，2026-09-23 起兩個消費頁（`create-project.html` D322、`publish-work.html` D323）已改交給各自最後一步全頁畫面 `js/publish-stage.js`（`stageFields()`），舊模組不再是這兩頁語言維度的入口；`opts.lsField` 隨已退場的 Lang switch 一併撤除，publish-work.html 已不再傳）／`spec`（片長＋上映日期）／`genres`（21 值多選）／`tags`／`age`／`pricing`（是否付費＋幣種＋逐畫質四數值＋平台費揭露）／`credits`（九角色名單＋備註）。API `window.ZtorWorkFields.render(host, kind)`／`renderAll(host, kinds)`／`collect()`（收成送審內容）／`checks({stepOf, stepLabel})`（規格 §8 就緒檢查項）；chip 與開關這類沒有原生事件的互動統一補一顆 document 級 `workfields:change`。宿主頁在容器上寫 `data-wf="file,audio,subs"` 宣告要哪幾塊。為什麼是 JS 元件不是 HTML partial：每個區塊都會自己工作（字幕可增可刪、劇照填滿長下一格、語言組不准重複、幣種切換改所有金額符號）。**2026-08-10（D182）`genres` 與 `age` 兩塊的值域搬到 `js/work-taxonomy.js`**，本檔只負責畫外框與就緒檢查、不再自帶清單（見下一列）。**2026-08-11 新增第三個使用場合與 `fill()`**：作品上架後的編輯（spec 5.1.2.2 §2.2.11）走 `publish-work.html?mode=edit`，用 `fill(work, {coverSrc})` 把既有內容寫回欄位——每個區塊自己知道怎麼還原（函式掛在該區塊的 `<section>` 上），`fill()` 只負責找出畫面上有哪些區塊並逐一呼叫。demo 卡見 `design-system.html` §4.123 | [partials/work-fields.js](./partials/work-fields.js) |
| ~~Lang switch~~ | — | **已退場 2026-08-24（D223）** | 頁級「內容檢視語言」切換（D220，2026-08-24 引入，掛在表單頂部、邊填邊切語系）。D223 同日裁示把翻譯檢視挪到「按下發布之後、真正送出之前」，改由新元件 Publish preview 接手（見下一列），這顆頁頂分頁不復存在。`partials/lang-switch.js` 已刪除（無自帶 CSS，直接刪除而非留 tombstone 檔）；四個消費頁（create-product／create-event／create-project／publish-work）已卸載 `<script>` 與對應的 `data-ls-field` 標記；`design-system.html` 的展示用消費點（曾是第 5 個 `<script>` 掛載）已一併移除。`sessionStorage` key `ztor-r22-content-lang` 只有該檔自己讀寫，刪檔即退場，未留殘值需遷移。i18n key `ls.*` 已改名遷移進 `pp.*`（`ls.default-badge`→`pp.default-badge` 沿用同一份文案；`ls.tabs.label`／`ls.banner` 改寫成新元件的 `pp.tabs-label`／`pp.banner`） | （已刪除） |
| Publish preview | 🟠 organism | ✓ App | 發布前預覽確認層（D223，2026-08-24，取代上一列 Lang switch）：純 JS 元件 `partials/publish-preview.js` ＋ `ds-components/publish-preview.css`，**殼固定用 `.payout-modal`／`.payout-dialog--xwide`（STYLE-DECISIONS Q27）**，語言分頁用 `.tabs`（Q8：主要檢視切換）、預覽/列表切換用 `.segmented`（Q8：控件層 toggle），列表用 `.ztor-table`（沿用 admin-platform-fees.html 表格塞 input 的先例）。呼叫式 API（非宣告式掃描）：`window.ztorPublishPreview.open({ fields, previewClone, previewSlots, onConfirm })`。`fields`＝可翻譯欄位陣列 `{ key, el, kind, labelKey, labelFallback, group }`，`el` 是承載預設語言值的真實表單元素或 `{ value }` 快照物件（給沒有穩定 DOM 節點的動態清單用，見下）；選填的 `group` 是列表檢視的分組小標字串——連續欄位共用同一個 `group` 值時，`renderListPanel()` 只在該組第一列前插一次跨欄小標（**2026-08-24 新增，唯一消費者 create-product.html 的詳細規格列與商品選項**：兩者是列數不定的動態清單，開預覽層當下即時掃表單／JS 狀態收集、空的不收集，欄位鍵是位置索引非穩定 id，見 ASSUMPTIONS.md LANG-008）。`previewClone`（選填）＝要複製進預覽面板的既有預覽卡節點（create-event 的 `.event-preview-card`）；不給就用內建 generic 名稱＋描述卡（create-project、publish-work）。`previewRender`（選填，與 `previewClone` 互斥）＝宿主頁自訂渲染函式 `(container, api) => void`，本檔只提供 `api.getValue(key)`／`api.isDefault`／`api.bindEditable(el, key)` 三個介面、不猜測頁面該長什麼樣——**目前唯一消費者 create-product.html**：使用者 2026-08-24 追加裁示，商品預覽要「長得像買家前台商品頁」，`buildShopItemPreview()` 用本站既有 token／`kv-list`／`btn` 重刻一個結構參照 `ztor-eshop-fe` shop-item 頁的 `.cp-shopmock`（外部 repo 的呈現參考，非產品權威；尺寸／顏色／購物車／取貨列是裝飾性佔位，見 ASSUMPTIONS.md LANG-007），頁面層 CSS、不 promote 進 ds-components。`previewSlots`＝選擇器對照表，在複製節點的 `id` 被剝除**之前**先解析，插回文件後不會留下重複 id。翻譯草稿模型延續自退場的 Lang switch：每個欄位、每個語系各自一份記憶體草稿，**存在模組層級（非逐次 open 重置）**——「返回編輯」關掉再重開仍看得到剛才的修改；未手動編輯過的語系即時鏡射預設語言目前值（模擬重新生成翻譯，原型無真翻譯，見 ASSUMPTIONS.md）。預覽檢視裡非預設語系欄位是 `contenteditable`；列表檢視裡非預設語系每欄是 `input`/`textarea`、**預設語言欄唯讀**（它是母本，改母本要回表單）；兩種檢視讀寫同一份 `DRAFTS` store，切換不丟改動。只攔「新發布」路徑：四個宿主頁在編輯已上架內容的分支（Save changes／saveEdit）不呼叫，直接走原本的儲存邏輯（呈現假設，見 ASSUMPTIONS.md）。**2026-09-23 起全面退場**：create-product.html（D317）、create-event.html（D310）、create-bundle.html（D321）、event-localization.html（D310）、product-localization.html（D319）、create-project.html（D322）、publish-work.html（D323）已依序全部改走全頁 Publish stage（本檔上方 Publish stage 條目，design-system.html §4.216），本元件不再有任何產品頁消費者，僅剩 design-system.html／design-components.html 的 demo 卡在用。**退場候選**，待使用者確認後執行墓碑（見 UI-CHANGES 九十九）。**2026-09-22（D305／D306）擴充**：`open()` 新增 `currencies`／`baseCurrency`（給了才有幣別軸——語言 `.tabs` 旁一組五幣別 `.segmented`（Q8 控件層 toggle：切的是「同一份價格用哪個幣別看」，與語言分頁「看哪一份翻譯」不同層；五個三字碼並排剛好一列、不用下拉），兩軸獨立）、`prices`（`{ key, label|labelKey, groupKey, priceObj:{ base, amount, override }, locked }`，票種與組合包各一列）、`views`（`[{ key:'ticket'|'bundles', labelKey }]`，previewRender 的 api 帶 `view`／`views`／`setView`，切換控件由宿主渲染器畫——fan-event-page 畫在頁裡）、`mode:'publish'|'save'`（主鈕「確認發布」／「儲存」、標題「發布前確認」／「預覽與在地化」、次鈕「返回編輯」／「取消」）；`onConfirm(result)` 收 `{ translations, prices, mode }`（prices 只含有覆寫的幣別）。api 多 `currency`／`priceIn(priceObj, key)`／`money(priceObj, key)`／`fmt(n)`。列表檢視由一張表改兩張 `.pp-table-block`——翻譯表（既有）＋價格表 `.pp-price-table`（列＝票種／組合包分組 `.pp-group-row`、欄＝五幣別；基準幣別欄 `.pp-cell--base` 唯讀＋「基準」徽章、bookyay 鎖定列附 `.pp-price-lock`；其餘欄 `.pp-cell--price`＝數字輸入框＋ `.pp-price-meta`（「已覆寫」徽章＋「重設為換算值」ghost 鈕，只在覆寫時露出）、覆寫中的格 `.pp-cell--overridden` 輸入框外框改 `--selected-ink`）。價格草稿 `PRICES` 與翻譯草稿同樣存模組變數（「返回編輯」不丟）；基準價一改（amount 與上次不同）該列覆寫清空重算（§7.15，比照 §7.4 翻譯）。換算與格式化轉問 `js/events-store.js`（`fx`／`priceIn`／`fmtMoney`，固定示範匯率），本檔不自帶匯率。新增 consumer：`event-detail.html`（`mode:'save'`，儲存後 `ztorEvents.setOverrides` 落 localStorage、票種卡與在地化摘要重畫）；`create-event.html` 改走 `previewRender`＝Fan event page（§4.215）、fields 擴到名稱／亮點／描述／活動內含物／需攜帶物品／活動須知／條款與細則／行銷同意／領取與入場方式／票種名稱，prices＝票種＋組合包，bookyay 帶入 base HKD 且鎖。create-product／create-project／publish-work 這句現只存歷史意義——三頁已分別於 D317／D322／D323 改走全頁 Publish stage，價格軸／幣別軸的邏輯現由該元件自己的 `priceOverrides`／`currencies` 承接（見上方 Publish stage 條目）。**2026-09-22（前台鏡像）**：新選項 `shell:'fan'`——殼加 `.pp-dialog--fan`（1376px＝前台 .container 1280＋48×2，讓 §4.215 的前台版型在 1440 視窗下內容寬與前台一致）、預覽槽加 `.pp-preview-slot--fan`（去內距）；視圖切換由渲染器畫在 `.pp-fan-viewbar`。只有 create-event／event-detail 帶這個選項。demo 卡見 `design-system.html` §4.130 | [partials/publish-preview.js](./partials/publish-preview.js) |
| Archived gate | 🟢 atom | ✓ App | 封存態唯讀的事件層閘門（主規格 §7.14「封存與不可刪除」· D284／D288，2026-09-18）：純 JS 元件 `partials/archived-gate.js`，**無自帶 CSS、不畫任何東西**。`window.ZtorArchivedGate.install(rootEl, { block, allow })`——`rootEl` 帶 `data-archived` 時，凡是會改資料的控件（預設清單：`input`／`textarea`／`select`／`.switch`／`.segmented__btn`／`.radio-list__item`／`.radio-card`／`.zcheck`／`.chip__remove`／選片器的 `[data-fp-add]`／`[data-fp-remove]`／`.upload-tile`／`[data-linkf-reset]`，頁面可用 `block` 追加）在 capture 階段被攔下（click／keydown Enter・Space／beforeinput／input／change 一律 `stopImmediatePropagation`＋`preventDefault`），到不了頁面 handler；導覽與查看類永遠放行（`a`／`summary`／分頁 `.tabs__item`／`.section-nav__item`／`[data-rowdis]`／`.leave-dialog`，頁面可用 `allow` 追加，例如「解除封存」鈕——2026-09-22 · D298 起已封存頁唯一的動作，按下回到已下架）。為什麼要有：D288 規定封存唯讀涵蓋所有分頁與逐列操作，三個細節頁原本各自「藏掉會改資料的入口」，藏不到的（逐列「⋯」的上架開關與鎖定、關聯分頁的選片器）就漏了；閘門在事件層兜底，既有隱藏／停用保留當第一道。consumer：product-detail.html／bundle-detail.html／auction-detail.html。demo 卡見 `design-system.html` §4.202 | [partials/archived-gate.js](./partials/archived-gate.js) |
| Work taxonomy | 🟢 atom | ✓ App | 題材（分類）與年齡分級的值域單一來源（主規格 §7.1.2 題材軸 21 值、§7.11 年齡分級六級，D182）：純 JS 元件 `js/work-taxonomy.js`，**無自帶 CSS**，畫面用既有的 `chip`（多選）與 `input.css` 的 `.select`（單選）。為什麼要獨立一支：這兩個值域各有三個落點（題材＝作品上架 F11、建立項目影視組 F4；年齡＝上架 F13、建立項目影視組 F4、項目公開資訊 §2.2.8），規格明講三處是同一份資料，實作也只能有一份清單——2026-08-10 前 `project-detail.html` 自己寫了一組 `PG-13／G／PG／R／NC-17`，同一部片在三個畫面可以顯示三個不同的分級。API `window.ZtorWorkTaxonomy.GENRES`／`AGES`／`genreKey(g)`／`ageOptionsHtml(selected)`／`genreChipsHtml(attr)`／`hydrate(root)`。宿主頁只留空殼：`<select data-taxonomy-age>` 與 `<div class="chip-group" data-taxonomy-genres>`，載入時就地填好、chip 開關走 document 委派，頁面不必寫 JS。載入順序要排在 `partials/work-fields.js` 之前（後者吃它的 `AGES` 與 `genreChipsHtml`），也要排在 `js/zselect.js` 之前（選項要先在，`<select>` 才升級得對）。消費頁：`publish-work.html`、`create-project.html`（影視組類型相關欄位＋影片路徑作品欄位）、`project-detail.html`（公開資訊年齡分級）。沒有載 `js/i18n.js` 的頁面（例如 `design-system.html`）吃內建英文退路，不會顯示成字典鍵。值域一覽見 `design-system.html` §4.123 | [js/work-taxonomy.js](./js/work-taxonomy.js) |
| Manual entry modal | 🟠 organism | ✓ App | 補登站外收入的彈窗（spec 5.1.8.2 · Earnings F10）：純 JS 模組 `partials/manual-entry-modal.js`，**無自帶 CSS**——重用唯一的 canonical dialog 殼（`payout-modal.css` 的 `.payout-modal`／`.payout-dialog`）與共用表單 helper（Pattern 5.1「Modal shell」）。它給的是 markup 而不是行為：`window.ZTOR_PARTIALS.manualEntryModal` ＝完整彈窗的模板字串，宿主頁注入後自己接開關與送出；介面文案走 `data-i18n`、示意值寫在模板裡。為什麼要收成模組：同一個彈窗有三個收入頁要開，各寫一份就會分岔。消費頁：`earnings.html`、`earnings-overview.html`、`earnings-ztor.html`。demo 卡見 `design-system.html` §4.123 | [partials/manual-entry-modal.js](./partials/manual-entry-modal.js) |
| Source upload modal | 🟠 organism | ✓ App | 上傳外部平台匯出資料檔的彈窗（spec 5.1.7.8 F1／D165）：純 JS 模組 `partials/source-upload-modal.js`，**無自帶 CSS**，同樣重用 `.payout-modal`／`.payout-dialog` 殼。做成彈窗而不是就地展開：來源明細本身是一顆 popover（`.src-status__panel`），兩層浮層會互相蓋住，而上傳是一段有頭有尾的任務（Q27 裁決的「編輯／新增一筆資料」一律用 dialog 殼）。`openFor(platform)` 帶入平台決定標題與匯出路徑那一句（`sp`／`yt`／`ig`／`tt`；StreetVoice 不開放上傳故無分支）。文案刻意不寫死檔案格式與大小——那四件事仍是〔產品待確認〕（ASSUMPTIONS PG-026～028）。消費頁：`fan-analytics.html`。demo 卡見 `design-system.html` §4.123 | [partials/source-upload-modal.js](./partials/source-upload-modal.js) |
| Fee exception modal | 🟠 organism | ✓ App | 逐 Creator 覆寫平台費率的彈窗（spec 5.1.0.3 F3／D141）：純 JS 模組 `partials/fee-exception-modal.js`，**無自帶 CSS**，殼同上。一個殼兩種模式：add 顯示 Creator 搜尋（`[data-fx-creator-field]`）、edit 固定 Creator 並藏起搜尋；平台費樹由宿主頁 clone 自己的 `#fee-tree` 注入 `[data-fx-tree]`，支付手續費一律唯讀（全站統一、不逐 Creator）。消費頁：`admin-platform-fees.html`。demo 卡見 `design-system.html` §4.123 | [partials/fee-exception-modal.js](./partials/fee-exception-modal.js) |
| Login lang prompt | 🟠 organism | ✓ App | 登入後詢問「登入畫面剛選的語言」是否要成為帳號預設語言（D225，2026-08-25）。**沿革（前身已退場、本輪語意改寫，非原樣復活）**：2026-08-24 曾有 ~~Lang mismatch dialog~~（D221 引入，`partials/lang-mismatch-dialog.js`），處理的是當時短暫存在的「顯示語言」與「預設語言」雙概念是否一致；D222 同日把語言收回單一概念（`window.ztorLang`）後，雙概念不復存在，該檔即刪除、掛載點與旗標寫入一併撤除。**本檔問的是完全不同的一件事**：站上只有一個語言值，比對的是「登入畫面剛選的語言」vs.「帳號已存的預設語言」，兩者出處都在同一次登入流程裡，只是沿用同一種 `.payout-modal` 殼與開關寫法（形似神不似，語意模型整個重寫，見下）。純 JS 元件 `partials/login-lang-prompt.js`，**無自帶 CSS**，殼固定用 `.payout-modal`／`.payout-dialog--narrow`（STYLE-DECISIONS Q27）。**狀態模型**：`login.html` 選語言只呼叫 `window.ztorLang.preview()`（新 API，只套用本頁顯示、不寫 localStorage、不廣播 `ztor:lang-changed`，見 `js/i18n.js`），**不再**直接 `set()` 覆寫帳號預設值；選擇同步記 sessionStorage `ztor-r22-login-lang-pick`。三條登入成功路徑（email／phone／第三方 OAuth）落地前寫一次性 sessionStorage 旗標 `ztor-r22-login-lang-pending`（`login.html` 的 `markJustLoggedIn()`）。掛了本檔的 shell 頁載入時：沒有 pending 旗標就完全零開銷；有 pending 但沒有 pick（使用者在登入畫面沒動過選擇器）→ 直接清旗標、不跳；有 pick 且等於帳號預設語言 → 清旗標、不跳；有 pick 且不同 → 先 `ztorLang.preview(pick)` 把畫面套成登入時選的語言，再開確認框，讓使用者看到的就是「剛選的」那個語言。兩顆按鈕：「Use {picked}」＝`ztorLang.set(pick)`（成為帳號預設語言，介面已經在該語言、維持不動）／「Keep {default}」＝`ztorLang.preview(default)`（帳號值不變，介面切回帳號預設語言）。**Esc／點遮罩等同「Keep {default}」**（呈現假設，見 ASSUMPTIONS.md）——未主動選「改用」就當作沒有要換帳號預設值。任一選擇（含 Esc／遮罩）都清掉兩把 sessionStorage 旗標，同一次登入只問一次，換頁／重整不再跳。i18n key 全新獨立 `loginlangprompt.*`（`title`／`body`／`use`／`keep`，`{picked}`/`{default}` 插值語言可讀名稱），**不沿用**已清除的 `langmismatch.*`。掛載點：`index.html`／`creators.html`（兩條登入落點）；`window.ztorLoginLangPrompt.show(picked, default)` 留給 demo 卡手動觸發、不走旗標。demo 卡見 `design-system.html` §4.131 | [partials/login-lang-prompt.js](./partials/login-lang-prompt.js) |
| Content block injector | 🟠 organism | ✓ App | 儀表板與收入管理反覆出現的區塊，由同一支渲染器產生：`js/components.js`，**無自帶 CSS**——每支渲染器吐出的都是既有元件（`data-list`／`table`／`kpi`／`alert`／`card`／`bento`）。機制與 `sidebar.js` 相同：頁面放 `<div data-component="NAME" data-key="KEY">` 佔位，載入時由共用渲染器填好，再對新節點重跑 icons／i18n。為什麼要有：這個靜態原型沒有建置步驟，這是唯一能讓區塊真的「改一次、每頁同步」的做法——交易列格式兩頁共用、不會再分岔，各頁仍給自己的資料列。渲染器：`transaction-list`（`table:true` 與 data-list 兩種輸出）／`ops-summary`／`alerts`／`activity-list`／`events-projects`／`ext-data-status`／`insight-split`；資料集住同一支檔案的 `DATA`。API `window.ZtorComponents.mount(root)`／`RENDERERS`／`DATA`。載入順序：晚於 `icons.js`／`i18n.js`、早於 `reveal.js`。消費頁：`index.html`、`earnings.html`、`earnings-overview.html`、`earnings-ztor.html`、`audience-report.html`。demo 卡見 `design-system.html` §4.123 | [js/components.js](./js/components.js) |
...
| Leave dialog | 🟠 organism | ✓ App | 建立／編輯流程的返回離開確認彈窗（spec §5.2.4），由 `partials/wizard-chrome.js` 注入、6 建立頁共用。 **2026-09-11 新消費情境**：`e-shop.html` 的「顯示於商店」確認彈窗（D263）直接用這個殼（`.leave-dialog`＋`__scrim`／`__card`／`__title`／`__body`／`__actions`），標題依開／關兩態換句——它是站上唯一的確認彈窗語彙，不另造第二種。兩態同殼：有未存編輯→問儲存（儲存並離開 primary／不儲存就離開 outline）；未編輯→純離開（離開 primary）。頂列無儲存入口的頁面加 `.wizard[data-leave-simple]` 可強制永遠走純離開態（2026-07-31，admin-ip-bank-entry）。`.leave-dialog`[data-open]＞`__scrim`＋`__card`＞`__close`/`__title`/`__body`/（選配）`__list`＞`__list-item`/`__actions`（堆疊滿寬 btn）；Esc/scrim/× 取消。2026-06-29 自 shared.css `.wizard-leave*` promote。**2026-09-18 D284**：新增 `__list`（一列一項、細分隔線、icon＋名稱）放在內文與操作之間，給擋下類彈窗列名稱；封存確認與草稿刪除確認亦用同一個殼——刪除的主鈕換 `.btn--destructive`，封存可逆故維持 `.btn--primary`。**2026-09-18 D288**：`__list` 的兩個 consumer 情境改為——(1) 單售**下架**時仍在上架中的組合包裡：列出組合包、主鈕「一同下架這些組合包」（product-detail.html；D284 時掛在封存、現已搬到下架，封存改為單純確認）；(2) 組合包**重新上架**（D289）：有已下架成員時列出「會一起重新上架的成員」、主鈕「一併重新上架」（確認＝成員連帶重新上架）；有已封存成員時擋下、列出成員、只有一顆「知道了」（取消鈕收起＝純告知）。**2026-09-22（D298）**：擋下內文改「先解除封存這些成員，再上架組合包」——已封存的出口只有「解除封存」（回到已下架、不確認、其他不動），這組重新上架彈窗只從已下架組合包的上架開關進來（bundle-detail.html；e-shop 的已封存列改走解除封存、不再開這組彈窗）。下架確認本身（三種販售管道）也走這個殼、主鈕 primary（可逆） | [leave-dialog.css](./ds-components/leave-dialog.css) |
...
| App shell | 🟠 organism | ✓ Project | Global page frame: `.app` + `.main` + `.page`（`.page` max-width 1280px）. Sidebar mode makes `.main` one continuous `--surface-page` sheet on `--surface-shell`, with a 16px top gap and 28px top-left corner。**窄版變體 `.page--narrow`（1056px，2026-07-20 Q21）**：只給 Detail rail（§4.52）的兩欄詳情頁——右欄固定 300px，容器留在 1280 會讓左欄行長過長；其餘頁面維持 1280。Q21 原裁全站收窄，同日改為僅變體套用。Consumer：product-detail.html、order-detail.html、bundle-detail.html、auction-detail.html **側欄選中項的光暈 2026-09-01 不再被裁掉**（使用者回報「光暈被切掉了」）：`.app-sidebar > nav` 為了在極矮視窗下讓步而帶著 `overflow-y: auto`，而 `overflow` 只要有一軸不是 visible，另一軸也會跟著變成裁切軸——這一層的內容框於是就是裁切線，選中項那圈往外暈開的橘色 box-shadow 剛好被裁在項目的邊上、變成一個硬邊方框。做法是左右各吃側欄自己的內距（14px）再用等量負外距抵銷：內容位置一格沒動，光暈從此暈在側欄自己的寬度裡。 | [shared.css](./shared.css) **側欄收合態下點有子項目的母項目（2026-09-01）**：先展開側欄、再打開那一組，強制展開不照 toggle 語意——收合態的子選單被 CSS 藏著，只翻 `data-state` 看起來像沒反應，而本來就 open 的那一組會在同一下被收掉。底部 actions 的帳號／幣別群組不適用（它們有自己的浮出選單）。 |
| Page intro | 🟡 molecule | ✓ Project | Product page H1 + sub + optional actions; eyebrow retired **`__lead` ＋ `__media`（2026-08-13 使用者指示「需要有活動圖片在左邊」）**：標題左邊放主視覺時，把圖與文字收進 `.page-intro__lead` 當左欄（`flex: 1 1 320px`），動作列維持右欄。basis 給固定值而不是 auto——`.page-intro` 是 `flex-wrap:wrap`，換行的判斷在壓縮之前，basis:auto 會拿標題的自然寬去比，長活動名稱還沒開始壓縮就先把動作列擠到下一行（實測 1194px 視窗）。`__media` 是直式 2:3、96px 寬（窄畫面 64px），沒有圖時由消費頁整塊收起、不留空框。消費頁：event-detail、series-detail。**2026-08-31 使用者指示「靠上方」**：`__lead` 的對齊由 `flex-end` 改 `flex-start`——圖比文字塊高，底線切齊等於在徽章列上方留一段空白，頁首讀起來像掉下去；頂端對齊後徽章與圖的上緣同一條線。同輪新增 `__sub--stack`（`> span { display:block }`）：副標裡「什麼時候」與「在哪裡」是兩種事實，串成一條時長場地名會決定換行點，讀者每次看到的分組都不一樣；消費頁把兩行各包一個 `<span>`。消費頁：event-detail（第一行開演＋開放入場、第二行地點）。每行行首帶一顆 `.ztor-icon`（calendar／map-pin）標明那一行是什麼——圖示是那個分類最短的標籤，比再寫一次「開演」「地點」省；排法與 `.data-list__meta .ztor-icon` 同一套（行內、15px、`vertical-align:-2px`），不另發明第二種行首圖示。**ztorUI 換裝（2026-08-28 Wave 2b §10）同檔內的麵包屑 `.page-crumb`**：由 `--brand-ink`（Wave 1 之後是亮橘）改 `--foreground-muted`，新增 `.page-crumb > span:last-child { color: var(--foreground) }` 表達「當前層」；返回圓鈕靜止態維持既有裁決（裸箭頭、不畫圓），hover 的不透明 `--accent` 圓改玻璃圓鈕（`--ztu-glass-bg` ＋ blur ＋ `inset 0 0 0 1px var(--border)`，用 inset 陰影當邊避免 hover 時抖動）。**`__highlight`（2026-09-22 D300 / spec 5.1.6.3 §2.1）**：標題下方、副標之上的一句短文字（活動亮點，例「台北唯一一場」）；15px 與副標同級、`--brand-ink`（橘色的字、對比量過），讓它讀成標題的註腳而不是第二個標題；沒有亮點的活動由消費頁 `hidden` 收起。與 `.event-preview-card__highlight`／`.review-row__value--highlight` 同一視覺角色、同一配方。消費頁：event-detail | [page-intro.css](./ds-components/page-intro.css) |
...
| Field system | 🟡 molecule | ✓ Project | ONE form field = label / hint / control slot（控件重用 atom）；多欄位怎麼成組、堆疊＝Pillar 5 · Form assembly，非本元件。單獨與 Form section 內皆維持基礎密度 gap 6／欄距 16；產品建立頁同樣遵守此節奏。`.field__hint`／`.field__error` 字級 2026-08-25（Q73）由 `--fs-11` 提到 `--fs-12`（與 `.form-section__sub` 同輪，使用者裁示「用 12、偶數為主」——Q21 已認定兩者是同一種輔助說明角色，只改一邊會變成同角色兩字級）。`.field__hint` 顏色 2026-07-20（Q21）改回 `--muted-foreground`，推翻 2026-07-16 「提亮成 `--foreground-muted` 以便在卡填色上讀得清楚」的決定——說明文字要明確退到輔助層；全站用 `.field` 的頁面一併生效，並與同輪改成同色的 `.form-section__sub` 對齊。**2026-07-22 追加 `.field-readout`**：欄位改「只顯示、不可填」時用它取代 `.input`／`.amount-field`（ui 字體／`--fs-14`／`--foreground`、不畫框不留 input 內距，讀起來是文字不是空欄）；用於商品明細把價格／成本／總量上限改唯讀，改值一律走編輯 popup。**2026-08-06 追加 `.field__hint--ok`／`--warn`**（2026-09-11 promote）：說明行依輸入即時換色——`--ok` 成功綠（有省到）、`--warn` 狀態紅（數字不合法），字級與位置不變；首用建立組合／組合詳情的組合折扣與限時折扣提示句（接手原本 create-bundle 頁內的 `.cb-warn`／`.cb-save`）。**`.field__error`**：欄位級錯誤訊息，位置與字級同 `.field__hint`，顏色改 `--status-error`（可讀的狀態紅，非破壞性操作專用的 `--destructive`；與 bundle-editor 的 `.field__hint.fc-hint--over` 同一支色）；隱藏／顯示用 `[hidden]`，控件同步標 `aria-invalid="true"` 由既有的 `.input[aria-invalid="true"]`（shared.css）畫紅框。首個消費者＝`earnings-sony.html` 捐贈彈窗的金額欄（超過可提領金額／低於所選組織的最低捐款額）。**2026-08-18 追加 `.field-readout--row`**：值 ＋ 行尾一顆動作圖示。複製／開啟這類動作作用的對象就是左邊那個值，貼著它才讀得出關係；擺成底下自成一行的按鈕時，要先讀完按鈕的字才知道它在對誰作用。值過長在自己那一格裁掉（`min-width:0` ＋ ellipsis），不擠動作那一欄。搭 `.field-readout__value` 使用；首個消費情境＝活動詳情的線上房間連結（掃碼器卡同日已先做過同一件事）。**2026-09-18 追加 `.field__hint--fact`**：事實說明——字級、位置、顏色同 `.field__hint`，差別只在**檢視模式（`[data-mode="view"]`）照常顯示**：shared.css 把檢視模式的 `.field__hint` 整批收起（多數說明是填寫指引，唯讀時是誤導），但「原價＝VIP $4,200 × 1 ＋ Tee $600」這種算出來的事實在唯讀時仍是這一格的一部分；例外規則寫在 shared.css 的那一條（`:not(.field__hint--fact)`）。首用組合詳情的原價算式、含票券的平台費口徑、票券成員的數量池說明（D292／D293）。**2026-09-22 `.field__hint` 的寫作準則（STYLE-DECISIONS Q123，適用範圍＝表單分段）**：一段內最多三個文字層級，hint 屬最底那一層；只寫「做決定才需要的資訊」（上限、單位、後果、留空會怎樣），每欄最多一句；把標籤換句話說、重述畫面已表達的操作、講內部公式一律不寫。規則優先用畫面表達（狀態改用列的修飾類與徽章、「選填」交給 placeholder、「不用填」交給「沒有輸入框」），不用句子。首輪落地＝組合包三段（bundle-editor `layout:'split'`、create-bundle） | [field-system.css](./ds-components/field-system.css) · [input.css](./ds-components/input.css) **2026-09-21 追加兩段式面板的檢視態讀數（Q119 使用者裁決：檢視態的表單欄位一律純文字、按「編輯」才變輸入框，與表格的 Q118 同原則；取代 2026-07-27「回到編輯畫面格式、只是不能編輯」與 2026-09-11「檢視態＝disabled 外觀」）**：`js/view-mode.js` 在 `[data-editable]` 面板裡每個控件根（`.amount-field`〔含 `.zstep`〕、`.date-input`、單獨的 `.input`／`.textarea`、`[data-vm-group]` 併句組）後面寫一顆 `<span class="field-readout" data-vm-readout data-vm-kind="input|textarea|amount|zstep|date|group|tags">`，控件標 `data-vm-control`；`shared.css` 檢視態 `display:none` 控件、`display:block` 讀數，編輯態反過來。讀數文字：空值「—」、placeholder 永不當值、金額＝前綴單位＋千分位（頁面可 `ZtorViewMode.configure(panel, { format })` 掛自己的格式函式，product-detail 傳 `money()` 與定價表同寫法）、後綴單位是字就隔空白（`1 pcs`）符號貼著（`10%`）、`datetime-local` 的 T 換空白、textarea 讀數 `pre-wrap`；控件 inline style 的 `flex`／`width`／`max-width`／`grid-*` 原封抄給讀數，角色／姓名這種靠 flex 比例分欄的列檢視態仍兩欄對齊。重算時機：面板 `data-mode` 變 view（MutationObserver）、子樹重畫、`i18n:applied`、各頁 `applyReadonly` 明呼 `ZtorViewMode.sync(panel)`。**不碰**：`.variant-table`（Q118 由頁面 JS 畫 `.variant-cell--ro`）、`.tag-input`（chips 就是值，shared.css 去框去 ×，零 chip 才補「—」）、下拉（`.zselect__trigger`／`.select` 由 shared.css 去框去箭頭留字）、`[data-edit-exempt]` 區段、以及例外清單——開關 `.switch`、單選卡 `.segmented.radio-cards`、單選清單 `.radio-list`、選擇卡 `.selection-card`、checkbox 維持原元件但 `pointer-events:none`（狀態展示不是可填框）。同輪 `[data-mode="view"] .field__req` 收起（必填星號是填寫提示）。消費頁：product-detail 商品設定、bundle-detail 組合設定、event-detail 總覽／設定、project-detail 設定、store-settings 店面／銷售預設（`.ss-url` 前綴＋讀數同行，store-settings.css）。DS html 4.x Field system「view mode / edit mode」示範可實際切換。 |
| Form section | 🟠 organism | ✓ Project | No-card section skeleton (title + sub + top divider + spacing) for create / wizard flows; scopes field label / hint presentation under `.form-section`（承載 Field 的組合殼，2026-07-08 自 🟡 重標；表單配方見 Pillar 5 · Form assembly）。`.form-section--outlined` 為建立流程正式採用的變體：白天以 `--surface-page` 作 sheet 底、`--card` 填色；黑夜改以 `--surface-shell`（`#1C1D1E`）填色浮在最深的 content（`--surface-page #0C0D0D`）上（2026-07-17 midnight-v2 先改 `--muted`→`--card`——壓暗後 `--muted` 與頁底過近、區塊會消失；2026-08-01 使用者比對同頁 `.list-toolbar`／`.alert-inset` 皆用 `--surface-shell`，裁示三者統一用 `#1C1D1E`，再改 `--card`→`--surface-shell`）；**無外框線**（2026-07-16 Q14 使用者裁示去 border，靠填色對比分區）；**浮起感**（2026-07-17 Q18 修訂 Q14）＝疊 `--shadow-card`（E2 resting）＋`--shadow-edge-top`（頂緣高光），仍無 1px 邊框、改由填色＋陰影＋上緣光共同分區；圓角 `--radius-xl`（16px）、內距 `--sp-16`（對齊 Figma node 781-4166；原圓角 6／內距 32）。可見 outlined siblings 用 `--sp-24` 分隔，跨過 `[hidden]` 條件區塊不留空白。採用頁：create-product／-auction／-bundle／-event／-project／register-ip／admin-ip-bank-entry；[section-test.html](./section-test.html) 保留作視覺驗證。`.form-section--modal` 已於 2026-07-17 退場：原採用它的建立取貨場次 modal（`partials/pickup-session-modal.js`）當日改成頁籤式分區（重用 Tabs 的 `.tabs` + `.tab-panel`，不再疊直填色面板），此變體無其他消費者故移除。**2026-07-20（Q21）字級拉平（全站 form-section 消費頁一起生效）**：`.form-section__title` `--fs-18`→`--fs-14`，與 `.field__label` 同級——區塊標題不再靠放大字級建立層級，改由卡片邊界承擔（**此點已於 2026-08-11 由 Q65 部分推翻，見本條末**）；`.form-section__sub` `--fs-14`→`--fs-11` 且色階由 `--foreground-muted` 壓暗成 `--muted-foreground`，與 `.field__hint` 同級——區塊副標與欄位說明本來就是同一種「輔助說明」角色。同檔尾追加 `.form-footnote`：表單底部置中小字說明（如 Stripe 保障文案），`--fs-12` / `--muted-foreground`，margin-top 22px 非 token（2026-07-09 自 create-product/auction 頁內樣式 promote，create-campaign 的 `.fc-footnote` 樣式不同、維持獨立）。**2026-07-22 追加 `.form-section__head--actions`**：標題列帶右側動作的變體——把標題群包進 `.form-section__head-titles`、動作群放 `.form-section__head-actions`（按鈕／⋯選單），head 改 flex 兩端對齊；只在需要動作的區段啟用（預設 head 仍是堆疊 title＋sub）。用於商品明細庫存卡（補貨紀錄鈕＋新增/編輯/補貨選單）。**2026-08-09 追加 `.form-section__subhead` 與 `.form-section__grouplabel`**（自 `create-event.html` 頁內 `.ce-subhead` promote）：一個 `.form-section` 裡有好幾組東西時（例：場次區段的「場次日期與時間」／「場次細節」）用 `__subhead` 分組，比拆成多張卡合適——它們回答的是同一個問題的幾個部分；字型跟 `.form-section__title` 一樣走 `--font-display`，**字級停在 `--fs-14`**（2026-08-11 Q65 把區塊標題升到 `--fs-16` 之後，這一級自然落在標題與欄位標籤之間，層級由「比標題小一階」＋上緣 hairline＋留白三者共同承擔；Q21 時期兩者同為 14px、完全靠邊界分層，該做法已由 Q65 部分推翻），卡片頂端第一個小標不畫分隔線（`.form-section__head + .form-section__subhead` 歸零 margin/border——上面就是區段標題本人）。`.form-section__grouplabel` 是小標底下的欄位分組（例如「每一場的人數」統轄最少／最多兩欄），Q21 之後與欄位標籤同字級，靠上方留白與網格分群，不另造第四種字級。**2026-08-10 追加 `.form-section > .segmented.radio-cards:has(~ :not([hidden]))` 補 16px 下距**：區段內垂直節奏一直靠 `.field` 自帶的 `margin-bottom` 16px 撐開，選項卡群組（`.segmented.radio-cards`）不是 `.field`、身上沒有那段留白，直接放進區段會與下一個元素貼成 0px——「選了才出現的欄位」最明顯，揭示出來的欄位標籤會貼在卡片下緣（建立活動「販售方式」二選一、「取票方式」三選一皆是）。**條件寫成「後面還有看得見的東西」而不是 `:not(:last-child)`**：這種卡片群組下面接的往往正是「選了才出現」的揭示區，收合時那些元素帶著 `hidden` 仍留在 DOM 上，`:not(:last-child)` 只看結構、看不出它們是隱形的，預設收合狀態下區段底部會白白多出 16px；`:has(~ :not([hidden]))` 問的是「後面有沒有任何一個還顯示著的兄弟」，同時也處理了「第一個揭示區藏著、第二個才是展開那個」的情形（取票方式選到店自取時，順豐那一塊是隱藏的），用相鄰選擇器 `+` 會漏掉。實測七種狀態：販售方式 直接販售 0px／限時販售 16px；取票方式 電子門票 0px／順豐 16px／到店自取 16px；建立商品 單一規格 0px／多選項 16px。**2026-08-11（Q65）區塊標題字級 `--fs-14`→`--fs-16`，部分推翻 Q21 第 1 點（全站 21 個消費頁一起生效）**：Q21 的「靠卡片邊界承擔層級」在一張卡只有一組欄位時成立，但建立活動這種一步三張卡、卡內還有 `__subhead` 與 `.field__label` 的版面，三層字全是 14px，掃過去分不出誰統轄誰。改成區塊標題比小標與欄位標籤大一階；`.form-section__subhead` 與 `.field__label` 維持 `--fs-14`，Q21 其餘四點（`__sub` 11px／`field__hint` 色階／`kpi` 底色／頁寬變體）不動〔**`__sub` 11px 已於 2026-08-25 由 Q73 推翻：`--fs-11`→`--fs-12`**，使用者裁示「用 12、偶數為主」——同一個角色（標題底下的說明句）不再留兩個字級，與 `.control-row__sub`／`.field__hint` 對齊〕 **不巢狀（2026-08-13，Q66 全站落地）**：`.form-section--outlined .form-section--outlined` 內層自動翻成薄膜。兩層本來吃同一個表面色，套起來只剩陰影在分層；Q66 稽核當時「0 處」，之後長出 6 處（建立活動票務、系列詳情場次），所以規則從「不要這樣用」升級成「這樣用時內層自動翻」。外層不動（頁面層分組，方向正確）。**ztorUI 換裝（2026-08-28 Wave 2a）大幅改寫本節開頭描述的表面配方**：`.form-section--outlined` 本體改玻璃四件套（`--ztu-glass-bg` ＋ `backdrop-filter: blur(var(--ztu-blur-glass))` ＋ 1px `--border` ＋ `--shadow-card`/`--shadow-edge-top`），**取代上文「白天 `--surface-page`／黑夜 `--surface-shell` 填色、無外框線」的舊配方**——2026-07-16 Q14 去邊框的理由（實色卡邊是多餘的線）在玻璃化後不成立，玻璃需要那道 1px 把厚度畫出來，邊框因此回歸；同時**刪除** `[data-theme="dark"] .form-section--outlined { background: var(--surface-shell) }` 這條 2026-08-01 定案的深色實色覆寫（留著會用同權重排序把玻璃蓋回實色），r2.2 全站唯一一個「深色卡面不等於 `--card`」的例外至此不再需要存在。巢狀內層（本節上述的「內層自動翻成薄膜」）同輪改吃 `--ztu-film`（原 `--nest-surface`），並新增 `backdrop-filter: none`／`border: 0`／`border-radius: var(--radius-lg)`。連帶：`preview-column.css` 原本「右欄對齊左欄無邊框」的 `.preview-col .card, .preview-col .preview-card { border: 0; box-shadow: … }` 整條移除（留墓碑）——左欄長出 1px 邊後這條的效果反轉成破壞對齊。**2026-09-09 浮動選單抬層**：`.form-section:has(.dropdown[open])` 給 `position:relative; z-index:2`——`--outlined` 帶 `backdrop-filter`，每張卡都是自己的堆疊脈絡，卡內選單的 `z-index:30` 只在卡內有效，排在後面的兄弟卡照樣蓋過它（商品明細「當前庫存」的列尾 kebab 被下面三張設定卡蓋住即此）。用 `:has()` 只在真的開著時抬升，平常不動層級。。**`.form-section__actions`（2026-09-09 promote）**：卡片進入編輯模式時的卡底動作列——取消／儲存靠右、左側槽放說明或擋錯訊息（`.field__hint`／`.field__error` 以 `margin-right:auto` 靠左），上緣一條 `--border-soft` 與內容分開；配方同 `variant-option__actions`。首個消費者：商品明細「當前庫存」的鎖定就地編輯（單一規格與多選項共用同一條）。**`__head--rule`（2026-09-11）**：標題列下一條 1px `--border-soft` 線，用 `--card-pad` 負外距出血到卡內緣、線上 16 線下 24（使用者指著電子商店商店預覽欄：「加一條分隔線，並將間距做出來」）；卡內容不是表單欄位而是另一個世界的畫面時用它。首個消費者 `e-shop.html` 的預覽欄；`preview-column.css` 對它補回 24 的下距（該檔原本把欄內標題列下距收成 16）。**2026-09-22 `.form-section__sub`／`.bd-sec__sub` 的使用準則（STYLE-DECISIONS Q123，適用範圍＝表單分段）**：段標題已經是主詞，副標只能把它換句話說，所以表單分段的段副標一律不寫；真的非講不可的事貼到它實際作用的那一格去講。元件本身不撤（非表單情境如預覽欄、說明性區塊仍可用）。首輪落地＝組合包三段（建立活動第 6 步、create-bundle）與 DS 頁的票券表 demo。**2026-09-22 新增 `.form-section--dense`（D311）**：同一套段標題與段間 hairline，上下留白由 44 收成 `--sp-20`、首段不留上內距、末段不留下內距。彈窗裡一次要放四個區段時，44px 的段距會把第二段以後全部推出捲動區；只改節奏、不改層級語彙（標題仍是 `.form-section__title`，段與段之間仍是一條 hairline），所以它與頁面級的區段讀起來是同一種東西。消費頁：`create-event.html` 與 `event-detail.html` 的單張門票彈窗。**2026-09-23（STYLE-DECISIONS Q125，使用者反饋）`--dense` 自己持有一把完整的節奏梯子**：標籤→控制項 `--sp-6`（`.field` 自己的 gap）／欄位→欄位 `--sp-16`（變體的 `row-gap`）／區塊標題→第一個內容 `--sp-12`／區塊→區塊 `--sp-32`（上下各 `--sp-16`）／彈窗 body 內距 `--sp-24`（`payout-modal.css` 的 `.payout-dialog__body:has(> .form-section--dense)`，對齊卡片的 `--card-pad`）。取代原本「段上下各 `--sp-20` ＋ 成員自帶 `margin-bottom`」的湊法：舊寫法標題→內容 24 比欄位→欄位 16 還大（標題離自己的內容比離別人遠）、段內最後一個 `.field` 的 16 疊在段上下內距上讓段間實際變成 56、而且節奏綁在「成員是不是 `.field`」上（Q103 的病）。作用域刻意停在這個變體——`.field { margin-bottom }` 是全站共用，動它會牽動每一個表單頁；`--dense` 只有兩個票券彈窗在用。`display:grid` 會蓋掉 `[hidden]` 的預設值，所以補 `.form-section--dense[hidden] { display:none }`。**`.form-section__title--quiet`（2026-09-23，Q125）**：區塊標題要由「區塊層的控制項」（跟隨開關／bookyay 徽章）或「必須連著讀的一組」（算式列表）換來；兩者都沒有、只是幾個各自獨立的欄位時標題退到群組小字標那一階（字級降一階、字重 regular、`--muted-foreground`、0.04em 字距——同 `.bd-sub__title`／`.bpc__title`／`.bd-tbl__gname`／`.dropdown__cap`，不另造第四種標題樣式；小型大寫是 `shared.css` 給全站標題的共同設定，兩階都有，分辨靠字級／色階／字距），彈窗標題已經交代脈絡且兩者都沒有時整個標題列不畫。首批：門票彈窗的「基本」無標題、「入場」輕標題，「價格與數量」「購買條件」「販售」維持正常那一階 **2026-09-23（STYLE-DECISIONS Q125 修訂，使用者反饋「用分隔線好像不是很明確」「間距還是太小」）新增 `.form-section--card`**：彈窗裡的分段改由**卡片**承擔，分隔線在這個作用域退場。站上既有的分段答案本來就是卡片（`settings.css` 的 `.settings-section` ＝「把卡片用 gap 排開」），所以不另造第三種分段做法。配方＝巢狀層 L2：`--layer-2-surface` ＋ `inset 0 0 0 1px var(--layer-2-line)`、`--radius-lg`（圓角降一階：這一階卡坐在圓角 24 的彈窗殼裡又內縮 24，同心圓角的內圈要比外圈小，同 `.form-section--outlined` 的巢狀規則）、`border: 0`（同時收掉 `.form-section + .form-section` 的 hairline）、**不掛 `backdrop-filter`**（卡坐在玻璃彈窗上，巢狀層再模糊只取樣得到母層，同 `.form-section--outlined .form-section--outlined` 與 `.payout-bank-card` 的判斷）。**內距 `--sp-20`、不用頁面級卡的 `--card-pad`（24）**：與卡間 gap 同值（卡內留白＝卡間留白，一把尺量到底），且 620px 的彈窗再讓出 8px 寬度不划算。**卡與卡的 gap 歸容器**（Q103）：`payout-modal.css` 的 `.payout-dialog__body:has(> .form-section--card)` 改 `display:grid`＋`gap: var(--sp-20)`＋`align-content:start`（body 是撐滿高度的捲動區，grid 預設 stretch 會在內容短時把卡拉高）。段內節奏（6／16／12）與標題三階由 `--dense` 原樣提供，本修飾類只換「分界怎麼畫」與「四邊留白」；`--dense` 的首段／末段歸零內距因此改成 `:not(.form-section--card)`（卡的內距是卡自己的留白，不是段距的一半）。兩張卡的內容對內容＝20＋20＋20＝60px。消費頁：`create-event.html`／`event-detail.html` 的單張門票彈窗（兩處皆 `--dense --card` 併用）。 | [form-section.css](./ds-components/form-section.css) |
| Radio card | 🟡 molecule | ✓ Project | Side-by-side selectable cards (title/sub) built on Segmented; flat 1px `--border` card, no shadow, gap 12 (Q13 2026-07-16, Figma node 781-4386); selected = small centered orange dot (no ring, no card outline), unselected shows no marker; single-line cards (no sub) vertically center text + dot, title+sub cards stay top-aligned; optional icon-marker variant. **Hover (2026-08-09, per user)**: every radio card lifts its background to `color-mix(--foreground 6%, --accent)` on hover. That fill used to live only on `--gate`, so the plain cards (single/multiple options, unlimited/limited, pickup method) had no hover feedback at all — same component, the only difference being a lead icon, so the feedback shouldn't come in two grades. **The orange dot does not move down with it**: the gate has no persistent selection, so its hover dot is the only preview of what you're about to pick; on a plain radio card the selected one already carries a dot, and a second dot under the cursor would read as "both are selected". **`--3` variant (Q28, 2026-07-24)**: 3-column grid (overrides the default 2-up) for create-project's gate + in-flow「專案類型」picker. In the **in-flow form** (`--3` without `--gate`) the selected card keeps the Q13 orange dot **and** lifts its background to `color-mix(--foreground 6%, --accent)` (2026-07-25, per user, to make the in-flow selection clearer); the gate (`--gate`) has no persistent selection so it gets no such fill. **`--gate` variant (Q28, 2026-07-24)**: create-project gate only — each card stacks a lead icon (`.radio-card__lead`, constant `--muted-foreground` — no recolor when active) on top, then title + sub with a roomier gap (`--sp-6`); the gate is a click-to-proceed picker with **no persistent selection**, so cards show **no dot at rest** (the base active dot is overridden to none); the orange dot appears **top-right (`::after`) only on hover** — the brighter hover fill it used to declare for itself now lives on the base rule above (2026-08-09), so `--gate` only keeps the dot half; dot + brighter fill together remain the hover affordance. In-flow form uses plain `--3` (title-only, no icon). **`--list` variant (2026-08-09 promoted from create-event.html's page-local `#ce-bky-list` id override)**: single-column list (`grid-template-columns: 1fr`) instead of the default grid, for the bookyay import gate's search results (one row per venue, not a 2-up grid) — see §4.120 Source import. Reuses the `--gate` card face (same "pick and move on" semantics) but paints the selected row in the `--gate` **hover** look (brighter fill + orange dot) because this list still needs a visible "which one is picked" cue before the user presses Import, unlike the plain gate which has no persistent selection. `--list` and `--gate` stack on the same container without conflict. **`--stack` variant (Q71, 2026-08-25, demo-eshop-styles.html :462-464)**: single column (replaces an earlier framed vertical layout the user rejected — "直排有框那版太醜"); card face and selected marker unchanged from base (top-right orange dot, no fill change, Q74) — distinct from `--list` (tints the selected row's background), don't conflate the two. No consuming page yet, demo-only as of 2026-08-25. **⚠ 2026-08-10 起 `--list` 為退場候選（同日反覆兩次，最終狀態）**：bookyay 帶入閘門的結果清單改用 Owner lookup 下拉（見 §4.120 Source import），`--list` 因此失去唯一消費頁。同日一度被建立活動的「要怎麼建這些門票？」選擇彈窗接手，但使用者隨後指示那個彈窗改回並排兩欄（見下）——`--list` 因此再次沒有消費頁，只剩 design-system 展示。保留待使用者裁決，不自行退場。 **`align-items: stretch`（2026-08-09 修根因）**：三張並排卡高度曾對不齊（如「三選一取票方式」某張副標兩行、某張一行，卡就長不一樣高）——grid 的預設 `align-items` 本來就是 `stretch`，但 `segmented.css` 的 `.segmented { align-items: center }`（給軌道內文字置中用）沒被蓋掉：`.segmented.radio-cards`（0,2,0）這條規則本身沒設 `align-items`，瀏覽器照特異度回頭吃 `.segmented`（0,1,0）那條，等於把 grid 的 stretch 蓋成 center，同列的卡因此各自用自己內容的高度置中。修法是在 `.segmented.radio-cards` 顯式補回 `align-items: stretch`，卡片就會填滿所在列的最大高度。**ztorUI 換裝（2026-08-28 Wave 2b）**：`.radio-cards .segmented__btn--active`（含 `:hover`）選中面改涼白玻璃——`color-mix(--ztu-cool-white 11%, transparent)` ＋ `blur(20px) saturate(1.1)` ＋ 雙層白內光（`--ztu-rim-strong`），字色維持 `--foreground`；**沒有**左緣橘線（2026-08-28 使用者裁決「不要線」），選中指示仍交給本檔既有的右上圓點 `::after`。連帶刪除 `--3`／`--list` 兩個變體原本用 `color-mix(--foreground 6%, --accent)` 提亮已選卡的 `background` 覆寫——玻璃卡面上那是最刺眼的材質斷層，base 的新已選態本身已比未選中亮一階，delta 已由 base 承擔。膠囊圓角（沙盒實測 `border-radius: 9999px`）判定為換皮沙盒覆蓋層的特異度誤傷、非設計裁決，本體維持 `--radius-xl`（24px）未跟進，詳見 `docs/wave2-熔接紀錄.md`「不搬清單 2」 | [radio-card.css](./ds-components/radio-card.css) |
| Check card | 🟡 molecule | ✓ App | Multi-select pill card (Cosmos layout, Q71 定案 2026-08-25 — demo-eshop-styles.html :646-701). `.check-cards` 兩欄等寬 grid，每張 `.check-card` 藥丸左文字、右一個 28px 圓形 `.check-card__mark`（未選＋、已選勾）；`.check-cards--sm` 小尺寸變體改隨內容寬、換行排列（38px 藥丸、24px 標記）；可選 `.check-card__group` 分組小標與 `.check-card__count` 計數列，兩者皆跨兩欄。已選（`.check-card--on`）整格染 `color-mix(--primary 28%, transparent)`＋文字轉 `--selected-ink`——與姊妹元件 [Radio card](#radio-card)（單選，底不變＋右上橘點）刻意分開兩種已選畫法（Q74 定案：多選一次會亮好幾格，需要比橘點更強的訊號；單選永遠只亮一格，橘點就夠）。demo-eshop-styles.html only as of 2026-08-25，**待套用**、尚無消費頁 | [check-card.css](./ds-components/check-card.css) |
| Radio list | 🟡 molecule | ✓ Project | Lightweight vertical 1-of-N picker (2026-07-17): radio dot + title (optional one-line sub) per row. 指示器（2026-07-17 Q19 精修）：未選＝13px 細環（1.25px `--border`）、已選＝粗環消失只留 8px 實心橘點（`--primary`）；transparent rows, hover `--accent`（2026-07-21 由 `--muted` 改回 Q9 裁決值——暗色 `--muted` 比卡還深、hover 像凹下去）, no card frame/shadow. Data choice, not view switch (that's Segmented). Rows without `.radio-list__sub` vertically center dot + title. **`--menu`／`--menu-compact`／`--menu-row` 變體已於 2026-07-24 移除（Q28，tombstone 見 radio-list.css）**：曾用於 create-project 的「專案類型」picker，經使用者裁示改用建立商品同款 `.segmented.radio-cards`（見 Radio card，灰卡＋右上小橘點），list-menu 樣式全數退場。**變體 `--check`（2026-09-23 · 優惠碼適用範圍 D272 可混選）**：可複選的選項列——同一種列（標題＋副說明、同樣的 hover 與節奏），左側記號換成 checkbox atom（`.zcheck__control`，重用 checkbox.css、不另刻方塊）；列本體是 `<label>`（點整列＝點那顆 checkbox），選中態由 `:checked` 自己畫、不靠 `--active` class；停用列（例如已勾「全部」時的其餘層）整列轉灰、不吃 hover。**圓點與方框不可互換**：圓點＝只能選一個、方框＝可以多選，形狀就是「這裡能不能複選」的答案。consumer：store-settings 優惠碼彈窗的「適用範圍」（Phase 4 三級可混選；Phase 1 的二選一維持基底圓點）。**變體 `--collapsible`（2026-07-21 · Figma 856-22782）**：收合式，`.radio-list__trigger`（圓點＋文字＋`.radio-list__chevron`）＋`.radio-list__options[hidden]`，外框 1px `--border` ＋ `--radius-xl`，展開時觸發列填 `--input-surface`、chevron 轉 180°、`[data-open]` 標開合；展開的清單**不列目前已選項**（2026-09-11 使用者裁示「已選的不用出現」，反轉 2026-07-21 照 Figma 保留兩處各出現一次的裁示；CSS 只藏 `.radio-list__item--active`，JS 狀態真相不變）。行為由 [partials/radio-list.js](./partials/radio-list.js) 統一接線（開合、觸發列文字＋`data-i18n` 同步、外點與 Esc 關閉、派發 `radio-list:change`），頁面只留自己的欄位揭示邏輯。其他 consumer：create-product/-bundle/-auction（預覽卡下的上架列）2026-07-21 起用 `--collapsible`。**D241（2026-09-04）修正範圍**：那一列現在是 Listing controls（§4.205）三列中的**上架列**，而且只有**建立頁**用本元件——細節頁（product-detail／bundle-detail）改用 Switch（§4.6），因為東西已經存在、只剩開著還是關著，再擺一份三選一等於要創作者重講一次已經發生過的決定；另外兩列（顯示於商店、開賣）從來沒用過本元件。此前記載的「五頁一律 `--collapsible`」已不成立。**新消費情境（2026-08-09）**：`create-event.html` 票種卡購票規則區塊（購票條件／限購／折扣）的單選改用本元件（基準版，非 `--collapsible`）——原本這裡當日一度 promote 出 `.zradio`（drawn radio，見 Checkbox §4.96），同日使用者裁決「建活動頁的樣式一律以既有 design system 為主」，改吃既有 `.radio-list`，不留第二種單選視覺；元件定位擴大為「輕量單選列、資料選擇，**也含表單裡的規則選項**」 | [radio-list.css](./ds-components/radio-list.css) |
| Date input placeholder | 🔵 atom | ✓ Project | 原生 `<input type="date|datetime-local|time">` 的 placeholder 裝飾層（2026-07-21）：原生欄位不吃 `placeholder`、空值時自己畫「年/月/日 --:--」且用一般內文色，看起來像已填值。`[data-empty="true"]`＝日曆 icon（`--sp-12`）＋淡灰「選擇日期」（`--sp-40`、`--muted-foreground`），原生 `::-webkit-datetime-edit` 藏起來、input 內距推到 `--sp-40`；`[data-empty="false"]`＝icon 與 placeholder 都收掉、內距回基礎 `--sp-12`、日期用 `--foreground`（常駐 icon 會吃掉 28px，設定頁 120px 的窄時間欄會被切字）。date／datetime-local／time 三型共用同一句文案 `field.pick-date`（使用者 2026-07-21 裁示）。**2026-08-06 新增選配的自訂文案**：欄位掛 `data-ph-key`（字典 key）或 `data-ph`（直接給字）就換一組，不掛維持「選擇日期」，既有約 40 處不受影響；首個消費者＝建立活動售票規則的起訖兩格（「開始」「結束」）——兩格並排時各自說自己是哪一端，比重複寫同一句有用。原生日曆鈕攤平成整格透明覆蓋層，點整格用 `showPicker()` 開選單。由 [partials/date-input.js](./partials/date-input.js) 執行期自動包裝全站約 40 個欄位（含 MutationObserver 接住 modal 這類後注入的節點），頁面 markup 維持乾淨的 `.input`，不得手寫 `.date-input` 外層 **依 type 分型（2026-08-13 修，取代上句「三型共用同一句文案」）**：圖示與文案跟著 `input` 的 type 走——`date`＝日曆／選擇日期、`time`＝**時鐘／選擇時間**、`datetime-local`＝日曆／選擇日期時間（新增 `field.pick-time`／`field.pick-datetime` 兩個 key）。此前三型一律日曆＋「選擇日期」，於是只填時間的欄位上寫著「選擇日期」、旁邊還畫一個日曆（使用者圈出建立活動的開放入場那一格）。欄位自己掛 `data-ph-key`／`data-ph` 時仍然優先。 | [date-input.css](./ds-components/date-input.css) |
| Collapse group | 🟡 molecule | ✓ Project | 一整組欄位收在盒子裡，折疊時只留一行：chevron ＋ 標題 ＋ 盒內值的一行摘要（`10/02（五）· 19:00–21:30 · 18:00 開場`）。首個用途＝活動的場次（2026-08-13 使用者裁決，三個版型比過選 A 案「一行摘要」；B 案三欄事實列與 C 案徽章列同輪否決，demo 留在 `docs/session-collapse-demo.html`）。**可點的是整列**（`.collapse-head__toggle` 吃滿剩餘寬度）——要瞄準 16px 的 chevron 才點得開，等於沒有入口；列尾的次要動作（移除）放在切換鈕**外面**，按鈕不能包按鈕。**展開時摘要收起**（CSS 處理）：同一組數字不在畫面上出現兩次。摘要單行截斷不換行，換行會把標題列撐成兩層、失去「一行掃過去」的意義。`--todo` 修飾類給還沒填完的那一組（警示墨色「日期尚未設定」）。**行為（同輪一併裁定）**：只有一場不折（盒子也不畫）／多場預設全折／剛新增的那一場自動展開／可同時展開好幾場（不是手風琴，對照兩場時間是常見動作）。展開狀態存在消費頁的 Set 而不是 DOM——清單會整段重繪，寫在 DOM 上會被洗掉。消費頁：create-event（場次那一步，定點與巡迴兩種盒）、event-detail（場次分頁，切換鈕掛 `data-view-safe` 讓檢視模式也開得了） | [collapse-group.css](./ds-components/collapse-group.css) |
| Control row | 🟡 molecule | ✓ Project | Bordered standalone row: left label/sub + right control (switch / number / button)。**`.control-group`（2026-07-21 新增）**：開關列＋它揭示出來的表單包成同一個外框（`--radius-xl` 外框、群組內的 `.control-row` 交出自己的框與圓角、揭示區 `.control-group__body` 帶 1px 上分隔線＋`--sp-16` 內距）——原本揭示欄位是裸的散在框外，讀起來像另一件事，包起來才看得出「這塊歸這個開關管」（使用者裁示）。可巢狀（開啟折扣 ▸ 開啟限時折扣）。寬度限制要下在內層欄位、不能下在 `__body`，否則分隔線會跟著縮短。12 組分佈於 create-product／create-bundle／bundle-detail／product-detail／create-event；沒有揭示表單的開關列維持單獨 `.control-row`（create-auction、create-project）。**`--plain` 與 `__body--top`（2026-08-10 新增）**：`.control-group--plain` 是「整塊就是內容、沒有開關列與揭示區兩段」的外框盒（自帶 `--sp-16` 內距），首個消費情境是建立活動票務那一步——一個場次一個框，門票一多才看得出哪幾張屬於同一場。**只有一場時不畫框（2026-08-10 第二輪，使用者指示）**：`create-event.html` 頁內邏輯改成場次數 > 1 才掛 `control-group control-group--plain`，只有一場時 `.ce-tiergroup` 只剩定位用的外層 div、沒有外框也沒有組標題——框與標題的職責都是「把這幾張圈成同一場、跟別場分開」，全部門票都屬於同一場時沒有要分辨的對象。`.control-group__body--top` 收掉上緣分隔線，給「揭示區是群組裡唯一一塊」的情形用。**新消費頁（2026-08-09）**：create-event.html 的「販售方式」——`.control-row` 開關「發布後直接開賣」＋ `.control-group__body` 放「開賣／停售日期與時間」欄位，揭示方向與既有消費頁相反（**關掉**開關才展開時間欄，其餘消費頁一律**開啟**才展開）；元件本身只認 `__body` 的 `[hidden]`，揭示條件是哪個開關值一律由消費頁的行內邏輯決定，元件層不因此分岔 **開放入場的線框層（2026-08-13 使用者指示「UI 上加一個層級，用線框的 layer」）**：場次盒內的「開放入場」整組（時間／開演前兩格＋說明＋早到規則）再包一層 `.control-group--plain`——它與上面的日期、起訖時間不同層：那些回答「這一場什麼時候」，這一組回答「門什麼時候開、早到怎麼辦」。填色層只到 L2（見 §Pillar 6 / nest.css），第三層起一律用 1px 線框表達層級，故此處用線框、不再疊底色。兩個消費頁同步：`create-event.html`（場次那一步）與 `event-detail.html`（場次分頁）。⚠ `event-detail.html` 此前**漏連 `control-row.css`**，多場次盒的外框其實一直沒生效（class 掛著、樣式沒到），同輪補上。 **盒內抬頭的間距（2026-08-13 使用者指示）**：`--plain` 盒本身也掛 `.field` 時（＝它裝的是一組欄位、不是單一控制項），`gap` 由 `.field` 的 6px 提為 `--sp-16`，盒內第一行的組抬頭才不會黏在欄位上；`.rule-sub` 的 `margin-top` 同時歸零，間距統一由 gap 發、不兩層相加。 **`--plain` 的角色收斂（2026-08-13）**：卡內的 L2 分組面改用填色的 `.card--muted`（Q66），`--plain` 線框只留給**第三層**——目前的用例是場次盒內的「開放入場」。建立活動的場次盒與票務「一場一盒」同輪由線框改填色，兩步同一種東西不再長成兩種樣子。 **`.control-row__link`（2026-08-21 · D215）**：副標句子裡的行內連結（12px、底線、hover 轉主文字色），不是這一列的動作——動作在右邊。首個用例是建立商品的尺寸指南「預設衣服 · 褲子 · 帽子尺寸指南。**點擊查看**」，點下去開商店那份指南的編輯器。 **`.control-row__actions`（2026-08-20 · D211）**：一列右側不只一個動作時橫排（gap 8px）——在此之前每一條 `.control-row` 的右側都只放一個控件。第一個用例是建立商品的尺寸指南「編輯／移除」。 **建立商品的尺寸指南用線框層（2026-08-20 使用者指示「改成線框的 layer」）**：那一組（沿用商店／專屬指南兩種狀態列）包一層 `.control-group--plain`，與旁邊裸的詳細規格欄位分開。它坐在 `.form-section--outlined` 裡、名義上是第二層，照 Q66 該用填色的 `.card--muted`；使用者當次裁示要線框，理由是這一區只有一列、填色會比它的份量重。線框層的既有用例（建立活動的「開放入場」）是第三層，本例是使用者裁示的例外，不改 Q66 的通則。 | [control-row.css](./ds-components/control-row.css) |
...
| Edge shadow（工具）| ⚪ utility | ✓ Project | `.edge-shadow`：把 `--shadow-header` 變成「只露下緣、內縮、兩端漸淡」的邊緣陰影（`::before`＋clip）。電子商店庫存條使用（wizard header 2026-09-23 改膠囊後退出）；其他元素加 class 即覆用 | [shared.css](./shared.css) |
...
| KV list | 🟢 atom | ✓ Project | 唯讀鍵值列（鍵左值右、逐列 hairline）：詳情頁 meta 卡、狀態摘要。值可編輯請改用 Field system；逐列 Edit 請用 Review row。`.kv--lead` 供有 `[hidden]` 條件列時手動指定群組首列；`.kv[hidden]` 已顯式歸零 display（元件為 flex，會蓋過 UA 對 hidden 的預設）。**2026-08-25（Q72）列線由 `--border` 改 `--border-soft`**：同樣是「同一組內、列與列之間」的分隔，站上其他列式版面都用 `--border-soft`，本元件原本用 `--border`（#373839），與其他列擺在同一張卡裡就是兩種亮度的橫線交錯（使用者回報「分隔線看起來有點混亂」）。兩級規則＝**列線 `--border-soft`、組線 `--border`**。本輪只改顏色、**內距維持 `--sp-8` 不動**（demo 頁另有 12px 的節奏覆寫，屬該頁版面選擇，未提升為元件預設）。消費頁：product-detail／auction-detail／order-detail／pickup-detail／manage-ip／admin-video-review／bundle-detail／create-event／create-product。詳見 §4.50 | [kv-list.css](./ds-components/kv-list.css) |
| Calc list | 🟡 molecule | ✓ App | 算式列表（2026-09-22 D311 自 `bundle-editor.css` 的 `.bd-calc-list` promote）：一行一項、列首一個運算子（＋ − ＝）、末列是結果；`--form` 變體讓列裡放輸入控件（右欄收 140px、改置中對齊），🪦 `.calc-list__extra`／`.calc-list__extra-row`（掛在某一列底下的附屬控制）2026-09-23 隨例外平台費退場（D315），沒有其他消費者。與 KV list 的差別：那支是唯讀事實列、沒有運算關係。消費頁：create-event（單張門票彈窗的價格與數量）、event-detail（票種彈窗）、create-bundle 與 `js/bundle-editor.js`（原價合計）。詳見 §4.217 **2026-09-23（Q125 修訂）坐進卡片式區塊時去框**：`.form-section--card .calc-list { padding:0; border:0; border-radius:0 }`——「價格與數量」整段變成一張卡之後，算式列表再自帶一圈外框就是卡中卡（同一條邊界畫兩次）。**列與列之間的分隔線與列內距保留**：那是一個算式單元自己的內部結構（哪幾項加起來等於哪一項），不是分區。寫成情境規則而不是修飾類——條件是「它坐在哪裡」，放進卡就該去框、沒有第二種選擇，不該讓每個消費點各自記得掛 class。卡片之外的消費者（`create-bundle.html`、`bundle-detail.html`、`js/bundle-editor.js` 的定價段）外觀完全不受影響。 | [calc-list.css](./ds-components/calc-list.css) |
...
| Detail rail | 🟠 organism | ✓ Project | 詳情頁兩欄殼：可編輯主欄（Tabs＋分頁）＋ sticky 唯讀 meta 右欄（1fr / 300px，≤1100px 收單欄並取消 sticky）。元件段只講殼；「何時用、右欄放什麼」見 Pillar 5 §5.1 *Detail + persistent rail*。詳見 §4.52。**`.detail-cards`（2026-09-09）**：一個分頁的主體需要整頁寬時（首個消費者＝商品明細「銷售設定」，逐規格表有八欄），原本擺在右欄的設定卡改放主體下方——**一張一行、各自吃滿整頁寬**（2026-09-09 使用者裁示；並排會多一條垂直切分線，讓三張互不相關的設定看起來像同一組），卡距與上方主體卡之間一律 `--sp-24`（2026-09-11 修正：原本 gap `--sp-16`、與主體卡之間 0，整頁單欄後讀起來是 24／0／16 三種距離；容器內的卡另外壓掉 form-section 那條兄弟間距，否則會疊成 48）。單欄 grid 而不是裸堆疊，是為了讓卡距走同一個 gap、不靠 margin 疊加 | [detail-rail.css](./ds-components/detail-rail.css) |
...
| Stock split | 🟡 molecule | ✝ 退場 | 庫存分配長條（2026-09-10 · D258 建，**2026-09-11 退場**：商品庫存分頁改版時它講的分配已由四塊 KPI 磚與逐組合表承擔，長條是第二遍；使用者裁示去蕪存菁）。`stock-split.css` 留墓碑，原始內容見 git 歷史。 |
| Restock log | 🟡 molecule | ✝ 退場 | 補貨紀錄表（2026-07-22 建，**2026-09-11 退場**：改成「一張單一筆、點開看明細」的 Stock history——供應商／備註是單的屬性，固定欄塞不下）。`restock-log.css` 留墓碑。 |
| Stock history | 🟡 molecule | ✓ Project | 庫存歷史紀錄（2026-09-11，自 `lab-stock-history.html` H1 promote）：一張補貨單或一次鎖定一筆，主列＝日期／類型徽章／逐組合變動摘要＋供應商 · 方式／狀態，點開看逐組合數量、供應商、預計到貨（補貨中附到貨確認）、備註。清單型 row-disclosure 上疊一層，本檔只擁有日期欄、徽章槽、變動數字、鍵／值格。 |
| Lock sets | 🟠 organism | ✓ Project | 組合包鎖定套數區（2026-09-11）：「鎖定幾套 N」一個欄位 ＋ 外框清單（組合中的商品｜商品庫存｜未鎖定｜已鎖定），單一選項商品自動 N × 用量、多選項可展開逐規格選填分配（Σ ≤ N × 用量）。貼邊髮線分列、展開整組一起 hover；由 lab-bundle-lock.html L3 定案 promote。**2026-09-11 D268**：逐規格列（`.lockset__child`）的名稱格放唯讀縮圖 `.variant-thumb--ro`（沿用 variant-builder，寬度由它自己鎖 34px；實線＝該組合自己的圖、虛線＝沿用商品主圖），縮排照舊坐在母列名稱底下；縮圖與組合名間距 `--sp-12`（2026-09-12 Q117 全站統一，本來就在名稱格內、沒有獨立欄）。demo 見 §4.210 | [lock-sets.css](./ds-components/lock-sets.css) |
| Profile avatar row | 🟡 molecule | ✓ Project | 頭像＋名字＋「變更相片」橫條（2026-09-11，照粉絲端編輯個人檔案抽屜）：薄膜底、圓形頭像 56px、名字跟顯示名稱欄位同步；首用商店設定「店面」分頁。demo 見 §4.211 | [profile-avatar-row.css](./ds-components/profile-avatar-row.css) |
| Social links | 🟠 organism | ✓ Project | 社群連結清單（2026-09-11）：平台一列一格（平台名＋垃圾桶／網址欄）、虛線「新增」鈕展開未加平台清單；資料 `js/store-profile-store.js`、畫與接線 `partials/social-links.js`；首用商店設定「店面」分頁。demo 見 §4.212 | [social-links.css](./ds-components/social-links.css) |
| Bundle preview card | 🟡 molecule | ✓ Project | 組合包編輯器右欄的粉絲端預覽卡（2026-09-21，由 `docs/bundle-create-demo-2026-09-21.html` promote）：以 preview-card 為底，加內含清單（票券 → 商品 → 額外權益）＋可點的票種／場次／規格 chip（選不到的 `.chip:disabled` 劃掉）、劃線原價＋售價＋「省 N%」、剩 N 組／售罄、沒選齊就停用的「加入購物車」示意鈕；媒體格 2:1（Q120 待裁決）。粉絲端組合包畫面上游未定案（ASSUMPTIONS BDL-001／002）——呈現推導、非前台規格。markup 由 `js/bundle-editor.js` `fanHTML()` 產生。demo 見 §4.213 | [bundle-preview-card.css](./ds-components/bundle-preview-card.css) |
| Fan event page | 🟠 organism | ✓ Project | 粉絲視角活動頁＝**前台 shop-item 頁的 1:1 鏡像**（2026-09-22 D305 promote；同日使用者裁決「預覽 UI 必須和前台一模一樣，只是資料和機制套用我們的流程」）：發布前預覽確認與活動詳情「預覽與在地化」的預覽檢視，票券頁／票務商品頁兩視圖。markup 與 class 名照前台兩頁 `<main>` 的 DOM（證據 `docs/fe-mirror-2026-09-22/`），CSS 是前台實際命中的規則加 `.fep-shop` 前綴（`ds-components/fan-shop.css`，含前台 :root token；裸 rgb／px 為已註記例外，不得在 CS 其他頁引用）。資料全部來自我們的流程；購買類按鈕不動作；元件支援相關活動三張卡標「非本活動」，但 2026-09-23 起兩個消費頁（`create-event.html`、`event-localization.html`）都改傳 `related:false` 不畫（B 反饋「一樣移除」，與 Fan product page 同輪裁決）。demo 見 §4.215（就地渲染，仍示範 `related` 有值的樣子） | [fan-shop.css](./ds-components/fan-shop.css) |
| Fan product page | 🟠 organism | ✓ Project | 粉絲視角商品頁＝**前台 shop-item 頁商品分支的 1:1 鏡像**（2026-09-23，D310 決定四；商品側的 Fan event page，同一套前台鏡像做法）：建立商品「開始販售」全頁「預覽與發布」的預覽檢視，單一視圖（商品沒有票券／組合包兩視圖的差異）。markup 與 class 名照前台 `shop-detail-render.js` 商品分支，CSS 是 `ds-components/fan-shop.css` 第 4 段「商品頁元件」（延續第 1–3 段的前台 `.fep-shop` token、裸值同為已註記例外）。資料全部來自建立商品表單；購買類按鈕不動作；無店家資料時不畫店家列；元件支援相關商品固定四張標「非本商品」的佔位卡，但 2026-09-23 起消費頁 `create-product.html` 改傳 `related:false` 不畫（B 反饋「你可能也喜歡都移除」）。demo 見 §4.218（就地渲染，仍示範 `related:true` 的樣子） | [fan-shop.css](./ds-components/fan-shop.css) |
...
| Nest | 🔵 atom | ✓ Project | 嵌在卡片底緣的滿版子層（左右下三邊切齊母卡外緣），用於「切換模式後長出一整組設定」。**Q71（2026-08-25）**：拿掉向上陰影，改用上緣 1px `border-top: var(--nest-line)` 分層——亮色兩層同為白靠這道線分層，深色 `--nest-line` 透明、改靠 `--nest-surface` 4% 薄膜分層。層級系統只有兩層填色，L3+ 改邊框。與 `.card--muted`（卡片內單純換底色的靜態子區塊）分工不同、不可混用。詳見 §4.53 | [nest.css](./ds-components/nest.css) |
...
| Brand card | 🟡 molecule | ✓ App | 品牌合作目錄卡：唯讀鎖住的 Ztor↔品牌條款（`.brand-deal`）＋創作者可編輯的活動區分開陳列。**ztorUI 換裝（2026-08-28 Wave 3）卡面改玻璃**，詳見 §4.94 | [brand-card.css](./ds-components/brand-card.css) |
...
| Checkbox | 🟢 atom | ✓ App | 全站自繪 checkbox（含 indeterminate），取代原生方塊；port 自既有統一設計系統。**`.zcheck--lg`（Q71 2026-08-25 新增）**：24px 大尺寸方框，掛在包住 `.zcheck__control` 的外層容器上——用於控件獨立佔一格、旁邊沒有緊貼標籤撐場面的位置（如清單列右側單獨的核取欄），16px 在那種位置份量不夠；圓角同步升級 `--radius-sm`→`--radius`，勾維持元件預設尺寸不變。**2026-09-02 修「裸 control」兩個病**（檔頭一直寫著支援「表格儲存格裡不帶 label 的裸 `.zcheck__control`」，但那條路徑其實是壞的，因為既有消費點都在 `.zcheck` 或 flex 容器裡）：① `.zcheck__control` 補 `display:inline-block`——`<span>` 預設 inline 不吃 width／height，方塊實測收成 0×0；② `.zcheck__box` 補 `pointer-events:none`——畫出來的方塊蓋在真 input 之上、又排在 DOM 後面，點擊被它接走，勾不起來（有 `<label>` 時 label 會轉發，所以此前沒發作）。詳見 §4.96 | [checkbox.css](./ds-components/checkbox.css) |
| Detail sheet | 🟠 organism | ✓ App | 清單頁點列開細節頁的全螢幕 iframe 覆蓋層，取代整頁導航；10 頁共用，全站範圍最廣的元件之一。詳見 §4.97 **頭列改版（2026-08-13 使用者指示）**：底色由 `--card` 改 `--surface-page`（黑），與 iframe 內容同一個面——浮層讀起來是「一整頁被抬起來」，不是「一張卡上壓了一條工具列」。左邊的「返回<清單名>」退場、原地改成**整頁開啟**（`data-sheet-openfull`，帶文字）；關閉只留右上角的 ✕。理由：返回與 ✕ 是同一個動作（都關掉浮層回清單），真正缺的是跳去完整頁面的出口，那本來只是一顆沒有文字的圖示鈕。`hereName()` 與 i18n `sheet.back` 留墓碑。13 個消費頁一次生效。 **殼層換裝（2026-08-31 使用者裁示 Q97）**：浮層外框改吃 ztorUI 那套材質——背板 `--sheet-veil` 暗紗（`--ztu-canvas` 60%、**不模糊、不可動 opacity**）、面板玻璃底＋`--ztu-blur-heavy` 40px 模糊＋四角 `--radius-shell`＋頂緣內光＋`--ztu-shadow-card`、留白 `--ztu-shell-gutter` 四邊等寬（不再貼底）、頭列改透明＋`--border-soft` 分隔線、控件 hover 由 `--accent` 換 `--ztu-film`、iframe 整片透明。配套在 `shared.css` 內嵌模式：`.main` 的不透明底改透明，並關掉 `<body>` 的檯面色、暖光與四層星空——否則 iframe 會把檯面再畫一次，浮層讀起來像「換了一張新背景」而不是「一塊毛玻璃蓋在原本那頁上」（2026-08-31 使用者回饋）。內容頁一行未改。 | [detail-sheet.css](./ds-components/detail-sheet.css) **2026-09-01 面板的毛玻璃搬到 `::before` 負層**：`backdrop-filter` 掛在面板本體時，面板會成為一個 **backdrop root**，裡面 iframe 的所有 `backdrop-filter` 在部分瀏覽器整組失效（詳情頁黏住的頂列因此沒有玻璃，使用者用對照開關證實）。搬到 `::before`（`absolute; inset:0; z-index:-1`）之後視覺完全不變——負層畫在面板底色之上、流內內容之下，糊的仍是背後那一頁——但 iframe 從後代變成兄弟，內側的玻璃就活過來。**不要搬回本體。** |
...
|---|---|---|
| default | — | **2026-08-31 加字影（Q99）**：`text-shadow: var(--ztu-accent-ink-shadow)`（`0 1px 3px` 45% 黑，同日由 `0 1px 2px` 30% 加重一階）——份量刻意只夠把白字從橘面上抬起一點，橘面本身就亮，再重就變浮雕。<br>**ztorUI 換裝（2026-08-28 Wave 2b）**：`background: var(--ztu-accent-surface)`（78% 橘漸層玻璃）＋`backdrop-filter: var(--ztu-accent-blur)`＋`box-shadow: var(--ztu-accent-glow)`（四層一次寫完的光暈）＋`::before` 霧光色層（`var(--bloom-surface)`，內縮 3px 模糊 5px），text `--primary-foreground`。取代舊值 `background: var(--primary)`＋`box-shadow: var(--shadow-raise)` 的實色方塊。⚠ 已知取捨：白字疊在 78% 橘玻璃上實測約 2.4:1，低於 WCAG AA 4.5:1，2026-08-26 使用者已裁決接受 |
| hover | `:hover` | **2026-08-31 改強（Q99）**：primary 三件一起動——`::after` 疊一層 `--ztu-accent-wash-hover`（同色薄洗，只補間 opacity 所以進出是柔的）＋光暈換 `--ztu-accent-glow-hover`（更外擴、內光與描邊各提一階）＋收尾 `filter: brightness(1.04)`。原本只有 `brightness(1.07)`，半透明漸層整顆提亮 7% 在深色檯面上幾乎看不出來。色票仍不換（漸層沒有「亮一階」的對應值）。<br>**outline 改吃 `--control-raise-hover`**（深色＝同族薄膜 12%，靜止是 4%）＋ border 提到 `--ztu-glass-rim`：原本 hover 換成 `--accent`（深色是 `#2A2B2C` 實色深灰），從薄膜掉回一塊比周圍更暗的色塊，在玻璃面上會讀成「按下去變暗了」。<br>ghost gets `color-mix(--foreground 6%)` fill |
| focus | `:focus-visible` | `outline: 2px solid var(--ring)`, offset 2px |
| disabled | `:disabled` / `[aria-disabled="true"]` | 非填色變體（ghost／outline／soft／icon）：`opacity: 0.45`、`pointer-events: none`<br>填色變體（`--primary`／`--destructive`）**改灰底灰字**（2026-08-04，Q50，換裝後不變）：`background: var(--muted)`、`color: var(--muted-foreground)`、`border-color: var(--border)`、`opacity: 1`、`cursor: not-allowed`。**ztorUI 換裝新增收尾（2026-08-28 Wave 2b）**：另補 `backdrop-filter: none`、`box-shadow: none`、`::before { content: none }`，把霧光色層與光暈一併收掉——否則灰底按鈕底下會透出一團橘光（沙盒本身沒做這步，是熔接進本體時的必要收尾） |

| busy | `[aria-busy="true"]` | **2026-09-02 新增**（首個消費者：創作者活動管理的「重新檢查」）：`cursor: progress` ＋ 內部 `.ztor-icon` 跑 `btn-busy-spin`（0.7s linear infinite），其餘一切維持原狀。<br>**刻意不等同 disabled**：停用態退到 45% 不透明，講的是「你現在不能按」；忙碌講的是「你按過了，它正在做」，按鈕該維持原來的重量。文字由消費頁換成進行式（「檢查中…」）。<br>重複點擊由消費頁的 JS 旗標擋，**不用 `pointer-events: none`**——那會讓忙碌中的按鈕連 hover 回饋都沒有，讀起來像整顆壞掉。<br>狀態鉤子用 `aria-busy` 而非自訂 class：這個狀態本來就有語意屬性，螢幕閱讀器也讀得到。`prefers-reduced-motion` 下不轉 |
...
| `.btn--sm` / `.btn--lg` | Compact 32px (`--control-h-xs`; 28 until 2026-09-11) / large 44px (`--control-h-md`); default `.btn` = 36px (`--control-h-sm`) |
...
- primary bg `--ztu-accent-surface`（2026-08-28 起，見 §1.1b） · primary blur `--ztu-accent-blur` · primary glow `--ztu-accent-glow` · primary hover `--ztu-accent-wash-hover`／`--ztu-accent-glow-hover` · primary 字影 `--ztu-accent-ink-shadow` · outline hover `--control-raise-hover` · `::before` 霧光 `--bloom-surface` · text `--primary-foreground` · outline surface `--card` / `--muted` · outline border `--border`（平面，無 shadow） · ghost text `--foreground-muted` → `--foreground` · radius `--radius` (primary) / `--radius-md` (outline) · focus ring `--ring` · motion `--duration` / `--easing` · font `--font-ui`（Poppins，見 §1.2）
...
> **`--icon-circle` 的底色（2026-09-11 換裝補漏）**：預設改 `--ztu-film` 薄膜底＋`--foreground-muted` 圖示、hover `--ztu-glass-strong`（使用者指著黏住頂列的返回鍵「顏色沒有照 DS」——Wave 2 把控件底統一成薄膜時漏了這一支，實色 `--muted` 在玻璃列上是一顆陷下去的深色圓）。薄膜是相對底色的，下面「面向假設」那段的問題隨之消失，留作沿革：舊預設 `--muted` 底＋`--muted-foreground` 圖示假設鈕坐在 `--card`（#212223）之類的亮面上。坐到 `--surface-page`（#0C0D0D，全站最暗）時 `--muted`（#161718）與底只差一階、整顆會消失。這種情境要在消費端 scope 提亮，別改元件預設。已知案例：建立專案閘門的 `.wizard__gate-close`（2026-07-26 使用者反饋「太暗」，改 `--card` 底＋1px `--border`＋`--foreground-muted` 圖示，見 shared.css）；composer 的送出鈕坐在卡片上，不受影響。
...
**Sizes** — `.badge` 4×10 padding, 12px / 500, `--radius-pill`（**ztorUI 換裝 2026-08-28 Wave 2a：原 `--radius-md`，改全圓膠囊**，見「Q1 形狀＝可否互動的線索」 一節——語意上仍是純顯示徽章，圓角換的是換裝的視覺語彙不是互動性）。`.ztor-badge` 2×8 padding, 12px. `.ztor-metric-pill` 4×10 padding, 14px.

**States**

| State | Selector | Change |
|---|---|---|
| default | `.badge` | **ztorUI 換裝（2026-08-28 Wave 2a）**：bg `--ztu-film`（原 `--muted` 實色），text `--foreground-muted`, no ring, no dot |
| (variant) | `.badge--success` etc. | bg `color-mix(--status 14%, surface)` (orange 30% · accent 16%), text = the saturated hue token — 六個語意染色變體不受換裝影響（仍是彩色 tint，換裝只換中性徽章的底） |
...
| Class / modifier | Effect |
|---|---|
| `.badge` | Flat neutral tag, `--radius-pill`（2026-08-28 換裝，原 `--radius-md`）, no dot / no ring |
...
| `.badge--neutral` | `--ztu-film` background（2026-08-28 換裝，原 `--muted`） |
...
- neutral bg `--ztu-film`（2026-08-28 換裝，原 `--muted`） + `color-mix` of `--status-success` / `--status-error` / `--status-info` / `--status-accent` / `--primary` against `--card` (tints track light/dark automatically) · text = matching hue tokens (`--status-*` / `--primary` for orange) or darkened for warning / `--foreground-muted` for neutral · radius `--radius-pill`（2026-08-28 換裝，原 `--radius-md`，badge） / `--radius` (ztor-badge) · font `--font-ui` · **no box-shadow**

**商品與組合的狀態徽章是推導出來的，不是欄位（spec §7.14 · D241，2026-09-04）**

上架品項有三個互相獨立的開關（上架 `listed`／顯示於商店 `shown`／開賣 `saleStart`–`saleEnd`）再加一個可售量 `qty`，徽章是它們算出來的結果。八態的推導由 `js/listing-state.js` 的 `deriveStatus(entity, ctx, now)` 持有，**優先序固定、各頁不得自行調換**：

| 主徽章 | tone → class | 推導條件 |
|---|---|---|
| Draft／未完成 | neutral → `.badge--neutral` | 尚未發布；沒有開關、沒有連結，池裡還沒有這件商品 |
| Unlisted／已下架 | neutral → `.badge--neutral` | `listed` 關著，或上架日期與時間未到，或下架日期與時間已過。公開與非公開連結都失效 |
| Hidden／已隱藏 | neutral → `.badge--neutral` | 已上架但 `shown` 關著；商店找不到，持非公開連結仍可買，也能被組合包帶著賣 |
| Ended／販售結束 | neutral → `.badge--neutral` | 停售日期與時間已過；不會自己回來，要改停售日期與時間 |
| Sold out／售罄 | error → `.badge--error` | 該販售管道 `qty === 0`。鎖定的販售管道賣完鎖定量即售罄，池裡還有未鎖定的數量也一樣 |
| Coming soon／即將開賣 | info → `.badge--info` | 開賣日期與時間在未來；看得到、看得到時間，不能買 |
| Low stock／急需補貨 | warning → `.badge--warning` | `qty` 已達或低於低庫存門檻（不限量的池不會走到這裡） |
| Live／販售中 | success → `.badge--success` | 上架＋顯示＋在販售窗口內＋有貨 |

- **`hidden` 排在 `ended` 與 `soldout` 之前**：清單列只有一個徽章位置，「商店裡找不到」是比較該先講的事。細節頁擺得下兩顆，改叫 `deriveFlags(entity, ctx, now)`——它回 `{ status, hidden }`，`status` 跳過 hidden 那一層，`hidden` 旗標另掛第二顆徽章（例：售罄 ＋ 已隱藏並排）。
- **class 走 `ListingState.badgeClass(status)`、文案走 `status.*` 這組 i18n key**（`shop.status.draft`／`live`／`coming`／`ended`／`soldout`／`low`／`hidden`／`unlisted`）。舊的 `e-shop.status.*`、`e-shop.row.*`、`product-detail.badge.*`、`bd.badge.*` 保留到各頁改版為止，改完再一併清。
- **`qty` 是「哪一個販售管道」的可售量**：單售用 `channelQty(product, 'single')`、某個組合包用 `channelQty(product, { bundle: id })`、組合包自己用 `bundleQty(bundle, productsById)`。同一件商品在不同販售管道的狀態本來就不同，元件層不猜。
- tone 只用既有的五個變體，不為這八態新增顏色。四個 neutral（draft／unlisted／hidden／ended）是刻意的：它們都是「現在不賣，但沒有異常」，用同一個安靜的殼，差異由文字承擔。
...
<span class="badge badge--outline">Paid</span>          <!-- 帳的搬動：提款／作廢沖銷／扣款 -->
...
**States（2026-09-21 補）** — `.chip--active` 已選、`.chip--static` 唯讀、`.chip:disabled`／`[aria-disabled="true"]` 選不到（淡化＋劃線＋`not-allowed`，hover 不變色；首用組合包預覽卡 §4.213）。
...
**Variants** — Base `.chip`, `.chip--active` (selected), `.chip--static` (read-only, no hover), `.chip--value` (quiet fill for a creator-entered value, 2026-07-21), `.chip--add` (dashed border, Q71 2026-08-25). Container `.chip-group`; row wrapper `.filter-row` + `.filter-row__actions`.

**Sizes** — Single size (6 × 12px padding, 12px / 500).

**Q71 borderless pass (2026-08-25)** — the whole family lost its 1px `--border`: unselected fill is now `--input-surface` (same interactive-fill answer as every other control, Q19), hover is `--accent`. The family reads as a row of "tags" rather than a row of buttons. `.chip--add` is the deliberate exception that keeps a border (dashed) — see States. **ztorUI 換裝（2026-08-28 Wave 2b）疊在 Q71 之上、未推翻它**：無框化的結論不變，只是底色與已選態的材質換了一批新值，見下方 States。

**States**

| State | Selector | Change |
|---|---|---|
| default | — | **ztorUI 換裝（2026-08-28）**：bg `--ztu-film`（原 Q71 定案的 `--input-surface`），text `--foreground-muted`, no border, `--radius-pill` |
| hover | `:hover` | **ztorUI 換裝**：bg `--ztu-glass-strong`（12% 白玻璃，原 `--accent` 實色）, text `--foreground` |
| active | `.chip--active`（含 `:hover`） | **ztorUI 換裝（2026-08-28 Wave 2b）**：bg `--ztu-glass-strong` ＋ `backdrop-filter: blur(var(--ztu-blur-glass))` ＋ `box-shadow: inset 0 1px 0 var(--ztu-rim-soft)`（頂緣內光）, text `--ztu-orange-hi`。取代 2026-07-27 Q8-A 的「13% 橘 tint 色塊＋橘字」——橘 tint 色塊是換裝前全站唯一還在用色塊當選中態的地方，2026-08-28 使用者當面指出與測試 DS 不同，改成「表面中性玻璃、顏色只留在光與字」（同一輪一併改的還有側欄選中、狀態標籤、分頁燈條） |
| static | `.chip--static` | `cursor: default`, hover 底跟著 `.chip` 改吃 `--ztu-film`（原 `--input-surface`），text `--foreground-muted`（Q71 的「唯讀不變色」原則不變） |
| value | `.chip--value`（含 `:hover`） | bg `--ztu-film`（原 `--input-surface`）, text `--foreground`；仍與預設底色相同，保留只為語意標記——**合併候選**，下一輪盤點處理 |
| add | `.chip--add` | 1px dashed `--border`, transparent bg — Q71 唯一保留邊框的變體，語意是「這排的下一格」而非篩選項，不受換裝影響 |
...
| `.chip--active` | Selected state — 中性玻璃殼 ＋ 頂緣內光 ＋ 橘字（2026-08-28 換裝，原為 Q8-A 2026-07-27 的橘 tint 色塊） |
| `.chip--static` | Read-only chip (e.g. supported-regions list) |
| `.chip--value` | A value the creator just entered — quiet fill, never inverted. Distinct from `.chip--active` (a chosen filter) and from `.tag-input .chip--active` (an applied tag, orange per Q19). **⚠ 零消費（2026-07-21）**：唯一消費者是建立商品的選項值，同日改成逐值 `.input`（見 Variant builder 條目）後這個變體失去用途，只剩 DS 頁 demo。**退場候選、待使用者裁決**，未經確認前不移除。Q71（2026-08-25）後與預設外觀相同，又多一個合併理由 |
| `.chip--add` | Q71（2026-08-25）新增：虛線框「新增」格，唯一保留邊框的 chip 變體 |
...
- bg `--ztu-film`（預設／`--static`／`--value`，2026-08-28 換裝，原 `--input-surface`） · hover bg `--ztu-glass-strong`（原 `--accent`） · active bg `--ztu-glass-strong` ＋ `backdrop-filter: var(--ztu-blur-glass)` ＋ `inset 0 1px 0 var(--ztu-rim-soft)`, text `--ztu-orange-hi`（2026-08-28 換裝，原 `--selected-surface`／`--selected-ink` 橘 tint） · text `--foreground-muted` → `--foreground` · count `--muted-foreground` · border `--border`（僅 `--add`，虛線，Q71——其餘變體皆無框） · radius `--radius-pill` · motion `--duration` / `--easing` · font `--font-ui`（Poppins）
...
| State | Selector | Change |
|---|---|---|
| default (off) | `.switch` | **ztorUI 換裝（2026-08-28 Wave 2b）**：track `--ztu-film`（原 `--accent` 實色）, ring `1px --border`, knob `color-mix(--ztu-accent-fg 62%, transparent)` ＋ `box-shadow: var(--ztu-shadow-pop)`（原 `--card` 實色滑塊＋`--shadow-raise-strong`）left:2px |
| checked (on) | `.switch--on` | **ztorUI 換裝**：track `--ztu-accent-surface` ＋ `backdrop-filter: var(--ztu-accent-blur)` ＋ `box-shadow: var(--ztu-accent-glow)`（與 `.btn--primary` 同源配方），border 轉透明，knob `--ztu-accent-fg` left:18px（原 track `--primary` 實色、ring `rgba(23,23,23,0.12)`、knob `--primary-foreground`）。連帶刪除舊有的 `[data-theme="dark"] .switch::after`／`.switch--on::after` 覆寫——滑塊已改明寫白色，不再需要那組因應半透明 `--card` 的補丁 |
...
- off track `--ztu-film`（2026-08-28 換裝，原 `--muted`）, ring `--border` · on track `--ztu-accent-surface` ＋ `--ztu-accent-blur` ＋ `--ztu-accent-glow`（原 `--primary`） · knob `color-mix(--ztu-accent-fg 62%, transparent)` ＋ `--ztu-shadow-pop`（off，原 `--card`） / `--ztu-accent-fg`（on，原 `--primary-foreground`） · radius `--radius-pill` · knob motion `left 150ms ease`, track `--duration` / `--easing`
...
| State | Selector | Change |
|---|---|---|
| default | — | neutral glass surface (`--ztu-glass-bg` + blur + 1px `--border` + top-edge rim), `--foreground-muted` text, `--radius-xl` corners and a 10px icon gap; the leading icon carries `--ztu-orange-hi` + a single soft glow so the "pay attention" cue lives in the icon, not the surface |
...
| Class / modifier | Effect |
|---|---|
| `.info-banner` | Neutral glass information banner (flex, centered vertically) |
| `.info-banner__icon` | Leading Tabler `info` icon, 20px desktop / 18px mobile |

**Token usage** (→ Pillar 2 Role)

- bg `--ztu-glass-bg` + `backdrop-filter: blur(var(--ztu-blur-glass))` · border `--border` · top rim `--shadow-edge-top` · text `--foreground-muted` · icon `--ztu-orange-hi` + `drop-shadow` glow from `--ztu-orange` · radius `--radius-xl` · type `--font-ui`
...
**2026-08-28 換裝**（Wave 3 C-1 使用者裁決）：面從實色 `--accent` 改中性玻璃，跟卡片家族同語言（Q77）；「這是說明、要留意」不再靠實色橘底表達，改由 leading icon 的橘色與柔光承擔，量級比 filter-tabs 選中字光更收斂（單層、半徑更小）——banner 是被動說明，不是使用者選中的互動態。
...
| State | Selector | Change |
|---|---|---|
| default | — | dashed `--border`, text `--muted-foreground`；bg `--ztu-film`（2026-08-28 換裝，原 `--input-surface` 實色填色底——巢狀/凹槽家族統一改吃薄膜，同 `.nest`／`.card--muted`，刻意不上 `backdrop-filter`） |
| hover | `:hover` | border → `--muted-foreground`, bg → `--ztu-glass-strong`（2026-08-28 換裝，原 `--accent` 不透明深灰）；**還沒填的格子邊框同時收細成 `.5px`**（2026-08-09 使用者指示：hover 已經有底色在說「可以點」，邊框不必再加粗宣示）。已填的格子是實線邊，粗細不動。**border-style 刻意不隨換裝改動**——這支元件靠 border-style 承載狀態（空的投放區 dashed、`.is-filled` 轉 solid），統一成 1px solid 會抹掉唯一的視覺訊號 |
...
- bg `--ztu-film`（2026-08-28 換裝，原 `--input-surface`） · border `--border`(dashed) · radius `--radius-md` · text `--muted-foreground` / title `--foreground` · hover bg `--ztu-glass-strong`（2026-08-28 換裝，原 `--accent`）
...
| State | Selector | Change |
|---|---|---|
| default | — | **ztorUI 換裝（2026-08-28 Wave 2a）**：bg `--ztu-film`（`background-color`，原 `--input-surface`；刻意不用 `background` 簡寫，避免歸零 `.select` 箭頭的 `background-image`）, `0 0 0 1px var(--border)` shadow edge, text `--foreground`。`.select` 的 chevron data-URI 同輪換色：`stroke='%23737373'` → `stroke='%23ffffff' stroke-opacity='0.7'` |
| focus | `:focus` | `outline: none`; `0 0 0 1px var(--ring)` + **ztorUI 換裝**：`0 0 0 3px color-mix(in srgb, var(--ring) 20%, transparent)` soft glow（原 `4px` / `15%`，仍走 `--ring`） |
...
| `.select--bare.select--with-prefix` | 兩個 modifier 併用時補回左內距（2026-08-24，登入頁語言選擇器是第一個消費者）。`.select--bare` 用 `padding` 簡寫、又排在 `.select--with-prefix` 之後，同權重下會整個蓋掉後者的 `padding-left`，圖示便疊到文字上。要在無框的 select 裡放前置圖示就會撞到這條，規則寫在 `input.css` 的 `.select--bare` 區塊之後 |
...
| `.select` | Native `<select>`, OS arrow dropped。**箭頭由 zselect.js 畫**（`.zselect__caret`，2026-07-28 起每個 `.select` 都會被升級成自訂下拉）——頁面不要再自己掛 `.select-wrap__icon`，會疊出兩個 chevron（建立商品的分類欄踩到，2026-08-20 移除；該 class 在 input.css 留墓碑、已無消費頁） |
...
- bg `--ztu-film`（2026-08-28 換裝，原 `--input-surface`） · text `--foreground` · edge `--border` (1px shadow) · radius `--radius` · focus `--ring` edge + soft glow ring (3px/20%，換裝) · font `--font-body`（Poppins） · font size `--fs-14` · padding `--sp-12` (left/right prefix `--sp-32`)
...
| State | Selector | Change |
|---|---|---|
| default | — | **ztorUI 換裝（2026-08-28 Wave 2a）**：`--ztu-glass-bg` 半透明玻璃底（原 `--card` 實色）＋ `backdrop-filter: blur(var(--ztu-blur-glass))` ＋ **邊框回歸**`border: 1px solid var(--border)`（原 2026-07-26 Q23 決 C 去邊框、純陰影浮起；玻璃需要 1px 邊把厚度畫出來）＋ `--shadow-card` + `--shadow-edge-top`（token 值本身已在 Wave 1 對齊沙盒深投影）, `--radius-xl` |
| link hover | `.card__head .card__link:hover` | Link color → `--foreground` (chevron tracks via currentColor) |
| `.card--muted`（卡中卡） | `.card--muted` | **ztorUI 換裝**：`--ztu-film` 薄膜底（原 `--nest-surface` 4%）, `backdrop-filter: none`, `border: 1px solid var(--nest-line)`, `border-radius: var(--radius-lg)`, `box-shadow: none`——巢狀層刻意不上模糊、不疊陰影，避免讀成「浮在卡上的第二張卡」 |
| 浮層內的 `.card`（如 `.payout-dialog .card`） | `:is(.payout-dialog, .payout-modal, .dropdown__menu, .zselect__panel, .preview-panel__sheet, .detail-sheet, .overlay-surface) .card`＋`html[data-embed] .card` | **ztorUI 換裝**：退回 `--ztu-film` 薄膜＋關模糊＋`border: 1px solid var(--nest-line)`＋無陰影——浮層本身已是實色（見 §3.2），裡面再放一張會透的玻璃卡就是看穿兩層玻璃；祖先清單本波從原本只有 `.payout-dialog` 擴編成本站所有實色浮層容器。**2026-08-31 補 `html[data-embed]`**：detail-sheet 用 iframe 開詳情頁，祖先選擇器過不了 iframe 邊界，浮層裡那一頁的卡片永遠選不到這條規則，就帶著給實色檯面用的重投影坐在玻璃上（使用者「陰影太重了」）。同輪一併補進 `kpi.css`／`alert.css`／`preview-card.css` 的對應規則 |
...
- bg `--ztu-glass-bg`（2026-08-28 換裝，原 `--card`）／`.card--muted` 用 `--ztu-film` · backdrop-filter `--ztu-blur-glass` · `--foreground`, `--foreground-muted`, `--muted-foreground` (title / hint) · `--border`（**2026-08-28 起重新是卡緣**，玻璃需要 1px 邊；換裝前 2026-07-26 Q23 曾去邊只留陰影，現已回歸） · `--shadow-card`, `--shadow-edge-top` (edge) · `--radius-xl` · `--font-ui`, `--font-body`（Poppins）
...
- ✅ Do rely on the玻璃四件套（`--ztu-glass-bg` ＋ blur ＋ 1px `--border` ＋ `--shadow-card`/`--shadow-edge-top`）for the section edge（2026-08-28 換裝現況；`.funding-panel--card`／`.fc-bundle` 等 `.card` 系變體同步）。歷史：2026-07-13 Q3 純邊框無陰影 → 2026-07-26 Q23 決 C 去邊框改純陰影浮起 → 2026-08-28 換裝邊框回歸、與陰影並存。
...
.kpi[.kpi--success|--warning|--destructive]   (optional color state, tints __value only)
 ...
 └ .kpi__delta   (+/- · success default; .kpi__delta--neg = error)
   ...
   .kpi__unit    (2026-09-11：跟在 __value 數字後面的計數單位「件／人」，--fs-12 輔助色、對齊基線；金額不用)
...
| State | Selector | Change |
|---|---|---|
| default | — | **ztorUI 換裝（2026-08-28 Wave 2a）**：`--ztu-glass-bg` 半透明玻璃底（原 2026-07-26 還原的 `--card` 實色）＋ `backdrop-filter: blur(var(--ztu-blur-glass))` ＋ `border: 1px solid var(--border)`（原無邊框）＋ `--shadow-card`＋`--shadow-edge-top`；`.kpi__value` 數字排版同輪換（Wave 2b §11）：`font-weight` 500→`--fw-light`(300)、`letter-spacing` `-0.6px`→`-0.02em`、新增 `line-height: var(--lh-tight)`(1.1)，字級 28px 不變 |
| nested-in-card | `.card .kpi`, `.form-section--outlined .kpi` | **ztorUI 換裝**：退成薄膜 `--ztu-film`（原 2026-07-20 Q21 的 `--input-surface`），`backdrop-filter: none`, `border: 1px solid var(--nest-line)`, `border-radius: var(--radius-lg)`——照 `.card--muted` 兩件式配方，卡系容器內的 KPI 才不會玻璃疊玻璃。**祖先清單變動**：`.ip-hero` 已從選擇器移除（它是頁首橫幅、不是玻璃卡容器，裡面的 KPI 現在維持玻璃） |
...
| Class / modifier | Effect |
|---|---|
| `.kpi` | Tile container — column, 6px gap, 玻璃四件套（`--ztu-glass-bg` ＋ blur ＋ 1px `--border` ＋ `--shadow-card`＋`--shadow-edge-top`，2026-08-28 換裝，原無邊框純陰影） |
| `.kpi__label` | Uppercase 12px / 0.4px tracking, subtle; flex for leading icon |
| `.kpi__value` | `--font-numeric`（Satoshi，非換裝的 Poppins；不套 `--font-dot` 見 §1.1b）28px / **300 / -0.02em**（2026-08-28 換裝，原 500 / -0.6px）＋ `line-height: var(--lh-tight)`（新增） |
...
| `.kpi__unit` | 2026-09-11：計數單位小字，放在 `.kpi__value` 裡數字之後（`17<span class="kpi__unit">件</span>`）。首個消費者：商品詳情商品庫存分頁的四塊磚（在庫／單售可售／鎖給組合／補貨中）。 |
| `.kpi__meta` | 12px subtle neutral footnote (alternative to — or, on F2, alongside — delta); may wrap an `.card__link` for a deep-link |
| `.kpi__cur` | 2026-09-11：金額的幣別前綴（NT$／$）縮成 12px 輔助色小字、貼在數字前，數字維持大字級——同 `.kpi__unit` 的角色、只是在前面（使用者：「單位可以小一點，現在數字爆版了」）。首用商品／組合詳情銷售摘要。 |
| `.kpi__meta--clip` | 2026-09-11：註腳限一行、超寬以 … 截斷。首用商品／組合詳情「關聯中」磚（寫入端只寫非 0 的段、全 0 整行 `hidden`） |
| `.kpi--tappable` 當 `<a>` | 2026-09-11：整塊磚可以是 `<a href>`（去底線、字色跟磚），首用商品／組合詳情淨利磚整塊連到收入管理，右上 `.kpi__chevron`；取代磚內的 `.kpi__link` 文字連結（使用者：「像關聯中一樣一個箭頭在右上就好」） |
...
- bg `--ztu-glass-bg`（2026-08-28 換裝，原 `--card`）／巢狀 `--ztu-film`（`.card`／`.form-section--outlined` 內，原 `--input-surface`，`.ip-hero` 已移出祖先清單）; `--border`（**2026-08-28 起重新是卡緣**）; `--shadow-card`、`--shadow-edge-top`（邊界）; `--radius-xl` · `--muted-foreground` (label, meta) · `--status-success` (positive delta / `.kpi--success`), `--destructive` (negative delta / `.kpi--destructive`), `--status-warning` (`.kpi--warning`, color-mixed) · `--font-ui`（Poppins） · 數值走 `--font-numeric`（Satoshi，不受換裝影響）
...
| `.alert--bar` | Bar density。**ztorUI 換裝（2026-08-28 Wave 2a）**：玻璃四件套——`--ztu-glass-bg` 半透明底 ＋ `backdrop-filter: blur(var(--ztu-blur-glass))` ＋ `border: 1px solid var(--border)` ＋ `--shadow-card`＋`--shadow-edge-top`（原 `--card` 實色底＋純 `--shadow-card` 無頂緣內光、無邊框）；坐在浮層或另一張玻璃卡（`.card`／`.form-section--outlined`）裡時退回 `--ztu-film` 薄膜＋關模糊。`.alert__title` + inline `.alert__meta`, optional `.alert__cta`, `.alert__dismiss` ✕ |
...
| `.alert-inset` ＋ `.alert--inset-card` | 收窄置中的頁面通知條（2026-07-26 promoted 自 e-shop `#eshop-stock-bar`，Figma 856:25546）。**兩層結構**：外層 `.alert-inset` 只管定位（sticky `top:--sp-16`、`max-width:1280` + `margin-inline:auto` + `padding-inline:28px`，與 `.page` 同配方，須為 `.main` 第一個子元素）；內層 `.alert.alert--bar.alert--inset-card` 才是視覺卡，12/16 內距＋`--radius-xl`，**表面 2026-08-28 起交回 `.alert--bar` 自己的玻璃配方**（換裝前深色自帶 `background: var(--surface-shell)`＋`box-shadow: none` 覆寫，本波已移除，本變體現在只管內距與圓角）；淺色仍維持 `html[data-theme="light"] .alert--inset-card` 的 `--card` 白底＋`--shadow-card` 例外。無 icon、無狀態色。分兩層的原因：單層 `margin-inline:max(...)` 在 `.main` flex 直欄容器裡寬螢幕會右側溢出。**黏頂遮罩 `.alert-inset::before`**（2026-08-01 使用者裁示，**2026-08-28 換裝再改**）：原本補一層 `--surface-page` 同色底遮住黏頂與過捲的縫隙；Wave 1 把內容區改透明見底之後，這塊補色遮罩從「跟底同色」變成「浮在檯面上的一塊黑方角」，把外殼右上角的弧蓋掉，本波改成 `background: none`（純裝飾層留空盒不影響佈局，日後要復原只要把漸層寫回來）。`z-index:-1` 使它坐在卡片後面——外層有 z-index、自成堆疊脈絡，負值只落到該脈絡底層、仍蓋在頁面內容之上。**z-index 為 10**（2026-08-01 由 6 抬升）：6 與 `.list-toolbar` 同層，同層時 DOM 在後面的贏、整條分頁工作列會蓋過通知；10 高過一般頁面內容與工作列（6），仍低於黏頂的 `.list-dock`（20）／篩選浮層與下拉（30）／`app-topbar`（50）。**與黏頂工作列的接手**：頁面若讓 `.list-toolbar` 也 sticky 在同一個 `top`（屬各頁版面決定、寫在頁面 `<style>`，見 list-toolbar.css 檔頭），給它 `z-index: 12`，貼頂時工作列卡與通知條同寬同圓角、不透明且高 2px，剛好完整接手＝通知讓位（首例 `earnings-sony.html`）。Consumers：E-Shop 低庫存提醒（F2）、Events 情境提醒（`js/scenario.js` 注入）、Earnings Sony 版佣金提示 |
...
- `--status-warning`, `--destructive`, `--status-success`, `--status-info` (icon tint via `color-mix` on `--card`, glyph color on `--row`, CTA) · `--card`, `--muted` (card icon chip, close hover) · `--border` (card divider, rule between stacked rows); `--foreground`, `--foreground-muted`, `--muted-foreground` · `--radius-md`, `--radius-sm`; `--font-ui` · `.alert--bar` bg `--ztu-glass-bg`／薄膜 `--ztu-film`, blur `--ztu-blur-glass`, border `--border`（2026-08-28 換裝，見上表）
...
| active | `.tabs__item--active` | Color `--foreground` + `--primary` (orange) bottom border（base 樣式不受換裝影響；`--underline-short`／`--underline-label` 兩個變體的 `::after` 底線改走發光燈條，見下方 Class API） |
...
| `.tabs--underline-short` | Opt-in modifier: base gray divider off, active underline shortened + centered (`--sp-12` inset each side) instead of full-width. Separator left to the consuming context (E-Shop F3 type switch)。**ztorUI 換裝（2026-08-28 Wave 2b）**：`::after` 底線從 `background: var(--primary)` ＋ `border-radius: 2px` 改成「發光燈條」——長度方向五段漸層（兩端透明、中段最亮，用 `--ztu-orange-hi`）＋ `--radius-pill` ＋ 三層外光暈（6px 70% → 16px 45% → 22px 28%，最後一層下偏 2px），比 `--ztu-accent-glow`（主要按鈕）弱一階，避免跟右上角 CTA 搶焦 |
| `.tabs--underline-label` | Opt-in, pairs with `--underline-short`: active underline spans only the label span, excluding a trailing `.tabs__item-count`. Needs the label wrapped in a child element. **2026-07-26 起為 `.list-toolbar` 主軸分頁的標準配方之一**（projects／my-ip／e-shop／events／tier-settings）。**2026-07-31 使用者裁決升格為鐵律：這一組只能裝在 `.list-toolbar` 的 58px 實色殼裡**——底線是照容器下緣畫的（`bottom: calc(-1 * var(--sp-12))`），沒有殼就變成浮在標籤下方一截無所依附的橘線。要一排沒有殼的分頁，改用 `.tabs--card`（自己是一張卡）或 base `.tabs` 的 hairline。**2026-09-21 起第二種合格的殼＝§4.214 Section tabs 的 `.section-tabs`（44px、align-items:stretch，長表單頂列的分節分頁）**。底線材質同 `.tabs--underline-short`，2026-08-28 一起換成發光燈條 |
...
### 4.19 Composer（已退場 2026-08-25，tombstone）

> **2026-08-25 退場：** 元件巡檢報告判定零消費——全庫只有 design-system.html 掛過 `<link>`，連產品頁的 `<link>` 都沒有，比一般零消費元件更進一步，是純展示、從未進入任何工作流程。與同名相近的 `post-composer.css`（project-detail.html 在用）是不同構想，不是誤植重複。使用者裁示「沒用就刪」，樣式已移除、檔案保留為墓碑。以下保留原始說明供追溯。
...
**`_layer`** · organism — Canonical 64px **sticky** app topbar: brand logo left, semantic nav with a sliding highlight pill + mega-dropdown groups, right action cluster (theme · search · notifications · avatar). This is the **real shipped topbar** (`.app-topbar`, injected by `sidebar.js`); product pages load it from `ds-components/header.css` via a `shared.css` `@import`, and `design-system.html` links it directly. **D220 (2026-08-24) retired the old shell language toggle; D221 (same day) revised D220 and brought it back in a new form; D222 (same day) withdrew D221's split into two independent states; D223 (same day) simplified the collapsed row's copy.** The account menu (both topbar dropdown and sidebar subnav) carries an expandable **default-language** row (`window.ztorLang` — `.app-topbar__dropdown-option--toggle`/`--lang` and `.app-sidebar__sub-link--lang-toggle`/`--lang`, built by `js/sidebar.js`'s `langMenuHtml()`/`langOptionsHtml()`, patched live by `refreshLangRows()` on `ztor:lang-changed` — see §4.x NavigationMenu for the rendered demo). **D223**: the collapsed toggle row shows only the current language name + chevron (dropped the "Default language" label and the two-line title/sub layout so indent and type size match the plain rows above it — Profile / Settings / Payments); the toggle carries `aria-label` (`settings.lang.toggle-label`) since the visible text no longer says "language". Language is a **single concept** with **three** entry points — settings.html "Default language" row, this account-menu row, and `login.html` — all three change the same value, which is both the interface language and the input language for new content. See §4.x Settings — Language and `partials/publish-preview.js` (the separate, still-independent content-view language for translatable fields inside create/publish flows — since D223 shown after the publish click via a pre-publish preview/confirm modal, not the retired page-top Lang switch tabs).
...
└─ .data-list__row (grid minmax(0,1fr) auto, gap 12, padding 12px 0, border-bottom --border)
   ├─ .data-list__row--disabled (2026-09-09；列還在、看得見但不可選：只 __row-main 降到 opacity .5，右側徽章與連結維持原強度)
   ├─ .data-list__row--go (2026-09-11；整列是入口 role="button"：cursor pointer，hover／focus 只提亮行尾 __go 箭頭、列底不塗色。欄目名放 __meta 在上、值放 __title 在下——值比欄目名重要。首個消費者：product-detail 商品設定概覽)
...
| State | Selector | Change |
|---|---|---|
| default | — | **ztorUI 換裝（2026-08-28 Wave 2a）**：bg `--ztu-film`（原 `--card` 實色）, 1px `--border`, radius `--radius-md`。刻意不加控件家族那圈 focus/box-shadow 環——已有一條真 border，兩者疊加會讀成 2px 粗邊 |
| hover | `:hover` | **ztorUI 換裝**：bg `--ztu-glass-strong`（原 `--accent`） |
...
| Class / modifier | Effect |
|---|---|
| `.field-pill` | 薄膜底＋1px 線框 pill（2026-08-28 換裝，原白底實色） |
...
**Token usage** — surface `--ztu-film`（2026-08-28 換裝，原 `--card`） · border `--border` · hover `--ztu-glass-strong`（原 `--muted`） · icon `--muted-foreground` · focus `--ring` · radius `--radius-md`
...
| State | Selector | Change |
|---|---|---|
| default | `.segmented__btn` | 透明底、`--foreground-muted` 文字，`border-radius: var(--radius-pill)`（2026-08-28 換裝，原 `--radius-md`） |
| hover | `:hover` | 文字 → `--foreground` |
| active | `.segmented__btn--active` | **ztorUI 換裝（2026-08-28 Wave 2b）**：`background: var(--ztu-accent-surface)` ＋ `backdrop-filter: var(--ztu-accent-blur)` ＋ `box-shadow: var(--ztu-pill-active-glow)`（發光橘膠囊，比 `.btn--primary` 安靜一階）, text `--ztu-accent-fg`（白）。取代原本的「白色浮起 pill、橘字」（`--card` 底＋`0 1px 2px` 柔陰影） |

**Class API**

| Class | Effect |
|---|---|
| `.segmented` | 軌道容器。**ztorUI 換裝（2026-08-28）**：`--ztu-film` 6% 白薄膜（原 `--muted` 實色，2026-07-16 由 `color-mix --foreground 5%` 改純 `--muted`），`border-radius: var(--radius-pill)`（原 `--radius-lg`）＋ 1px `--border`，3px padding——實色軌道會把下面發光的選中段框成一塊貼紙，玻璃薄膜才讓光透得出來 |
| `.segmented__btn` | 段；靜音文字 |
| `.segmented__btn--active` | 發光橘膠囊（2026-08-28 換裝，原白色浮起 pill＋橘字，見上表） |
...
**Token usage** — track `--ztu-film`（2026-08-28 換裝，原 `--muted`） ＋ 1px `--border` · active `--ztu-accent-surface` ＋ `--ztu-accent-blur` ＋ `--ztu-pill-active-glow`, text `--ztu-accent-fg`（2026-08-28 換裝，原 `--card` 底＋文字 `--primary`） · text `--foreground-muted` → `--foreground` · radius `--radius-pill`（2026-08-28 換裝，原 `--radius-lg`/`--radius-md`）
...
**surface 層** — 卡本體坐 `--ztu-glass-bg` 玻璃底（2026-08-28 換裝，原 `--card` 實色）＋ `backdrop-filter: blur(var(--ztu-blur-glass))` ＋ 1px `--border` ＋ `--shadow-card`／`--shadow-edge-top`；坐在浮層或另一張玻璃卡裡時退回 `--ztu-film` 薄膜＋關模糊（見 §1.1b／§3.2）；放在預覽面板 `__body` 的 `--muted`（灰）底上浮起。

**Class API**

| Class | Effect |
|---|---|
| `.preview-card` | 玻璃卡容器（2026-08-28 換裝，原白卡實色 1px border） |
...
| Class | Effect |
|---|---|
| `.readiness` | 玻璃卡容器（2026-08-28 換裝：`--ztu-glass-bg` ＋ `backdrop-filter: blur(var(--ztu-blur-glass))` ＋ 1px `--border`，原 `--card` 實色＋`--shadow-card`）；懸浮版 `.readiness__pop .readiness` 退回 `--popover` 實色（浮層一律不透明，見 §3.2） |
...
| `.readiness__chip(.--ready)` / `.readiness__anchor` | `__pop` 的兩種掛法：chip＝footer 常駐就緒指示；anchor＝直接包住主動作按鈕、hover／focus 才浮出檢查卡（anchor 的 hover 目標是外層容器而非按鈕，因為未備齊時按鈕 `disabled`、部分瀏覽器不送滑鼠事件給它） |

**Usage** — create-product 依類型換檢查項（實體 5／數位 5；拍賣已獨立成 create-auction，8 項）；待補項標 pending、不擋上架。建立流程共用。檢查項與硬性必填的對應見規格 §4.6（D026 待協調）；實體型的「1+ gallery images」2026-08-31 依 D226 撤除。
...
### 4.22h0 Section nav（側欄分節導覽，2026-08-17）

分頁**裡面**再分節時用的左側清單。與 `tabs.css` 的分工：`.tabs` 是頁面層級（橫排、少量、每格一個大主題）；`.section-nav` 住在其中一格裡（直排、可以多、每節是同一主題的不同切面）。五項以上橫排會擠成兩行或被截斷，直排天生可長。

- `.section-nav-layout` 兩欄 grid（**140px** 導覽＋內容，2026-08-31 由 168px 收窄），900px 以下疊成上下、導覽改橫向可捲。這一欄的視覺重量來自每列的 `border-bottom`——標籤多半兩三個字、線卻拉滿整欄寬，收窄之後線跟著短。**140 接近下限**：全站最長的標籤是 fan-detail 的「Behavior timeline」（13px／字重 500 實測 114px ＋ 左右內距 8px ＝ 122px），再窄會換行。設定頁的 `.settings-layout` 同輪一併由 220px 對齊到 140px——兩者長得一樣、item 樣式也共用，不該有兩個寬度；**要改一起改**。
- **間距（2026-08-17 使用者指示加大）**：與上方頁首之間 `--sp-48`（原 24）、兩欄之間 `--sp-40`（原 24）。頁首是「這是哪一場活動」、底下是「這一場的內容」，兩者是不同層級；欄距則要讓人看出左邊是選單、右邊是它選出來的結果。兩個值都寫在元件裡，不靠呼叫端各給一個 `.mt-*`。
- **已選態＝分隔線式（Q71/Q75 定案 2026-08-25，取代原本的品牌橘 tint／藥丸答案）**：每列 1px `--border-soft` 下緣線、末列去線，無底色、無藥丸、無 icon；已選只換文字色（`--selected-ink`）＋字重（medium），不換底色。三頁同款：product-detail（本輪由橫排 tabs 改為分節導覽）、series-detail、settings。
- **原本叫 `.settings-nav`**、只長在設定頁；活動詳情的「售票與名單」要用同一個東西時提升成中性名稱。舊 class 仍列在同一組選擇器裡，`settings.html` 的 markup 不必改，`settings.css` 只留容器的吸附位移；item 視覺（含分隔線式已選態）自 Q75 起由本檔 `section-nav.css` 統一提供。
- **與詳情頂列的關係（2026-09-11 兩輪）**：上午先做「共存」（橫列 `top` 讓出 `--detail-bar-h`），同日使用者裁決「兩層固定要合併成一個」（Q115）——有 `.detail-topbar` 的頁面，`[data-nav] > .list-toolbar` **不再自己 sticky**（`shared.css` 以 `.page:has(> .detail-topbar)` 關掉），改由頂列在黏住後**收編**它：橫列捲到頂列底下時，`.tabs` 搬進頂列的 `[data-dock-tabs]` 槽（起點對齊內容欄，左邊留給縮圖＋名稱）、動作群搬進 `[data-dock-actions]`（放整頁動作右邊），原位留高不留內容；往回捲或切分頁還回去。黏住態的動作只留 icon（`[data-dock-icon]`），編輯中例外。邏輯在 `js/detail-topbar.js`，探索紀錄 `lab-detail-dock.html`。沒有頂列的頁（取貨場次詳情）維持原本的 sticky 與 `--detail-bar-h` 讓位。

#### Progress timeline（交付時間軸，2026-08-17）
...
兩列不隨日期移動：`--system`（完成作品）釘在最上方、不帶日期；`--start`（項目建立）釘在最下方，軸線在這裡停——軸線關在 `--start` 而不是 `:last-child`，因為渲染時它之後還可能被追加節點。系統節點**不存自己的狀態**，直接讀 `js/work-review-store.js` 的送審件。

**沿革**：2026-08-17 上午照元件檔頭先行補寫（當時消費頁尚未接上），同日消費頁落地後回頭對齊；同日下午依 D194 改寫成日期驅動的時間流。✝ 同輪墓碑：`.ptl__loose` 家族（軸尾公告盒）與里程碑的上移／下移鈕退場。完整規格見 §4.51b。

#### `[hidden]` 一律說了算（2026-08-17，全站規則）

`shared.css` 有一條 `[hidden] { display: none !important; }`。**新寫元件時不必再補 `.x[hidden]`。**

為什麼要全域：CSS 的 class 只要自己寫了 `display`，就會蓋過 UA stylesheet 的 `[hidden] { display:none }`——於是 JS 設了 `el.hidden = true` 的元素照樣佔著版面。站上原本散落 **80 條**同一種補丁（`.alert[hidden]`、`.badge[hidden]`、`.data-list__row[hidden]`…），而 2026-08-17 一天之內又踩到三次：

- `.bento[hidden]` — 名單那節留下 98px 空白，被當成間距問題查了半天
- `.nav-sections > [data-nav-panel][hidden]` — 沒選到的分節全部畫出來疊成一長串
- `.tabs__item[hidden]` — 沒有系列的活動仍看得到「系列場次」那一顆（點了沒反應）

逐支補永遠補不完，因為錯誤發生在「新寫一支帶 display 的 class」的當下，而那時沒有人會想到 `[hidden]`。用 `!important` 是必要的（要贏過所有 class），語意上也成立——`[hidden]` 的意思就是「這個元素現在不存在於畫面上」，沒有「隱藏但仍佔位」這種合法用法。全站 81 支元件檔已 grep 確認無此需求。

既有的 80 條逐支補丁與本條不衝突，留著不動。

#### 側欄 ＋ 分節橫列的組裝規則（2026-08-17）

活動詳情落地後的完整組裝，其他「一頁多區域、每區域再多節」的頁面照這個走：

| 層 | 元件 | 說明 |
|---|---|---|
| 主要區域（左） | `.section-nav-layout` ＋ `.section-nav` | 直排、吸附。藥丸外形與品牌橘已選態同 `.filter-tabs__item`；**高度 58px、字級 13px 對齊分節橫列**——兩排是同一層級的導覽（一個選區域、一個選分節），量體不一致會讀成有主從 |
| 分節（上） | `.list-toolbar` ＋ `.tabs--underline-short --underline-label --count-plain` | 與清單頁的類型切換同一組。**殼不能省**：那組修飾子的底線是照容器下緣畫的，沒有殼就變成浮空的橘線（見 tabs.css 的使用鐵律） |
| 內容 | `.nav-sections > [data-nav-panel]` | **各節全部排在同一頁、不互斥切換**；節內 `--sp-18`、節與節之間 `--sp-32`，直屬子元素外距歸零（照 stack.css：間距歸容器） |

**分節橫列是錨點，不是切換器**（2026-08-17 使用者裁決）：這幾節本來就是同一件事的不同段落（設定＝活動內容→場次→地點→發布→連結），互斥切換會逼人記住「剛剛那一節寫了什麼」才能對照下一節；排在一起可以直接往下讀，也還能一鍵跳。實作三件事：

- 點擊 → `scrollIntoView({ behavior:'smooth', block:'start' })`；`scroll-margin-top: 78px` 讓節首不貼著吸附橫列的底緣。
- 橫列 `position: sticky; top: 0`——它的角色從「切換」變成「跳到哪一節 ＋ 我現在在哪一節」，捲下去看不到就兩個作用都沒了。
- 捲動同步標「跨過上緣參考線的最後一節」。三個實測踩到的坑：**捲到頂**時沒有任何一節命中參考線（要 fallback 到第一節）；**捲到底**時最後一節碰不到參考線（要判斷 `scrollHeight - scrollTop - clientHeight < 4`）；**點擊後**平滑捲動途中會經過別節，要暫停同步並在落定後重新標記一次。

**間距**：分節橫列與內容之間也是 `--sp-18`（`.nav-sections { margin-top }`），並把 `.list-toolbar` 自己的 8px 歸零，讓間距只有一個來源。不改 `.list-toolbar` 的預設值，是因為在清單頁它底下緊接的是次層篩選列（同一組控制項的上下兩排，8px 是對的）；這裡它底下是內容區，兩者是「控制」與「被控制的東西」，距離該拉開。

### 4.22h1 Stat row · Meter list · Day bars（總覽階段主卡的三支，2026-08-17）

活動詳情總覽改成「一個階段一張主卡」時新增的三支。共同前提：**它們都住在別人的卡裡**，自己沒有表面——所以不能拿 `.kpi`（那是「一個數字一張卡」的量體）來充數，塞進卡片會變成卡中卡。

| Class | 用途 | 關鍵約定 |
|---|---|---|
| `.stat-row` / `.stat` | 卡最上方那排主要數字 | `.stat__of` 讓分母與數字同一行——「200 / 600」是一個值，拆兩行會讀成兩個數字；斜線與數字之間 `white-space: nowrap`，窄欄時整段一起換行（2026-08-18 補）。數字只是卡的註腳（而不是卡在講的東西）時用 `--inline`：欄名與值收成同一行、值 12／欄名 11，欄數靠 `grid-auto-flow: column` 自動等分，兩欄三欄都對得齊；0 值連欄名一起退成 `--faint-ink`。同一支元件的密度變體，不另建第二支「卡內數字排」（2026-09-02 補） |
| `.todo-list__row` | 一件可勾、可改、可刪的待辦 | 與 `.data-list`（唯讀事實列）分工：文字是使用者自己打的才用 todo-list。`__input` 刻意不叫 `.input`——頁面的兩段式編輯會鎖住所有 `.input`，而勾一件待辦不是在編輯那筆資料 |
| `.todo-list__row--system` | 系統項目 | 不可刪，行尾放的是通往設定的入口；刪掉等於連那個入口一起沒了 |
| `.todo-list__row--done` | 已完成 | 劃掉並淡化，但**留在原地**——勾完就跳到最下面，會讓人找不到剛剛勾的是哪一件 |
| todo-list 的刪除鈕 | 行尾 | 平常不出現，hover 或鍵盤走到那一列才浮出來（沒有 hover 的裝置一律顯示）——一排垃圾桶會把待辦清單讀成一份要整理的資料 |
| `.meter-list__row` | 逐項水位，三欄一線（名稱 → 量條 → 數字） | 量條欄設寬度上限；主要數字用前景色不加粗（等寬數字裡換色比換字重好認） |
| `.meter-list--media` | 最左邊多一欄縮圖 | 名稱欄的上限跟著放寬到 280px：掛圖的清單多半有第二行（「這一組裝了什麼」），180px 會把商品名切掉、等於第二行白寫 |
| `.meter-list__thumb` | 一張 36px 縮圖 | 第二張起以 −12px 疊在前一張上，外圈用卡片底色描一圈，疊住的地方才看得出是兩張圖 |
| `.meter-list__name--stack` | 名稱改上下兩行（名稱 ＋ 內容物） | 兩行各自截斷，長商品名不會把整列撐開 |
| `.meter-list__meter` | `--media` 的第三欄：數字在上、量條在下 | 兩者講的是同一件事的兩種說法，分成左右兩欄要眼睛走兩趟才配得起來（2026-08-17 使用者指示「放在進度條上方靠右」） |
| `.meter-list__left` | `--media` 的第四欄：還剩多少，貼齊整列最右 | 與左邊那組數字是兩種性質——「119 / 180」是既成事實，「還有 61 位」是接下來要處理的事。一列從左到右因此是「這是什麼 → 賣得如何 → 還差多少」三件事（原 `__num-sub` 做成數字底下的第二行，仍隸屬那一格、讀起來像續行，同日取代） |

搭配：`.stock-bar--total`（總量條 8px 變體）、`.card__subhead`（卡內小節標，比 `.card__title` 低兩階的標籤語彙）。

**為什麼不是繼續用 KPI 列**（2026-08-17 使用者裁決「KPI 和大表格中有重複」）：原本總覽是「KPI 列 → 階段主卡」兩段，但兩者講同一件事——售票中的 KPI 寫「已售 200/600」，售票進度卡再寫一次還畫了水位；進行中的「已報到」與現場報到台重複；已結束的與結案卡幾乎逐項對應。同一個數字在同一畫面出現兩次，讀的人會以為那是兩個不同的數字。數字因此收進主卡頭部，KPI 列整條退場。
...
| `.empty-card--framed` | 虛線外框、仍不填底色 | 空狀態直接落在畫布上、上下都是同一片黑時，「這一節到哪裡為止」看不出來，底下那一節的標題會讀成這一節的一部分。虛線是「這裡預留了位置、還沒有東西」，實心底色是「這裡有一組東西」，兩者不是同一句話 |
| `.empty-card__icon` | 40×40 淡化 chip（`--muted` 底）內嵌 Tabler |
| `.empty-card__title / __text` | 14/600 標題、淡化說明（max 320） |
| `.empty-card__cta` | 選配，用 `.btn` |

**空狀態不吃卡片底色**（2026-08-17 使用者裁決，全站規則，由元件層自動生效）

卡片的底色是在說「這裡有一組東西」。裡面沒有東西時，那塊底色反而替「什麼都沒有」畫出一個實心方塊——版面上最顯眼的一格，內容是零。空狀態要安靜：一個圖示、一句說明、一顆下一步，落在畫布上就好。

規則寫在元件層，呼叫點不必改：

```css
.card:has(> .empty-card):not(:has(> :not(.empty-card):not(.info-banner))) { background:none; border:none; box-shadow:none; padding:0 }
```

讀成「這張卡有空狀態子元素，而且沒有任何不是空狀態、也不是 `.info-banner` 的子元素」。三個設計判斷：

- 不用 `:only-child`：有些頁面在同一張卡放兩種空狀態變體、用 `hidden` 互換（`projects.html`），那時畫面上仍然只有一個空狀態。
- `.info-banner` 不算「別的東西」：它是對這個空狀態的補充說明（「為什麼是空的、要去哪裡設定」），與空狀態是同一則訊息的兩句話。
- 卡片同時有標題列或真的內容時**不生效**——那時底色框的是那個區塊，不是空狀態。唯一的存量例外是 `store-settings.html` 的代碼設定卡（空狀態下方還有說明、表格與按鈕）。

2026-08-17 全站實測：41 個空狀態中 40 個已無底色，剩下的 1 個就是上述刻意例外。
...
| `.event-preview-card__highlight` | 亮點（2026-09-22 D300 / 5.1.6.1 F2）：名稱下一行的一句短文字（例「台北唯一一場」），12px、`--brand-ink`；沒填由消費頁 `hidden` 整行收起、不顯佔位。與 `.page-intro__highlight`／`.review-row__value--highlight` 同一視覺角色、同一配方 |
...
| `.ztor-table__row--ledger` | 交易明細的帳務異動列（提款／作廢沖銷／扣款，2026-09-05 · D250）：整列降到 `--muted-foreground`。它回答的是「餘額為什麼變了」，不是「賺了多少」，份量低於收入列。選擇器帶 `tbody` 才壓得過 `.ztor-table tbody td` |
| `.ztor-table__amt` / `--muted` | 金額欄：靠右、等寬數字、不換行。不換行是必要的——窄欄會把「−$5,202.50」的負號折到上一行，單獨一行的負號會被讀成破折號。`--muted` 是扣項欄那種次要金額 |
| `.tx-status` / `__when` | 狀態格：徽章＋（僅待結算列）該筆的可動用時間（「12/08 可提領」）。創作者對交易明細最常問「這筆什麼時候能領」，只給一個狀態詞等於要他自己推 |
| `.tx-na` | 不適用欄位的「—」。**不填 −$0.00 或 0**——填零會被讀成「這筆真的扣了 0 元」。提款與作廢沖銷沒有逐筆扣項，那兩格就是不適用 |
| `.tx-detail__kv` | 展開層的鍵值摘要（提款／作廢沖銷／扣款用；收入事件用的是 `__ladder` 瀑布），複用 `kv-list.css` 的 `.kv`，這裡只給寬度上限與較緊的列距 |
...
RANK-BARS  ul.rank-bars > li.rank-bar (grid 1fr 48px) > .rank-bar__track [__fill + __content(dot+label)] + .rank-bar__pct; variant .rank-bars--barline drops __fill and appends .project-bar.rank-bar__bar (grid-column 1/-1) as a second row; variants .rank-bar--amount (grid 1fr auto 48px, one __amt column) / .rank-bar--stats (grid 1fr auto auto 48px, two __amt columns) / .rank-bar--stats3 (grid 1fr auto 48px, one __amt column) / .rank-bar--head (column-name row, no track)
...
| **surface** | `.card.chart-card`（兩個 class 一起） | `.chart-card` **沒有自己的表面**——它只把 padding 歸零、交給 `__head`／`__body`／`__foot` 各自補回。單獨用會是一張透明的圖直接躺在畫布上，旁邊的 `.card` 卻有底色、圓角與陰影（2026-08-17 在活動詳情踩過） |
...
| `.linechart--mini` | Compact `--axes` variant for a KPI tile: 72px plot, 30px y-axis gutter, tighter x-label spacing. Y-axis carries **only the head and tail ticks** (max and 0) — four ticks overlap at 72px, and at this size the question is "how high did it get", not "what is each band worth". X-axis carries the first and last label only (`--sparse` with two values). Same geometry and generator as the full chart (`js/linechart.js`); only size and tick density differ. Replaced the axis-less `.sparkline` in the project-detail backers tile on 2026-09-01 (使用者：「曲線圖要有細節 例如縱橫的數值」) |
| `.rank-bars--barline` / `.rank-bar__bar` | Variant that moves the proportion out from under the label into its own full-width `.project-bar` row (6px, shared.css) — `__fill` is absent in this variant, and the label always sits flush with the row start. **Project detail only** (plan breakdown + royalty region / streaming platform): the user asked for it site-wide on 2026-09-01 and then narrowed it back to that one page, so two answers coexist on purpose — see STYLE-DECISIONS Q100 before spreading either one |
...
| `.rank-bar--stats` / `.rank-bar--stats3` / `.rank-bar--head` | Table-shaped variants for the project-detail plan breakdown. `--stats` = plan · sold · raised · share (two `__amt` columns, crowdfunding); `--stats3` = plan · sold · share (pre-orders have no raised amount, so that column is dropped rather than filled with an em dash). `--head` is the column-name row: same track, no fill, one step down in size and ink |
...
│  add modifier on the container:
│    .bento--top       → align-items: start（各卡保留自然高度，不被拉齊）
│    .bento--stack-lg  → span 修飾子延後到 ≥1440px 才生效（裝表格的那幾排）
│    .bento--stack-md  → span 修飾子延後到 ≥1200px 才生效（不是表格但仍需寬度的排）
└─ direct children .bento > *  → grid-column: span 12 (full row default)
   ...
   一個欄位要放兩張以上的卡時，用 .bento__stack 當那一格的容器：
     .bento__stack.bento--span-7  (flex column, gap 同 .bento)
     └─ .card, .card, …
...
- Span modifiers（放子元素，≥900px 生效）：`.bento--span-3`、`--span-4`、`--span-5`、`--span-6`、`--span-7`、`--span-8`、`--span-9`、`--span-12`
- `.bento--top`（放容器）：該排改頂端對齊、各卡保留自然高度
- `.bento--stack-lg`（放容器）：該排的 span 修飾子延後到 ≥1440px 才生效，給裝表格的排用
- `.bento--stack-md`（放容器）：同上但門檻 1200px，給不是表格、卻仍需要一點寬度的排用
- `.bento__stack`（當一格的容器）：把兩張以上的卡直向疊進同一個 grid 欄位，用來讓兩欄高度收齊
...
| <1440px + `--stack-lg` | `@media (max-width: 1439.98px)` | 該排的子元素一律回到 span 12（給裝表格的卡用） |
| <1200px + `--stack-md` | `@media (max-width: 1199.98px)` | 同上，門檻改 1200px（給不是表格但仍需寬度的排） |
...
| `.bento--top` | 容器改 `align-items: start`；一排裡內容量差很多時，矮的卡不被拉長成空白 |
| `.bento--stack-lg` | 容器內的 span 修飾子延後到 ≥1440px 生效；<1440px 一律單欄 |
| `.bento--stack-md` | 同 `--stack-lg`，門檻 1200px；<1200px 一律單欄 |
| `.bento__stack` | 一個 grid 欄位裡直向疊多張卡的容器；自己吃 `--span-N`，內部 flex column、gap 與 `.bento` 同值 |
| `.bento__stack--fill` | 讓這一疊與旁邊那欄齊底（2026-08-28，儀表板兩欄版面）：疊本身 `height:100%`，**只讓最後一張卡** `flex:1` 吃掉剩餘高度。不平均分配是刻意的——平均分會把帶圖表的卡一起撐高、圖的縱橫比跟著跑掉；用在最矮、內容量最少的那張才不留痕跡。與 `--top` 互斥（那條讓每欄各自保留自然高度，等高就無從談起） |
...
- ✅ Do add `.bento--top` when一排裡兩張卡的內容量差很多（拉齊會讓矮的那張空一大塊）。
- ✅ Do 先想能不能用 `.bento__stack` 把兩張矮卡疊進同一欄——高度收齊了就不需要 `--top`，也不會留下空白。
...
- ❌ Don't 把 `--top` 當預設：同量級的卡等高比較整齊，只有高度落差大的那排才需要。
...
| `.dropdown__menu` | Floating panel — right-aligned, `min-width: 230px`, `--radius-lg`, `--shadow-float` (E3); padding `--sp-6` 上下、水平 0（2026-07-22 使用者裁示，Figma node 861:34183 比對——面板不留水平白，白留給選項自己）。**ztorUI 換裝（2026-08-28 Wave 2a §3.6）**：底色改 `var(--popover)`＋`backdrop-filter: none`＋**重新加回** `border: 1px solid var(--ztu-glass-rim)`（原 `--card`＋吃 `_tokens.css` 檔尾集中式毛玻璃治理段，該段本波已整段退役——浮層一律不透明是換裝後的全站規則，見 §3.2；2026-07-21 拿掉的「多餘 border」在換裝後以新配方重新加回，不是走回頭路，是玻璃卡與實色浮層兩套語彙各自的邊界需求） |
...
- bg `--popover`（2026-08-28 換裝，原 `--card`） / hover `--muted` · text `--foreground` · radius `--radius-lg` / `--radius-md`（`.dropdown__item--toggle` 例外，顯式歸零，見上方 Class API）· border `--ztu-glass-rim`（2026-08-28 換裝重新加回，見上方 Class API）· shadow `--shadow-float` · toggle 分隔線 `--border`（2026-07-22 使用者裁示，由 `--border-soft` 改：深色模式下 `--border-soft`＝`#202122` 跟面板底 `--card`＝`#212223` 只差 1 色階，線幾乎看不見；`--border` 是 Q22 已經為同理由全站提亮過的 hairline token（深色現行 `#373839`，Q22 當時 `#333435`、2026-07-26 再提亮一階定案），比另開新色階或動 `--border-soft`（全站多處消費）風險小） · focus `--ring` · font `--font-ui` (inherited)
...
**Variants** — Bank card: default / `--selected` (inset 2px foreground ring) / `--add` (dashed). Form grid: 2-col / `--single`. **`.payout-dialog__split`** (2026-08-09; first consumer = the ticket edit dialog in `create-event.html`, §4.114 Ticket tier card) — splits `__body` into a main column (fields to fill in) + a side column, `align-items: start` so the side column never stretches to the main column's height; collapses to a single column ≤640px, side column falls to the end. **Side width is `--payout-split-side` since 2026-08-10**, defaulting to the original hard-coded 200px (ticket edit dialog unchanged); create-event's Ticket setup sets 360px for the generated-ticket list, which cannot fit five fields in 200px. A variable rather than a second modifier, because the only difference is one number. **`.payout-dialog__split-side` (2026-08-10, optional)** — makes the side column `position: sticky` with its own `overflow-y`. For dialogs where the left column is the settings and the right column is what those settings produce: if the side scrolls away with the main column you can no longer see the result, which is the whole reason for splitting. Its own `max-height: 100%` makes a long result list scroll inside the column instead of stretching the dialog. Reverts to `static` at ≤640px, where stacking already scrolls and sticking would just cover what follows.
...
**`_layer`** · organism — **2026-08-13 三項更新**：（1）**商品規格（F7）的範本編輯改成彈窗**——原本清單↔編輯在同一張卡裡互換，捲到哪、現在在哪一層都得自己記；改成 `.payout-modal` ＋ `.payout-dialog--wide` 浮起來之後，背景那份清單留在原位，關掉就回到剛才那一行。`.ss-spec-editor__head`／`.ss-spec-editor__foot`／`[data-spec-view]` 隨之退場（墓碑留在 CSS）。（2）**幣別（F6）鎖成唯讀**：原生 `select` 加 `disabled`（`zselect` 觸發鈕自動跟著 disabled），預設帶一個幣別，「選擇幣別」空選項退場（D186）。（3）**代理優惠碼整支只在 Phase 4**：型別切換列、代理專屬區塊、清單的代理碼列與「自用」meta 標籤掛 `data-feat="full"`，分頁開場白做成 `data-feat`／`data-feat-off` 成對兩句（D185）。另外 **優惠碼的新增只有一顆按鈕**（自用與代理在彈窗裡切），與下文「清單底部兩顆按鈕」的舊敘述不同，以本段為準。以下描述其餘結構：E-Shop 商店層級設定的 **popup**（`store-settings.html`，spec 5.1.5.5 / D035 / D067），由 E-Shop F3「商店設定」按鈕以 **embed-modal iframe** 開啟；popup 外框承擔標題與關閉，**頁面本身無全域導航／麵包屑／頁首**，動作改置底部提交列（`.ss-actionbar`：See as fan ｜ Discard ｜ Save changes，sticky 底部；spec F1 設定動作與預覽）。IA：**店面門面**（`.ss-identity-card`）常駐置頂，用 **Base44／Facebook 式身分帶**（封面 `.ss-band__cover` ＋疊加 logo 頭像 `.ss-band__avatar` ＋店名／網址／簡介為文字），**逐欄就地編輯**（`.ss-edit`：文字態 ↔ 內嵌 input/textarea/select，✓/Enter 確認、✕/Esc 取消，`.is-editing` 切換）；品牌素材就是封面＋頭像（各自有編輯鈕），不另設上傳框；整頁用滿標準 1280px 的 `.page` 欄寬（非兩欄詳情頁，不套 `.page--narrow`）；其下**付款／出貨／幣別／尺寸指南設定／優惠碼**五個對等設定群組以 **tab 切換**（`.tabs` + `.ss-tabpanel`，一次處理其一；出貨 tab 用 `.settings-row`）。**優惠碼 tab（2026-08-10，spec F8／D183）**沿用商品規格那一組的兩層檢視（清單 `.ztor-table` ＋ 列尾 kebab ↔ 單組編輯，同樣不開次級 popup）。清單六欄（代碼／折扣／期間／狀態／賣掉／動作）——**種類不另闢一欄**，它是代碼的屬性，收進代碼欄第二行（`.ss-code-cell`，代碼用等寬字、下面一行寫自用或代理·對方名字）；「賣掉」欄直接寫「筆數 · 金額」，哪個代理在賣不必點進去才知道。**種類在「新增」的當下就決定**（清單底部兩顆按鈕：新增一組／新增代理碼），編輯頁只用唯讀 `.badge` 讀出來、不放切換器——碼發出去後改型別會讓已成交的歸屬失去意義，規格也把「建立後可否改型」標成待確認。編輯頁的版面語言與 F7 一致：返回鈕自成一行、主識別欄位（代碼）在 `.ss-spec-editor__head`，其餘設定逐條 `.settings-row`（左說明右控件），控件用 `.ss-code-num`／`.ss-code-date`／`.ss-code-partner` 收窄、不吃滿整條右欄；代理專屬的三塊（推廣者、分成、代理銷售紀錄）整組以 `[data-code-partner-only]` 顯隱，紀錄用唯讀 `.data-list.ss-code-record`（`max-width: 520px`）坐在 `.ss-spec-block` 分隔線之下；另有 **粉絲視角預覽（See as fan）** 以畫面分割開啟（`.preview-panel--inset` + `.ss-fan`）。本元件補基礎控制項沒有的欄位型別：網址前綴（`.ss-url`）、`$` 金額前綴（`.amount-field`，見 Amount field 元件）、唯讀 Stripe 狀態卡（`.ss-status`）、品牌素材上傳（`.ss-brand` + Upload tile），以及**拖曳排序清單**（`.ss-order`）與粉絲預覽內容（`.ss-fan`）。
...
└─ .review-row__item (padding 14px 0, hairline border-bottom --border-soft; none on :last-child)
   ...
   └─ .review-row__value (font-ui 13px; wraps by default, no truncation)
      ...
      └─ .review-row__value--highlight (event highlight under the name — 12px, --brand-ink; D300)
...
| `.review-row__value--highlight` | 活動亮點（2026-09-22 D300）：名稱列下方第二行，12px、`--brand-ink`，與名稱共用同一顆 Edit；沒填由消費頁 `hidden` 收起 |
| `.review-row__item--kv` | Inline modifier: grid `130px 1fr` label/value, single line, no action |

**Token usage** (→ Pillar 2 Role)

- `--border-soft` (row dividers · kv 列線 Q72) · `--muted-foreground` (empty state, kv label, action default) · `--foreground` (value, action hover) · `--foreground-muted` (action default) · `--border` (action underline) · `--font-ui` · `--lh-comfy`
...
| State | Selector | Change |
|---|---|---|
| default | `.kv` | `1px solid var(--border-soft)` top hairline（Q72，2026-08-25）, `--sp-8` vertical padding |
...
```
.stock-bar                         track (--ztu-film, 5px, pill radius, overflow hidden — 2026-08-28 換裝，原 --muted)
...
| Class / modifier | Effect |
|---|---|
| `.stock-bar` | Track — 5px tall, `--ztu-film` background（2026-08-28 換裝，原 `--muted`）, `--radius-pill`, `overflow:hidden`, `--sp-8` vertical margin |
...
- `--ztu-film`（track，2026-08-28 換裝，原 `--muted`） · `--primary` (fill) · `--destructive` (low fill) · `--radius-pill` · `--sp-8` (vertical margin)
...
### 4.51b Progress timeline

**Purpose** — 把一段交付故事畫成同一張圖：答應做什麼、做到哪、一路上跟支持者說了什麼。為項目詳情的「進度」分頁而建（spec [5.1.2.2-專案詳情.md](../../documents/5.1.2.2-專案詳情.md) §2.2.10 v3.0、D193、D194）。2026-08-17 由「可手動排序的里程碑清單」改寫成**由真實日期驅動的單軸時間流**：未來在上、今天在中、過去在下。

**Anatomy**
- `.ptl` — 容器；定義 `--ptl-dot`（軸點直徑 28px）、`--ptl-date`（日期欄寬 64px）與 `--ptl-gap`
- `.ptl__node` — 軸上一列；grid 三欄＝日期欄／軸點／內容。`::before` 是連到下一列的軸線
- `.ptl__date` — 日期欄（gutter）。必須 `align-self: start`，否則日期會置中到整列的中線。**跨年的日期年自己一行**——**這一條是規格約束、不是本層的選擇**（spec [5.1.2.2](../../documents/5.1.2.2-專案詳情.md) §2.2.10「日期與時間的型別」，2026-08-20 D209）：日期欄的日期不得在數字中間被折斷（欄寬只有 64px，「2025/12/16」會斷成「2025/12/1」＋「6」，被折斷的數字要讀兩次才知道是幾號），跨年時年獨立成行、哪一行是年跟隨該語言的日期寫法。本層只決定行高、字級與對齊。呼叫端輸出兩個 `.ptl__date-part`，順序照該語言的日期寫法（中文年在上、英文年在下）；第一行沿用 `--ptl-line` 行高維持與軸點、標題同線，第二行收到 `--lh-comfy`，兩行才讀成一個日期
- `.ptl` — 整條軸。`max-width: 760px`（2026-08-18）：每一列都是「日期＋一行標題」，滿版之後標題與右端動作之間會留下一大片空白
- 生命週期事件節點（2026-08-19，D202）：開始收單／達標成功／送審／審核結果等系統事實，與「項目建立」同款——`.ptl__node--start` ＋ `pmark--ghost` 空心小點，只有名字與日期，無動作無內容
...
- `.ptl__update`［`--release`］＞ `__update-title`／`__update-body`（`__update-head` 排標題與可見度）— 一則更新的內容區塊，兩種擺法：掛在某顆里程碑底下（`--nested`，只留一段文字），或**自己站在軸上**（`.ptl__node--post` 裡，2026-08-20 D209——日期搬到日期欄、標題列改用 `.ptl__head` 讓標題落在軸點中線上）

**Variants** — `--todo`／`--doing`／`--done`／`--alert`（里程碑狀態）；`--system`（釘頂的完成作品）／`--today`（今天）／`--start`（系統事實：項目建立與生命週期事件，只有名字與日期）／`--foot`（軸的終點，不留下方間距、不畫分隔線、軸線在此停）／`--day`（按天收合的更新列）；`--collapsed`（收起 `.ptl__fold`）。**`--start` 與 `--foot` 是兩件事**（2026-08-20 拆開，D209）：兩條規則原本都掛在 `--start`，而 `--start` 當時只有一個成員「項目建立」、它永遠是最後一列；D202 之後生命週期事件沿用同一個修飾子長相，於是軸中間每出現一個事件，它與下一列就黏在一起、中間沒有分隔線、軸線也斷一截。**這條分工是規格約束**（spec [5.1.2.2](../../documents/5.1.2.2-專案詳情.md) §2.2.10「組成」）：生命週期事件在軸上與其他列同等份量，「軸到這裡結束」與「這一列比較不重要」不得共用同一套處理。`.ptl__update--release` 標出「完成作品」那則貼文。

**軸向與排序** — 單一排序軸是真實日期。未來段由遠而近往下排、沒有預計日期的排在未來段最上方；今天在中間；過去段由近而遠往下排。里程碑落位的日期：已完成的用完成日（提前／延後完成都落在真的做完那一天，原定日期以 `.ptl__meta` 附註），其餘用預計日期。逾期的節點不留在原本的日期位置，排在今天正下方。

**今天那一格** — 整條軸上唯一會換說法的位置。`.ptl__lede` 依情境改寫（倒數、逾期、里程碑日、當天完成、全部完成、沒有日期、已取消…），`__lede-num` 把天數拉成品牌墨、`--alert` 下轉紅。發文框內嵌在它身上，**是活的 DOM**：消費頁每次重繪都要先把它從舊軸上摘下、重繪後掛回插槽，否則使用者打到一半的字與 zselect 的接線會被洗掉。

**兩列不隨日期移動** — `--system`（完成作品）釘在軸的最上方且不帶日期（它的日期是上映日，可能還沒定）；`--start`（項目建立）釘在最下方，也是軸線的終點。軸線關在 `--start` 而不是 `:last-child`，因為渲染時它之後還可能被追加節點。

**收合** — `--day` 列預設收合、里程碑預設展開。收合狀態存在消費頁的 JS 裡，不存在 DOM——每次重繪都會把 DOM 洗掉。

**收合的單位是「兩顆里程碑之間」，展開後一則一列**（2026-08-20，D209）。收合時 `--day` 那一列說出這一段有幾則；展開時那一列留下來當控制項，**每一則各自成為 `--post`：自己的日期欄、自己的一顆 `--ghost` 點**。改法的理由是這條軸的承諾是「一顆點＝一件事」，三則發生在不同日子的貼文擠在同一顆點底下，軸上會讀成「這裡發生了一件事」。連帶影響：展開與收合**必須重繪**（那幾列是兄弟節點，不是收在 `.ptl__fold` 裡的一塊，切 class 藏不掉也長不出來）。

**軸上哪些東西點得開** — 軸是掃視用的，一列只放讀得下去的份量；其餘進詳情彈窗。兩種可點的目標，同一套 hover／focus 處理（負的左右 margin 讓底色比文字往兩側各長 8px，不然亮起來像文字被框住；鍵盤走 `:focus-visible`，兩種操作看到同一個狀態）：
- `.ptl__update[data-up-open]`（2026-08-19，D205）— 每一則更新，含掛在里程碑底下的完成宣告。
- `.ptl__head[data-ms-open]`（2026-08-20，D208）— **已完成**的里程碑，整條標題列。更新貼文只有兩種（D206），兩種都要點得開，而里程碑貼文在軸上的樣子就是那一列里程碑——只讓底下那一小段內文可點，等於這一類貼文有一半的呈現點不開。**未完成的不可點**：紀錄回答的是「這顆發生過什麼」，還沒完成的那一顆能寫的只有軸上已經寫著的預計日期。消費頁的處理器負責分流（有完成宣告就開那則貼文的紀錄、沒有就開里程碑自己的），並排除 `.ptl__acts`（右端的 ⋯）——按選單不該同時開彈窗。
...
**Consumers** — `project-detail.html`（進度分頁）。

**✝ 2026-08-17 墓碑（D194）** — `.ptl__loose` / `__loose-head` / `__loose-title` / `__loose-sub`（軸尾的公告盒）退場：未掛里程碑的更新改依發布時間排進同一條軸，不再有第二個盒子。同輪撤除里程碑的上移／下移鈕（順序由預計日期決定），對應的 i18n key `pd-prog.ms.up`／`pd-prog.ms.down`／`pd-prog.loose.*` 一併移除。

**CSS** — [`progress-timeline.css`](./ds-components/progress-timeline.css)
...
**`_layer`** · atom — 嵌在卡片「底部」的滿版子層：左右與下緣切齊母卡外緣。用於「切換某個模式後，下方長出一整組設定表單」的情境。對齊 Figma node 856:27798（2026-07-21 使用者裁示）。**Q71（2026-08-25）**：拿掉向上打的陰影，改用上緣 1px `border-top: var(--nest-line)` 跟卡面分開。

**Anatomy** — 單一區塊。負 margin `calc(var(--sp-16) * -1)` 抵銷母卡左／右／下三邊的 `--sp-16` 內距；上方保留 `--sp-16` 與前一個元素（如 `.segmented`）的間距。

**層級系統：只有兩層填色**

- **L1 卡片** `--card`：絕對色（亮 `#FFFFFF`／深 `#212223`）。**不參與疊加**——DevTools 檢視時看到的就是真正的顏色。母卡本身 2026-08-28 換裝後已改吃 `--ztu-glass-bg` 玻璃底，這裡的 `--card` 描述的是 nest 分層數學原本立足的實色基準，非本波改動範圍。
- **L2 巢狀層**：**ztorUI 換裝（2026-08-28 Wave 2a）改吃 `--ztu-film`**（6% 白薄膜，原 `--nest-surface` 4% 半透明疊加）。`--nest-surface` 本身仍是活 token（`bundle-editor.css`／`funding-panel.css`／`payout-modal.css` 等十餘支未進本輪範圍的元件仍在用），只有 `.nest` 與 `.card--muted`／`.kpi.card--muted` 這幾支換了引用對象，理由與配方見 §1.1b「巢狀薄膜」。
...
- `.nest` ＝ 滿版貼齊母卡底緣、上緣髮絲線分層的「疊上去的一層」。適合「模式切換後長出來的整組表單」，要讀起來像另一個平面。
- 判斷句：這塊東西是「卡片裡的一段」還是「疊在卡片上的另一層」？前者用 `--muted`，後者用 `.nest`。

**Behavior** — `border-radius` 取 `--radius-xl`，與母卡外圓角同值（三邊已重合，同值才會完全貼齊）。**Q71（2026-08-25）**：分層靠 `border-top: 1px solid var(--nest-line)`——亮色兩層同為白，靠這道線分層；深色 `--nest-line` 為 `transparent`（線本身不可見、無害），改靠 `--ztu-film` 6% 薄膜分層（2026-08-28 換裝，原 `--nest-surface` 4%）。母卡仍以 `.form-section--outlined:has(> .nest) { overflow: hidden; }` 裁切，讓 nest 自己的外圓角與母卡完全貼齊。⚠ 母卡內若同時有需要溢出的浮層（dropdown、popover）會被一併裁掉，放 nest 前先確認。`.nest[hidden]` 強制 `display:none`，蓋過消費端自帶的 `display:flex`（如 `.variant-builder`）。刻意不加 `backdrop-filter`——母卡已是玻璃，子層再模糊只會取樣到母卡自己，且母卡有 `overflow: hidden`，溢出效果本來就會被裁掉。

**Token usage** — `--ztu-film`（2026-08-28 換裝，原 `--nest-surface`）、`--nest-line`、`--radius-xl`、`--sp-16`。既有 `--card`／`--input-surface`／`--accent`／`--border` 完全不動。`--shadow-nest-up` 已於 Q71 從本元件移除，仍供 [Funding panel](#funding-panel) 使用，非死 token。
...
### 4.54b PDP preview（粉絲端商品頁鏡像）

**`_layer`** · organism ｜ **source tier** · SiteSpecific ｜ **surface 層** · 不畫底色也不畫外框，坐在 `.preview-col` 裡；深淺底皆可。

**為什麼有這一支（2026-09-04 · D248）** — 建立商品右欄那句「買家在 Ztor 看到的樣子」，本來配的是 Studio 自己畫的一張小卡（縮圖＋名稱＋價格＋一句描述），跟粉絲端實際長相沒有關係。使用者裁示把**粉絲端商品頁（`ztor.vercel.app/shop-item.html`）手機版的版型**搬進來，資料接建立流程當下的欄位——那句話才是真的。原本的 `preview-card`（§4.54）留給其他建立流程。

**它是搬過來的設計，不是 Studio 的控制項** — 版型與比例照粉絲端量測（375 視窗、內容寬 343）。有三處刻意違反 Studio 自己的慣例，因為粉絲端就是那樣：

- 尺寸 chip 直角（粉絲端的 `--radius-md` 是 0），Studio 的控制項一律圓角
- 主要行動是白底深字的膠囊鈕（粉絲端 `.btn--mono`：商品頁已經很多品牌橘，CTA 再用橘就沒主次），Studio 的主要動作是橘底
- 價格用近白而不是品牌橘（粉絲端 LEVER 2「安靜的價格」）

這些值寫在本檔內、不外流成 Studio 的按鈕或 chip 變體。顏色與間距仍走 r2.3 的 token，只是角色對應到粉絲端的用法：`--foreground`≈text-primary、`--muted-foreground`≈text-secondary、`--faint-ink`≈text-tertiary、`--ztu-film`≈white-alpha-4、`--primary`≈yellow-500。

**Anatomy**

- `.pdp-pv__gallery` > `__frame`（`--img-portrait` 2:3，粉絲端量到 343×515）＋ `__img`／`__icon`（空狀態）＋ `__thumbs` > `__thumb`（`.is-active`）；縮圖列兩張以上才出現
- `.pdp-pv__buy` > `__eyebrow`（分類）· `__title` · `__prices`（`__price` ＋ 折扣時的 `__was` 劃線原價）· `__lead`（描述，三行截斷）
- `__variants` > `__variant` > `__vlabel` ＋ `__opts` > `__chip`（`.is-selected`）——一個選項一列，值是直角 chip
- `__actions` > `__qty`（`__qbtn`／`__qval`）＋ `__add`
- `__delivery`——買家怎麼拿到東西的一句話

**不可互動** — 數量器與加入購物車都是 `<span>`：這是粉絲端的一張畫面，不是創作者要操作的控制項。

**空狀態沿用 `.is-empty`** — 與 `preview-card` 同一種語言（斜體、淡色占位字），不另立第二套；完全沒有資料的區塊直接不出現。

**外框與內距（2026-09-04）** — 鏡像本身不畫底也不畫框，外面套一張與左欄同款的 `form-section--outlined`（使用者裁示「預覽要包在一個 section 層裡面」）；標題與說明也搬進卡內、用 `form-section__head`（「字要在 section 內」），不再是卡外一行。欄只有 270px，所以預覽欄內的區塊卡把內距由 24px 收成 `--sp-16`、head 的下邊距同步收一階（規則寫在 `preview-column.css`，scope 限 `.preview-col`，左欄不受影響）。其他建立流程的預覽仍是「卡外標題」（`.preview-col__head`），那些頁還沒改成 section。

**欄寬與顯示尺寸** — 版型是手機的 360px 內容寬，但整支以 `zoom: var(--pdp-pv-zoom, 0.75)` 縮到 3/4 顯示（2026-09-04 使用者裁示，先「縮小 1/3」再改「只要縮小 1/4」），所以 `.preview-split--phone` 的欄寬是 **270px**＝360 × 3/4 的實際佔位。**縮的是顯示尺寸、不是版型**：字級、間距、圖框比例一起縮，看到的仍是同一張手機畫面；把欄位排窄的話字級不會跟著縮，那就不是那個版型了。264px 的 `--narrow` 兩種都不是。

**Token usage** — `--img-portrait`／`--ztu-film`／`--border`·`--border-soft`／`--foreground`·`--muted-foreground`·`--faint-ink`／`--primary`／`--fs-24`·`--fs-14`·`--fs-12`／`--fw-bold`·`--fw-medium`／`--radius-pill`／`--sp-6`~`--sp-24`／`--lh-tight`·`--lh-comfy`。

**Consumers** — `create-product.html`（右欄即時預覽）。

**CSS** — [`pdp-preview.css`](./ds-components/pdp-preview.css)

---

### 4.55 Restock log（✝ 2026-09-11 退場）

補貨紀錄表，2026-07-22 建。2026-09-11 商品庫存分頁改版時先擴成六欄的「庫存歷史紀錄」，同日使用者裁決（探索頁 `lab-stock-history.html` H1）改成「一張單一筆、點開看明細」——供應商、方式、預計到貨、備註是**單**的屬性、一單可同時補好幾個組合，逐組合一列的固定欄表塞不下也會重複講。由 §4.208 Stock history 取代；`restock-log.css` 留墓碑、原始內容見 git 歷史。
...
| `.stock-tip--anchored` | 浮卡改 `position:absolute` 貼著觸發格（`top: calc(100% + --sp-6)`，寬度上限 400px）；給坐在玻璃外框（`backdrop-filter` 會把 fixed 子孫的參考框換成外框）裡的清單用，2026-09-22 D299 新增 |
| `.stock-tip--up` | 只配 `--anchored`：往上開（`bottom: calc(100% + --sp-6)`）；由消費頁 JS 依觸發格是否在視窗下半切換 |
...
**Usage** — 掛在 E-Shop 商品清單每一列的「狀態」／「庫存」儲存格；別的頁面若要做類似「hover 看明細」，先確認資料是否真的有選項層級（或其他值得補充的資訊如門檻）可拆，沒有就別硬套。**第二個消費者（2026-09-22，D299）**：商店設定優惠碼清單的「範圍」欄——碼綁了多件時格子只寫件數（`{n} 項`），hover／focus 浮出綁了哪幾件；一列＝名稱（放 `__qty`，主墨色）＋種類（放 `__name`，次墨色：單售／組合包／活動票種／品項類型），走 `--anchored`（清單外框是玻璃、fixed 定位會跑掉）。件數文案由頁面在 `i18n:applied` 重填（i18n 沒有 `{n}` 參數）。
...
**Consumers** — `e-shop.html`（Products 全部 5 列＋Bundles 兩列）；`store-settings.html`（優惠碼清單「範圍」欄，`--anchored`，2026-09-22）。
...
- `.list-toolbar` — 殼層工作列。**ztorUI 換裝（2026-08-28 Wave 2a）**：改吃與通知條相同的玻璃四件套（`--ztu-glass-bg` ＋ `backdrop-filter: blur(var(--ztu-blur-glass))` ＋ 1px `--border` ＋ `--shadow-card`/`--shadow-edge-top`），原深色實色 `--surface-shell`——理由是 Wave 1 已把星空底紋鋪在殼層坐的那張畫布上，實色底會把底紋切出一塊方形補丁（淺色主題仍維持 `--card` ＋ `--shadow-card` 不受影響）；四角 `--radius-xl` 16px、固定 58px 高、`align-items: stretch`。
...
- **`.owner-lookup__result[disabled]` 移回本檔 ＋ `.owner-lookup__flag` 新增（2026-08-13）**：選不了的那一列（字轉 `--muted-foreground`、游標維持箭頭、hover 不上底色），右邊由消費頁掛一顆狀態標籤說明原因；元件不規定標籤用什麼字，`__flag` 只負責靠右推到底、不參與壓縮。三個決定：
  - **不另設 `--disabled` class**：按鈕要真的擋住點擊，`disabled` 屬性是非加不可的，再多一個 class 就是同一件事寫兩遍，還會出現「有 class 沒 disabled」的破窗。
  - **從 `artist-picker.css` 移回本檔**：那支是 `.owner-lookup__*` 家族的樣式卻寄放在別支，而 artist-picker 目前零消費頁（見 §4.92）——沒有頁面會載入它，bookyay 閘門的已匯入列因此會完全沒有樣式。
  - **改用明確字色、不用整列 `opacity`**（原本 artist-picker 版是 `opacity: .6`）：整列淡化會連右邊那顆標籤一起淡掉，但那顆標籤正是「為什麼點不動」的答案，是這一列最該讀得清楚的東西。
  第一個消費端是 bookyay 帶入閘門的「已匯入」——同一場 bookyay 活動在 ztor 上只會存在一次，已經帶過的仍要列出來（搜得到才回答得了「這場我建過了沒」；濾掉的話畫面會說「找不到相符的活動」，那是錯的訊息），只是不能再選。
...
**ztorUI 換裝（2026-08-28 Wave 3 B 級巡檢補修，配方抄 `card.css` 的 `.card` 黃金基準／Q77 裁決）** — `.brand-card` 卡面改玻璃四件套：`background: var(--ztu-glass-bg)` ＋ `backdrop-filter: blur(var(--ztu-blur-glass))`（含 `-webkit-` 前綴）＋ `border: 1px solid var(--border)` ＋ `box-shadow: var(--shadow-card), var(--shadow-edge-top)`；hover 態陰影升級為 `--shadow-card-hover`。原本用 `inset box-shadow` 模擬邊框、無模糊、靜態無陰影，hover 才補一層 `--shadow-card`。巢狀的唯讀條款面板 `.brand-deal` 改吃 `--ztu-film`（薄膜，不上模糊，做法同 `.card--muted`），取代原本的 `--muted` 實色底。
...
- `.zcheck--lg`（**Q71 定案 2026-08-25**）— 24px 大尺寸變體，掛在包住 `.zcheck__control` 的外層容器上（不是控件本身）：`.zcheck--lg .zcheck__control { width/height: 24px }`。用於控件獨立佔一格、旁邊沒有緊貼標籤撐場面的位置（如清單列右側單獨的核取欄）——16px 在那種位置份量不夠。方框圓角同步升級 `--radius-sm`（3px）→`--radius`（6px）；勾維持元件預設尺寸 5×9 不變（框變大、勾不變，勾在框裡的留白才是對的），光學上移補償量隨框等比放大 −1.5px→−2px。demo-eshop-styles.html only as of 2026-08-25，待套用到實際消費頁。
...
**Anatomy**
- `.detail-sheet` — 根層＝**暗紗，不做模糊**（濃度旋鈕 `--sheet-veil`，現值 `--ztu-canvas` 60%）；留白 `--ztu-shell-gutter` 四邊等寬。整個視窗都糊掉的話面板就沒有東西可以透過去，玻璃感會消失。**紅線：這一層不可以動 `opacity`**——帶 opacity 的祖先會成為 backdrop root，面板的 `backdrop-filter` 會變成有宣告卻糊不到東西（2026-08-31 踩過）。入場淡入改動 `background-color`
- `.detail-sheet__panel` — **唯一做模糊的一層**：`--ztu-glass-bg` 玻璃底＋`--ztu-blur-heavy`（40px）＋四角 `--radius-shell`＋頂緣 `--ztu-glass-rim` 內光＋`--ztu-shadow-card` 投影（ztorUI 換裝 2026-08-31）
- `.detail-sheet__frame` — **整片透明**，面板的玻璃才透得到底下那一頁；配套是 `shared.css` 內嵌模式關掉檯面色、暖光與四層星空（否則 iframe 會把檯面再畫一次，浮層看起來像換了一張新背景而不是一塊玻璃）
- `.detail-sheet__head` / `.detail-sheet__back` — 帶「回哪裡」文字，不是孤零零的 ✕；頭列本身透明、分隔線用 `--border-soft`，與內容是同一片玻璃（與 `.app-topbar`／`.wizard__top` 同一套殼層語彙）
...
- `.vault-reach` — 「誰進得來」的讀數卡。**2026-09-01 撤除吸頂**（使用者裁示「這個不用 fix」）：原本 `position: sticky; top: --sp-16`，釘住的代價是它會蓋掉下一張卡的標題；而它為了補上下兩道縫，要用 `.is-stuck`（JS 掛）＋ `::before`／`::after` 各鋪一塊方角的**實色**頁面底色——那個手法的前提是「卡片不透明」，同日這張卡依 Wave 2a 規則改成玻璃面之後就露餡，變成卡片外圍一圈深色方框。兩件事一起解：不再吸頂，補縫的虛擬元素與 `js/media-vault.js` 的 `bindStuckWatch()` 一併退場（墓碑留在兩處原地）。**留下的教訓**：用實色補縫來遮 sticky 的接縫，等於把「這張卡永遠不透明」寫進實作；材質一改就壞。
...
- `.zstep:has(.zstep__input:disabled)` / `[readonly]` — 唯讀／停用時整欄隱藏按鈕（2026-09-22 起 `stepper.js` 的 `syncLimits` 對停用／唯讀欄也把兩顆鈕保持 `disabled`，重算極限時不再把鈕還原成可點外觀——bookyay 鎖定欄 D308）
...
**ztorUI 換裝（2026-08-28 Wave 2b）** — 底色 `--card` → `--popover`（深色下同為 `#212223`，值沒變、來源正名）；上緣陰影收進新增的 `--ztu-shadow-pop-up` token（`--ztu-shadow-pop` 的向上版，見 §1.1b），原本 §1.5b 記錄的裸值 `box-shadow: 0 -6px 16px rgba(0, 0, 0, 0.08)`（寫死的淺色模式值，深底上等於沒有）已收斂，**該筆裸值記錄已失效**。刻意**不**套玻璃四件套：這一列會做淡入淡出，而 `backdrop-filter` 表面不可淡入淡出（見 `sticky-actions.css` 檔頭）。圓角維持 0。
...
**View mode** — 坐在 `[data-mode="view"]` 面板裡時，`shared.css` 的「藏掉所有按鈕」白名單自 2026-09-11 起直接放行 `.zselect__trigger`（此前要逐顆 `<select data-view-safe>`，漏一顆就少一個欄位）；唯讀時觸發鈕跟著原生 select 一起 `disabled`，只顯示目前值。 **2026-09-21（Q119）**：檢視態的觸發鈕畫成純文字——`shared.css` 把 `[data-mode="view"] .zselect__trigger`／`.select` 的框、薄膜底、內距與 `.zselect__caret` 收掉，字級改 `.field-readout` 的 `--fs-14` ui 字體、前景色（不再是 disabled 淡墨），`pointer-events:none` 雙保險；觸發鈕上的字本來就跟著 select 的 MutationObserver 同步，換語言也不用另外處理。`[data-edit-exempt]` 區段排除。

**Dependencies** — `js/zselect.js`（執行期把 `<select class="select">` 升級成觸發鈕＋自繪 listbox；原生 select 留著照樣派發 `change`/`input`）；面板語彙抄自 `.dropdown__menu`（`--radius-lg`、`--shadow-float`；底色 2026-08-28 ztorUI 換裝後同步改為 `var(--popover)` ＋ `backdrop-filter: none` ＋ `border: 1px solid var(--ztu-glass-rim)`，原 `--card` 實色，見 §4.26／§3.2），非硬性 import。2026-07-28 修過 double-scroll-bar bug：`.zselect__native` 用 `position:absolute` 卻沒配 `top/left`，導致它停在文件流原始位置、把捲動容器撐高出一條看不見的捲軸；補上 `top:0;left:0` 解決，仍保持 `absolute`（非 `display:none`）以維持可提交／可量測。**2026-08-10 起原 `<select>` 的 `data-view-safe` 會一起搬到觸發鈕**：觸發鈕本體是 `<button>`，會被 `shared.css` 那條「`[data-mode="view"]` 底下藏掉所有按鈕」的規則掃到——那條規則針對的是動作鈕，表單控件在唯讀面板裡仍要看得見選到的值。所以坐在可切檢視／編輯的面板裡的 select 要自己標 `data-view-safe`（首個消費者：`project-detail.html` 公開資訊的年齡分級）；不標的維持原行為。 **2026-08-18 兩項**：(1) 支援選項圖示——原生 `<option>` 標 `data-icon` ＋ icon registry 的鍵，腳本在面板渲染與 `syncLabel()` 兩處各畫一次、進 DOM 後呼叫 `ztorIcons.applyIcons()`。收合與展開必須畫同一顆——選完之後圖示消失會讀成「選到的不是剛剛那一個」。圖示一律中性灰、不隨選取換色（選取由勾號與粗體承擔）。首個消費者：`create-project.html` 的內容類型（與閘門第二關那組磚同一份 `CATS`、同一顆圖示）。(2) 勾號移到列尾（全站行為，非單頁覆寫）。
...
**動作列（2026-09-11）** — `<select data-zselect-action="new-session" data-zselect-action-i18n="cp.delivery.session.new" data-zselect-action-icon="plus">`：面板最底下多一列 `.zselect__option--action`。不是選項——點了不改 value、不派 change，只派 `zselect:action`（bubbles，`detail.action`）並關閉面板，頁面自己接去開彈窗。鍵盤 ↑↓ 走得到、Enter 觸發；打字跳選不會跳到它。
...
- `.field__hint.tier-card__hint-new`（2026-08-11 新增）— 票種欄的「這個名字會建一個新票種」即時說明，疊在 `.field__hint` 上轉 `--foreground`（比 hint 的灰亮一階）。它報的是**狀態**（接下來會多出一個票種）而非一般欄位說明，讀者要看得出自己正在新建而不是沿用既有票種。**不用品牌橘**：橘在站上代表「已選中／進行中」，這裡只是陳述後果。權重同樣寫成 0,2,0——`field-system.css` 在本檔之後載入，單一 class 會輸給 `.field__hint`
...
**單張門票彈窗改單欄四區塊（2026-09-22，D311）** — 彈窗殼仍是 `.payout-modal`／`.payout-dialog`（Q27），但內容整個重排：**分頁（設定／購買條件）退場**，改成四個 `.form-section--dense`——基本（門票名稱＋縮成一格的門票圖片）／價格與數量（`.calc-list--form` 算式列表：票價 ＝ 粉絲付 − 平台費 ＝ 創作者實收，數量刻意排在列表之外；2026-09-23 D316 收掉「＋ 手續費」那一列，平台費與支付費一律由創作者負擔）／入場（提前入場分鐘數＋「跟隨場次／自訂」二選一）／購買條件（區塊標題右側一顆「跟隨活動預設」開關：開＝三組規則的繼承值摘要三列，關＝`rulesHTML` 攤開可改，copy-on-write 不變）。標題列多一行副標（場次日期・場地），頁尾左側是 `.payout-dialog__foot-note`「N 個欄位待修正」。**2026-09-23（Q125）區塊標題的分量重排**：「基本」不畫標題列（彈窗標題與副標已經是脈絡，門票名稱與門票圖片兩格彼此無關、各自的標籤也說完了）、「入場」改 `.form-section__title--quiet` 輕標題（這一層沒有區塊級控制項），「價格與數量」（算式單元＋bookyay 徽章）與「購買條件」（跟隨開關）維持正常那一階；`event-detail.html` 的票種彈窗同一套判斷，第四個區塊「販售」有暫停開關、維持正常。同輪把彈窗內的節奏收成一把梯子（6／16／12／32／24），見 Pillar 4 的 Form section 條目。

- **平台費那一列是純顯示（2026-09-23，D315 撤銷 D311 的例外平台費）** — 這一列標所屬葉節點、費率與來源（「平台預設」或「這位創作者的覆寫」兩種），creator 與 Admin 看到的是同一個唯讀列，門票彈窗沒有任何編輯入口；費率的唯一設定處是 Admin 的平台費率設定頁（5.1.0.3）。費率兩層解析（creator 覆寫 → General）住在 `js/platform-fees.js`，葉節點名稱共用費率設定頁的 `fees.dim.*`／`fees.leaf.*`，不另抄一份中文字串。🪦 D311 的例外平台費開關、費率 % 欄與 `.calc-list__extra` 一併退場。
- **bookyay 帶入的票** — 價格與數量整段改唯讀事實列（`.calc-list` 不帶 `--form`）＋區塊標題一枚「bookyay 帶入」徽章，不再逐欄掛鎖徽章與清空鈕（D308 同一口徑）。
- 🪦 tombstone：`ruleHeadHTML`／`syncModalRuleHead`（分頁頂端那一行提示＋「改回跟隨活動預設」按鈕）、`grossFieldHTML`（「總價」唯讀欄——它把算出來的數字做成看起來能填的欄位，又排在票價之前，加法關係完全看不見）、`modalTab`。
- `event-detail.html` 的票種彈窗（D311 決定六）同一套結構：基本／價格與數量（同一組算式列）／入場／販售（暫停販售）。**購買條件在詳情頁沒有**——發布後改購買條件的規則上游未定（2026-08-11 裁決未變），所以最後一個區塊是「販售」。

**Consumers** — `create-event.html`（spec 5.1.6.1 步驟 4／5）、`event-detail.html`（票務分頁的票種彈窗）。
...
**Purpose** — 兩個維度都能被使用者增減的資料表（spec 5.1.5.5 F7.2 · D171）。用在商店設定的商店尺寸指南與建立商品的商品專屬指南（同一支編輯器，D211）：列是尺碼、欄是量測項，而「這件衣服要量哪幾個部位」本來就因商品而異，所以欄不能寫死。站上其他表格的欄位都是已知固定集合，這是第一支欄數由使用者決定的表。

**Q112（2026-09-11）**：`--radius-xl` 外框內的列左右內距下限 `--sp-16`（首欄文字不壓在圓弧上；右側 icon 欄不動）。
...
**`.sce-block__tabs`（2026-08-21 · D214）** — 尺碼制（國際／US／EU／JP）是同一張尺寸表的四個分頁，不是四張表：一列＝一個實體尺碼，四種標示法只是它的四個名字，所以每一制各存一份**尺碼欄**、量測值四制共用。分頁沿用 `.tabs`（`--count-plain`），計數欄位借來標「這一制還有幾格沒填」；`.tabs` 預設的 20px 下外距在這裡歸零，間距改由下方那句說明承接。切換與逐制資料在 `partials/size-guide-modal.js`。

**`.sce-block__head`（2026-08-20 · D212）** — 子塊抬頭：左邊標題＋說明，右邊放屬於這張表的控制項。目前唯一的用例是尺寸表右上的量測單位（公分／英吋）——那兩個值是絕對換算，切換是「換一種看法」而不是另外填一份資料，所以它跟著表走、不是一條獨立設定列。換算以公分為真值（`cell.dataset.cm`），畫面依單位算出來顯示到小數一位；不拿畫面數字反覆相乘，來回切兩次就會四捨五入漂掉。尺碼制（國際／US／EU／JP）不適用這個做法——各國尺碼是慣例對照、不是換算公式，它留在上面當設定列。

**`.sce-block`（2026-08-20 由 store-settings.css 搬來）** — 編輯器裡各子塊之間的呼吸（尺寸表／身高體重／量測方式／版型建議／免責）：上下 20px 內距＋一條 `--border-soft` 上分隔線，不各自畫框（Q32）。彈窗裡第一個子塊不畫上線（head 已經有一條）。原名 `.ss-spec-block`、住在 `store-settings.css`；編輯器抽成共用 partial 之後建立商品也會用到它，只住在商店設定那一支就不成立了。

**指南切換（2026-08-21 · D216）** — 從建立商品的「點擊查看」開進來時，彈窗頂端多一列「看哪一份」（下拉 `.select`＋`.sg-picker__select`，`opts.guides` 有幾份就列幾份——份數會長，segmented 撐不住），每一份的整份表單各自快照、換回來原封不動；該模式下名稱欄不出現（切換列已經是識別）。從商店設定點單一份進來時不帶 `guides`，行為不變。

**編輯器是共用 partial（2026-08-20 · D211）** — `partials/size-guide-modal.js` 持有整個彈窗的 markup 與互動（`window.ztorSizeGuide.open()`／`.close()`），兩個地方在編同一張表：商店設定的商店尺寸指南、建立商品的商品專屬指南。差別只有一個參數 `showName`——商品專屬那份沒有名字（它只屬於那一件商品）。彈窗殼沿用 `.payout-modal`／`.payout-dialog--wide`。

**Dependencies** — `.input`（格子與欄名）、`.ztor-icon`、`.payout-modal`／`.payout-dialog--wide`（彈窗殼）、`.spec-row`（量測方式逐列）。增刪列／欄、`--sce-cols` 同步與欄名改動時的 aria-label 同步由 `partials/size-guide-modal.js` 處理（2026-08-20 起，此前是 `store-settings.html` 的行內控制器）。消費頁：`store-settings.html`（spec 5.1.5.5 F7.2）、`create-product.html`（spec 5.1.5.2 §4.1 F2 · D211）。
...
**Purpose** — 一個活動辦好幾場時（使用者說法「系列活動」），建立流程要能一場一場列出來，清單頁又不能因此爆量。兩件事分兩個地方做：**建立時**用 `.session-list` 逐場填日期與場地；**清單上**用 Row disclosure（§4.204）把整個系列收成一列、點開才展開。
...
**Anatomy（清單上的群組列＝Row disclosure §4.204，2026-09-04 改 L3 整組換層）**
- 群組容器 — `.rowdis__group` 包住母列＋所有子列（表格型不加 `--gutter`：`.product-list__row` 自帶左右內距，面板邊緣與資料列外緣重合就是對的）
- 母列 — `.product-list__row.rowdis__head[data-rowdis-row]`（整列可點）；展開把手 `.rowdis__toggle[data-rowdis][aria-expanded]` **放在名稱文字之後、同一格 `.product-list__title` 內**，那一格再掛 `.rowdis__label` 讓文字吃剩下的寬度並自己截斷、按鈕固定寬不縮
- 子列 — `.product-list__row.rowdis__child[data-rowdis-child][hidden]`；零縮排，欄位與表頭同一像素對齊，階層改由展開時 `.rowdis__group` 那塊 L2 薄膜面板說
- 互動（點擊、鍵盤、子列顯隱）走 `js/row-disclosure.js`，本頁不自寫 toggle

**2026-08-06 到 2026-09-03 之間這件事住在 `product-list.css`**（`__row--group`／`__row--child`／`__group-toggle`／`__group-chevron`）。同期 `data-list.css` 也長了一支自己的 `__row--child`，同一個視覺角色兩個答案；`STYLE-DECISIONS.md` Q107 裁決「要有新的」之後兩組一起退場、CSS 原處留墓碑，改由 §4.204 這一支承接。**`product-list` 仍擁有欄位版面**（grid 欄、列高、hover），揭露語意與階層視覺歸新元件。

**展開把手為什麼不放最右的操作欄** — 清單頁的 `.main` 是 `overflow-x: hidden`，視窗窄一點時右側幾欄會被切掉且捲不到（既有問題，不只影響本元件）。把手收在最右的操作欄等於沒有入口；樹狀清單的展開三角本來也長在最前面。

**把手最終落在名稱之後、同一格內（2026-08-18，使用者指示，6 處統一調整）** — 最初做法是把手放在名稱最前面、母列的 `.product-list__product` 整體改 flex（把手＋內文）。但這樣把手與縮圖之間多占一欄，群組列的名稱會比其他列晚一格起跑，整張清單的左緣因此斷掉。改成把手收進 `.product-list__title` 內、跟在文字後面——名稱那一格本身是單行截斷，所以只讓「有把手的那幾列」的 `.product-list__title` 變成一條 flex，其餘列不受影響。
...
**群組列的第二個消費場景：取貨名單（2026-09-03／D240）** — 取貨場次詳情（`pickup-detail.html`）的名單改成一列＝一個領取單位之後，買 5 件的買家會佔 5 列。以買家分組收斂：母列＝買家（列首寫「本場次已領 M／N」），子列＝那位買家的每一件領取單位；只買一件的買家不分組、直接一列。**沿用本節既有的 `__row--group`／`__row--child`，不另開第二種分組列做法**——站上分組列因此只有一種答案，`STYLE-DECISIONS.md` 不需要新增待裁決題。行為與活動清單一致，只多一條：**篩了狀態也一律攤開**（篩「待核銷」時收著的母列等於看不到還沒領的是哪幾件）。

**Dependencies** — `.field`／`.input`（場次欄位）、`.btn`（`--outline`／`--icon`）、`.badge`、`.switch`（開關）、`.form-section--outlined`＋`__head--actions`（區段外框與標題列開關）、`.ztor-icon`。無獨立 JS 檔；場次資料、序號重編與收入試算的場次倍數由 `create-event.html` 的行內控制器處理，清單的展開／收合由 `events.html`（系列場次）與 `pickup-detail.html`（取貨名單的買家分組）各自的行內控制器處理。
...
- 取消系列中的一場＝與取消一場獨立活動完全相同的作廢規則（F5），屬於系列不改變對該場持票人的義務。
...
- `__head` ＞ `__thumb`（選配封面縮圖，40px 寬、`aspect-ratio: 2/3` ＝全站直式標準）＋ `.badge`（狀態）＋ `__title`
...
**選配的封面縮圖（2026-08-11）** — `__thumb` 只在項目詳情的作品區段（spec 5.1.2.2 §2.2.11）用：那一區要先讓創作者認出「這是哪一件作品」再讀狀態。更新區段那一塊不放，因為它講的是「那則貼文」、識別已由所在區段交代。沒有這個元素的既有消費者完全不受影響。

**Consumers** — `project-detail.html`（項目更新區段的作品審核狀態、作品區段的作品識別與編輯入口）、`admin-video-review.html`（送審件頁首與審核動作）。產品缺口見 ASSUMPTIONS UIA-109／UIA-111／WEDIT-001。
...
**已經帶過的那一場：列出來但選不了（2026-08-13，使用者指示）** — 同一場 bookyay 活動在 ztor 上只會存在一次，帶過就不能再帶。清單裡仍然列出它、`<button>` 上加 `disabled` ＋ `aria-disabled`（灰掉的樣式由 owner-lookup.css 的 `[disabled]` 接手，不必另外加 class），右邊掛一顆 `.badge.badge--neutral.owner-lookup__flag`「已匯入」說明為什麼點不動。刻意不從結果裡濾掉——使用者搜尋的目的常常正是「確認這場我是不是已經建過了」，濾掉會回答成「找不到相符的活動」，那是錯的訊息。

**帶入後直接落在「票務商品」那一步（2026-08-13，使用者指示）** — 帶入之後的基本資料、場次、票種、票務四步整份由 bookyay 決定、也整份鎖住，一步一步按過去等於翻四頁唯讀畫面；票務商品是帶入後第一件真的要創作者決定的事，落點就放這裡。被跳過的四步在進度列上照樣標「已完成」（進度列以「編號小於當前步」判定完成，不必另外記帳），要回頭看內容也照樣點得進去。
...
- `.quick-result__row` — grid 兩行（第一行：名稱跨滿整列；第二行：金額／數量／標籤／動作）
- `.quick-result__name` — 名稱，超長截字尾
- `.quick-result__num` — 出現兩次（金額、數量），等寬數字
- `.quick-result__tag` — 「跟隨活動預設」或「已自訂」說明；`.quick-result__tag--own` 加深顏色標出已自訂（不會跟著活動預設走的那幾張要看得出來）
- `.quick-result__end`（選配）— 重設鈕

**兩行一列（2026-08-10 改版）** — 原本是一列五欄（`1fr 88 64 148 108`，扣掉間距至少要 470px）。票務設定彈窗改成左右分欄之後，這份清單住進 360px 的側欄，五欄橫排會把每一欄都壓成一兩個字。改成名稱獨佔第一行、金額·數量·標籤·動作排第二行；欄位靠 grid 自動流排，**markup 一行都不用改**（名稱 `grid-column: 1 / -1` 跨滿整列，後面四個依序落位）。數量同輪由裸數字改成帶單位（「400 張」）——橫排時靠欄位位置就讀得出那是張數，直排之後位置線索消失，數字得自己說明自己。原本 ≤720px 的斷點一併撤除：兩行一列本身就是窄版面的排法，不必再換一次。
...
**Purpose** — 一顆圓、裡面一個字：這個人是誰。沒有真實頭像檔的原型階段，姓名首字就是識別。2026-09-02 promote——同一個做法此前被複製三次（`creators.html` 的 `.creator-ava` 40px 與 `.creator-ava--lg` 64px、`creator-detail.html` 的 `.cd-ava` 64px），三份的底色配方 `color-mix(in srgb, var(--primary) 16%, var(--muted))` 一字不差，差別只有尺寸；創作者活動管理頁是第四次要用，照「第一次出現就 promote」補建，同輪把前兩頁一併換過來，站上只剩這一份。

**`_layer`** · atom

**Anatomy**
- `.ztor-avatar` — 預設 40px（名冊列）
- `.ztor-avatar--sm` — 32px（密度優先的小卡，如 roster-picker 的 creator 卡）；字級退到 `--fs-13`。2026-09-02 加：那張卡同日壓成「一列身分 ＋ 一列小標籤」的兩列量體，40px 的圓在 70px 高的卡上會變成卡面最大的東西，識別圓喧賓奪主
- `.ztor-avatar--lg` — 64px（詳情頁頁首、建檔表單）；字級同步升到 `--fs-22`，否則 64px 的圓裡浮著一個 14px 的字，讀起來是「圓沒填滿」

**與 `.admin-table__avatar` 的分工** — 那一支是 admin 表格列內的袖珍版（28px、吃 `--accent` 中性底），屬 `admin-ip-bank-table.css` 自己的一部分；本支是通用的、吃品牌 tint。尺寸與底色都不同，不是同一個視覺角色，不合併。

**Surface 層** — `--card` 玻璃面、`--background` 畫布與 `--muted` 三層都成立（底是品牌 tint 混 `--muted`，與三層都有色相差），不需要邊框。
...
**為什麼不再叫「主從兩欄」** — 上午那一版是左 340px 名單、右內容；340px、左欄吸附、左欄自捲、窄畫面折疊成一顆按鈕，這些理由全部綁在「兩欄」上。改成上下堆疊之後它們一條都不成立，名字留著只會讓下一個人照著兩欄的假設用它。

**該用哪一支** — 「左欄選的是**同一個東西的哪一段**」→ `.section-nav-layout`（§4.22h0，140px、短標籤）／設定頁的 `.settings-layout`；「要選的是**哪一個對象**，而且每個對象要帶頭像、名稱與幾個進度數字」→ 本支。後者塞不進 140px 的側欄，橫著排成卡片比擠成一條窄列好讀。

**Anatomy**
```
.roster-picker                 直排容器（節奏由這一層的 gap 持有）
 ├ .roster-picker__pick        上段：選人區（同樣 gap 持有節奏）
 │   ├ .roster-picker__bar     工作列（搜尋＋篩選標籤，可換行）
 │   │   └ .roster-picker__search   搜尋（與 .field-pill 同掛，只加寬度規則）
 │   ├ .roster-picker__grid    卡片網格 repeat(auto-fill, minmax(260px, 1fr))
 │   │   └ .roster-picker__card     一張卡＝一個對象（與 .selection-card 同掛）
 │   │       ├ .roster-picker__card-media  32px 識別槽（放 .ztor-avatar--sm）
 │   │       └ .roster-picker__card-text   右欄：名稱在上、狀態列在下
 │   │           ├ .roster-picker__card-name
 │   │           └ .roster-picker__card-status  狀態列（二選一）
 │   │               └ .roster-picker__card-stats  數字（與 .stat-row--inline 同掛）
 │   │                  或 .badge --warning「未綁定」
 │   └ .roster-picker__nomatch  「沒有符合的對象」
 └ .roster-picker__body        下段：選定那一位的內容
     └ .roster-picker__detail  選定者那一段
         └ .roster-picker__head  標題列 → __head-text（__head-title／__head-meta）／__head-actions
             └ .roster-picker__head-actions  動作組（可整組 [hidden]）
                 └ .roster-picker__head-sync  動作前面的一行狀態字（如「最後檢查 …」）
```

**已選態＝借用，不是新畫一種** — 卡片本體同時掛 `.selection-card`，卡面、圓角、陰影、hover 與已選態全部由那一支提供：已選是 **1px 橘色 outline ＋ 中性陰影**（2026-06-24／06-25 使用者裁示：陰影不帶橘色、橘色是線匡）。本支不寫任何 `--active` 規則，站上「可選取的卡被選中」維持單一答案。
...
- **可匯入不上卡**（2026-09-02 第三輪）→ **2026-09-02 D238 整個值退場**：該輪的說法是「它留在下段摘要與『有待匯入』篩選裡」；同日改自動匯入之後，那兩處也一併撤除——綁定後活動自動搬進來，沒有「還沒搬進來」的場次可以數。卡上的兩個數字（已匯入／待設定）不受影響。
- **0 值** — 已綁定但兩欄都是 0 是真的查過的結果，照樣顯示；`.stat__value--zero` 讓值與欄名一起退到 `--faint-ink`。

卡因此**仍然等高**：徽章比一行小標籤高 6px，交給網格預設的 stretch 拉齊——一排卡掃過去，眼睛比的是那一行寫了什麼，不是輪廓差 6px。

**撤掉的資訊（2026-09-02 前兩輪）** — 店鋪識別（`__card-meta`，仍可搜尋、只是不顯示）與最後匯入時間（`__card-last`，上游同輪已從規格 5.1.0.6 F2 移除，D235）。

**數字為什麼不畫成一條進度條** — 可匯入 ＋ 已匯入 ＝ 池總數、待設定是已匯入的子集，三者不是同一個整體的三份；分段進度條或圓餅會宣稱一個不存在的整體。

**名稱不用 `.selection-card__title`** — 刻意不掛它的 class：`.selection-card__title` 在 shared.css 的「標題一律大寫」名單裡，而卡上放的是一個人的名字（資料，不是區段標題）。大寫會把 Denise Lonely 喊成 DENISE LONELY，也跟下段標題列的同一個名字對不上。
...
**Surface 層** — 卡直接坐 `--background` 畫布（`.selection-card` 自帶玻璃面＋陰影；再包一張 `.card` 會變成玻璃疊玻璃、卡的邊界糊掉）。工作列的 `.field-pill` 與 `.filter-tabs` 也是坐畫布的元件。

**Token usage** — 間距全走 `--sp-*`、字級 `--fs-13/15/20`、色彩 `--border`／`--muted-foreground`／`--foreground`、字體 `--font-display`。無裸值；卡面色彩與陰影由 `.selection-card` 提供，卡上數字的字級與色彩由 `.stat-row--inline` 提供。
...
### 4.204 Row disclosure（可展開子列）

**Purpose** — 一筆清單項目其實是好幾筆時：母列收成一列，底下每一筆點開才出現。站上「可展開子列」這個視覺角色的**單一答案**。2026-09-03 依 `STYLE-DECISIONS.md` Q107 的裁決「要有新的」建立，取代此前並存的兩份實作——表格型清單的 `product-list.css` `__row--group`／`__row--child`（2026-08-06 為活動系列場次而建）與清單型的 `data-list.css` `__row--child`（源自專案詳情的收支小計展開列）。**2026-09-04 改寫成 L3「整組換層」**：使用者從探索頁 `lab-row-layers.html` 的五個候選裡挑定這一版，階層不再靠縮排與左緣線說，改由「這一組坐在哪一層平面上」說——收合時整列與其他資料列同層，展開時母列與它的子項一起被包進一塊 L2 薄膜面板，子項欄位與表頭嚴格對齊、零縮排。

**`_layer`** · molecule ｜ **source tier** · App ｜ **surface 層** · 收合時無樣式、與容器同層；展開時成 L2 薄膜面板（`--ztu-film` 底、`--border-soft` 描邊）；第二層（見下方 `.rowdis__frame`）用 1px `--border` 框，不再疊第三層填色

**它擁有揭露與階層那塊面，不擁有欄位** — 歸它的是三件事：把手的位置與轉向、`aria-expanded` 狀態、以及「這一組展開時坐在哪一層面」（收合同層 vs. 展開後的 L2 面板／第二層的框）。欄寬、欄數與每一格的內容仍歸它疊上去的那個容器（`.product-list__row` 的 grid 欄，或 `.data-list__row-main`／`__amount`）。所以它是與容器列**同時掛**的一層 class，不是另一種清單——這也是它能同時服務兩種容器骨架、而不必把兩套欄位模板合併的原因。

**Anatomy**
- `.rowdis__group` — 群組容器，包住「母列＋它的子項」（新增這一層 wrapper 是 L3 模型的核心）。收合無樣式；展開時（`:has()` 偵測內部把手 `aria-expanded="true"`）成為 L2 薄膜面板——`--ztu-film` 底、四周一圈 inset box-shadow 畫的 1px `--border-soft`、`--radius-lg` 圓角，面板內部的列底線轉透明。**間距**（2026-09-04 使用者於 `lab-rowdis-spacing.html` 調參定案）：`padding-block: --sp-12`、`margin-block: --sp-8`；左右一律 `padding-inline` 搭配等值負 `margin-inline`，讓面板往外長而列的 x 位置不動——表格型 `--sp-8`（疊上列自帶的 `--sp-10`，內容距面板邊 18px），清單型走 `--gutter` 的 `--sp-16`（列本身無內距，一次補到位）。兩者都必須大於 `--radius-lg` 的 16px 半徑，否則左上角的標題會卡進圓角的弧線裡
  - `--gutter`（清單型容器要加）— `.data-list__row` 本身左右無內距，由這個變體把左右內距**覆寫**成 `--sp-16`（不是疊加），讓內容距面板邊 16px、與表格型的 18px 視覺等距；一樣是負外距往外長、等值內距把列推回原位，欄位起點完全不動。表格型（`.product-list__row` 自帶 `--sp-10`）吃基準的 `--sp-8` 就夠
  - 捲動容器補償 — 表格型的清單包在 `.product-list-scroll`（`overflow-x: auto`）裡，面板往外長的 8px 會被它剪掉（左側溢出在 LTR 捲不到，圓角被切平）。`row-disclosure.css` 因此帶一條 `.product-list-scroll:has(.rowdis__group)` 把捲動容器本身往外撐同樣的 8px。寫在本元件而不是 `product-list.css`：這是本元件的出血造成的需求，跟著它一起維護、一起退場
  - `--static`（唯讀群組用）— 面板恆亮，不看把手的 `aria-expanded`。給「名單不可展開、單位一律攤開，但『這幾件屬於同一買家』要跟可展開的清單一樣清楚」的情境（scanner 的 F3 名單）
- `.rowdis__head` — 母列，須是 `.rowdis__group` 的直接子節點（`:has(> .rowdis__head …)` 這樣寫）；帶 `data-rowdis-row` 時整列可點（此時才給 `cursor: pointer`），只有把手可點的用法游標歸按鈕本身
- `.rowdis__lead` — **列首欄**（2026-09-04 · Q109）：列最前緣的一個窄格，寬 `--rowdis-lead-w`（`--sp-28`，等於 `.btn--icon.btn--xs` 的邊長）。母列在這裡放把手、子列放它在該組內的序號、沒有群組的一般列留空。詳見下方「列首欄」
- `.rowdis__num` — 子列的序號，純文字、不可聚焦；`--fs-12`／`--column-head-ink`／等寬數字，比照表頭欄名的份量
- `.rowdis__label`（選配，**2026-09-04 起正式頁面已不消費**）— 把手與「單行截斷的標題」並排的那一格。把手改站列首欄之後，標題格裡沒有要防截斷的並排元素，五個消費頁與 DS demo 都已移除。CSS 規則留著只為了凍結保存的探索頁 `lab-row-hover.html`；那批探索頁清理時本規則跟著退場。新頁面不要再用它
- `.rowdis__toggle` — 把手；外觀吃 `.btn.btn--icon`（尺寸由頁面決定），本支只加「不被壓縮」與「隨 `aria-expanded` 轉 180°」
- `.rowdis__child` — 子列；零縮排，只退一階字級與文字色（`--fs-13`／`--foreground-muted`），自帶 `[hidden] { display: none }`
- **子列不重複母列已經說過的欄位**（2026-09-04）— 群組列已經標了買家與來源，子列的同名欄位若與母列相同就留空，只有**不同**時才印出來。取貨名單的實例：同一位買家在同一場次可能同時有商品的取貨資格與活動票券（規格 5.1.5.15 F4.1「混合來源」），那時候每一件的來源才真的不一樣、值得逐列標。留空不影響搜尋——搜尋吃的是列上的 `data-*` 屬性，不是畫面上的文字。
- `.rowdis__frame` — 第二層的框（見下方「為什麼第二層是框」），1px `--border`，同樣用負外距＋等值內距讓框比裡面的列寬一點、列的 x 位置不動

**Variants**
- `.rowdis__group--gutter` — 見上方 Anatomy；清單型容器加、表格型不加
- `.rowdis__group--static` — 見上方 Anatomy；唯讀群組加
- `.rowdis__child--compact` — 清單型容器的密度：上下留白收窄（`--sp-8`）。⚠ 這個變體現在**只管密度、不管階層**——列線與組尾線都已改由 `.rowdis__group` 那塊面承擔，不是它的職責

**States** — 收合／展開（`aria-expanded` 在把手上）驅動 `.rowdis__group`／`.rowdis__frame` 亮不亮那塊面，純 CSS `:has()`，不經過 JS。**面板亮著時，母列與子列的 hover 改由本支定義**（見下方「面板內 hover」），取代容器（`.product-list__row`／`.data-list__row`）原本提供的通用 hover；面板外的列與收合狀態的母列仍吃容器提供的通用 hover，本支不介入。focus-visible 由 `.btn` 提供，本支不另定義。

**Token usage** — `--sp-28`（列首欄寬，經 `--rowdis-lead-w`）／`--fs-12`／`--column-head-ink`（子列序號）／`--sp-4`（把手與標題的間距）／`--sp-6`（`.rowdis__frame` 的負外距＋內距）／`--sp-8`（compact 的上下留白、`.rowdis__group` 的上下留白）／`--sp-10`（`--gutter` 的負外距＋內距，對齊表格型容器的列內距）／`--fs-13`（子列字級）／`--foreground-muted`（子列文字）／`--ztu-film`（展開面板底色）／`--ztu-glass-strong`（面板內列 hover 底色）／`--border-soft`（面板描邊）／`--border`（第二層框）／`--radius-lg`（面板圓角）／`--radius-md`（第二層框圓角）／`--duration`／`--easing`（把手轉向、面板內動作選單 opacity 過渡）。無裸值。

**列首欄：展開在最前緣、`⋯` 在最末端（2026-09-04 · Q109，指名參考 Braintrust）** — 此前把手貼在標題文字後面（列的中段），與列尾的 `⋯` 動作選單讀不出「這是同一列的兩個控制項」，而且每一組的標題長度不同時把手的 x 位置跟著浮動，一整欄的把手站不成一條線。使用者看過 Mobbin 上八個實例（Midday／Evernote／Xero／Salesforce／Braintrust／ClickUp／Asana／Zoho CRM）後裁定改採業界一致的做法：**展開控制在列的最前緣、`⋯` 在最末端，兩者分踞兩端、從不相鄰**。

- **這一欄從來不空著，也從來不是「有分組才長出來」** — 同一張表裡的每一列都有它，包括沒有群組的一般列（Braintrust 亦然）。所以它是欄位版面的一部分：**表頭也要跟著多一欄**，否則表頭與資料列會差一格。
- **三種內容** — 母列放 `.rowdis__toggle`（chevron）／子列放 `.rowdis__num`（它在該組內的序號，1 起算）／一般列留空。
- **寬度歸元件** — `row-disclosure.css` 在 `:root` 定義 `--rowdis-lead-w: var(--sp-28)`，消費頁的 `grid-template-columns` 與格子本身都引用它，兩邊不各寫一次數值。28px 不是隨手取的：它等於 `--control-h-xs`（`.btn--icon.btn--xs` 的邊長），欄寬剛好包住把手。變數掛 `:root` 是因為軌道寫在消費頁的 grid 容器上，那個容器不在 `.rowdis__lead` 的祖先鏈上讀不到元件自己的 class（同一手法見 `preview-panel.css` 的 `--preview-w`）。
- **序號是資料、不是控制項** — 純文字、`tabIndex` -1，鍵盤 Tab 不會停在上面；這一欄唯一的可聚焦元素是母列的把手。
- **窄框（≤760px）不藏這一欄** — 把手是鍵盤使用者唯一的展開入口（整列可點只服務指標與觸控），所以堆疊版面裡它仍釘在第一列第一格，其餘欄位往右順移一軌。
- **群組列的其餘欄位大多留空，只在標題下方留一個彙總**（「1 of 5 collected」「3 dates」）——照 Braintrust 的做法，群組列講整組的事，逐筆的值歸子列。

**只在表格型容器套用（2026-09-04 判斷）** — `pickup-detail.html`（名單）與 `events.html`（系列場次）兩個表格型消費頁已導入。三個清單型消費頁維持把手現況，理由各自不同：`order-detail.html` 有兩層展開，組合成員那一列同時是第一層的子列（該有序號）與第二層框的母列（該有把手），一個列首格放不下兩種角色，Braintrust 的單層模型沒有答案；`project-detail.html` 的列首已經是 ＋／−／＝ 的記號晶片（那一列的身分標記），而把手貼著金額擺才讀得出「這個數字展開成它的組成」；`scanner.html` 的名單是 `--static` 唯讀恆亮、根本沒有把手可搬，列首同樣被記號晶片佔著。三者都不是欄格線結構、也沒有表頭可對齊，硬套只會多一欄空格。

**為什麼一圈邊用 inset box-shadow 畫、不用 border** — `border` 會佔版位，母列與面板外的資料列會因此差 1px、欄位對不齊；`box-shadow` 疊在既有版面上不吃版位，欄位起點因此一格都不動。

**為什麼面板內部的列底線轉透明** — 邊界已經由那塊面在說，裡面再畫幾條線就變成兩套說法。用後代選擇器（不是 `>` 直屬）才涵蓋第二層框裡的列，限定在 `.rowdis__head`／`.rowdis__child` 兩個角色上、不誤傷列裡其他有邊框的東西（例如 `.data-list__icon` 的 1px 晶片框）。

**為什麼第二層是框、不是第三層填色** — 縮排退場後第二層需要另一個訊號。依既有規則辦：`ds-components/nest.css` 檔頭明文「只有兩層填色（L1 卡片 `--card`／L2 薄膜），第三層以後不再疊填色、改用 1px 邊框表達層級」。`order-detail.html` 的「訂單品項 → 組合成員 → 成員各自的領取單位」正好落在這一格——不是新規則，是那條既有規則在這個情境的套用。

**為什麼狀態放在按鈕上而不是母列上** — 按鈕才是可聚焦、可用鍵盤操作的那個元素；`aria-expanded` 放在它身上，Enter 與 Space 由原生 `<button>` 免費提供，讀屏也讀得到狀態。母列只是點擊區域的擴大。

**面板內 hover（H1 提亮一階＋H4 動作浮現，2026-09-04）** — 面板亮著時（展開，或 `--static` 恆亮），母列與子列的 hover 改吃本支的規則，取代容器（`.product-list__row`／`.data-list__row`）原本提供的「浮起玻璃卡」通用 hover（`STYLE-DECISIONS.md` Q34）。這是 Q34 的範圍例外，見 Q108；面板外的列、以及收合狀態的母列，仍吃 Q34 的通用規則、完全不變。

- **H1 提亮一階** — hover 時底色在面板底 `--ztu-film` 之上再提亮一階（`--ztu-glass-strong`）。不浮起、不模糊、不加陰影；`--gutter` 的清單型列自 2026-09-11 起有自己的內距與圓角（列往外長 `--sp-8` 再用等值內距推回、`--radius-md`，文字 x 不動）——使用者指出亮面「沒有 padding 也沒有圓角」像一條貼邊的方塊，要跟面板的內距與圓角一起看——同一平面上的暫時強調，並明確中和容器 hover 帶來的 `position:relative`／`backdrop-filter`／`box-shadow`。
- **H4 動作浮現** — 同一個 hover 讓該列右側的動作選單（⋮ kebab，`.dropdown` 元件）由 `opacity: .5` 提到 `1`；`:focus-within`（掛在 `.dropdown` 本身）與 `[open]` 也同樣不透明，鍵盤 Tab 到選單或選單已展開時都看得見，不是只有滑鼠使用者才看到。
- **母列子列同一階** — 兩者吃同一條規則，不分強弱。母列的可點性已由手型游標與 chevron 交代，用亮度差分強弱既讀不出來，又要多發明一個「半階」新值、多一條要維護的規則。

**為什麼面板內不能沿用通用的浮起玻璃卡（三個矛盾）**
1. **方向反了** — 玻璃卡底色 `--ztu-glass-bg`（5% 白）比面板底 `--ztu-film`（8.6% 白）更暗，hover 讀起來是往下沉、不是浮起，違反 Surface-Layer Contrast「淺灰填只在比自己亮的底上成立」。
2. **模糊沒有意義** — `backdrop-filter` 在面板內只會取樣到面板自己（同一教訓 `ds-components/nest.css` 檔頭已記過一次）。
3. **語意打架** — 一列浮出面板，跟「這一組是同一個平面」矛盾。

探索過程見 `lab-row-hover.html`（基準＋ H1–H5 六個候選）；裁決紀錄見 `STYLE-DECISIONS.md` Q108。

**✝ 退場（2026-09-04，縮排模型整組退場）** — `--rowdis-indent` 變數、`.rowdis__child--level-2`／`--level-3` 兩個縮排刻度變體、`.rowdis__child` 的 `padding-inline-start`、`.rowdis__child::before` 的 2px 左緣階層線、`--compact` 底下原本承擔階層的 `border-bottom: 0` 與組尾線，一併退場，CSS 原處留墓碑。退場原因：縮排把子項的欄位推離表頭，欄位對不齊；左緣線與組尾線是「用幾條線拼出一個群組」，邊界因此永遠是暗示而非事實。連帶：`shared.css` 的 `.pd-deep` 曾覆寫 `--rowdis-indent` 對齊 44px 文字起點，隨本次一併移除（零縮排之後沒有要對齊的東西）。依據：`STYLE-DECISIONS.md` Q107（元件的存在）＋ 2026-09-04 L3 裁決（本次的模型）。

**JS module — `js/row-disclosure.js`**

- **用途** — 展開／收合的互動。此前三個頁面各寫一份（`pickup-detail.html` 的 `data-group-toggle`、`order-detail.html` 的 `data-od-toggle`、`scanner.html` 的頁內函式），收斂後消費頁只寫 markup。面板本身純 CSS `:has()` 驅動，不經過這支 JS——JS 只切 `aria-expanded` 與子列的 `hidden`。
- **對外契約（data-attribute）** — `data-rowdis="<gid>"` 掛在把手（必須是 `<button>`，自帶 `aria-expanded`）；`data-rowdis-child="<gid>"` 掛在每一條子列（初始態由 markup 的 `hidden` 決定）；`data-rowdis-row`（選配）掛在母列＝整列可點，列內的連結、選單與其他按鈕一律放行。
- **對外契約（API）** — `window.rowDisclosure.init(root, { onToggle })`（登記容器與 callback，給「展開只是可見性其中一個條件」的頁面重算用）／`set(head, open)`／`toggle(head)`／`sync(root)`／`isOpen(head)`。
- **點擊走 document 級單一委派** — 動態渲染的列不必重新綁定；`init()` 只登記容器，不會再加第二顆監聽器（加了會讓同一次點擊被處理兩次、等於沒切）。
- **Consumer 頁** — `pickup-detail.html`（買家分組，帶 `onToggle` 重算搜尋與狀態篩選）· `order-detail.html`（品項展開單位、組合展開成員、成員再展開單位，兩層）· `events.html`（系列場次，帶 `onToggle` 重算「子列可見＝母列過篩選且展開」，搜尋中一律攤開）· `project-detail.html`（收支小計，`init(deep)` 即可，無 callback）。`scanner.html` 只用 CSS 的 `--static` 面板，名單唯讀、不展開，不載這支 JS。
...
### 4.205 Listing controls

**Purpose** — 「有沒有上架」「在網站上找不找得到」「能不能購買和結帳」是三件不同的事。spec §7.14（D241）把它們定成三個互相獨立的開關——上架（Listed）／顯示於商店（Shown）／開賣（On sale）——本元件是那三列的骨架。D241 之前這張卡叫「上架設定」，「上架」一個詞同時背了「發布了沒」與「在網站上找不找得到」，現在拆開。

**`_layer`** · organism ｜ **source tier** · App ｜ **surface 層** · 不畫底色也不畫外框，白底與 `--muted` 灰底皆可（外面那個盒子歸頁面：建立頁預覽欄的 `.card`、細節頁的 `.form-section--outlined`）

**它擁有列，不擁有控制** — 歸它的只有四件事：標題＋一行說明的排法、控制擺在哪、排程欄何時展開、三列之間的距離。控制本身原樣重用 `switch`／`radio-list--collapsible`／`.field`＋`.input`／`link-field`，本檔不畫任何開關或輸入框。

**兩個 section、每個 section 兩層（2026-09-04 · D245）** — 原本一個 section 三列平鋪（上架／顯示／開賣）。規格上三者互相獨立沒錯，對創作者卻是兩個不同時間點的決定：「這件東西要不要、什麼時候出現在商店」是現在的事，「什麼時候可以結帳」是出現之後的事。所以拆成**上架設定**（上架主決定＋第 2 層的上架與下架時間、顯示於商店、非公開連結）與**開賣設定**（開賣主決定＋第 2 層的開賣與停售時間）兩個 section。

每個 section 的規則一樣：第一層只放一個主決定，由它推導出來的設定收進 `.lctl__derived` 第 2 層。建立頁的「不上架」＝草稿，第 2 層與整個開賣設定一併不出現；細節頁下架時第 2 層**不收起**（非公開連結留在畫面上灰化，比整塊消失誠實）。列標題不重寫 section 標題已經說過的字——上架設定裡不再有一列叫「上架」。

**顯示於商店不在第 2 層裡，而且排在它上面（2026-09-04 · D247，同日修訂）** — 第 2 層 `.lctl__derived` 裝的是「上架這件事的排程」（什麼時候上、什麼時候下）；找不找得到是東西一上架就存在的另一個問題，所以它緊接在主決定後面（中間一條 `.lctl__rule`），排程盒排在它下面。這一列**不給自己的外框**，就是一條 `.lctl__row`——有框會讀成「兩個對等的盒子」，實際上它只是一列開關；隱藏時要長出的非公開連結或說明用列自己的 `.lctl__body`。上架沒成立時它與第 2 層一起收起。

**下架與停售也各有自己的主決定（2026-09-04 · D246）** — 這兩個時間原本是第 2 層裡的裸欄位，配一句「留空＝不自動下架／不自動停售」，等於要創作者從「沒填」推回「不會發生」。改成明示的開關列（`.lctl__row` ＋ `switch`，開了才展開 `.lctl__sched` 的時間欄，時間欄此時必填；關掉會清空已填的值）。用 `switch` 而不是第二個 `radio-list`：它是二元的排／不排，形態與上層的主決定不同，一眼看得出誰包著誰。

**哪一列用哪種控制由消費頁決定**

- **建立頁**（create-product／create-bundle／create-auction）：上架列用 `radio-list--collapsible` 三選一（不上架／立刻上架／定時上架），選「定時上架」才展開排程欄。建立時「要不要上架」與「什麼時候上架」是同一個決定。
- **開賣的主決定**：兩頁型都用 `radio-list--collapsible` 二選一（跟著上架一起開賣／定時開賣）。選「跟著上架一起開賣」就不問開賣時間；停售時間兩種情況都可以設。
- **細節頁**（product-detail／bundle-detail）：上架列改用 `switch`。東西已經存在了，這裡只剩「現在開著還是關著」一個二元狀態；再擺一份三選一等於要創作者重講一次已經發生過的決定。

**Anatomy**

- `.lctl` — 三列的容器
- `.lctl__row`（`--choice`）— 一個開關一列；`--choice` ＝控制是整列寬的 radio-list、擺在標題下方，行尾不留控制位
- `.lctl__head` > `.lctl__titles`（`.lctl__title` ＋ `.lctl__hint`）＋ `.lctl__control` — 左標題右控制
- `.lctl__body[hidden]` — head 底下的整列寬控制（收合式 radio-list，或關掉「顯示於商店」時展開的 `link-field`）
- `.lctl__sched[hidden]` > `.lctl__grid` > `.field` ×2 — 排程欄，兩個時間欄並排，720px 以下收成一欄
- `.lctl__rule` — 主決定與第 2 層之間的髮絲線（2026-09-04 使用者裁示）。用負左右外距抵銷母卡的 `--card-pad`、畫到卡的內緣：它分的是這個區塊的「決定」與「決定之後的事」兩段，不是第 2 層那個盒子自己的邊（在盒子上加 `border-top` 只會停在它的圓角框裡）。
- `.lctl__derived[hidden]` — 第 2 層附屬設定（2026-09-04 · D245）：主決定成立之後才存在的一組設定。要不要有外框由消費頁決定：加 `.control-group.control-group--plain`（1px 內描邊、無填色、卡片級圓角）就有框。2026-09-04 兩次裁示的結果是**上架設定的第 2 層有框**（它可能同時裝上架時間與定時下架，框把「排程」圈成一組）、**開賣設定的第 2 層沒有框**，本類只管內部節奏（`gap`）與把 `.field` 的下邊距交回容器。markup：`<div class="control-group control-group--plain lctl__derived">`
- `.lctl__note` — 整區共用的一行淡說明（目前是時區待確認），吃 `--faint-ink`

**Variants** — `.lctl--stack`：欄本來就窄的情境（建立流程 264px 的 `preview-column--narrow`）明說「這裡一律單欄」，不靠猜視窗寬度。

**垂直節奏歸容器** — `.lctl` 自己的 `gap`（`--sp-24`）持有節奏，不用 `.lctl__row + .lctl__row`（STYLE-DECISIONS Q103）。列內的 `.field` 明寫 `margin-bottom: 0`，否則 16px 下邊距會與 gap 疊成兩段距離。

**`__hint` 與 `.field__hint` 刻意同款** — 同字級（`--fs-12`）同色（`--muted-foreground`）。那是同一個角色（這一列還要注意什麼），不該變成第二種輔助說明樣式。

**它畫的是哪些產品規則（§7.14）**

- 下架日期與時間、開賣日期與時間、停售日期與時間三個都是**選填**，表單不得預設任何一個——預設值等於替創作者決定「什麼時候收」。
- 剩下的校驗只有「停售要晚於開賣」（`ListingState.validateSaleWindow`）。D239 的「販售窗口必須落在可見性窗口內」對**商品**已廢除（活動與項目仍有效）。
- 隱藏時的非公開連結由 `ListingState.privateLinkFor()` 產生；下架時公開與非公開連結一起失效。

**Token usage** — `--sp-24`（三列的節奏）／`--sp-10`·`--sp-12`·`--sp-2`（列內間距）·`--fs-14`（標題）·`--fs-12`（說明）·`--muted-foreground`·`--faint-ink`·`--foreground`·`--font-ui`·`--lh-comfy`。無裸值。

**Dependencies** — 組成：`switch`（§4.6）、`radio-list`（§4.1）、`field-system`（§4.1）、`input`（§4.8）、`link-field`（§4.206）。數字與狀態：`js/listing-state.js`。

**Consumers（規劃中）** — `create-product.html`／`create-bundle.html`／`product-detail.html`／`bundle-detail.html`／`e-shop.html`。D241 的頁面遷移另一輪處理，本輪（2026-09-04）只出共用地基與 DS 文件。

**CSS** — [`listing-controls.css`](./ds-components/listing-controls.css) ｜ **JS（推導）** — [`listing-state.js`](./js/listing-state.js)

---

### 4.206 Link field

**Purpose** — 一條系統產生、人不能編輯、但要能整條帶走的連結。首個角色是隱藏商品或組合的**非公開連結**（spec §7.14）：創作者複製給人，或重置作廢舊的那條。

**`_layer`** · molecule ｜ **source tier** · App ｜ **surface 層** · `__row` 自帶 `--input-surface` 底與 1px `--border`，白底與灰底上都成立

**回饋長在按鈕上** — 「已複製」直接換掉按鈕自己的字，不跳 toast：手剛離開的地方就是眼睛在看的地方，回饋跑到畫面角落等於沒有回饋（沿用 `media-vault.js` 既有的判斷）。同時整列亮 1.6 秒（`--copied`），讓回饋不只有三個字那麼寬。

**與 vault-share 的關係** — `vault-share.css` 的 `.vshare__link`（媒體庫的鑰匙連結）是站上第一個做這件事的地方，本支把共同部分抽成 DS 級答案。**`.vshare__link` 這一輪不動**：媒體庫那條連結還多帶「剛產生」的橘框強調，與這裡的中性預設不是同一件事。別照抄它的 markup；日後確認兩者要同一個外觀時再收斂。

**Anatomy**

- `.linkf`（`--copied` 短暫回饋／`--disabled` 連結已作廢）
- `.linkf__label` — 選配；用 `.field__label` 也可以
- `.linkf__row` > `input.input.linkf__input`（readonly，`--font-mono`／`--fs-12`）＋ `button.btn.linkf__act`（複製，`--copy` 帶 92px 下限寬度）＋ 選配的重置鈕
- `.linkf__hint` — 一行說明

**States** — default／`--copied`（`--primary` 邊框＋`--selected-surface`）／`--disabled`（`--muted` 底、`--locked-field-ink` 文字、動作鈕不吃點擊）。**disabled 靠顏色與游標表達，不疊 `opacity` 弱化整塊**（STYLE-DECISIONS Q58）。

**Variant** — `--stack`：窄欄（約 250–300px，`product-detail.html`／`bundle-detail.html` 右欄）放不下「輸入框＋兩顆鈕」同一列，`__row` 改 `flex-wrap`，輸入框獨佔第一行（`flex:1 1 100%`），兩顆動作鈕落到第二行靠右（`justify-content:flex-end`，同站上動作列一貫的靠右慣例）。

**為什麼輸入框轉透明** — 外框已由 `__row` 畫過一次，輸入框再畫一次會變成框中框。

**JS module — `js/link-field.js`**

- **對外契約（data-attribute）** — `data-linkf` 掛在根節點；`data-linkf-copy` 掛在複製鈕；`data-linkf-reset` 掛在重置鈕（選配）。
- **重置不自己決定新連結長什麼樣**：它發一顆 `linkfield:reset` 事件，由頁面回答（通常呼叫 `ListingState.resetPrivateLink(entity)`），再用 `linkField.setValue(root, url)` 寫回。連結格式屬產品規則，不歸元件。
- **對外契約（API）** — `window.linkField.setValue(root, url)`／`value(root)`／`copy(root)`。
- **點擊走 document 級單一委派**：之後才渲染出來的連結列不必重新綁定。非安全來源沒有 clipboard API 時退回離屏 input ＋ `execCommand`。
- **i18n** — `link.copy`／`link.copied`／`link.reset`／`link.reset-hint`／`link.private-h`／`link.private-hint`／`link.copy-failed`。

**Token usage** — `--input-surface`·`--border`·`--border-soft`·`--muted`·`--primary`·`--selected-surface`·`--locked-field-ink`·`--muted-foreground`·`--foreground`·`--radius-lg`·`--control-h-sm`·`--font-mono`·`--font-ui`·`--fs-12`·`--sp-6`·`--duration`·`--easing`。無裸值（92px 的按鈕下限寬度是防跳動的視覺補償、非 token，與 `.vshare__copy` 同值）。

**Dependencies** — 組成：`button`（§4.2）、`input`（§4.8）。疊在：`listing-controls`（§4.205）的顯示列。

**Consumers（規劃中）** — `product-detail.html`／`bundle-detail.html`／`create-product.html`／`create-bundle.html`。

**CSS** — [`link-field.css`](./ds-components/link-field.css) ｜ **JS** — [`link-field.js`](./js/link-field.js)

---

### 4.207 Stock allocation

**Purpose** — 一顆商品的庫存池怎麼分給它的販售管道（spec §7.14 · D241）。建立商品時池裡建一筆商品、電子商店建一個「單售」販售管道；它加入的每個組合包都是另一個販售管道。任何販售管道都可以從池裡**鎖定** N 件給自己。

**`_layer`** · organism ｜ **source tier** · App ｜ **surface 層** · 只畫列線與欄軌，底色歸容器

**兩件事一定要看得到** — 那正是最容易搞錯的規則：

1. **逐列的可售量**。鎖定的販售管道**不吃沒有被鎖定的庫存量**，所以每一列都要有自己的數字，不能最上面擺一個共用的。鎖定的販售管道賣完鎖定量就是售罄，即使池裡還有未鎖定的數量，也要再鎖一次才會回來。
2. **所有販售管道都鎖定時的提醒**。那時沒有被鎖定的庫存量沒有人能賣，末列旁掛一顆 chip 講清楚剩幾件（`stock.all-locked`）。

**Anatomy**

- `.salloc`（`--bundle` 組合視角）
- `.salloc__row--head` — 欄標題，caption 字級，吃 `--column-head-ink`
- `.salloc__row--pool` — 目前在庫（用 `stock-readout` 的大數字或可編的 `amount-field`）
- `.salloc__row` ×N — 每個販售管道一列：`.salloc__name`（＋`.salloc__sub`）／`.salloc__lock`（`amount-field` 欄位槽）／`.salloc__qty`（唯讀可售量）
- `.salloc__row--free` — 末列的未鎖定量；可帶 `.salloc__chip`
- `.salloc__row--error` ＋ `.salloc__error` — 鎖定量超過上限；錯誤行字級與顏色比照 `.field__error`

**Variants** — `--bundle`：四欄而非三欄（成員名／該成員沒有被鎖定的庫存量唯讀／鎖定給本組合的量可填／本組合可售量唯讀），末列是「組合可售＝成員最低在庫」。

**欄寬是變體不是頁面上的數字** — `--salloc-lock-col`／`--salloc-qty-col` 兩個本地變數持有欄寬，消費頁要調整就覆寫變數，不在 style 屬性寫死 px。

**數字全部來自 `js/listing-state.js`** — `freeQty(product)`／`channelQty(product, channel)`／`bundleQty(bundle, productsById)`／`allChannelsLocked(product, bundles)`，欄位上限走 `validateLock(product, channel, wanted)`（上限＝沒有被鎖定的庫存量＋該販售管道目前已鎖定的量，因為調整自己的鎖定量時原本那份本來就能重新分配）。**頁面不得自己再算一次 min(成員) 或「池減鎖定」**。

**可售量欄用等寬數字靠右** — 這一欄是要往下掃著比大小的（組合可售量＝取最小值），對不齊就掃不動。

**Token usage** — `--border`·`--border-soft`（列線）·`--column-head-ink`（欄標題）·`--foreground`·`--muted-foreground`·`--status-error`·`--font-ui`·`--font-numeric`·`--fs-12`·`--fs-14`·`--sp-2`·`--sp-6`·`--sp-10`·`--sp-12`。欄寬 132px／96px（窄版 104／72）為本地變數的預設值、非 token。無裸色。

**Dependencies** — 組成：`amount-field`（§4.1）、`stock-readout`（§4.54）、`chip`（§4.5）、`field-system`（§4.1）。數字：`js/listing-state.js`。

**Consumers（規劃中）** — `product-detail.html`（定價與庫存分頁）／`bundle-detail.html`（`--bundle` 變體）／`create-bundle.html`（成員鎖定量）。

**CSS** — [`stock-allocation.css`](./ds-components/stock-allocation.css) ｜ **JS（推導）** — [`listing-state.js`](./js/listing-state.js)

---

### 4.208 Stock history（庫存歷史紀錄）

**Purpose** — 一件商品的庫存變動紀錄：補貨單（立即／計時）與鎖定變動合併一張清單，**一張單一筆**——補貨單＝方式／供應商／預計到貨／備註＋逐組合數量（5.1.5.6 §5 的單據層／品項層），鎖定＝一次儲存的逐組合前 → 後。歷史紀錄多數時候是掃（上次補多少、隔多久），偶爾才查細節，所以主列乾淨、細節收在展開層（2026-09-11 使用者裁決 H1，探索頁 `lab-stock-history.html`）。

**`_layer`** · molecule ｜ **source tier** · Project ｜ **surface 層** · 列與展開面板歸 data-list／row-disclosure，本檔不畫底色

**Anatomy**（清單型 row-disclosure 上疊一層）

- `.data-list > .rowdis__group.rowdis__group--gutter` — 一張單一組
- `.data-list__row.rowdis__head` — 主列：`.shist__date`（固定寬 92px 日期，列與列的主體才對齊）→ `.shist__type`（類型徽章：補貨 `badge--info`／鎖定 `badge--neutral`）→ `.data-list__body`（`__title`＝逐組合變動摘要「M +40 · L +25」，數字掛 `.shist__delta` 等寬；`__meta`＝供應商 · 方式（計時補貨帶預計到貨）／鎖定＝鎖給誰）→ `.shist__end`（狀態徽章＋ `.rowdis__toggle`）
- `.data-list__row.rowdis__child.rowdis__child--compact.shist__child` — 明細一列一項：`.shist__k`（鍵，88px）＋ `.shist__v`（值）；順序＝逐組合 → 供應商 → 預計到貨（補貨中時右側放「到貨確認」）→ 備註
- 展開層左側 `padding-left: calc(92px + --sp-12)` 讓出日期欄，明細與主體對齊（清單型的「零縮排」＝對齊主體）

**States** — 補貨中（`badge--warning`，到貨確認在展開層與「補貨中」卡兩處都能按，同一筆）／已完成（`badge--success`）。

**Token usage** — `--muted-foreground`（日期、鍵）·`--foreground`（值、變動數字）·`--fs-12`／`--fs-13`·`--sp-12`／`--sp-8`；底色、列線、面板全部來自 data-list 與 row-disclosure。

**Dependencies** — 組成：`data-list`（§4.x）、`row-disclosure`（§4.204）、`badge`、`button`；展開互動走 `js/row-disclosure.js`。

**Consumers** — `product-detail.html`（商品庫存分頁的頁內卡＝最近四筆、「查看全部」彈窗＝全部；同一份資料同一支 render）。

**CSS** — [`stock-history.css`](./ds-components/stock-history.css)
...
Best-practice assembly recipes — how components combine to meet a creator's goal. A Pattern is not a component (Pillar 4) and not a wireframe (the page implementation). Each card has five grids: `trigger` (when to use) / `must` (≥2 non-negotiables) / `should` (advisable) / `must-not` (anti-patterns) / `_edge-cases` (empty / error / new-user / mobile / offline). Four categories, eleven cards（頁面級 5 張＋中間層 4 張，2026-07-08；2026-07-20 新增 Detail + persistent rail，共十張；2026-09-08 新增 Destructive confirm，共十一張）。Component sections demo the unit only — assembly rules live here（判準見 §4.0）。

**html 對照（本輪拆頁）** — `design-system.html` 這輪把原本擠在一張表格裡的十張卡拆成十個獨立小節，每個都掛了可跳轉的錨點、並用真元件渲染出組合實例＋「實際使用頁」連結；下方每張卡的標題都補上編號與對應錨點 id，方便對照。html 端的 Trigger／Must 是精簡過的展示文案，語意與下方全文版一致，不是另一套規則。

| # | Pattern | html 錨點 |
|---|---|---|
| 5.1.1 | Dashboard home | `#pattern-dashboard` |
| 5.1.2 | Tabbed section view | `#pattern-tabbed` |
| 5.1.3 | Filter + list | `#pattern-filter-list` |
| 5.1.4 | Lifecycle states | `#pattern-lifecycle` |
| 5.1.5 | Multi-step wizard | `#pattern-wizard` |
| 5.1.6 | Form assembly | `#pattern-form-assembly` |
| 5.1.7 | Settings page | `#pattern-settings` |
| 5.1.8 | Modal shell | `#pattern-modal` |
| 5.1.9 | Split preview | `#pattern-split-preview` |
| 5.1.10 | Detail + persistent rail | `#pattern-detail-rail` |
| 5.1.11 | Destructive confirm | `#pattern-destructive-confirm` |

#### 5.1.1 Dashboard home (Layout) · `#pattern-dashboard`
...
#### 5.1.2 Tabbed section view (Layout) · `#pattern-tabbed`
...
#### 5.1.3 Filter + list (Interaction) · `#pattern-filter-list`
...
#### 5.1.4 Lifecycle states (Lifecycle / State) · `#pattern-lifecycle`
...
#### 5.1.5 Multi-step wizard (Workflow) · `#pattern-wizard`
...
#### 5.1.6 Form assembly (Workflow) · `#pattern-form-assembly`

- **trigger**: Any create / edit form — wizard steps, settings forms, create-product / -event / -project sections.
- **must**: Every field is a `Field`（label + optional hint + exactly one control slot; the control reuses an atom — Input / Switch / Segmented / textarea）; group fields into `Form section`s（title + grey sub; sibling sections auto-divide via the component's own `+` rule; base gap 6 / field spacing 16）；side-by-side fields use `Form grid`, never hand-rolled columns. **Section 切分＝一個 section 一個決定**（2026-09-01 自 create-product 抽取）：切區依「創作者要做的決定」不依欄位型別——價格與折扣是兩個決定就是兩個 section，庫存與逐規格表分開；判別句：「這個 section 的標題能不能一句話回答『這區在決定什麼』」。**條件分支控顯隱、不重排**：跨型分支（實體／數位／多規格）用 `data-*-show` 控 section 顯隱，section 順序恆定、絕不為分支建第二份表單；間距規則跨越 `[hidden]`（`~` 規則）。**Section 內分層階梯固定四級**：`__head`（title 必有）→ `__subhead`（僅當同一決定下有兩群欄位）→ `form-grid`／`field` → `nest`（卡中卡，僅補充性子塊）——用不到的層級跳過，不得自創第五級。**改變路由的動作**（如類型切換）放第一個 section 的 `__head--actions` 右端，不混進表單體。 建立流程的 `.wizard__sheet--sectioned` 使用 `--surface-page` 作底；`.form-section--outlined` 以 `--card` 填色形成區塊（亮暗一致；dark 原用 `--muted`、2026-07-17 midnight 改 `--card`）（**無外框線**，Q14），所有可見 outlined siblings 以 `--sp-24` 間隔並跨越 `[hidden]`。採用頁為 create-product／-auction／-bundle／-event／-project／register-ip／admin-ip-bank-entry。
- **should**: **Section 順序照商品的生命敘事**（create-product 的驗證序，其他建立流程沿用、依產品語意跳過不適用的環節）：識別（它是什麼）→ 呈現（素材）→ 交付物（內容檔案）→ 售賣條件（規格 → 價格 → 折扣 → 庫存）→ 履約（交付／取貨）→ 購買規則（限購）→ 發現（標籤、關聯）→ 保障（結帳與保障）；**上架這類「看著成品才做的決定」放右欄預覽卡下方**（split-preview 頁型，見 5.1.9），不排進左欄敘事。Section 的 `__sub` 只在需要說服或解釋規則時出現（素材、取貨有；價格、標題不用）。Order fields identity → content → settings → risk disclosures; put character counts as right-aligned `field__hint--count`（form-section scope）; keep one sticky action bar per form for save / next.
...
#### 5.1.7 Settings page (Layout) · `#pattern-settings`
...
#### 5.1.8 Modal shell (Interaction) · `#pattern-modal`
...
#### 5.1.9 Split preview (Layout) · `#pattern-split-preview`
...
#### 5.1.10 Detail + persistent rail (Layout) · `#pattern-detail-rail`
...
#### 5.1.11 Destructive confirm (Interaction) · `#pattern-destructive-confirm`

2026-09-08 建，第一個消費者是訂單詳情的「作廢品項」（`order-detail.html` 的 `#od-void-modal`）。站上原本每個破壞性動作各自處置（刪除商品、刪草稿都在頁內註「確認流程待規格」、原型暫用原生 `confirm()`），這張卡把做法定下來：**沒有新的 modal CSS——它是既有 Modal shell（§5.1.8）加上 `.btn--destructive` 的組裝規則**，所以規則寫在 Pattern、不寫成元件。

- **trigger**: 一個按下去就收不回來的動作——作廢明細、刪除商品／草稿／活動、停用帳號。可逆的動作（存草稿、下架、切換顯示）不走這張卡，直接做完給 toast。
- **must**: 入口用 `.btn--ghost.btn--destructive`（紅字、不搶焦點），確認那一顆才是 `.btn--destructive`（透明底＋紅框紅字，Q37 2026-08-06 裁決）；彈窗重用 canonical 對話框殼（`.payout-modal` ／ `.payout-dialog` 頭／身／腳）；內容順序固定四段——**動到哪一項**（具體名稱與數量，不只說「這個項目」）→ **前置條件**（要先在別的系統做完什麼，例如「請先在 Stripe 完成人工退款」）→ **會發生什麼**（逐條列後果：憑證失效、庫存回補、寄信給誰）→ **不可逆**；完成後給 toast 說出實際發生了什麼，不是只說「成功」。
- **should**: 停用的入口按鈕保留在畫面上並用 `title` 說明為什麼不能按（不可作廢、沒有權限），不要直接隱藏——看不到的按鈕會被讀成「這個功能不存在」；後果條目照「使用者最在意的」排序（買家會收到什麼放最後一條，因為那是唯一會傳出去的動作）。
- **must-not**: 不可用原生 `confirm()`；不可把後果寫成一句籠統的「此操作無法復原」就了事；不可讓確認鈕成為整個彈窗唯一的視覺焦點以外的東西（不要同時放兩顆紅的）；不可在確認前就先改畫面狀態。
- **_edge-cases**: `empty` → 沒有可作廢對象時不開窗，入口本來就該停用；`error` → 失敗時彈窗留著、body 內用 `Alert --row` 說明，不要關窗吞掉；`new-user` → 不需要導覽，四段內容本身就是說明；`mobile` → 對話框滿寬，兩顆動作維持右對齊；`offline` → 確認鈕停用並說明原因。
...
R 2.2 chart family lives in [`ds-components/chart.css`](./ds-components/chart.css). Three sub-patterns:
...
- **分組面（疊上去的一層）往亮**：`--nest-surface`（深色 `rgba(222,223,233,.04)`／亮色 `transparent`）＋ `--nest-line`（深色 `transparent`／亮色 `rgba(16,17,20,.08)`）。消費者：`.nest`（模式切換長出來的整層）、`.card--muted`（卡片裡的一段分組）、`.bd-sec`（組合包彈窗的分卡）。亮色兩層同為白，靠 `--nest-line` 才分得出來。
- **凹槽與襯底往暗**：`--muted` `#161718`。消費者：`.segmented` 的軌道（軌道凹、滑塊才浮）、媒體井（圖片未載入的佔位格）、進度條底。**這一類不在翻向範圍**。

- **控件（浮在表面上的按鈕）往亮**：`--control-raise`（深色 `rgba(222,223,233,.04)`／亮色 `var(--card)`）。消費者：`.btn--outline`。**不要用絕對表面色**——寫死 `--card` 時，同一顆按鈕在畫布上比底亮、在卡裡同色、在 L2 分組面裡反而更暗（Q67）。

判斷句：這塊東西是「疊在容器上的一層」還是「容器上挖出來的一個凹槽」？前者往亮，後者往暗。控件一律往亮。

**`.form-section--outlined` 是頁面層的分組，不放進卡或彈窗**：它吃絕對色 `--surface-shell` `#1C1D1E`，坐在畫布 `#0C0D0D` 上方向正確，但比 `--card` `#212223` **暗**——放進卡或彈窗就會往下沉一階。卡／彈窗內要分組，用上面那組往亮的。

證據與稽核：`docs/階層稽核-2026-08-13.md`（53 頁逐頁量合成後亮度）、STYLE-DECISIONS Q24／Q42／Q66。

### 6.0.2 卡內分組的間距階梯（Q71，2026-08-25）

同一條「多組資訊要不要疊層」的判準軸線，分頁面級（L0，直接畫在畫布上）與卡片級（L1，見 [Card group §4.133](#card-group)）兩種版面各自的定案間距值。判準本身、三級制（排版→線框→新層）見 Card group 條目，本節只記兩種版面各自的數字，供沒有現成元件可套、要手動疊間距時對齊。

**L0 畫布版**（直接落在畫布上、不是卡片內部）：

| 項目 | 值 | 說明 |
|---|---|---|
| 列距（同組內） | `--sp-40`（40px） | 卡片版是 14+14；L0 少了卡緣可以代替留白說話，值放大近三倍 |
| 組分隔線上下留白 | 各 `--sp-56`（56px） | 卡片版是各 32px |
| 組分隔線寬度 | 滿版（畫到畫布最外緣） | 讀成「跨過整個版面的斷點」，不是某一組自己的下框 |
| 組內內容左右內縮 | `--sp-16`（16px） | 線畫到最外緣、內容退進來一階，兩者才不會擠在同一條邊上 |

**卡片版（L1，`.card`/`.ztor-card` 內部）** — 完整定義見 [Card group §4.133](#card-group)，摘要：

| 項目 | 值 |
|---|---|
| 小標 → 內容 | 32px（合併值） |
| 組分隔線上下留白 | 各 32px＋線（總留白 64＋線） |
| 卡頭（T-A2）標題上距 | 24px |
| 卡頭（T-A2）線下距 | 32px |

兩種版面刻意不同軌：L0 沒有卡片邊界可以借力，得靠更大的留白與滿版線自己把「這裡換一組了」講清楚；L1 有卡緣幫忙分界，同樣的訊號可以用小一截的值傳達。**不要把兩套數字混用**——卡片內部出現 40／56 這種 L0 尺度會顯得鬆散失焦，畫布上直接用 14／32 這種 L1 尺度則分不出組界。
...
| `grid.breakpoints.lg` | `900px` | **R 2.2 canonical breakpoint** — topbar nav collapses, bento children fall back to span-12, hero single-column |
...
R 2.2 has 5 canonical page templates. Each composes Pillar 4 components in a specific Pillar 6 structure:
...
The DSS v1.4 standard is output-agnostic. Ztor Creator Studio · R 2.2 ships as CSS Custom Properties (the canonical implementation in [`ds-components/_tokens.css`](./ds-components/_tokens.css)). Below are equivalent representations for other targets.
...
Filled with Ztor Creator Studio · R 2.2's actual values where the 7-Pillar structure maps cleanly. Empty / per-need fields left as placeholders.
...
- **2026-08-26** — md 追上 html 這輪的大幅整理（版本號、Pattern cards 拆頁、Radius token 補完、三處內容過時）。
  - **版本標籤**：全檔 12 處「R 2.1」版本標籤（標題、Identity Overview、Pillar 0 Record、Pillar 1 Color 概述、Pillar 4 layer 表頭與範例、chart family、breakpoint、page templates、Appendix A/B）改 R 2.2；2 處歷史沿革句保留不動（`notes` 欄的「R 2.1.6」特定子版本事件、§3.3 dark surface 說明裡「原 r2.1 制」的新舊對照）。
  - **§5.1 Pattern cards** — html 這輪把十張卡從單一表格拆成十個獨立小節（各自掛錨點＋真元件渲染組合實例＋實際使用頁連結）；md 十張卡標題補上編號 5.1.1–5.1.10 與對應 html 錨點 id，並在小節開頭補一張「# / Pattern / html 錨點」對照表。內容核對後與 html 語意一致，未發現需要改寫的規則。
  - **§1.4 Radius** — 補齊六個具名 token 完整列表（`--radius-sm` 3px／`--radius` 6px／`--radius-md` 別名／`--radius-lg` 8px／`--radius-xl` 16px／`--radius-shell` 28px／`--radius-pill` 9999px，值取自 `_tokens.css`），原本的語意角色表（`radius-button-primary` 等）改標「Semantic role」保留在下方。
  - **§4.117 Session list（product-list 群組列）** — 展開把手位置的記載跟上 2026-08-18 的實際改動：由「放在名稱前面（`__product` 改 flex）」改記為「放在名稱文字之後、同一格 `.product-list__title` 內」，並補一段說明原本前置做法為何造成清單左緣斷掉（對照 UI-CHANGES.md 2026-08-18 條目）。
  - 其餘三處核對點（store-settings 獨立頁面骨架、media-vault `.vault-keys`、scanner 多狀態、size-chart-editor 尺碼制／單位切換／指南切換）md 既有記載已與 html 一致，未變動。
...
- **2026-08-09** — 建立活動第三輪：四支既有元件的視覺語彙收斂，全站生效。
  ...
  - **§4.1 Inventory Control row** — `.control-group` 新增消費頁 create-event.html（販售方式：「發布後直接開賣」開關，關掉才展開「開賣／停售日期與時間」欄位），計數 11→12 組、5 頁；記錄此消費頁的揭示方向與既有頁相反（關才展開，非開才展開），元件本身不因此分岔，揭示條件一律由消費頁決定。
...
### 4.209 State check（狀態勾）

**Purpose** — 一排唯讀的開／關狀態指示：每個狀態一顆小圓標記＋標籤。回答「這件商品現在上架了嗎、找得到嗎、買得到嗎」三個是非題，一眼掃完。

**`_layer`** · molecule ｜ **source tier** · Project ｜ **surface 層** · 無自己的表面，坐在 data-list 列的 title 位置。

**Anatomy** — `.state-checks`（`--stack` 直排）> `.state-check`（`--on`／`--scheduled`／`--off`／`--ended`）> `.state-check__mark`（含 icon：check／clock／x／minus）＋ `.state-check__label`。**四種長相（2026-09-11 lab-state-check.html 定案）**：成立＝實色綠底＋畫布色勾；排定未到＝實色藍底無外框＋畫布色時鐘（字前景色）（2026-09-11 兩輪比較後定案：實色底＝有事在發生）；不成立＝薄膜圓底＋輔助色叉；已結束＝輔助色實心圓＋一橫。沒上架時的顯示／開賣也用「不成立」那顆（使用者裁示，原本另有破折號的「不適用」）。一格要講「現況＋排程」時直排兩顆，排程那顆自己是「排定」狀態（使用者：「換行顯示的同時，也要是一個狀態」）。

**Behavior** — 唯讀；不是 checkbox 也不是 switch（使用者裁示「做成 checkbox 的形式但設計不一定要用 checkbox」——它只呈現、不輸入），點的是所在整列（`data-list__row--go` 前往設定）。值由頁面推導：上架＝listed 開；顯示於商店＝上架且 shown 開；開賣＝上架且此刻在販售窗口內（立即開賣，或定時開賣已到且未停售）。

**Token usage** — `--status-success`／`--status-info`（開／排定：實色底）＋ `--background`（勾／時鐘）、`--ztu-glass-strong`（不成立的圓底）、`--muted-foreground`（已結束實心圓、叉）；（2026-09-11 使用者：「icon 顏色髒髒的」，原 `--status-success-fill` 淡染底＋ `--status-success-ink` 深勾在 16px 裡糊成一團）、`--border`（關的空心圓）、`--foreground`／`--muted-foreground`、`--fs-13`、`--sp-8`（標記與字）／`--sp-16`（並排的狀態之間）／`--sp-6`（直排）。

**Consumers** — product-detail.html、bundle-detail.html（商品設定概覽「上架與開賣」列）。

### 4.210 Lock sets（鎖定套數）

**Purpose** — 組合包的庫存鎖定區：填「鎖定幾套 N」，底下一張唯讀為主的清單列出組合內每一件商品的庫存、未鎖定量與「已鎖定＝N × 每套用量」；多選項商品可展開逐規格選填分配。規則由使用者 2026-09-11 重定義（組合鎖「套」，不逐商品填件數；規格 D266 待補）；視覺是 `lab-bundle-lock.html` L3 逐輪修訂定案。

**`_layer`** · organism ｜ **source tier** · Project ｜ **surface 層** · 清單自帶 1px `--border` 外框（`--radius-xl`），框內貼邊 `--border-soft` 髮線分列；無薄膜、無實色表面（Q102）。

**Anatomy** — `.lockset__sets`（鎖定套數：`__sets-titles` > `__sets-title` ＋ `__sets-hint`（`.is-over` 超上限紅字）｜`__sets-ctl` > amount-field）→ `.lockset__title`（清單標題＋一句規則）→ `.lockset`（外框）> `.lockset__head`（四欄表頭）＋ `.lockset__row`（`--sub`＝兩行列）×N；每列＝`.lockset__name`（`data-list__thumb--lg` 縮圖 ＋ `.lockset__text` > 名稱一行 … ＋ `.lockset__sub`）＋ `.lockset__num` ×2（商品庫存／未鎖定）＋ `.lockset__lockval`（`__lockval-row` 數字靠右貼單位 ＋ `__lockval-sub` 「已指定 x 件」／`.is-over` 「超過鎖定量」）或 `.lockset__nolock`（不鎖定：「共用未鎖定量」）＋ `.lockset__end`（把手）。多選項商品整組包在 `.rowdis__group.rowdis__group--gutter`，規格列 `.lockset__row.lockset__child` 的已鎖定欄是 100px 的 amount-field（7 位數）。

**Behavior** — 空白／0＝不鎖定；N 的上限＝各商品 floor(未鎖定 ÷ 每套用量) 取最小（提示寫「最多 N 套（受「x」限制）」），超過只標紅、不寫進模型；有鎖定時組合可售量＝N（再與限量上限取小）。逐規格分配 Σ ≤ N × 用量，少分配＝只限總數、規格不拘；0 件不寫「已指定」。所有數字由 `js/listing-state.js` 的 `bundleLockSets`／`bundleMaxSets`／`bundleAllocOf`／`bundleQty` 算，寫回成員商品一律 `applyBundleLock`。展開／收合由 row-disclosure 管，展開的整組一起 hover、組內不畫線。

**Token usage** — `--border`／`--border-soft`／`--radius-xl`（框與線）、`--ztu-glass-strong`（hover）、`--column-head-ink`／`--fs-12`（表頭）、`--font-ui`／`--fs-14`／`--fs-13`／`--fs-12`、`--foreground`／`--foreground-muted`／`--muted-foreground`、`--status-error`（超量）、`--sp-2/8/12/14/16/24/28/32`、`--control-h-xs`（規格輸入框）。欄軌 `--lockset-col`（`minmax(64px, 100px)`）／`--lockset-colgap`（`clamp(--sp-12, 4%, --sp-32)`）是元件本地變數，窄容器自動收斂。

**Consumers** — bundle-detail.html（組合庫存分頁「商品庫存鎖定」）、create-bundle.html（F4 庫存）。探索紀錄：lab-bundle-lock.html（L1／L2／L3 三案，L3 定案）。

### 4.211 Profile avatar row（頭像列）

**Purpose** — 商店設定「店面」分頁最上面那一條：圓形頭像、顯示名稱、右端一顆 ghost「變更相片」；照粉絲端「編輯個人檔案」抽屜的版型（2026-09-11 使用者：「可編輯欄位參考這裡」）。

**`_layer`** · molecule ｜ **source tier** · Project ｜ **surface 層** · `--ztu-film` 薄膜＋ `--ztu-rim-soft` 邊。

**Anatomy** — `.profile-avatar-row` > `img.profile-avatar-row__img`（56px 圓）＋ `.profile-avatar-row__name`（一行、截斷）＋ `.btn.btn--ghost.btn--sm` ＋ `input[type=file][hidden]`。

**Behavior** — 名字跟顯示名稱欄位同步；檢視態由 shared.css 的白名單藏掉按鈕；接線在 `partials/social-links.js`。

**Consumers** — store-settings.html。

### 4.212 Social links（社群連結清單）

**Purpose** — 已加的平台一列一格（平台名＋垃圾桶在上、網址輸入在下），底下一條虛線「新增社群連結」，點開展出還沒加的平台清單（icon＋名稱，已加過的不再列）。照粉絲端「編輯個人檔案」抽屜的「社群連結」段（2026-09-11）。

**`_layer`** · organism ｜ **source tier** · Project ｜ **surface 層** · 無自己的面，坐在玻璃卡內；輸入欄照 `.input`。

**Anatomy** — `.social-links[data-social-links]` > `.social-links__row`×n（`__head` > `__label`［`__mark`］＋ `.btn--icon.social-links__remove`；`input.input.social-links__url`）＋ `button.social-links__add`（`.is-open`）＋ `.social-links__picker[hidden]` > `.social-links__option`×n。

**Behavior** — 資料 `js/store-profile-store.js`，畫與接線 `partials/social-links.js`；移除鈕 hover 轉 `--destructive`。

**Consumers** — store-settings.html。

### 4.213 Bundle preview card（組合包預覽卡）

**Purpose** — 創作者建組合包時右欄那張「粉絲看到的」卡（建立活動第 6 步，`js/bundle-editor.js` `layout:'split'`）。2026-09-21 由 `docs/bundle-create-demo-2026-09-21.html` promote（使用者裁示「做上正式」）。回答「粉絲買這一組會看到什麼」：內含清單（順序與編輯器分卡一致：票券 → 商品 → 額外權益）、劃線原價與售價、剩餘組數、一顆「加入購物車」示意鈕。**選擇器可以點**（票種、場次、尺寸、顏色）——不點就看不到「從 $Y 起」怎麼收斂成一個數字、也看不到剩餘不足的票種對粉絲的意思；點選只影響這張卡，不寫回表單。粉絲端組合包畫面上游未定案（ASSUMPTIONS BDL-001／BDL-002）：呈現推導、不是前台規格。

**`_layer`** · molecule ｜ **source tier** · Project ｜ **surface 層** · 沿用 preview-card（坐在彈窗內時翻成 `--ztu-film` 薄膜、無模糊）；本檔不加自己的面。

**Anatomy** — `.bpc__title`（右欄小標，卡外）→ `.preview-card.bpc` > `.preview-card__media`（**2:1**，不是 preview-card 的直式 2:3——340px 右欄裡直式圖會把內含清單推到摺線下，STYLE-DECISIONS Q120 待裁決）＋ `.preview-card__body` > `.preview-card__row`（`__name`［`.is-empty`］＋ `.ztor-badge` 活動狀態）· `.preview-card__desc` · `.bpc__list` > `.bpc__row` ×N（16px icon ＋ `.bpc__what`［`.is-empty`］＋ `.bpc__pick` ×N > `.bpc__picklabel` ＋ `.chip-group` > `.chip`［`.chip--active`］［`:disabled`］＋ 選配的尾句 `.bpc__picklabel`）· `.bpc__price` > `.bpc__now` ＋ `.bpc__was` ＋ `.ztor-badge--warning`（省 N%）· `.bpc__meta`（剩 N 組／`.ztor-badge--error` 售罄／每場各一組註記）· `.btn.btn--primary.bpc__cta`［disabled］· `.bpc__why` → `.bpc__note`（卡外一句）。

**Behavior** — chip 帶 `data-bd-fan-sess`／`data-bd-fan-tier`／`data-bd-fan-var`（＋`-opt`、`-val`）；編輯器把粉絲的選擇存在側邊對照表（不在組合包紀錄上），表單每改一次就地重畫這張卡。價格＝選到的票種 × 每組張數 ＋ 各商品選到的規格價（沒選＝最低價）再 ×（1 − 折扣 %）；沒選齊寫「從 $Y 起」、選齊落到單一數字。票種 chip 在粉絲挑的那一場剩餘 < 每組張數時停用（D296 決定四 a）；規格值在沒有任何含該值的規格有庫存時停用（D295）。CTA 在沒選齊或可售套數 0（售罄）時停用並寫一句理由（「先選票種、尺寸」）。純示意：不寫任何狀態。

**Token usage** — `--font-display`／`--fs-14`／`--muted-foreground`（小標）、`--border-soft`（清單與價格分隔線）、`--font-ui`／`--fs-13`／`--foreground`（內含文字）、`--fs-11`（選擇器標籤）、`--font-numeric`／`--fs-22`（售價）、`--fs-13`（劃線原價，同 `.cb-basestrike`＝Q35 現況 A，不開第三種）、`--fs-12`（meta／why／note）、`--lh-relaxed`、`--sp-6/8/10/12/14`。顏色只有文字階層，徽章與 chip 各自帶色。

**Consumers** — create-event.html（第 6 步組合包彈窗右欄，`layout:'split'`，markup 由 `fanHTML()` 產生）；create-bundle.html（2026-09-21 第 2 批，頁面級：外殼是建立流程預覽欄的 `form-section--outlined`（2026-09-04 裁示），卡本身是這支元件、markup 由頁內自己的 `fanHTML()` 產生，chip 帶 `data-cb-fan-tier`／`data-cb-fan-var`）。bundle-detail.html 沒有右軌，粉絲視角維持「以粉絲身分預覽」的商店鏡像、不硬加這張卡。探索紀錄：docs/bundle-create-demo-2026-09-21.html。

### 4.214 Section tabs（分節分頁）

**Purpose** — 一頁很長、分成幾段的表單（組合包的「內容物／定價與庫存／命名與上架」三段）在頂端放一排固定的分頁：點了平滑捲到那一段，捲動時亮著目前看到的那一段（scrollspy）。2026-09-21 使用者反饋「新增票務商品的 popup 要有 fix 在最上面的 tab 可以快速滾動到指定區塊」；電子商店的建立組合頁是同一個心智模型（UI-CHANGES 五十四），同一支元件裝兩處。**不是切換面板的分頁**（那是 §4.15 Tabs ＋ `.tab-panel`）——三段都在畫面上，分頁只負責跳轉與定位；也不是 §4.22h0 Section nav（詳情頁左側、切換面板）。

**`_layer`** · molecule ｜ **source tier** · App ｜ **surface 層** · 透明，坐在宿主頂列的面上（彈窗 `--popover` 72%、建立流程頂列玻璃）；本檔不加自己的面。

**Anatomy** — `nav.section-tabs[data-section-tabs]`（殼：固定高 `--control-h-md` 44px、`display:flex; align-items:stretch`）> `.tabs.tabs--underline-short.tabs--underline-label` > `button.tabs__item[data-st-tab="key"]` > `<span>`標籤（目前段 `.tabs__item--active` ＋ `aria-current="location"`）。捲動區裡的目標段掛 `[data-st-section="key"]`。殼就是 Q38 那組配方要求的「殼」：item 撐滿 44px，發光底線（`bottom: -12px`）落在殼的下緣，與 `.list-toolbar` 同一個機制。

**Placement（只准一條固定列）** — 兩個消費處都已經有一條固定頂列，本元件**併進那條頂列當第二排**、不另外再 sticky 一條浮在內容上（STYLE-DECISIONS Q115 的精神）：`--dialog` 放在 `.payout-dialog__head` 與 `__body` 之間（都在捲動區外；內距 `--sp-6`／`--sp-20` 讓標籤與標題同一條左緣、下緣 1px `--border`）；`--wizard` 放在 `.wizard__top` 裡 `.wizard__top-bar` 之後（寬度對齊 `.wizard__body--narrow` 內容欄、內距 `--sp-28 − --sp-14`）。好處：捲動偏移不必扣固定列高度、右欄預覽卡的 sticky 不會被蓋到。`position: sticky; top: var(--section-tabs-top, 0)` 留作第三種宿主（直接放進捲動區）的後備。

**Behavior（`js/section-tabs.js`）** — `data-st-scroller`（選配）指定捲動容器：`nav.closest` → 同層 `querySelector` → 全文件；沒給就找最近的可捲祖先、再沒有就 window。`data-st-offset`（選配，逗號分隔）列出蓋在捲動區上緣的固定列（建立流程傳 `.wizard__top`），跳轉與定位都扣掉它的下緣；nav 自己坐在捲動區裡時也算一個。點分頁：目標＝段的上緣 − 偏移 − 16px，`scrollTo({behavior:'smooth'})`（`prefers-reduced-motion` 改 `auto`）；捲動途中 scrollspy 讓路，**以「到站」解除**（|scrollTop − 目標| ≤ 2、捲到底、或 `scrollend`），保險 2.5 秒——不用固定毫秒，因為平滑捲動的時長瀏覽器不保證（分頁在背景還會延後起跑），時間猜不準會讓選中態閃回經過的段。scrollspy：scroll 事件 rAF 節流，「上緣 ≤ 偏移＋17px 的最後一段」＝目前段；捲到底就算最後一段（否則太短的末段永遠選不到）。鍵盤：←／→（循環）、Home／End 在分頁間移焦點，Enter／Space 是 `<button>` 原生。`ZtorSectionTabs.init(root?)` 掃 `[data-section-tabs]`、`data-st-ready` 防重複；動態重畫的宿主（bundle-editor 每次 `render()` 換 innerHTML）畫完再呼叫一次。初始化時把自己的高度寫成 `--section-tabs-h` 在捲動容器上（沒有容器就 `<html>`），同一捲動區的其他 sticky 元件可以加進去（create-bundle：`.preview-split { --preview-col-top: calc(96px + var(--section-tabs-h, 0px)) }`）。

**Token usage** — `--control-h-md`（殼高）、`--sp-6`／`--sp-20`／`--sp-28`／`--sp-14`（內距）、`--border`（`--dialog` 下緣線）、`--foreground`（目前段字色）；分頁本體的字級、底線與光暈全部來自 §4.15 Tabs 的 `--underline-short`／`--underline-label`，本檔不重複宣告。

**Consumers** — create-event.html › 第 6 步「新增組合包」彈窗（`js/bundle-editor.js` `sectionTabsHTML()`，`layout:'split'` 專用；三段 `.bd-sec--stack` 掛 `data-st-section`）；create-bundle.html › `.wizard__top` 第二排（三張 `form-section--outlined.cb-sec` 掛 `data-st-section`）。募資兩頁（`layout:'sections'`）沒有這一排。

**Do & Don't**

- ✅ Do 把它併進已有的固定頂列；畫面上只准一條固定列。
- ✅ Do 讓分頁標籤與段落標題用同一組 i18n key（`cpp.bd.sp.sec.*`），改名一次到位。
- ❌ Don't 拿它切換面板——三段要同時在畫面上，否則 scrollspy 沒有東西可以對。
- ❌ Don't 把 `.tabs--underline-short` 直接裸放在頂列裡——底線要靠 44px 的殼落在下緣（Q38 鐵律）。

**CSS** — [`section-tabs.css`](./ds-components/section-tabs.css) ｜ **JS** — `js/section-tabs.js`

### 4.215 Fan event page（粉絲視角活動頁，前台 shop-item 鏡像）

**Purpose** — 發布前預覽確認（主規格 §7.4）用於活動的預覽檢視，D305（2026-09-22）把它從一張活動卡改成「粉絲視角完整頁」：內容等同粉絲端活動頁會呈現的全部設定，活動有票券頁與票務商品頁兩個視圖（沒有組合包時只有票券頁）。**2026-09-22 使用者裁決：「預覽 UI 必須做得和這兩個一模一樣，只是資料和機制套用我們的流程與功能」**（票券頁 `https://ztor.vercel.app/shop-item.html?id=ev-score-live`、票務商品頁 `…?id=ev-themesong-night&sets=1#bundles`）。所以本元件自同日起不再是用站上 token 重刻的結構鏡像，而是**前台的真實 markup 與 CSS**：`js/fan-event-page.js` 輸出前台兩頁 `<main>` 的 DOM 結構與 class 名（證據 `docs/fe-mirror-2026-09-22/ticket.main.html`／`bundles.main.html`），`ds-components/fan-shop.css` 是那兩頁實際命中的全部 CSS 規則（Playwright 抽出，加 `.fep-shop` 前綴、`:root`／`html`／`body` 改 `.fep-shop`、@font-face 移除改用站上自架字型）。只有資料換成我們的：名稱、亮點、描述、卡司、須知、取票方式、票種、組合包（允許票種 × 張數、商品、權益、售價／原價）全部來自建立活動流程或 events-store。兩個宿主——`create-event.html` 發布前、`event-detail.html` 的「預覽與在地化」——共用同一支渲染函式。

**與前台的差別（刻意）** — 前台專有互動拿掉：`pdp-topbar` 保留 markup 但不可點、「情境展示」浮鈕與 `pdp-sheet` 抽屜不畫、購買類按鈕（購票／購買／購買組合／收藏／購物車）`data-pp-inert` 不動作；相關活動三張卡標「非本活動」（`relatedFromStore()` 從 events-store 撈別的活動，宿主呼叫）；亮點（D300）前台沒有位置，放名稱下方一行 `.pdp-buy__highlight`；退換票表單沒有此欄，前台那句當平台固定文案（`fep.refund.fixed`）；描述是一個可翻譯欄位，整段放前台的 lead 段（一個 contenteditable 拆不成 lead＋para）；金額補成前台的「NT$ 1,200」（符號後空格）。細節見 ASSUMPTIONS UIA-167。

**`_layer`** · organism ｜ **source tier** · Project（前台鏡像） ｜ **surface 層** · 自帶前台的近黑檯面 `--bg-primary`（#0a0a0a），坐在 `.payout-dialog__body` 上；殼用 `shell:'fan'` 放寬到 1376px（`.pp-dialog--fan`，見 §4.130）。

**⚠ Token 紀律例外（已註記）** — `fan-shop.css` 是前台鏡像、不是本 design system 的元件：它刻意保留前台的裸 `rgb()`／`rgba()` 與 px 字級（`--fs-*`、`.status-tag` 的 12px…），token 定義在檔頭 `.fep-shop { … }`、不與 `_tokens.css` 混用；**不得在 Creator Studio 其他頁引用**（唯一消費者是 `js/fan-event-page.js`）。check_ds_sync 檢查 5／10 的裸色數已手動登記進 `ds-baseline.json`（`component_css.fan-shop.css`）；前台改了就重抓覆蓋整檔，不手改視覺。

**Anatomy（＝前台 shop-item 的 `<main>`）** — `.fep-shop` > `main` > `.section.section-bg-page.section--flush-top` > `.pdp[data-type=event]` > `.pdp-topbar`（手機頂欄）＋ `.pdp__page` > `.container` > `.page-head__crumbs`（活動 › 類型 › 名稱）＋ `.pdp__grid` > `.pdp-gallery`（`__strip` 縮圖條＋`__stage` > `__frame` 2:3 主圖＋`__thumbs-mobile`）＋ `.pdp-buy`（sticky）> `.pdp-buy__meta`（`.status-tag--green|yellow|cat`：狀態／組合包／類型／地區）＋ `h1.pdp-buy__title`（可翻譯）＋ `.pdp-buy__highlight` ＋ `.pdp-host`（頭像＋主辦名＋「前往創作者商店 ›」）＋ `.pdp-event-meta` > `__row` ×N（日期／時間／時長／場館＋`small` 地址／語言／優先購／限購）＋ `.pdp-event-price`（`__k` 票價｜組合價、`__v` 區間、`__link`「N 種票 ›」｜「N 種組合 ›」）＋ `.pdp-buy__actions`（`.btn--mono.btn--lg.pdp-buy__add` 購票｜選擇組合＋`.pdp-buy__heart`）；`section.pdp-details--event` > `.pdp-details__inner.container` > `h2.pdp-details__heading` ＋ `p.pdp-details__lead`（描述）＋ `.pdp-details__cols` > `__left`（`.pdp-lineup__list > __chip`、`ul.pdp-notices`）＋ `__right`（`.pdp-details__shipping-block` ×2：取票方式、退換票）＋ `details.pdp-terms--tiers[open]`（票券頁：`.pdp-tiers--info > .pdp-tier--info[.is-oos]` > `__main`（`__label`＋`__meta`）＋`__price`＋`.btn--yellow-ghost.btn--sm.pdp-tier__buy`）＋ `details.pdp-terms`（購票條款，收合）；票務商品頁多 `section.pdp-recs--bundles` > `.pdp-recs__headrow`（組合方案＋「只買票 ›」）＋ `.pdp-recs__rail > article.pdp-rec-card--bundle` ×N（`__media`＋`.rf-flag` 組合包、`__cat`「票 ＋ 商品 · 較單買省 NT$ X」、`__title`、`ul.__parts > li[data-kind=ticket|goods]`、`ul.__perks`、`__note`、`__price`＋`s` 原價、`__actions` 購買組合＋購物車）；`section.pdp-recs` 相關活動 > `a.pdp-rec-card` ×3；`.shop-buybar`。CS 端的視圖切換 `.pp-fan-viewbar > .segmented` 畫在 `.fep-shop` 外面。

**Behavior** — 兩個宿主把各自資料整理成同一份 `model`（結構見 `js/fan-event-page.js` 檔頭：`gallery`／`video`／`organizerAvatar`／`bundles[].tickets{names,qty}`／`bundles[].goods[]`／`related[]`）交給本檔渲染，本檔只知道「粉絲頁長什麼樣」、不知道資料來源。可翻譯欄位由宿主給 `{ key }`，本檔以 `api.getValue(key)` 取當前語系值並掛 `api.bindEditable`（非預設語系時 contenteditable，空值 `.is-empty:empty::before` 畫佔位）；價格由 `api.money(priceObj, priceKey)` 依所選幣別格式化（覆寫優先、否則換算，`js/events-store.js` 的 `priceIn`）再補成「NT$ 1,200」；區間「NT$ 1,200 – 2,800」。前台固定文案（票價／日期／場館／購票／組合方案／退換票…）走 i18n `fep.*`（en／zh）、跟介面語言；內容欄跟預覽語系；預覽語系英文時 `.fep-shop[lang=en]` 換 Satoshi／Inter（前台 `html[lang=en]` 同規則）。有組合包時票券頁／票務商品頁可切（`.pp-fan-viewbar` 與頁內「只買票 ›」），購買區與票種節聯動換成組合價與組合方案。

**Token usage** — 全部是前台自己的 token（`--bg-primary`／`--bg-secondary`／`--text-primary|secondary|tertiary|disabled`／`--border-subtle|default|strong`／`--yellow-500`／`--brand-rgb`／`--font-cjk-display|text`／`--font-latin`／`--fs-h2…caption`／`--space-1…12`／`--radius-*`／`--glass-*`／`--lg-*`），定義在 `fan-shop.css` 最前面的 `.fep-shop { … }`；與本站 `_tokens.css` 不混用。
...
### 4.216 Publish stage（發布前預覽確認的全頁畫面）

**Purpose** — D310（2026-09-22）把「發布前看一次」從浮層改成**全頁畫面**：多步驟流程＝流程的最後一步（建立活動第 8 步「預覽與發布」，5.1.6.1 §4.8），發布後再開＝一個獨立全頁（`event-localization.html`，5.1.6.3 §2.13）。兩個宿主共用本元件。取代的是 D223／D305 那層浮層（`partials/publish-preview.js`）在活動側的角色；浮層本身留給 create-product／create-project／publish-work（單頁表單下一輪才改，D310 決定四）。

**`_layer`** · organism ｜ **source tier** · SiteSpecific ｜ **surface 層** · 主區是預覽自己的檯面（`.fep-shop` 前台近黑底），側欄是一張 `--card` 卡（`--border` 邊、`--radius-lg`），坐在 `.wizard__body` 的頁面底上。

**為什麼不是 `wizard-split` 的右軌** — 那條軌是「摘要卡」用的（320、卡片式、唯讀）；這條側欄是控制台：三段清單每一列都可點、底部帶主動作，而且要在沒有 wizard 的獨立頁也成立。sticky 位移走 `--pstage-top`（wizard 宿主設成頂列高度、獨立頁用預設值），不寫死數字。

**Anatomy** — `.pstage`（兩欄 grid，`minmax(0,1fr)` ＋ `--pstage-side-w` 320）> `.pstage__main`（`position:relative`，粉絲視角預覽由宿主的 `previewRender` 畫進來；浮動 `.pstage-edit` 以本欄定位，hover 中的區塊加 `.pstage-zone--hot` 畫品牌橘外框＋淡橘底，2026-09-23）＋ `.pstage__side`（sticky 卡）> `.pstage__side-inner`（可捲）> `.pstage-sec` ×3 > `.pstage-sec__title`（群組小字標：display 字型＋小型大寫＋字距＋muted，Q123）＋ `ul.pstage-list > li > button.pstage-row[.pstage-row--active]` > `.pstage-row__name`（幣別加 `--code` 等寬）＋ `.pstage-row__state`；段底 `.pstage-sec__foot`（還原自動翻譯／逐欄位對照／價格表，各佔滿一列）；檢核段 `.pstage-check` 沿用 `.readiness__list/__item/__mark/__banner`，每列的字換成 `button.pstage-linkbtn`（可點跳回步驟）；未儲存的變更段 `.pstage-sum > __row`（`__k`／`__v`）或 `.pstage-sum__none`；底部 `.pstage__foot`（主鈕＋次鈕）。兩張表**都**掛在 `body` 的 `.drawer.drawer--full`（見 Drawer，2026-09-23 起整頁；先只改翻譯表，同輪使用者追加「價格表也可以統一」，兩表改同一種殼）——**翻譯表**：`table-layout: fixed`、欄位名欄固定 160px（其餘四語系平分，否則會被擠成一字一行、最後一個語系溢出）、`input.pp-cell-field` 高度提到 `--control-h-md`、`textarea.pp-cell-field` 最小高度 `calc(var(--control-h-md) * 4)`、全欄 `vertical-align:top`，欄位名格與唯讀原文格補頂距對齊輸入框第一行；**價格表**：原本為了塞進 880 的 `--wide` 面板把輸入框收到 96、儲存格限寬 112，整頁後改回一般尺度——`table-layout: fixed`、項目欄固定 200px（五幣別平分其餘）、`.pp-price-input` 寬度 100%、高度同升到 `--control-h-md`，項目名格與基準幣別格同樣補頂距對齊；「已覆寫」徽章與「重設為換算值」維持上下疊（兩者都靠右，疊著比並排好掃，這條不隨整頁改變）。表格本身沿用 publish-preview.css 的 `.pp-table`／`.pp-cell` 家族——**消費頁要一併連 `publish-preview.css`**。

**Behavior** — `window.ztorPublishStage.mount(host, { fields, prices, currencies, baseCurrency, views, previewRender, mode, translation, editZones, onEdit, checks, primaryKey, secondaryKey, onPrimary, onSecondary })` → `{ refresh, syncChecks, result, unmount }`。翻譯草稿（DRAFTS／EDITED）與價格草稿（PRICES）是模組變數、**不隨每次 mount 重置**（回去改一個字再回來，改過的翻譯與覆寫都還在）；基準值一變該列覆寫清空重算（§7.15，比照 §7.4）。預覽語系與預覽幣別預設跟宿主走（介面語言／基準幣別），使用者點過才固定。預設語系欄在預覽與翻譯表都唯讀（母本回表單改，D223），其餘語系在預覽裡就地 contenteditable。價格不就地改：預覽裡的價格帶 `data-fep-price-key`（`js/fan-event-page.js`／`js/fan-product-page.js` 掛），點了開價格表並聚焦對應那一格。`mode:'publish'` 時第三段是 `checks()` 給的發布前檢核，沒全過主鈕 disabled；`mode:'save'` 時第三段是未儲存的變更摘要、主鈕「儲存」。`onPrimary` 收到 `{ translations, prices, mode }`。

**`translation` 選項（D312，2026-09-23）** — `'auto' | 'manual'`，選填、預設 `'auto'`。auto（原行為）：切到還沒填的語系時，該欄位的草稿直接鏡射預設語言目前值——等於「自動翻譯」，讀者看不出這是原文還是已確認的譯文。manual：其他語系**不**把原文抄進草稿（未填＝空字串），預覽與翻譯表改用 `displayValue()` 對未填欄位 fallback 顯示原文（畫面上看得到內容、草稿裡仍是空）；側欄語言列狀態字串改為「未翻譯 Not translated」（`pstage.lang.untranslated`）或「已翻譯 n of N translated」（`pstage.lang.translated`，n＝`filledCount(lang)`、N＝`totalFields()`＝翻譯表列數）；翻譯表輸入框在未填格加 `placeholder="{原文}"`（`[data-stage-drawer="trans"] .pp-cell-field::placeholder`，`--muted-foreground`）；還原鈕文案由「還原自動翻譯」（`pstage.lang.reset`）換成「清除譯文 Clear translation」（`pstage.lang.clear`），行為也從「回填原文」改成「清空該語系草稿」。兩種模式共用同一個 `result()` 結構（`translations[key][lang]`，空字串＝未翻譯），差別只在「空字串」這件事會不會被自動填滿。**消費頁**：`create-event.html`、`event-localization.html` 掛 `translation:'manual'`（活動內容需要人工確認翻譯，主規格 §7.4「活動例外：手動翻譯」）；`create-product.html`、`create-project.html` 未傳此選項、沿用 `'auto'`。

**`priceOverrides` 選項（D318，2026-09-23，修訂 D306）** — `true | false`，選填、預設 `true`＝現行行為（各幣別皆可覆寫）；`prices[]` 每列可用 `prices[i].overridable: true | false` 個別覆寫這個全域設定（列級優先於全域，統一由 `rowOverridable(p)` 判斷，`js/publish-stage.js` 第 163–167 行）。`priceOverrides:false`（或該列 `overridable:false`）＝該列非基準幣別全面唯讀、只顯示換算值：無輸入框、無「已覆寫」徽章、無「重設為換算值」（沿用基準格同一套唯讀樣式 `.pp-cell--readonly.pp-cell--base`，不掛鎖圖示——那是 bookyay 鎖定專用，唯讀是電子商店規則本身不給覆寫）；側欄幣別段的「已覆寫 N 項」計數（`overrideCount()`／`overrideTotal()`）跳過唯讀列；全部列都唯讀時價格表提示句改用 `pstage.drawer.price-hint-readonly`（「其他幣別依匯率自動換算，電子商店不可個別調整。」），部分列唯讀時沿用原提示句（對仍可覆寫的列仍成立）；`collectResult()` 唯讀列不輸出覆寫值，即使草稿裡有殘留舊值也不帶出來。**消費頁**：`create-product.html` 與 `product-localization.html` 傳 `priceOverrides:false`（電子商店的定價幣別唯讀，D318 決定 2）；`create-event.html`、`event-localization.html` 未傳此選項、沿用 `true`（活動的票價與組合包維持 D306 可逐幣別覆寫，D318 決定 3）；`create-bundle.html` 列級 `prices[0].overridable = !!ticketMember()`（含票券成員可覆寫、純商品唯讀，D318）、不傳全域 `priceOverrides`；`create-project.html`（D322，2026-09-23）未傳此選項、沿用 `true`（可覆寫）——專案是否應比照電子商店唯讀屬〔產品待確認〕，見主規格 §8.28 第 10 項、ASSUMPTIONS UIA-177。

**Do / Don’t** — Do：把「發布前看一次」做成流程的最後一步或一個獨立頁，讓預覽拿到整個寬度。Do：內容需要人工確認翻譯品質的宿主掛 `translation:'manual'`，不要放任自動鏡射的譯文被當成已確認內容發布。Don’t：這個畫面之後再加一層「確認發布」浮層（本畫面就是那層確認，D310）。Don’t：把「編輯」鈕插進粉絲頁的 markup——那份 markup 是前台 1:1 鏡像，而且可翻譯欄位是 contenteditable，塞進去會被當成內容編輯；用浮動的 `.pstage-edit`。

**Token usage** — `--sp-2/6/8/10/16/20/24/32`、`--card`／`--border`／`--radius-lg`／`--radius-sm`、`--accent`（列 hover）、`--selected-surface`／`--selected-surface-hover`／`--selected-ink`（已選中那一列，Q8）、`--foreground`／`--foreground-muted`／`--muted-foreground`、`--font-display`／`--font-mono`、`--fs-11/12/13`、`--fw-regular`。

**Consumers** — `create-event.html`（第 8 步「預覽與發布」，`mode:'publish'`，`translation:'manual'`，未傳 `priceOverrides`＝`true`，`editZones` 對應步驟 2／3／5／6／7）；`event-localization.html`（活動詳情「預覽與在地化」的全頁版，`mode:'save'`，`translation:'manual'`，未傳 `priceOverrides`＝`true`）；`create-product.html`（「開始販售」驗證通過後的全頁畫面，`mode:'publish'`，未傳 `translation`＝`'auto'`，`priceOverrides:false`，`previewRender`＝ Fan product page，§4.218）；`create-bundle.html`（「下一步：預覽」驗證通過後的全頁畫面，D321，`mode:'publish'`，未傳 `translation`＝`'auto'`，列級 `prices[0].overridable = !!ticketMember()`〔含票券成員可覆寫、只含商品唯讀，D318〕、不傳全域 `priceOverrides`，`previewRender`＝ Fan bundle page，§4.219）；`product-localization.html`（商品／組合商品細節頁「預覽與在地化」的全頁版，D319，`mode:'save'`，未傳 `translation`＝`'auto'`，`priceOverrides:false`，列級 `overridable` 依組合包成員組成給值，`previewRender`＝ Fan product page；`?id=` 畫單售商品、`?type=bundle&id=` 畫組合包，同一頁吃兩種，理由見 ASSUMPTIONS UIA-175）；`create-project.html`（最後一步「預覽與發布」，D322，`mode:'publish'`，`primaryKey` 依 `isFilmFlow()` 切送出審核／發布項目，未傳 `translation`＝`'auto'`，未傳 `priceOverrides`＝`true`，`editZones` 十二個節點，`previewRender`＝ Fan project page，§4.220）；`publish-work.html`（作品上架最後一步「預覽與送審」，D323，`mode:'publish'`，`primaryKey`＝`pw.submit`〔送出審核，D179：送出＝送審〕，未傳 `translation`＝`'auto'`，未傳 `priceOverrides`＝`true`〔基準＝作品自己的定價幣別，F14〕，`editZones` 八個節點，`previewRender`＝ Fan project page（`model.kind:'golive'`、`model.work:true`），§4.220）；`design-system.html` §4.216 就地掛載 demo（`priceOverrides` 未傳＝`true`，示範可覆寫版）。

**CSS** — [`publish-stage.css`](./ds-components/publish-stage.css) ｜ **JS** — `js/publish-stage.js`

---

### 4.217 Calc list（算式列表）

**Purpose** — 一組彼此有加減關係的金額。把運算子畫出來、結果列坐在同一個框裡，「這個數字是那兩個加出來的」用看的就成立；做成一欄一欄的獨立欄位時，關係只存在讀者的腦子裡。2026-09-22（D311）自 `bundle-editor.css` 的 `.bd-calc-list` promote——單張門票彈窗的「價格與數量」與組合包的「原價合計」是同一個視覺角色，站上不留兩份做法（`.bd-calc-list`／`__how`／`__total` 在 bundle-editor.css 暫留成**遷移別名**——`bundle-detail.html` 的定價卡本輪由另一個 session 持有、沒有跟著改 class；canonical 是本元件，該頁遷移的那一輪連同別名一起刪）。

**`_layer`** · molecule ｜ **source tier** · App ｜ **surface 層** · 坐在表單分段或彈窗 body 的面上，靠 1px `--layer-line` 外框成形（不填色——它裡面還有輸入框與唯讀讀數兩種東西，再填一層會變成「填色框裡疊填色控件」）。

**Anatomy**

```
.calc-list[.calc-list--form]        外框（1px --layer-line、--radius-lg）
 └ .kv                              一列（複用 kv-list.css）
    ├ .kv__k                        項目名
    │   ├ .calc-list__op            運算子欄（＋ − ＝），列首固定 --sp-16，壓暗一階
    │   └ .calc-list__how           這一項怎麼算（--fs-12 muted，display:block 自成一行）
    └ .kv__v                        金額（--font-numeric、tabular-nums）；--form 時收成 140px 放控件
 ├ .kv.calc-list__sum               小計列：鍵回 --foreground
 └ .kv.calc-list__total             結果列：鍵回 --foreground、值升到 --fs-16
```

**Variants** — `--form`：列裡有輸入控件時用。唯讀版的 `.kv` 是 baseline 對齊（兩段文字），放進輸入框之後框會比鍵高出半行，所以改 `align-items:center`；右欄收成固定 140px，讓輸入框與底下的唯讀金額右緣對齊（唯讀格 `.calc-list__ro` 補 `--sp-10` 右內距，數字才與框內文字同一條線）。

**Do / Don’t** — Do：只在金額之間**真的有運算關係**時用（加總、扣抵、結果）。Do：一個框裡只放大一次值（結果列），放大兩次就分不出誰是最後一個數字。Don’t：拿它當唯讀事實清單用——那是 KV list（§4.24c）的工作，沒有運算子、沒有結果列。Don’t：把不是金額的欄位（數量、日期）塞進來，運算子會讓人以為它也參與計算——單張門票彈窗的「數量」刻意排在列表**之外**。

**Token usage** — `--layer-line`／`--radius-lg`／`--sp-4/8/10/14/16`／`--fs-12/16`／`--font-numeric`／`--muted-foreground`／`--foreground`。

**Consumers** — `create-event.html`（單張門票彈窗的價格與數量：票價 ＝ 粉絲付 − 平台費 ＝ 創作者實收，`--form`，D316 起沒有手續費那一列；bookyay 帶入者改唯讀版）；`event-detail.html`（票種彈窗，同一組列）；`js/bundle-editor.js` SPLIT 版定價段與 `create-bundle.html`（原價合計，唯讀版）。

**CSS** — [`calc-list.css`](./ds-components/calc-list.css)

---

### 4.218 Fan product page（粉絲視角商品頁，前台 shop-item 商品分支鏡像）

**Purpose** — 建立商品「開始販售」全頁「預覽與發布」（§4.216 Publish stage）用於商品的預覽檢視（D310 決定四，2026-09-23）：Fan event page（§4.215）的商品版，同一套「前台 1:1 鏡像」做法——內容等同粉絲端商品頁會呈現的全部設定，只有一個視圖（商品沒有票券／組合包那種兩視圖的差異）。`js/fan-product-page.js` 讀前台原始碼（`assets/shop-detail-render.js` 的 `renderGoods()`／`goodsBuyHtml()`／`priceBlock()`／`variantsHtml()`／`qtyHtml()`／`deliveryMicroHtml()`／`stockLineHtml()`／`detailsSectionHtml()`／`relatedHtml()`／`buyBarHtml()`）輸出對應的 DOM 結構與 class 名；CSS 是 `ds-components/fan-shop.css` 第 4 段「商品頁元件」，延續第 1–3 段（Fan event page）已建好的 `.fep-shop` 前台 token，不新增裸值例外。只有資料換成我們的——名稱、圖庫、分類、選項（顏色／尺寸）、價格、折扣、庫存、規格、取貨與退貨全部來自建立商品表單；購買類互動一律不動作；元件支援相關商品四張卡（標「非本商品」），但 2026-09-23 起 `create-product.html` 改傳 `related:false` 不畫（B 反饋「你可能也喜歡都移除」，預覽只看這件商品本身）。

**`_layer`** · organism ｜ **source tier** · SiteSpecific ｜ **surface 層** · 與 Fan event page 同一份 `.fep-shop` 前台近黑底。

**⚠ Token 紀律例外（同 §4.215 的既有註記）** — `fan-shop.css` 是前台鏡像、不是本 design system 的元件；第 4 段沿用第 1–3 段已註記的例外（裸 `rgb()`／px 字級，不與 `_tokens.css` 混用），不得在 Creator Studio 其他頁引用（唯一消費者是 `js/fan-product-page.js`）。裸色數量已併入 `fan-shop.css` 既有的 `ds-baseline.json` 登記，未新增例外項目。

**Anatomy（＝前台 shop-item 商品分支的 `<main>`）** — `.fep-shop` &gt; `main` &gt; `.section.section-bg-page.section--flush-top` &gt; `.pdp[data-type=goods]` &gt; `.pdp-topbar`（手機頂欄，不可點）＋ `.pdp__page` &gt; `.container` &gt; `.page-head__crumbs`（商店 › 分類 › 名稱）＋ `.pdp__grid` &gt; `.pdp-gallery[data-fpp-gallery]`（縮圖條＋2:3 主圖，>1 張才有左右箭頭）＋ `.pdp-buy` &gt; `.pdp-buy__meta`（選填徽章）＋ `h1.pdp-buy__title`（可翻譯）＋ `.pdp-host--shop`（店家列，`model.shop` 給了才畫）＋ `.pdp-price[data-fpp-price]`（`__now`／`__was`／`__save`，價格節點帶 `data-fep-price-key`／`data-fpp-price-key`）＋ `.pdp-stock[data-fpp-stock]`（限量且剩 ≤10 件才有）＋ `.pdp-variants[data-fpp-variants]`（`.pdp-variant[data-variant=colour|size]` ×N，顏色＝`.pdp-swatch`、其餘＝`.pdp-size-chip`，`.pdp-variant__guide` 尺寸指南鈕掛第一個非顏色列）＋ `.pdp-buy__actions`（`.pdp-qty` 數量、`.btn--mono.pdp-buy__add` 加入購物車／已售完、`.pdp-buy__heart` 收藏）＋ `.pdp-delivery[data-fpp-delivery]`（交付方式一行）；`section.pdp-details` &gt; `.pdp-details__inner.container` &gt; `h2` 商品詳情＋`p.pdp-details__lead`（描述，可翻譯）＋`.pdp-details__cols` &gt; `__left`（`.pdp-details__specs-block[data-fpp-specs]` &gt; `dl.pdp-specs`）＋`__right`（`.pdp-details__shipping-block[data-fpp-returns]`，`model.returns` 有值才畫）；`section.pdp-recs` 你可能也喜歡 &gt; `a.pdp-rec-card` ×4（標「非本商品」）；`.shop-buybar`（手機底部購買列）。

**Behavior** — 宿主（`create-product.html`）把表單資料整理成 `model`（結構見 `js/fan-product-page.js` 檔頭：`category`／`badge`／`gallery`／`name`／`shop`／`price`／`variants`／`discountPct`／`stock`／`sizeGuide`／`delivery`／`desc`／`specs`／`returns`／`related`）交給本檔渲染，本檔只知道「粉絲商品頁長什麼樣」、不知道資料來源。可翻譯欄位（`name`／`desc`）由宿主給 `{ key }`，本檔以 `api.getValue(key)` 取當前語系值並掛 `api.bindEditable`；單一選項給一個價格，多選項取逐組合最低價、價格不同時加「起」（`fpp.price.from`）；折扣 `discountPct > 0` 才畫折後價＋劃線原價＋「省 N」（`fpp.price.save`），換算後的幣別也照同比例打折。庫存只在限量且剩 ≤10 件時顯示「僅剩 N 件」（`fpp.stock.left`），0 件時 CTA 轉「已售完」（`fpp.cta.soldout`）並停用。選項：顏色畫色票（`swatchStyle()` 取色號，沒有色號退回 `--border-strong`）、其餘畫尺寸格；有尺寸指南時掛在第一個非顏色選項列（`.pdp-variant__guide`），沒有這種列就自成一列（`.pdp-variant--reflone`）。價格不就地改：`data-fep-price-key`／`data-fpp-price-key` 供宿主的 `js/publish-stage.js` 認得，點了開價格表並聚焦對應那一格（與 Fan event page 同一機制）。`related` 為真時畫相關商品固定四張佔位卡（`items` 給陣列時取前 4 筆，否則畫 4 張空卡），全部標「非本商品」；`related:false`（`create-product.html` 目前值，2026-09-23 起）或未傳時整段 `section.pdp-recs` 不畫。

**Token usage** — 與 Fan event page 同一批前台 token（定義在 `fan-shop.css` 最前面的 `.fep-shop { … }`），第 4 段新增的類別沿用同一批、無新 token。

**Consumers** — `create-product.html`（「開始販售」驗證通過後的全頁「預覽與發布」，`previewRender` 呼叫 `buildFanProductModel()`）；`product-localization.html`（細節頁的「預覽與在地化」，D319，`previewRender` 呼叫 `productModel()`；`?type=bundle` 時改呼叫 `bundleModel()`——組合包沒有專屬的粉絲頁渲染器，封面＝組合展示圖、價格＝組合售價（折扣走 `discountPct`）、庫存＝可售套數、規格表＝內容物一列＋逐項額外權益，呈現假設見 ASSUMPTIONS UIA-175）；`design-system.html` §4.218 就地渲染 demo（替身 api）。

**CSS** — [`fan-shop.css`](./ds-components/fan-shop.css)（第 4 段）｜ **JS** — `js/fan-product-page.js`

---

### 4.219 Fan bundle page（粉絲視角組合包頁，前台 shop-item 兩個組合分支鏡像）

**Purpose** — 建立組合「下一步：預覽」後全頁「預覽與發布」（§4.216 Publish stage）用於組合包的預覽檢視（D321，2026-09-23，比照 D317 建立商品改兩步驟）：Fan event page（§4.215）／Fan product page（§4.218）的組合版，同一套「前台 1:1 鏡像」做法——內容等同粉絲端組合包頁會呈現的全部設定，依成員組成分兩種視圖：純商品套組（`kind:'goods'`，前台 `renderBundle`）與含活動票券組合（`kind:'eventset'`，前台 `renderEventSet`，多一段「座位區域」與活動資訊框）。`js/fan-bundle-page.js` 讀前台原始碼（`assets/shop-detail-render.js` 的 `renderBundle()`／`bundleBuyHtml()`／`bundlePickerHtml()`／`bundleContentsHtml()`／`renderEventSet()`／`setCrumbsHtml()`／`setBuyHtml()`／`setTiersHtml()`／`setContentsHtml()`／`setPerksHtml()`）輸出對應的 DOM 結構與 class 名；CSS 是 `ds-components/fan-shop.css` 第 5 段「組合包頁元件」，延續第 1–4 段（Fan event page／Fan product page）已建好的 `.fep-shop` 前台 token，不新增裸值例外。只有資料換成我們的——名稱、素材、成員（票券成員的允許票種 × 每組張數、商品成員的規格）、售價、原價合計、組合折扣、可售套數、組合專屬權益全部來自建立組合表單；購買類互動一律不動作；元件支援相關推薦四張佔位卡（前台 markup），但 2026-09-23 起 `create-bundle.html` 不傳 `related`、該段不畫（B 反饋「你可能也喜歡都移除」，與 Fan event page／Fan product page 同輪裁決）。

**`_layer`** · organism ｜ **source tier** · SiteSpecific ｜ **surface 層** · 與 Fan event page／Fan product page 同一份 `.fep-shop` 前台近黑底。

**⚠ Token 紀律例外（同 §4.215／§4.218 的既有註記）** — `fan-shop.css` 是前台鏡像、不是本 design system 的元件；第 5 段沿用第 1–4 段已註記的例外（裸 `rgb()`／px 字級，不與 `_tokens.css` 混用），不得在 Creator Studio 其他頁引用（唯一消費者是 `js/fan-bundle-page.js`）。裸色數量已併入 `fan-shop.css` 既有的 `ds-baseline.json` 登記，未新增例外項目。

**Anatomy（＝前台 shop-item 兩個組合分支的 `<main>`）** — `.fep-shop[data-fbp-view=bundle|eventset]` &gt; `main` &gt; `.section.section-bg-page.section--flush-top` &gt; `.pdp[data-type=bundle|eventset]` &gt; `.pdp-topbar`（手機頂欄，不可點）＋ `.pdp__page` &gt; `.container` &gt; `.page-head__crumbs`（純商品套組＝商店 › 名稱；含票券組合＝活動 › 活動型態 › 活動名 › 組合名）＋ `.pdp__grid` &gt; `.pdp-gallery[data-fbp-gallery]`（組合自己的素材，結構同商品頁縮圖條＋主圖）＋ `.pdp-buy` &gt; `.pdp-buy__meta`（狀態／類型徽章）＋ `h1.pdp-buy__title`（可翻譯）＋ 店家列（`goods`，`.pdp-host--shop`）或活動 kicker＋主辦列（`eventset`，`.pdp-set__host`）＋（`eventset` 才有）`.pdp-event-meta[data-fbp-meta]`（內容／日期／時間／場館／專屬權益逐列）＋ `.pdp-price[data-fbp-price]`（`__now`／`__was`／`__save`，價格節點帶 `data-fep-price-key`／`data-fbp-price-key`）＋ `.pdp-stock[data-fbp-stock]`（限量且剩 ≤10 組才有，「僅剩 N 組」）＋（`goods` 才有）`.pdp-bundle-picker[data-bundle-picker][data-fbp-picker]`（多選項商品成員的規格 chip，不預選）＋ `.pdp-buy__actions`（`eventset`＝「購買組合」單鈕；`goods`＝有選項＝「選擇你的組合」、無選項＝「加入套組」，皆帶數量步進器與收藏鈕；售完＝已售完並停用）；（`eventset` 才有）`section.pdp-bundle--tiers[data-fbp-tiers]`「座位區域 · 任選一種」（票券成員允許票種 >1 才畫，每列＝該票種的組合價＋購買鈕）；`section.pdp-bundle[data-fbp-members]`「套組內容 · 共 N 件」&gt; `.pdp-bundle__grid` &gt; `.pdp-bundle-item` ×N（一個成員一張卡：媒體格＋名稱＋規格說明＋查看單品／查看活動連結）；`section.pdp-perks[data-fbp-perks]`「組合專屬權益 · 共 N 項」&gt; `ul.pdp-perks__list` &gt; `li.pdp-perks__item`（星形圖示＋可翻譯文字）；`section.pdp-details[data-fbp-desc]`「組合介紹」&gt; `p.pdp-details__lead`（組合自己的說明，可翻譯；`model.desc` 留空整段不畫）；`section.pdp-recs` 相關推薦 &gt; `a.pdp-rec-card` ×4（`related` 有值才畫，全部標「非本商品／非本活動」）；`.shop-buybar`（手機底部購買列）。兩種 `kind` 的區塊順序不同：`goods` 是套組內容→權益→詳情；`eventset` 是說明→座位區域→套組內容→權益（前台 `renderBundle`／`renderEventSet` 的原始順序）。

**Behavior** — 宿主（`create-bundle.html`）把表單資料整理成 `model`（結構見 `js/fan-bundle-page.js` 檔頭：`kind`／`gallery`／`name`／`desc`／`shop`／`event`／`members`／`price`／`stock`／`perks`）交給本檔渲染，本檔只知道「粉絲組合包頁長什麼樣」、不知道資料來源。可翻譯欄位（`name`／`desc`／逐項 `perks`）由宿主給 `{ key }`，本檔以 `api.getValue(key)` 取當前語系值並掛 `api.bindEditable`。`members` 每筆 `{ kind:'ticket', qty, tiers }` 或 `{ kind:'product', name, qty, priceObj, options }`；票券成員允許票種 >1 時視為「粉絲任選一種」（購買欄與套組內容卡都標「任選」）。價格：`price.priceObj`／`price.priceKey` 是組合售價（最低組合），`price.wasObj` 是原價合計（單買加總，只換算不可覆寫），`price.from` 為真時加「起」；座位區域每列的價格＝組合售價＋該票種與最低票種的價差 × 每組張數 × (1 − 組合折扣 %)，差額依匯率換算、不吃該列自己的覆寫（同表單「粉絲看到的」卡的算法）。庫存 `stock` 為 `null` 或空即不限量不顯示，≤10 組才顯示「僅剩 N 組」，0＝售完（CTA 停用）。價格不就地改：`data-fep-price-key`／`data-fbp-price-key` 供宿主的 `js/publish-stage.js` 認得，點了開價格表並聚焦對應那一格（與 Fan event page／Fan product page 同一機制）。`related` 為真值時畫相關推薦固定四張佔位卡；`create-bundle.html` 目前不傳（未定義／`false`）時整段不畫。

**Token usage** — 與 Fan event page／Fan product page 同一批前台 token（定義在 `fan-shop.css` 最前面的 `.fep-shop { … }`），第 5 段新增的類別沿用同一批、無新 token。

**Consumers** — `create-bundle.html`（「下一步：預覽」驗證通過後的全頁「預覽與發布」，`previewRender` 呼叫 `buildFanBundleModel()`，幣別覆寫依 D318 用 `prices[0].overridable = !!ticketMember()` 列級旗標——含票券成員可覆寫、只含商品唯讀，不傳全域 `priceOverrides`）；`design-system.html` §4.219 就地渲染 demo（替身 api，`kind:'goods'`／`kind:'eventset'` 各一份）。

**CSS** — [`fan-shop.css`](./ds-components/fan-shop.css)（第 5 段）｜ **JS** — `js/fan-bundle-page.js`

---

### 4.220 Fan project page（粉絲視角專案頁，前台共創／預購頁與標題頁鏡像）

**Purpose** — 建立專案最後一步「預覽與發布」（§4.216 Publish stage）用於專案的預覽檢視（D322，2026-09-23，比照 D310 建立活動全頁化、D317／D321 建立商品／組合改兩步驟）：Fan event page（§4.215）／Fan product page（§4.218）／Fan bundle page（§4.219）的專案版，同一套「前台 1:1 鏡像」做法——內容等同粉絲端專案頁會呈現的全部設定，依 `model.kind` 分三種：直接發行（`golive`，前台 `title.html` 的 `.detail-hero`＋逐段 `.detail-section`）、共創（`fund`）與預購（`preorder`，兩者共用前台 `cocreate-project.html` 的 `.cocreate-detail`）。`js/fan-project-page.js` 讀前台原始碼（`cocreate-project.html`：`?kind=music&state=active`／`?kind=preorder&state=active`；`title.html`）輸出對應的 DOM 結構與 class 名；CSS 是 `ds-components/fan-shop.css` 第 6 段「專案頁元件」，延續第 1–5 段（Fan event page／Fan product page／Fan bundle page）已建好的 `.fep-shop` 前台 token，不新增裸值例外。只有資料換成我們的——名稱、海報、圖庫、關鍵字、描述或故事、方案（回饋套組／預購方案）、預算分配、交付時程全部來自建立專案表單；購買／支持類互動一律不動作；剛發布的專案一律畫成「計畫進行中、0 人支持、0%」；不畫相關推薦、留言討論、浮動支持條、製作進度時間軸（建立流程沒有這批資料）。**2026-09-23（D323）作品上架流程重用**：`publish-work.html` 的最後一步「預覽與送審」沿用本元件（只用直接發行影片版型，`model.kind:'golive'`），新增三個選填 model 欄位供這個消費頁使用——`model.work`（布林旗標，作品模式下沒有故事也沒有條列時不畫空的「關於這部作品」段）、`model.credits`（有值才畫「創作團隊」段）、`model.clips`（有值才畫「預告與花絮」段）；create-project.html 不傳這三欄，畫面與先前一致。

**`_layer`** · organism ｜ **source tier** · SiteSpecific ｜ **surface 層** · 與 Fan event page／Fan product page／Fan bundle page 同一份 `.fep-shop` 前台近黑底。

**⚠ Token 紀律例外（同 §4.215／§4.218／§4.219 的既有註記）** — `fan-shop.css` 是前台鏡像、不是本 design system 的元件；第 6 段沿用第 1–5 段已註記的例外（裸 `rgb()`／px 字級，不與 `_tokens.css` 混用），不得在 Creator Studio 其他頁引用（唯一消費者是 `js/fan-project-page.js`）。裸色數量已併入 `fan-shop.css` 既有的 `ds-baseline.json` 登記，未新增例外項目。

**Anatomy（＝前台共創／預購頁與標題頁的 `<main>`）** — `.fep-shop[data-fpj-view=golive|fund|preorder]` &gt; `main` &gt; 共創／預購（`.cocreate-detail`）：`.detail-back`（固定頂端返回列，標題頁沒有）＋ `.detail-hero` &gt; `.detail-hero__bg`（模糊背景，海報為圖片時才有）＋ `.container.container--wide.detail-hero__inner` &gt; `.detail-head` &gt; `.detail-head__poster[data-fpj-poster]`（2:3 海報，空時畫圖示佔位）＋ `.detail-head__main` &gt; `.detail-titlebar` &gt; `h1.detail-title[data-fpj-title]`（可翻譯，共創／預購另帶 `.cocreate-cat-tag` 分類標籤）＋（有 `info` 連結時）`.detail-titlebar__aside`＋（`m.creator` 有值才有）`.cocreate-hero__by`（發起人，字首圓头像）＋ `.detail-meta.cocreate-hero__status-line`（「計畫進行中」＋資訊列）＋（`m.keywords` 有值才有）`.detail-keywords[data-fpj-keywords]`＋ `fundPanel()`（`.cocreate-fund[data-fpj-fund]`：金額列／募資或預購目標／進度條／倒數與期間說明／「我要支持」＋預告片＋收藏分享）；`.detail-body` &gt; （`m.tiers` 有值才有）`tiersSection()`（`.reward-tiers[data-fpj-tiers]` &gt; `.reward-tier` ×N：名稱／價格／權益清單／支持人數／名額或不限量／「選擇 →」）＋ `.detail-section[data-fpj-about]`「關於這部作品／關於這些商品」（`.cocreate-prose[data-fpj-desc]`＋選填 `metaList()`）＋ `.detail-section`「共創計畫說明／預購計畫說明」（`.cocreate-prose[data-fpj-story]`）＋（`m.delivery` 有值才有）`deliverySection()`（`[data-fpj-delivery]`：預期交付就地日期欄 `[data-fpj-delivery-input]`＋選填 `.project-timeline` 三態里程碑）＋（`kind==='fund'` 且 `m.budget` 有值才有）`budgetSection()`（`.budget-split[data-fpj-budget]`：分段長條＋逐分類百分比與換算金額圖例）＋（`m.media` 有值才有）`.detail-section[data-fpj-media]`「概念圖／照片／劇照」（`.media-grid` 最多 4 張）。直接發行（`.title-detail`）：`.detail-hero` &gt; `.detail-head` &gt; `.detail-head__poster[data-fpj-poster]`＋ `.detail-head__main` &gt; `h1.detail-title[data-fpj-title]`＋（`m.facts` 有值才有）`.detail-meta`（年份／分級／片長／語言）＋（`m.keywords` 有值才有）`.detail-keywords[data-fpj-keywords]`＋ `.detail-desc[data-fpj-desc]`（可翻譯）＋ `actionsRow()`（`.detail-actions[data-fpj-price]`：預告片鈕＋價格鈕 `.detail-price`，價格節點帶 `data-fep-price-key`／`data-fpj-price-key`，或免費時「立即觀看」）；`.detail-body` &gt; `.detail-section[data-fpj-about]`「關於這部作品」（`m.story` 有值時 `[data-fpj-story]` 故事段＋選填 `metaList()`）＋（`m.media` 有值才有）`.detail-section[data-fpj-media]`＋（`m.credits` 有值才有）`creditsSection()`（`[data-fpj-credits]`「創作團隊」：逐角色一列，角色名走介面字、人名原樣）＋（`m.clips` 有值才有）`clipsSection()`（`[data-fpj-clips]`「預告與花絮」：卡片式縮圖列，有影格用影格、否則退回主視覺、再無留空底＋`play` 圖示；D323，僅 publish-work.html 這條路徑傳）。

**Behavior** — 宿主（`create-project.html`／`publish-work.html`）把表單資料整理成一份 `model`（結構見 `js/fan-project-page.js` 檔頭：`kind`／`film`／`work`／`name`／`desc`／`story`／`creator`／`poster`／`media`／`hasTrailer`／`facts`／`langLabel`／`langValue`／`keywords`／`meta`／`access`／`price`／`fund`／`delivery`／`tiers`／`budget`／`credits`／`clips`）交給本檔渲染，本檔只知道「粉絲專案頁長什麼樣」、不知道資料來源；`work`／`credits`／`clips` 三個選填欄位僅 D323（publish-work.html）傳入，見上方 Purpose 與 Anatomy。可翻譯欄位（`name`／`desc`／`story`／逐方案 `name`／`benefits`／逐預算分類 `label`）由宿主給 `{ key }`，本檔以 `api.getValue(key)` 取當前語系值並掛 `api.bindEditable`。金額一律用前台「代碼＋空格＋千分位」寫法（`codeMoney()`），不補符號。共創／預購募資面板固定顯示 0 人支持、0%、進度條 `--bar-fill:0%`——剛建立的專案沒有真實募得數字。交付時程的預期交付日期是本檔唯一的表單控件（`<input type="date" data-fpj-delivery-input>`），宿主監聽其 `change` 寫回自己的單一來源（`#cpp-delivery` 或預購的 `#po-delivery`）並呼叫 `stage.refresh()`；未填時顯示提醒句「必填 · 粉絲在支持前會看到這個日期」。預算分類配色依序輪替站上既有圖表色（不沿用前台固定三色，分類數量與名稱由創作者自填、不固定）。價格不就地改：`data-fep-price-key`／`data-fpj-price-key` 供宿主的 `js/publish-stage.js` 認得，點了開價格表並聚焦對應那一格（與 Fan event page／Fan product page／Fan bundle page 同一機制）。本檔不畫相關推薦、留言討論、浮動支持條與製作進度時間軸（建立流程階段沒有對應資料，不生造佔位內容）；創作團隊改為選填（`m.credits` 有值才畫，見上方 Anatomy），create-project.html 不傳、publish-work.html（D323）傳。

**Token usage** — 與 Fan event page／Fan product page／Fan bundle page 同一批前台 token（定義在 `fan-shop.css` 最前面的 `.fep-shop { … }`），第 6 段新增的類別沿用同一批、無新 token。

**Consumers** — `create-project.html`（最後一步「預覽與發布」驗證通過後的全頁畫面，D322，`previewRender` 呼叫 `buildFanModel()`）；`publish-work.html`（作品上架最後一步「預覽與送審」，D323，`previewRender` 呼叫同名 `buildFanModel()`，傳 `model.work`／`model.credits`／`model.clips`）；`design-system.html` §4.220 就地渲染 demo（替身 api，直接發行與共創各一份；demo 未涵蓋 D323 的 `work`／`credits`／`clips` 三欄）。

**CSS** — [`fan-shop.css`](./ds-components/fan-shop.css)（第 6 段）｜ **JS** — `js/fan-project-page.js`
