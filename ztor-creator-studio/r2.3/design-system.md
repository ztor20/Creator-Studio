...
**Consumers** — `create-event.html`（發布前，`previewRender` 呼叫 `buildFanModel()`，`shell:'fan'`）；`event-detail.html`（「預覽與在地化」，`previewRender` 呼叫 `locModel()`，`shell:'fan'`）；`design-system.html` §4.215 就地渲染 demo（替身 api）與 §4.130 彈窗 demo。墓碑：`ds-components/fan-event-page.css`（`.fep` 重刻版，2026-09-22 同日退場，同日建同日廢——使用者裁決鏡像前台後沒有任何消費者）。
