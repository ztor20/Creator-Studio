...
**小結**：19 支中判定為「無法用既有 token 表達、應保留」的例外原共 6 處（brand-card、checkbox、media-vault ×4、fans-guide ×2、sticky-actions、chart-tip）；**sticky-actions 已於 2026-08-28 ztorUI 換裝收斂進 `--ztu-shadow-pop-up`，現餘 5 處**；`detail-sheet.css:34` 判定為**應收斂**、非合理例外，記錄於此供下一輪 CSS 治理處理（本次僅記錄，未動 CSS）。棘輪基準 `ds-baseline.json` 是否要為上述合理例外調高對應檔案的允許值，由下游負責 CSS／baseline 的 agent 決定。
...
| Next step | 🟡 molecule | ✓ App | 一句話＋一顆主要動作，放在卡的最後一段；與 `.info-banner`（純說明）、`.insight-row`（算出來的結論）的分工是「只有這一支帶動作」 | [next-step.css](./ds-components/next-step.css) |
...
| Issue panel | 🟠 organism | ✓ App | 待處理事項面板：把「還等著創作者處理的事」整批攤開，依來源模組分組、一次展開一則（2026-08-28 promote，使用者裁決把儀表板同一份 `dash-alerts` 的三個出口——一條摘要通知條 `.alerts-strip`、「待處理事項」彈窗 `ops-pending`、「查看全部」彈窗 `todo-all`——併成單一頁內面板，三者同日移除）。資訊層次：面板標題列（可收合）→ 分組（模組 icon＋模組名＋筆數）→ 事項卡（計數徽章＋標題＋相對時間＋展開鈕）→ 展開後的受影響細項清單（每項左緣一條細直線）＋一行「下一步」。**表面律（Q77）：卡面一律中性**——`--ztu-film` ＋ `--nest-line` 一條細邊、不上 backdrop 模糊（配方同 `.card--muted`，巢狀層再模糊只會取樣到自己的母面）；嚴重度只出現在 `__count` 那顆染色膠囊上，沿用 badge.css 的 `--error`／`--warning`／`--info`，本元件不另造色票。參照圖給的紅底卡沒有照做：五張紅底同時出現讀起來是「整個工作站在燒」，實際上那是五件各有下一步的例行事務。面板本身是頁面級表面，玻璃四件套直接吃 `.card`，本檔只補排版與焦點樣式（`tabindex="-1"` ＋ `:focus-visible`，因為 F2 的「待處理事項」KPI 卡點了會捲到它並把焦點交給它——只捲不移焦點會把鍵盤使用者留在原地）。**開合**：兩層都是原生 `<button aria-expanded>` ＋ `aria-controls`，CSS 只轉 chevron、顯示與否交給 `hidden`；次要控制鈕（`__aside` 的暫緩／鎖頭）與 toggle **並排**不內嵌（按鈕不能包按鈕，包了讀屏會把整列唸成單一控制項）。**筆數口徑**：面板總數與各組筆數一律排除已軟關（snoozed）的項目，與 F2 那顆同名 KPI 對齊；已軟關的卡照樣渲染、`--snoozed` 降階並沉到組底（spec 5.1.1 §F4「當下不需要處理但仍要看得到」）。**「下一步」那一行不是推薦**：站上沒有推薦引擎，文字與去處都取自該筆告警既有的 `cta`／`ctaHref`（ASSUMPTIONS ISSUE-001）。與鄰居的分工——`alert.css` 的 `.alert--card` 是「單則通知」一則一張、彼此獨立；`collapse-group.css` 是表單欄位的折疊盒、折疊態只留一行摘要值、只有一層；`accordion.css` 是 FAQ 語彙、整列都是文字沒有徽章與右側控制鈕。三者都保留、不互相取代。渲染器 `data-component="issue-panel"`（js/components.js），資料集 `dash-alerts`；細項 `details[]` 與相對時間 `ago` 由資料端提供，渲染器不生成任何產品事實。消費：儀表板 F4。 | [issue-panel.css](./ds-components/issue-panel.css) |
...
|---|---|---|
| default | — | **ztorUI 換裝（2026-08-28 Wave 2b）**：`background: var(--ztu-accent-surface)`（78% 橘漸層玻璃）＋`backdrop-filter: var(--ztu-accent-blur)`＋`box-shadow: var(--ztu-accent-glow)`（四層一次寫完的光暈）＋`::before` 霧光色層（`var(--bloom-surface)`，內縮 3px 模糊 5px），text `--primary-foreground`。取代舊值 `background: var(--primary)`＋`box-shadow: var(--shadow-raise)` 的實色方塊。⚠ 已知取捨：白字疊在 78% 橘玻璃上實測約 2.4:1，低於 WCAG AA 4.5:1，2026-08-26 使用者已裁決接受 |
| hover | `:hover` | bg 不換色票（漸層無「亮一階」對應值），改 `filter: brightness(1.07)` 整顆提亮；outline tints to `--muted`; ghost gets `color-mix(--foreground 6%)` fill |
...
- primary bg `--ztu-accent-surface`（2026-08-28 起，見 §1.1b） · primary blur `--ztu-accent-blur` · primary glow `--ztu-accent-glow` · `::before` 霧光 `--bloom-surface` · text `--primary-foreground` · outline surface `--card` / `--muted` · outline border `--border`（平面，無 shadow） · ghost text `--foreground-muted` → `--foreground` · radius `--radius` (primary) / `--radius-md` (outline) · focus ring `--ring` · motion `--duration` / `--easing` · font `--font-ui`（Poppins，見 §1.2）
...
| 浮層內的 `.card`（如 `.payout-dialog .card`） | `:is(.payout-dialog, .payout-modal, .dropdown__menu, .zselect__panel, .preview-panel__sheet, .detail-sheet, .overlay-surface) .card` | **ztorUI 換裝**：退回 `--ztu-film` 薄膜＋關模糊＋`border: 1px solid var(--nest-line)`＋無陰影——浮層本身已是實色（見 §3.2），裡面再放一張會透的玻璃卡就是看穿兩層玻璃；祖先清單本波從原本只有 `.payout-dialog` 擴編成本站所有實色浮層容器 |
...
- `.section-nav-layout` 兩欄 grid（168px 導覽＋內容），900px 以下疊成上下、導覽改橫向可捲。
