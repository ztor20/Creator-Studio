...
| Detail rail | 🟠 organism | ✓ Project | 詳情頁兩欄殼：可編輯主欄（Tabs＋分頁）＋ sticky 唯讀 meta 右欄（1fr / 300px，≤1100px 收單欄並取消 sticky）。元件段只講殼；「何時用、右欄放什麼」見 Pillar 5 §5.1 *Detail + persistent rail*。詳見 §4.52。**`.detail-cards`（2026-09-09）**：一個分頁的主體需要整頁寬時（首個消費者＝商品明細「銷售設定」，逐規格表有八欄），原本擺在右欄的設定卡改放主體下方——**一張一行、各自吃滿整頁寬**（2026-09-09 使用者裁示；並排會多一條垂直切分線，讓三張互不相關的設定看起來像同一組），卡距沿用 `--sp-16`。單欄 grid 而不是裸堆疊，是為了讓卡距走同一個 gap、不靠 margin 疊加 | [detail-rail.css](./ds-components/detail-rail.css) |
...
| Stock split | 🟡 molecule | ✓ App | 庫存分配長條（2026-09-10 · D258）：一條看完「這些庫存分給誰」——一段一個販售管道（單售 `--chart-2`／組合包 `--chart-5`），末段是還留在庫存池、還沒分出去的量（薄膜色）。底下 `__legend` 逐段帶數字，所以長條只負責比例、不負責讀數。**它取代了「已鎖定」那一格數字**：已鎖定恆等於目前在庫減未鎖定，三個數字裡永遠有一個是另外兩個的差，讀的人得先做減法才知道比例。與 `stock-bar` 分工——那支是「賣掉多少／還剩多少」的單段進度（限量才有分母），本元件是「同一批庫存分給哪些管道」的多段分配，任何商品都成立。顏色走圖表色票不走狀態色：段落是分類、不是好壞。消費頁：商品明細「當前庫存」卡頭。 | [stock-split.css](./ds-components/stock-split.css) |
