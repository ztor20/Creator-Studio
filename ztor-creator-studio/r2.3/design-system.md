...
**View mode** — 坐在 `[data-mode="view"]` 面板裡時，`shared.css` 的「藏掉所有按鈕」白名單自 2026-09-11 起直接放行 `.zselect__trigger`（此前要逐顆 `<select data-view-safe>`，漏一顆就少一個欄位）；唯讀時觸發鈕跟著原生 select 一起 `disabled`，只顯示目前值。
