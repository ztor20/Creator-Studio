...
**Anatomy**
- `.rowdis__group` — 群組容器，包住「母列＋它的子項」（新增這一層 wrapper 是 L3 模型的核心）。收合無樣式；展開時（`:has()` 偵測內部把手 `aria-expanded="true"`）成為 L2 薄膜面板——`--ztu-film` 底、四周一圈 inset box-shadow 畫的 1px `--border-soft`、`--radius-lg` 圓角，面板內部的列底線轉透明
  - `--gutter`（清單型容器要加）— `.data-list__row` 本身左右無內距，由這個變體負外距先讓面板往外長、再用等值內距把列推回原本的 x 位置，欄位起點因此完全不動。表格型（`.product-list__row` 自帶 `--sp-10` 左右內距）不加這個變體，面板邊緣與資料列外緣重合就是對的
...
**顯示於商店不在第 2 層裡（2026-09-04 · D247）** — 第 2 層 `.lctl__derived` 裝的是「上架這件事的排程」（什麼時候上、什麼時候下）；顯示是上架成立之後的另一個問題，擠進同一個盒子會被讀成排程的一部分。改成薄膜之外的獨立列：沒有揭示內容時用 [`control-row`](#control-row)（有外框的單列），隱藏時要長出非公開連結或說明的頁面用 [`control-group`](#control-row)（外框列＋揭示區同一框）。上架沒成立時它與第 2 層一起收起。
...
- `.lctl__derived[hidden]` — 第 2 層附屬設定（2026-09-04 · D245）：主決定成立之後才存在的一組設定。外觀套既有的線框盒 `.control-group.control-group--plain`（1px 內描邊、無填色、卡片級圓角；2026-09-04 使用者裁示由填色的 `.card.card--muted` 改成線框），本類只管內部節奏（`gap`）與把 `.field` 的下邊距交回容器。markup：`<div class="control-group control-group--plain lctl__derived">`
