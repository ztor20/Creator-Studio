...
| Stock allocation | 🟠 organism | ✓ App | 一個庫存池怎麼分給各販售管道（§7.14 · D241）：目前在庫一列、每個管道一列（名稱＋鎖定量＋該管道可售量）、末列沒有被鎖定的庫存量。逐列各有自己的可售量，因為鎖定的管道不吃沒有被鎖定的庫存量；所有管道都鎖定時末列掛一顆 chip 說明剩幾件沒人能賣。變體 `--bundle` 換成組合視角（一列一個成員，多一欄成員沒有被鎖定的庫存量）。詳見 §4.207 | [stock-allocation.css](./ds-components/stock-allocation.css) |
...
| Stat row | 🟡 molecule | ✓ App | 卡內大數字排（取代把 `.kpi` 塞進卡片）；`--inline` 是把同一排收成「欄名與值同一行、字級小」的標籤化變體（2026-09-02；同日短暫存在過的 `--compact` 已退場，墓碑在 CSS 檔內） | [stat-row.css](./ds-components/stat-row.css) |
...
<span class="badge badge--outline">Paid</span>          <!-- 帳的搬動：提款／退款／扣款 -->
...
| `.ztor-table__row--ledger` | 交易明細的帳務異動列（提款／退款／扣款，2026-09-05 · D250）：整列降到 `--muted-foreground`。它回答的是「餘額為什麼變了」，不是「賺了多少」，份量低於收入列。選擇器帶 `tbody` 才壓得過 `.ztor-table tbody td` |
...
| `.tx-na` | 不適用欄位的「—」。**不填 −$0.00 或 0**——填零會被讀成「這筆真的扣了 0 元」。提款與退款沒有逐筆扣項，那兩格就是不適用 |
| `.tx-detail__kv` | 展開層的鍵值摘要（提款／退款／扣款用；收入事件用的是 `__ladder` 瀑布），複用 `kv-list.css` 的 `.kv`，這裡只給寬度上限與較緊的列距 |
