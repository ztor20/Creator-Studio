...
| Stat row | 🟡 molecule | ✓ App | 卡內大數字排（取代把 `.kpi` 塞進卡片）；`--inline` 是把同一排收成「欄名與值同一行、字級小」的標籤化變體（2026-09-02；同日短暫存在過的 `--compact` 已退場，墓碑在 CSS 檔內）。**2026-09-09 第二個消費者**：商品明細「當前庫存」卡頭——目前在庫（限量時 `__of` 帶「/ 上限」）／已鎖定／未鎖定（`__meta` 放「N 件沒人拿得到」提醒）／單售可售；原本的超大 `stock-readout`、管道分配表的池列與限量「上限數量」欄位都收進來，同一個數字只講一次 | [stat-row.css](./ds-components/stat-row.css) |
...
| Film picker | 🟡 molecule | ✓ App | 電影關聯可搜尋多選（spec 5.1.5.2 §4.5 F12／5.1.5.1 §2.14／D140）：純 JS 元件 `partials/film-picker.js`，**建於 tag-input＋chip 之上、無自帶 CSS**——搜尋輸入格（`.tag-input__entry` type=search）過濾候選、建議 `.chip-group` 點選加入、已選 `.chip--active.chip--removable` 移除；候選來自 `window.ztorFilms`（films-store）。API `window.ZTOR_PARTIALS.createFilmPicker(host,{selected,onChange})`→`{getSelected()}`。consumer：create-product.html（§4.5 獨立區）、product-detail.html（關聯分頁）、**create-bundle.html／bundle-detail.html（關聯分頁；2026-09-11 使用者指示組合也要有電影關聯）**。候選為 mock（UIA-053）、可搜尋（BR-NEW-1）。demo 卡見 `design-system.html` §4.123 | [partials/film-picker.js](./partials/film-picker.js) |
...
| Sold out／售罄 | error → `.badge--error` | 該管道 `qty === 0`。鎖定的管道賣完鎖定量即售罄，池裡還有未鎖定的數量也一樣 |
...
- **`qty` 是「哪一個管道」的可售量**：單售用 `channelQty(product, 'single')`、某個組合包用 `channelQty(product, { bundle: id })`、組合包自己用 `bundleQty(bundle, productsById)`。同一件商品在不同管道的狀態本來就不同，元件層不猜。
...
**Purpose** — 一顆商品的庫存池怎麼分給它的販售管道（spec §7.14 · D241）。建立商品時池裡建一筆商品、電子商店建一個「單售」管道；它加入的每個組合包都是另一個管道。任何管道都可以從池裡**鎖定** N 件給自己。

**`_layer`** · organism ｜ **source tier** · App ｜ **surface 層** · 只畫列線與欄軌，底色歸容器

**兩件事一定要看得到** — 那正是最容易搞錯的規則：

1. **逐列的可售量**。鎖定的管道**不吃沒有被鎖定的庫存量**，所以每一列都要有自己的數字，不能最上面擺一個共用的。鎖定的管道賣完鎖定量就是售罄，即使池裡還有未鎖定的數量，也要再鎖一次才會回來。
2. **所有管道都鎖定時的提醒**。那時沒有被鎖定的庫存量沒有人能賣，末列旁掛一顆 chip 講清楚剩幾件（`stock.all-locked`）。
...
- `.salloc__row` ×N — 每個管道一列：`.salloc__name`（＋`.salloc__sub`）／`.salloc__lock`（`amount-field` 欄位槽）／`.salloc__qty`（唯讀可售量）
...
**數字全部來自 `js/listing-state.js`** — `freeQty(product)`／`channelQty(product, channel)`／`bundleQty(bundle, productsById)`／`allChannelsLocked(product, bundles)`，欄位上限走 `validateLock(product, channel, wanted)`（上限＝沒有被鎖定的庫存量＋該管道目前已鎖定的量，因為調整自己的鎖定量時原本那份本來就能重新分配）。**頁面不得自己再算一次 min(成員) 或「池減鎖定」**。
