...
| Detail overview | 🟠 organism | ✓ App | 詳情頁總覽的三欄骨架（2026-08-31 使用者裁決）。三欄各自回答一個問題，這是分欄依據：**左（窄）＝數字**（KPI 直排，`.detail-overview__kpis`）、**中（寬）＝進行到哪**（上架追蹤／狀態卡／進度摘要，同一條時間線）、**右＝是什麼 ＋ 要你動手**（識別事實、方案、下一步、通知；使用者通則「有通知的就往右放一欄」）。軌道 `3fr / 5fr / 4fr`；1080px 併兩欄（KPI 回橫排）、899px 單欄；右欄可加 `--sticky`。**不用 `.bento`**：bento 一列一列往下鋪、要再包巢狀 bento 才做得出三根柱子，這裡的三欄彼此不對列。消費頁：project-detail（唯一）。**2026-08-31 使用者裁決：其餘詳情頁**先不套用****——各頁總覽的結構差異太大：商品／組合／競標是 form-section 堆疊、右欄沒有可放的識別卡（要新建才有，屬新增內容）；IP 詳情總覽只有一張卡，切三欄會有兩欄空著；訂單／取貨／品牌活動／創作者／粉絲／系列根本沒有分頁式總覽。活動詳情結構上可套（總覽已是兩欄），同輪一併暫緩。日後要套時先回頭讀這條，別把它當成沒做完的待辦 | [detail-overview.css](./ds-components/detail-overview.css) |
...
| Class | 用途 | 關鍵約定 |
|---|---|---|
| `.stat-row` / `.stat` | 卡最上方那排主要數字 | `.stat__of` 讓分母與數字同一行——「200 / 600」是一個值，拆兩行會讀成兩個數字；斜線與數字之間 `white-space: nowrap`，窄欄時整段一起換行（2026-08-18 補） |
...
- `.product-list__row--group[data-group][aria-expanded]` — 母列；展開把手 `.product-list__group-toggle` **放在名稱文字之後、同一格 `.product-list__title` 內**（`.product-list__title:has(.product-list__group-toggle)` 讓有把手的那幾列變成一條 flex：文字吃剩下的寬度並自己截斷、按鈕固定寬不縮），`.product-list__group-chevron` 隨 `aria-expanded` 轉 180°
