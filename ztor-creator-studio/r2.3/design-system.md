...
| State check | 🟡 molecule | ✓ Project | 唯讀的開／關狀態排（2026-09-11）：每個狀態一顆 16px 圓標記＋標籤，開＝`--status-success-fill` 底＋勾＋前景色標籤、關＝`--border` 空心圓＋輔助色標籤。**不是輸入控件**——點的是所在整列。首用商品／組合詳情「商品設定概覽」的「上架與開賣」列（使用者：「將上架／顯示／開賣拆出來，做成 checkbox 的形式，但設計不一定要用 checkbox」），值由三組開關與排程推導：上架、顯示於商店（上架且顯示）、開賣（上架且此刻在販售窗口內）。demo 見 §4.209 | [state-check.css](./ds-components/state-check.css) |
...
- **與黏住的詳情頂列共存（2026-09-11）**：頁面同時有 `.detail-topbar`（黏住後 fixed 在捲動容器上緣）時，`[data-nav] > .list-toolbar` 的 `top` 與各節的 `scroll-margin-top` 都加上 `--detail-bar-h`（js/detail-topbar.js 黏住時寫進 `.page`：頂列下緣到捲動容器上緣的距離）；沒有這個變數就回到 0。首個消費者：商品詳情的商品設定分頁。
...
**Anatomy** — `.state-checks` > `.state-check`（`--on`）> `.state-check__mark`（含 `check` icon）＋ `.state-check__label`。
...
**Token usage** — `--status-success-fill`／`--status-success-ink`（開）、`--border`（關的空心圓）、`--foreground`／`--muted-foreground`、`--fs-13`、`--sp-6`／`--sp-16`。
