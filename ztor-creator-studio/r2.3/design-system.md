...
- `demo-layer-system.html` 表面階層提案頁（18 處，分兩類）— **irreducible**：
  - `.lyr` 作用域的提案 token（`--layer-2-surface`／`--layer-line`／`--layer-lift`／`--layer-2-edge`）：這一頁在提案一組層級 token，那些 alpha 值就是被提案的對象本身，改用既有 token 會讓提案頁失去它要說明的東西。刻意不進 `_tokens.css`；待 `STYLE-DECISIONS.md` Q61／Q63／Q64 裁決後才 promote，屆時這幾行會從本頁消失。
  - `.lightbox`／`.lightbox--film` 固定亮色預覽（Q62 的 A／B 對照）：Q62 裁的是**亮色**的分層做法，預覽框必須在夜間模式下也維持亮色才看得到差別，所以不能跟主題 token 走——同 `selection-card.css` 主題色票的理由。
  （2026-08-11 記錄，供 check_ds_sync 檢查 10 的基準對照。）
...
模型出自 Figma `856:27798`（`demo-layer-system.html` 有可視化）：**L0 畫布實色 → L1 卡片實色 → L2 半透明薄膜（疊在誰身上就跟著誰亮一階）→ L3 不再疊填色、只留線框**。
