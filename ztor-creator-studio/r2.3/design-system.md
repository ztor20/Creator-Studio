...
| Lock sets | 🟠 organism | ✓ Project | 組合包鎖定套數區（2026-09-11）：「鎖定幾套 N」一個欄位 ＋ 外框清單（組合中的商品｜商品庫存｜未鎖定｜已鎖定），單一選項商品自動 N × 用量、多選項可展開逐規格選填分配（Σ ≤ N × 用量）。貼邊髮線分列、展開整組一起 hover；由 lab-bundle-lock.html L3 定案 promote。demo 見 §4.210 | [lock-sets.css](./ds-components/lock-sets.css) |
