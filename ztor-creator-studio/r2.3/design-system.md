...
|---|---|---|---|
| `--ztu-glass-bg` | `rgba(255,255,255,0.07)` | 玻璃卡半透明底 | `.card`／`.kpi`／`.alert--bar`／`.readiness`／`.preview-card`／`.form-section--outlined`／`.list-toolbar`／`.app-topbar`／`.wizard__top`／`.wizard__bottom`／filter-tabs 已選殼 等（14 處） |
...
**Behavior** — `window.ztorPublishStage.mount(host, { fields, prices, currencies, baseCurrency, views, previewRender, mode, editZones, onEdit, checks, primaryKey, secondaryKey, onPrimary, onSecondary })` → `{ refresh, syncChecks, result, unmount }`。翻譯草稿（DRAFTS／EDITED）與價格草稿（PRICES）是模組變數、**不隨每次 mount 重置**（回去改一個字再回來，改過的翻譯與覆寫都還在）；基準值一變該列覆寫清空重算（§7.15，比照 §7.4）。預覽語系與預覽幣別預設跟宿主走（介面語言／基準幣別），使用者點過才固定。預設語系欄在預覽與翻譯表都唯讀（母本回表單改，D223），其餘語系在預覽裡就地 contenteditable。價格不就地改：預覽裡的價格帶 `data-fep-price-key`（`js/fan-event-page.js` 掛），點了開價格表並聚焦對應那一格。`mode:'publish'` 時第三段是 `checks()` 給的發布前檢核，沒全過主鈕 disabled；`mode:'save'` 時第三段是未儲存的變更摘要、主鈕「儲存」。`onPrimary` 收到 `{ translations, prices, mode }`。

**Do / Don’t** — Do：把「發布前看一次」做成流程的最後一步或一個獨立頁，讓預覽拿到整個寬度。Don’t：這個畫面之後再加一層「確認發布」浮層（本畫面就是那層確認，D310）。Don’t：把「編輯」鈕插進粉絲頁的 markup——那份 markup 是前台 1:1 鏡像，而且可翻譯欄位是 contenteditable，塞進去會被當成內容編輯；用浮動的 `.pstage-edit`。
...
**Consumers** — `create-event.html`（第 8 步「預覽與發布」，`mode:'publish'`，`editZones` 對應步驟 2／3／5／6／7）；`event-localization.html`（活動詳情「預覽與在地化」的全頁版，`mode:'save'`）；`design-system.html` §4.216 就地掛載 demo。
