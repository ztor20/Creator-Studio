...
**單張門票彈窗改單欄四區塊（2026-09-22，D311）** — 彈窗殼仍是 `.payout-modal`／`.payout-dialog`（Q27），但內容整個重排：**分頁（設定／購買條件）退場**，改成四個 `.form-section--dense`——基本（門票名稱＋縮成一格的門票圖片）／價格與數量（`.calc-list--form` 算式列表：票價 ＋ 手續費 ＝ 粉絲付 − 平台費 ＝ 創作者實收，數量刻意排在列表之外）／入場（提前入場分鐘數＋「跟隨場次／自訂」二選一）／購買條件（區塊標題右側一顆「跟隨活動預設」開關：開＝三組規則的繼承值摘要三列，關＝`rulesHTML` 攤開可改，copy-on-write 不變）。標題列多一行副標（場次日期・場地），頁尾左側是 `.payout-dialog__foot-note`「N 個欄位待修正」。**2026-09-23（Q125）區塊標題的分量重排**：「基本」不畫標題列（彈窗標題與副標已經是脈絡，門票名稱與門票圖片兩格彼此無關、各自的標籤也說完了）、「入場」改 `.form-section__title--quiet` 輕標題（這一層沒有區塊級控制項），「價格與數量」（算式單元＋bookyay 徽章）與「購買條件」（跟隨開關）維持正常那一階；`event-detail.html` 的票種彈窗同一套判斷，第四個區塊「販售」有暫停開關、維持正常。同輪把彈窗內的節奏收成一把梯子（6／16／12／32／24），見 Pillar 4 的 Form section 條目。

- **新增「例外平台費」（Admin 專屬）** — 掛在平台費那一列底下的 `.calc-list__extra`：開關（選填、預設關）＋ 費率 %（必填 0–100）；只有 Admin 代操時整塊出現（`window.ztorCreator.adminScope()`，沒有 sidebar.js 時退回同一把 `ztor.role`／`?import=&handle=` 深連結），creator 本人看到的是唯讀的平台費列＋「例外 X%（Admin 設定）」。費率三層解析（本票例外 → creator 覆寫 → General）住在 `js/platform-fees.js`，葉節點名稱共用費率設定頁的 `fees.dim.*`／`fees.leaf.*`，不另抄一份中文字串。
...
- `event-detail.html` 的票種彈窗（D311 決定六）同一套結構：基本／價格與數量（同一組算式列與例外平台費）／入場／販售（暫停販售）。**購買條件在詳情頁沒有**——發布後改購買條件的規則上游未定（2026-08-11 裁決未變），所以最後一個區塊是「販售」。
...
**Consumers** — `create-event.html`（第 8 步「預覽與發布」，`mode:'publish'`，`translation:'manual'`，`editZones` 對應步驟 2／3／5／6／7）；`event-localization.html`（活動詳情「預覽與在地化」的全頁版，`mode:'save'`，`translation:'manual'`）；`create-product.html`（「開始販售」驗證通過後的全頁畫面，`mode:'publish'`，未傳 `translation`＝`'auto'`，`previewRender`＝ Fan product page，§4.218）；`design-system.html` §4.216 就地掛載 demo。
...
.calc-list[.calc-list--form]        外框（1px --layer-line、--radius-lg）
 ...
 └ .calc-list__extra                掛在某一列底下的附屬控制（縮排到運算子欄之後）
     └ .calc-list__extra-row        它的第一行：文字靠左、開關靠右
```
...
**Consumers** — `create-event.html`（單張門票彈窗的價格與數量：票價 ＋ 手續費 ＝ 粉絲付 − 平台費 ＝ 創作者實收，`--form`；bookyay 帶入者改唯讀版）；`event-detail.html`（票種彈窗，同一組列）；`js/bundle-editor.js` SPLIT 版定價段與 `create-bundle.html`（原價合計，唯讀版）。
...
**Behavior** — 宿主（`create-product.html`）把表單資料整理成 `model`（結構見 `js/fan-product-page.js` 檔頭：`category`／`badge`／`gallery`／`name`／`shop`／`price`／`variants`／`discountPct`／`stock`／`sizeGuide`／`delivery`／`desc`／`specs`／`returns`／`related`）交給本檔渲染，本檔只知道「粉絲商品頁長什麼樣」、不知道資料來源。可翻譯欄位（`name`／`desc`）由宿主給 `{ key }`，本檔以 `api.getValue(key)` 取當前語系值並掛 `api.bindEditable`；單一選項給一個價格，多選項取逐組合最低價、價格不同時加「起」（`fpp.price.from`）；折扣 `discountPct > 0` 才畫折後價＋劃線原價＋「省 N」（`fpp.price.save`），換算後的幣別也照同比例打折。庫存只在限量且剩 ≤10 件時顯示「僅剩 N 件」（`fpp.stock.left`），0 件時 CTA 轉「已售完」（`fpp.cta.soldout`）並停用。選項：顏色畫色票（`swatchStyle()` 取色號，沒有色號退回 `--border-strong`）、其餘畫尺寸格；有尺寸指南時掛在第一個非顏色選項列（`.pdp-variant__guide`），沒有這種列就自成一列（`.pdp-variant--reflone`）。價格不就地改：`data-fep-price-key`／`data-fpp-price-key` 供宿主的 `js/publish-stage.js` 認得，點了開價格表並聚焦對應那一格（與 Fan event page 同一機制）。相關商品固定四張佔位卡（`items` 給陣列時取前 4 筆，否則畫 4 張空卡），全部標「非本商品」。

**Token usage** — 與 Fan event page 同一批前台 token（定義在 `fan-shop.css` 最前面的 `.fep-shop { … }`），第 4 段新增的類別沿用同一批、無新 token。

**Consumers** — `create-product.html`（「開始販售」驗證通過後的全頁「預覽與發布」，`previewRender` 呼叫 `buildFanProductModel()`）；`design-system.html` §4.218 就地渲染 demo（替身 api）。
