...
## 2026-07-01（第九輪：接進 r2.1 正位＋上線）

- **D** 把測試版複製進 `r2.1/`：新增 `r2.1/create-campaign.html`、`r2.1/funding-simulate.html`；資產路徑由 `../r2.1/...` 改同目錄；`funding-simulate` 的「關閉」接 `projects.html`。`.fc-*` 為本頁專屬樣式、不覆寫共用 class/不重定義 token（check_ds_sync 第8項 PASS），暫留頁內，跨頁重用再 promote。
- PR：ztor20/Creator-Studio **#57**（含 r2.1 兩頁＋ r2.1_funding-test 全套；取代 #56）。
- 上線：deploy.sh → Vercel，已驗證 200 → https://ztor-v2-creator-studio.vercel.app/create-campaign.html 、 /funding-simulate.html 。
- 註：`r2.1_funding-test/` 沙盒夾與 `r2.1/` 正式頁目前並存（依「複製」指示）；整合穩定後可退役沙盒夾。設計規格仍在測試夾、未搬進 `documents/`（待 IA 定案後歸位）。

## 2026-06-30（第八輪：前台試算抽成獨立頁）

- **B** 「前台試算」從 wizard 第 4 步抽成**獨立頁** `funding-simulate.html`；create-campaign 回到 3 步，最後一步主動作改「創建送出」→ 以 `sessionStorage('fc-sim')` 帶 `{goal,totalShares,equity,bundles}` 跳轉到該頁（直接開啟用種子示範）。
- **B** 獨立頁頁尾「上一步」回創建、「關閉」（正式版回專案列表 projects.html；獨立 demo 版回創建起點）。仍 demo 專用、不寫進設計規格書。
- 之後接進 main 版測試版時再接線（依使用者指示，先做成獨立頁）。
