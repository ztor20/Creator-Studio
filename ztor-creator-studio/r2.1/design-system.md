...
| Stock tip | 🟢 atom | ✓ App | E-Shop 商品清單「狀態」／「庫存」欄 hover/focus 浮出的資訊卡：多選項商品攤到單一選項組合、組合商品攤到「成員 · 選項組合」；單一選項商品（2026-07-23 擴大範圍）改顯示低庫存門檻——限量版算真實門檻（cap 的 10%），不限量版無固定門檻、如實顯示「不限量」而非捏造數字。列徽章為「急需補貨」時，選項清單只留真的需要補貨的項目（此規則不影響單一選項的門檻資訊）。定位由 JS 算（`position: fixed`），viewport 上半部往下開、下半部往上開，避免被 sticky 頂欄擋到或超出視窗。詳見 §4.56 | [stock-tip.css](./ds-components/stock-tip.css) |
...
- **無選項資料**（單一選項商品，如 zine／acetate／pin）：兩行——**目前庫存**（讀該列狀態，`data-status="low"／"out"` 對應上色，其餘視為健康）＋**低庫存門檻**。目前庫存這行是 2026-07-23 當天二次修正加的：原本只顯示門檻一行，使用者反饋「應該要列出庫存數量」——只顯示「不限量，無固定門檻」而不帶回實際庫存數字，跟列上明明掛著「急需補貨」徽章的視覺語境對不上，讀起來像答非所問。門檻行邏輯不變：限量版本（`data-edition="limited"`）從列上的「在庫 / 上限」算出真實門檻（`lowThr(上限)`，跟選項層級同一套 10% 規則）；不限量版本沒有固定上限、10% 規則沒有基準（spec §7.2／D105 標「產品待確認」），如實顯示「不限量，無固定門檻」，不捏造一個數字。
...
- **列徽章是「急需補貨」（`e-shop.row.low`）時，選項清單只留 `low`／`out` 的選項**（2026-07-23 使用者裁示）——這種列 hover 是要立刻看到哪裡出問題，健康（`ok`）的選項不佔位置；徽章是其他狀態（Live／Sold Out）時清單維持完整、不過濾。判斷依據是該列狀態格裡有沒有 `[data-i18n="e-shop.row.low"]`，不是看選項本身的 status——避免徽章語意以後改了但濾掉的條件沒跟著同步。**這條規則不適用單一選項商品的門檻資訊**——門檻只有一行、跟「選項健不健康」無關，不會被這條規則濾掉。
...
- ✅ 單一選項商品顯示低庫存門檻，不限量版本如實顯示「無固定門檻」，不捏造數字。
