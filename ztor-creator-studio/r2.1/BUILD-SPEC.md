...
| Restock order (lines) | `.restock-lines / .restock-lines__group / .restock-line(__main/__img/__text/__name/__meta) / .input.restock-line__qty / .restock-line__after` + 重用 `.payout-dialog` 外殼・`.segmented`・`.data-list` | 補貨彈窗（spec 5.1.5.6 v1.4，D104）：一次補貨＝一張補貨單——①單據層（整單填一次）＝方式 `.segmented`（立即/計時）＋供應商/預計到貨/備註（`.payout-field`）；②品項層 `.restock-lines`＝每個要補的品項一列 `.restock-line`（識別＋狀態 badge＋目前庫存/門檻＋數量輸入＋即時補後讀數）。多規格商品／組合成員以 `.restock-lines__group` 分組（多規格成員展開為規格列）。數量留空＝不補；取代 D100/D101 的 tab 面板（tab 退場）。方式立即→隱藏 ETA/到貨確認、計時→顯示 ETA(必填)+到貨確認。`[data-restock-modal] .payout-field[hidden]{display:none}` 補回 ETA 隱藏。由 `partials/restock-modal.js` 的 `createRestock()`（openSingle/openVariants/openBundle→openOrder）掛載於 e-shop.html、product-detail.html。送出對數量＞0 的品項生效、商品細節頁逐品項 append 補貨紀錄（`.data-list`），皆 demo（口徑依上游 §7.2，UIA-006/007；同單混合方式、部分收貨待確認）|
