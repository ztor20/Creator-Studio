...
| Restock panel | `.restock-panel([hidden]) / .restock-identity(__img/__main/__title/__meta) / .restock-after` + 重用 `.payout-dialog` 外殼・`.segmented`・`.tabs`・`.data-list` | 補貨彈窗（spec 5.1.5.6，D100）：重用 canonical `.payout-dialog` 外殼＋`.payout-form-grid`/`.payout-field`；核心＝單一商品面板 `.restock-panel`（商品識別 `.restock-identity`＋方式＋數量＋補後讀數 `.restock-after`）。方式切換用 `.segmented`（立即/計時）、tab 模式用 `.tabs`（組合＝逐成員、多規格商品＝逐啟用規格各一面板分別填，D101）、商品細節頁補貨紀錄用 `.data-list`（多規格逐筆標規格）。restock 專屬只有 panel/identity/after。由 `partials/restock-modal.js` 的 `createRestock()` 工廠掛載於 e-shop.html、product-detail.html；入口＝商品列補貨鈕／組合列補貨鈕／細節頁 Restock 鈕／F2 低庫存（間接）。立即→In stock、計時→Restocking／到貨確認→In stock、細節頁 append 補貨紀錄，皆 demo（口徑依上游 §7.2，UIA-006/007）|
