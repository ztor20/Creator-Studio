...
索引     index.html              · Dashboard           · 完整 ✓
        ...
        pickup.html             · 取貨管理（E-Shop ▾） · F1 總覽 KPI + F2 場次清單 + 場次詳情（F6 scanner 存取/F4 可核銷項目/F5 名單/F8 核銷紀錄）✓（spec 5.1.5.11 / D111）
...
| Pickup management | `.scanner-access(__qr/__main/__label/__url/__actions/__pw) / .qr-box(--lg/--disabled) / .pickup-detail__header/__meta / .pickup-stats / .pickup-select__row(.is-checked · __box/__img/__text)` | 取貨管理工作區（spec 5.1.5.11 / D111）SiteSpecific organism：F6 scanner 存取卡（URL＋密碼＋faux-QR）＋建立場次多選列。QR＝`window.ztorFauxQr()` 生成 finder+偽隨機模組（UIA-046，非真碼）。場次清單/可核銷項目/名單/核銷紀錄重用 `.data-list`、KPI 用 `.kpi.bento`、狀態篩選 `.filter-tabs`、詳情分頁 `.tabs`。全 token 化、無裸色。用於 pickup.html；建立場次 popup `partials/pickup-session-modal.js`（F3）亦掛 create-product.html／product-detail.html |
