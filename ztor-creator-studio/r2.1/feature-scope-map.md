...
| 鍵 | 顯示名 | 類型 | 規則 | 說明 |
|---|---|---|---|---|
| `full` | 最終完整版 | 累進 | `all` | 開發目標，全部功能（預設） |
| `p1-next` | Phase 1 ＋ Next | 累進 | `tier:p1,next` | 首發＋規劃追加 |
| `p1` | Phase 1 | 累進 | `tier:p1` | 內部首發 |
| `test` | 測試版（特殊） | 特殊 | `route:create-project.html=funding-test/create-campaign.html` | 建立專案改接募資建立流程（create-campaign 部署複本），其餘同最終版 |
