...
| Q5 | hover 浮起規則 | **A**：可點卡片 hover 浮起（借 `--shadow-float`），清單列 hover 只換底色。先在 design-system.md 定義「什麼算卡片、什麼算列」 | 2026-07-13 | 卡片＝浮起、列＝換底的標準區分 | ✅ 已執行 2026-07-13 |
...
| Q20 | 清單列縮圖／圖示晶片：填色無邊框 vs 描邊框 | **統一描邊框，分兩個家族各對齊自己的真實基準**：(A) 「商品/專案照片佔位圖」家族——`.product-list__thumb`（orders／pickup）與 `.project-list__icon`（projects）改為 52×52／`--muted` 底／1px `--border-soft`／icon 色 `--muted-foreground`，對齊 `.product-list__image--placeholder` 的**真實呈現值**（07-20 二次修正：首版誤把對齊對象抓成 `.product-list__image` 的「基礎規則」`--card`／`--border`／52px，但該 class 全站 28 處使用皆搭配 `--placeholder` 變體、基礎規則從未單獨呈現過，畫面上實際是 `--placeholder` 覆蓋後的 `--muted`／`--border-soft`；首版也沒同步改尺寸，仍是舊的 44px。二次修正後以 Playwright 量測 computed style 逐項核對訂單/專案與電子商店一致）。(B) 「儀表板圖示晶片」家族——`.data-list__icon`（15 頁）改為 `--card` 底＋1px `--border`（40×40 尺寸不變，與 `.alert--card .alert__icon` 同尺寸家族但邊框語意獨立處理），**不套用家族 (A) 的 52px／`--muted` 值**，因為角色不同（家族 A 是「無照片時的佔位圖」，家族 B 是「資訊列的語意圖示」）。反白變體 `.product-list__thumb--cover` 邊框設成與自身填色同色，不露中性描邊 | 2026-07-18（07-20 追加 data-list__icon；07-20 二次修正家族 A 對齊值＋尺寸）| 使用者裁示（截圖指定；07-20 再次截圖指出訂單管理與電子商店仍不同，促成二次修正）；亦符合 Q3「邊框優先」的既有方向 | ✅ 已執行（元件層一次生效，家族 A 已 Playwright 逐項量測驗證）|
