...
| Q20 | 清單列縮圖／圖示晶片：填色無邊框 vs 描邊框 | **統一描邊框**：`.product-list__thumb`（orders／pickup）、`.project-list__icon`（projects）、`.data-list__icon`（15 頁儀表板資訊列，2026-07-20 追加）由「`--muted` 填色、無邊框」改為「`--card` 底＋1px `--border`」，與同元件既有的 `.product-list__image`（e-shop／events／pickup，本來就是描邊框）一致——同一個「清單列縮圖／圖示晶片」角色站上只留一種做法。反白變體 `.product-list__thumb--cover` 的邊框設成與自身填色同色，不露中性描邊。`.data-list__icon` 原設計是與 `.alert--card .alert__icon`「同族」（兩者都 40×40／radius 10），但 `alert__icon` 底色由狀態變體決定、無邊框語意本就不同，改邊框不影響兩者的視覺配對關係 | 2026-07-18（07-20 追加 data-list__icon）| 使用者裁示（截圖指定：把填色無邊框那種全部換成描邊框那種；07-20 明確要求含 data-list__icon 一起改）；亦符合 Q3「邊框優先」的既有方向 | ✅ 已執行（元件層一次生效）|
