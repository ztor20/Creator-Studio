...
- **【delta 膠囊：保留設計，只換墨色】** 使用者裁示「綠色半透明的設計要保留，只是不要黑底」。**膠囊配方一個字沒改**（12% 半透明染色＋pill＋內距），改的是兩件事：
  ...
  - **墨色跟著面走、不跟著主題走**：新增 `--status-success-ink`／`--destructive-ink`。關鍵認知——**實色橘卡在深色主題底下依然是一個「亮底」**，所以主題型 token（像 `--brand-ink`）解不了它。`.kpi--hero` 在卡片這一層重新定義這兩個 token，custom property 往下繼承到膠囊，亮暗兩主題同時成立，不必比特異性也不用 `!important`。色相不動、只降明度到 green-950 `#052E16`／rose-950 `#4C0519`，維持「一眼看出是綠／紅」。
...
- **【試過但退場的三版】** 純黑底（使用者否決）、白色薄膜（否決）、完全不要底（否決——那會讓膠囊不再讀作「綠的」，犧牲了色相識別）。最終版本是唯一同時滿足「半透明染色膠囊」「無黑底」「對比達標」三個條件的解。
- **【誠實記錄的代價】** 12% 綠疊在飽和橘上色相位移很小（`#e9aa47` vs 卡的 `#ffa33f`），所以 hero 卡上的「綠」主要由墨色承載、不太靠底色。**不刻意調高 alpha 去逼出綠**——那會把半透明膠囊變成實色色塊，反而犧牲掉使用者要保留的設計。
