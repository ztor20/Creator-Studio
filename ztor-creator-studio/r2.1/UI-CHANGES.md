...
- **【唯一讓出狀態色的地方】** `.kpi--hero` 內的 `.kpi__delta` 由綠色膠囊改為白色薄膜＋深墨字。原本的 `color-mix(--status-success 12%, --card)` 混的是 `--card`——那已經不是這張卡的底色了；而且綠配橘會互相震盪。漲跌方向改由 `+`／`−` 與文案承載。**scoped 在此變體內**，全站其他 `.kpi__delta` 的 gel-not-verdict 規則不變。label／meta 用同一支深墨壓透明度做層級（不另挑灰，灰在橘底會發濁）。
