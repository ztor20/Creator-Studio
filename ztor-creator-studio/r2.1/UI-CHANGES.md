...
- **兩層**：①單據層（`.segmented` 方式立即/計時＋供應商/預計到貨/備註，整單填一次）②品項層 `.restock-lines`＝每個要補的品項一列 `.restock-line`（識別＋狀態 badge＋目前庫存/門檻＋數量輸入＋即時「→ 補後」讀數）。
...
- 追加（2026-06-26）：selection-card 已選態從元件改——①橘色改成**線框**（`outline:1.5px solid var(--primary); outline-offset:-1.5px`），box-shadow 維持中性 `--shadow-card`（不再把橘色放進陰影，依使用者「陰影不該有橘色、應該是線匡橘色」）；②icon 變體的 icon 由 `--foreground-muted`(#4D4D4D 偏深) 改 `--muted-foreground`(#737373 較淺的灰)；③移除 `.selection-card--icon.selection-card--active` 的重複規則（全域 active 已涵蓋）。主題挑選器（含 swatch）以 `:has()` 例外＋`outline:0` 維持原本「淡橘底＋2px 環」不變。驗證：建立流程卡 outline 1.5px 橘、陰影無橘、icon #737373；swatch 卡 outline:0、保留 tint＋環。check_ds_sync PASS。
