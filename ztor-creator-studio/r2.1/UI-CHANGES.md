...
- **【唯一讓出狀態色的地方】** `.kpi--hero` 內的 `.kpi__delta` **不套膠囊**：無底、深墨字、左右內距歸零，與 label／value 切齊同一條左緣（實測三者 `left` 皆 294px）。base 的 `color-mix(--status-success 12%, --card)` 混的是 `--card`——已經不是這張卡的底色了，合成後是深墨綠 `rgb(38,57,46)`，鋪在橘上就是一塊黑斑。同日先後試過**純黑底**與**白色薄膜**，兩版都被使用者否決，最終裁示「這裡不要底」——問題不在挑哪一種深色，而在這個位置不該有一塊色塊。漲跌方向由 `+`／`−` 與文案承載（橘底上綠字僅 ~1.4:1，本來就放不了狀態色）。**scoped 在此變體內**，全站其他 `.kpi__delta` 維持原本的染色膠囊、gel-not-verdict 規則不變。label／meta 用同一支深墨壓透明度做層級（不另挑灰，灰在橘底會發濁）。
