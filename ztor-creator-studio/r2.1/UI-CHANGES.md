...
## 2026-07-21 · 下拉選單選項拿掉四周內距（B 反饋導入）

使用者接續反饋：選項四周不要有 padding。

- **【B】** `ds-components/dropdown-menu.css`：`.dropdown__item` 的 `padding: 9px var(--sp-10)` 改 `padding: 0`。選項的間距現在全靠面板自身 `--sp-6` 內距＋icon／文字的 `gap` 撐出來，視覺上會比原本緊湊（icon／文字貼近選單左右緣、行距變窄）——使用者已看過截圖確認這就是要的效果。
- **【D】** `design-system.md`／`.html` 同步：Sizes／Class API 兩處「9×10 padding」的描述改為「無內距」。
- 驗證：Playwright 量測 computed style 確認 `.dropdown__item` padding 為 0；`check_ds_sync.py` 全 PASS。
