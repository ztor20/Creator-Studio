...
- **【B】** 庫存數字一律唯讀，改變數量只能透過右欄「補貨」。單選項用新元件 `.stock-readout`（顯示字級數字＋單位／分母＋狀態徽章）取代 `.input`；多選項的表格庫存欄用新增的 `.variant-cell--stock`（低於門檻時 `--stock-low` 轉紅）取代 `.input`。**這一條牴觸規格 §2.10，已記入 ASSUMPTIONS UIA-066，未寫回 `documents/`。**
