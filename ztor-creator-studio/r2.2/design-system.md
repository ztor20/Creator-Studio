...
**聚焦時整份列出、不套用輸入框裡的關鍵字**——選過之後框裡放的是那一場的名稱，拿它當關鍵字會只剩一列，等於看不到其他選項。已選中的那一列在重開的清單裡以 `aria-selected="true"` 標出。連帶：`.source-gate__empty`（無結果說明文字）撤除，那句話改畫在下拉裡的 `.owner-lookup__result--empty`；`.radio-cards--list` 因此一度失去唯一消費頁，同日由建立活動的「要怎麼建這些門票？」選擇彈窗接手，退場候選狀態解除。
