...
**行為約定（消費頁負責）** — ①母列整列可點＝展開／收合（明確的展開鈕與 ⋮ 裡那一項一律有效，連結與選單本身放行）；子列點擊才進那一場。②子列**不進分頁計數**——它跟母列算同一筆。③子列可見 ＝ 母列通過篩選 **且** 展開；只看展開的話，篩掉母列後子列會孤零零留在畫面上。④**搜尋中一律攤開**：搜到的字可能就長在某一場身上，收著等於「找到了但看不到」。
...
**聚焦時整份列出、不套用輸入框裡的關鍵字**——選過之後框裡放的是那一場的名稱，拿它當關鍵字會只剩一列，等於看不到其他選項。已選中的那一列在重開的清單裡以 `aria-selected="true"` 標出。連帶：`.source-gate__empty`（無結果說明文字）撤除，那句話改畫在下拉裡的 `.owner-lookup__result--empty`；`.radio-cards--list` 因此失去唯一的正式消費頁（僅剩 design-system 展示），列為退場候選、待使用者裁決。
