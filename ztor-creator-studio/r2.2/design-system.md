...
| ~~Lang mismatch dialog~~ | — | **已退場 2026-08-24（D222）** | 登入後顯示語言／預設語言不一致確認彈窗（D221，2026-08-24 引入）。D222 同日裁示語言收回單一概念，「顯示語言」與「預設語言」不再是兩個可能不一致的值，這顆彈窗要處理的情境不復存在，隨即退場。`partials/lang-mismatch-dialog.js` 已刪除（無自帶 CSS，無其餘檔案依賴，直接刪除而非留 tombstone 檔）；掛載點 `index.html`／`creators.html` 已卸載 `<script>`；`js/sidebar.js`／`login.html` 已撤除對應的 `ztor-r22-langcheck` 旗標寫入。i18n key `langmismatch.*` 已清除 | （已刪除） |
