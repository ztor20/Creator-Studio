...
| Bundle editor | 🟠 molecule | ✓ Project | 共創回饋套組編輯器（2026-07-24 promote，第二消費者 project-detail 支持方案照 create-campaign 新增套組重做）。每張套組＝`.card.fc-bundle`：head（標題＋移除）＋名稱/描述（`.form-grid` 2 欄）＋含股份/名額（`.form-grid` 2 欄）＋套組商品列。商品列 `.fc-item-row`＝96px 上傳縮圖／名稱+描述兩行（`.fc-item-fields`）／移除鈕。滿版加高新增鈕 `.fc-add`（56px，新增套組）／`.fc-add-item`（48px，新增商品）。style-only，增刪由消費頁 JS；重用 `.card`/`.field`/`.form-grid`/`.input`/`.upload-tile`/`.btn`。固定尺寸：96px 縮圖、56/48px 鈕高。**create-campaign 仍保留頁內同名 `.fc-*` 副本，待遷移到本元件**（治理待辦）。 | [bundle-editor.css](./ds-components/bundle-editor.css) |
