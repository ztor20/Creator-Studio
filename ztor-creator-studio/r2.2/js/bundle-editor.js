/* js/bundle-editor.js — 回饋套組編輯器（共用模組）
   ...
   現在兩個頁面共用同一份 markup 產生器：
     · create-project.html  「回饋套組」步驟（wizard-split，池＝#fd-slots 即時值）
     · project-detail.html  「方案與承諾 › 支持方案」（bento，池＝原型固定值）
...
(function () {
  ...
  function mount(opts) {
    ...
    var T = opts.t || function (k) { return (window.i18nT && window.i18nT(k)) || k; };
    ...
    function newBundle(seed) {
      ...
      var b = {
        ...
        slots: 0,
        avail: 'auto',        // auto | limited（2026-07-28：unlimited → auto，見 maxUnits）
        cap: '',
    ...
    /* ── 價格（2026-07-30 使用者裁決：唯讀、自動計算）──────────────────────
       原價   ＝ 含分潤名額 × 每名額單價 ＋ 所選商店商品定價加總
       套組價 ＝ 原價 − min(原價 × 套組優惠%, 可折抵上限)
       可折抵上限 ＝ 商品定價加總（maxDiscount）——**分潤名額的價值不可折讓**
       上限的百分比說法 ＝ maxPct()＝floor(商品定價加總 ÷ 原價 × 100)
    ...
    function listPrice(b) { return shareValue(b) + itemsTotal(b); }
    /* 可折抵上限＝商品那一段。純回饋（0 名額）的卡上限等於原價，所以可以折到 $0（100%）；
       只含名額、沒有商品的卡上限是 0，一分都不能折（0%）。 */
    function maxDiscount(b) { return itemsTotal(b); }
    ...
    function isValid(b) {
      return String(b.name).trim().length > 0 && (slotCount(b) > 0 || b.items.length > 0);
    }
    ...
    function priceHintText(b) {
      ...
      if (!s && !n) return T('cpp.bd.price.calc.empty');
      var parts = [];
      if (s > 0) {
    ...
    function summaryMeta(b) {
      ...
      var s = num(b.slots);
      if (Number.isFinite(s) && s > 0) bits.push(s + ' ' + T(s === 1 ? 'cpp.bd.n.slot' : 'cpp.bd.n.slots'));
    ...
    function cardHTML(b, i) {
      ...
      '<div class="card fc-bundle' + (b.collapsed ? ' fc-bundle--collapsed' : '') +
        ...
        /* ── 卡片內的順序＝從「輸入」走到「結果」（2026-07-30 重排）───────────
           名稱 → 一句話說明 → 名額｜販售上限 → 商店商品 → 額外權益 → 定價區塊。
          ...
          '<div class="form-grid">' +
            ...
            '<div class="field">' +
              ...
              '<div class="segmented" role="radiogroup" aria-label="' + esc(T('cpp.bd.avail')) + '">' +
                '<button type="button" class="segmented__btn' + (b.avail === 'auto' ? ' segmented__btn--active' : '') +
                  '" role="radio" aria-checked="' + (b.avail === 'auto') + '" data-bd-avail="auto">' +
                  esc(T('cpp.bd.avail.auto')) + '</button>' +
              ...
              (b.avail === 'limited'
                ? '<label class="field__label mt-8">' + esc(T('cpp.bd.cap')) + '</label>' +
                  ...
                  '<div class="field__hint">' + esc(T('cpp.bd.cap.hint')) + '</div>'
