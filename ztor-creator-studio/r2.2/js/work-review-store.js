...
(function () {
  ...
  /* v4（2026-08-17 下午）：退件那一筆改掛已成立的項目（落差 L5），另補一筆文檔類的
    ...
    return [
      ...
      },
      {
        /* 2026-08-17：這一筆原本掛在 `dragon-tiger-gate`（預購、進行中）。送審的前置條件
           是項目已成立，所以那個組合在同一頁上會自相矛盾——作品區塊給一顆可按的
           「修改後重送」，發文彈窗卻說要等項目成立（矩陣盤點落差 L5）。改掛同為預購、
           狀態已成立的 `north-point-rain`，兩個入口才講同一句話；`dragon-tiger-gate`
