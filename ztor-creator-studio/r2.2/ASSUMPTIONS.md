...
| UIA-104 | 項目詳情頁的狀態徽章：`published` 的字樣依類型分流——共創顯示「共創中」、預購顯示「預購中（Taking orders）」。規格把各發行模式下 Published／Succeeded 的對外字樣列為待確認（D166 產品待確認第 3 項），這裡先照類型講對話，避免預購項目被標成「共創中」。上游定案後以規格為準。 | project-detail.html 狀態徽章 | 2026-08-04 | 待上游確認 |
...
| PG-029 | **預購方案的販售上限總和與最少訂購數的關係未做校驗（2026-08-03，D166 待確認第 3 項）**：`documents/decisions.md` D166 待確認第 3 項明寫「販售數量上限與最少訂購數是否互相校驗（例如所有方案上限總和不得低於最少訂購數）〔產品待確認〕」——若一個預購項目的最少訂購數是 100，但所有方案（`js/bundle-editor.js` 各卡的 `cap`）上限總和只有 60，理論上永遠無法達標，但原型不會提示或阻擋這種狀態。`create-project.html` 的預購方案步驟（`tiers-pre`）與 `project-detail.html` 的預購方案分頁都沒有讀取 §5.3.1 F27 的「最少訂購數」去跟本步驟的方案上限總和比對。**2026-08-04 更新（D167 已定案）**：D167 已裁定校驗規則本身——「全數限量時可售份數總和須 ≥ 最少預購數，任一方案不限量則跳過」——這條缺口不再是「上游未定義」，而是「規格已定、原型尚未實作跨欄位校驗」 | **規格已定案（D167）**：全數限量時方案上限總和須 ≥ 最少預購數、任一方案不限量則跳過；原型仍未實作此跨欄位校驗，即使方案上限總和低於最少預購數也不提示，留為待辦 |
