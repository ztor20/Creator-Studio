...
- **附件入口不接上傳**：發文框的圖片／影片／音樂三顆按鈕按下只說明一句，原型不做真的上傳（UIA-119）。

**跟著 D194 一起撤除的東西（不是本層自行刪的）**
...
- **里程碑的示範資料由項目狀態推導，不逐項目手寫**（`js/project-progress-store.js`）。上游的里程碑初值來自建立流程 §4.3 F10，原型沒有那份資料。三顆通用里程碑（前期製作完成／主體拍攝殺青／成片定剪）的完成度依項目狀態換算：進行中＝第一顆進行中、已成立＝第二顆進行中、準備中與已上線＝三顆都完成、已取消＝停在第一顆。手寫種子會跟項目狀態脫鉤，出現「項目已上線、里程碑還停在未開始」這種畫面自打嘴巴的情況。
