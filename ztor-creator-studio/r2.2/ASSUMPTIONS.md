...
- **入口做成項目詳情的「作品」分頁**。規格 §2 的資訊分群把「作品」列為與總覽／內容／公開資訊／承諾／金流並列的一群，§3 佈局又明說分頁怎麼分組屬呈現決策，所以獨立一個分頁。**出現條件不是類型而是「這個項目真的有一件作品」**：影片家族 ＋ `js/work-review-store.js` 裡有這一件的紀錄。直接發佈在建立時就送審，所以建完就有；共創與預購要等第一次送審之後才出現（對齊 §2.0 類型表那一列）。
...
| PG-039 | **逐規格（SKU）圖片是上游沒有的資料欄位（2026-08-10 使用者指示，原型先行）**：`documents/5.1.5.2-建立商品流程.md` 的多選項規格（§4.1④）只定義價格／庫存／總量／SKU／單件成本五個逐規格欄位，**沒有「這個規格自己的圖片」**；F1 素材也只定義商品層級的主圖＋附圖。使用者指示每個規格組合要能單獨上傳一張圖，沒上傳就沿用商品主圖。原型已實作（`create-product.html` 規格表列首縮圖＋「規格圖片」彈窗，狀態存在 `varData[key].img`）。**待上游確認**：(a) 這個欄位是否納入規格；(b) 規格圖與商品附圖的關係——是獨立一張，還是要從附圖裡挑；(c) 買家端哪些版位吃規格圖（商品頁選到該規格時換圖？訂單／取貨清單的縮圖？），未定前買家端一律仍顯示商品主圖；(d) 是否必填、有沒有張數或尺寸的額外規範（目前沿用全站直式 750×1125）；(e) 兩階層規格（顏色×尺寸）時，圖應該掛在「顏色」這一層還是每個組合各一張——目前是每個組合各一張，顏色相同的組合要各傳一次。 | 裁決後同步 `documents/5.1.5.2` §4.1④ 與 F1；未核准前不得視為正式功能，買家端不接線 |
