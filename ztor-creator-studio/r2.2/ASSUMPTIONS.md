...
| PG-029 | **預購方案的販售上限總和與最少訂購數的關係未做校驗（2026-08-03，D166 待確認第 3 項）**：`documents/decisions.md` D166 待確認第 3 項明寫「販售數量上限與最少訂購數是否互相校驗（例如所有方案上限總和不得低於最少訂購數）〔產品待確認〕」——若一個預購項目的最少訂購數是 100，但所有方案（`js/bundle-editor.js` 各卡的 `cap`）上限總和只有 60，理論上永遠無法達標，但原型不會提示或阻擋這種狀態。`create-project.html` 的預購方案步驟（`tiers-pre`）與 `project-detail.html` 的預購方案分頁都沒有讀取 §5.3.1 F27 的「最少訂購數」去跟本步驟的方案上限總和比對 | 上游定義兩者是否互相校驗後再補驗證；原型暫不做跨欄位校驗，即使方案上限總和低於最少訂購數也不提示 |
