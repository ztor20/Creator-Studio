...
## TOUR-001 · 活動形式「定點／巡迴」（2026-08-10）— ⚠ 同日撤回，已從原型移除

**狀態**：**已撤回**。2026-08-10 當日先依使用者指示實作，同日使用者再指示「刪除活動形式，只會有定點活動」，整組已從 `site/r2.2/create-event.html` 移除（含一站一盒的場次版面、逐站場地欄位、巡迴的場地必填分支）。場地回到單一一份、所有場次共用，**與上游規格 5.1.6.1 原本的模型一致**，因此不再有規格落差。本條目保留為決策紀錄：若日後要重做巡迴，底下的「需要上游裁決的問題」仍然有效。
