...
- **【A】頁首補三顆依階段互斥的鈕**：發布並開賣（draft／scheduled）、取消活動（draft／scheduled／on-sale）、再辦一場（ended／cancelled）。此前只有編輯／刪除／預覽，草稿填完就是死路，而總覽的橫幅還在宣告 `Draft → Scheduled / On Sale →（活動進行）→ Completed` 這條狀態機。
...
- **【D】mock 補一場可開賣的活動**：`taipei-nye` 補兩個票種，讓「已排程 → 開賣」有完整示範；其餘已排程活動票種仍空，示範的是被擋下來的那一半。
