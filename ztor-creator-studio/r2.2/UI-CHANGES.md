...
- **【B】** 貼頂列**四角全圓 ＋ 上緣留 8px**，推翻 2026-07-28 的「上緣切平」裁決。當初切平是想讓它讀成「頁首外框的延伸」，但實際貼頂時它會把 `.main` 左上角那個 28px 圓角（`shared.css:501`，`--radius-shell` 只給左上）整塊填成直角——使用者連兩次回報「圓角不見了」指的就是這個。`top` 由 `0` 改 `--sp-8`：貼死在頂端時兩個上角貼著面板上緣、看不出是圓的。恢復後這一條讀成浮在面板裡的卡，面板自己的圓角也露得出來。六頁一起生效。
