...
- **未做·分頁**：貼文明細最多 10 列（自訂期間），加分頁器是裝飾。等假資料量真的長起來再說。
- **新發現·未修**：期間倍率與期間長度對不上——「自訂」涵蓋 12 個月，總額卻比只有 6 個月的「年迄今」少（記 ASSUMPTIONS ESP-009）。倍率全分頁共用，改動會動到每一個數字，等裁決。
- 實測（`devserver.py 4332 r2.2`）：兩個 persona × 四期 × 四型，查看全部清單加總、貼文明細加總、貼文類型表加總三者皆等於該型／該期總額；Top 10 無期外貼文；check_ds_sync PASS。
...
- **不新造元件**：彈窗殼沿用 `payout-dialog--narrow`，選項用 `.radio-cards--gate`（Q28 已裁決「閘門卡點了就走、不保留選取態」，正是這裡的語意）＋ `.radio-cards--list` 一列一個。**副標各是一整句「什麼時候該選它」，兩欄會把每句折成四行**，一列一個才讀得順。
- **【D】`.radio-cards--list` 退場候選狀態解除**：它在第三輪因 bookyay 閘門改用下拉而失去唯一消費頁，同日由這個選擇彈窗接手。DS 兩份文件的退場標記同步更正。
