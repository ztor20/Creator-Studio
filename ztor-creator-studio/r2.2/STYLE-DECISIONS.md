...
### Q43：IP 卡／IP hero／商品詳情 hero 三個顯示框仍是 3:4／2:3，接近但不等於新的直式標準 0.806（2026-07-31 提出，待裁決）

Q39 顯示端續作把清單縮圖與卡片封面收斂成單一直式 `--img-portrait`（750/930 ≈ 0.806）之後，站上還留著三個「看起來也是直式、但比例數值不同」的顯示框，尚未被本輪收斂觸及——它們是 Q30／Q31 分別裁決過的既有結論，本題不擅自覆蓋那兩則裁決，只記錄新標準出現後產生的不一致。

- 現況 A（`3/4` ＝ 0.75）：`.ip-hero__cover`（`shared.css:1234`，IP 詳情頁封面，Q31 裁決範圍）、`.ipm-card__cover`（`shared.css:1256`，IP Market 上架卡封面，Q30 裁決範圍）。
- 現況 B（`2/3` ≈ 0.667）：`.pd-hero__cover`（`shared.css:1364`，商品詳情頁 hero，GoFundMe 預覽 wireframe 版型，2026-07-24 起用於取代 `.ip-hero`）。
- 現況 C（`--img-portrait` ＝ 750/930 ≈ 0.806）：本輪收斂的 7 支清單縮圖／卡片封面元件（見 `design-system.md` Pillar 4），以及全站上傳槽本身（Q39）。
- 三個數字都描述「直式」，但彼此不相等、也都不等於新標準；三者是否該收斂到 `--img-portrait`、還是各自有理由維持獨立比例（IP 卡與商品 hero 的版面比例可能是刻意配合各自的內容密度，非隨意選的數字），未經查證不擅自認定。
- 待確認：(a) 這三支是否也該併入 `--img-portrait`，統一全站「直式顯示框」只剩一個數字；(b) 若維持獨立比例，是否需要在 design-system.md 明確標註「這三支刻意不吃 Q39 標準」以避免之後又被誤認成待收斂的遺漏；(c) `.pd-hero__cover` 的 2:3 是否與 Q39 上傳槽的 0.806 存在裁切落差需要驗證（同一張直式原圖分別套 0.806 與 0.667 的框，裁掉的比例不同）。
- 本題僅記錄矛盾與證據，不選邊；裁決權在使用者。

### Q37：卡片內移除／刪除動作，站上多數用 ghost 文字鈕、本輪套組編輯器改實色紅鈕（2026-07-30 提出，待裁決）

套組編輯器第五批把卡片底部的「移除這個套組」鈕改成滿版卡片內的**實色**紅色按鈕，落地時發現這與站上其餘「刪除/移除一筆資料」的既有慣例不同——多數地方用比較安靜的 ghost（無底色）文字風格按鈕，只有已經過一次確認彈窗前置動作的**最終確認**按鈕才用實色。

- 現況 A（既有，卡片／表單內就地刪除）：`.btn.btn--ghost.btn--destructive`——`edit-event.html:370`（`#ee-delete` 刪除活動）、`manage-ip.html:269`（`#mi-delete` 開啟刪除確認彈窗的入口鈕）、`create-auction.html:393`（`#ca-delete`）、`create-product.html:720`（`#cp-delete`）、`create-product.html:831`（商品選項值的逐列刪除，`btn--ghost btn--destructive btn--sm`——與套組卡「移除」同屬「卡片內巢狀項目的移除動作」，是最直接可比的先例）。這五處都是頁面／卡片上**直接可見、隨手可按**的位置。
- 現況 A'（既有，彈窗內的最終確認）：`.btn.btn--destructive`（無 ghost，實色）——僅 `manage-ip.html:312`（`#mi-del-confirm`「Delete permanently」），且只出現在**已經按過一次 ghost 鈕、彈窗跳出警告之後**的最後一步。
- 現況 B（本輪新增）：`js/bundle-editor.js:532` `<button class="btn btn--destructive btn--sm" type="button" data-bd-remove>`——直接坐在展開卡片底部（`.fc-bundle__foot`），不經確認彈窗、也不是 ghost，第一次點擊即刪除整張套組（原型層級無 undo）。
- 兩者角色相同（「移除這一筆」），但視覺份量不同：既有慣例把「份量重」的實色紅鈕留給已經過一次警示的最終確認步驟，卡片/表單上的第一入口一律用份量較輕的 ghost；本輪套組卡把最重的視覺直接放在第一入口，且該動作本身不可逆。
- 選項與取捨：**A** 套組卡移除鈕改回 `.btn.btn--ghost.btn--destructive.btn--sm`，與 `create-product.html:831` 的巢狀刪除鈕同款，成本低（純 class 替換，行為不變）；**B** 維持現況實色，理由是套組卡刪除的後果（整組欄位、商品引用、優惠設定一次清空）比一般表單刪除更重，用視覺份量對應後果的嚴重度，但等於在站上開了「無確認彈窗也可用實色」的先例；**C** 加確認彈窗（比照 `manage-ip.html`），維持現在的實色鈕但只在確認彈窗內生效，卡片上的入口改 ghost——與既有慣例完全對齊，但套組數通常不多（建立流程 demo 3-5 張），加彈窗是否是必要的額外一步待使用者判斷。
- 建議：A——套組卡的「移除」與 `create-product.html:831` 的商品選項值刪除是同一種角色（卡片內移除一個子項），沒有理由份量不同；若使用者認為套組刪除後果較重、值得加防呆，C 是更完整的解法，但屬於行為變更（新增彈窗）不只是換 class，需另外確認。裁決權在使用者，本輪暫不動 `js/bundle-editor.js` 既有實作，只記錄矛盾。
